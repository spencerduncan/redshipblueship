...
| World-state dimensions | region × {childDay, childNight, adultDay, adultNight} (`location_access.h:149-152`) | region × u64 time-slice mask (45 slices [2026-09-26: 46 since PR #729, O5 adopted; `static_assert(TIME_SLICE_COUNT == 46)` in `Logic.h`]) × `canStayOverTime` (`Logic.h:99-102`); no age; Link's form is inventory (`CAN_BE_*`, `Logic.h:278-284`) | — |
...
1. **O5** — 45 slices; a 46th would be a global re-pin (2.2). [2026-09-26: adopted by PR #729 (#643): `TIME_NIGHT2_AM_05_30` added, 46 slices pinned by `static_assert`; no golden moved, so the feared global re-pin did not happen under the pinned profiles.]
...
| P5 | Export surfaces per §4.1 on both sides, incl. the OoT detach/re-attach rule | D4 "each engine's exported query surface" (`:683`) | none exist -- DELIVERED (PR #714, PR #715; coordinator PR #701 + #717): MM's surface in `games/mm/2s2h/Rando/ComboLogicEngineSingleExe.cpp` (with the snapshot/restore pair), OoT's in `games/oot/soh/Enhancements/randomizer/ComboLogicEngineOoT.cpp` (`beginQuery` is the `Logic::Reset(true)` detach, `endQuery` the re-attach); neither is wired into a production path |
...
| P7 | Decide O5 (45 vs 46 slices) inside a re-pin increment | O5 (`:672`) | 45; no consumer needs the 46th today -- DELIVERED (PR #729, #643; ADR 0010 amendments of 2026-09-26): 46 slices, `TIME_ALL_SLICES` = `0x3FFFFFFFFFFF`, count and half-day tiling pinned by `static_assert`s; the three golden rows that existed then passed with the golden files untouched on that commit |
| P8 | O6 tooling: the static negation probe and the CI grow-check | O6 (`:673`) | baseline is clean in both dialects (§1.2, §2.3); nothing locks it -- IN FLIGHT (wave 5, lane K8): the CI grow-check over both engines and the static negation probe; starts after the multiplicity contract (lane K4) merges -- DELIVERED (PR #734): the `ComboLogicMonotonicity` rando row (G1-G4 over both real engines, each red half observed), `.github/scripts/check-monotonicity-negations.py` as CI job `monotonicity-probe` with a read baseline (the three `StoneCount() == 3` sites, monotone), and the review rule in `.claude/worker-prompts.md`; see `docs/monotonicity.md` |
...
| P10 | MM per-trick vocabulary lands so T freezes into identity | §3.3, O9; #578 | absent -- IN PROGRESS: substrate + 10 of 86 keys bound (PR #686, PR #696); the remaining plain widenings are #697 (#578 part 3, in flight) -- 2026-09-26: 25 of 86 bound after PR #703 and PR #713 (20 reserved, 41 bindable and unconsulted); #578 closed, #697 open for the owed seams (ADR 0010 amendment of that date) -- PARTIAL, re-counted 2026-09-26 at `4a3bf058`: 86 keys declared in `StaticData/TrickIds.h`, 20 reserved in `Tricks.cpp`, **26 bound** (`kBoundTricks` and the distinct `MM_TRICK(...)` keys under `Logic/` agree; the 26th is `MMRT_DEKU_STICK_FIGHTING`, PR #729 / #719), 40 bindable and unconsulted; #697 still open |
...
| P12 | Measure the linked fixpoint's creation-time cost against the ~30 s floor | Consequences (`:746-758`); #582 | unmeasured (§6.3) -- MEASURED (PR #722, `combo-logic-measure`; #645 comment of 2026-09-22): 2 alternations and 4-10 ms per linked round, 18.9-26.1 s extrapolated per attempt (0.63-0.87x the 30 s floor), ten retries do not fit the 2x total budget; placement converges, the GOAL proof does not, because both engines de-duplicate assumed items by id (multiplicity ruled 2026-09-26, lane K4) -- RE-MEASURED under ABI 3 (PR #728; #645 comment of 2026-09-26): still 2 alternations; 10.5-10.7 ms per in-fill round at 32-512 rows, 17.9 ms at 2489; `beat-either` proved on the 321+191 bag (3/3 seeds, first attempt, 0 roll-backs); `beat-both` blocked only by `RSBS_COMBO_LOGIC_BAG_CAP` 512 (#727); with the caps at 4096 (uncommitted experiment) the whole 2489-row bag proves `beat-both` first attempt in 44.5-50.9 s = 1.49-1.70x the floor, so the linear 0.63-0.87x extrapolation is superseded |
...
| P14 | Decide which OoT restricted pools join the bag (recommendation: only the general pass, §4.6) | D3 "one bag" | open -- AS MEASURED by PR #722: the bag was read as OoT's general advancement pass plus MM's whole shuffled pool, but OoT's half is every owned host whose placed item `IsAdvancement()`, which is a SUPERSET of the general pass (the restricted passes' advancement placements are indistinguishable afterwards); the decision itself is still open -- DECIDED for wave 6 (`.claude/lanes/laneK9.md`, 2026-09-26; implementation in flight, not yet on `main`): the bag is OoT's LAST GENERAL PASS only, as §4.6 reads it: `progression`-class copies (O8 table, PR #725) of both games enter as required rows, copies beyond the frozen non-plentiful pool as `RSBS_COMBO_BAG_SURPLUS`; `renewable`, `junk` and `trap` never enter and go to each game's own leftover pass; OoT's restricted-pass items (dungeon items, songs, shops, rewards under their own settings) stay out, setting-conditionally from the frozen record |
...
6. O5 carrier: the consumer list for a 46th time slice (`Logic.h:75`,
   `:89-91`; `TimeLogic.cpp:18-31`). (P7) -- DELIVERED, PR #729 (#643).
