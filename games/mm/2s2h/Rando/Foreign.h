 ...
 *
 *  ADR 0010 increment 1.3 (#500): host candidates are the intersection of
 *  IsEligibleHost with the world's REACHABLE-CHECK CLOSURE
 *  (Rando::Logic::ComputeReachableCheckSet) — a foreign item is never pinned
 *  onto a check MM's own logic cannot reach under this world's frozen
 *  settings. If that intersection runs short of the pool, FEWER are placed
 *  (cap ≠ promise; RSBS_FOREIGN_PLACEMENT_CAP semantics) and the shortfall is
 *  recorded loudly in the spoiler — never an unreachable placement, and (in
 *  this duplicate-overlay increment) never a failed generation. Throws only
 *  on a placement-table insert refusal with candidates remaining, which is a
 *  structural defect, not supply. */
int PlaceForeignItems();

/** Outcome of the last PlaceForeignItems run (RAM-only, session-scoped): the
 *  numbers the spoiler's shortfall record and the CI locks read. Valid for
 *  the world generated by the most recent run in this process. */
struct PlacementStats {
    int requested = 0;              /**< pinned pool size asked for */
    int placed = 0;                 /**< placements actually recorded */
    int eligibleHosts = 0;          /**< checks IsEligibleHost accepts (ungated) */
    int reachableEligibleHosts = 0; /**< ...that are also in the reachable closure */
};
