...
void MM_EnOwl_ChangeMode(EnOwl* enOwl, EnOwlActionFunc actionFunc, EnOwlFunc unkFunc, SkelAnime* skelAnime,
...
void flyOnApproach(EnOwl* enOwl, PlayState* play) {
    ...
    if (enOwl->actor.xzDistToPlayer < 200.0f) {
        ...
        if (owlType == ENOWL_GET_TYPE_2) { // Winter Village owl
            ...
            MM_EnOwl_ChangeMode(enOwl, func_8095B9FC, func_8095C484, &enOwl->skelAnimeFlying,
            ...
            MM_Flags_SetSwitch(MM_gPlayState, ENOWL_GET_SWITCH_FLAG(&enOwl->actor));
...
void RegisterSkipOwlInteractions() {
    ...
    COND_VB_SHOULD(VB_OWL_TELL_ABOUT_SHRINE, CVAR, {
        ...
        MM_EnOwl_ChangeMode(enOwl, flyOnApproach, func_8095C484, &enOwl->skelAnimePerching,
