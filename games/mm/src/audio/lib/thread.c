...
void MM_AudioMgr_CreateNextAudioBuffer(s16* samples, u32 num_samples) {
    ...
    MM_AudioLoad_DecreaseSampleDmaTtls();
    MM_AudioLoad_ProcessLoads(gAudioCtx.resetStatus);
    MM_AudioLoad_ProcessScriptLoads();

    if (gAudioCtx.resetStatus != 0) {
        if (MM_AudioHeap_ResetStep() == 0) {
            if (gAudioCtx.resetStatus == 0) {
    ...
    s32 writtenCmds;
    MM_AudioSynth_Update(gAudioCtx.curAbiCmdBuf, &writtenCmds, samples, num_samples);
...
void AudioThread_ProcessGlobalCmd(AudioCmd* cmd) {
    ...
    switch (cmd->op) {
        ...
        case AUDIOCMD_OP_GLOBAL_INIT_SEQPLAYER:
            // 2S2H [Custom Audio] the second argument (seqId) was `cmd->arg1`changed to use the upper half of
            // `cmd->asInt so it can be 16 bit.
            MM_AudioLoad_SyncInitSeqPlayer(cmd->arg0, (cmd->asInt >> 16) & 0x7FF, cmd->arg2);
        ...
        case AUDIOCMD_OP_GLOBAL_INIT_SEQPLAYER_SKIP_TICKS:
            // 2S2H [Custom Audio] the second argument (seqId) was `cmd->arg1`changed to use the upper half of
            // `cmd->asInt so it can be 16 bit.
            MM_AudioLoad_SyncInitSeqPlayerSkipTicks(cmd->arg0, (cmd->asInt >> 16) & 0x7FF, cmd->asInt & 0xFFFF);
        ...
        case AUDIOCMD_OP_GLOBAL_SYNC_LOAD_INSTRUMENT:
            MM_AudioLoad_SyncLoadInstrument(cmd->arg0, cmd->arg1, cmd->arg2);
            break;

        case AUDIOCMD_OP_GLOBAL_ASYNC_LOAD_SAMPLE_BANK:
            MM_AudioLoad_AsyncLoadSampleBank(cmd->arg0, cmd->arg1, cmd->arg2, &gAudioCtx.externalLoadQueue);
            break;

        case AUDIOCMD_OP_GLOBAL_ASYNC_LOAD_FONT:
            MM_AudioLoad_AsyncLoadFont(cmd->arg0, cmd->arg1, cmd->arg2, &gAudioCtx.externalLoadQueue);
            break;
        ...
        case AUDIOCMD_OP_GLOBAL_ASYNC_LOAD_SEQ:
            ...
            // can be 16 bit.
            MM_AudioLoad_AsyncLoadSeq(cmd->asUShort & 0x7FF, cmd->arg1, cmd->arg2, &gAudioCtx.externalLoadQueue);
            break;

        case AUDIOCMD_OP_GLOBAL_DISCARD_SEQ_FONTS:
            MM_AudioLoad_DiscardSeqFonts(cmd->asUShort & 0x7FF);
            break;
...
u8* AudioThread_GetFontsForSequence(s32 seqId, u32* outNumFonts, u8* buff) {
    return MM_AudioLoad_GetFontsForSequence(seqId, outNumFonts, buff);
}
    ...
    switch (cmd->op) {
        ...
        case AUDIOCMD_OP_CHANNEL_SET_FILTER:
            ...
            if (channel->filter != NULL) {
                MM_AudioHeap_LoadFilter(channel->filter, filterCutoff >> 4, filterCutoff & 0xF);
            }
            break;
