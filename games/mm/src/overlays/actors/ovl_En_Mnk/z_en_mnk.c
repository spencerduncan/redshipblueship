...
void EnMnk_MonkeyTiedUp_ChangeAnim(EnMnk* this, s32 animIndex, u8 animMode, f32 morphFrames) {
    MM_Animation_Change(&this->skelAnime, sMonkeyTiedUpAnimations[animIndex], 1.0f, 0.0f,
                     MM_Animation_GetLastFrame(&sMonkeyTiedUpAnimations[animIndex]->common), animMode, morphFrames);
    MM_Animation_Change(&this->propSkelAnime, sMonkeyTiedUpPoleAnimations[animIndex], 1.0f, 0.0f,
                     MM_Animation_GetLastFrame(&sMonkeyTiedUpPoleAnimations[animIndex]->common), animMode, morphFrames);
...
void EnMnk_MonkeyHanging_ChangeAnim(EnMnk* this, s32 animIndex, u8 animMode, f32 morphFrames) {
    MM_Animation_Change(&this->skelAnime, sMonkeyHangingAnimations[animIndex], 1.0f, 0.0f,
                     MM_Animation_GetLastFrame(&sMonkeyHangingAnimations[animIndex]->common), animMode, morphFrames);
    MM_Animation_Change(&this->propSkelAnime, sMonkeyHangingRopeAnimations[animIndex], 1.0f, 0.0f,
                     MM_Animation_GetLastFrame(&sMonkeyHangingRopeAnimations[animIndex]->common), animMode, morphFrames);
    this->animIndex = animIndex;
}
...
void EnMnk_Monkey_ChangeAnim(EnMnk* this, s32 animIndex, u8 animMode, f32 morphFrames) {
    ...
    if (animIndex == 100) {
        animIndex = (s32)MM_Rand_ZeroFloat(3.99f) + 7;
    }
    if ((animIndex == 4) || (animIndex == 6)) {
        MM_Animation_Change(&this->skelAnime, sMonkeyAnimations[animIndex], -1.0f,
                         MM_Animation_GetLastFrame(&sMonkeyAnimations[animIndex]->common), 0.0f, animMode, morphFrames);
    } else {
        MM_Animation_Change(&this->skelAnime, sMonkeyAnimations[animIndex], 1.0f, 0.0f,
                         MM_Animation_GetLastFrame(&sMonkeyAnimations[animIndex]->common), animMode, morphFrames);
    }
    this->unk_3E0 = 0;
    this->animIndex = animIndex;
}
...
void EnMnk_Monkey_SetupWaitToRunAndWaitAtEachPoint(EnMnk* this, PlayState* play) {
    ...
    MM_Animation_PlayLoop(&this->skelAnime, &object_mnk_Anim_008814);
...
void EnMnk_Monkey_StartInvisible(EnMnk* this, PlayState* play) {
    ...
    if ((MONKEY_GET_SWITCH_FLAG(&this->picto.actor) != 0x7F) &&
        MM_Flags_GetSwitch(play, MONKEY_GET_SWITCH_FLAG(&this->picto.actor))) {
...
void EnMnk_MonkeyTiedUp_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_InitFlex(play, &this->propSkelAnime, &gMonkeyTiedUpPoleSkel, &object_mnk_Anim_003584,
...
void EnMnk_MonkeyHanging_Init(Actor* thisx, PlayState* play) {
    ...
    MM_Actor_ChangeCategory(play, &play->actorCtx, &this->picto.actor, ACTORCAT_PROP);
    this->actionFunc = EnMnk_MonkeyHanging_StruggleBeforeDunk;
    this->picto.actor.textId = 0x8E8;
    MM_SkelAnime_InitFlex(play, &this->propSkelAnime, &gMonkeyHangingRopeSkel, &gMonkeyHangingStruggleAnim,
...
void EnMnk_Init(Actor* thisx, PlayState* play) {
    ...
    MM_Actor_SetScale(&this->picto.actor, 0.012f);
    MM_ActorShape_Init(&this->picto.actor.shape, 0.0f, MM_ActorShadow_DrawCircle, 12.0f);
    this->actionFunc = EnMnk_DoNothing;
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &gMonkeySkel, &object_mnk_Anim_0105DC, this->jointTable,
                       this->morphTable, OBJECT_MNK_2_LIMB_MAX);
    MM_Animation_PlayLoop(&this->skelAnime, &object_mnk_Anim_0105DC);
    this->flags = 0;
    this->unk_3D4 = 0;
    this->unk_3E0 = 0;
    this->unk_3CC = 0;
    ...
    switch (MONKEY_GET_TYPE(thisx)) {
        case MONKEY_0:
            if (!MM_Flags_GetSwitch(play, MONKEY_GET_SWITCH_FLAG(&this->picto.actor)) ||
                MM_Flags_GetSwitch(play, MONKEY_GET_SWITCH_FLAG(&this->picto.actor) + 1) ||
...
void EnMnk_Destroy(Actor* thisx, PlayState* play) {
    ...
    if ((MONKEY_GET_TYPE(&this->picto.actor) == MONKEY_TIED_UP) && (this->flags & MONKEY_FLAGS_2000)) {
        MM_Item_Give(play, ITEM_SONG_SONATA);
...
void EnMnk_SpawnWaterEffects(EnMnk* this, PlayState* play) {
    ...
    MM_EffectSsGRipple_Spawn(play, effSpawnPos, 150, 500, 0);
    MM_EffectSsGSplash_Spawn(play, effSpawnPos, NULL, NULL, 0, 200);
}
...
s32 EnMnk_FollowPath(EnMnk* this, u16 flags) {
    ...
    MM_Math_SmoothStepToS(&this->picto.actor.world.rot.y, arcTan, 2, 0xBB8, 0xC8);
    MM_Math_SmoothStepToS(&this->picto.actor.shape.rot.y, this->picto.actor.world.rot.y, 2, 0xBB8, 0xC8);
...
void func_80AB5F6C(EnMnk* this) {
    ...
    if ((s32)this->skelAnime.curFrame == 0) {
        if (this->unk_3C8 < 0) {
            ...
            do {
                animIndex = D_80AB9DD8[(s32)MM_Rand_ZeroFloat(7.0f)];
            ...
            switch (animIndex) {
                case 100:
                    this->unk_3C8 = -(s32)(MM_Rand_ZeroFloat(6.0f) + 6.0f);
                    break;

                case 2:
                    this->unk_3C8 = MM_Rand_ZeroFloat(6.0f) + 6.0f;
                    break;

                default:
                    this->unk_3C8 = MM_Rand_ZeroFloat(2.0f) + 1.0f;
                    break;
            }
...
void func_80AB60FC(EnMnk* this, PlayState* play) {
    ...
    if (EnMnk_FollowPath(this, 0) != MONKEY_FOLLOWPATH_RESULT_FOLLOWING) {
        ...
        if (this->pathIndex == ADDITIONAL_PATH_INDEX_NONE) {
            MM_Flags_SetSwitch(play, MONKEY_GET_SWITCH_FLAG(&this->picto.actor) + 1);
            MM_Actor_Kill(&this->picto.actor);
            return;
        }
    }
...
void func_80AB61E8(EnMnk* this, PlayState* play) {
    ...
    if ((MM_Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && MM_Message_ShouldAdvance(play)) {
        switch (play->msgCtx.currentTextId) {
            case 0x7D9:
                MM_Message_ContinueTextbox(play, 0x7DA);
                EnMnk_Monkey_SetAnim(this, 3);
                break;

            case 0x7DA:
                MM_Message_ContinueTextbox(play, 0x7DB);
...
void func_80AB6304(EnMnk* this, PlayState* play) {
    ...
    if (this->flags & MONKEY_FLAGS_4) {
        MM_Message_ContinueTextbox(play, 0x7D9);
...
void EnMnk_Monkey_WaitToRunAndWaitAtEachPoint(EnMnk* this, PlayState* play) {
    ...
    if (!(this->flags & MONKEY_FLAGS_40)) {
        if (((s32)this->skelAnime.curFrame == 0) || MM_Animation_OnFrame(&this->skelAnime, 14.0f)) {
            EnMnk_SpawnWaterEffects(this, play);
        }
    }
...
void EnMnk_Monkey_TalkAfterRun(EnMnk* this, PlayState* play) {
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if (this->flags & MONKEY_FLAGS_4) {
            ...
            MM_Animation_PlayOnce(&this->skelAnime, &object_mnk_Anim_009CC0);
        }
    }
}
    ...
    if (EnMnk_FollowPath(this, MONKEY_FOLLOWPATH_FLAGS_LONGRANGE) != MONKEY_FOLLOWPATH_RESULT_FOLLOWING) {
        ...
        if (MONKEY_GET_TYPE(&this->picto.actor) == MONKEY_OUTSIDECHAMBER) {
            ...
            if (switchFlag != MONKEY_SWITCH_FLAG_NONE) {
                MM_Flags_SetSwitch(play, switchFlag + 1);
            }
            MM_Actor_Kill(&this->picto.actor);
        } else {
            ...
            if (MM_Message_ShouldAdvance(play)) {
                switch (play->msgCtx.choiceIndex) {
                    case 1:
                        ...
                        MM_Message_ContinueTextbox(play, 0x7D7);
                        break;

                    case 0:
                        Audio_PlaySfx_MessageCancel();
                        EnMnk_Monkey_SetAnim(this, 7);
                        MM_Message_ContinueTextbox(play, 0x7D6);
                        this->unk_3E0 = 6;
                        break;
                }
            }
        ...
        case TEXT_STATE_EVENT:
            if (MM_Message_ShouldAdvance(play)) {
                switch (play->msgCtx.currentTextId) {
                    case 0x7D0:
                        EnMnk_Monkey_SetAnim(this, 3);
                        MM_Message_ContinueTextbox(play, 0x7D1);
                        this->unk_3E0 = 5;
                        break;
                    ...
                    case 0x7D6:
                        EnMnk_Monkey_SetAnim(this, 3);
                        MM_Message_ContinueTextbox(play, 0x7D4);
                        break;

                    case 0x7D1:
                        EnMnk_Monkey_SetAnim(this, 7);
                        MM_Message_ContinueTextbox(play, 0x7D2);
                        this->unk_3E0 = 4;
                        break;

                    case 0x7D4:
                        EnMnk_Monkey_SetAnim(this, 5);
                        MM_Message_ContinueTextbox(play, 0x7D5);
                        this->unk_3E0 = 6;
                        break;
...
void EnMnk_Monkey_MoveRelativeToPlayer(EnMnk* this, PlayState* play) {
    ...
    this->picto.actor.world.pos.x =
        (MM_Math_SinS(angleToPlayer) * this->approachPlayerRadius) + this->picto.actor.home.pos.x;
    this->picto.actor.world.pos.z =
        (MM_Math_CosS(angleToPlayer) * this->approachPlayerRadius) + this->picto.actor.home.pos.z;
}
...
void EnMnk_Monkey_WaitToTalkAfterApproach(EnMnk* this, PlayState* play) {
    ...
    if (Actor_TalkOfferAccepted(&this->picto.actor, &play->state)) {
        ...
        if (MONKEY_GET_SWITCH_FLAG(&this->picto.actor) != 0x7F) {
            MM_Flags_SetSwitch(play, MONKEY_GET_SWITCH_FLAG(&this->picto.actor));
        }
...
void EnMnk_Monkey_SetupDrop(EnMnk* this) {
    ...
    MM_Animation_Change(&this->skelAnime, &object_mnk_Anim_008814, 1.0f, 10.0f,
                     MM_Animation_GetLastFrame(&object_mnk_Anim_008814), ANIMMODE_ONCE, 0.0f);
}
...
s32 EnMnk_PlayerIsInTalkRange(EnMnk* this, PlayState* play) {
    ...
    if ((this->picto.actor.xzDistToPlayer < 100.0f) && (this->picto.actor.xzDistToPlayer > 20.0f)) {
        ...
        if ((absYawTowardsPlayer > 0x4000) && MM_Player_IsFacingActor(&this->picto.actor, 0x3000, play)) {
            return true;
        }
    }
    return false;
...
void EnMnk_MonkeyTiedUp_TeachSong(EnMnk* this, PlayState* play) {
    ...
    MM_SkelAnime_Update(&this->skelAnime);
    MM_SkelAnime_Update(&this->propSkelAnime);
    ...
    if ((MM_Message_GetState(&play->msgCtx) == TEXT_STATE_CHOICE) && !(this->flags & MONKEY_FLAGS_4000) &&
...
void EnMnk_MonkeyTiedUp_ReactToWrongInstrument(EnMnk* this, PlayState* play) {
    ...
    if ((MM_Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && MM_Message_ShouldAdvance(play)) {
        switch (play->msgCtx.currentTextId) {
            ...
            case 0x8D6:
                MM_Message_ContinueTextbox(play, 0x8DB);
                EnMnk_MonkeyTiedUp_SetAnim(this, MONKEY_TIEDUP_ANIM_CONFUSED);
                break;
...
void EnMnk_MonkeyTiedUp_TalkUnused(EnMnk* this, PlayState* play) {
    MM_SkelAnime_Update(&this->skelAnime);
    MM_SkelAnime_Update(&this->propSkelAnime);
    if (MM_Actor_TextboxIsClosing(&this->picto.actor, play)) {
    ...
    if ((MM_Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && MM_Message_ShouldAdvance(play)) {
        switch (play->msgCtx.currentTextId) {
            case 0x8CA:
                ...
                if (play->curSpawn != 1) {
                    MM_Message_ContinueTextbox(play, 0x8CB);
                    SET_WEEKEVENTREG(WEEKEVENTREG_88_01);
                } else {
                    MM_Message_ContinueTextbox(play, 0x8EB);
            ...
            case 0x8CC:
                ...
                this->unk_3E0 = 2;
                MM_Message_ContinueTextbox(play, 0x8CD);
                break;

            case 0x8CD:
                EnMnk_MonkeyTiedUp_SetAnim(this, MONKEY_TIEDUP_ANIM_SHAKEHEAD);
                MM_Message_ContinueTextbox(play, 0x8CE);
                break;

            case 0x8CE:
                EnMnk_MonkeyTiedUp_SetAnim(this, MONKEY_TIEDUP_ANIM_KICKUPANDDOWN);
                MM_Message_ContinueTextbox(play, 0x8CF);
                break;
            ...
            case 0x8D3:
                if ((gSaveContext.save.playerForm == PLAYER_FORM_FIERCE_DEITY) ||
                    ...
                    EnMnk_MonkeyTiedUp_SetAnim(this, MONKEY_TIEDUP_ANIM_CONFUSED);
                    MM_Message_ContinueTextbox(play, 0x8D7);
                }
                break;
...
void EnMnk_MonkeyTiedUp_WaitForInstrument(EnMnk* this, PlayState* play) {
    ...
    if (Actor_OcarinaInteractionAccepted(&this->picto.actor, &play->state)) {
        ...
        this->picto.actor.csId = this->csIdList[0];
        MM_Message_StartTextbox(play, this->picto.actor.textId, NULL);
        CutsceneManager_Queue(this->picto.actor.csId);
...
void EnMnk_MonkeyTiedUp_Wait(EnMnk* this, PlayState* play) {
    ...
    MM_SkelAnime_Update(&this->skelAnime);
    if (MM_SkelAnime_Update(&this->propSkelAnime)) {
        this->cueId--;
        if (this->cueId < 0) {
            this->cueId = 4;
            EnMnk_MonkeyTiedUp_ChangeAnim(this, MONKEY_TIEDUP_ANIM_KICKAROUND, ANIMMODE_ONCE, -5.0f);
    ...
    if (Actor_OcarinaInteractionAccepted(&this->picto.actor, &play->state)) {
        ...
        Actor_OfferTalk(&this->picto.actor, play, 120.0f);
               ...
               MM_Player_IsFacingActor(&this->picto.actor, 0x3000, play)) {
        this->picto.actor.textId = 0x8CA;
...
void EnMnk_MonkeyHanging_RaiseFromDunk2(EnMnk* this, PlayState* play) {
    ...
    if (this->unk_3C8 > 0) {
        ...
        MM_Message_ContinueTextbox(play, 0x8EA);
    }
}
...
void EnMnk_MonkeyHanging_MakeSplash(EnMnk* this, PlayState* play) {
    ...
    MM_Math_Vec3f_Copy(&effSpawnPos, &this->picto.actor.focus.pos);
    effSpawnPos.y += 20.0f;
    MM_EffectSsGSplash_Spawn(play, &effSpawnPos, &sMonkeyHangingSplashPrimColor, &sMonkeyHangingSplashEnvColor, 0, 300);
}
...
void EnMnk_MonkeyHanging_Plead(EnMnk* this, PlayState* play) {
    ...
    if ((MM_Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && MM_Message_ShouldAdvance(play)) {
        switch (play->msgCtx.currentTextId) {
            case 0x8E8:
                MM_Message_ContinueTextbox(play, 0x8E9);
                break;

            case 0x8E9:
                this->actionFunc = EnMnk_MonkeyHanging_Dunk2;
                MM_Message_CloseTextbox(play);
                MM_Player_SetCsActionWithHaltedActors(play, &this->picto.actor, PLAYER_CSACTION_WAIT);
                this->unk_3C8 = 60;
                break;
...
void EnMnk_Monkey_TalkBeforeGuideThroughWoods(EnMnk* this, PlayState* play) {
    MM_Math_SmoothStepToS(&this->picto.actor.world.rot.y, this->picto.actor.yawTowardsPlayer, 2, 0xBB8, 0xC8);
    MM_Math_SmoothStepToS(&this->picto.actor.shape.rot.y, this->picto.actor.world.rot.y, 2, 0xBB8, 0xC8);
    MM_SkelAnime_Update(&this->skelAnime);
...
void EnMnk_Monkey_SetupTalkBeforeGuideThroughWoods(EnMnk* this) {
    switch ((u32)MM_Rand_ZeroFloat(6.0f)) {
        case 0:
            EnMnk_Monkey_SetAnim(this, 3);
            break;
...
void EnMnk_Monkey_TalkAfterSaved(EnMnk* this, PlayState* play) {
    ...
    if ((MM_Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && MM_Message_ShouldAdvance(play)) {
        switch (play->msgCtx.currentTextId) {
            case 0x8E5:
                MM_Message_ContinueTextbox(play, 0x8E6);
                EnMnk_Monkey_SetAnim(this, 3);
                break;

            case 0x8E6:
                MM_Message_ContinueTextbox(play, 0x8E7);
                EnMnk_Monkey_SetAnim(this, 5);
                break;
...
void EnMnk_Monkey_WaitToTalkAfterSaved(EnMnk* this, PlayState* play) {
    ...
    if (Actor_TalkOfferAccepted(&this->picto.actor, &play->state)) {
        ...
        EnMnk_Monkey_SetAnim(this, 7);
    } else if ((this->picto.actor.xzDistToPlayer < 100.0f) && MM_Player_IsFacingActor(&this->picto.actor, 0x3000, play)) {
...
void EnMnk_Update(Actor* thisx, PlayState* play) {
    ...
    if (!(this->flags & MONKEY_FLAGS_2)) {
        MM_Actor_UpdateBgCheckInfo(play, &this->picto.actor, 20.0f, 25.0f, 0.0f, UPDBGCHECKINFO_FLAG_4);
        if (this->picto.actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
            this->flags |= MONKEY_FLAGS_40;
        }
    }
...
void EnMnk_Monkey_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (limbIndex == OBJECT_MNK_2_LIMB_04) {
        MM_Matrix_MultVec3f(&sMonkeyFocusPosOffset, &this->picto.actor.focus.pos);
    }
}
...
void EnMnk_MonkeyTiedUp_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_DrawFlexOpa(play, this->propSkelAnime.skeleton, this->propSkelAnime.jointTable,
    ...
    EnMnk_Monkey_DrawFace(this, play);
    MM_Matrix_Mult(&this->unk_36C, MTXMODE_NEW);
    MM_SkelAnime_DrawFlexOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                          EnMnk_Monkey_OverrideLimbDraw, EnMnk_Monkey_PostLimbDraw, &this->picto.actor);
}
