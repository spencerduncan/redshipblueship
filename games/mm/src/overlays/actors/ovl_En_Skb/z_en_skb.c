...
void MM_EnSkb_Init(Actor* thisx, PlayState* play);
void MM_EnSkb_Destroy(Actor* thisx, PlayState* play);
void MM_EnSkb_Update(Actor* thisx, PlayState* play);
void MM_EnSkb_Draw(Actor* thisx, PlayState* play);
...
static AnimationInfo MM_sAnimationInfo[STALCHILD_ANIM_MAX] = {
...
void func_809947B0(PlayState* play, EnSkb* this, Vec3f* inPos) {
    ...
    f32 sp40 = (MM_Rand_ZeroOne() - 0.5f) * 6.28f;
    s32 pad;

    pos.y = this->actor.floorHeight;
    pos.x = (MM_Math_SinF(sp40) * 15.0f) + inPos->x;
    pos.z = (MM_Math_CosF(sp40) * 15.0f) + inPos->z;
    accel.x = MM_Rand_CenteredFloat(1.0f);
    accel.z = MM_Rand_CenteredFloat(1.0f);
    velocity.y += (MM_Rand_ZeroOne() - 0.5f) * 4.0f;
    MM_EffectSsHahen_Spawn(play, &pos, &velocity, &accel, 0, ((MM_Rand_ZeroOne() * 5.0f) + 12.0f) * 0.8f,
                        HAHEN_OBJECT_DEFAULT, 10, NULL);
    func_800BBFB0(play, &pos, 10.0f, 1, 150, 0, 1);
}

void func_8099495C(EnSkb* this, PlayState* play) {
    MM_SkelAnime_Init(play, &this->skelAnime, &gStalchildSkel, &gStalchildStandUpAnim, this->jointTable, this->morphTable,
                   STALCHILD_LIMB_MAX);
    this->unk_3DC = 0;
    func_80994E2C(this);
}
...
void func_809949C4(EnSkb* this, PlayState* play) {
    MM_SkelAnime_Init(play, &this->skelAnime, &gStalchildSkel, &gStalchildSitLaughAnim, this->jointTable, this->morphTable,
                   STALCHILD_LIMB_MAX);
    this->unk_3DC = 1;
    func_809952D8(this);
}
...
void func_80994A30(EnSkb* this, PlayState* play) {
    MM_SkelAnime_Init(play, &this->skelAnime, &gStalchildSkel, &gStalchildSitTapToesAnim, this->jointTable,
...
void func_80994A9C(EnSkb* this, PlayState* play) {
    MM_SkelAnime_Init(play, &this->skelAnime, &gStalchildSkel, &gStalchildSwingOnBranchAnim, this->jointTable,
                   this->morphTable, STALCHILD_LIMB_MAX);
    this->unk_3DC = 1;
    func_809954F8(this);
}
...
void func_80994DA8(EnSkb* this, PlayState* play) {
    if (MM_Actor_IsFacingPlayer(&this->actor, 0x11C7) && (this->actor.xzDistToPlayer < 60.0f) &&
        (MM_Player_GetMask(play) != PLAYER_MASK_CAPTAIN)) {
        func_80995C24(this);
    } else {
        func_80995A30(this);
    }
...
void func_80994E2C(EnSkb* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_1);
...
void func_80994E94(EnSkb* this, PlayState* play) {
    ...
    MM_Math_ApproachZeroF(&this->actor.shape.yOffset, 1.0f, 800.0f);
    MM_Math_ApproachF(&this->actor.shape.shadowScale, 25.0f, 1.0f, 2.5f);
    if ((play->gameplayFrames % 2) == 0) {
        func_809947B0(play, this, &this->actor.world.pos);
    }

    if (MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame) && (this->actor.shape.yOffset == 0.0f)) {
        func_80995A30(this);
    }
    this->unk_3D0++;
}
...
void func_80994F7C(EnSkb* this, PlayState* play) {
    if (Actor_TalkOfferAccepted(&this->actor, &play->state)) {
        this->unk_3E2 = 1;
        if (this->unk_3E0 == 1) {
            MM_Message_StartTextbox(play, 0x13F8, &this->actor);
        } else {
            MM_Message_StartTextbox(play, 0x13F6, &this->actor);
            this->unk_3E0 = 1;
        }
        this->actionFunc = func_80995190;
        this->actor.speed = 0.0f;
    } else if (MM_Actor_IsFacingPlayer(&this->actor, 0x2AAA) && !(this->collider.base.acFlags & AC_HIT)) {
...
void func_80995068(EnSkb* this, PlayState* play) {
    if (Actor_TalkOfferAccepted(&this->actor, &play->state)) {
        ...
        if (this->unk_3E0 == 1) {
            MM_Message_StartTextbox(play, 0x13F8, &this->actor);
            if (this->unk_3DE == 2) {
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_11);
            }
        } else {
            MM_Message_StartTextbox(play, 0x13F6, &this->actor);
            this->unk_3E0 = 1;
        ...
        this->actionFunc = func_80995190;
        this->actor.speed = 0.0f;
    } else if (MM_Player_GetMask(play) != PLAYER_MASK_CAPTAIN) {
    ...
    switch (MM_Message_GetState(&play->msgCtx)) {
        ...
        case TEXT_STATE_EVENT:
            if (MM_Message_ShouldAdvance(play)) {
                MM_Message_StartTextbox(play, 0x13F7, &this->actor);
                if (this->unk_3DE == 2) {
                    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_11);
                }
            }
...
void func_809952D8(EnSkb* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_5);
...
void func_8099533C(EnSkb* this, PlayState* play) {
    ...
    } else if (MM_Actor_IsFacingPlayer(&this->actor, 0x2AAA) && (this->actor.xzDistToPlayer < 200.0f)) {
        this->actor.hintId = TATL_HINT_ID_STALCHILD;
        this->actor.colChkInfo.mass = MASS_HEAVY;
        func_8099571C(this);
    }
...
void func_809953E8(EnSkb* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_6);
    this->actor.colChkInfo.mass = MASS_IMMOVABLE;
    this->unk_3DE = 10;
    this->actionFunc = func_8099544C;
    this->actor.shape.shadowScale = 0.0f;
...
void func_809954F8(EnSkb* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_7);
    this->actor.colChkInfo.mass = MASS_IMMOVABLE;
    this->unk_3DE = 11;
    this->actionFunc = func_8099556C;
...
void func_8099556C(EnSkb* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 22.5f)) {
        this->unk_3D4 = MM_Rand_Next() % 0x7D0;
    }

    this->actor.shape.rot.x = MM_Math_SinS(this->unk_3D4 * sp26) * 0x4E20;
    if (MM_Player_GetMask(play) == PLAYER_MASK_CAPTAIN) {
        this->actor.flags &= ~(ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_HOSTILE);
        this->actor.flags |= (ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_FRIENDLY);
        func_80994F7C(this, play);
...
void func_8099571C(EnSkb* this) {
    ...
    this->actor.shape.shadowScale = 0.0f;
    if (this->unk_3DE == 9) {
        Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_8);
        ...
        Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_9);
...
void func_80995818(EnSkb* this, PlayState* play) {
    ...
    MM_Math_ApproachF(&this->actor.shape.shadowScale, 25.0f, 1.0f, 2.5f);
    MM_Math_SmoothStepToS(&this->actor.shape.rot.x, 0, 0x10, 0x7D0, 0x64);
...
void func_809958F4(EnSkb* this) {
    MM_Animation_Change(&this->skelAnime, &gStalchildStandUpAnim, -1.0f, MM_Animation_GetLastFrame(&gStalchildStandUpAnim),
...
void func_8099599C(EnSkb* this, PlayState* play) {
    MM_Math_ApproachF(&this->actor.shape.yOffset, -8000.0f, 1.0f, 500.0f);

    if ((play->gameplayFrames % 2) != 0) {
        func_809947B0(play, this, &this->actor.world.pos);
    }

    MM_Math_ApproachF(&this->actor.shape.shadowScale, 0.0f, 1.0f, 2.5f);
...
void func_80995A30(EnSkb* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_0);
...
void func_80995A8C(EnSkb* this, PlayState* play) {
    if (MM_Player_GetMask(play) == PLAYER_MASK_CAPTAIN) {
        ...
        this->actor.colChkInfo.mass = MASS_HEAVY;
        Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_12);
        func_8099504C(this);
        return;
    }

    if ((this->unk_3D8 != 0) && ((play->gameplayFrames % 16) == 0)) {
        this->unk_3DA = MM_Rand_CenteredFloat(50000.0f);
    }

    MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer + this->unk_3DA, 1, 0x2EE, 0);
    this->actor.world.rot.y = this->actor.shape.rot.y;
    if (MM_Animation_OnFrame(&this->skelAnime, 8.0f) || MM_Animation_OnFrame(&this->skelAnime, 15.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_STALKID_WALK);
    }

    if ((this->actor.xzDistToPlayer > 800.0f) || func_80996594(this, play)) {
        func_809958F4(this);
    } else if (MM_Actor_IsFacingPlayer(&this->actor, 0x11C7) && (this->actor.xzDistToPlayer < 60.0f)) {
        func_80995C24(this);
    }
}

void func_80995C24(EnSkb* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_2);
...
void func_80995C84(EnSkb* this, PlayState* play) {
    if (MM_Animation_OnFrame(&this->skelAnime, 3.0f) && (this->unk_3E4 == 0)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_STALKID_ATTACK);
        this->unk_3E4 = 1;
...
void func_80995D3C(EnSkb* this) {
    MM_Animation_Change(&this->skelAnime, &gStalchildAttackAnim, -0.4f, this->skelAnime.curFrame - 1.0f, 0.0f,
...
void func_80995F98(EnSkb* this) {
    if ((this->unk_3DE == 9) || (this->unk_3DE == 0xA)) {
        ...
        this->actor.world.rot.y = this->actor.yawTowardsPlayer;
        Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_3);
        if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
...
void func_809960AC(EnSkb* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        ...
        MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 1, 0x9C4, 0);
    }
...
void func_809961E4(EnSkb* this, PlayState* play) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, STALCHILD_ANIM_4);
    this->unk_3D8 |= 0x40;
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
    ...
    this->unk_3E4 = 0;
    this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
    MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 40, NA_SE_EN_STALKID_DEAD);
    this->unk_3DE = 7;
...
void func_80996284(EnSkb* this, PlayState* play) {
    ...
    MM_Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, 0x10);
    func_80996474(this);
}
...
void func_8099672C(EnSkb* this, PlayState* play) {
    ...
    if ((this->unk_3DE != 6) && (this->unk_3DE != 7)) {
        ...
        if (this->collider.base.acFlags & AC_HIT) {
            ...
            if (!MM_Actor_ApplyDamage(&this->actor) && (this->actor.colChkInfo.damageEffect != 3) &&
...
void func_80996BEC(EnSkb* this, PlayState* play) {
    ...
    MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 30, NA_SE_EV_ICE_BROKEN);

    for (i = 0; i < bodyPartsCount; i++) {
        yaw = MM_Math_Vec3f_Yaw(&this->actor.world.pos, &this->bodyPartsPos[i]);
        sp84.x = MM_Math_SinS(yaw) * 3.0f;
        sp84.z = MM_Math_CosS(yaw) * 3.0f;
        sp84.y = (MM_Rand_ZeroOne() * 4.0f) + 4.0f;
        MM_EffectSsEnIce_Spawn(play, &this->bodyPartsPos[i], 0.6f, &sp84, &D_80997558, &D_80997550, &D_80997554, 30);
    }
}
    ...
    if (this->actionFunc != func_8099630C) {
        if (this->drawDmgEffTimer < 20) {
            if (this->actionFunc == func_809963D8) {
                ...
                MM_Math_SmoothStepToF(&this->drawDmgEffScale, 0.0f, 0.5f, 0.01f, 0.0f);
            }
...
void MM_EnSkb_Update(Actor* thisx, PlayState* play) {
    ...
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 15.0f, 30.0f, 60.0f,
...
s32 MM_EnSkb_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    EnSkb* this = (EnSkb*)thisx;
    s32 pad;
    s16 MM_sins;
...
void MM_EnSkb_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (this->drawDmgEffTimer != 0) {
        if ((limbIndex == STALCHILD_LIMB_PELVIS) || (limbIndex == STALCHILD_LIMB_RIGHT_THIGH) ||
            ...
            MM_Matrix_MultVec3f(&D_80997564, &this->bodyPartsPos[this->bodyPartsCount]);
            this->bodyPartsCount++;
        }
    }
}
...
void MM_EnSkb_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, MM_EnSkb_OverrideLimbDraw,
                      MM_EnSkb_PostLimbDraw, &this->actor);
    if (this->drawDmgEffTimer > 0) {
