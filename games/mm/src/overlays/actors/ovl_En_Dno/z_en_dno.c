...
void func_80A711D0(EnDno* this, PlayState* play, Vec3f* vec) {
    f32 rand = MM_Rand_ZeroOne() * 0.5f;

    MM_Lights_PointGlowSetInfo(&this->lightInfo, vec->x, vec->y, vec->z, (127.5f * rand) + 127.5f,
...
s32 func_80A71424(s16* arg0, s16 arg1, s16 yawToPlayer, s16 rotY, s16 arg4, s16 arg5) {
    ...
    if (arg4 >= ABS(temp_v0)) {
        ret = MM_Math_ScaledStepToS(arg0, arg1 + temp_v0, arg5);
    } else {
        ret = MM_Math_ScaledStepToS(arg0, arg1, arg5);
    }
    ...
    do {
        ...
        if (actor != NULL) {
            if (BG_CRACE_MOVEBG_GET_TYPE(actor) == BG_CRACE_MOVEBG_TYPE_OPENING) {
                MM_Flags_SetSwitch(play, BG_CRACE_MOVEBG_GET_SWITCH_FLAG(actor));
            }
            actor = actor->next;
        }
    } while (actor != NULL);
...
void func_80A7153C(EnDno* this, Vec3f* arg1, Vec3f* arg2) {
    f32 sp1C = MM_Math_CosS(this->actor.home.rot.y);
    f32 sp18 = MM_Math_SinS(this->actor.home.rot.y);
        ...
        if (crace != NULL) {
            if (BG_CRACE_MOVEBG_GET_TYPE(&crace->dyna.actor) == BG_CRACE_MOVEBG_TYPE_CLOSING &&
                ...
                if (SubS_LineSegVsPlane(&crace->dyna.actor.home.pos, &crace->dyna.actor.home.rot, &D_80A73B2C,
                    ...
                    MM_Math_Vec3f_Diff(&this->actor.world.pos, &crace->dyna.actor.home.pos, &sp7C);
                    Matrix_RotateYS(-crace->dyna.actor.home.rot.y, MTXMODE_NEW);
                    MM_Matrix_MultVec3f(&sp7C, &sp70);
...
void func_80A71788(EnDno* this, PlayState* play) {
    ...
    do {
        actor = SubS_FindActor(play, actor, ACTORCAT_BG, ACTOR_BG_CRACE_MOVEBG);
        if (actor != NULL) {
            MM_Flags_UnsetSwitch(play, BG_CRACE_MOVEBG_GET_SWITCH_FLAG(actor));
            actor = actor->next;
        }
    } while (actor != NULL);
}
...
void EnDno_Init(Actor* thisx, PlayState* play) {
    ...
    while (true) {
        ...
        if (actor == NULL) {
            ...
            MM_ActorShape_Init(&thisx->shape, 0.0f, MM_ActorShadow_DrawCircle, 21.0f);
            MM_SkelAnime_InitFlex(play, &this->skelAnime, &gDekuButlerSkel, &gDekuButlerIdleAnim, this->jointTable,
            ...
            MM_Animation_Change(&this->skelAnime, sAnimationSpeedInfo[EN_DNO_ANIM_IDLE_WITH_CANDLE].animation, 1.0f, 0.0f,
                             MM_Animation_GetLastFrame(sAnimationSpeedInfo[EN_DNO_ANIM_IDLE_WITH_CANDLE].animation),
        ...
        if (CHECK_WEEKEVENTREG(WEEKEVENTREG_27_01)) {
            if (!(this->unk_3B0 & 0x20)) {
                SubS_ChangeAnimationBySpeedInfo(&this->skelAnime, sAnimationSpeedInfo, EN_DNO_ANIM_PRAYER_LOOP,
                                                &this->animIndex);
                this->actor.shape.rot.y = MM_Actor_WorldYawTowardActor(&this->actor, this->unk_460);
            }
        } else {
...
void func_80A71C3C(EnDno* this, PlayState* play) {
    switch (this->animIndex) {
        ...
        case EN_DNO_ANIM_PRAYER_LOOP:
            MM_Math_SmoothStepToS(&this->actor.shape.rot.y, MM_Actor_WorldYawTowardActor(&this->actor, this->unk_460), 2,
                               0xE38, 0x222);
            break;
    ...
    switch (MM_Message_GetState(&play->msgCtx)) {
        case TEXT_STATE_NONE:
            if (!(this->unk_3B0 & 0x10) ||
                MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0xE38)) {
                switch (this->textId) {
                    case 0x80B:
                        SubS_ChangeAnimationBySpeedInfo(&this->skelAnime, sAnimationSpeedInfo,
                ...
                switch (this->animIndex) {
                    case EN_DNO_ANIM_IMPLORE_START:
                        ...
                        MM_Math_ScaledStepToS(&this->actor.shape.rot.y,
                                           MM_Actor_WorldYawTowardActor(&this->actor, this->unk_460), 0x71C);
                        break;
        ...
        case TEXT_STATE_DONE:
            switch (play->msgCtx.currentTextId) {
                ...
                case 0x80C:
                    if (MM_Message_ShouldAdvance(play)) {
                        MM_Message_ContinueTextbox(play, 0x80D);
                    }
                    break;

                case 0x80D:
                    if (MM_Message_ShouldAdvance(play)) {
                        MM_Message_ContinueTextbox(play, 0x80E);
                    }
                    break;
                ...
                case 0x80E:
                    ...
                    } else if ((this->animIndex == EN_DNO_ANIM_IDLE) && MM_Message_ShouldAdvance(play)) {
                        MM_Message_CloseTextbox(play);
                        func_80A71B68(this, play);
                    }
                    break;
                ...
                case 0x80F:
                    if (MM_Message_ShouldAdvance(play)) {
                        MM_Message_ContinueTextbox(play, 0x810);
                    }
                    break;
...
void func_80A72438(EnDno* this, PlayState* play) {
    ...
    SubS_ChangeAnimationBySpeedInfo(&this->skelAnime, sAnimationSpeedInfo, EN_DNO_ANIM_IDLE_WITH_CANDLE,
                                    &this->animIndex);
    this->actor.textId = 0;
    if (MM_Flags_GetSwitch(play, EN_DNO_GET_RACE_STARTED_SWITCH_FLAG(&this->actor))) {
        this->unk_454 = 1.0f;
    }
    ...
    switch (MM_Message_GetState(&play->msgCtx)) {
            ...
            switch (this->unk_328) {
                case 0:
                    if (this->animIndex == EN_DNO_ANIM_IDLE_WITH_CANDLE) {
                        if (MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x2D8)) {
                            SubS_ChangeAnimationBySpeedInfo(&this->skelAnime, sAnimationSpeedInfo,
                        ...
                        if (MM_Flags_GetSwitch(play, EN_DNO_GET_RACE_STARTED_SWITCH_FLAG(&this->actor))) {
                            MM_Message_StartTextbox(play, 0x801, &this->actor);
                        } else if (MM_Player_GetMask(play) == PLAYER_MASK_SCENTS) {
                            MM_Message_StartTextbox(play, 0x806, &this->actor);
                        } else {
                            MM_Message_StartTextbox(play, 0x800, &this->actor);
                        }
                        SubS_ChangeAnimationBySpeedInfo(&this->skelAnime, sAnimationSpeedInfo,
                                                        EN_DNO_ANIM_IDLE_WITH_CANDLE, &this->animIndex);
                    }
                ...
                case 2:
                    if (MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x2D8)) {
                        ...
                        MM_Message_StartTextbox(play, 0x802, &this->actor);
                    }
                    break;

                case 3:
                    MM_Message_StartTextbox(play, 0x804, &this->actor);
                    break;
        ...
        case TEXT_STATE_FADING:
            if (((play->msgCtx.currentTextId == 0x800) || (play->msgCtx.currentTextId == 0x801)) &&
                ...
                MM_Math_SmoothStepToF(&this->unk_454, 1.0f, 1.0f, 0.1f, 0.01f);
                if (this->skelAnime.curFrame <= 23.0f) {
                    this->unk_452 = 3;
                    if (MM_Animation_OnFrame(&this->skelAnime, 23.0f)) {
                ...
                if (MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
                    ...
                    MM_Message_StartTextbox(play, 0x803, &this->actor);
                }
            }
            break;
            ...
            switch (play->msgCtx.currentTextId) {
                ...
                case 0x802:
                    if (MM_Message_ShouldAdvance(play)) {
                        ...
                        MM_Actor_OfferGetItem(&this->actor, play, this->getItemId, 60.0f, 60.0f);
                        MM_Message_CloseTextbox(play);
                ...
                case 0x806:
                    if (MM_Message_ShouldAdvance(play)) {
                        MM_Message_ContinueTextbox(play, 0x800);
                    }
                    break;
...
void func_80A72C04(EnDno* this, PlayState* play) {
    ...
    MM_Math_Vec3f_Copy(&this->unk_334, &this->actor.world.pos);
    ...
    MM_Flags_SetSwitch(play, EN_DNO_GET_RACE_STARTED_SWITCH_FLAG(&this->actor));
    this->actionFunc = func_80A730A0;
}

void func_80A72CF8(EnDno* this, PlayState* play) {
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 0.0f, 0.0f, 0.0f, UPDBGCHECKINFO_FLAG_4);
    MM_Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, ACTOR_DOOR_WARP1, this->actor.world.pos.x + 80.0f,
...
s32 EnDno_ActorPathing_UpdateActorInfo(PlayState* play, ActorPathing* actorPath) {
    ...
    if ((temp_v0 <= 0x4000) && (temp_v0 >= -0x4000)) {
        MM_Math_SmoothStepToF(&thisx->speed, 15.0f, 0.8f, 1.0f, 0.01f);
    } else {
        if (thisx->xzDistToPlayer <= 80.0f) {
            MM_Math_SmoothStepToF(&thisx->speed, 8.0f, 0.5f, 0.5f, 0.01f);
        } else if (thisx->xzDistToPlayer <= 360.0f) {
            MM_Math_SmoothStepToF(&thisx->speed, 7.0f, 0.5f, 0.5f, 0.01f);
        } else {
            MM_Math_SmoothStepToF(&thisx->speed, 3.5f, 0.5f, 0.5f, 0.01f);
        }
    }
    ...
    if (actorPath->distSqToCurPoint < SQ(thisx->speed)) {
        ...
        MM_Math_ScaledStepToS(&thisx->world.rot.x, actorPath->rotToCurPoint.x, temp_v0_2);
        MM_Math_ScaledStepToS(&thisx->world.rot.y, actorPath->rotToCurPoint.y, (s32)(sp2C * sp38) + 0x71C);
    }
...
void func_80A730A0(EnDno* this, PlayState* play) {
    ...
    MM_Math_Vec3f_Copy(&this->actor.world.pos, &this->unk_334);
    this->actor.world.pos.y += MM_Math_SinS(this->unk_3AC) * (4.0f + MM_Math_SinS(this->unk_3AE));
    this->unk_3AC += 4500;
    this->unk_3AE += 1000;
    ...
    if (this->actorPath.flags & ACTOR_PATHING_REACHED_END_PERMANENT) {
        MM_Math_Vec3f_Copy(&this->actor.world.pos, &this->actorPath.curPoint);
...
void func_80A73244(EnDno* this, PlayState* play) {
    ...
    this->unk_328 = 2;
    this->actor.speed = 0.0f;
    MM_Flags_UnsetSwitch(play, EN_DNO_GET_RACE_STARTED_SWITCH_FLAG(&this->actor));
    gSaveContext.timerStates[TIMER_ID_MINIGAME_1] = TIMER_STATE_STOP;
    this->unk_44E = 0;
...
void func_80A732C8(EnDno* this, PlayState* play) {
    ...
    if (this->unk_44E == 0) {
        if (MM_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 0x71C)) {
            this->unk_3B0 |= 4;
            this->unk_44E = 3;
        ...
        if (MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
            ...
            if (this->skelAnime.curFrame >= 20.0f) {
                MM_Math_SmoothStepToF(&this->unk_454, 0.0f, 1.0f, 0.125f, 0.01f);
            }
...
void EnDno_Update(Actor* thisx, PlayState* play) {
    ...
    MM_Actor_SetFocus(&this->actor, 51.0f);
}
...
void EnDno_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (phi_v0 == true) {
        ...
        if (limbIndex == DEKU_BUTLER_LIMB_PARASOL_HANDLE) {
            MM_Matrix_Scale(this->unk_454, this->unk_454, this->unk_454, MTXMODE_APPLY);
    ...
    if ((this->unk_3B0 & 1) && (limbIndex == DEKU_BUTLER_LIMB_CANDLE_WICK)) {
        ...
        MM_Matrix_MultVec3f(&D_80A73B40, &sp84);
        func_80A711D0(this, play, &sp84);
        MM_Matrix_ReplaceRotation(&play->billboardMtxF);
        MM_Matrix_Scale(0.15f, 0.15f, 1.0f, MTXMODE_APPLY);
        MM_Matrix_Translate(0.0f, -3200.0f, 0.0f, MTXMODE_APPLY);
