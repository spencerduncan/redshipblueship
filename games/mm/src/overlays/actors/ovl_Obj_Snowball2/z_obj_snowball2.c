...
void func_80B38E88(ObjSnowball2* this, PlayState* play) {
    ...
    if (this->unk_1AE == 0) {
        ...
        if (temp_v0 >= 0) {
            MM_Item_DropCollectible(play, &this->actor.world.pos, (ENOBJSNOWBALL2_GET_7F00(&this->actor) << 8) | temp_v0);
            this->unk_1AE = 1;
        }
    }
}
...
void func_80B38EFC(ObjSnowball2* this, PlayState* play) {
    ...
    for (i = 0; i < 2; i++) {
        sp8C.x = ((MM_Rand_ZeroOne() * 30.0f) - 15.0f) + this->actor.world.pos.x;
        sp8C.y = ((MM_Rand_ZeroOne() * 20.0f) - 10.0f) + sp88;
        sp8C.z = ((MM_Rand_ZeroOne() * 30.0f) - 15.0f) + this->actor.world.pos.z;

        sp98.x = (MM_Rand_ZeroOne() - 0.5f) * 1.6f;
        sp98.y = -1.5f;
        sp98.z = (MM_Rand_ZeroOne() - 0.5f) * 1.6f;

        spA4.x = sp98.x * -0.06f;
        spA4.y = sp98.y * -0.06f;
        spA4.z = sp98.z * -0.06f;

        func_800B0E48(play, &sp8C, &sp98, &spA4, &D_80B3A914, &D_80B3A918, (s32)(MM_Rand_ZeroOne() * 40.0f) + 10,
                      (s32)(MM_Rand_ZeroOne() * 30.0f) + 10);
    }
}
...
void func_80B39108(ObjSnowball2* this, PlayState* play) {
    ...
    for (i = 0, phi_s5 = 0; i < 11; i++, phi_s5 += (0x10000 / 11)) {
        temp_s3 = (s32)(MM_Rand_ZeroOne() * 5957.0f) + phi_s5;

        if ((MM_Rand_Next() >> 0x1E) == 0) {
            phi_s1 = 0x20;
        } else {
            phi_s1 = 0x40;
        }
        ...
        temp_f22 = MM_Math_SinS(temp_s3);
        temp_f24 = MM_Math_CosS(temp_s3);
        temp_f12 = (MM_Rand_ZeroOne() * 1.6f) - 0.8f;
        ...
        MM_EffectSsKakera_Spawn(play, &spD0, &spDC, &spD0, -300, phi_s1, 30, 0, 0, (temp >> 0x1D) + 8, phi_v0, 0, 50, -1,
                             OBJECT_GOROIWA, D_80B3A91C[i & 3]);

        spD0.x += (MM_Rand_ZeroOne() - 0.5f) * 40.0f;
        spD0.y += (MM_Rand_ZeroOne() - 0.3f) * 45.0f;
        spD0.z += (MM_Rand_ZeroOne() - 0.5f) * 40.0f;

        func_800B0E48(play, &spD0, &gZeroVec3f, &D_80B3A92C, &D_80B3A914, &D_80B3A918,
                      (s32)(MM_Rand_ZeroOne() * 70.0f) + 10, (s32)(MM_Rand_ZeroOne() * 100.0f) + 10);
    }
    spD0.y = (temp_f2 * temp_f20) + spC8;
}
...
void func_80B39470(Actor* thisx, PlayState* play) {
    ...
    for (phi_s0 = 0, i = 0; i < 5; i++, phi_s0 += (0x10000 / 5)) {
        sp58.x = this->actor.world.pos.x + (MM_Math_SinS((s32)(MM_Rand_ZeroOne() * 7200.0f) + phi_s0) * 15.0f);
        sp58.z = this->actor.world.pos.z + (MM_Math_CosS((s32)(MM_Rand_ZeroOne() * 7200.0f) + phi_s0) * 15.0f);
        MM_EffectSsGSplash_Spawn(play, &sp58, NULL, NULL, 0, 200);
    }
...
void func_80B395C4(PlayState* play, Vec3f* arg1) {
    MM_EffectSsGRipple_Spawn(play, arg1, 200, 700, 0);
}
...
void func_80B39638(PlayState* play, Vec3f* arg1) {
    ...
    D_80B3A938 += (s16)(MM_Rand_Next() >> 0x11);

    for (i = 0; i < 2; i++) {
        D_80B3A938 += 0x8000;
        temp_f20 = MM_Rand_ZeroOne();
        temp_f22 = MM_Math_SinS(D_80B3A938) * temp_f20;
        temp_f24 = MM_Rand_ZeroOne();
        temp_f2 = MM_Math_CosS(D_80B3A938) * temp_f20;
        ...
        MM_EffectSsIceSmoke_Spawn(play, &sp98, &sp8C, &sp80, (s32)(MM_Rand_ZeroOne() * 30.0f) + 68);
    }
}
...
void func_80B39834(Actor* thisx, PlayState* play) {
    ...
    for (i = 0; i < 3; i++) {
        MM_EffectSsBubble_Spawn(play, &this->actor.world.pos, 0.0f, 20.0f, 30.0f, (MM_Rand_ZeroOne() * 0.11f) + 0.03f);
    }
}
...
void func_80B39908(ObjSnowball2* this, PlayState* play) {
    ...
    if (this->collider.elements[0].base.acElemFlags & ACELEM_HIT) {
        ...
        for (i = 0; i < 4; i++) {
            ...
            func_800B0E48(play, &sp94, &spA0, &spAC, &D_80B3A914, &D_80B3A918, (s32)(MM_Rand_ZeroOne() * 40.0f) + 10,
                          (s32)(MM_Rand_ZeroOne() * 30.0f) + 10);
        }
    }
}
...
void func_80B39B5C(ObjSnowball2* this, PlayState* play) {
    MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 40, NA_SE_EV_SMALL_SNOWBALL_BROKEN);
}
...
void ObjSnowball2_Init(Actor* thisx, PlayState* play) {
    ...
    MM_ActorShape_Init(&this->actor.shape, 200.0f, NULL, 12.5f);
...
void func_80B39C9C(ObjSnowball2* this, PlayState* play) {
    ...
    if (MM_Actor_HasParent(&this->actor, play)) {
        ...
        MM_Player_PlaySfx(GET_PLAYER(play), NA_SE_PL_PULL_UP_SNOWBALL);
        func_80B39F60(this);
...
void func_80B3A0D8(ObjSnowball2* this) {
    ...
    this->unk_1A8 = MM_Rand_S16Offset(-5000, 4000);
    this->unk_1AA = MM_Rand_S16Offset(-800, 1600);
...
void func_80B3A13C(ObjSnowball2* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & (BGCHECKFLAG_WATER | BGCHECKFLAG_WATER_TOUCH)) {
        ...
        } else if ((((play->gameplayFrames % 16) == 0) || ((MM_Rand_Next() >> 0x10) == 0)) &&
...
void func_80B3A500(ObjSnowball2* this, PlayState* play) {
    ...
    if (((play->gameplayFrames % 16) == 0) || ((MM_Rand_Next() >> 0x10) == 0)) {
