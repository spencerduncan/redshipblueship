...
void MM_EnBomBowlMan_Init(Actor* thisx, PlayState* play);
void MM_EnBomBowlMan_Destroy(Actor* thisx, PlayState* play);
void MM_EnBomBowlMan_Update(Actor* thisx, PlayState* play);
void MM_EnBomBowlMan_Draw(Actor* thisx, PlayState* play);
...
ActorProfile En_Bom_Bowl_Man_Profile = {
    ...
    /**/ OBJECT_CS,
    /**/ sizeof(EnBomBowlMan),
    /**/ MM_EnBomBowlMan_Init,
    /**/ MM_EnBomBowlMan_Destroy,
    /**/ MM_EnBomBowlMan_Update,
    /**/ MM_EnBomBowlMan_Draw,
};
...
static AnimationHeader* MM_sAnimations[ENBOMBOWLMAN_ANIM_MAX] = {
...
static u8 MM_sAnimationModes[ENBOMBOWLMAN_ANIM_MAX] = {
...
void func_809C49CC(EnBomBowlMan* this) {
    ...
    if ((this->animIndex == ENBOMBOWLMAN_ANIM_11) &&
        (MM_Animation_OnFrame(&this->skelAnime, 4.0f) || MM_Animation_OnFrame(&this->skelAnime, 8.0f) ||
         MM_Animation_OnFrame(&this->skelAnime, 12.0f))) {
        Actor_PlaySfx(&this->actor, NA_SE_EV_BOMBERS_WALK);
    }
    ...
    if ((this->animIndex == ENBOMBOWLMAN_ANIM_15) && MM_Animation_OnFrame(&this->skelAnime, 15.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EV_BOMBERS_LAND);
    }

    if ((this->animIndex == ENBOMBOWLMAN_ANIM_6) && MM_Animation_OnFrame(&this->skelAnime, 8.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EV_BOMBERS_LAND);
    }
}
...
void func_809C4BC4(EnBomBowlMan* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(gSaveContext.save.saveInfo.bomberCode); i++) {
        MM_Math_Vec3f_Copy(&sp7C, &this->actor.home.pos);
        code = gSaveContext.save.saveInfo.bomberCode[i];
        if (code == 1) {
            MM_Math_Vec3f_Copy(&this->actor.world.pos, &D_809C61A0[i]);
            this->unk_2DC[code - 1] = this;
        } else {
            MM_Math_Vec3f_Copy(&sp7C, &D_809C61A0[i]);
            bomBowlMan = (EnBomBowlMan*)MM_Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, ACTOR_EN_BOM_BOWL_MAN,
...
void func_809C4DA4(EnBomBowlMan* this, PlayState* play) {
    MM_Math_SmoothStepToS(&this->unk_290, this->unk_2D4, 1, 0x1388, 0);
    ...
    if (this->unk_2B8 == 0) {
        ...
        if (player->transformation == PLAYER_FORM_HUMAN) {
            MM_Message_StartTextbox(play, D_809C618C[0], &this->actor);
        } else {
            MM_Message_StartTextbox(play, D_809C6198[0], &this->actor);
        }
    }

    if ((this->unk_2BC == 0) && (MM_Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && MM_Message_ShouldAdvance(play)) {
        Player* player = GET_PLAYER(play);
        s32 pad;
        s32 sp28 = false;
        ...
        switch (this->unk_2C0) {
            case 0:
                this->unk_2C0 = 1;
                D_809C6104 = 1;
                Camera_SetTargetActor(MM_Play_GetCamera(play, CutsceneManager_GetCurrentSubCamId(this->csIdList[2])),
                                      &this->unk_2D8->actor);
                this->unk_2D4 = 0;
                this->unk_2BC = 10;
        ...
        if (!sp28) {
            if (player->transformation == PLAYER_FORM_HUMAN) {
                MM_Message_ContinueTextbox(play, D_809C618C[this->unk_2C0]);
            } else {
                MM_Message_ContinueTextbox(play, D_809C6198[this->unk_2C0]);
            }
        }
    }
}
...
void func_809C51B4(EnBomBowlMan* this, PlayState* play) {
    ...
    if ((play->msgCtx.bombersNotebookEventQueueCount == 0) &&
        ((play->msgCtx.msgMode == MSGMODE_NONE) || (MM_Message_GetState(&play->msgCtx) == TEXT_STATE_DONE))) {
...
void func_809C53A4(EnBomBowlMan* this) {
    this->actor.draw = MM_EnBomBowlMan_Draw;
...
void func_809C5408(EnBomBowlMan* this, PlayState* play) {
    ...
    MM_Math_SmoothStepToS(&this->actor.world.rot.y, MM_Math_Vec3f_Yaw(&this->actor.world.pos, &player->actor.world.pos), 1,
                       0x1388, 0);
    MM_Math_SmoothStepToS(&player->actor.shape.rot.y, MM_Math_Vec3f_Yaw(&player->actor.world.pos, &this->actor.world.pos), 1,
                       0x1388, 0);
...
void func_809C5738(EnBomBowlMan* this, PlayState* play) {
    s32 pad[3];
    s16 yaw = MM_Math_Vec3f_Yaw(&this->actor.world.pos, &this->unk_2A0);
...
void func_809C59A4(EnBomBowlMan* this, PlayState* play) {
    MM_Actor_OfferGetItem(&this->actor, play, GI_BOMBERS_NOTEBOOK, 300.0f, 300.0f);
    this->unk_29C = 1;
...
void func_809C5BF4(EnBomBowlMan* this, PlayState* play) {
    ...
    if (D_809C6100 != 0) {
        ...
        if (this->unk_2F4 == 0) {
            subCam = MM_Play_GetCamera(play, CutsceneManager_GetCurrentSubCamId(this->csIdList[3]));
            ...
            if (D_809C6100 > 5) {
                ...
                if (player->transformation == PLAYER_FORM_HUMAN) {
                    this->unk_2C0 = 5;
                    MM_Message_StartTextbox(play, D_809C618C[this->unk_2C0], &this->actor);
                } else {
                    this->unk_2C0 = 3;
                    MM_Message_StartTextbox(play, D_809C6198[this->unk_2C0], &this->actor);
                }
                this->unk_29C = 0;
                this->actionFunc = func_809C4DA4;
            } else {
...
void MM_EnBomBowlMan_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->unk_2F2 == 0) {
        ...
        if (this->unk_2F0 >= 3) {
            this->unk_2F0 = 0;
            this->unk_2F2 = (s32)MM_Rand_ZeroFloat(60.0f) + 20;
        }
    }
...
s32 MM_EnBomBowlMan_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    EnBomBowlMan* this = (EnBomBowlMan*)thisx;
...
void MM_EnBomBowlMan_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_gSPSegment(POLY_OPA_DISP++, 0x08, Lib_SegmentedToVirtual(D_809C6214[this->unk_2F0]));
    MM_gSPSegment(POLY_OPA_DISP++, 0x09, Lib_SegmentedToVirtual(D_809C6220[this->unk_2F4]));
    MM_gSPSegment(POLY_OPA_DISP++, 0x0A, Lib_SegmentedToVirtual(D_809C6200[this->unk_2F4]));

    Scene_SetRenderModeXlu(play, 0, 1);
    MM_SkelAnime_DrawFlexOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                          MM_EnBomBowlMan_OverrideLimbDraw, NULL, &this->actor);
