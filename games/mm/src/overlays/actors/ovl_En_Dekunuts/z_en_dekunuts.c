...
void MM_EnDekunuts_Init(Actor* thisx, PlayState* play);
void MM_EnDekunuts_Destroy(Actor* thisx, PlayState* play);
void MM_EnDekunuts_Update(Actor* thisx, PlayState* play);
void MM_EnDekunuts_Draw(Actor* thisx, PlayState* play);
...
ActorProfile En_Dekunuts_Profile = {
    ...
    /**/ sizeof(EnDekunuts),
    /**/ MM_EnDekunuts_Init,
    /**/ MM_EnDekunuts_Destroy,
    /**/ MM_EnDekunuts_Update,
    /**/ MM_EnDekunuts_Draw,
};
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 1, 18, 32, MASS_IMMOVABLE };
...
void MM_EnDekunuts_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_Init(play, &this->skelAnime, &gDekuScrubSkel, &gDekuScrubIdleAnim, this->jointTable, this->morphTable,
                   DEKU_SCRUB_LIMB_MAX);
...
void func_808BD428(EnDekunuts* this) {
    MM_Animation_PlayOnceSetSpeed(&this->skelAnime, &gDekuScrubUpAnim, 0.0f);
...
void func_808BD49C(EnDekunuts* this, PlayState* play) {
    ...
    if (!phi_v1 && (this->actor.params == ENDEKUNUTS_GET_FF00_0) && (MM_Player_GetMask(play) != PLAYER_MASK_STONE) &&
...
void func_808BD78C(EnDekunuts* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &gDekuScrubLookAroundAnim);
    this->unk_190 = 2;
...
void func_808BD7D4(EnDekunuts* this, PlayState* play) {
    ...
    if ((this->unk_190 == 0) || ((this->actor.xzDistToPlayer < 120.0f) && MM_Player_GetMask(play) != PLAYER_MASK_STONE)) {
        func_808BDC9C(this);
    }
}

void func_808BD870(EnDekunuts* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gDekuScrubIdleAnim, -3.0f);
...
void func_808BDA08(EnDekunuts* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gDekuScrubSpitAnim);
...
void func_808BDA4C(EnDekunuts* this, PlayState* play) {
    ...
    if (this->actor.params == ENDEKUNUTS_GET_FF00_2) {
        ...
        pitch = MM_Actor_WorldPitchTowardPoint(&this->actor, &sp58);
        pitch = CLAMP(pitch, -0x3800, -0x2000);
        if (this->skelAnime.curFrame < 7.0f) {
            MM_Math_ScaledStepToS(&this->actor.world.rot.x, pitch, 0x800);
        } else {
            MM_Math_ScaledStepToS(&this->actor.world.rot.x, 0, 0x800);
        }
    }
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        val = MM_Math_CosS(this->actor.world.rot.x) * 15.0f;
        pos.x = (MM_Math_SinS(this->actor.shape.rot.y) * val) + this->actor.world.pos.x;
        pos.y = (this->actor.world.pos.y + 12.0f) - (MM_Math_SinS(this->actor.world.rot.x) * 15.0f);
        pos.z = (MM_Math_CosS(this->actor.shape.rot.y) * val) + this->actor.world.pos.z;
        params = (this->actor.params == ENDEKUNUTS_GET_FF00_2) ? ENDEKUNUTS_GET_FF00_2 : ENDEKUNUTS_GET_FF00_0;

        if (MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_NUTSBALL, pos.x, pos.y, pos.z, this->actor.world.rot.x,
                        this->actor.shape.rot.y, 0, params) != NULL) {
            Actor_PlaySfx(&this->actor, NA_SE_EN_NUTS_THROW);
        }
    } else if ((this->unk_190 >= 2) && MM_Animation_OnFrame(&this->skelAnime, 12.0f)) {
        MM_Animation_MorphToPlayOnce(&this->skelAnime, &gDekuScrubSpitAnim, -3.0f);
        if (this->unk_190 != 0) {
            this->unk_190--;
        }
    }
...
void func_808BDC9C(EnDekunuts* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gDekuScrubBurrowAnim, -5.0f);
    this->unk_190 = 0;
...
void func_808BDD54(EnDekunuts* this, PlayState* play) {
    ...
    MM_Math_ApproachF(&this->actor.world.pos.x, this->actor.home.pos.x, 0.5f, 3.0f);
    MM_Math_ApproachF(&this->actor.world.pos.z, this->actor.home.pos.z, 0.5f, 3.0f);
}

void func_808BDE7C(EnDekunuts* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gDekuScrubUnburrowAnim, -3.0f);
...
void func_808BDF60(EnDekunuts* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &gDekuScrubRunAnim);
    this->unk_190 = 2;
    this->unk_18C = 0;
    this->collider.base.acFlags |= AC_ON;
...
void func_808BDFB8(EnDekunuts* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->actor.speed, 7.5f, 1.0f);
    if (!MM_Math_SmoothStepToS(&this->actor.world.rot.y, this->unk_192, 1, 0xE38, 0xB6)) {
    ...
    if ((this->unk_18D == 0) && (MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) < 20.0f) &&
...
void func_808BE1CC(EnDekunuts* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &gDekuScrubPantingAnim);
    this->unk_190 = 3;
    this->actor.speed = 0.0f;
    if (this->unk_18D != 0) {
        this->unk_18D--;
...
void func_808BE294(EnDekunuts* this, s32 arg1) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gDekuScrubDamageAnim, -3.0f);
    ...
    MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 255, COLORFILTER_BUFFLAG_OPA,
                         MM_Animation_GetLastFrame(&gDekuScrubDamageAnim));
}
...
void func_808BE484(EnDekunuts* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gDekuScrubDieAnim);
...
void func_808BE4D4(EnDekunuts* this, PlayState* play) {
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        MM_EffectSsDeadDb_Spawn(play, &sp40, &gZeroVec3f, &gZeroVec3f, &D_808BEF90, &D_808BEF94, 200, 0, 13);
        MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 11, NA_SE_EN_EXTINCT);
        sp40.y = this->actor.world.pos.y + 10.0f;
        MM_EffectSsHahen_SpawnBurst(play, &sp40, 3.0f, 0, 12, 3, 15, HAHEN_OBJECT_DEFAULT, 10, NULL);
        MM_Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, 0xE0);
        MM_Actor_Spawn(&play->actorCtx, play, ACTOR_OBJ_ETCETERA, this->actor.home.pos.x, this->actor.home.pos.y,
                    this->actor.home.pos.z, 0, this->actor.home.rot.y, 0,
                    DEKU_FLOWER_PARAMS(DEKU_FLOWER_TYPE_PINK_WITH_INITIAL_BOUNCE));
        MM_EffectSsHahen_SpawnBurst(play, &this->actor.home.pos, 6.0f, 0, 6, 2, 15, OBJECT_DEKUNUTS, 10,
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if ((this->drawDmgEffType != ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX) ||
            ...
            if ((this->actor.colChkInfo.mass == 50) || (this->actor.params != ENDEKUNUTS_GET_FF00_0)) {
                if ((this->actor.params != ENDEKUNUTS_GET_FF00_1) && !MM_Actor_ApplyDamage(&this->actor)) {
...
void MM_EnDekunuts_Update(Actor* thisx, PlayState* play) {
    ...
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 20.0f, this->collider.dim.radius, this->collider.dim.height,
    ...
    if (this->drawDmgEffAlpha > 0.0f) {
        if (this->drawDmgEffType != ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX) {
            ...
            this->drawDmgEffScale = CLAMP_MAX(this->drawDmgEffScale, 0.55f);
                   ...
                   !MM_Math_StepToF(&this->drawDmgEffFrozenSteamScale, 0.55f, (33.0f / 1600.0f))) {
            Actor_PlaySfx_Flagged(&this->actor, NA_SE_EV_ICE_FREEZE - SFX_FLAG);
        }
    }
}

s32 MM_EnDekunuts_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    EnDekunuts* this = (EnDekunuts*)thisx;
    f32 arg1;
    f32 arg2;
    f32 arg3;
    ...
    if (this->actionFunc == func_808BDA4C) {
        if (limbIndex == DEKU_SCRUB_LIMB_SNOUT) {
            ...
            MM_Matrix_Scale(arg1, arg2, arg3, MTXMODE_APPLY);
...
void MM_EnDekunuts_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, MM_EnDekunuts_OverrideLimbDraw,
    ...
    MM_Gfx_DrawDListOpa(play, gDekuScrubFlowerDL);
