...
static AnimationHeader* MM_sAnimations[ENBSB_ANIM_MAX] = {
...
static u8 MM_sAnimationModes[ENBSB_ANIM_MAX] = {
...
void EnBsb_ChangeAnim(EnBsb* this, s32 animIndex) {
    ...
    MM_Animation_Change(&this->skelAnime, MM_sAnimations[this->animIndex], 1.0f, 0.0f, this->animEndFrame,
                     MM_sAnimationModes[this->animIndex], -2.0f);
}
...
void func_80C0B31C(PlayState* play, EnBsb* this, Vec3f* pos) {
    ...
    f32 angle = (MM_Rand_ZeroOne() - 0.5f) * 12.56f;
    f32 effectScale;
    s32 i;
    s32 pad[4];

    effectPos.y = this->actor.floorHeight;
    effectPos.x = MM_Math_SinF(angle) * 5.0f + pos->x;
    effectPos.z = MM_Math_CosF(angle) * 5.0f + pos->z;
    hahenAccel.x = MM_Rand_CenteredFloat(1.0f) * 0.5f;
    hahenAccel.z = MM_Rand_CenteredFloat(1.0f) * 0.5f;
    hahenVelocity.y += (MM_Rand_ZeroOne() - 0.5f) * 15.0f;

    MM_EffectSsHahen_Spawn(play, &effectPos, &hahenVelocity, &hahenAccel, 0, MM_Rand_ZeroOne() * 5.0f + 10.0f,
    ...
    if (!this->unk_02AE) {
        ...
        for (i = 0; i < 10; i++) {
            if ((i % 2) == 0) {
                MM_Math_Vec3f_Copy(&effectPos, &this->unk_0304);
            } else {
                MM_Math_Vec3f_Copy(&effectPos, &this->unk_02F8);
            }
            effectPos.x += MM_Rand_CenteredFloat(5.0f);
            effectPos.y += MM_Rand_CenteredFloat(5.0f);
            effectPos.z += MM_Rand_CenteredFloat(5.0f);
            effectAccel.y = -1.0f;
            effectVelocity.x = (MM_Rand_ZeroOne() - 0.5f) * 4.0f;
            effectVelocity.y = (MM_Rand_ZeroOne() * 10.0f) + 4.0f;
            effectVelocity.z = (MM_Rand_ZeroOne() - 0.5f) * 4.0f;
            effectScale = MM_Rand_CenteredFloat(1.0f) * 0.002f + 0.005f;
            EnBsb_InitializeEffect(this, &effectPos, &effectVelocity, &effectAccel, effectScale,
                                   (s32)MM_Rand_ZeroFloat(10.0f) + 30);
        }
    }
}
...
void EnBsb_Init(Actor* thisx, PlayState* play) {
    ...
    MM_Actor_SetScale(&this->actor, 0.04f);
    this->alpha = 255;
    this->actor.colChkInfo.mass = MASS_IMMOVABLE;

    if (ENBSB_GET_PARAMS_8000(&this->actor)) {
        MM_SkelAnime_Init(play, &this->skelAnime, &object_bsb_Skel_00C3E0, &object_bsb_Anim_004894, this->jointTable,
    ...
    MM_SkelAnime_Init(play, &this->skelAnime, &object_bsb_Skel_00C3E0, &object_bsb_Anim_0086BC, this->jointTable,
                   this->morphTable, OBJECT_BSB_LIMB_MAX);
...
void func_80C0B970(EnBsb* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 8.0f) || MM_Animation_OnFrame(&this->skelAnime, 14.0f)) {
...
void func_80C0BA58(EnBsb* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 2.0f) || MM_Animation_OnFrame(&this->skelAnime, 4.0f) ||
        MM_Animation_OnFrame(&this->skelAnime, 6.0f) || MM_Animation_OnFrame(&this->skelAnime, 8.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_KTIA_WALK);
        Actor_RequestQuake(play, BREG(10) + 1, BREG(11) + 2);
        if (MM_Animation_OnFrame(&this->skelAnime, 4.0f) || MM_Animation_OnFrame(&this->skelAnime, 8.0f)) {
            func_80C0B31C(play, this, &this->unk_0304);
        } else {
            func_80C0B31C(play, this, &this->unk_02F8);
        }
...
s32 func_80C0BC30(EnBsb* this) {
    if ((this->actor.world.pos.z < -2180.0f) && (this->actor.world.pos.z > -2470.0f) &&
        ...
        MM_Math_SmoothStepToS(&this->unk_0316.x, sp38.x, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0316.y, sp38.y, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0316.z, sp38.z, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0310.x, sp30.x, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0310.y, sp30.y, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0310.z, sp30.z, 1, 0x7D0, 0);
        return true;
    }

    if (this->unk_0316.z != 0) {
        MM_Math_SmoothStepToS(&this->unk_0316.x, 0, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0316.y, 0, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0316.z, 0, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0310.x, 0, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0310.y, 0, 1, 0x7D0, 0);
        MM_Math_SmoothStepToS(&this->unk_0310.z, 0, 1, 0x7D0, 0);
    }
...
void func_80C0BE1C(EnBsb* this, PlayState* play) {
    if (this->subCamId != SUB_CAM_ID_DONE) {
        MM_Math_ApproachF(&this->subCamEye.x, this->subCamEyeNext.x, 0.5f, 30.0f);
        MM_Math_ApproachF(&this->subCamEye.y, this->subCamEyeNext.y, 0.5f, 30.0f);
        MM_Math_ApproachF(&this->subCamEye.z, this->subCamEyeNext.z, 0.5f, 30.0f);
        MM_Math_ApproachF(&this->subCamAt.x, this->subCamAtNext.x, 0.5f, 30.0f);
        MM_Math_ApproachF(&this->subCamAt.y, this->subCamAtNext.y, 0.5f, 30.0f);
        MM_Math_ApproachF(&this->subCamAt.z, this->subCamAtNext.z, 0.5f, 30.0f);
        MM_Math_ApproachF(&this->subCamFov, this->subCamFovNext, 0.5f, 10.0f);
...
void func_80C0BFE8(EnBsb* this, PlayState* play) {
    ...
    if ((yaw < 0x4300) && !(this->actor.xzDistToPlayer > 300.0f)) {
        ...
        if ((play->msgCtx.ocarinaMode == OCARINA_MODE_EVENT) && (play->msgCtx.lastPlayedSong == OCARINA_SONG_SONATA)) {
            ...
            MM_Actor_ChangeCategory(play, &play->actorCtx, &this->actor, ACTORCAT_BOSS);
...
void func_80C0C0F4(EnBsb* this, PlayState* play) {
    ...
    if (!CutsceneManager_IsNext(this->csIdList[0])) {
        ...
        MM_Flags_SetSwitch(play, this->switchFlag1);
...
void func_80C0C238(EnBsb* this, PlayState* play) {
    ...
    if (this->unk_02A4 == 1) {
        ...
        MM_Math_ApproachF(&this->actor.shape.shadowScale, 25.0f, 1.0f, 2.5f);
...
void func_80C0C484(EnBsb* this, PlayState* play) {
    ...
    MM_Math_SmoothStepToS(&this->actor.world.rot.y, var_a1, 2, 0x2EE, 5);
...
void func_80C0C6A8(EnBsb* this, PlayState* play) {
    ...
    if ((this->animIndex == ENBSB_ANIM_2) && (curFrame >= this->animEndFrame) && (this->unk_0294 == 0)) {
        ...
        MM_Math_SmoothStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 1, 0x7D0, 0);
...
void func_80C0C86C(EnBsb* this) {
    ...
    this->unk_0294 = MM_Rand_S16Offset(0, 30);
...
void func_80C0C8EC(EnBsb* this, PlayState* play) {
    ...
    if ((this->unk_0294 == 0) && (fabsf(player->actor.world.pos.y - this->actor.world.pos.y) >= 100.0f) &&
        ...
        if ((this->unk_0294 == 0) && MM_Actor_IsFacingPlayer(&this->actor, 0x11C7) &&
...
void func_80C0CA28(EnBsb* this, PlayState* play) {
    ...
    MM_Actor_SpawnFloorDustRing(play, &this->actor, &this->unk_0304, 5.0f, 10, 8.0f, 1000, 100, true);
    MM_Actor_SpawnFloorDustRing(play, &this->actor, &this->unk_02F8, 5.0f, 10, 8.0f, 1000, 100, true);
...
void func_80C0CB3C(EnBsb* this, PlayState* play) {
    ...
    if (this->actor.velocity.y > 0.0f) {
        MM_Math_ApproachF(&this->actor.world.pos.x, player->actor.world.pos.x, 0.5f, 30.0f);
        MM_Math_ApproachF(&this->actor.world.pos.z, player->actor.world.pos.z, 0.5f, 30.0f);
        return;
    }
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        MM_Actor_SpawnFloorDustRing(play, &this->actor, &this->actor.world.pos, this->actor.shape.shadowScale, 10, 8.0f,
                                 2000, 100, true);
...
void func_80C0CF4C(EnBsb* this) {
    this->actor.speed = 0.0f;
    this->unk_02A4 = 0;
    MM_Animation_Change(&this->skelAnime, &object_bsb_Anim_000C50, -1.0f, this->skelAnime.curFrame - 1.0f, 0.0f,
                     ANIMMODE_ONCE, 0.0f);
    this->unk_0294 = 10;
    Actor_PlaySfx(&this->actor, NA_SE_EN_KTIA_PAUSE_K);
    this->unk_02B4 = 7;
...
void func_80C0D00C(EnBsb* this) {
    ...
    if (this->actor.world.pos.z < -1300.0f) {
        Matrix_RotateYS((s32)MM_Rand_CenteredFloat(0x4000) + this->actor.yawTowardsPlayer, MTXMODE_NEW);
    ...
    MM_Math_Vec3f_Copy(&this->unk_0298, &vec);
    Actor_PlaySfx(&this->actor, NA_SE_EN_KITA_DAMAGE);
...
void func_80C0D3C0(EnBsb* this, PlayState* play) {
    ...
    this->subCamEye.x = this->subCamEyeNext.x = this->unk_02E0.x + MM_Math_SinS(this->actor.yawTowardsPlayer) * 300.0f;
    this->subCamEye.y = this->subCamEyeNext.y = this->unk_02E0.y - 30.0f;
    this->subCamEye.z = this->subCamEyeNext.z = this->unk_02E0.z + MM_Math_CosS(this->actor.yawTowardsPlayer) * 300.0f;

    this->subCamAt.x = this->subCamAtNext.x = this->unk_02E0.x + MM_Math_SinS(this->actor.yawTowardsPlayer) * 10.0f;
    this->subCamAt.y = this->subCamAtNext.y = this->unk_02E0.y - 10.0f;
    this->subCamAt.z = this->subCamAtNext.z = this->unk_02E0.z + MM_Math_CosS(this->actor.yawTowardsPlayer) * 10.0f;
...
void func_80C0D51C(EnBsb* this, PlayState* play) {
    ...
    if (this->unk_02A4 == 0) {
        ...
        MM_Player_SetCsActionWithHaltedActors(play, &this->actor, PLAYER_CSACTION_81);
        this->subCamId = CutsceneManager_GetCurrentSubCamId(this->actor.csId);
        this->unk_02A4 = 1;
    }

    if (this->unk_02A8 == 2) {
        f32 sin = MM_Math_SinS(this->actor.world.rot.y) * 380.f;
        f32 cos = MM_Math_CosS(this->actor.world.rot.y) * 380.f;
        ...
        this->subCamAtNext.x = MM_Math_SinS(this->actor.world.rot.y) * 10.0f + this->unk_02E0.x;
        this->subCamAtNext.y = this->unk_02E0.y - 63.0f;
        this->subCamAtNext.z = MM_Math_CosS(this->actor.world.rot.y) * 10.0f + this->unk_02E0.z;
    } else {
        this->subCamAtNext.x = MM_Math_SinS(this->actor.world.rot.y) * 10.0f + this->unk_02E0.x;
    ...
    if (this->unk_02A8 == 0) {
        ...
        MM_Math_SmoothStepToS(&this->unk_02AC, 0x1000, 1, 0x1F4, 0);
        ...
        MM_Math_SmoothStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 1, 0x3E8, 0);
        yaw = ABS_ALT((s16)(this->actor.yawTowardsPlayer - this->actor.world.rot.y));
    }
    ...
    if ((this->unk_02A8 != 0) && (yaw < 0x100) && (this->actor.world.pos.y < (this->actor.floorHeight + 30.0f))) {
        ...
        if (this->animIndex < ENBSB_ANIM_9) {
            ...
            MM_Actor_SpawnFloorDustRing(play, &this->actor, &this->actor.world.pos, this->actor.shape.shadowScale - 20.0f,
...
void func_80C0DB18(EnBsb* this, PlayState* play) {
    ...
    switch (this->unk_1118) {
        case 0:
            MM_Math_SmoothStepToS(&this->actor.world.rot.y, MM_Math_Vec3f_Yaw(&this->actor.world.pos, &sp64), 1, 0x7D0, 0);

            sin = MM_Math_SinS(this->actor.world.rot.y);
            MM_Math_ApproachF(&this->actor.world.pos.x, sp64.x, 0.5f, fabsf(sin * 3.2f));
        ...
        case 1:
            ...
            MM_Math_SmoothStepToS(&this->actor.world.rot.y, MM_Math_Vec3f_Yaw(&this->actor.world.pos, &this->unk_02EC), 1,
                               0x7D0, 0);
        ...
        case 2:
            yaw = MM_Math_Vec3f_Yaw(&this->actor.world.pos, &this->subCamEye);
            MM_Math_SmoothStepToS(&this->actor.world.rot.y, yaw, 1, 0x7D0, 0);

            if (ABS_ALT((s16)(this->actor.world.rot.y - yaw)) < 0x100) {
                Actor_PlaySfx(&this->actor, NA_SE_EN_KITA_LAUGH_K);
                EnBsb_ChangeAnim(this, ENBSB_ANIM_21);
                this->unk_1118++;
                MM_Message_StartTextbox(play, 0x1541, &this->actor);
            }
            break;
        ...
        case 4:
            sp64.x = -100.0f;
            yaw = MM_Math_Vec3f_Yaw(&this->actor.world.pos, &sp64);
            MM_Math_SmoothStepToS(&this->actor.world.rot.y, yaw, 1, 0x7D0, 0);
        ...
        case 6:
            ...
            if ((play->gameplayFrames % 8) == 0) {
                MM_Math_Vec3f_Copy(&sp64, &this->actor.world.pos);
                sp64.x += MM_Rand_CenteredFloat(40.0f);
                sp64.z += MM_Rand_CenteredFloat(40.0f);
                MM_Actor_SpawnFloorDustRing(play, &this->actor, &sp64, MM_Rand_ZeroFloat(10.0f) + 5.0f, MM_Rand_S16Offset(1, 3),
                                         4.0f, 1000, 100, true);
            }

            MM_Math_ApproachF(&this->actor.shape.yOffset, -7000.0f, 0.5f, 200.0f);
...
void func_80C0E1C0(EnBsb* this, PlayState* play) {
    ...
    if (this->animIndex == ENBSB_ANIM_16) {
        if (MM_Animation_OnFrame(&this->skelAnime, 30.0f)) {
    ...
    if (this->unk_02A4 == 0) {
        if (CHECK_WEEKEVENTREG(WEEKEVENTREG_23_04)) {
            ...
            for (i = 0; i < ENBSB_BODYPART_MAX; i++) {
                EnBsb* newActor = (EnBsb*)MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_BSB, this->actor.world.pos.x,
...
void func_80C0E3B8(EnBsb* this) {
    ...
    MM_Animation_Change(&this->skelAnime, &object_bsb_Anim_004894, 1.0f, D_80C0F8D0,
                     MM_Animation_GetLastFrame(&object_bsb_Anim_004894), 2, 0.0f);
...
void func_80C0E4FC(EnBsb* this, PlayState* play) {
    if (this->unk_0294 != 0) {
        ...
        MM_Math_ApproachF(&this->actor.gravity, -3.0f, 0.5f, 0.5f);
        MM_Math_ApproachF(&this->actor.velocity.y, -10.0f, 0.5f, 0.5f);
            ...
            if (this->actor.colChkInfo.health <= 0) {
                ...
                for (i = 0; i < ARRAY_COUNT(this->colliderElements); i++) {
                    if (this->collider.elements[i].base.acElemFlags & ACELEM_HIT) {
                        ...
                        MM_CollisionCheck_BlueBlood(play, NULL, &sp48);
                    }
                }
                func_80C0D00C(this);
            }
    ...
    if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_532)) {
        ...
        if (this->cueId != play->csCtx.actorCues[cueChannel]->id) {
            ...
            switch (play->csCtx.actorCues[cueChannel]->id) {
                ...
                case 6:
                    MM_Flags_SetSwitch(play, this->switchFlag2);
...
void EnBsb_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->unk_02B0 == OBJECT_BSB_LIMB_NONE) {
        ...
        if ((!func_80C0E9CC(this, play)) && (this->unk_02B4 != 14)) {
            ...
            this->actor.focus.pos.x = this->unk_02E0.x + MM_Math_SinS(this->actor.world.rot.y) * 20.0f;
            this->actor.focus.pos.y = this->unk_02E0.y;
            this->actor.focus.pos.z = this->unk_02E0.z + MM_Math_CosS(this->actor.world.rot.y) * 20.0f;
            ...
            if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
                ...
                MM_Math_ApproachZeroF(&this->unk_0298.x, 1.0f, 2.0f);
                MM_Math_ApproachZeroF(&this->unk_0298.z, 1.0f, 2.0f);
            }
        } else {
            f32 curFrame = this->skelAnime.curFrame;
        ...
        MM_Actor_UpdateBgCheckInfo(play, &this->actor, 30.0f, 60.0f, 40.0f,
...
void EnBsb_PostLimbDrawOpa(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (this->unk_02B0 == OBJECT_BSB_LIMB_NONE) {
        if (limbIndex == OBJECT_BSB_LIMB_0A) {
            MM_Matrix_MultVec3f(&gZeroVec3f, &this->unk_02E0);
        }
        if (limbIndex == OBJECT_BSB_LIMB_11) {
            MM_Matrix_MultVec3f(&gZeroVec3f, &this->unk_0304);
        }
        if (limbIndex == OBJECT_BSB_LIMB_14) {
            MM_Matrix_MultVec3f(&gZeroVec3f, &this->unk_02F8);
        }
...
void EnBsb_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->alpha == 255) {
        ...
        MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, EnBsb_OverrideLimbDrawOpa,
