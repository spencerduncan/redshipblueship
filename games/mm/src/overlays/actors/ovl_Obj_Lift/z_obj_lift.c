...
void MM_ObjLift_Init(Actor* thisx, PlayState* play);
void MM_ObjLift_Destroy(Actor* thisx, PlayState* play);
void MM_ObjLift_Update(Actor* thisx, PlayState* play);
void MM_ObjLift_Draw(Actor* thisx, PlayState* play);
...
void func_8093D3C0(ObjLift* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(D_8093DD60); i++) {
        ...
        velocity.y = (MM_Rand_ZeroOne() * 10.0f) + 6.0f;
        ...
        MM_EffectSsKakera_Spawn(play, &pos, &velocity, actorPos, -0x100, rand, 15, 15, 0,
                             ((MM_Rand_ZeroOne() * 50.0f) + 50.0f) * this->dyna.actor.scale.x, 0, 32, 50, -1,
...
void MM_ObjLift_Init(Actor* thisx, PlayState* play) {
    ...
    if ((this->unk_178 <= 0) && MM_Flags_GetSwitch(play, OBJLIFT_GET_SWITCH_FLAG(&this->dyna.actor))) {
        MM_Actor_Kill(&this->dyna.actor);
        return;
    }

    DynaPolyActor_LoadMesh(play, &this->dyna, &gDampeGraveBrownElevatorCol);
    this->unk_160 = MM_Rand_Next() >> 0x10;
    this->unk_162 = MM_Rand_Next() >> 0x10;
    this->unk_164 = MM_Rand_Next() >> 0x10;
    func_8093D760(this);
}
...
void func_8093D760(ObjLift* this) {
    ...
    this->dyna.actor.draw = MM_ObjLift_Draw;
}
    ...
    if (MM_DynaPolyActor_IsPlayerOnTop(&this->dyna)) {
        if (this->timer <= 0) {
            if (OBJLIFT_GET_7(&this->dyna.actor) == 7) {
                ...
                MM_Quake_SetSpeed(quakeIndex, 10000);
...
void func_8093D8B4(ObjLift* this, PlayState* play) {
    if (this->timer <= 0) {
        ...
        this->unk_160 += 0x2710;
        this->unk_174 = MM_Math_SinS(this->unk_160) * 300.0f;
        this->unk_176 = MM_Math_CosS(this->unk_160) * 300.0f;
        this->unk_162 += 0x4650;
        this->unk_168.y = MM_Math_SinS(this->unk_162);
        this->unk_164 += 0x4650;
        this->unk_168.x = MM_Math_SinS(this->unk_164) * 3.0f;
        this->unk_168.z = MM_Math_CosS(this->unk_164) * 3.0f;
    }
    if ((this->timer & 3) == 3) {
        MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->dyna.actor.world.pos, 16, NA_SE_EV_BLOCK_SHAKE);
    }
}
...
void func_8093DA48(ObjLift* this, PlayState* play) {
    ...
    MM_Math_Vec3f_Copy(&pos, &this->dyna.actor.prevPos);
    pos.y += yOffsets[OBJLIFT_GET_1(&this->dyna.actor)];
    this->dyna.actor.floorHeight =
        MM_BgCheck_EntityRaycastFloor5(&play->colCtx, &this->dyna.actor.floorPoly, &bgId, &this->dyna.actor, &pos);
    if ((yOffsets[OBJLIFT_GET_1(&this->dyna.actor)] - 0.001f) <=
        ...
        MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->dyna.actor.world.pos, 20, NA_SE_EV_BOX_BREAK);
        if (this->unk_178 > 0) {
            func_8093DB70(this);
            DynaPoly_DisableCollision(play, &play->colCtx.dyna, this->dyna.bgId);
        } else {
            MM_Flags_SetSwitch(play, OBJLIFT_GET_SWITCH_FLAG(&this->dyna.actor));
            MM_Actor_Kill(&this->dyna.actor);
        }
    }
}
...
void MM_ObjLift_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_Gfx_DrawDListOpa(play, gDampeGraveBrownElevatorDL);
}
...
void func_8093DC90(Actor* thisx, PlayState* play) {
    ...
    MM_Math_Vec3f_Sum(&this->dyna.actor.world.pos, &this->unk_168, &pos);
    ...
    MM_Matrix_SetTranslateRotateYXZ(pos.x, pos.y, pos.z, &rot);
