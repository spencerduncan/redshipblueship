...
static AnimationInfoS MM_sAnimationInfo[ENAND_ANIM_MAX] = {
...
void EnAnd_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &gAndSkel, NULL, this->jointTable, this->morphTable,
                       OBJECT_AND_LIMB_MAX);
...
void EnAnd_Draw(Actor* thisx, PlayState* play) {
    static TexturePtr MM_sMouthTextures[] = { gAndMouthNeutralTex, gAndMouthSmileTex };
    static TexturePtr MM_sEyeTextures[EYE_TEXTURES_COUNT] = {
        gAndEyeOpenTex,
        gAndEyeClosingTex,
        gAndEyeClosedTex,
        gAndEyeOpeningTex,
    ...
    MM_gSPSegment(POLY_OPA_DISP++, 0x09, Lib_SegmentedToVirtual(MM_sMouthTextures[0]));
