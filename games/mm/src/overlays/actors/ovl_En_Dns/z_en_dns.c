...
void MM_EnDns_Init(Actor* thisx, PlayState* play);
void MM_EnDns_Destroy(Actor* thisx, PlayState* play);
void MM_EnDns_Update(Actor* thisx, PlayState* play);
void MM_EnDns_Draw(Actor* thisx, PlayState* play);
...
static AnimationInfoS MM_sAnimationInfo[EN_DNS_ANIM_MAX] = {
...
void func_8092C5C0(EnDns* this) {
    ...
    if (((this->animIndex == EN_DNS_ANIM_WALK) || (this->animIndex == EN_DNS_ANIM_WALK_MORPH) ||
        ...
        (MM_Animation_OnFrame(&this->skelAnime, 0.0f) || MM_Animation_OnFrame(&this->skelAnime, 3.0f))) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_NUTS_WALK);
    }
}

s32 MM_EnDns_ChangeAnim(EnDns* this, s32 animIndex) {
...
void func_8092C740(EnDns* this, PlayState* play) {
    ...
    MM_Math_ApproachS(&this->unk_2CE, temp, 4, 0x2AA8);
    this->unk_2CE = CLAMP(this->unk_2CE, -0x3FFC, 0x3FFC);

    MM_Math_Vec3f_Copy(&sp28, &player->actor.world.pos);
    sp28.y = player->bodyPartsPos[PLAYER_BODYPART_HEAD].y + 3.0f;
    MM_Math_Vec3f_Copy(&sp34, &this->actor.world.pos);
    sp34.y += 10.0f;
    temp = MM_Math_Vec3f_Pitch(&sp34, &sp28);
    MM_Math_ApproachS(&this->unk_2CC, temp, 4, 0x2AA8);
    ...
    if (((this->unk_2C6 & SUBS_OFFER_MODE_MASK) != SUBS_OFFER_MODE_NONE) &&
        ...
        if (ENDNS_GET_4000(&this->actor)) {
            ...
            if (this->unk_2D2 != 0) {
                ...
                MM_EnDns_ChangeAnim(this, EN_DNS_ANIM_WALK);
            }
...
s32 func_8092CC68(PlayState* play) {
    ...
    if (!MM_Play_InCsMode(play) && (player->actor.bgCheckFlags & BGCHECKFLAG_GROUND) &&
        (player->transformation != PLAYER_FORM_DEKU)) {
        bgId = player->actor.floorBgId;
        if (MM_SurfaceType_GetSceneExitIndex(&play->colCtx, player->actor.floorPoly, bgId) != 4) {
            ret = true;
        }
    }
...
s32 func_8092CCEC(Actor* thisx, PlayState* play) {
    ...
    this->unk_2D6 = MM_Math_Vec3f_Yaw(&gZeroVec3f, &sp3C);
    this->unk_2D4 = MM_Math_Vec3f_Yaw(&gZeroVec3f, &sp30);
    this->unk_2EC = MM_Math_Vec3f_DistXZ(&sp30, &gZeroVec3f);
    sp2E = MM_Math_Vec3f_Yaw(&gZeroVec3f, &sp3C);
    sp2E -= MM_Math_Vec3f_Yaw(&gZeroVec3f, &sp30);
    this->unk_2D8 = (MM_Rand_ZeroOne() * 182.0f) + 182.0f;
...
s32 func_8092CE38(EnDns* this) {
    ...
    if ((this->unk_2C6 & 0x200) || MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
        MM_EnDns_ChangeAnim(this, D_8092DE00[this->unk_2D2]);
        this->unk_2C6 &= ~0x200;
        ...
        if (this->unk_2D2 == 0) {
            ...
            if (MM_Animation_OnFrame(&this->skelAnime, 0.0f) || MM_Animation_OnFrame(&this->skelAnime, 6.0f) ||
                MM_Animation_OnFrame(&this->skelAnime, 13.0f)) {
                Actor_PlaySfx(&this->actor, NA_SE_EN_NUTS_WALK);
            }
...
void func_8092D1B8(EnDns* this, PlayState* play) {
    ...
    if (!ENDNS_GET_4000(&this->actor) || (this->unk_2D2 != 0)) {
        ...
        MM_Math_ApproachS(&this->actor.shape.rot.y, sp22, 3, 0x2AA8);
    }
}
...
void func_8092D330(EnDns* this, PlayState* play) {
    ...
    if (ABS_ALT(temp) < 0xC16) {
        MM_Math_ApproachS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 3, 0x2AA8);
        this->actor.world.rot.y = this->actor.shape.rot.y;
    } else {
        this->actor.world.pos.x = MM_Math_SinS(this->unk_2D4) * this->unk_2EC;
        this->actor.world.pos.z = MM_Math_CosS(this->unk_2D4) * this->unk_2EC;
        this->unk_2D4 += this->unk_2D8;
        sp30.x = MM_Math_SinS(this->unk_2D4) * this->unk_2EC;
        sp30.z = MM_Math_CosS(this->unk_2D4) * this->unk_2EC;
        MM_Math_ApproachS(&this->actor.shape.rot.y, MM_Math_Vec3f_Yaw(&this->actor.world.pos, &sp30), 3, 0x2AA8);
    ...
    if (ENDNS_GET_4000(&this->actor) && (this->unk_2D2 == 0)) {
        ...
        if (ENDNS_GET_4000(&this->actor)) {
            if (!CHECK_EVENTINF(EVENTINF_15)) {
                ...
                MM_EnDns_ChangeAnim(this, EN_DNS_ANIM_DANCE);
            }
            this->unk_2CC = 0;
            this->unk_2CE = 0;
            this->unk_2C6 |= 0x10;
        ...
        MM_Math_ApproachS(&this->actor.shape.rot.y, sp2E, 3, 0x2AA8);
    }
}
...
void EnDns_HandleCutscene(EnDns* this, PlayState* play) {
    ...
    if (Cutscene_IsCueInChannel(play, this->cueType)) {
        ...
        if (this->cueId != (u8)cueId) {
            MM_EnDns_ChangeAnim(this, sCsAnimIndex[cueId]);
            this->cueId = cueId;
        }
        ...
        if (((this->animIndex == EN_DNS_ANIM_SURPRISE_START) || (this->animIndex == EN_DNS_ANIM_RUN_START)) &&
            ...
            MM_EnDns_ChangeAnim(this, this->animIndex + 1);
        }
...
void MM_EnDns_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_Init(play, &this->skelAnime, &gKingsChamberDekuGuardSkel, NULL, this->jointTable, this->morphTable,
...
void EnDns_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (limbIndex == KINGS_CHAMBER_DEKU_GUARD_LIMB_HEAD) {
        func_8092D954(this->unk_2CC, this->unk_2CE + this->actor.shape.rot.y, &this->unk_218, &this->unk_224, phi_v1,
                      phi_v0);
        MM_Matrix_Translate(this->unk_218.x, this->unk_218.y, this->unk_218.z, MTXMODE_NEW);
        MM_Matrix_Scale(this->actor.scale.x, this->actor.scale.y, this->actor.scale.z, MTXMODE_APPLY);
