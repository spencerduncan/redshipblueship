...
void BgDblueElevator_CheckWaterBoxInfo(BgDblueElevator* this, PlayState* play2) {
    ...
    this->isWithinWaterBoxXZ =
        MM_WaterBox_GetSurfaceImpl(play, &play->colCtx, this->dyna.actor.world.pos.x, this->dyna.actor.world.pos.z,
...
void BgDblueElevator_SpawnRipplesAndSplashes(BgDblueElevator* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(D_80B929D0) - 1; i++) {
        ...
        for (j = 0; j < 7; j++) {
            ...
            ripplePos.x += (MM_Rand_ZeroOne() - 0.5f) * 20.0f;
            ripplePos.z += (MM_Rand_ZeroOne() - 0.5f) * 20.0f;
            MM_Matrix_MultVec3f(&ripplePos, &splashPos);
            splashPos.x += this->dyna.actor.world.pos.x;
            splashPos.z += this->dyna.actor.world.pos.z;
            MM_EffectSsGSplash_Spawn(play, &splashPos, NULL, NULL, 0, (MM_Rand_ZeroOne() * 400.0f) + 210.0f);
        }
    }

    for (i = 0; i < ARRAY_COUNT(sLargeRipplesLives); i++) {
        ripplePos.x = ((MM_Rand_ZeroOne() - 0.5f) * 60.0f) + this->dyna.actor.world.pos.x;
        ripplePos.y = this->waterSurfacePosY;
        ripplePos.z = ((MM_Rand_ZeroOne() - 0.5f) * 60.0f) + this->dyna.actor.world.pos.z;
        MM_EffectSsGRipple_Spawn(play, &ripplePos, 1000, 3000, sLargeRipplesLives[i]);
    }

    for (i = 0; i < ARRAY_COUNT(sSmallRipplesLives); i++) {
        var_fs0 = MM_Rand_ZeroOne();
        ...
        MM_EffectSsGRipple_Spawn(play, &ripplePos, 400, 800, sSmallRipplesLives[i]);
    }
    MM_Matrix_Pop();
}
...
s32 BgDblueElevator_GetWaterFlowFromCeiling(BgDblueElevator* this, PlayState* play) {
    ...
    if (MM_Flags_GetSwitch(play, BG_DBLUE_ELEVATOR_GET_SWITCH_FLAG(thisx, 0))) {
...
s32 BgDblueElevator_GetWaterFlowFromPipes(BgDblueElevator* this, PlayState* play) {
    ...
    if (MM_Flags_GetSwitch(play, BG_DBLUE_ELEVATOR_GET_SWITCH_FLAG(thisx, 1)) &&
        MM_Flags_GetSwitch(play, BG_DBLUE_ELEVATOR_GET_SWITCH_FLAG(thisx, 2)) &&
        MM_Flags_GetSwitch(play, BG_DBLUE_ELEVATOR_GET_SWITCH_FLAG(thisx, 3))) {
...
void BgDblueElevator_Move(BgDblueElevator* this, PlayState* play) {
    ...
    if ((waterFlow == BG_DBLUE_ELEVATOR_WATER_FLOW_STOPPED) ||
        (waterFlow == BG_DBLUE_ELEVATOR_WATER_FLOW_BOTH_DIRECTIONS)) {
        isDeactivated = MM_Math_StepToF(&this->posStep, 0.0f, ptr->decelerationStep);
    } else {
        isDeactivated = false;
        MM_Math_StepToF(&this->posStep, ptr->targetPosStep, ptr->accelerationStep);
    }
    ...
    if (MM_Math_SmoothStepToF(&this->posOffset, targetPosOffset, 0.4f, posStep, 1.0f) < 0.001f) {
        hasReachedEnd = true;
    } else {
        hasReachedEnd = false;
    }
...
void BgDblueElevator_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_Gfx_DrawDListOpa(play, gGreatBayTempleObjectElevatorDL);
}
