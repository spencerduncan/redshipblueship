...
void func_809CE068(BgSpdweb* this) {
    ...
    for (i = 0; i < this->collider.count; i++) {
        for (j = 0; j < ARRAY_COUNT(sp64); j++) {
            MM_Matrix_MultVec3f(this->collider.elements[i].dim.vtx + j, &sp64[j]);
        }
        MM_Collider_SetTrisVertices(&this->collider, i, &sp64[0], &sp64[1], &sp64[2]);
    }
}
...
void func_809CE234(BgSpdweb* this, PlayState* play) {
    ...
    if ((this->unk_162 % 3) == 0) {
        phi_s2 = MM_Rand_ZeroOne() * 10922.0f;
        spB4.y = 0.0f;
        spA8.y = this->dyna.actor.world.pos.y;

        for (i = 0; i < 6; i++) {
            temp_s0 = (s32)MM_Rand_CenteredFloat(0x2800) + phi_s2;
            temp_f24 = MM_Math_SinS(temp_s0);
            ...
            temp_f20 = MM_Math_Vec3f_DistXZ(&this->dyna.actor.home.pos, &spA8) * (1.0f / 120.0f);
            if (temp_f20 < 0.7f) {
                temp_f20 = 1.0f - temp_f20;
                temp_f24 = MM_Math_SinS(BINANG_ROT180(temp_s0));
                temp_f22 = MM_Math_CosS(BINANG_ROT180(temp_s0));
            }
            ...
            MM_EffectSsDeadDb_Spawn(play, &this->dyna.actor.home.pos, &spB4, &gZeroVec3f, &D_809CF208, &D_809CF20C, 0x32,
                                 8, 0xE);
            phi_s2 += 0x2AAA;
        }

        MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->dyna.actor.home.pos, 11, NA_SE_EN_EXTINCT);
    }
}
...
void func_809CE4C8(BgSpdweb* this, PlayState* play) {
    ...
    if (MM_Player_IsBurningStickInRange(play, &sp40, 70.0f, 50.0f)) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (trisElem->base.acElemFlags & ACELEM_HIT) {
                if (this->collider.elements[i].base.acHitElem->atDmgInfo.dmgFlags & 0x800) {
                    MM_Math_Vec3s_ToVec3f(&this->dyna.actor.home.pos, &trisElem->base.acDmgInfo.hitPos);
                    func_809CEE74(this);
                    return;
                }
    ...
    this->dyna.actor.world.pos.y =
        (MM_Math_SinF(this->unk_162 * (M_PIf / 6)) * this->unk_164) + this->dyna.actor.home.pos.y;
    MM_Math_ApproachZeroF(&this->unk_164, 1.0f, 0.8f);

    if (this->unk_162 == 4) {
        if ((this->unk_161 != 0) || (MM_DynaPolyActor_IsPlayerOnTop(&this->dyna) && (this->unk_164 > 2.0f))) {
...
void func_809CE830(BgSpdweb* this, PlayState* play) {
    ...
    if ((this->unk_162 % 3) == 0) {
        temp_s3 = MM_Rand_ZeroOne() * 0x2AAA;
        cosQ = MM_Math_CosS(this->dyna.actor.shape.rot.x);
        sinQ = MM_Math_SinS(this->dyna.actor.shape.rot.x);
        cosF1 = MM_Math_CosS(this->dyna.actor.shape.rot.y);
        sinF1 = MM_Math_SinS(this->dyna.actor.shape.rot.y);
        ...
        for (i = 0; i < 6; i++) {
            temp_s0 = (s32)MM_Rand_CenteredFloat(0x2800) + temp_s3;
            sinF2 = MM_Math_SinS(temp_s0);
            cosF2 = MM_Math_CosS(temp_s0);
            ...
            temp_f28 = MM_Math_Vec3f_DistXYZ(&this->dyna.actor.home.pos, &spD0) * (1.0f / 90.0f);
            if (temp_f28 < 0.65f) {
                temp_f28 = 1.0f - temp_f28;
                sinF2 = MM_Math_SinS(BINANG_ROT180(temp_s0));
                cosF2 = MM_Math_CosS(BINANG_ROT180(temp_s0));
            }
            ...
            MM_EffectSsDeadDb_Spawn(play, &this->dyna.actor.home.pos, &spDC, &gZeroVec3f, &D_809CF208, &D_809CF20C, 0x3C,
                                 8, 0xE);
            temp_s3 += 0x2AAA;
        }
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if (i == ARRAY_COUNT(this->colliderElements)) {
            this->dyna.actor.home.pos.x =
                ((90.0f * MM_Math_SinS(this->dyna.actor.shape.rot.x)) * MM_Math_SinS(this->dyna.actor.shape.rot.y)) +
                this->dyna.actor.world.pos.x;
            this->dyna.actor.home.pos.y =
                (90.0f * MM_Math_CosS(this->dyna.actor.shape.rot.x)) + this->dyna.actor.world.pos.y;
            this->dyna.actor.home.pos.z =
                ((90.0f * MM_Math_SinS(this->dyna.actor.shape.rot.x)) * MM_Math_CosS(this->dyna.actor.shape.rot.y)) +
                this->dyna.actor.world.pos.z;
        }
        func_809CEE74(this);
    } else if ((player->heldItemAction == PLAYER_IA_DEKU_STICK) && (player->unk_B28 != 0)) {
        MM_Math_Vec3f_Diff(&player->meleeWeaponInfo[0].tip, &this->dyna.actor.world.pos, &sp3C);
        sp38 = MM_Math_SinS(-this->dyna.actor.shape.rot.x);
        sp34 = MM_Math_CosS(-this->dyna.actor.shape.rot.x);
        sp30 = MM_Math_SinS(-this->dyna.actor.shape.rot.y);
        sp2C = MM_Math_CosS(-this->dyna.actor.shape.rot.y);
        ...
        if ((fabsf(temp_f10) < 70.0f) && (fabsf(sp58) < 10.0f) && (temp_f18 < 160.0f) && (temp_f18 > 20.0f)) {
            MM_Math_Vec3f_Copy(&this->dyna.actor.home.pos, &player->meleeWeaponInfo[0].tip);
            func_809CEE74(this);
        }
    }
...
void BgSpdweb_Draw(Actor* thisx, PlayState* play) {
    ...
    if (thisx->params == BGSPDWEB_FF_1) {
        ...
        MM_gSPDisplayList(&gfx[2], object_spdweb_DL_000060);
    } else {
        MM_Matrix_Translate(0.0f, (thisx->home.pos.y - thisx->world.pos.y) * 10.0f, 0.0f, MTXMODE_APPLY);
        MM_Matrix_Scale(1.0f, ((thisx->home.pos.y - thisx->world.pos.y) + 10.0f) * 0.1f, 1.0f, MTXMODE_APPLY);

        MATRIX_FINALIZE_AND_LOAD(&gfx[1], play->state.gfxCtx);
        MM_gSPDisplayList(&gfx[2], object_spdweb_DL_0012F0);
    }
