...
void MM_EnWf_Init(Actor* thisx, PlayState* play);
void MM_EnWf_Destroy(Actor* thisx, PlayState* play);
void MM_EnWf_Update(Actor* thisx, PlayState* play);
void MM_EnWf_Draw(Actor* thisx, PlayState* play);
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 8, 50, 100, MASS_HEAVY };
...
void MM_EnWf_Init(Actor* thisx, PlayState* play) {
    ...
    MM_ActorShape_Init(&this->actor.shape, 0.0f, MM_ActorShadow_DrawCircle, 70.0f);
    this->actor.shape.shadowAlpha = 150;
    Collider_InitAndSetJntSph(play, &this->collider1, &this->actor, &MM_sJntSphInit, this->collider1Elements);
    Collider_InitAndSetCylinder(play, &this->collider2, &this->actor, &MM_sCylinderInit1);
    Collider_InitAndSetCylinder(play, &this->collider3, &this->actor, &MM_sCylinderInit2);
    ...
    if (this->unk_295) {
        ...
        this->actor.child = MM_Actor_Spawn(&play->actorCtx, play, ACTOR_OBJ_ICE_POLY, this->actor.world.pos.x,
                                        this->actor.world.pos.y, this->actor.world.pos.z, 0, this->actor.shape.rot.y, 0,
                                        OBJICEPOLY_PARAMS_ALT(temp_s0, OBJICEPOLY_SWITCH_FLAG_NONE));
        if (this->actor.child != NULL) {
            Player* player = GET_PLAYER(play);

            this->actor.child->xzDistToPlayer = MM_Actor_WorldDistXZToActor(&this->actor, &player->actor);
            this->unk_2A4 = (temp_s0 * 0.5f) + 45.0f;
            this->unk_29C = 0x32000 / this->unk_2A4;
            this->actor.shape.rot.y = MM_Actor_WorldYawTowardActor(&this->actor, &player->actor);
            this->actor.world.pos.x -= this->unk_2A4 * MM_Math_SinS(this->actor.shape.rot.y);
            this->actor.world.pos.z -= this->unk_2A4 * MM_Math_CosS(this->actor.shape.rot.y);
        }
    }
    ...
    if (this->actor.params == 0) {
        MM_SkelAnime_InitFlex(play, &this->skelAnime, &gWolfosNormalSkel, &gWolfosWaitAnim, this->jointTable,
                           this->morphTable, WOLFOS_NORMAL_LIMB_MAX);
        this->actor.hintId = TATL_HINT_ID_WOLFOS;
        MM_CollisionCheck_SetInfo(&this->actor.colChkInfo, &sDamageTable2, &MM_sColChkInfoInit);
        this->collider1.elements[0].base.atDmgInfo.damage = 8;
        this->collider1.elements[1].base.atDmgInfo.damage = 8;
        this->actor.colChkInfo.health = 6;
    } else {
        MM_SkelAnime_InitFlex(play, &this->skelAnime, &gWolfosWhiteSkel, &gWolfosWaitAnim, this->jointTable,
                           this->morphTable, WOLFOS_WHITE_LIMB_MAX);
        this->actor.hintId = TATL_HINT_ID_WHITE_WOLFOS;
        MM_CollisionCheck_SetInfo(&this->actor.colChkInfo, &sDamageTable1, &MM_sColChkInfoInit);
    }
    ...
    this->unk_2A2 = MM_Rand_ZeroFloat(96.0f);
}

void MM_EnWf_Destroy(Actor* thisx, PlayState* play) {
    EnWf* this = (EnWf*)thisx;

    MM_Collider_DestroyJntSph(play, &this->collider1);
    MM_Collider_DestroyCylinder(play, &this->collider2);
    MM_Collider_DestroyCylinder(play, &this->collider3);
}
...
s32 func_80990948(PlayState* play, EnWf* this, s16 arg2) {
    ...
    if ((!this->unk_295 ||
        ...
        (this->unk_2A8 < MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos))) {
        func_80993524(this);
        return true;
    }
    ...
    if (arg2) {
        if (!MM_Actor_IsFacingPlayer(&this->actor, 7000)) {
            func_80992A74(this, play);
            return true;
        }

        sp2E = player->actor.shape.rot.y - this->actor.shape.rot.y;
        if ((this->actor.xzDistToPlayer <= 80.0f) && !MM_Actor_OtherIsTargeted(play, &this->actor)) {
...
void func_80990C6C(EnWf* this, PlayState* play, s32 arg2) {
    static Vec3f MM_sAccel = { 0.0f, 0.1f, 0.0f };
    ...
    if (this->actor.floorPoly != NULL) {
        ...
        if (floorType != FLOOR_TYPE_5) {
            if ((floorType == FLOOR_TYPE_15) || (floorType == FLOOR_TYPE_14)) {
                phi_s1 = &D_809942EC;
                phi_s6 = MM_Rand_ZeroFloat(150.0f) + 350.0f;
                arg2 += 2;
            } else {
                phi_s1 = &D_809942E8;
                phi_s6 = MM_Rand_ZeroFloat(100.0f) + 250.0f;
            }

            for (i = 0; i < arg2; i++) {
                pos.x = MM_Rand_CenteredFloat(50.0f) + this->actor.world.pos.x;
                pos.y = MM_Rand_ZeroFloat(5.0f) + this->actor.world.pos.y;
                pos.z = MM_Rand_CenteredFloat(50.0f) + this->actor.world.pos.z;
                func_800B0F18(play, &pos, &gZeroVec3f, &MM_sAccel, phi_s1, phi_s1, phi_s6, 5,
...
void func_80990E4C(EnWf* this, PlayState* play) {
    if (MM_Animation_OnFrame(&this->skelAnime, 1.0f) && (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
...
void func_80990FC8(EnWf* this) {
    MM_Animation_Change(&this->skelAnime, &gWolfosRearUpFallOverAnim, 0.5f, 0.0f, 7.0f, ANIMMODE_ONCE_INTERP, 0.0f);
    this->unk_2A0 = 5;
    this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED;
...
void func_80991040(EnWf* this, PlayState* play) {
    if (this->unk_2A0 != 0) {
        ...
        MM_Math_StepToF(&this->actor.shape.shadowScale, 70.0f, 14.0f);
        this->unk_2A0--;
        if (this->unk_2A0 == 0) {
            Actor_PlaySfx(&this->actor, NA_SE_EN_WOLFOS_APPEAR);
        }
...
void func_809910F0(EnWf* this) {
    ...
    MM_Animation_Change(&this->skelAnime, &gWolfosRearUpFallOverAnim, 0.5f, 0.0f, 7.0f, ANIMMODE_ONCE_INTERP, -5.0f);
    this->unk_2A0 = 5;
...
void func_80991174(EnWf* this, PlayState* play) {
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if (this->unk_2A0 > 0) {
            this->actor.scale.y -= this->actor.scale.x * 0.2f;
            MM_Math_StepToF(&this->actor.shape.shadowScale, 0.0f, 14.0f);
            this->unk_2A0--;
        } else {
            func_80990F0C(this);
        }
...
void func_80991200(EnWf* this) {
    this->collider2.base.acFlags |= AC_ON;
    MM_Animation_MorphToLoop(&this->skelAnime, &gWolfosWaitAnim, -4.0f);
    this->unk_2A0 = (s32)MM_Rand_ZeroFloat(10.0f) + 2;
...
void func_80991280(EnWf* this, PlayState* play) {
    ...
    if (!func_8099408C(play, this) && !func_80990948(play, this, false)) {
        ...
        if ((this->actor.xzDistToPlayer < 80.0f) && (player->meleeWeaponState != PLAYER_MELEE_WEAPON_STATE_0) &&
            ...
            this->unk_2A0--;
            if (this->unk_2A0 == 0) {
                if (MM_Actor_IsFacingPlayer(&this->actor, 0x1555)) {
...
void func_80991438(EnWf* this) {
    this->collider2.base.acFlags |= AC_ON;
    MM_Animation_MorphToLoop(&this->skelAnime, &gWolfosRunAnim, -4.0f);
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void func_8099149C(EnWf* this, PlayState* play) {
    ...
    if (!func_8099408C(play, this)) {
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x1F4);
        this->actor.world.rot.y = this->actor.shape.rot.y;
        if (MM_Actor_OtherIsTargeted(play, &this->actor)) {
            sp2C = 150.0f;
        } else {
            sp2C = 0.0f;
        }
        ...
        if (!func_80990948(play, this, false)) {
            if (!MM_Actor_IsFacingPlayer(&this->actor, 0x11C7)) {
                ...
                if (!MM_Actor_OtherIsTargeted(play, &this->actor) &&
                    ((MM_Rand_ZeroOne() > 0.03f) || ((this->actor.xzDistToPlayer <= 80.0f) && (sp28 < 0x38E0)))) {
                    func_80991C04(this);
                } else if (MM_Actor_OtherIsTargeted(play, &this->actor) && (MM_Rand_ZeroOne() > 0.5f)) {
                    func_8099223C(this);
                } else {
                    func_80991948(this);
                }
...
void func_80991738(EnWf* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gWolfosSidestepAnim, -4.0f);
...
void func_80991948(EnWf* this) {
    ...
    this->actor.world.rot.y = this->actor.shape.rot.y;
    this->unk_2A0 = (s32)MM_Rand_ZeroFloat(30.0f) + 30;
...
void func_809919F4(EnWf* this, PlayState* play) {
    ...
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer + this->unk_29A, 0x9C4);
    ...
    if (!func_8099408C(play, this) && !func_80990948(play, this, false)) {
        ...
        if ((MM_Math_CosS(sp26 - this->actor.shape.rot.y) < -0.85f) && !MM_Actor_OtherIsTargeted(play, &this->actor) &&
            (this->actor.xzDistToPlayer <= 80.0f)) {
            func_80991C04(this);
        } else {
            this->unk_2A0--;
            if (this->unk_2A0 == 0) {
                if (MM_Actor_OtherIsTargeted(play, &this->actor) && (MM_Rand_ZeroOne() > 0.5f)) {
                    func_8099223C(this);
                } else {
                    func_80991200(this);
                    this->unk_2A0 = (s32)MM_Rand_ZeroFloat(3.0f) + 1;
                }
            }
        }
    }
...
void func_80991C04(EnWf* this) {
    this->collider2.base.acFlags |= AC_ON;
    MM_Animation_PlayOnce(&this->skelAnime, &gWolfosSlashAnim);
...
void func_80991C80(EnWf* this, PlayState* play) {
    ...
    onAnim15thFrame = MM_Animation_OnFrame(&this->skelAnime, 15.0f);

    if ((onAnim15thFrame && !MM_Actor_IsTargeted(play, &this->actor) &&
         (!MM_Actor_IsFacingPlayer(&this->actor, 0x2000) || (this->actor.xzDistToPlayer >= 100.0f))) ||
        MM_SkelAnime_Update(&this->skelAnime)) {
        if (!onAnim15thFrame && (this->unk_2A0 != 0)) {
            ...
            this->unk_2A0--;
        } else if (!MM_Actor_IsFacingPlayer(&this->actor, 0x1554) && !onAnim15thFrame) {
            func_80991200(this);
            this->unk_2A0 = (s32)MM_Rand_ZeroFloat(5.0f) + 5;
            if (sp30 >= 0x32C9) {
                this->unk_298 = 7;
            }
        } else if ((MM_Rand_ZeroOne() > 0.7f) || (this->actor.xzDistToPlayer >= 120.0f)) {
            func_80991200(this);
            this->unk_2A0 = (s32)MM_Rand_ZeroFloat(5.0f) + 5;
        } else {
            this->actor.world.rot.y = this->actor.yawTowardsPlayer;
...
void func_80991FD8(EnWf* this) {
    ...
    MM_Animation_Change(&this->skelAnime, &gWolfosSlashAnim, -0.5f, this->skelAnime.curFrame - 1.0f, phi_f0,
...
void func_80992068(EnWf* this, PlayState* play) {
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if (!MM_Actor_IsFacingPlayer(&this->actor, 0x1554)) {
            func_80991200(this);
            this->unk_2A0 = (s32)MM_Rand_ZeroFloat(5.0f) + 5;
            if (sp28 > 0x32C8) {
                this->unk_298 = 30;
...
void func_8099223C(EnWf* this) {
    this->collider2.base.acFlags &= ~AC_ON;
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gWolfosBackflipAnim, -3.0f);
    this->unk_2A0 = 0;
    this->actor.speed = -6.0f;
    this->actor.shape.rot.y = this->actor.yawTowardsPlayer;
    this->actor.world.rot.y = this->actor.yawTowardsPlayer;
...
void func_809922B4(EnWf* this, PlayState* play) {
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if (!MM_Actor_OtherIsTargeted(play, &this->actor) && (this->actor.xzDistToPlayer < 170.0f) &&
            (this->actor.xzDistToPlayer > 140.0f) && (MM_Rand_ZeroOne() < 0.2f)) {
            func_80991438(this);
...
void func_8099245C(EnWf* this) {
    this->collider2.base.acFlags &= ~AC_ON;
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gWolfosDamagedAnim, -4.0f);
...
void func_809924EC(EnWf* this, PlayState* play) {
    ...
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0xBB8);
...
void func_809926D0(EnWf* this) {
    this->collider2.base.acFlags &= ~AC_ON;
    MM_Animation_Change(&this->skelAnime, &gWolfosBackflipAnim, -1.0f, MM_Animation_GetLastFrame(&gWolfosBackflipAnim), 0.0f,
                     ANIMMODE_ONCE, -3.0f);
    this->unk_2A0 = 0;
...
void func_80992784(EnWf* this, PlayState* play) {
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x9C4);
    if (MM_SkelAnime_Update(&this->skelAnime) &&
...
void func_8099282C(EnWf* this) {
    ...
    this->unk_2A0 = 10;
    this->actor.speed = 0.0f;
    MM_Animation_Change(&this->skelAnime, &gWolfosBlockAnim, -1.0f, MM_Animation_GetLastFrame(&gWolfosBlockAnim), 0.0f,
                     ANIMMODE_ONCE, -2.0f);
...
void func_809928CC(EnWf* this, PlayState* play) {
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        } else if (MM_Actor_IsFacingPlayer(&this->actor, 0x4000) && (this->actor.xzDistToPlayer < 60.0f) &&
...
void func_80992A74(EnWf* this, PlayState* play) {
    ...
    temp_f0 = MM_Math_SinS((player->actor.shape.rot.y + this->unk_29A) - this->actor.yawTowardsPlayer);
    ...
    this->actor.speed = 6.0f;
    this->actor.world.rot.y = this->actor.shape.rot.y;
    this->unk_2A0 = (s32)MM_Rand_ZeroFloat(10.0f) + 5;
...
void func_80992B8C(EnWf* this, PlayState* play) {
    ...
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer + this->unk_29A, 0x7D0);
    if (this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
...
void func_80992D6C(EnWf* this) {
    this->collider2.base.acFlags &= ~AC_ON;
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gWolfosRearUpFallOverAnim, -4.0f);
    this->actor.world.rot.y = this->actor.yawTowardsPlayer;
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        this->actor.speed = -6.0f;
    }
...
void func_80992E0C(EnWf* this, PlayState* play) {
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if (this->unk_295) {
            MM_Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, 0x70);
        } else {
...
void func_80993018(EnWf* this, PlayState* play) {
    ...
    if ((this->actor.child != NULL) && (this->actor.child->update != NULL)) {
        if (func_80990EAC(this)) {
            ...
            s16 temp_v0 = MM_Actor_WorldYawTowardActor(this->actor.child, &this->actor);
            s16 temp_v1 = BINANG_SUB(temp_v0 - this->actor.child->yawTowardsPlayer, 0x8000);

            if (ABS_ALT(temp_v1) > 0x800) {
                func_80993148(this);
            } else {
                MM_Math_ScaledStepToS(&this->actor.shape.rot.y, BINANG_ROT180(temp_v0), 0x800);
            }
        }
    } else {
        func_80990ED4(this);
...
void func_80993194(EnWf* this, PlayState* play) {
    ...
    if ((this->actor.child != NULL) && (this->actor.child->update != NULL)) {
        if (func_80990EAC(this)) {
            ...
            sp36 = MM_Actor_WorldYawTowardActor(this->actor.child, &this->actor);
            sp34 = MM_Math_SmoothStepToS(&sp36, BINANG_ROT180(this->actor.child->yawTowardsPlayer), 10, this->unk_29C, 16);
            this->actor.world.pos.x = (MM_Math_SinS(sp36) * this->unk_2A4) + this->actor.child->world.pos.x;
            this->actor.world.pos.z = (MM_Math_CosS(sp36) * this->unk_2A4) + this->actor.child->world.pos.z;
            if (sp34 < -0x800) {
                sp32 = sp36 - 0x4000;
            } else if (sp34 > 0x800) {
                sp32 = sp36 + 0x4000;
            ...
            MM_Math_ScaledStepToS(&this->actor.shape.rot.y, sp32, 0x800);
        }
    } else {
        func_80990ED4(this);
    }
...
void func_809933A0(EnWf* this, PlayState* play) {
    ...
    if ((this->actor.child != NULL) && (this->actor.child->update != NULL)) {
        if (func_80990EAC(this)) {
            ...
            sp2E = MM_Actor_WorldYawTowardActor(this->actor.child, &this->actor);
            MM_Math_ScaledStepToS(&sp2E, this->actor.child->yawTowardsPlayer, this->unk_29C >> 2);
            sp2C = sp2E - this->actor.child->yawTowardsPlayer;
            this->actor.world.pos.x = (MM_Math_SinS(sp2E) * this->unk_2A4) + this->actor.child->world.pos.x;
            this->actor.world.pos.z = (MM_Math_CosS(sp2E) * this->unk_2A4) + this->actor.child->world.pos.z;
            if (sp2C > 0) {
                phi_a1 = sp2E - 0x4000;
            } else {
                phi_a1 = sp2E + 0x4000;
            }
            MM_Math_ScaledStepToS(&this->actor.shape.rot.y, phi_a1, 0x800);
...
void func_8099357C(EnWf* this, PlayState* play) {
    ...
    if (this->unk_295) {
        if ((this->actor.child != NULL) && (this->actor.child->update != NULL)) {
            sp2E = MM_Actor_WorldYawTowardActor(&this->actor, this->actor.child);
            if (func_80990EAC(this)) {
                this->actor.child = NULL;
                func_80991738(this);
                return;
            ...
            MM_Math_SmoothStepToS(&this->actor.shape.rot.y, sp2E, 10, 0x800, 0x10);
            this->actor.world.rot.y = this->actor.shape.rot.y;

            if (MM_Actor_WorldDistXZToActor(&this->actor, this->actor.child) < (this->unk_2A4 + 10.0f)) {
                func_80993148(this);
            }
        } else {
            this->actor.child = NULL;
        ...
        MM_Math_SmoothStepToS(&this->actor.shape.rot.y, MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos), 10,
                           0x800, 0x10);
        this->actor.world.rot.y = this->actor.shape.rot.y;
        if (MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) < 40.0f) {
            func_809910F0(this);
        }
    }
...
void func_80993738(EnWf* this, PlayState* play) {
    if ((this->actionFunc == func_80991280) && (this->unk_298 != 0)) {
        this->unk_29E = MM_Math_SinS(this->unk_298 * 4200) * 8920.0f;
        ...
        MM_Math_ScaledStepToS(&this->unk_29E, 0, 0x3E8);
    } else if (this->actionFunc != func_809923E4) {
        MM_Math_ScaledStepToS(&this->unk_29E, this->actor.yawTowardsPlayer - this->actor.shape.rot.y, 0x3E8);
...
void func_8099386C(EnWf* this, PlayState* play) {
    ...
    if ((this->collider2.base.acFlags & AC_HIT) || (this->collider3.base.acFlags & AC_HIT)) {
        if ((!(this->collider2.base.acFlags & AC_HIT) && (this->collider3.base.acFlags & AC_HIT)) ||
            !MM_Actor_IsFacingPlayer(&this->actor, 0x4A38)) {
            this->actor.colChkInfo.damage *= 4;
        }

        if (this->collider2.base.acFlags & AC_HIT) {
            MM_Actor_SetDropFlag(&this->actor, &this->collider2.elem);
            collider = &this->collider2;
        } else {
            MM_Actor_SetDropFlag(&this->actor, &this->collider3.elem);
        ...
        if (((this->drawDmgEffType != ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX) ||
            ...
            if (this->actor.colChkInfo.damageEffect == 1) {
                ...
                if (this->actor.colChkInfo.damageEffect == 2) {
                    ...
                    this->drawDmgEffScale = 0.75f;
                    this->drawDmgEffAlpha = 4.0f;
                    MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_CLEAR_TAG, collider->elem.acDmgInfo.hitPos.x,
...
void MM_EnWf_Update(Actor* thisx, PlayState* play) {
    ...
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 32.0f, 30.0f, 60.0f,
    ...
    if (this->actor.bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_GROUND_TOUCH)) {
        ...
        MM_Math_ScaledStepToS(&this->actor.shape.rot.x, 0, 0x258);
        MM_Math_ScaledStepToS(&this->actor.shape.rot.z, 0, 0x258);
    }
    ...
    if (this->collider2.base.acFlags & AC_ON) {
        ...
        MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->collider3.base);
    ...
    if (this->collider1.base.atFlags & AT_ON) {
        if (!(this->collider1.base.atFlags & AT_BOUNCED)) {
            MM_CollisionCheck_SetAT(play, &play->colChkCtx, &this->collider1.base);
        } else {
            func_80991FD8(this);
        }
    }
    ...
    if (this->drawDmgEffAlpha > 0.0f) {
        ...
        } else if (!MM_Math_StepToF(&this->drawDmgEffFrozenSteamScale, 0.75f, 0.01875f)) {
            Actor_PlaySfx_Flagged(&this->actor, NA_SE_EV_ICE_FREEZE - SFX_FLAG);
        }
    }
}

s32 MM_EnWf_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    EnWf* this = (EnWf*)thisx;
...
void MM_EnWf_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    EnWf* this = (EnWf*)thisx;
    Vec3f sp20;

    MM_Collider_UpdateSpheres(limbIndex, &this->collider1);
...
void MM_EnWf_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != func_80990F50) {
        ...
        MM_SkelAnime_DrawFlexOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                              MM_EnWf_OverrideLimbDraw, MM_EnWf_PostLimbDraw, &this->actor);
...
s32 func_8099408C(PlayState* play, EnWf* this) {
    ...
    if (temp_v0 != NULL) {
        temp_v1 = MM_Actor_WorldYawTowardActor(&this->actor, temp_v0) - this->actor.shape.rot.y - this->unk_29E;
