...
void MM_EnCrow_Init(Actor* thisx, PlayState* play);
void MM_EnCrow_Destroy(Actor* thisx, PlayState* play);
void MM_EnCrow_Update(Actor* thisx, PlayState* play);
void MM_EnCrow_Draw(Actor* thisx, PlayState* play);

void MM_EnCrow_SetupFlyIdle(EnCrow* this);
void MM_EnCrow_FlyIdle(EnCrow* this, PlayState* play);
void MM_EnCrow_SetupDiveAttack(EnCrow* this);
void MM_EnCrow_DiveAttack(EnCrow* this, PlayState* play);
void EnCrow_CheckIfFrozen(EnCrow* this, PlayState* play);
void MM_EnCrow_Damaged(EnCrow* this, PlayState* play);
void MM_EnCrow_SetupDie(EnCrow* this);
void MM_EnCrow_Die(EnCrow* this, PlayState* play);
void MM_EnCrow_SetupRespawn(EnCrow* this);
void MM_EnCrow_TurnAway(EnCrow* this, PlayState* play);
void MM_EnCrow_Respawn(EnCrow* this, PlayState* play);
...
void MM_EnCrow_SetupFlyIdle(EnCrow* this) {
    ...
    this->actionFunc = MM_EnCrow_FlyIdle;
...
void MM_EnCrow_FlyIdle(EnCrow* this, PlayState* play) {
    ...
    if ((this->actor.parent != NULL) && (this->actor.parent->home.rot.z == 0)) {
        ...
        dist = MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.parent->world.pos);
    } else {
        dist = 450.0f;
        this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED;
    }
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
        ...
        this->yawTarget = MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos);
    }

    if ((MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->yawTarget, 5, 0x300, 0x10) == 0) && onAnimFirstFrame &&
        (MM_Rand_ZeroOne() < 0.1f)) {

        yaw = (MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos) - this->actor.shape.rot.y);
        if (yaw > 0) {
            this->yawTarget += MM_Rand_S16Offset(0x1000, 0x1000);
        } else {
            this->yawTarget -= MM_Rand_S16Offset(0x1000, 0x1000);
        }
    ...
    if ((this->actor.depthInWater > -40.0f) || (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
        ...
        this->pitchTarget = -MM_Rand_S16Offset(0x800, 0x800);
    } else if (this->actor.world.pos.y > (this->actor.home.pos.y + 50.0f)) {
        this->pitchTarget = MM_Rand_S16Offset(0x800, 0x800);
    }

    if (!MM_Math_SmoothStepToS(&this->actor.shape.rot.x, this->pitchTarget, 0xA, 0x100, 8) && onAnimFirstFrame &&
        (MM_Rand_ZeroOne() < 0.1f)) {
        if (this->actor.home.pos.y < this->actor.world.pos.y) {
            this->pitchTarget -= MM_Rand_S16Offset(0x400, 0x400);
        } else {
            this->pitchTarget += MM_Rand_S16Offset(0x400, 0x400);
        }
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        MM_Math_ScaledStepToS(&this->actor.shape.rot.x, -0x100, 0x400);
    }
    ...
    if ((this->timer == 0) &&
        ...
        (this->actor.depthInWater < -40.0f) && (MM_Player_GetMask(play) != PLAYER_MASK_STONE)) {
        if (dist < this->actor.xzDistToPlayer) {
            this->actor.child = this->actor.parent;
        } else {
            this->actor.child = &player->actor;
        }
        MM_EnCrow_SetupDiveAttack(this);
    }
}

void MM_EnCrow_SetupDiveAttack(EnCrow* this) {
    this->timer = 300;
    this->actionFunc = MM_EnCrow_DiveAttack;
...
void MM_EnCrow_DiveAttack(EnCrow* this, PlayState* play) {
    ...
    isFacingActor = MM_Actor_ActorAIsFacingActorB(&this->actor, this->actor.child, 0x2800);
    ...
    if (isFacingActor) {
        ...
        pitchTarget = MM_Actor_WorldPitchTowardPoint(&this->actor, &targetPos);
        pitchTarget = CLAMP(pitchTarget, -0x3000, 0x3000);
        MM_Math_SmoothStepToS(&this->actor.shape.rot.x, pitchTarget, 2, 0x400, 0x40);
    } else {
        MM_Math_SmoothStepToS(&this->actor.shape.rot.x, -0x800, 2, 0x100, 0x10);
    }

    if (isFacingActor || (MM_Actor_WorldDistXZToActor(&this->actor, this->actor.child) > 80.0f)) {
        MM_Math_SmoothStepToS(&this->actor.shape.rot.y, MM_Actor_WorldYawTowardActor(&this->actor, this->actor.child), 4,
                           0xC00, 0xC0);
    }

    if (((this->timer == 0) || ((&player->actor != this->actor.child) && (this->actor.child->home.rot.z != 0)) ||
         ((&player->actor == this->actor.child) &&
          ((MM_Player_GetMask(play) == PLAYER_MASK_STONE) || (player->stateFlags1 & PLAYER_STATE1_800000))) ||
...
void MM_EnCrow_SetupDamaged(EnCrow* this, PlayState* play) {
    f32 scale;
    ...
    this->actionFunc = MM_EnCrow_Damaged;
}
...
void MM_EnCrow_Die(EnCrow* this, PlayState* play) {
    ...
    if (MM_Math_StepToF(&this->actor.scale.x, 0.0f, stepScale)) {
        if (this->actor.params == GUAY_TYPE_NORMAL) {
            sDeadCount++;
        ...
        MM_EnCrow_SetupRespawn(this);
    }
    this->actor.scale.z = this->actor.scale.y = this->actor.scale.x;
}

void MM_EnCrow_SetupTurnAway(EnCrow* this) {
    this->timer = 100;
    this->pitchTarget = -0x1000;
    ...
    this->actionFunc = MM_EnCrow_TurnAway;
}
...
void MM_EnCrow_TurnAway(EnCrow* this, PlayState* play) {
    ...
    MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->yawTarget, 3, 0xC00, 0xC0);
    MM_Math_SmoothStepToS(&this->actor.shape.rot.x, this->pitchTarget, 5, 0x100, 0x10);
...
void MM_EnCrow_SetupRespawn(EnCrow* this) {
    if (sDeadCount == GUAY_NUMBER_OF_DEAD_TO_SPAWN_MEGAGUAY) {
        ...
        this->collider.elements[0].dim.worldSphere.radius =
            MM_sJntSphInit.elements[0].dim.modelSphere.radius * 0.03f * 100.0f;
    } else {
        this->actor.params = GUAY_TYPE_NORMAL;
        this->collider.elements[0].dim.worldSphere.radius = MM_sJntSphInit.elements[0].dim.modelSphere.radius;
    }
    MM_Animation_PlayLoop(&this->skelAnime, &gGuayFlyAnim);
    ...
    this->drawDmgEffAlpha = 0.0f;
    this->actionFunc = MM_EnCrow_Respawn;
}
...
void MM_EnCrow_Respawn(EnCrow* this, PlayState* play) {
    ...
    if (this->timer == 0) {
        MM_SkelAnime_Update(&this->skelAnime);
        this->actor.draw = MM_EnCrow_Draw;
        if (this->actor.params != GUAY_TYPE_NORMAL) {
            scaleTarget = 0.03f;
        } else {
            scaleTarget = 0.01f;
        }
        if (MM_Math_StepToF(&this->actor.scale.x, scaleTarget, scaleTarget * 0.1f)) {
            this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED;
            this->actor.flags &= ~ACTOR_FLAG_UPDATE_CULLING_DISABLED;
...
void MM_EnCrow_UpdateDamage(EnCrow* this, PlayState* play) {
    if (this->collider.base.acFlags & AC_HIT) {
        this->collider.base.acFlags &= ~AC_HIT;
        MM_Actor_SetDropFlag(&this->actor, &this->collider.elements[0].base);

        if (this->actor.colChkInfo.damageEffect == GUAY_DMGEFF_STUN) {
            MM_EnCrow_SetupTurnAway(this);
            ...
            MM_EnCrow_SetupDamaged(this, play);
        }
    }
}
...
void MM_EnCrow_Update(Actor* thisx, PlayState* play) {
    ...
    MM_EnCrow_UpdateDamage(this, play);
    this->actionFunc(this, play);
    scale = this->actor.scale.x * 100.0f;
    this->actor.world.rot.y = this->actor.shape.rot.y;
    this->actor.world.rot.x = -this->actor.shape.rot.x;

    if (this->actionFunc != MM_EnCrow_Respawn) {
        ...
        MM_Actor_UpdateBgCheckInfo(play, &this->actor, 12.0f * scale, 25.0f * scale, 50.0f * scale,
    ...
    if (this->actionFunc == MM_EnCrow_DiveAttack) {
    ...
    MM_Actor_SetFocus(&this->actor, height);

    if ((this->actor.colChkInfo.health != 0) && MM_Animation_OnFrame(&this->skelAnime, 3.0f)) {
    ...
    if (this->drawDmgEffAlpha > 0.0f) {
        ...
        } else if (!MM_Math_StepToF(&this->drawDmgEffScale, 0.5f, 0.5f * 0.025f)) {
            Actor_PlaySfx_Flagged(&this->actor, NA_SE_EV_ICE_FREEZE - SFX_FLAG);
        }
    }
}

s32 MM_EnCrow_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    EnCrow* this = (EnCrow*)thisx;

    if (this->actor.colChkInfo.health != 0) {
        if (limbIndex == OBJECT_CROW_LIMB_UPPER_TAIL) {
            rot->y += TRUNCF_BINANG(0xC00 * MM_Math_SinF(this->skelAnime.curFrame * (M_PIf / 4)));
        } else if (limbIndex == OBJECT_CROW_LIMB_TAIL) {
            rot->y += TRUNCF_BINANG(0x1400 * MM_Math_SinF((this->skelAnime.curFrame + 2.5f) * (M_PIf / 4)));
        }
    }
    return false;
}

void MM_EnCrow_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    EnCrow* this = (EnCrow*)thisx;
...
void MM_EnCrow_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_DrawFlexOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                          MM_EnCrow_OverrideLimbDraw, MM_EnCrow_PostLimbDraw, &this->actor);
