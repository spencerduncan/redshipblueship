...
void MM_EnPoh_Init(Actor* thisx, PlayState* play);
void MM_EnPoh_Destroy(Actor* thisx, PlayState* play);
void MM_EnPoh_Update(Actor* thisx, PlayState* play2);
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 3, 25, 50, 50 };
...
void MM_EnPoh_Init(Actor* thisx, PlayState* play) {
    ...
    Collider_InitAndSetJntSph(play, &this->colliderSph, &this->actor, &MM_sJntSphInit, this->colliderSphElements);
    ...
    this->unk_18D = 32;
    this->unk_190 = MM_Rand_S16Offset(700, 300);
    this->lightNode = MM_LightContext_InsertLight(play, &play->lightCtx, &this->lightInfo);
    MM_Lights_PointGlowSetInfo(&this->lightInfo, this->actor.home.pos.x, this->actor.home.pos.y, this->actor.home.pos.z,
                            255, 255, 255, 0);
    MM_SkelAnime_Init(play, &this->skelAnime, &gPoeSkel, &gPoeFloatAnim, this->jointTable, this->morphTable, POE_LIMB_MAX);
...
void MM_EnPoh_Destroy(Actor* thisx, PlayState* play) {
    ...
    MM_Collider_DestroyJntSph(play, &this->colliderSph);
...
void func_80B2C910(Vec3f* vec, PlayState* play) {
    ...
    if (activeCam != NULL) {
        MM_Math_Vec3f_Diff(&activeCam->eye, &activeCam->at, &sp20);
        temp_f0 = MM_Math3D_Vec3fMagnitude(&sp20);
        if (temp_f0 > 1.0f) {
        ...
        MM_Math_Vec3f_Copy(vec, &gZeroVec3f);
    }
}
...
void func_80B2C9B8(EnPoh* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->actor.world.pos.y, player->actor.world.pos.y, 1.0f);
    this->actor.world.pos.y += 2.5f * MM_Math_SinS(this->unk_18D * 0x800);
    if (this->unk_18D != 0) {
        this->unk_18D--;
    }
...
void func_80B2CA4C(EnPoh* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &gPoeDisappearAnim);
    this->unk_18E = MM_Rand_S16Offset(2, 3);
...
void func_80B2CAA4(EnPoh* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 0.0f) && (this->unk_18E != 0)) {
        this->unk_18E--;
    }
...
void func_80B2CB60(EnPoh* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &gPoeFloatAnim);
    this->unk_18E = MM_Rand_S16Offset(15, 3);
...
void func_80B2CBBC(EnPoh* this, PlayState* play) {
    ...
    if (MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) > 400.0f) {
        this->unk_192 = MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos);
    }

    MM_Math_ScaledStepToS(&this->actor.world.rot.y, this->unk_192, 0x71C);
    func_80B2C9B8(this, play);
...
void func_80B2CD64(EnPoh* this, PlayState* play) {
    ...
    if (this->actor.xzDistToPlayer > 280.0f) {
        func_80B2CB60(this);
               ...
               !MM_Player_IsFacingActor(&this->actor, 0x2AAA, play)) {
        func_80B2CEC8(this);
    }
...
void func_80B2CEC8(EnPoh* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gPoeAttackAnim, -6.0f);
    this->unk_18E = 12;
    this->actor.speed = 0.0f;
...
void func_80B2CF28(EnPoh* this, PlayState* play) {
    ...
    func_80B2C9B8(this, play);
    if (this->unk_18E >= 10) {
        MM_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 0xAAA);
...
void func_80B2CFF8(EnPoh* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gPoeDamagedAnim, -6.0f);
...
void func_80B2D0E8(EnPoh* this) {
    ...
    MM_Animation_PlayOnceSetSpeed(&this->skelAnime, &gPoeAppearAnim, 0.0f);
...
void func_80B2D300(EnPoh* this, PlayState* play) {
    ...
    this->unk_18E++;
    if (this->unk_18E < 8) {
        sp38 = MM_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) + 0x4800;
        if (this->unk_18E < 5) {
            sp3A = (this->unk_18E * 0x1000) - 0x4000;
            sp44.y = (MM_Math_SinS(sp3A) * 23.0f) + (this->actor.world.pos.y + 40.0f);
            sp40 = MM_Math_CosS(sp3A) * 23.0f;
            sp44.x = (MM_Math_SinS(sp38) * sp40) + this->actor.world.pos.x;
            sp44.z = (MM_Math_CosS(sp38) * sp40) + this->actor.world.pos.z;
        } else {
            sp44.y = this->actor.world.pos.y + 40.0f + (15.0f * (this->unk_18E - 5));
            sp44.x = (MM_Math_SinS(sp38) * 23.0f) + this->actor.world.pos.x;
            sp44.z = (MM_Math_CosS(sp38) * 23.0f) + this->actor.world.pos.z;
        }
...
void func_80B2D5DC(EnPoh* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gPoeFloatAnim);
...
void func_80B2D6EC(EnPoh* this, PlayState* play) {
    MM_SkelAnime_Update(&this->skelAnime);
    if (MM_Math_ScaledStepToS(&this->actor.world.rot.y, this->unk_192, 0x71C)) {
        func_80B2CB60(this);
    }
...
void func_80B2D7D4(EnPoh* this, PlayState* play) {
    ...
    if (this->unk_18C == 0) {
        this->unk_190 = MM_Rand_S16Offset(100, 50);
...
void func_80B2DAD0(EnPoh* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gPoeFleeAnim, -5.0f);
    this->actor.speed = 5.0f;
...
void func_80B2DB44(EnPoh* this, PlayState* play) {
    ...
    MM_SkelAnime_Update(&this->skelAnime);
    sp24 = MM_Math_SinS(this->unk_18D * 0x800) * 3.0f;
    this->actor.world.pos.x -= sp24 * MM_Math_CosS(this->actor.shape.rot.y);
    this->actor.world.pos.z += sp24 * MM_Math_SinS(this->actor.shape.rot.y);
...
void func_80B2DD2C(EnPoh* this, PlayState* play) {
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) || (this->actor.floorHeight <= BGCHECK_Y_MIN)) {
        MM_EffectSsHahen_SpawnBurst(play, &this->actor.world.pos, 6.0f, 0, 1, 1, 15, OBJECT_PO, 10, gPoeLanternDL);
        func_80B2E0B0(this);
    }
...
void func_80B2E230(EnPoh* this, PlayState* play) {
    ...
    this->actor.world.pos.y = (MM_Math_SinS(this->unk_18D * 0x800) * 5.0f) + this->actor.home.pos.y;
    if (this->unk_18D) {
        this->unk_18D--;
    }
...
void MM_EnPoh_Update(Actor* thisx, PlayState* play2) {
    ...
    if ((this->actionFunc == func_80B2CF28) && (this->unk_18E < 10)) {
        ...
        MM_CollisionCheck_SetAT(play, &play->colChkCtx, &this->colliderSph.base);
    }
    ...
    MM_CollisionCheck_SetOC(play, &play->colChkCtx, &this->colliderSph.base);
...
s32 MM_EnPoh_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx,
                           Gfx** gfx) {
    EnPoh* this = (EnPoh*)thisx;
...
void MM_EnPoh_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx, Gfx** gfx) {
    s32 bodyPartIndex;
    Vec3f sp60;
    EnPoh* this = (EnPoh*)thisx;
    s32 pad;

    MM_Collider_UpdateSpheres(limbIndex, &this->colliderSph);

    if ((this->actionFunc == func_80B2D300) && (this->unk_18E >= 2) && (limbIndex == POE_LIMB_TOP_CLOAK)) {
        MATRIX_FINALIZE_AND_LOAD((*gfx)++, play->state.gfxCtx);
        MM_gSPDisplayList((*gfx)++, gPoeBurnDL);
    }

    if (limbIndex == POE_LIMB_LANTERN) {
        if ((this->actionFunc == func_80B2D300) && (this->unk_18E >= 19) && (this->actor.scale.x != 0.0f)) {
            MM_Matrix_Scale(0.01f / this->actor.scale.x, 0.01f / this->actor.scale.x, 0.01f / this->actor.scale.x,
                         MTXMODE_APPLY);
        }
        MM_Matrix_Get(&this->unk_3D8);
        func_80B2C910(&sp60, play);
        MM_Lights_PointGlowSetInfo(&this->lightInfo, this->colliderSph.elements[0].dim.worldSphere.center.x + (s32)sp60.x,
...
void EnPoh_Draw(Actor* thisx, PlayState* play) {
    ...
    if ((this->unk_197 == 255) || (this->unk_197 == 0)) {
        ...
        gfx = MM_SkelAnime_Draw(play, this->skelAnime.skeleton, this->skelAnime.jointTable, MM_EnPoh_OverrideLimbDraw,
                             MM_EnPoh_PostLimbDraw, &this->actor, &gfx[3]);
        ...
        MM_gSPSegment(&gfx[2], 0x08, D_801AEF88);

        POLY_XLU_DISP = MM_SkelAnime_Draw(play, this->skelAnime.skeleton, this->skelAnime.jointTable,
                                       MM_EnPoh_OverrideLimbDraw, MM_EnPoh_PostLimbDraw, &this->actor, &gfx[3]);
    ...
    MM_Matrix_Put(&this->unk_3D8);

    MATRIX_FINALIZE_AND_LOAD(&gfx[2], play->state.gfxCtx);
    MM_gSPDisplayList(&gfx[3], gPoeLanternDL);
...
void func_80B2F37C(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc == func_80B2DD2C) {
        ...
        MM_Lights_PointGlowSetInfo(&this->lightInfo, this->actor.world.pos.x + sp7C.x, this->actor.world.pos.y + sp7C.y,
        ...
        MM_gSPSegment(
            POLY_XLU_DISP++, 0x08,
            MM_Gfx_TwoTexScroll(play->state.gfxCtx, 0, 0, 0, 0x20, 0x40, 1, 0, (this->unk_190 * -8) & 0x1FF, 0x20, 0x80));
        ...
        Matrix_RotateYF(BINANG_TO_RAD(MM_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) + 0x8000), MTXMODE_APPLY);

        MATRIX_FINALIZE_AND_LOAD(POLY_XLU_DISP++, play->state.gfxCtx);
        MM_gSPDisplayList(POLY_XLU_DISP++, gPoeSoulDL);
    }
