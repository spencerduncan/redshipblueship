...
void MM_EnArrow_Init(Actor* thisx, PlayState* play);
void MM_EnArrow_Destroy(Actor* thisx, PlayState* play);
void MM_EnArrow_Update(Actor* thisx, PlayState* play);
void MM_EnArrow_Draw(Actor* thisx, PlayState* play);
...
void MM_EnArrow_Init(Actor* thisx, PlayState* play) {
    ...
    if (ARROW_IS_ARROW(this->actor.params)) {
        MM_SkelAnime_Init(play, &this->arrow.skelAnime, &gameplay_keep_Skel_014560, &gameplay_keep_Anim_0128BC,
        ...
        if (this->actor.params < ARROW_TYPE_FIRE) {
            ...
            MM_Effect_Add(play, &this->unk_240, EFFECT_BLURE2, 0, 0, &D_8088C234);
        } else if (this->actor.params == ARROW_TYPE_FIRE) {
            MM_Effect_Add(play, &this->unk_240, EFFECT_BLURE2, 0, 0, &D_8088C258);
        } else if (this->actor.params == ARROW_TYPE_ICE) {
            MM_Effect_Add(play, &this->unk_240, EFFECT_BLURE2, 0, 0, &D_8088C27C);
        } else if (this->actor.params == ARROW_TYPE_LIGHT) {
            MM_Effect_Add(play, &this->unk_240, EFFECT_BLURE2, 0, 0, &D_8088C2A0);
        }
    }
...
void func_8088A594(EnArrow* this, PlayState* play) {
    ...
    if (this->actor.parent != NULL) {
        if (this->actor.params == ARROW_TYPE_DEKU_BUBBLE) {
            if (MM_Math_SmoothStepToF(&this->bubble.unk_144, 16.0f, 0.07f, 1.8f, 0.0f) > 0.5f) {
        ...
        switch (this->actor.params) {
            case ARROW_TYPE_SLINGSHOT:
                MM_Player_PlaySfx(player, NA_SE_IT_SLING_SHOT);
                break;

            case ARROW_TYPE_NORMAL_LIT:
            case ARROW_TYPE_NORMAL_HORSE:
            case ARROW_TYPE_NORMAL:
                MM_Player_PlaySfx(player, NA_SE_IT_ARROW_SHOT);
                break;

            case ARROW_TYPE_FIRE:
            case ARROW_TYPE_ICE:
            case ARROW_TYPE_LIGHT:
                MM_Player_PlaySfx(player, NA_SE_IT_MAGIC_ARROW_SHOT);

            case ARROW_TYPE_DEKU_BUBBLE:
                MM_Player_PlaySfx(player, NA_SE_PL_DEKUNUTS_FIRE);
                break;
        ...
        this->actionFunc = func_8088ACE0;
        MM_Math_Vec3f_Copy(&this->unk_228, &this->actor.world.pos);
        ...
        if (this->actor.params == ARROW_TYPE_DEKU_BUBBLE) {
            ...
            if ((this->actor.params == ARROW_TYPE_DEKU_NUT) && (this->actor.world.rot.x < 0)) {
                MM_Actor_SetScale(&this->actor, 0.009f);
                this->unk_260 = 40;
            } else {
...
void func_8088A7D8(PlayState* play, EnArrow* this) {
    ...
    MM_Animation_PlayOnce(&this->arrow.skelAnime, &gameplay_keep_Anim_012860);
    this->actor.world.rot.y += (s32)(0x6000 * (MM_Rand_ZeroOne() - 0.5f)) + 0x8000;
    this->actor.speed *= 0.02f + (0.02f * MM_Rand_ZeroOne());
...
void func_8088A894(EnArrow* this, PlayState* play) {
    ...
    MM_Math_Vec3f_Diff(&this->actor.world.pos, &this->unk_228, &sp68);
    ...
    temp_f0 = MM_Math3D_Vec3fMagnitudeSq(&sp68);
    if (temp_f0 < 1.0f) {
        return;
    }

    temp_f0 = sp4C / temp_f0;
    MM_Math_Vec3f_Scale(&sp68, temp_f0);
    MM_Math_Vec3f_Sum(&this->unk_264->world.pos, &sp68, &sp5C);
    if (MM_BgCheck_EntityLineTest1(&play->colCtx, &this->unk_264->world.pos, &sp5C, &sp50, &sp74, true, true, true, true,
                                &bgId)) {
        ...
        MM_Math_Vec3f_Copy(&this->unk_264->world.pos, &sp5C);
    }
}
...
void func_8088AA98(EnArrow* this, PlayState* play) {
    ...
    if (MM_WaterBox_GetSurface1(play, &play->colCtx, this->actor.world.pos.x, this->actor.world.pos.z, &sp50, &waterBox) &&
        ...
        if (sp44.y != 0.0f) {
            ...
            MM_EffectSsGSplash_Spawn(play, &sp44, NULL, NULL, 0, 300);
        }

        Actor_PlaySfx(&this->actor, NA_SE_EV_DIVE_INTO_WATER_L);

        MM_EffectSsGRipple_Spawn(play, &sp44, 100, 500, 0);
        MM_EffectSsGRipple_Spawn(play, &sp44, 100, 500, 4);
        MM_EffectSsGRipple_Spawn(play, &sp44, 100, 500, 8);
        ...
        if ((this->actor.params == ARROW_TYPE_ICE) || (this->actor.params == ARROW_TYPE_FIRE)) {
            if ((this->actor.params == ARROW_TYPE_ICE) && (func_8088B6B0 != this->actionFunc)) {
                MM_Actor_Spawn(&play->actorCtx, play, ACTOR_BG_ICEFLOE, sp44.x, sp44.y, sp44.z, 0, 0, 0, 300);
                MM_Actor_Kill(&this->actor);
                return;
            }
...
void func_8088ACE0(EnArrow* this, PlayState* play) {
        ...
        if (this->actor.params == ARROW_TYPE_DEKU_BUBBLE) {
            if (phi_a2 && (this->collider.elem.atHitElem->elemMaterial != ELEM_MATERIAL_UNK4) &&
                ...
                if ((this->collider.base.at != NULL) && (this->collider.base.at->id != ACTOR_OBJ_SYOKUDAI)) {
                    ...
                    this->actor.world.rot.y += BINANG_ROT180(TRUNCF_BINANG(MM_Rand_CenteredFloat(0x1F40)));
            ...
            if ((this->unk_262 != 0) || phi_a2) {
                MM_EffectSsStone1_Spawn(play, &this->actor.world.pos, 0);
            }

            MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 20, NA_SE_IT_DEKUNUTS_BUBLE_VANISH);

            if ((this->unk_262 != 0) && (this->actor.wallBgId == BG_ACTOR_MAX)) {

                MM_Math_Vec3f_Copy(&sp84.pos, &this->actor.world.pos);
                sp84.colPoly = this->actor.wallPoly;
                sp84.scale = this->bubble.unk_144;
                MM_EffectSs_Spawn(play, EFFECT_SS_SBN, 128, &sp84);
            }
        }
        MM_Actor_Kill(&this->actor);
        return;
    ...
    if (sp50 || (this->unk_262 != 0)) {
        if (!ARROW_IS_ARROW(this->actor.params)) {
            ...
            if (this->actor.params == ARROW_TYPE_DEKU_NUT) {
                ...
                MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_M_FIRE1, this->actor.world.pos.x, this->actor.world.pos.y,
            ...
            MM_EffectSsStone1_Spawn(play, &this->actor.world.pos, 0);
            MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 20, sp82);
            MM_Actor_Kill(&this->actor);
        } else {
            ...
            if (sp50 && (this->collider.elem.atHitElem->elemMaterial != ELEM_MATERIAL_UNK4)) {
                ...
                if ((sp7C->update != NULL) && !(this->collider.base.atFlags & AT_BOUNCED) &&
                    ...
                    MM_Math_Vec3f_Diff(&sp7C->world.pos, &this->actor.world.pos, &this->unk_268);
                ...
                MM_Animation_PlayOnce(&this->arrow.skelAnime, &gameplay_keep_Anim_0128BC);
                if (this->actor.params >= ARROW_TYPE_NORMAL_LIT) {
                    this->unk_260 = 60;
                } else {
                    this->unk_260 = 20;
        ...
        if (this->actor.params == ARROW_TYPE_DEKU_BUBBLE) {
            ...
            if (MM_Math_StepToF(&this->bubble.unk_144, 1.0f, 0.4f)) {
                this->unk_260 = 0;
            } else {
                this->bubble.unk_14A += TRUNCF_BINANG(this->bubble.unk_144 * (500.0f + MM_Rand_ZeroFloat(1400.0f)));
                this->actor.world.rot.x += TRUNCF_BINANG(500.0f * MM_Math_SinS(this->bubble.unk_14A));
                this->actor.shape.rot.x = this->actor.world.rot.x;

                this->bubble.unk_14C += TRUNCF_BINANG(this->bubble.unk_144 * (500.0f + MM_Rand_ZeroFloat(1400.0f)));
                this->actor.world.rot.y += TRUNCF_BINANG(500.0f * MM_Math_SinS(this->bubble.unk_14C));
        ...
        if ((this->unk_262 = MM_BgCheck_ProjectileLineTest(&play->colCtx, &this->actor.prevPos, &this->actor.world.pos,
            ...
            MM_Math_Vec3f_Copy(&this->actor.world.pos, &sp9C);
    ...
    if (this->unk_264 != NULL) {
        if (this->unk_264->update != NULL) {
            ...
            MM_Math_Vec3f_Sum(&this->unk_228, &this->unk_268, &sp60);
            MM_Math_Vec3f_Sum(&this->actor.world.pos, &this->unk_268, &sp54);

            if (MM_BgCheck_EntityLineTest1(&play->colCtx, &sp60, &sp54, &sp9C, &spAC, true, true, true, true, &bgId)) {
                ...
                MM_Math_Vec3f_Diff(&this->unk_264->world.pos, &this->actor.world.pos, &this->unk_268);
                this->unk_264->flags &= ~ACTOR_FLAG_ATTACHED_TO_ARROW;
                this->unk_264 = NULL;
            } else {
                MM_Math_Vec3f_Sum(&this->actor.world.pos, &this->unk_268, &this->unk_264->world.pos);
            }
...
void func_8088B630(EnArrow* this, PlayState* play) {
    MM_SkelAnime_Update(&this->arrow.skelAnime);
...
void MM_EnArrow_Update(Actor* thisx, PlayState* play) {
    static Vec3f MM_sVelocity = { 0.0f, 0.5f, 0.0f };
    ...
    if ((this->unk_263 != 0) ||
        ...
        !MM_Player_InBlockingCsMode(play, player)) {
    ...
    if (ARROW_IS_MAGICAL(this->actor.params)) {
        ...
        if (this->actor.child == NULL) {
            MM_Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, sp44[ARROW_GET_MAGIC_FROM_TYPE(this->actor.params)],
        ...
        func_800B0EB0(play, &this->unk_234, &MM_sVelocity, &MM_sAccel, &sPrimColor, &sEnvColor, 100, 0, 8);
    }

    MM_Math_Vec3f_Copy(&this->actor.home.pos, &this->actor.prevPos);
}
...
void func_8088B88C(PlayState* play, EnArrow* this, EnArrowUnkStruct* arg2) {
    ...
    MM_Matrix_MultVec3f(&arg2->unk_48, &this->unk_234);
    if (func_8088ACE0 == this->actionFunc) {
        ...
        MM_Matrix_MultVec3f(&sp4C[0], &sp40);
        MM_Matrix_MultVec3f(&sp4C[1], &sp34);
        if (this->actor.params < ARROW_TYPE_DEKU_NUT) {
            ...
            if (sp30) {
                MM_EffectBlure_AddVertex(MM_Effect_GetByIndex(this->unk_240), &sp40, &sp34);
            }
        }
    }
}
...
void MM_EnArrow_Draw(Actor* thisx, PlayState* play) {
    ...
    if (ARROW_IS_ARROW(this->actor.params)) {
        ...
        MM_SkelAnime_DrawLod(play, this->arrow.skelAnime.skeleton, this->arrow.skelAnime.jointTable, NULL, NULL,
        ...
        if (this->bubble.unk_149 >= 0) {
            ...
            MM_Collider_ResetQuadAT(play, &this->collider.base);
        }

        sp9C *= 0.002f;
        spA0 *= 0.002f;

        MM_Matrix_Scale(this->bubble.unk_144 * sp9C, this->bubble.unk_144 * sp9C, this->bubble.unk_144 * spA0,
                     MTXMODE_APPLY);
        MM_Matrix_Translate(0.0f, 0.0f, 460.0f, MTXMODE_APPLY);
        ...
        if (this->actor.speed == 0.0f) {
            ...
            MM_Matrix_ReplaceRotation(&gIdentityMtxF);
        ...
        u8 sp63 = (MM_Math_CosS(this->unk_260 * 5000) * 127.5f) + 127.5f;
        f32 sp5C;
