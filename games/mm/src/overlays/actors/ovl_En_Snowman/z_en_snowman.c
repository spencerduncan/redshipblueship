...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 2, 60, 80, 150 };
...
void EnSnowman_Init(Actor* thisx, PlayState* play) {
    ...
    if (EN_SNOWMAN_GET_TYPE(thisx) < EN_SNOWMAN_TYPE_SMALL_SNOWBALL) {
        MM_SkelAnime_InitFlex(play, &this->skelAnime, &gEenoSkel, &gEenoEmergeAnim, this->jointTable, this->morphTable,
                           EENO_LIMB_MAX);
        MM_SkelAnime_InitFlex(play, &this->snowPileSkelAnime, &gEenoSnowPileSkel, &gEenoSnowPileMoveAnim,
                           this->snowPileJointTable, this->snowPileMorphTable, EENO_SNOW_PILE_LIMB_MAX);
        MM_CollisionCheck_SetInfo(&thisx->colChkInfo, &MM_sDamageTable, &MM_sColChkInfoInit);
        ...
        if (EN_SNOWMAN_GET_TYPE(thisx) == EN_SNOWMAN_TYPE_LARGE) {
            ...
            MM_Actor_SpawnAsChild(&play->actorCtx, thisx, play, ACTOR_EN_SNOWMAN, thisx->world.pos.x, thisx->world.pos.y,
            ...
            if ((thisx->child == NULL) || (thisx->parent == NULL)) {
                if (thisx->child != NULL) {
                    MM_Actor_Kill(thisx->child);
                }

                if (thisx->parent != NULL) {
                    MM_Actor_Kill(thisx->parent);
                }
        ...
        thisx->world.rot.y = MM_Actor_WorldYawTowardActor(thisx, &player->actor);
        thisx->velocity.y = (MM_Actor_WorldDistXZToActor(thisx, &player->actor) * 0.035f) + -5.0f;
        ...
        thisx->world.pos.x += thisx->speed * MM_Math_SinS(thisx->world.rot.y);
        thisx->world.pos.y += thisx->velocity.y;
        thisx->world.pos.z += thisx->speed * MM_Math_CosS(thisx->world.rot.y);

        if (EN_SNOWMAN_GET_TYPE(thisx) == EN_SNOWMAN_TYPE_SMALL_SNOWBALL) {
            this->collider.dim.radius = 8;
            this->collider.dim.height = 12;
            this->collider.dim.yShift = -6;
            MM_ActorShape_Init(&thisx->shape, 0.0f, MM_ActorShadow_DrawCircle, 10.0f);
        } else {
            ...
            MM_Actor_SetScale(thisx, 0.006f);
            MM_ActorShape_Init(&thisx->shape, 16000.0f / 3.0f, MM_ActorShadow_DrawCircle, 170.0f);
            thisx->gravity = -1.5f;
        }
...
void EnSnowman_SpawnCircularDustEffect(EnSnowman* this, PlayState* play) {
    ...
    pos.y = (MM_Rand_ZeroFloat(10.0f) * this->eenoScale) + this->actor.world.pos.y;
    for (i = 0; i < 16; i++) {
        offset = (MM_Rand_ZeroFloat(10.0f) + 20.0f) * this->eenoScale;
        pos.x = (MM_Math_SinS(angle) * offset) + this->actor.world.pos.x;
        pos.z = (MM_Math_CosS(angle) * offset) + this->actor.world.pos.z;
        func_800B0DE0(play, &pos, &sDustVelocity, &gZeroVec3f, &MM_sDustPrimColor, &MM_sDustEnvColor,
...
void EnSnowman_SpawnBigSnowballFragmentEffects(EnSnowman* this, PlayState* play) {
    ...
    for (i = 0; i < 15; i++) {
        ...
        velocity.y = speed * MM_Math_SinS(altitude);
        velocity.z = (speed * MM_Math_CosS(altitude)) * MM_Math_CosS(azimuth);
        pos.x = (MM_Rand_ZeroFloat(10.0f) * velocity.x) + this->snowballPos.x;
        pos.y = (MM_Rand_ZeroFloat(8.0f) * velocity.y) + this->snowballPos.y;
        pos.z = (MM_Rand_ZeroFloat(10.0f) * velocity.z) + this->snowballPos.z;
        MM_EffectSsHahen_Spawn(play, &pos, &velocity, &MM_sAccel, 0,
                            MM_Rand_S16Offset((((i % 3) * 50) + 50), (((i % 3) * 25) + 25)), OBJECT_SNOWMAN, 20,
                            sSnowballFragmentDLs[i % 3]);
    }

    func_800B0DE0(play, &this->snowballPos, &gZeroVec3f, &gZeroVec3f, &MM_sDustPrimColor, &MM_sDustEnvColor, 1000, 150);
}

void EnSnowman_SetupMoveSnowPile(EnSnowman* this) {
    MM_Animation_PlayLoop(&this->snowPileSkelAnime, &gEenoSnowPileMoveAnim);
...
void EnSnowman_MoveSnowPile(EnSnowman* this, PlayState* play) {
    ...
    MM_SkelAnime_Update(&this->snowPileSkelAnime);
    ...
    if ((this->combineTimer == 0) && (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_SPLIT)) {
        ...
        EnSnowman_SetupCombine(this, play, &combinePos);
               ...
               (this->actor.xzDistToPlayer < this->attackRange) && (MM_Player_GetMask(play) != PLAYER_MASK_STONE) &&
        ...
        if (MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->snowPileTargetRotY, 0x100)) {
            this->turningOnSteepSlope = false;
        }
    ...
    } else if (MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) > 200.0f) {
        this->snowPileTargetRotY =
            MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos) + ((s32)MM_Rand_Next() >> 0x14);
    } else if (MM_Rand_ZeroOne() < 0.02f) {
        this->snowPileTargetRotY += (s16)(((MM_Rand_Next() >> 0x13) + 0x1000) * ((MM_Rand_ZeroOne() < 0.5f) ? -1 : 1));
    }
}

void EnSnowman_SetupEmerge(EnSnowman* this, PlayState* play) {
    MM_Animation_PlayOnce(&this->skelAnime, &gEenoEmergeAnim);
...
void EnSnowman_Emerge(EnSnowman* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->actor.scale.y, this->actor.scale.x,
                 (this->actor.scale.x * 0.6f) / MM_Animation_GetLastFrame(&gEenoEmergeAnim));
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        } else if (!(player->stateFlags1 & PLAYER_STATE1_800000) && (MM_Player_GetMask(play) != PLAYER_MASK_STONE)) {
            this->collider.base.acFlags |= AC_ON;
...
void EnSnowman_SetupReadySnowball(EnSnowman* this) {
    ...
    if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_LARGE) {
        ...
        MM_Animation_PlayOnce(&this->skelAnime, &gEenoLargeSnowballCreateAnim);
    } else {
        this->fwork.frameToStartHoldingSnowball = 6.0f;
        MM_Animation_PlayOnce(&this->skelAnime, &gEenoSmallSnowballCreateAnim);
    }
...
void EnSnowman_ReadySnowball(EnSnowman* this, PlayState* play) {
    ...
    if ((EN_SNOWMAN_GET_TYPE(&this->actor) != EN_SNOWMAN_TYPE_LARGE) && this->isHoldingSnowball &&
        ...
        pos.x = MM_Rand_CenteredFloat(10.0f) + this->snowballPos.x;
        pos.y = MM_Rand_CenteredFloat(10.0f) + this->snowballPos.y;
        pos.z = MM_Rand_CenteredFloat(10.0f) + this->snowballPos.z;
        func_800B0DE0(play, &pos, &sDustVelocity, &gZeroVec3f, &MM_sDustPrimColor, &MM_sDustEnvColor, 500, 30);
        ...
        if ((this->skelAnime.curFrame > 3.0f) && (this->skelAnime.curFrame < 14.0f) &&
            ((play->gameplayFrames % 2) != 0)) {
            pos.x =
                (this->actor.world.pos.x + (70.0f * MM_Math_SinS(this->actor.shape.rot.y))) + MM_Rand_CenteredFloat(40.0f);
            pos.y = this->actor.world.pos.y + MM_Rand_CenteredFloat(20.0f);
            pos.z =
                (this->actor.world.pos.z + (70.0f * MM_Math_CosS(this->actor.shape.rot.y))) + MM_Rand_CenteredFloat(40.0f);
            func_800B0DE0(play, &pos, &sDustVelocity, &gZeroVec3f, &MM_sDustPrimColor, &MM_sDustEnvColor, 1000, 150);
        }
    }

    if (MM_SkelAnime_Update(&this->skelAnime)) {
        EnSnowman_SetupThrowSnowball(this);
    } else if (MM_Animation_OnFrame(&this->skelAnime, this->fwork.frameToStartHoldingSnowball)) {
        if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_LARGE) {
...
void EnSnowman_SetupThrowSnowball(EnSnowman* this) {
    if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_LARGE) {
        MM_Animation_PlayOnce(&this->skelAnime, &gEenoLargeSnowballThrowAnim);
        this->fwork.frameToThrowSnowball = 17.0f;
    } else {
        MM_Animation_PlayOnce(&this->skelAnime, &gEenoSmallSnowballThrowAnim);
...
void EnSnowman_ThrowSnowball(EnSnowman* this, PlayState* play) {
    ...
    MM_Math_ApproachS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0xA, 0x1000);
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if ((this->work.snowballsToThrowBeforeIdling != 0) && (MM_Player_GetMask(play) != PLAYER_MASK_STONE) &&
            !(player->stateFlags1 & PLAYER_STATE1_800000)) {
            EnSnowman_SetupReadySnowball(this);
        } else {
    ...
    } else if (MM_Animation_OnFrame(&this->skelAnime, this->fwork.frameToThrowSnowball)) {
        ...
        MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_SNOWMAN, this->snowballPos.x, this->snowballPos.y,
                    this->snowballPos.z, 0, this->actor.yawTowardsPlayer, 0, params);
    }
}

void EnSnowman_SetupIdle(EnSnowman* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &gEenoIdleAnim);
    this->work.timer = 60;
...
void EnSnowman_SetupSubmerge(EnSnowman* this, PlayState* play) {
    MM_Animation_Change(&this->skelAnime, &gEenoEmergeAnim, -1.0f, MM_Animation_GetLastFrame(&gEenoEmergeAnim), 0.0f,
                     ANIMMODE_ONCE, -3.0f);
    EnSnowman_SpawnCircularDustEffect(this, play);
 ...
 */
void EnSnowman_Submerge(EnSnowman* this, PlayState* play) {
    MM_Math_StepToF(&this->actor.scale.y, this->actor.scale.x * 0.4f,
                 (this->actor.scale.x * 0.6f) / MM_Animation_GetLastFrame(&gEenoEmergeAnim));
...
void EnSnowman_SetupMelt(EnSnowman* this) {
    MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 255, COLORFILTER_BUFFLAG_OPA, 50);
    this->collider.base.acFlags &= ~AC_ON;
    this->work.timer = 50;
...
void EnSnowman_Melt(EnSnowman* this, PlayState* play) {
    ...
    if ((this->work.timer >= 38) && !(this->work.timer & 1)) {
        ...
        smokeVelocity.x = MM_Rand_CenteredFloat(1.5f) * smokeVelocity.y;
        smokeVelocity.z = MM_Rand_CenteredFloat(1.5f) * smokeVelocity.y;
        smokeVelocity.y += 0.8f;
        smokePos.x = ((smokeVelocity.x >= 0.0f ? 1.0f : -1.0f) * MM_Rand_ZeroFloat(20.0f) * this->eenoScale) +
                     this->actor.world.pos.x;
        smokePos.z = ((smokeVelocity.z >= 0.0f ? 1.0f : -1.0f) * MM_Rand_ZeroFloat(20.0f) * this->eenoScale) +
                     this->actor.world.pos.z;
        smokePos.y = this->actor.world.pos.y + 3.0f;
        MM_EffectSsIceSmoke_Spawn(play, &smokePos, &smokeVelocity, &gZeroVec3f, this->eenoScale * 300.0f);
    }
    ...
    if (this->work.timer == 0) {
        ...
        if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_SPLIT) {
            ...
            MM_Actor_Kill(this->actor.parent);
...
void EnSnowman_SetupDamaged(EnSnowman* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &gEenoDamageAnim);
...
void EnSnowman_Damaged(EnSnowman* this, PlayState* play) {
    ...
    if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_LARGE) {
        MM_Math_StepToF(&this->actor.scale.y, 1.3f * 0.01f, 0.7f * 0.001f);
    ...
    if (this->work.timer > 0) {
        ...
        MM_Math_Vec3f_Copy(&this->snowballPos, &this->actor.world.pos);
...
void EnSnowman_Dead(EnSnowman* this, PlayState* play) {
    ...
    for (i = 0; i < 15; i++) {
        altitude = MM_Rand_S16Offset(0x1000, 0x3000);
        azimuth = (MM_Rand_Next() >> 0x10);
        speed = MM_Rand_ZeroFloat(2.0f) + 4.0f;
        velocity.x = (speed * MM_Math_CosS(altitude)) * MM_Math_SinS(azimuth);
        velocity.y = speed * MM_Math_SinS(altitude);
        velocity.z = (speed * MM_Math_CosS(altitude)) * MM_Math_CosS(azimuth);
        pos.x = (MM_Rand_ZeroFloat(6.0f) * velocity.x) + this->actor.world.pos.x;
        pos.y = (MM_Rand_ZeroFloat(3.0f) * velocity.y) + this->actor.world.pos.y;
        pos.z = (MM_Rand_ZeroFloat(6.0f) * velocity.z) + this->actor.world.pos.z;
        MM_EffectSsHahen_Spawn(play, &pos, &velocity, &MM_sAccel, 0,
                            MM_Rand_S16Offset((((i % 3) * 20) + 20), (((i % 3) * 10) + 10)), OBJECT_SNOWMAN, 20,
                            sSnowballFragmentDLs[i % 3]);
    }

    func_800B0DE0(play, &this->actor.world.pos, &gZeroVec3f, &gZeroVec3f, &MM_sDustPrimColor, &MM_sDustEnvColor, 500, 30);
    MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 30, NA_SE_EN_YMAJIN_DEAD_BREAK);
    MM_Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, 0x60);
    if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_SPLIT) {
        EnSnowman_SetupSplitDoNothing(this);
    } else {
...
/**
 * Turns off collision on the Eeno before calling MM_Actor_Kill on it.
 */
void EnSnowman_SetupKill(EnSnowman* this) {
...
/**
 * Simply calls MM_Actor_Kill on the Eeno.
 */
void EnSnowman_Kill(EnSnowman* this, PlayState* play) {
    MM_Actor_Kill(&this->actor);
}
...
void EnSnowman_CreateSplitEeno(EnSnowman* this, Vec3f* basePos, s32 yRot) {
    ...
    this->eenoScale = 1.0f;
    this->actor.world.pos.x = (MM_Math_SinS(yRot) * 40.0f) + basePos->x;
    this->actor.world.pos.y = basePos->y;
    this->actor.world.pos.z = (MM_Math_CosS(yRot) * 40.0f) + basePos->z;
 ...
 * MM_zero so it gradually disappears, and its collision is disabled, while the larger Eeno's
...
void EnSnowman_SetupCombine(EnSnowman* this, PlayState* play, Vec3f* combinePos) {
    if (this->actor.colChkInfo.health == 0) {
        ...
        this->actor.flags |= ACTOR_FLAG_UPDATE_CULLING_DISABLED;
        MM_Math_Vec3f_Copy(&this->combinePos, combinePos);
...
void EnSnowman_Combine(EnSnowman* this, PlayState* play) {
    ...
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, MM_Actor_WorldYawTowardPoint(&this->actor, &this->combinePos), 0x1000);
    this->actor.world.rot.y = this->actor.shape.rot.y;
    ...
    if (MM_Actor_WorldDistXZToPoint(&this->actor, &this->combinePos) < 20.0f) {
        this->actor.speed = 0.0f;
    }

    if (MM_Math_StepToF(&this->actor.scale.x, this->fwork.targetScaleDuringCombine, 0.0005f)) {
...
void EnSnowman_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != EnSnowman_SplitDoNothing) {
        ...
        if (this->actionFunc != EnSnowman_Kill) {
            ...
            MM_Actor_UpdateBgCheckInfo(play, &this->actor, 30.0f, wallCheckRadius, 0.0f,
            ...
            if (this->actor.draw == EnSnowman_Draw) {
                MM_Actor_SetFocus(&this->actor, this->actor.scale.y * 1800.0f);
            } else {
                MM_Actor_SetFocus(&this->actor, this->actor.scale.y * 720.0f);
            }
...
void EnSnowman_UpdateSnowball(Actor* thisx, PlayState* play) {
    ...
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) || (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) ||
        ...
        if (EN_SNOWMAN_GET_TYPE(&this->actor) == EN_SNOWMAN_TYPE_SMALL_SNOWBALL) {
            scale = 10;
            for (i = 0; i < 3; i++) {
                MM_EffectSsHahen_SpawnBurst(play, &thisx->world.pos, 5.0f, 0, scale, scale >> 1, 3, OBJECT_SNOWMAN, 20,
                                         sSnowballFragmentDLs[i]);
                scale *= 2;
            }

            func_800B0DE0(play, &this->actor.world.pos, &gZeroVec3f, &gZeroVec3f, &MM_sDustPrimColor, &MM_sDustEnvColor, 500,
                          30);
            MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 20, NA_SE_EV_SMALL_SNOWBALL_BROKEN);
        } else {
            ...
            MM_Math_Vec3f_Copy(&this->snowballPos, &this->actor.world.pos);
            EnSnowman_SpawnBigSnowballFragmentEffects(this, play);
            MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 20, NA_SE_EV_SNOWBALL_BROKEN);
        }

        MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 20, NA_SE_IT_REFLECTION_WOOD);
        MM_Actor_Kill(&this->actor);
        return;
    }
    ...
    Actor_MoveWithGravity(&this->actor);
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 30.0f, this->collider.dim.radius * 0.6f,
...
void EnSnowman_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (sLimbToBodyParts[limbIndex] != BODYPART_NONE) {
        if (sLimbToBodyParts[limbIndex] == EN_SNOWMAN_BODYPART_4) {
            for (i = 0; i < ARRAY_COUNT(sBodyBottomBodyPartOffsets); i++) {
                MM_Matrix_MultVec3f(&sBodyBottomBodyPartOffsets[i], &this->bodyPartsPos[EN_SNOWMAN_BODYPART_4 + i]);
            }
        } else {
    ...
    if ((limbIndex == EENO_LIMB_RIGHT_HAND) && (this->isHoldingSnowball == true)) {
        ...
        if (EN_SNOWMAN_GET_TYPE(thisx) != EN_SNOWMAN_TYPE_LARGE) {
            MM_Matrix_Translate(800.0f, -600.0f, 0.0f, MTXMODE_APPLY);
        } else {
            MM_Matrix_Translate(300.0f, -2300.0f, -1900.0f, MTXMODE_APPLY);
            MM_Matrix_Scale(0.3f, 0.3f, 0.3f, MTXMODE_APPLY);
        }

        MATRIX_FINALIZE_AND_LOAD(&gfx[0], play->state.gfxCtx);
        MM_gSPDisplayList(&gfx[1], sSnowballDLs[EN_SNOWMAN_GET_TYPE(&this->actor)]);
...
void EnSnowman_DrawSnowPile(Actor* thisx, PlayState* play) {
    ...
    Gfx_SetupDL25_Opa(play->state.gfxCtx);
    MM_SkelAnime_DrawFlexOpa(play, this->snowPileSkelAnime.skeleton, this->snowPileSkelAnime.jointTable,
                          this->snowPileSkelAnime.dListCount, NULL, NULL, &this->actor);
}

void EnSnowman_DrawSnowball(Actor* thisx, PlayState* play) {
    MM_Gfx_DrawDListOpa(play, sSnowballDLs[EN_SNOWMAN_GET_TYPE(thisx) - EN_SNOWMAN_TYPE_SMALL_SNOWBALL]);
}
