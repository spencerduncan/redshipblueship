...
static AnimationInfo MM_sAnimationInfo[ENGG2_ANIM_MAX] = {
...
s32 func_80B3AC94(EnGg2* this, PlayState* play) {
    ...
    if ((this->actor.xzDistToPlayer < 250.0f) && (this->actor.xzDistToPlayer > 50.0f) &&
        ...
        MM_Math_SmoothStepToS(&this->unk_2F4, pitch, 4, 0x2AA8, 1);
    } else {
        MM_Math_SmoothStepToS(&this->unk_2F4, 0, 4, 0x2AA8, 1);
    }
...
void func_80B3ADD8(EnGg2* this) {
    ...
    if (temp_v0 >= 3) {
        ...
        this->unk_2EA = 2;
        this->unk_2EC = (s32)(MM_Rand_ZeroOne() * 60.0f) + 20;
    } else {
        this->unk_2EA = 1;
        this->unk_2EC = temp_v0;
    }
    ...
    if (curFrame == endFrame) {
        switch (this->animIndex) {
            case ENGG2_ANIM_0:
                ...
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, ENGG2_ANIM_1);
            ...
            case ENGG2_ANIM_8:
                ...
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, ENGG2_ANIM_5);
                this->actionFunc = func_80B3B120;
                break;

            case ENGG2_ANIM_5:
                this->animIndex = ENGG2_ANIM_6;
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, ENGG2_ANIM_0);
...
void func_80B3B4B0(EnGg2* this, PlayState* play) {
    ...
    for (i = 0; i < 5; i++) {
        sp64.x = (MM_Rand_Centered() * 50.0f) + this->unk_304.x;
        sp64.y = this->unk_304.y;
        sp64.z = (MM_Rand_Centered() * 50.0f) + this->unk_304.z;
...
void func_80B3B8A4(EnGg2* this) {
    ...
    this->actor.shape.yOffset = MM_Math_SinS(this->unk_2F2) * sp1C;
    this->actor.shape.shadowScale = 30.0f - (MM_Math_SinS(this->unk_2F2) * 5.0f);
...
void EnGg2_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx, Gfx** gfx) {
    ...
    if (limbIndex == OBJECT_GG_LIMB_04) {
        MM_Matrix_MultVec3f(&D_80B3C0A0, &this->unk_304);
    }
}
...
void EnGg2_Draw(Actor* thisx, PlayState* play) {
    ...
    if (CHECK_FLAG_ALL(this->actor.flags, ACTOR_FLAG_REACT_TO_LENS) || (this->unk_2F0 == 1)) {
        MM_gSPSegment(POLY_XLU_DISP++, 0x08, Lib_SegmentedToVirtual(D_80B3C0AC[this->unk_2EA]));
