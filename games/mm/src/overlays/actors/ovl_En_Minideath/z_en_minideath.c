...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 1, 15, 30, 10 };
...
void EnMinideath_Init(Actor* thisx, PlayState* play) {
    ...
    Collider_InitAndSetJntSph(play, &this->collider, thisx, &MM_sJntSphInit, this->colliderElements);
    ...
    if (CHECK_EVENTINF(EVENTINF_INTRO_CS_WATCHED_GOMESS)) {
        ...
        for (i = 0; i < MINIDEATH_NUM_EFFECTS; i++) {
            MM_Math_Vec3f_Sum(&thisx->world.pos, &this->effects[i].vel, &this->effects[i].pos);
        }
...
void EnMinideath_DropCollectible(Vec3f* spawnPos, PlayState* play) {
    if (sItemDropTimer == 0 && MM_Rand_ZeroOne() < 0.3f) {
        if (MM_Rand_ZeroOne() < 0.5f) {
            MM_Item_DropCollectible(play, spawnPos, ITEM00_ARROWS_10);
        } else {
            MM_Item_DropCollectible(play, spawnPos, ITEM00_MAGIC_JAR_BIG);
        }
...
void EnMinideath_RandomizeEffects(EnMinideath* this) {
    ...
    for (effect = this->effects, i = 0; i < MINIDEATH_NUM_EFFECTS; i++, effect++) {
        effect->vel.x = MM_Rand_CenteredFloat(80.0f);
        effect->vel.y = MM_Rand_ZeroFloat(40.0f);
        effect->vel.z = MM_Rand_CenteredFloat(80.0f);
        effect->animFrame = (s32)MM_Rand_ZeroFloat(9.0f) % MINIDEATH_ANIM_LENGTH;
        effect->angle.x = (s32)MM_Rand_Next() >> 17;
        effect->angle.y = (s32)MM_Rand_Next() >> 16;
        effect->state = 0;
        effect->timer = 0;
    }
}
...
void EnMinideath_UpdateEffects(EnMinideath* this, PlayState* play) {
    ...
    for (i = 0; i < MINIDEATH_NUM_EFFECTS; i++, effect++) {
        if (effect->state == 0) {
            effect->vel.x += 3.0f * MM_Math_CosS(effect->angle.x) * MM_Math_SinS(effect->angle.y);
            effect->vel.y += 3.0f * MM_Math_SinS(effect->angle.x);
            effect->vel.z += 3.0f * MM_Math_CosS(effect->angle.x) * MM_Math_CosS(effect->angle.y);
            ...
            if (effect->pos.y < this->actor.parent->home.pos.y) {
                ...
                MM_SoundSource_PlaySfxAtFixedWorldPos(play, &effect->pos, 11, NA_SE_EN_EXTINCT);
            ...
            if (effect->timer > 0) {
                ...
                MM_Math_Vec3f_Diff(&this->actor.parent->focus.pos, &this->actor.world.pos, &effect->vel);
                effect->state = 0;
...
void EnMinideath_SetupPreBattle(EnMinideath* this) {
    ...
    this->actor.world.pos.x = MM_Rand_CenteredFloat(600.0f) + this->actor.parent->world.pos.x;
    this->actor.world.pos.z = MM_Rand_CenteredFloat(600.0f) + this->actor.parent->world.pos.z;
    this->actor.shape.rot.y = (s32)MM_Rand_Next() >> 16;
...
void EnMinideath_PreBattle(EnMinideath* this, PlayState* play) {
    if (MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->moveDirection.y, 0x480) &&
        (this->actor.bgCheckFlags & BGCHECKFLAG_WALL)) {
        // Fly away from walls
        this->moveDirection.y = ((s32)MM_Rand_Next() >> 18) + this->actor.wallYaw;
    }
...
void EnMinideath_Intro1(EnMinideath* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->actor.speed, 15.0f, 0.5f);
    ...
    if (this->actor.child == NULL) {
        ...
        phi_v0 = -1;
        MM_Math_Vec3f_Copy(&sp38, &this->actor.child->world.pos);
        angle = 0;
    }
    ...
    if (phi_v0 != -1) {
        ...
        sp38.x = this->actor.parent->world.pos.x + MM_Math_SinS(angle) * parent->coreVelocity;
        sp38.z = this->actor.parent->world.pos.z + MM_Math_CosS(angle) * parent->coreVelocity;
        sp38.y = this->actor.parent->world.pos.y + phi_v0 * 20;
    }
    MM_Math_SmoothStepToS(&this->actor.shape.rot.y, MM_Actor_WorldYawTowardPoint(&this->actor, &sp38), 2, 0x1000, 0x100);
    if (this->actor.parent->scale.z > 0.0f) {
        sp38.y = this->actor.parent->world.pos.y + this->initialHeightDiffFromParent;
    }
    MM_Math_SmoothStepToS(&this->actor.shape.rot.x, MM_Actor_WorldPitchTowardPoint(&this->actor, &sp38), 2, 0x1000, 0x100);
...
void EnMinideath_Intro2(EnMinideath* this, PlayState* play) {
    MM_Actor_WorldDistXYZToPoint(&this->actor, &this->actor.parent->focus.pos);
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y,
                       MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.parent->focus.pos), 0x800);
    MM_Math_ScaledStepToS(&this->actor.shape.rot.x,
                       MM_Actor_WorldPitchTowardPoint(&this->actor, &this->actor.parent->focus.pos), 0x800);
...
void EnMinideath_CrowdParent(EnMinideath* this, PlayState* play) {
    ...
    this->actor.world.pos.x = this->actor.parent->world.pos.x + MM_Math_SinS(yaw) * 60.0f;
    this->actor.world.pos.z = this->actor.parent->world.pos.z + MM_Math_CosS(yaw) * 60.0f;
...
void EnMinideath_Scatter(EnMinideath* this, PlayState* play) {
    ...
    if (MM_Math_ScaledStepToS(&this->actor.shape.rot.x, this->moveDirection.x, 0x480)) {
        y = this->actor.world.pos.y;
        parentY = this->actor.parent->home.pos.y;

        if (y < parentY + 50.0f) {
            this->moveDirection.x = -0x800 - (MM_Rand_Next() >> 20);
...
void EnMinideath_Return(EnMinideath* this, PlayState* play) {
    s32 pad;
    f32 distToParent = MM_Actor_WorldDistXYZToPoint(&this->actor, &this->actor.parent->focus.pos);
    f32 velocityFactor = distToParent * (1.0f / 60.0f);

    if (velocityFactor > 1.0f) {
        velocityFactor = 1.0f;
    }
    MM_Math_StepToF(&this->actor.speed, 6.0f * velocityFactor, 0.5f);
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y,
                       MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.parent->focus.pos), 0x800);
    MM_Math_ScaledStepToS(&this->actor.shape.rot.x,
                       MM_Actor_WorldPitchTowardPoint(&this->actor, &this->actor.parent->focus.pos), 0x800);
...
void EnMinideath_Intro3(EnMinideath* this, PlayState* play) {
    ...
    stepDone =
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->spawnShapeYaw + this->actor.parent->shape.rot.y, 0x800);
    MM_Math_ScaledStepToS(&this->actor.shape.rot.x, 0, 0x800);
    sp26 = this->spawnShapeYaw + this->actor.parent->shape.rot.y;
    target.x = this->actor.parent->world.pos.x + MM_Math_SinS(sp26) * 60.0f;
    target.z = this->actor.parent->world.pos.z + MM_Math_CosS(sp26) * 60.0f;
...
void EnMinideath_SetupSwarmFollower(EnMinideath* this, PlayState* play) {
    ...
    this->actor.world.pos.x = this->actor.parent->focus.pos.x + MM_Math_SinS(this->actor.shape.rot.y) * 60.0f;
    this->actor.world.pos.z = this->actor.parent->focus.pos.z + MM_Math_CosS(this->actor.shape.rot.y) * 60.0f;
    this->actor.world.pos.y = this->actor.parent->focus.pos.y - MM_Math_SinS(this->actor.shape.rot.x) * 60.0f;
...
void EnMinideath_SwarmFollower(EnMinideath* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->actor.speed, 6.0f, 0.2f);
    if (this->timer > 0) {
        this->timer--;
    }
    ...
    if (this->targetActor->id != ACTOR_EN_MINIDEATH) {
        ...
        MM_Math_Vec3f_Copy(&targetFocus, &this->targetActor->focus.pos);
        if (MM_Actor_WorldDistXYZToPoint(&this->actor, &targetFocus) > 200.0f) {
            if (this->number < 10) {
                yawOffset = -0x1C00;
            } else {
                yawOffset = 0x1C00;
        ...
        if (((EnMinideath*)this->targetActor)->actionFunc != EnMinideath_SwarmFollower) {
            MM_Math_Vec3f_Copy(&targetFocus, &this->actor.parent->focus.pos);
        } else {
            MM_Math_Vec3f_Copy(&targetFocus, &this->targetActor->focus.pos);
        }
    }
    MM_Math_SmoothStepToS(&this->actor.shape.rot.y, MM_Actor_WorldYawTowardPoint(&this->actor, &targetFocus) + yawOffset, 5,
                       0xC00, 0x80);
    MM_Math_SmoothStepToS(&this->actor.shape.rot.x, MM_Actor_WorldPitchTowardPoint(&this->actor, &targetFocus), 5, 0xC00,
                       0x80);
    if ((this->collider.base.atFlags & AT_HIT) && this->timer > 0) {
        sNumSwarmHits++;
    }
    if (this->timer == 0 && MM_Actor_WorldDistXYZToPoint(&this->actor, &this->actor.parent->focus.pos) < 50.0f) {
        EnMinideath_SetupIntro3(this);
    }
}
...
void EnMinideath_SwarmLeader(EnMinideath* this, PlayState* play) {
    ...
    target.x = (MM_Math_SinS(this->actor.parent->shape.rot.y) * 20.0f) + this->actor.parent->focus.pos.x;
    target.z = (MM_Math_CosS(this->actor.parent->shape.rot.y) * 20.0f) + this->actor.parent->focus.pos.z;
...
void EnMinideath_Death1(EnMinideath* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->actor.speed, 5.0f, 0.2f);
    if (MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->moveDirection.y, 0x480) &&
        (this->actor.bgCheckFlags & BGCHECKFLAG_WALL)) {
        // Fly away from walls
        this->moveDirection.y = ((s32)MM_Rand_Next() >> 18) + this->actor.wallYaw;
...
void EnMinideath_Die(EnMinideath* this, PlayState* play) {
    ...
    MM_Math_ScaledStepToS(&this->actor.shape.rot.x, this->moveDirection.x, 0x480);
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        ...
        for (effect = this->effects, i = 0; i < MINIDEATH_NUM_EFFECTS; i++, effect++) {
            if (MM_Math_StepToF(&effect->vel.y, 0.0f, 7.0f)) {
                stepsComplete++;
            }
        }
...
void EnMinideath_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != EnMinideath_Dead) {
        ...
        if (this->actionFunc != EnMinideath_CrowdParent && this->actionFunc != EnMinideath_Intro3) {
            MM_Actor_UpdateBgCheckInfo(play, &this->actor, 30.0f, 50.0f, 80.0f, 7);
        }
        ...
        for (i = 0; i != MINIDEATH_NUM_EFFECTS; i++) {
            if (effect->state == 0) {
                MM_Math_Vec3f_Sum(&this->actor.world.pos, &effect->vel, &effect->pos);
