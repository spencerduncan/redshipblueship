...
void ObjMine_Path_MoveToWaypoint(ObjMine* this, s32 index) {
    MM_Math_Vec3s_ToVec3f(&this->actor.world.pos, &this->waypoints[index]);
}

s32 ObjMine_GetUnitVec3f(Vec3f* src, Vec3f* dst) {
    f32 magnitude = MM_Math3D_Vec3fMagnitude(src);
...
void ObjMine_AirWater_SpawnBomb(ObjMine* this, PlayState* play) {
    ...
    EnBom* bomb = (EnBom*)MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_BOM, x, y, z, 0, 0, 0, BOMB_TYPE_BODY);
...
void ObjMine_Air_CheckAC(ObjMine* this, s16* hitAngle, s16* torqueAngle) {
    ...
    MM_Math_Vec3s_ToVec3f(&centerPos, centerPos3s);
    yawToAttack = MM_Math_Vec3f_Yaw(&attackActor->world.pos, &centerPos);
    ...
    if (this->collider.elements[0].base.acHitElem->atDmgInfo.dmgFlags & 0x13820) {
        ...
        *torqueAngle = attackActor->shape.rot.y - yawToAttack;
        ...
        MM_Math_Vec3s_ToVec3f(&hitPos, hitPos3s);
        *hitAngle = MM_Actor_WorldYawTowardActor(attackActor, &this->actor);
        *torqueAngle = MM_Math_Vec3f_Yaw(&attackActor->world.pos, &hitPos) - yawToAttack;
    }
}
...
void ObjMine_Water_CheckAC(ObjMine* this, Vec3f* knockbackDir) {
    ...
    if (this->collider.elements[0].base.acHitElem->atDmgInfo.dmgFlags & 0x13820) {
        ...
        if (!ObjMine_GetUnitVec3f(&posDiff, knockbackDir)) {
            MM_Math_Vec3f_Copy(knockbackDir, &sStandardBasis.y);
        }
    }
}
...
s32 ObjMine_StepUntilParallel(Vec3f* value, Vec3f* target, f32 angleStep) {
    ...
    Vec3f prevValue;
    Vec3f perpNormal;
    f32 cosAngle = MM_Math3D_Cos(value, target);

    if (MM_Math_CosF(angleStep) <= cosAngle) {
        MM_Math_Vec3f_Copy(value, target);
        return true;
    }

    MM_Matrix_Push();
    MM_Math_Vec3f_Copy(&prevValue, value);
    MM_Math3D_Vec3f_Cross(value, target, &perpVec);
    if (ObjMine_GetUnitVec3f(&perpVec, &perpNormal)) {
        Matrix_RotateAxisS(RAD_TO_BINANG(angleStep), &perpNormal, MTXMODE_NEW);
        MM_Matrix_MultVec3f(&prevValue, value);
    } else {
...
void ObjMine_Air_SetBasis(ObjMine* this) {
    ...
    MM_Math3D_Vec3f_Cross(&sStandardBasis.x, &airChain->basis.y, &tempVec);
    ObjMine_GetUnitVec3f(&tempVec, &airChain->basis.z);

    MM_Math3D_Vec3f_Cross(&airChain->basis.y, &airChain->basis.z, &tempVec);
...
void ObjMine_Water_InitChain(ObjMine* this, s32 linkCount) {
    ...
    waterChain->swayPhaseVel = MM_Rand_Next() >> (0x20 - 13);
    ...
    for (i = 0, waterLink = waterChain->links; i < linkCount; i++, waterLink++) {
        ...
        waterLink->velocity.x = (MM_Rand_ZeroOne() - 0.5f) * swayVel;
        waterLink->velocity.y = (MM_Rand_ZeroOne() - 0.5f) * swayVel;
        waterLink->velocity.z = (MM_Rand_ZeroOne() - 0.5f) * swayVel;
    }
...
void ObjMine_Water_InitCollider(ObjMine* this, s32 linkCount) {
    MM_Matrix_Translate(this->actor.home.pos.x, this->actor.home.pos.y + (linkCount * LINK_SIZE) + ATTACH_OFFSET,
...
void ObjMine_Water_SetWorld(ObjMine* this) {
    ...
    if (linkCount == 0) {
        ...
        MM_Math_Vec3f_Sum(&lastLink->pos, &mineOffset, &this->actor.world.pos);
    }
}
...
void ObjMine_Water_WallCheck(ObjMine* this, PlayState* play) {
    ...
    if (waterChain->wallCheckDistSq > -1e-6f) {
        ...
        if (waterChain->wallCheckDistSq <= MM_Math3D_Dist2DSq(this->actor.home.pos.x, this->actor.home.pos.z,
            ...
            if (ObjMine_GetUnitVec3fNorm(&xzFromHome, &xzDirFromHome, &norm, &invNorm)) {
                ...
                if (MM_BgCheck_EntityLineTest1(&play->colCtx, &centerPos, &offsetPos, &result, &poly, true, false, false,
                                            true, &bgId)) {
...
void ObjMine_Water_ApplyForces(ObjMine* this) {
    ...
    for (i = 0, waterLink = waterChain->links, lastLinkAccel = sLastLinkAccel; i < linkCount;
        ...
        MM_Math_Vec3f_Copy(lastLinkAccel, &waterLink->accel);
        ...
        waterLink->accel.x += waterChain->swayXZ * MM_Math_SinS(swayPhase);
        waterLink->accel.y += waterChain->swayY;
        waterLink->accel.z += waterChain->swayXZ * MM_Math_CosS(swayPhase);
    }
    ...
    // frame.
    for (i = 0, waterLink = waterChain->links; i < linkCount; i++, waterLink++) {
        MM_Math_Vec3f_Copy(&tension, &gZeroVec3f);
...
void ObjMine_Water_UpdateLinks(ObjMine* this) {
    ...
    for (i = 0, prevBasisX = NULL, waterLink = waterChain->links; i < linkCount;
        ...
        MM_Math_Vec3f_Scale(&waterLink->velocity, waterChain->drag);
        MM_Math_Vec3f_Copy(&unusedLinkPos, &waterLink->pos);
        ...
        MM_Math_Vec3f_Diff(&waterLink->pos, &jointPos, &tempVec);

        if (ObjMine_GetUnitVec3fNorm(&tempVec, &diffDir, &diffNorm, &invNorm) && (diffNorm > LINK_SIZE / 3.0f)) {
            MM_Math_Vec3f_Copy(&newBasis.y, &waterLink->basis.y);
            ObjMine_StepUntilParallel(&newBasis.y, &diffDir, M_PIf / 30);

            tempBasisX = (prevBasisX == NULL) ? &sStandardBasis.x : prevBasisX;

            MM_Math3D_Vec3f_Cross(tempBasisX, &newBasis.y, &tempVec);

            // Skips change of basis if any of the basis vectors would be MM_zero.
            if (ObjMine_GetUnitVec3f(&tempVec, &newBasis.z)) {
                MM_Math3D_Vec3f_Cross(&newBasis.y, &newBasis.z, &tempVec);
        ...
        if (changeBasis) {
            MM_Math_Vec3f_Copy(&waterLink->basis.x, &newBasis.x);
            MM_Math_Vec3f_Copy(&waterLink->basis.y, &newBasis.y);
            MM_Math_Vec3f_Copy(&waterLink->basis.z, &newBasis.z);
        } else {
...
void ObjMine_Init(Actor* thisx, PlayState* play) {
    ...
    if (type == OBJMINE_TYPE_PATH) {
        MM_ActorShape_Init(&this->actor.shape, 0.0f, MM_ActorShadow_DrawCircle, 45.0f);
        ...
        MM_Matrix_Scale(this->actor.scale.x, this->actor.scale.y, this->actor.scale.z, MTXMODE_APPLY);
        MM_Collider_UpdateSpheres(0, &this->collider);
        this->actor.floorHeight = MM_BgCheck_EntityRaycastFloor5(&play->colCtx, &this->actor.floorPoly, &bgId,
...
void ObjMine_Path_Move(ObjMine* this, PlayState* play) {
    ...
    MM_Math_Vec3s_ToVec3f(&nextWaypoint, &this->waypoints[this->waypointIndex + 1]);
    MM_Math_Vec3f_Diff(&nextWaypoint, &thisx->world.pos, &thisx->velocity);
    distToWaypoint = MM_Math3D_Vec3fMagnitude(&thisx->velocity);
    ...
    if ((thisx->speed + 0.05f) < distToWaypoint) {
        ...
        MM_Math_Vec3f_Scale(&thisx->velocity, thisx->speed / distToWaypoint);
    ...
    thisx->floorHeight = MM_BgCheck_EntityRaycastFloor5(&play->colCtx, &thisx->floorPoly, &bgId, thisx, &thisx->world.pos);
    if (thisx->flags & ACTOR_FLAG_INSIDE_CULLING_VOLUME) {
        ...
        MM_Math3D_Vec3f_Cross(&sStandardBasis.y, &thisx->velocity, &yhatCrossV);
...
void ObjMine_Air_Chained(ObjMine* this, PlayState* play) {
    ...
    MM_Math_Vec3f_Copy(&airChain->translation, &airChain->basis.y);
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        torque = MM_Math_SinS(torqueAngle) * 0x96;
    ...
    if (airChain->knockback > 0.0001f) {
        xAccel = MM_Math_SinS(airChain->knockbackAngle) * airChain->knockback;
        zAccel = MM_Math_CosS(airChain->knockbackAngle) * airChain->knockback;
        airChain->velocity.x += xAccel;
        airChain->velocity.z += zAccel;
        MM_Math_StepToF(&airChain->knockback, 0.0f, AIR_KNOCKBACK * 0.5f);
    }

    // Applies chain sway, choosing a new sway randomly about every 32 frames
    if ((MM_Rand_Next() >> (0x20 - 5)) == 0) {
        airChain->swaySize = MM_Rand_ZeroOne() * airChain->swayMax;
        airChain->swayPhase = MM_Rand_Next() >> (0x20 - 16);
    }
    xAccel = MM_Math_SinS(airChain->swayPhase) * airChain->swaySize;
    zAccel = MM_Math_CosS(airChain->swayPhase) * airChain->swaySize;
    airChain->velocity.x += xAccel;
    airChain->velocity.z += zAccel;
    ...
    // speed. If speed is close to MM_zero when hitting wall, weakly eject it instead.
    if (airChain->wallCheckDistSq > -1e-6f) {
        if (airChain->wallCheckDistSq <= MM_Math3D_Dist2DSq(this->actor.world.pos.x, this->actor.world.pos.z,
                                                         this->actor.home.pos.x, this->actor.home.pos.z)) {

            MM_Actor_UpdateBgCheckInfo(play, &this->actor, 0.0f, AIR_RADIUS, 0.0f, UPDBGCHECKINFO_FLAG_1);
            ...
            if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) && (this->actor.wallPoly != NULL)) {
                ...
                if (ObjMine_GetUnitVec3fNorm(&xzVel, &xzDir, &xzSpeed, &invNorm)) {
                    ...
                    MM_Math3D_Vec3fReflect(&xzDir, &wallNormal, &reflectedDir);
    ...
    for (i = 0, airLink = airChain->links; i < linkCount; i++, airLink++) {
        ...
        spin = airLink->spin + (twistDiff * -0.05f) + (30.0f * MM_Rand_ZeroOne() - 15.0f);
        spin *= 0.995f;
        airLink->spin = spin;
...
void ObjMine_Water_Chained(ObjMine* this, PlayState* play) {
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        MM_Math_Vec3f_Scale(&waterChain->knockbackDir, WATER_KNOCKBACK);
    }
    ...
    if (MM_Rand_Next() >> (0x20 - 5) == 0) {
        s16 randAngle = MM_Rand_Next() >> (0x20 - 16);

        waterChain->swayXZ = MM_Math_SinS(randAngle) * 1.8f * waterChain->swayMax;
        waterChain->swayY = MM_Math_CosS(randAngle) * 0.2f * waterChain->swayMax;
        waterChain->swayPhaseVel = MM_Rand_Next() >> (0x20 - 13);
    }
