...
static AnimationInfo MM_sAnimationInfo[GINKO_ANIM_MAX] = {
...
void EnGinkoMan_Init(Actor* thisx, PlayState* play) {
    ...
    this->serviceFee = 0;
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &object_boj_Skel_00C240, &object_boj_Anim_0043F0, this->jointTable,
...
void EnGinkoMan_SetupIdle(EnGinkoMan* this) {
    ...
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_SITTING);
    ...
    if (Actor_TalkOfferAccepted(&this->actor, &play->state)) {
        if (GameInteractor_Should(VB_CONTINUE_BANKER_DIALOGUE, true, this)) {
            if (HS_GET_BANK_RUPEES() == 0) {
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_LEGSMACKING);
                MM_Message_StartTextbox(play, 0x44C, &this->actor);
                this->curTextId = 0x44C; // would you like to make an account
            } else {
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_SITTING);
                if ((CURRENT_DAY == 3) && (gSaveContext.save.isNight == true)) {
                    MM_Message_StartTextbox(play, 0x467, &this->actor);
                    this->curTextId = 0x467;
                } else {
                    MM_Message_StartTextbox(play, 0x466, &this->actor);
    ...
    switch (this->curTextId) {
        case 0x44C:
            ...
            if (CHECK_WEEKEVENTREG(WEEKEVENTREG_RECEIVED_BANK_WALLET_UPGRADE)) {
                MM_Message_StartTextbox(play, 0x44E, &this->actor);
                this->curTextId = 0x44E;
            } else {
                MM_Message_StartTextbox(play, 0x44D, &this->actor);
        ...
        case 0x44F:
            Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_LEGSMACKING);
            MM_Message_StartTextbox(play, 0x450, &this->actor);
        ...
        case 0x455: // you deposited a lot
            if (this->isNewAccount == true) {
                ...
                MM_Message_StartTextbox(play, 0x461, &this->actor);
                ...
                MM_Message_StartTextbox(play, 0x45A, &this->actor);
        ...
        case 0x45A:
            if (GameInteractor_Should(VB_PASS_FIRST_BANK_THRESHOLD,
                ...
                MM_Message_StartTextbox(play, 0x45B, &this->actor);
                ...
                MM_Message_StartTextbox(play, 0x45C, &this->actor);
                ...
                if (GameInteractor_Should(VB_PASS_SECOND_BANK_THRESHOLD_ALT,
                    ...
                    MM_Message_StartTextbox(play, 0x45D, &this->actor);
                    this->curTextId = 0x45D;
                } else if (this->previousBankValue < (s16)HS_GET_BANK_RUPEES()) {
                    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_SITTING);
                    MM_Message_StartTextbox(play, 0x45E, &this->actor);
                    this->curTextId = 0x45E;
                } else {
                    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_LEGSMACKING);
                    MM_Message_StartTextbox(play, 0x460, &this->actor);
        ...
        case 0x461:
            MM_Message_StartTextbox(play, 0x462, &this->actor);
            this->curTextId = 0x462;
            break;

        case 0x462:
            MM_Message_StartTextbox(play, 0x463, &this->actor);
            this->curTextId = 0x463;
            break;

        case 0x463:
            MM_Message_StartTextbox(play, 0x464, &this->actor);
        ...
        case 0x466:
        case 0x467:
            MM_Message_StartTextbox(play, 0x468, &this->actor);
        ...
        case 0x47E:
            if (this->choiceDepositWithdrawl == GINKOMAN_CHOICE_DEPOSIT) {
                if (HS_GET_BANK_RUPEES() >= 5000) {
                    MM_Message_StartTextbox(play, 0x45F, &this->actor);
                    this->curTextId = 0x45F;
                } else if (gSaveContext.save.saveInfo.playerData.rupees == 0) {
                    MM_Message_StartTextbox(play, 0x458, &this->actor);
                    this->curTextId = 0x458;
                } else {
                    MM_Message_StartTextbox(play, 0x479, &this->actor);
                    this->curTextId = 0x479;
                }
            } else if ((CURRENT_DAY == 3) && (gSaveContext.save.isNight == true)) {
                MM_Message_StartTextbox(play, 0x46D, &this->actor);

                this->curTextId = 0x46D;
            } else { // GINKOMAN_CHOICE_WITHDRAWL
                MM_Message_StartTextbox(play, 0x46B, &this->actor);
        ...
        case 0x46B:
            MM_Message_StartTextbox(play, 0x46E, &this->actor);
        ...
        case 0x474:
            if (HS_GET_BANK_RUPEES() == 0) {
                MM_Message_StartTextbox(play, 0x478, &this->actor);
        ...
        case 0x477:
            MM_Message_StartTextbox(play, 0x471, &this->actor);
            this->curTextId = 0x471;
            this->serviceFee = play->msgCtx.unk1206C;
            break;

        case 0x479:
            MM_Message_StartTextbox(play, 0x44F, &this->actor);
    ...
    switch (this->curTextId) {
        case 0x44E:
            if (play->msgCtx.choiceIndex == GINKOMAN_CHOICE_YES) {
                ...
                MM_Message_StartTextbox(play, 0x451, &this->actor);
        ...
        case 0x452:
            if (play->msgCtx.choiceIndex == GINKOMAN_CHOICE_YES) {
                if (gSaveContext.save.saveInfo.playerData.rupees < play->msgCtx.rupeesSelected) {
                    Audio_PlaySfx(NA_SE_SY_ERROR);
                    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_SITTING);
                    MM_Message_StartTextbox(play, 0x459, &this->actor);
                    this->curTextId = 0x459;
                } else {
                    Audio_PlaySfx_MessageDecide();
                    if (play->msgCtx.rupeesSelected >= 100) {
                        MM_Message_StartTextbox(play, 0x455, &this->actor);
                        this->curTextId = 0x455;
                    } else if (play->msgCtx.rupeesSelected >= 10) {
                        MM_Message_StartTextbox(play, 0x454, &this->actor);
                        this->curTextId = 0x454;
                    } else {
                        Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_SITTING);
                        MM_Message_StartTextbox(play, 0x453, &this->actor);
                    ...
                    MM_Rupees_ChangeBy(-play->msgCtx.rupeesSelected);
                ...
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_SITTING);
                if (HS_GET_BANK_RUPEES() == 0) {
                    MM_Message_StartTextbox(play, 0x456, &this->actor);
                    this->curTextId = 0x456;
                } else {
                    MM_Message_StartTextbox(play, 0x47D, &this->actor);
        ...
        case 0x468:
            if (play->msgCtx.choiceIndex == GINKOMAN_CHOICE_CANCEL) {
                Audio_PlaySfx_MessageCancel();
                MM_Message_StartTextbox(play, 0x470, &this->actor);
                ...
                if (!this->isStampChecked) {
                    ...
                    MM_Message_StartTextbox(play, 0x469, &this->actor);
                    this->curTextId = 0x469;
                } else {
                    MM_Message_StartTextbox(play, 0x47E, &this->actor);
        ...
        case 0x471:
            if (play->msgCtx.choiceIndex == GINKOMAN_CHOICE_YES) {
                if ((s32)HS_GET_BANK_RUPEES() < (play->msgCtx.rupeesSelected + this->serviceFee)) {
                    Audio_PlaySfx(NA_SE_SY_ERROR);
                    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_LEGSMACKING);
                    MM_Message_StartTextbox(play, 0x476, &this->actor);
                    ...
                    Audio_PlaySfx(NA_SE_SY_ERROR);
                    MM_Message_StartTextbox(play, 0x475, &this->actor);
                    this->curTextId = 0x475;
                } else {
                    Audio_PlaySfx_MessageDecide();
                    if (play->msgCtx.rupeesSelected >= 100) {
                        MM_Message_StartTextbox(play, 0x474, &this->actor);
                        this->curTextId = 0x474;
                    } else if (play->msgCtx.rupeesSelected >= 10) {
                        MM_Message_StartTextbox(play, 0x473, &this->actor);
                        this->curTextId = 0x473;
                    } else {
                        MM_Message_StartTextbox(play, 0x472, &this->actor);
                ...
                Audio_PlaySfx_MessageCancel();
                MM_Message_StartTextbox(play, 0x47C, &this->actor);
        ...
        switch (this->curTextId) {
            case 0x450:
                if (play->msgCtx.rupeesSelected == 0) {
                    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_SITTING);
                    MM_Message_StartTextbox(play, 0x457, &this->actor);
                    this->curTextId = 0x457;
                } else {
                    MM_Message_StartTextbox(play, 0x452, &this->actor);
            ...
            case 0x46E:
                if (play->msgCtx.rupeesSelected == 0) {
                    MM_Message_StartTextbox(play, 0x46F, &this->actor);
                    this->curTextId = 0x46F;
                } else if (gSaveContext.save.isNight == true) {
                    MM_Message_StartTextbox(play, 0x477, &this->actor);
...
void EnGinkoMan_BankAward(EnGinkoMan* this, PlayState* play) {
    ...
    if (MM_Actor_HasParent(&this->actor, play)) {
        ...
        if (!CHECK_WEEKEVENTREG(WEEKEVENTREG_RECEIVED_BANK_WALLET_UPGRADE)) {
            MM_Actor_OfferGetItem(&this->actor, play, GI_WALLET_ADULT + CUR_UPG_VALUE(UPG_WALLET), 500.0f, 100.0f);
        } else {
            MM_Actor_OfferGetItem(&this->actor, play, GI_RUPEE_BLUE, 500.0f, 100.0f);
        }
...
void EnGinkoMan_BankAward2(EnGinkoMan* this, PlayState* play) {
    if (Actor_TalkOfferAccepted(&this->actor, &play->state)) {
        if (!CHECK_WEEKEVENTREG(WEEKEVENTREG_RECEIVED_BANK_WALLET_UPGRADE) && (this->curTextId == 0x45B)) {
            ...
            MM_Message_StartTextbox(play, 0x47A, &this->actor);
            this->curTextId = 0x47A; // Message after receiving reward for depositing 200 rupees.
        } else {
            Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_SITTING);
            MM_Message_StartTextbox(play, 0x47B, &this->actor);
...
void EnGinkoMan_SetupStamp(EnGinkoMan* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_REACHING);
    this->actionFunc = EnGinkoMan_Stamp;
}

void EnGinkoMan_Stamp(EnGinkoMan* this, PlayState* play) {
    if ((this->curTextId == 0x464) && MM_Animation_OnFrame(&this->skelAnime, 10.0f)) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
        switch (this->curTextId) {
            case 0x464:
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_SITTING);
                MM_Message_StartTextbox(play, 0x465, &this->actor);
            ...
            case 0x469:
                ...
                play->msgCtx.rupeesTotal = HS_GET_BANK_RUPEES();
                if ((CURRENT_DAY == 3) && (gSaveContext.save.isNight == true)) {
                    MM_Message_StartTextbox(play, 0x46C, &this->actor);
                    this->curTextId = 0x46C;
                } else {
                    MM_Message_StartTextbox(play, 0x46A, &this->actor);
    ...
    if (this->actor.xzDistToPlayer > 160.0f) {
        if (this->animTimer == 0) {
            if (this->skelAnime.animation != &object_boj_Anim_004A7C) {
                this->animTimer = 40;
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_ADVERTISING);
            }
        }
    } else if ((this->animTimer == 0) && (this->skelAnime.animation != &object_boj_Anim_000AC4)) {
        this->animTimer = 40;
        Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, GINKO_ANIM_AMAZED);
    }
