...
void MM_EnLight_Init(Actor* thisx, PlayState* play);
void MM_EnLight_Destroy(Actor* thisx, PlayState* play);
void MM_EnLight_Update(Actor* thisx, PlayState* play);
void MM_EnLight_Draw(Actor* thisx, PlayState* play);
...
void MM_EnLight_Init(Actor* thisx, PlayState* play) {
    ...
    if (!ENLIGHT_GET_4000(&this->actor)) {
        if ((gSaveContext.gameMode == GAMEMODE_END_CREDITS) || ENLIGHT_GET_2000(&this->actor)) {
            MM_Lights_PointNoGlowSetInfo(&this->lightInfo, this->actor.world.pos.x,
                                      ((this->actor.params < 0) ? 1 : 40) + (s32)this->actor.world.pos.y,
                                      this->actor.world.pos.z, 255, 255, 180, -1);
        } else {
            MM_Lights_PointGlowSetInfo(&this->lightInfo, this->actor.world.pos.x,
                                    ((this->actor.params < 0) ? 1 : 40) + (s32)this->actor.world.pos.y,
                                    this->actor.world.pos.z, 255, 255, 180, -1);
        }
    ...
    MM_Actor_SetScale(&this->actor, D_808666D0[ENLIGHT_GET_F(&this->actor)].unk_07 * 0.0001f);

    this->unk_144 = (s8)(MM_Rand_ZeroOne() * 255.0f);

    if (ENLIGHT_GET_800(&this->actor)) {
        this->actor.update = func_80865F38;
        if (ENLIGHT_GET_1000(&this->actor) && MM_Flags_GetSwitch(play, ENLIGHT_SWITCH_FLAG(&this->actor))) {
...
void func_80865BF8(EnLight* this, PlayState* play) {
    ...
    if (this->actor.parent != NULL) {
        MM_Math_Vec3f_Copy(&this->actor.world.pos, &this->actor.parent->world.pos);
...
void MM_EnLight_Update(Actor* thisx, PlayState* play) {
    ...
    if (!ENLIGHT_GET_4000(&this->actor)) {
        ...
        f32 temp_f2 = (MM_Rand_ZeroOne() * 0.5f) + 0.5f;
        s32 radius = (this->actor.params < 0) ? 100 : ENLIGHT_GET_2000(&this->actor) ? 730 : 300;

        MM_Lights_PointSetColorAndRadius(&this->lightInfo, (u8)(sp28->unk_00.r * temp_f2), (u8)(sp28->unk_00.g * temp_f2),
...
void func_80865F38(Actor* thisx, PlayState* play) {
    ...
    if (ENLIGHT_GET_1000(&this->actor)) {
        if (MM_Flags_GetSwitch(play, ENLIGHT_SWITCH_FLAG(&this->actor))) {
            MM_Math_StepToF(&sp30, 1.0f, 0.05f);
            sp2C = true;
        } else {
            if (sp30 < 0.1f) {
                MM_Actor_SetScale(&this->actor, 0.0f);
            ...
            MM_Math_StepToF(&sp30, 0.0f, 0.05f);
        }
    } else if (MM_Flags_GetSwitch(play, ENLIGHT_SWITCH_FLAG(&this->actor))) {
        if (sp30 < 0.1f) {
            MM_Actor_SetScale(&this->actor, 0.0f);
            sp30 = 0.0f;
        }
    ...
    MM_Actor_SetScale(&this->actor, sp38->unk_07 * 0.0001f * sp30);

    if (!ENLIGHT_GET_4000(&this->actor)) {
        temp_f2 = (MM_Rand_ZeroOne() * 0.5f) + 0.5f;
        MM_Lights_PointSetColorAndRadius(&this->lightInfo, (u8)(sp38->unk_00.r * temp_f2), (u8)(sp38->unk_00.g * temp_f2),
...
void MM_EnLight_Draw(Actor* thisx, PlayState* play) {
    ...
    Matrix_RotateYS(BINANG_ROT180(MM_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) - this->actor.shape.rot.y), MTXMODE_APPLY);
