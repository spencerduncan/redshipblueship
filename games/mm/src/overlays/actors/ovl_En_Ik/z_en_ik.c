...
void MM_EnIk_Init(Actor* thisx, PlayState* play);
void MM_EnIk_Destroy(Actor* thisx, PlayState* play);
void MM_EnIk_Update(Actor* thisx, PlayState* play2);
void MM_EnIk_Draw(Actor* thisx, PlayState* play);
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 18, 25, 80, MASS_HEAVY };
...
void MM_EnIk_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &gIronKnuckleSkel, &gIronKnuckleWalkAnim, this->jointTable,
    ...
    Collider_InitAndSetTris(play, &this->colliderTris, &this->actor, &MM_sTrisInit, this->shieldColliderElements);
    Collider_InitAndSetQuad(play, &this->colliderQuad, &this->actor, &MM_sQuadInit);
    MM_CollisionCheck_SetInfo(&this->actor.colChkInfo, &sDamageTableArmor, &MM_sColChkInfoInit);
...
void EnIk_HitArmor(EnIk* this, PlayState* play) {
    ...
    MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_CLEAR_TAG, this->colliderCylinder.elem.acDmgInfo.hitPos.x,
...
s32 EnIk_ChooseAttack(EnIk* this) {
    ...
    if ((this->actor.xzDistToPlayer < 100.0f) && (fabsf(this->actor.playerHeightRel) < 150.0f)) {
        ...
        if (detectionThreshold >= absYawDiff) {
            ...
            return true;
        } else if ((this->drawArmorFlags) || ((absYawDiff > 0x4000) && (MM_Rand_ZeroOne() < 0.1f))) {
...
void EnIk_SetupIdle(EnIk* this) {
    f32 endFrame = MM_Animation_GetLastFrame(&gIronKnuckleHorizontalAttackAnim);
    ...
    MM_Animation_Change(&this->skelAnime, &gIronKnuckleEndHorizontalAttackAnim, 1.0f, endFrame, endFrame, ANIMMODE_ONCE,
                     this->timer);
...
void EnIk_SetupWalk(EnIk* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gIronKnuckleWalkAnim, -4.0f);
...
void EnIk_WalkTowardsPlayer(EnIk* this, PlayState* play) {
    ...
    MM_SkelAnime_Update(&this->skelAnime);
    if (MM_Animation_OnFrame(&this->skelAnime, 0.0f) || MM_Animation_OnFrame(&this->skelAnime, 16.0f)) {
    ...
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) && (ABS_ALT(yawDiff) >= 0x4000)) {
        ...
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, yawDiff, 0x320);
    } else {
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x320);
    }
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnIk_SetupRun(EnIk* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gIronKnuckleRunAnim, -4.0f);
...
void EnIk_RunTowardsPlayer(EnIk* this, PlayState* play) {
    ...
    MM_SkelAnime_Update(&this->skelAnime);
    if (MM_Animation_OnFrame(&this->skelAnime, 2.0f) || MM_Animation_OnFrame(&this->skelAnime, 9.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_IRONNACK_WALK);
    }
    yawDiff = this->actor.wallYaw - this->actor.shape.rot.y;
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) && (ABS_ALT(yawDiff) >= 0x4000)) {
        yawDiff = (this->actor.yawTowardsPlayer > 0) ? this->actor.wallYaw - 0x4000 : this->actor.wallYaw + 0x4000;
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, yawDiff, 0x4B0);
    } else {
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x4B0);
    }
    this->actor.world.rot.y = this->actor.shape.rot.y;
    if (!EnIk_IsChangingAction(this, play)) {
        EnIk_ChooseAttack(this);
...
void EnIk_SetupVerticalAttack(EnIk* this) {
    ...
    MM_Animation_Change(&this->skelAnime, &gIronKnuckleVerticalAttackAnim, playbackSpeed, 0.0f,
                     MM_Animation_GetLastFrame(&gIronKnuckleVerticalAttackAnim), ANIMMODE_ONCE_INTERP, -4.0f);
    this->timer = 0;
...
void EnIk_VerticalAttack(EnIk* this, PlayState* play) {
    ...
    if (!(this->skelAnime.curFrame < 7.0f) || !EnIk_IsChangingAction(this, play)) {
        if (MM_Animation_OnFrame(&this->skelAnime, 15.0f)) {
            ...
            particlePos.x = this->actor.world.pos.x + (MM_Math_SinS(this->actor.shape.rot.y + 0x6A4) * 70.0f);
            particlePos.z = this->actor.world.pos.z + (MM_Math_CosS(this->actor.shape.rot.y + 0x6A4) * 70.0f);
            particlePos.y = this->actor.world.pos.y;
            Actor_PlaySfx(&this->actor, NA_SE_EN_IRONNACK_HIT_GND);
            MM_Camera_AddQuake(GET_ACTIVE_CAM(play), 2, 25, 5);
            Rumble_Request(this->actor.xyzDistToPlayerSq, 180, 20, 100);
            MM_CollisionCheck_SpawnShieldParticles(play, &particlePos);
        }

        if ((this->skelAnime.curFrame > 13.0f) && (this->skelAnime.curFrame < 23.0f)) {
            this->colliderQuad.base.atFlags |= AT_ON;
            if (this->drawArmorFlags != 0) {
                this->actor.speed = MM_Math_SinF((this->skelAnime.curFrame - 13.0f) * (M_PIf / 20)) * 10.0f;
            }
        } else {
            this->colliderQuad.base.atFlags &= ~AT_ON;
            this->actor.speed = 0.0f;
        ...
        if ((this->drawArmorFlags) && (this->skelAnime.curFrame < 13.0f)) {
            MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x5DC);
            this->actor.world.rot.y = this->actor.shape.rot.y;
        }
...
void EnIk_SetupTakeOutAxe(EnIk* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gIronKnuckleEndVerticalAttackAnim);
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if (this->skelAnime.animation == &gIronKnuckleRecoverVerticalAttackAnim) {
            ...
            MM_Animation_Change(
                ...
                MM_Animation_GetLastFrame(&gIronKnuckleRecoverVerticalAttackAnim), ANIMMODE_ONCE_INTERP, 0.0f);
        }
    }
}
...
void EnIk_SetupHorizontalDoubleAttack(EnIk* this) {
    this->actor.speed = 0.0f;
    MM_Animation_Change(&this->skelAnime, &gIronKnuckleHorizontalAttackAnim, (this->drawArmorFlags) ? 1.3f : 1.0f, 0.0f,
                     MM_Animation_GetLastFrame(&gIronKnuckleHorizontalAttackAnim), ANIMMODE_ONCE_INTERP,
...
void EnIk_HorizontalDoubleAttack(EnIk* this, PlayState* play) {
    ...
    this->timer++;
    if (MM_Animation_OnFrame(&this->skelAnime, 1.0f) || MM_Animation_OnFrame(&this->skelAnime, 13.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_IRONNACK_SWING_AXE);
    }
    if (((this->skelAnime.curFrame > 1.0f) && (this->skelAnime.curFrame < 9.0f)) ||
        ...
        if (this->drawArmorFlags != 0) {
            ...
            this->actor.speed = MM_Math_SinF((M_PIf / 8) * phi_f2) * 4.5f;
        }
        this->colliderQuad.base.atFlags |= AT_ON;
    } else {
        this->colliderQuad.base.atFlags &= ~AT_ON;
...
void EnIk_SetupSingleHorizontalAttack(EnIk* this) {
    ...
    MM_Animation_Change(&this->skelAnime, &gIronKnuckleHorizontalAttackAnim, playSpeed, 12.0f,
                     MM_Animation_GetLastFrame(&gIronKnuckleHorizontalAttackAnim), ANIMMODE_ONCE_INTERP, 5.0f);
    this->timer = 0;
    this->blurEffectSpawnLock = -1;
...
void EnIk_SetupEndHorizontalAttack(EnIk* this) {
    MM_Animation_Change(&this->skelAnime, &gIronKnuckleEndHorizontalAttackAnim, (this->drawArmorFlags) ? 2.0f : 1.0f, 0.0f,
                     MM_Animation_GetLastFrame(&gIronKnuckleHorizontalAttackAnim), ANIMMODE_ONCE_INTERP, -4.0f);
...
void EnIk_SetupBlock(EnIk* this) {
    this->actor.speed = 0.0f;
    MM_Animation_Change(&this->skelAnime, &gIronKnuckleBlockAnim, 2.0f, 0.0f,
                     MM_Animation_GetLastFrame(&gIronKnuckleBlockAnim), ANIMMODE_ONCE, -2.0f);
    this->timer = 20;
...
void EnIk_SetupReactToAttack(EnIk* this, s32 arg1) {
    ...
    if (ABS_ALT(temp_v0) <= 0x4000) {
        MM_Animation_MorphToPlayOnce(&this->skelAnime, &gIronKnuckleFrontHitAnim, -4.0f);
    } else {
        MM_Animation_MorphToPlayOnce(&this->skelAnime, &gIronKnuckleBackHitAnim, -4.0f);
    }
...
void EnIk_SetupDie(EnIk* this) {
    this->actor.speed = 0.0f;
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gIronKnuckleDeathAnim, -4.0f);
    this->timer = 24;
...
void EnIk_Die(EnIk* this, PlayState* play) {
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        if (this->timer != 0) {
            ...
            for (i = 6 - (this->timer >> 2); i >= 0; i--) {
                effectPos.x = MM_Rand_CenteredFloat(80.0f) + this->actor.world.pos.x;
                effectPos.z = MM_Rand_CenteredFloat(80.0f) + this->actor.world.pos.z;
                effectPos.y = MM_Rand_CenteredFloat(50.0f) + (this->actor.world.pos.y + 20.0f);
    ...
    } else if (MM_Animation_OnFrame(&this->skelAnime, 23.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_IRONNACK_WALK);
    }
}
...
void EnIk_PlayCutscene(EnIk* this, PlayState* play) {
    ...
    if (CutsceneManager_IsNext(this->actor.csId)) {
        if (this->actor.csId != CS_ID_NONE) {
            ...
            subCamEye.x = this->actor.focus.pos.x + (MM_Math_SinS(this->actor.shape.rot.y - 0x2000) * 120.0f);
            subCamEye.y = this->actor.focus.pos.y + 20.0f;
            subCamEye.z = this->actor.focus.pos.z + (MM_Math_CosS(this->actor.shape.rot.y - 0x2000) * 120.0f);
...
void EnIk_UpdateDamage(EnIk* this, PlayState* play) {
    ...
    if (this->colliderTris.base.acFlags & AC_BOUNCED) {
        f32 endFrame = MM_Animation_GetLastFrame(&gIronKnuckleBlockAnim) - 2.0f;
        ...
        MM_Actor_SetDropFlag(&this->actor, &this->colliderCylinder.elem);
        this->colliderCylinder.base.acFlags &= ~AC_HIT;
        if ((this->actor.colChkInfo.damageEffect != DMG_EFF_IMMUNE) &&
            ((this->drawDmgEffType != ACTOR_DRAW_DMGEFF_FROZEN_NO_SFX) ||
             !(this->colliderCylinder.elem.acHitElem->atDmgInfo.dmgFlags & 0xDB0B3))) {
            MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 255, COLORFILTER_BUFFLAG_OPA, 12);
            this->invincibilityFrames = 12;
            EnIk_Thaw(this, play);
            this->colliderQuad.base.atFlags &= ~AT_ON;
...
void EnIk_UpdateArmor(EnIk* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->effects); i++) {
        ...
        if (ikEffect->enabled) {
            MM_Math_Vec3f_Sum(&ikEffect->pos, &ikEffect->velocity, &ikEffect->pos);
            ...
            if (ikEffect->pos.y < this->actor.floorHeight) {
                ...
                for (i = 0; i < 4; i++) {
                    effectPos.x = MM_Rand_CenteredFloat(20.0f) + ikEffect->pos.x;
                    effectPos.y = MM_Rand_ZeroFloat(20.0f) + ikEffect->pos.y;
                    effectPos.z = MM_Rand_CenteredFloat(20.0f) + ikEffect->pos.z;
                    func_800B3030(play, &effectPos, &gZeroVec3f, &gZeroVec3f, 40, 7, 2);
                }

                MM_SoundSource_PlaySfxAtFixedWorldPos(play, &ikEffect->pos, 11, NA_SE_EN_EXTINCT);
            }
        }
    }
}
...
void MM_EnIk_Update(Actor* thisx, PlayState* play2) {
    ...
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 75.0f, 30.0f, 30.0f,
    ...
    if (this->actionFunc == EnIk_Block) {
        ...
        MM_Collider_SetTrisVertices(&this->colliderTris, 0, &sp44, &sp50, &sp5C);
        sp5C.x = sp50.x;
        sp5C.y = sp44.y;
        sp5C.z = sp50.z;
        MM_Collider_SetTrisVertices(&this->colliderTris, 1, &sp44, &sp5C, &sp50);
        MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->colliderTris.base);
    }
    if (this->drawDmgEffAlpha > 0.0f) {
        ...
        } else if (!MM_Math_StepToF(&this->drawDmgEffFrozenSteamScale, 0.65f, 0.01625f)) {
...
void EnIk_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (this->drawArmorFlags == 0x1) {
        if (armorBodyPart >= IRON_KNUCKLE_ARMOR_BODYPART_CHEST_FRONT) {
            ...
            sp76 =
                sIronKnuckleArmorMarkings[armorBodyPart].unk04 + (((s32)MM_Rand_Next() >> 0x13) + this->actor.shape.rot.y);
            ikEffect->velocity.x = MM_Math_SinS(sp76) * 5.0f;
            ikEffect->velocity.y = 6.0f;
            ikEffect->velocity.z = MM_Math_CosS(sp76) * 5.0f;
    ...
    if ((limbIndex == IRON_KNUCKLE_LIMB_AXE) && (this->blurEffectSpawnLock != this->timer) &&
        ((this->actionFunc == EnIk_VerticalAttack) || (this->actionFunc == EnIk_HorizontalDoubleAttack) ||
         (this->actionFunc == EnIk_SingleHorizontalAttack))) {
        MM_Math_Vec3f_Copy(&vtxC, &this->colliderQuad.dim.quad[0]);
        MM_Math_Vec3f_Copy(&vtxD, &this->colliderQuad.dim.quad[1]);
        MM_Matrix_MultVec3f(&D_8092C1C8, &vtxB);
        MM_Matrix_MultVec3f(&D_8092C1D4, &vtxA);
        MM_Collider_SetQuadVertices(&this->colliderQuad, &vtxA, &vtxB, &vtxC, &vtxD);
        if (this->colliderQuad.base.atFlags & AT_ON) {
            MM_EffectBlure_AddVertex(MM_Effect_GetByIndex(this->effectIndex), &vtxB, &vtxA);
        }
...
void EnIk_UpdateArmorDraw(EnIk* this, PlayState* play) {
    ...
    if (this->drawArmorFlags == (0x1 | 0x2)) {
        ...
        for (i = 0; i < ARRAY_COUNT(this->effects); i++) {
            ikEffect = &this->effects[i];
            if (ikEffect->enabled) {
                MM_Matrix_SetTranslateRotateYXZ(ikEffect->pos.x, ikEffect->pos.y, ikEffect->pos.z, &ikEffect->rot);
...
void MM_EnIk_Draw(Actor* thisx, PlayState* play) {
    static Vec3f MM_sScale = { 0.53f, 0.53f, 0.53f };
    EnIk* this = (EnIk*)thisx;
    Gfx* gfx;
    Gfx** gfxArmorType;
    s32 pad;
    ...
    func_800BC620(&this->actor.focus.pos, &MM_sScale, 255, play);
