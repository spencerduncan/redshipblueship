...
void MM_ItemEtcetera_Init(Actor* thisx, PlayState* play);
void MM_ItemEtcetera_Destroy(Actor* thisx, PlayState* play);
void MM_ItemEtcetera_Update(Actor* thisx, PlayState* play);
...
void MM_ItemEtcetera_DrawThroughLens(Actor* thisx, PlayState* play);
void MM_ItemEtcetera_Draw(Actor* thisx, PlayState* play);
...
ActorProfile Item_Etcetera_Profile = {
    ...
    /**/ GAMEPLAY_KEEP,
    /**/ sizeof(ItemEtcetera),
    /**/ MM_ItemEtcetera_Init,
    /**/ MM_ItemEtcetera_Destroy,
    /**/ MM_ItemEtcetera_Update,
    /**/ NULL,
};
...
void MM_ItemEtcetera_SetupAction(ItemEtcetera* this, ItemEtceteraActionFunc actionFunc) {
...
void MM_ItemEtcetera_Init(Actor* thisx, PlayState* play) {
    ...
    s32 objectSlot = Object_GetSlot(&play->objectCtx, MM_sObjectIds[type]);
    ...
    this->getItemId = MM_sGetItemIds[type];
    this->futureActionFunc = func_8092009C;
    this->drawFunc = MM_ItemEtcetera_Draw;
    MM_Actor_SetScale(&this->actor, 0.25f);
    MM_ItemEtcetera_SetupAction(this, ItemEtcetera_WaitForObject);
    switch (type) {
        ...
        case ITEM_ETC_KEY_SMALL_CHEST_GAME:
            ...
            this->drawFunc = MM_ItemEtcetera_DrawThroughLens;
...
void func_8092009C(ItemEtcetera* this, PlayState* play) {
    ...
    MM_Actor_OfferGetItem(&this->actor, play, this->getItemId, 30.0f, 50.0f);
}

void func_809200F8(ItemEtcetera* this, PlayState* play) {
    if (MM_Flags_GetTreasure(play, ITEMETCETERA_GET_TREASUREFLAG(&this->actor))) {
        MM_Actor_Kill(&this->actor);
    }
}
