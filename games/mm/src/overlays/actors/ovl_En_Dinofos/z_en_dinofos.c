...
static ColliderJntSphElementInit MM_sJntSphElementsInit[9] = {
    {
        {
            ELEM_MATERIAL_UNK0,
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 4, 40, 100, 80 };
...
void EnDinofos_Init(Actor* thisx, PlayState* play) {
    ...
    MM_ActorShape_Init(&this->actor.shape, 0.0f, MM_ActorShadow_DrawFeet, 90.0f);
    MM_Effect_Add(play, &this->effectIndex, EFFECT_BLURE2, 0, 0, &sBlureInit);
    Collider_InitAndSetJntSph(play, &this->bodyAndFireCollider, &this->actor, &MM_sJntSphInit,
                              this->bodyAndFireColliderElements);
    Collider_InitAndSetQuad(play, &this->knifeCollider, &this->actor, &MM_sQuadInit);
    MM_CollisionCheck_SetInfo(&this->actor.colChkInfo, &MM_sDamageTable, &MM_sColChkInfoInit);
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &gDinolfosSkel, &gDinolfosIdleAnim, this->jointTable, this->morphTable,
                       DINOLFOS_LIMB_MAX);
...
void EnDinofos_Destroy(Actor* thisx, PlayState* play) {
    ...
    MM_Collider_DestroyJntSph(play, &this->bodyAndFireCollider);
    MM_Collider_DestroyQuad(play, &this->knifeCollider);
}
    ...
    if (EnDinofos_IsFacingPlayer(this)) {
        if (this->actor.xzDistToPlayer < 100.0f) {
            if (!MM_Actor_OtherIsTargeted(play, &this->actor) &&
                (((this->actionFunc != EnDinofos_Slash) && (MM_Rand_ZeroOne() > 0.35f)) ||
                 ((this->actionFunc == EnDinofos_Slash) && (MM_Rand_ZeroOne() > 0.8f)))) {
            ...
            if (((this->actionFunc != EnDinofos_Land) && (MM_Rand_ZeroOne() < 0.1f)) ||
                ((this->actionFunc == EnDinofos_Land) && (MM_Rand_ZeroOne() < 0.05f))) {
                EnDinofos_SetupChooseJump(this, DINOFOS_JUMP_TYPE_FORWARD);
            } else {
                EnDinofos_SetupCircleAroundPlayer(this, play);
            }
        } else if (MM_Rand_ZeroOne() < 0.8f) {
            EnDinofos_SetupWalk(this, play);
        } else {
            EnDinofos_SetupCircleAroundPlayer(this, play);
        }
    } else if (MM_Rand_ZeroOne() < 0.6f) {
...
s32 EnDinofos_Dodge(EnDinofos* this, PlayState* play) {
    ...
    if (actor != NULL) {
        angleToProjectile = (MM_Actor_WorldYawTowardActor(&this->actor, actor) - this->actor.shape.rot.y) - this->headRotY;
    ...
    if ((actor != NULL) && (actor->id == ACTOR_EN_BOM_CHU)) {
        angleToBombchu = (MM_Actor_WorldYawTowardActor(&this->actor, actor) - this->actor.shape.rot.y) - this->headRotY;
    ...
    if ((this->actor.xzDistToPlayer < 100.0f) && (player->meleeWeaponState != PLAYER_MELEE_WEAPON_STATE_0) &&
        this->actor.isLockedOn && (MM_Rand_ZeroOne() < 0.5f) && EnDinofos_IsFacingPlayer(this) &&
        MM_Player_IsFacingActor(&this->actor, 0x2000, play)) {
...
void EnDinofos_SetupIntroCutsceneBeforeFall(EnDinofos* this, PlayState* play) {
    ...
    MM_Animation_Change(&this->skelAnime, &gDinolfosJumpAnim, 1.0f, MM_Animation_GetLastFrame(&gDinolfosJumpAnim),
                     MM_Animation_GetLastFrame(&gDinolfosJumpAnim), ANIMMODE_ONCE, 0.0f);
    func_800BE33C(&subCam->eye, &subCam->at, &this->subCamRot, true);
    MM_Math_Vec3f_Diff(&this->actor.world.pos, &player->actor.world.pos, &diffToPlayer);
    ...
    this->subCamEyeStep = MM_Math_Vec3f_DistXYZ(&subCam->eye, &this->subCamEye) * 0.05f;
    this->subCamAtStep = MM_Math_Vec3f_DistXYZ(&subCam->at, &this->subCamAt) * 0.05f;
...
void EnDinofos_SetupIntroCutsceneFall(EnDinofos* this) {
    ...
    this->subCamEye.x = (MM_Math_SinS(this->actor.shape.rot.y - 0x0E00) * 86.0f) + this->actor.world.pos.x;
    this->subCamEye.y = this->actor.floorHeight + 6.0f;
    this->subCamEye.z = (MM_Math_CosS(this->actor.shape.rot.y - 0x0E00) * 86.0f) + this->actor.world.pos.z;
...
void EnDinofos_SetupIntroCutsceneLandAndBreatheFire(EnDinofos* this) {
    ...
    MM_Animation_PlayOnce(&this->skelAnime, &gDinolfosIntroAnim);
    this->subCamEye.x = (MM_Math_SinS(this->actor.shape.rot.y + 0x200) * 123.0f) + this->actor.world.pos.x;
    this->subCamEye.y = this->actor.floorHeight + 11.0f;
    this->subCamEye.z = (MM_Math_CosS(this->actor.shape.rot.y + 0x200) * 123.0f) + this->actor.world.pos.z;
...
void EnDinofos_IntroCutsceneLandAndBreatheFire(EnDinofos* this, PlayState* play) {
    ...
    if (this->skelAnime.curFrame > 35.0f) {
        if ((play->sceneId == SCENE_MITURIN) && MM_Animation_OnFrame(&this->skelAnime, 38.0f)) {
    ...
    if ((play->sceneId == SCENE_MITURIN) && MM_Animation_OnFrame(&this->skelAnime, 55.0f)) {
...
void EnDinofos_SetupIntroCutsceneYell(EnDinofos* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gDinolfosCryAnim, -3.0f);
    this->actionFunc = EnDinofos_IntroCutsceneYell;
}

void EnDinofos_IntroCutsceneYell(EnDinofos* this, PlayState* play) {
    if (MM_Animation_OnFrame(&this->skelAnime, 2.0f)) {
...
void EnDinofos_SetupIdle(EnDinofos* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gDinolfosIdleAnim, -4.0f);
    this->actionTimer = (s32)MM_Rand_ZeroFloat(20.0f) + 40;
    this->idleTimer = 30;
...
void EnDinofos_SetupWalk(EnDinofos* this, PlayState* play) {
    ...
    if (this->actionFunc != EnDinofos_Walk) {
        MM_Animation_MorphToLoop(&this->skelAnime, &gDinolfosWalkAnim, -4.0f);
        if (MM_Actor_OtherIsTargeted(play, &this->actor)) {
            targetDist = 170.0f;
        } else {
            targetDist = 70.0f;
    ...
    this->walkTimer = (s32)MM_Rand_ZeroFloat(10.0f) + 20;
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnDinofos_Walk(EnDinofos* this, PlayState* play) {
    ...
    if (!EnDinofos_Dodge(this, play)) {
        ...
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x100);
        this->actor.world.rot.y = this->actor.shape.rot.y;
        if (this->actor.xzDistToPlayer <= targetDist) {
            MM_Math_StepToF(&this->actor.speed, -7.0f, 0.5f);
        } else {
            MM_Math_StepToF(&this->actor.speed, 7.0f, 0.5f);
        }
...
void EnDinofos_TurnToPlayer(EnDinofos* this, PlayState* play) {
    MM_SkelAnime_Update(&this->skelAnime);
    if (MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->targetRotY, 0x400)) {
        EnDinofos_ChooseAction(this, play);
    }
    this->actor.world.rot.y = this->actor.shape.rot.y;
}
...
void EnDinofos_SetupCircleAroundPlayer(EnDinofos* this, PlayState* play) {
    ...
    if (this->actionFunc != EnDinofos_CircleAroundPlayer) {
        ...
        if (this->actionFunc == EnDinofos_DodgeProjectile) {
            ...
            MM_Animation_Change(&this->skelAnime, &gDinolfosSidestepAnim, this->actor.speed * (1.0f / 6.0f), 0.0f, 0.0f,
                             ANIMMODE_LOOP, -4.0f);
        }

        this->actor.world.rot.y = BINANG_ADD(this->actor.shape.rot.y, 0x4000);
        this->circlingRate = 0;
        this->sidestepTimer = (s32)MM_Rand_ZeroFloat(10.0f) + 5;
    }
...
void EnDinofos_CircleAroundPlayer(EnDinofos* this, PlayState* play) {
    ...
    if (!EnDinofos_Dodge(this, play)) {
        ...
        if (this->actor.xzDistToPlayer <= (70.0f + targetDistAdjustment)) {
            MM_Math_StepToF(&this->circlingRate, -4.0f, 1.5f);
        } else if ((90.0f + targetDistAdjustment) < this->actor.xzDistToPlayer) {
            MM_Math_StepToF(&this->circlingRate, 4.0f, 1.5f);
        } else {
            MM_Math_StepToF(&this->circlingRate, 0.0f, 5.65f);
        }

        this->actor.world.pos.x += MM_Math_SinS(this->actor.shape.rot.y) * this->circlingRate;
        this->actor.world.pos.z += MM_Math_CosS(this->actor.shape.rot.y) * this->circlingRate;
        MM_SkelAnime_Update(&this->skelAnime);
...
void EnDinofos_SetupChooseJump(EnDinofos* this, s32 jumpType) {
    if (jumpType == DINOFOS_JUMP_TYPE_FORWARD) {
        MM_Animation_MorphToPlayOnce(&this->skelAnime, &gDinolfosJumpAnim, 2.0f);
        EnDinofos_EnableBumperCollision(this);
    } else {
        MM_Animation_MorphToPlayOnce(&this->skelAnime, &gDinolfosJumpAnim, -1.0f);
...
void EnDinofos_ChooseJump(EnDinofos* this, PlayState* play) {
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x1000);
    this->actor.world.rot.y = this->actor.shape.rot.y;
    MM_SkelAnime_Update(&this->skelAnime);
...
void EnDinofos_JumpSlash(EnDinofos* this, PlayState* play) {
    MM_SkelAnime_Update(&this->skelAnime);
    this->attackTimer++;
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x1800);

    if ((this->actor.velocity.y < 0.0f) && (this->skelAnime.animation == &gDinolfosJumpAnim)) {
        this->knifeCollider.base.atFlags |= AT_ON;
        this->bodyAndFireCollider.base.acFlags |= AC_ON;
        MM_Animation_Change(&this->skelAnime, &gDinolfosAttackAnim, 1.0f, 7.0f, 13.0f, ANIMMODE_ONCE, -2.0f);
    }
...
void EnDinofos_SetupLand(EnDinofos* this) {
    if (this->actionFunc != EnDinofos_JumpSlash) {
        MM_Animation_PlayOnce(&this->skelAnime, &gDinolfosLandAnim);
    } else {
        this->skelAnime.endFrame = MM_Animation_GetLastFrame(&gDinolfosAttackAnim);
    }
...
void EnDinofos_Land(EnDinofos* this, PlayState* play) {
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if ((this->isJumpingBackward == true) && (this->actor.xzDistToPlayer < 280.0f) &&
            EnDinofos_IsFacingPlayer(this) && (MM_Rand_ZeroOne() < 0.6f)) {
...
void EnDinofos_SetupSlash(EnDinofos* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gDinolfosAttackAnim);
...
void EnDinofos_Slash(EnDinofos* this, PlayState* play) {
    this->attackTimer++;
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x800);
    this->actor.world.rot.y = this->actor.shape.rot.y;
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        MM_EffectBlure_AddSpace(MM_Effect_GetByIndex(this->effectIndex));
        this->knifeCollider.base.atFlags &= ~AT_ON;
        EnDinofos_ChooseAction(this, play);
    } else if (MM_Animation_OnFrame(&this->skelAnime, 7.0f)) {
        this->knifeCollider.base.atFlags |= AT_ON;
        EnDinofos_EnableBumperCollision(this);
...
void EnDinofos_SetupRecoil(EnDinofos* this) {
    MM_Animation_Change(&this->skelAnime, &gDinolfosAttackAnim, -1.0f, this->skelAnime.curFrame, 0.0f, ANIMMODE_ONCE,
                     0.0f);
...
void EnDinofos_SetupDamaged(EnDinofos* this, s32 colliderIndex) {
    MM_Animation_PlayOnce(&this->skelAnime, &gDinolfosHitAnim);
    ...
    this->bodyAndFireCollider.base.acFlags &= ~AC_ON;
    MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 255, COLORFILTER_BUFFLAG_OPA, 18);
    this->actionFunc = EnDinofos_Damaged;
}

void EnDinofos_Damaged(EnDinofos* this, PlayState* play) {
    MM_Math_StepToF(&this->actor.speed, 0.0f, 0.5f);
    if (MM_SkelAnime_Update(&this->skelAnime) && (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
        if (this->actor.colChkInfo.health == 0) {
            if (this->actor.csId == CS_ID_NONE) {
                EnDinofos_SetupDie(this);
            } else {
...
void EnDinofos_SetupStartBreatheFire(EnDinofos* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gDinolfosFireStartAnim, -5.0f);
    this->bodyAndFireCollider.base.acFlags |= AC_ON;
    this->actor.speed = 0.0f;
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnDinofos_StartBreatheFire(EnDinofos* this, PlayState* play) {
    ...
    } else if (!EnDinofos_Dodge(this, play) && MM_Animation_OnFrame(&this->skelAnime, 12.0f)) {
        this->actor.speed = 8.0f;
    }
}
...
void EnDinofos_SetupBreatheFire(EnDinofos* this, PlayState* play) {
    ...
    MM_Animation_PlayLoop(&this->skelAnime, &gDinolfosFireLoopAnim);
    this->attackTimer = 20;
    this->actor.speed = 0.0f;
...
void EnDinofos_BreatheFire(EnDinofos* this, PlayState* play) {
    s16 fireRotY = ((s32)MM_Rand_CenteredFloat(0x1000) + this->actor.shape.rot.y) + this->headRotY;
    Vec3f velocity;
    Vec3f accel;
    ColliderJntSphElementDim* dim;
    s32 i;
    ...
    MM_SkelAnime_Update(&this->skelAnime);
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x800);
    sin = MM_Math_SinS(fireRotY);
    cos = MM_Math_CosS(fireRotY);

    dimAlphaStep = 10 - this->attackTimer;
    dimAlphaStep = CLAMP_MIN(dimAlphaStep, 0);

    velocity.x = 11.0f * sin;
    velocity.y = MM_Rand_CenteredFloat(2.0f) + -5.4f;
    velocity.z = 11.0f * cos;
    accel.x = 0.9f * sin;
    accel.y = MM_Rand_CenteredFloat(0.6f) + 1.4f;
    accel.z = 0.9f * cos;
    Actor_PlaySfx_Flagged(&this->actor, NA_SE_EN_DODO_J_FIRE - SFX_FLAG);
    MM_EffectSsDFire_Spawn(play, &this->bodyPartsPos[DINOFOS_BODYPART_JAW], &velocity, &accel, 30, 22,
    ...
    for (i = DINOFOS_COLLIDER_FIRE_START_INDEX; i < end; i++) {
        dim = &this->bodyAndFireCollider.elements[i].dim;
        fireRotY =
            (s32)(MM_Math_CosF((this->attackTimer + ((i - 5) << 1)) * (M_PIf / 20)) * 0x2C00) + this->actor.shape.rot.y;

        dim->worldSphere.center.x =
            (s32)this->bodyPartsPos[DINOFOS_BODYPART_JAW].x + (s32)(MM_Math_SinS(fireRotY) * dim->modelSphere.center.z);
        ...
        dim->worldSphere.center.z =
            (s32)this->bodyPartsPos[DINOFOS_BODYPART_JAW].z + (s32)(MM_Math_CosS(fireRotY) * dim->modelSphere.center.z);
    }
...
void EnDinofos_SetupEndBreatheFire(EnDinofos* this, PlayState* play) {
    MM_Animation_PlayOnce(&this->skelAnime, &gDinolfosFireEndAnim);
    this->bodyAndFireCollider.base.atFlags &= ~AT_ON;
    if (play->sceneId == SCENE_MITURIN) {
        play->envCtx.lightSettingOverride = LIGHT_SETTING_OVERRIDE_NONE;
    }
...
void EnDinofos_SetupDie(EnDinofos* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gDinolfosDieAnim);
...
void EnDinofos_Die(EnDinofos* this, PlayState* play) {
    ...
    } else if (MM_Animation_OnFrame(&this->skelAnime, 26.0f)) {
...
void EnDinofos_SetupDodgeProjectile(EnDinofos* this, s16 angleToProjectile) {
    ...
    MM_Animation_Change(&this->skelAnime, &gDinolfosSidestepAnim, this->actor.speed * (1.0f / 7.5f), 0.0f, 0.0f,
                     ANIMMODE_LOOP, -4.0f);
    this->actor.world.rot.y = BINANG_ADD(this->actor.shape.rot.y, 0x4000);
...
void EnDinofos_PlayCutscene(EnDinofos* this, PlayState* play) {
    ...
    if (CutsceneManager_IsNext(this->actor.csId)) {
        ...
        if (this->actor.colChkInfo.health == 0) {
            subCamEye.x = (MM_Math_SinS(this->actor.shape.rot.y) * 150.0f) + this->actor.focus.pos.x;
            subCamEye.y = this->actor.focus.pos.y;
            subCamEye.z = (MM_Math_CosS(this->actor.shape.rot.y) * 150.0f) + this->actor.focus.pos.z;
...
void EnDinofos_RotateHead(EnDinofos* this, PlayState* play) {
    ...
    if ((this->actionFunc == EnDinofos_Idle) && (this->headTimer != 0)) {
        MM_Math_ScaledStepToS(&this->headRotY, MM_Math_SinS(this->headTimer * 1400) * 0x2C00, 0x300);
    } else if (this->actionFunc == EnDinofos_StartBreatheFire) {
        MM_Math_ScaledStepToS(&this->headRotY, MM_Math_CosF(M_PIf) * 0x2C00, 0x2C00 / 20);
    } else if (this->actionFunc == EnDinofos_BreatheFire) {
        this->headRotY = MM_Math_CosF(this->headTimer * (M_PIf / 20)) * 0x2C00;
...
s32 EnDinofos_UpdateDamage(EnDinofos* this, PlayState* play) {
    ...
    if (this->bodyAndFireCollider.base.acFlags & AC_HIT) {
        ...
        MM_Actor_SetDropFlagJntSph(&this->actor, &this->bodyAndFireCollider);
...
void EnDinofos_Update(Actor* thisx, PlayState* play2) {
    ...
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 25.0f, 30.0f, 60.0f,
    ...
    if (this->actionFunc != EnDinofos_Stunned) {
        if ((this->actor.depthInWater > 0.0f) && (this->actor.depthInWater < 10.0f)) {
            if (((play->gameplayFrames % 4) & 1) == 0) {
                MM_Math_Vec3f_Copy(&bodyPartPos,
                ...
                MM_EffectSsGRipple_Spawn(play, &bodyPartPos, 100, 320, 0);
            }
        }
    ...
    if ((this->actionFunc == EnDinofos_Die) || (this->actionFunc == EnDinofos_Damaged)) {
        MM_Math_ScaledStepToS(&this->headRotY, 0, 2000);
    }

    MM_Math_Vec3f_Copy(&this->actor.focus.pos, &this->bodyPartsPos[DINOFOS_BODYPART_UPPER_BODY]);
    this->actor.focus.rot.y = this->actor.shape.rot.y + this->headRotY;
    if (this->bodyAndFireCollider.base.acFlags & AC_ON) {
        MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->bodyAndFireCollider.base);
    }

    MM_CollisionCheck_SetOC(play, &play->colChkCtx, &this->bodyAndFireCollider.base);
    if (this->bodyAndFireCollider.base.atFlags & AT_ON) {
        MM_CollisionCheck_SetAT(play, &play->colChkCtx, &this->bodyAndFireCollider.base);
    }

    if (this->knifeCollider.base.atFlags & AT_ON) {
        MM_CollisionCheck_SetAT(play, &play->colChkCtx, &this->knifeCollider.base);
    }
    ...
    if (this->drawDmgEffAlpha > 0.0f) {
        ...
        } else if (!MM_Math_StepToF(&this->drawDmgEffFrozenSteamScale, 0.55f, (11.0f / 800.0f))) {
...
void EnDinofos_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx, Gfx** gfx) {
    ...
    MM_Collider_UpdateSpheres(limbIndex, &this->bodyAndFireCollider);
    ...
    if ((limbIndex == DINOLFOS_LIMB_RIGHT_HAND) && (this->timer2 != this->attackTimer) &&
        ...
        MM_Math_Vec3f_Copy(&prevKnifeBaseQuadPos, &this->knifeCollider.dim.quad[0]);
        MM_Math_Vec3f_Copy(&prevKnifeTipQuadPos, &this->knifeCollider.dim.quad[1]);
        MM_Matrix_MultVec3f(&sKnifeTipQuadOffset, &knifeTipQuadPos);
        MM_Matrix_MultVec3f(&sKnifeBaseQuadOffset, &knifeBaseQuadPos);
        MM_Collider_SetQuadVertices(&this->knifeCollider, &knifeBaseQuadPos, &knifeTipQuadPos, &prevKnifeBaseQuadPos,
                                 &prevKnifeTipQuadPos);
        if (this->knifeCollider.base.atFlags & AT_ON) {
            MM_EffectBlure_AddVertex(MM_Effect_GetByIndex(this->effectIndex), &knifeTipQuadPos, &knifeBaseQuadPos);
        }
    ...
    if ((limbIndex == DINOLFOS_LIMB_HEAD) && (this->actionFunc == EnDinofos_IntroCutsceneLandAndBreatheFire)) {
        if ((this->skelAnime.curFrame > 38.0f) && (this->skelAnime.curFrame <= 55.0f) &&
            ...
            dimAlphaStep = CLAMP_MIN(dimAlphaStep, 0);
            MM_Matrix_MultVec3f(&sCsFireVelocityOffset, &fireVelocity);
            fireVelocity.x -= matrix->mf[3][0];
            fireVelocity.y -= matrix->mf[3][1];
            fireVelocity.z -= matrix->mf[3][2];
            MM_EffectSsDFire_Spawn(play, &this->bodyPartsPos[DINOFOS_BODYPART_JAW], &fireVelocity, &gZeroVec3f, 30, 22,
                                255 - (dimAlphaStep * 20), 20, 3, 8);
            this->timer2 = this->cutsceneTimer;
        }
    }
