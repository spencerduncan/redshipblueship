...
void MM_ObjTsubo_Init(Actor* thisx, PlayState* play);
void MM_ObjTsubo_Destroy(Actor* thisx, PlayState* play2);
void MM_ObjTsubo_Update(Actor* thisx, PlayState* play);
void MM_ObjTsubo_Draw(Actor* thisx, PlayState* play2);
...
void func_8092762C(ObjTsubo* this, PlayState* play) {
    if (GameInteractor_Should(VB_POT_DROP_COLLECTIBLE,
        ...
        MM_Item_DropCollectibleRandom(play, NULL, &this->actor.world.pos, OBJ_TSUBO_P000F(&this->actor) << 4);
    }
}
...
void func_80927690(ObjTsubo* this, PlayState* play) {
    ...
    if (GameInteractor_Should(VB_POT_DROP_COLLECTIBLE, !this->unk_197 && (OBJ_TSUBO_ZROT(&this->actor) != 2), this)) {
        ...
        if (itemDrop > ITEM00_NO_DROP) {
            MM_Item_DropCollectible(play, &this->actor.world.pos, (OBJ_TSUBO_PFE00(&this->actor) << 8) | itemDrop);
            this->unk_197 = true;
        }
    }
}
...
void ObjTsubo_SpawnBoes(ObjTsubo* this, PlayState* play, s32 arg2) {
    MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_MKK, this->actor.world.pos.x, this->actor.world.pos.y,
...
void ObjTsubo_SpawnGoldSkulltula(ObjTsubo* this, PlayState* play, s32 arg2) {
    ...
    if (func_809275C0(this, play)) {
        ...
        child = MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_SW, this->actor.world.pos.x, this->actor.world.pos.y,
                            this->actor.world.pos.z, 0, MM_Rand_Next() >> 0x10, 0, params);
        if (child != NULL) {
...
void MM_ObjTsubo_Init(Actor* thisx, PlayState* play) {
    ...
    MM_Actor_SetScale(&this->actor, sPotTypeData[type].scale);
    this->actor.shape.rot.z = 0;
    this->actor.world.rot.z = 0;
    MM_Collider_InitCylinder(play, &this->cylinderCollider);
    MM_Collider_SetCylinder(play, &this->cylinderCollider, &this->actor, &MM_sCylinderInit);
    MM_Collider_UpdateCylinder(&this->actor, &this->cylinderCollider);
...
void MM_ObjTsubo_Destroy(Actor* thisx, PlayState* play2) {
    ...
    MM_Collider_DestroyCylinder(play, &this->cylinderCollider);
}
...
void ObjTsubo_PotBreak1(ObjTsubo* this, PlayState* play) {
    ...
    for (i = 0, rot = 0; i < 18; rot += 0x4E20, i++) {
        sin = MM_Math_SinS(rot);
        cos = MM_Math_CosS(rot);
        pos.x = sin * 8.0f;
        pos.y = MM_Rand_ZeroOne() * 12.0f + 2.0f;
        pos.z = cos * 8.0f;
        velocity.x = pos.x * 0.23f;
        velocity.y = MM_Rand_ZeroOne() * 5.0f + 2.5f;
        velocity.z = pos.z * 0.23f;
        MM_Math_Vec3f_Sum(&pos, &this->actor.world.pos, &pos);
        randf = MM_Rand_ZeroOne();
        if (randf < 0.2f) {
            phi_s0 = 0x60;
        } else if (randf < 0.6f) {
            phi_s0 = 0x40;
        ...
        scale = MM_Rand_ZeroOne() * 110.0f + 15.0f;
        MM_EffectSsKakera_Spawn(play, &pos, &velocity, &this->actor.world.pos, -260, phi_s0, 20, 0, 0, scale, 0, 0, 50, -1,
...
void ObjTsubo_MagicPotBreak1(ObjTsubo* this, PlayState* play) {
    ...
    for (i = 0, rot = 0; i < 20; rot += 0x4E20, i++) {
        ...
        cos = MM_Math_CosS(rot);
        pos.x = sin * 10.0f;
        pos.y = MM_Rand_ZeroOne() * 20.0f + 2.0f;
        pos.z = cos * 10.0f;
        ...
        scale = MM_Rand_ZeroOne() * 160.0f + 15.0f;
        MM_EffectSsKakera_Spawn(play, &pos, &velocity, &this->actor.world.pos, -340, phi_s0, 20, 0, 0, scale, 0, 0, 50, -1,
                             typeData->objectId, typeData->shardDL);
    }
...
void ObjTsubo_PotBreak2(ObjTsubo* this, PlayState* play2) {
    ...
    for (rot = 0, i = 0; i < 5; i++, rot += 0x10000 / 5) {
        pos.x = worldPos->x + MM_Math_SinS((s32)(MM_Rand_ZeroOne() * 6000) + rot) * 15.0f;
        pos.z = worldPos->z + MM_Math_CosS((s32)(MM_Rand_ZeroOne() * 6000) + rot) * 15.0f;
    ...
    for (i = 0, rot = 0; i < 13; rot += 0x4E20, i++) {
        ...
        MM_Math_Vec3f_Sum(&pos, worldPos, &pos);
        if (MM_Rand_ZeroOne() < .2f) {
            phi_s0 = 0x40;
        } else {
            phi_s0 = 0x20;
        }
        scale = MM_Rand_ZeroOne() * 105.0f + 10.0f;
        MM_EffectSsKakera_Spawn(play, &pos, &velocity, worldPos, -170, phi_s0, 50, 5, 0, scale, 0, 0, 70, -1,
                             typeData->objectId, typeData->shardDL);
    }
}
...
void ObjTsubo_MagicPotBreak2(ObjTsubo* this, PlayState* play2) {
    ...
    for (rot = 0, i = 0; i < 5; i++, rot += 0x10000 / 5) {
        pos.x = worldPos->x + MM_Math_SinS((s32)(MM_Rand_ZeroOne() * 6000) + rot) * 30.0f;
        pos.z = worldPos->z + MM_Math_CosS((s32)(MM_Rand_ZeroOne() * 6000) + rot) * 30.0f;
        MM_EffectSsGSplash_Spawn(play, &pos, NULL, NULL, 0, 350);
    }
    pos.x = worldPos->x;
    pos.z = worldPos->z;
    ...
    for (i = 0, rot = 0; i < 15; rot += 0x4E20, i++) {
        ...
        cos = MM_Math_CosS(rot);
        pos.x = sin * 10.0f;
        pos.y = (MM_Rand_ZeroOne() * 15.0f) + 2.0f;
        pos.z = cos * 10.0f;
        velocity.x = pos.x * 0.3f;
        velocity.y = (MM_Rand_ZeroOne() * 4.0f) + 2.0f;
        velocity.z = pos.z * 0.3f;
        ...
        scale = (MM_Rand_ZeroOne() * 150.0f) + 10.0f;
        MM_EffectSsKakera_Spawn(play, &pos, &velocity, worldPos, -170, phi_s0, 50, 5, 0, scale, 0, 0, 70, -1,
                             typeData->objectId, typeData->shardDL);
    }
}
...
void ObjTsubo_PotBreak3(ObjTsubo* this, PlayState* play2) {
    ...
    for (i = 0, rot = 0; i < 13; i++, rot += 0x4E20) {
        ...
        velocity.y = (MM_Rand_ZeroOne() * 4.0f) + 4.0f;
        ...
        MM_EffectSsKakera_Spawn(play, &pos, &velocity, &this->actor.world.pos, -170, phi_s0, 50, 3, 0,
                             (MM_Rand_ZeroOne() * 105.0f) + 10.0f, 0, 0, 70, -1, typeData->objectId, typeData->shardDL);
    }

    for (i = 0; i < 7; i++) {
        MM_EffectSsBubble_Spawn(play, &thisx->world.pos, 20.0f, 30.0f, 40.0f, (MM_Rand_ZeroOne() * 0.06f) + 0.09f);
        MM_EffectSsBubble_Spawn(play, &thisx->world.pos, 10.0f, 10.0f, 10.0f, (MM_Rand_ZeroOne() * 0.08f) + 0.09f);
    }
}
...
void func_80928928(ObjTsubo* this, PlayState* play) {
    ...
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 15.0f, 15.0f, 0.0f, UPDBGCHECKINFO_FLAG_4 | UPDBGCHECKINFO_FLAG_40);
...
void func_809289B4(ObjTsubo* this) {
    if (GameInteractor_Should(VB_POT_DRAW_BE_OVERRIDDEN, true, this)) {
        this->actor.draw = MM_ObjTsubo_Draw;
    }
...
void func_809289E4(ObjTsubo* this, PlayState* play) {
    ...
    if (MM_Actor_HasParent(&this->actor, play)) {
        ...
        if (!this->unk_195) {
            Actor_MoveWithGravity(&this->actor);
            MM_Actor_UpdateBgCheckInfo(play, &this->actor, 15.0f, 15.0f, 0.0f,
                                    UPDBGCHECKINFO_FLAG_4 | UPDBGCHECKINFO_FLAG_40);
            if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) &&
                (MM_DynaPoly_GetActor(&play->colCtx, this->actor.floorBgId) == NULL)) {
                this->unk_195 = true;
                this->actor.flags &= ~ACTOR_FLAG_UPDATE_CULLING_DISABLED;
            }
        }
        if ((this->actor.xzDistToPlayer < 800.0f) || (gSaveContext.save.entrance == ENTRANCE(GORON_RACETRACK, 1))) {
            MM_Collider_UpdateCylinder(&this->actor, &this->cylinderCollider);
            MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->cylinderCollider.base);
            if (this->actor.xzDistToPlayer < 150.0f) {
                MM_CollisionCheck_SetOC(play, &play->colChkCtx, &this->cylinderCollider.base);
...
void func_80928E74(ObjTsubo* this) {
    ...
    D_80929500 = (MM_Rand_ZeroOne() - 0.78f) * 4500.0f;
    D_80929508 = (MM_Rand_ZeroOne() - 0.5f) * 2000.0f;
    D_80929504 = 0;
    D_8092950C = 0;
...
void func_80928F18(ObjTsubo* this, PlayState* play) {
    ...
    if ((this->actor.bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_GROUND_TOUCH | BGCHECKFLAG_WALL)) || atHit ||
        ...
        Actor_MoveWithGravity(&this->actor);
        MM_Math_StepToS(&D_80929504, D_80929500, 150);
        MM_Math_StepToS(&D_8092950C, D_80929508, 150);
        ...
        MM_Collider_UpdateCylinder(&this->actor, &this->cylinderCollider);
        MM_CollisionCheck_SetOC(play, &play->colChkCtx, &this->cylinderCollider.base);
        MM_CollisionCheck_SetAT(play, &play->colChkCtx, &this->cylinderCollider.base);
    }
}
...
void func_8092926C(ObjTsubo* this, PlayState* play) {
    ...
    if (this->unk_194 > 0) {
        ...
        if (MM_Math_StepToF(&this->actor.scale.x, scale, scale * 0.1f)) {
            this->actor.flags |= ACTOR_FLAG_CAN_PRESS_SWITCHES;
            func_809289B4(this);
        }
        ...
        if (this->unk_19A >= 0) {
            if (this->unk_19A == 0) {
                ...
                if (MM_Rand_ZeroOne() < 0.1f) {
                    this->unk_19A = MM_Rand_S16Offset(40, 80);
                } else {
                    this->unk_19A = 8;
                }
            } else {
...
void MM_ObjTsubo_Draw(Actor* thisx, PlayState* play2) {
    ...
    MM_Gfx_DrawDListOpa(play, sPotTypeData[OBJ_TSUBO_GET_TYPE(thisx)].modelDL);
}
