...
static AnimationInfo MM_sAnimationInfo[EN_RAILGIBUD_ANIM_MAX] = {
...
static CollisionCheckInfoInit2 MM_sColChkInfoInit = { 8, 0, 0, 0, MASS_IMMOVABLE };
...
void EnRailgibud_SpawnOtherGibdosAndSetPositionAndRotation(EnRailgibud* this, PlayState* play) {
    ...
    // Gibdos created by MM_Actor_SpawnAsChild will go through this function to set
    // their position and rotation, but they will not be able to spawn any more
    // Gibdos themselves because sCurrentGibdoIndex will be non-MM_zero.
    if (sCurrentGibdoIndex == 0) {
        s32 i;

        for (i = 1; (i < this->pathCount) && (i < 10); i++) {
            sCurrentGibdoIndex++;
            MM_Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, ACTOR_EN_RAILGIBUD, 0.0f, 0.0f, 0.0f, 0, 0, 0,
                               this->actor.params);
        }
    ...
    this->actor.world.rot.y = this->actor.shape.rot.y = MM_Math_Vec3f_Yaw(&this->actor.world.pos, &targetPos);
...
void EnRailgibud_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &gGibdoSkel, &gGibdoRedeadIdleAnim, this->jointTable, this->morphTable,
                       GIBDO_LIMB_MAX);
...
void EnRailgibud_SetupWalkInCircles(EnRailgibud* this) {
    ...
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_WALK);
...
void EnRailgibud_WalkInCircles(EnRailgibud* this, PlayState* play) {
    ...
    if ((this->actor.xzDistToPlayer <= 100.0f) && func_800B715C(play) && (MM_Player_GetMask(play) != PLAYER_MASK_GIBDO)) {
        this->actor.home = this->actor.world;
        EnRailgibud_SetupAttemptPlayerFreeze(this);
    }

    MM_Math_SmoothStepToS(&this->headRot.y, 0, 1, 0x64, 0);
    MM_Math_SmoothStepToS(&this->torsoRot.y, 0, 1, 0x64, 0);
    ...
    yRotation = MM_Math_Vec3f_Yaw(&this->actor.world.pos, &targetPos);
    if (MM_Math_Vec3f_DistXZ(&this->actor.world.pos, &targetPos) > 60.0f) {
        MM_Math_SmoothStepToS(&this->actor.world.rot.y, yRotation, 1, 0x190, 0xA);
...
void EnRailgibud_SetupAttemptPlayerFreeze(EnRailgibud* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_IDLE);
...
void EnRailgibud_WalkToPlayer(EnRailgibud* this, PlayState* play) {
    ...
    MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0xFA);
    this->actor.world.rot = this->actor.shape.rot;
    MM_Math_SmoothStepToS(&this->headRot.y, 0, 1, 0x64, 0);
    MM_Math_SmoothStepToS(&this->torsoRot.y, 0, 1, 0x64, 0);

    if (EnRailgibud_PlayerInRangeWithCorrectState(this, play) && MM_Actor_IsFacingPlayer(&this->actor, 0x38E3)) {
...
void EnRailgibud_SetupGrab(EnRailgibud* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_GRAB_START);
...
void EnRailgibud_Grab(EnRailgibud* this, PlayState* play) {
    ...
    switch (this->grabState) {
        case EN_RAILGIBUD_GRAB_START:
            ...
            if (MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame) && (inPositionToAttack == true)) {
                this->grabState = EN_RAILGIBUD_GRAB_ATTACK;
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_GRAB_ATTACK);
            } else if (!(player->stateFlags2 & PLAYER_STATE2_80)) {
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_GRAB_END);
                this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED;
        ...
        case EN_RAILGIBUD_GRAB_ATTACK:
            if (this->grabDamageTimer == 20) {
                ...
                MM_Player_PlaySfx(player, damageSfxId);
...
void EnRailgibud_SetupGrabFail(EnRailgibud* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_DAMAGE);
...
void EnRailgibud_GrabFail(EnRailgibud* this, PlayState* play) {
    ...
    MM_Math_SmoothStepToS(&this->headRot.y, 0, 1, 0x12C, 0);
    MM_Math_SmoothStepToS(&this->torsoRot.y, 0, 1, 0x12C, 0);
    if (MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
...
void EnRailgibud_TurnAwayAndShakeHead(EnRailgibud* this, PlayState* play) {
    ...
    if (this->headShakeTimer > 60) {
        ...
        this->headRot.y = MM_Math_SinS(this->headShakeTimer * 4000) * (0x256F * ((60 - this->headShakeTimer) / 60.0f));
        this->headShakeTimer++;
    }
}
...
void EnRailgibud_WalkToHome(EnRailgibud* this, PlayState* play) {
    MM_Math_SmoothStepToS(&this->headRot.y, 0, 1, 0x64, 0);
    MM_Math_SmoothStepToS(&this->torsoRot.y, 0, 1, 0x64, 0);
    if (MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) < 5.0f) {
        ...
        MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.home.rot.y, 1, 0xC8, 0xA);
        this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnRailgibud_Damage(EnRailgibud* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
        ...
        if ((this->drawDmgEffTimer > 0) && (this->drawDmgEffType == ACTOR_DRAW_DMGEFF_FIRE) &&
            ...
            MM_SkelAnime_InitFlex(play, &this->skelAnime, &gRedeadSkel, NULL, this->jointTable, this->morphTable,
                               GIBDO_LIMB_MAX);
...
void EnRailgibud_SetupDead(EnRailgibud* this) {
    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_DEATH);
    this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
...
void EnRailgibud_Dead(EnRailgibud* this, PlayState* play) {
    if (this->deathTimer > 300) {
        ...
        MM_Math_SmoothStepToS(&this->headRot.y, 0, 1, 0xFA, 0);
        MM_Math_SmoothStepToS(&this->torsoRot.y, 0, 1, 0xFA, 0);
        this->deathTimer++;
    }
...
void EnRailgibud_SpawnEffectsForSinkingIntoTheGround(EnRailgibud* this, PlayState* play, s32 arg2) {
    ...
    if ((play->gameplayFrames & arg2) == 0) {
        rand = MM_Rand_Next();
        rockFragmentPos.x += 15.0f * MM_Math_SinS(rand);
        rockFragmentPos.z += 15.0f * MM_Math_CosS(rand);
        rockFragmentAccel.x = MM_Rand_Centered();
        rockFragmentAccel.z = MM_Rand_Centered();
        rockFragmentVelocity.y += MM_Rand_Centered() * 4.0f;
        MM_EffectSsHahen_Spawn(play, &rockFragmentPos, &rockFragmentVelocity, &rockFragmentAccel, 0,
                            (MM_Rand_Next() & 7) + 10, HAHEN_OBJECT_DEFAULT, 10, NULL);
...
void EnRailgibud_SpawnDust(PlayState* play, Vec3f* basePos, f32 randomnessScale, s32 dustCount, s16 dustScale,
    ...
    for (i = dustCount; i >= 0; i--) {
        dustVelocity.x = (MM_Rand_ZeroOne() - 0.5f) * randomnessScale;
        dustVelocity.z = (MM_Rand_ZeroOne() - 0.5f) * randomnessScale;

        dustPos.x = basePos->x + dustVelocity.x;
        dustPos.y = ((MM_Rand_ZeroOne() - 0.5f) * randomnessScale) + basePos->y;
        dustPos.z = basePos->z + dustVelocity.z;

        dustVelocity.x *= 0.5f;
        dustVelocity.z *= 0.5f;
        func_800B1210(play, &dustPos, &dustVelocity, &dustAccel,
                      TRUNCF_BINANG(MM_Rand_ZeroOne() * dustScale * 0.2f) + dustScale, scaleStep);
    }
}
...
s32 EnRailgibud_PlayerInRangeWithCorrectState(EnRailgibud* this, PlayState* play) {
    ...
    if (MM_Player_GetMask(play) == PLAYER_MASK_GIBDO) {
        return false;
    }

    if ((MM_Actor_WorldDistXYZToPoint(&player->actor, &this->actor.home.pos) < 100.0f) &&
...
s32 EnRailgibud_MoveToIdealGrabPositionAndRotation(EnRailgibud* this, PlayState* play) {
    ...
    targetPos.x -= 25.0f * MM_Math_SinS(player->actor.shape.rot.y);
    targetPos.z -= 25.0f * MM_Math_CosS(player->actor.shape.rot.y);
    distanceFromTargetPos = Math_Vec3f_StepTo(&this->actor.world.pos, &targetPos, 10.0f);
    distanceFromTargetAngle = MM_Math_SmoothStepToS(&this->actor.shape.rot.y, player->actor.shape.rot.y, 1, 0x1770, 0x64);
    this->actor.world.rot.y = this->actor.shape.rot.y;
    if (GET_PLAYER_FORM == PLAYER_FORM_HUMAN) {
        distanceFromTargetYOffset = MM_Math_SmoothStepToF(&this->actor.shape.yOffset, -1500.0f, 1.0f, 150.0f, 0.0f);
    }
...
void EnRailgibud_MoveGrabbedPlayerAwayFromWall(EnRailgibud* this, PlayState* play) {
    ...
    if ((this->actionFunc == EnRailgibud_Grab) && (this->grabState != EN_RAILGIBUD_GRAB_RELEASE)) {
        MM_Actor_UpdateBgCheckInfo(play, &this->actor, 30.0f, 20.0f, 35.0f, UPDBGCHECKINFO_FLAG_1);
    } else {
    ...
    if ((this->actionFunc == EnRailgibud_Grab) && (this->grabState == EN_RAILGIBUD_GRAB_START) &&
        ...
        targetPos = player->actor.world.pos;
        targetPos.x += 10.0f * MM_Math_SinS(this->actor.wallYaw);
        targetPos.z += 10.0f * MM_Math_CosS(this->actor.wallYaw);
...
void EnRailgibud_CheckForGibdoMask(EnRailgibud* this, PlayState* play) {
    if ((this->actionFunc != EnRailgibud_Grab) && (this->actionFunc != EnRailgibud_Damage) &&
        ...
        } else if (MM_Player_GetMask(play) != PLAYER_MASK_GIBDO) {
            this->actor.flags &= ~(ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_FRIENDLY);
            this->actor.flags |= (ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_HOSTILE);
            if (this->type == EN_RAILGIBUD_TYPE_REDEAD) {
                this->actor.hintId = TATL_HINT_ID_REDEAD;
...
void EnRailgibud_CheckIfTalkingToPlayer(EnRailgibud* this, PlayState* play) {
    if ((this->textId == 0) && (this->type == EN_RAILGIBUD_TYPE_GIBDO)) {
        if (Actor_TalkOfferAccepted(&this->actor, &play->state)) {
            ...
            MM_Message_StartTextbox(play, 0x13B2, &this->actor);
            this->textId = 0x13B2;
            Actor_PlaySfx(&this->actor, NA_SE_EN_REDEAD_AIM);
            this->actor.speed = 0.0f;
        } else if (CHECK_FLAG_ALL(this->actor.flags, (ACTOR_FLAG_ATTENTION_ENABLED | ACTOR_FLAG_FRIENDLY)) &&
                   !(this->collider.base.acFlags & AC_HIT)) {
        ...
        switch (MM_Message_GetState(&play->msgCtx)) {
            case TEXT_STATE_EVENT:
                if (MM_Message_ShouldAdvance(play)) {
                    MM_Message_StartTextbox(play, 0x13B3, &this->actor);
                    this->textId = 0x13B3;
                }
                break;
...
void EnRailgibud_SinkIntoGround(EnRailgibud* this, PlayState* play) {
    if (this->sinkTimer != 0) {
        this->sinkTimer--;
    } else if (MM_Math_SmoothStepToF(&this->actor.shape.yOffset, -9500.0f, 0.5f, 200.0f, 10.0f) < 10.0f) {
        MM_Actor_Kill(&this->actor);
    } else {
        ...
        if (this->cueId != play->csCtx.actorCues[cueChannel]->id) {
            ...
            switch (play->csCtx.actorCues[cueChannel]->id) {
                ...
                case 2:
                    ...
                    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_SLUMP_START);
                    break;

                case 3:
                    this->csAnimIndex = EN_RAILGIBUD_ANIM_CONVULSION;
                    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_CONVULSION);
                    break;

                case 4:
                    this->csAnimIndex = EN_RAILGIBUD_ANIM_ARMS_UP_START;
                    Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_ARMS_UP_START);
                    break;
            ...
            if (this->csAnimIndex == EN_RAILGIBUD_ANIM_SLUMP_START) {
                ...
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_SLUMP_LOOP);
            } else if (this->csAnimIndex == EN_RAILGIBUD_ANIM_ARMS_UP_START) {
                this->csAnimIndex = EN_RAILGIBUD_ANIM_ARMS_UP_LOOP;
                Actor_ChangeAnimationByInfo(&this->skelAnime, MM_sAnimationInfo, EN_RAILGIBUD_ANIM_ARMS_UP_LOOP);
