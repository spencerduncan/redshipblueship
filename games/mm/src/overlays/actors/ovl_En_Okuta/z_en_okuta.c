...
void MM_EnOkuta_Init(Actor* thisx, PlayState* play2);
void MM_EnOkuta_Destroy(Actor* thisx, PlayState* play);
void MM_EnOkuta_Update(Actor* thisx, PlayState* play2);
void MM_EnOkuta_Draw(Actor* thisx, PlayState* play);

void MM_EnOkuta_SetupWaitToAppear(EnOkuta* this);
void MM_EnOkuta_WaitToAppear(EnOkuta* this, PlayState* play);
void MM_EnOkuta_SetupAppear(EnOkuta* this, PlayState* play);
void MM_EnOkuta_Appear(EnOkuta* this, PlayState* play);
void MM_EnOkuta_SetupHide(EnOkuta* this);
void MM_EnOkuta_Hide(EnOkuta* this, PlayState* play);
void EnOkuta_SetupFloat(EnOkuta* this);
void EnOkuta_Float(EnOkuta* this, PlayState* play);
void MM_EnOkuta_SetupShoot(EnOkuta* this, PlayState* play);
void MM_EnOkuta_Shoot(EnOkuta* this, PlayState* play);
void EnOkuta_Damaged(EnOkuta* this, PlayState* play);
void MM_EnOkuta_SetupDie(EnOkuta* this);
void MM_EnOkuta_Die(EnOkuta* this, PlayState* play);
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 4, 15, 60, 100 };
...
void MM_EnOkuta_Init(Actor* thisx, PlayState* play2) {
    ...
    if ((EN_OKUTA_GET_TYPE(thisx) == EN_OKUTA_TYPE_RED_OCTOROK) ||
        ...
        MM_SkelAnime_Init(play, &this->skelAnime, &gOctorokSkel, &gOctorokAppearAnim, this->jointTable, this->morphTable,
                       OCTOROK_LIMB_MAX);
        ...
        thisx->floorHeight =
            MM_BgCheck_EntityRaycastFloor5(&play->colCtx, &thisx->floorPoly, &bgId, thisx, &thisx->world.pos);
        ...
        MM_EnOkuta_SetupWaitToAppear(this);
    } else {
        MM_ActorShape_Init(&thisx->shape, 1100.0f, MM_ActorShadow_DrawCircle, 18.0f);
 ...
 */
void MM_EnOkuta_Freeze(EnOkuta* this) {
...
void MM_EnOkuta_SpawnBubbles(EnOkuta* this, PlayState* play) {
    s32 i;
 ...
 */
void MM_EnOkuta_SpawnSplash(EnOkuta* this, PlayState* play) {
    MM_EffectSsGSplash_Spawn(play, &this->actor.home.pos, NULL, NULL, 0, 1300);
}

void MM_EnOkuta_SpawnRipple(EnOkuta* this, PlayState* play) {
    ...
    if (((play->gameplayFrames % 7) == 0) && (diffY < 50.0f) && (diffY >= -20.0f)) {
        ...
        MM_EffectSsGRipple_Spawn(play, &pos, 250, 650, 0);
    }
}
...
f32 EnOkuta_GetFloatHeight(EnOkuta* this) {
    ...
    // `MM_EnOkuta_Init` sets the Octorok's home y-position to be the water's surface, so this ensures that the Octorok
...
void MM_EnOkuta_SpawnProjectile(EnOkuta* this, PlayState* play) {
    Vec3f pos;
    Vec3f velocity;
    ...
    if (MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_OKUTA, pos.x, pos.y, pos.z, this->actor.shape.rot.x,
...
void MM_EnOkuta_SetupWaitToAppear(EnOkuta* this) {
    ...
    this->actionFunc = MM_EnOkuta_WaitToAppear;
}
...
void MM_EnOkuta_WaitToAppear(EnOkuta* this, PlayState* play) {
    ...
    if (this->actor.xzDistToPlayer < 480.0f && this->actor.xzDistToPlayer > 200.0f) {
        if (EN_OKUTA_GET_TYPE(&this->actor) == EN_OKUTA_TYPE_RED_OCTOROK) {
            MM_EnOkuta_SetupAppear(this, play);
...
void MM_EnOkuta_SetupAppear(EnOkuta* this, PlayState* play) {
    this->actor.draw = MM_EnOkuta_Draw;
    this->actor.shape.rot.y = this->actor.yawTowardsPlayer;
    this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED;
    MM_Animation_PlayOnce(&this->skelAnime, &gOctorokAppearAnim);
    MM_EnOkuta_SpawnBubbles(this, play);
    this->actionFunc = MM_EnOkuta_Appear;
}
...
void MM_EnOkuta_Appear(EnOkuta* this, PlayState* play) {
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        if (curFrame <= 4.0f) {
            MM_Actor_SetScale(&this->actor, curFrame * 0.25f * 0.01f);
    ...
    if ((MM_Animation_OnFrame(&this->skelAnime, 3.0f)) || (MM_Animation_OnFrame(&this->skelAnime, 15.0f))) {
        MM_EnOkuta_SpawnSplash(this, play);
    }
}

void MM_EnOkuta_SetupHide(EnOkuta* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gOctorokHideAnim);
    this->actionFunc = MM_EnOkuta_Hide;
}
 ...
 */
void MM_EnOkuta_Hide(EnOkuta* this, PlayState* play) {
    MM_Math_ApproachF(&this->actor.world.pos.y, this->actor.home.pos.y, 0.5f, 30.0f);
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        if (curFrame >= 4.0f) {
            MM_Actor_SetScale(&this->actor, (6.0f - curFrame) * 0.5f * 0.01f);
        }
    }
...
void EnOkuta_Float(EnOkuta* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 0.5f)) {
    ...
    if ((this->actor.xzDistToPlayer > 560.0f) ||
        ...
        s16 yawDiff = MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 3, 0xE38, 0x38E);
        ...
        if ((ABS_ALT(yawDiff) < 0x38E) && ((((EN_OKUTA_GET_TYPE(&this->actor) == EN_OKUTA_TYPE_RED_OCTOROK)) &&
            ...
            MM_EnOkuta_SetupShoot(this, play);
        }
    }
}

void MM_EnOkuta_SetupShoot(EnOkuta* this, PlayState* play) {
    MM_Animation_PlayOnce(&this->skelAnime, &gOctorokShootAnim);

    if (this->actionFunc != MM_EnOkuta_Shoot) {
        // `MM_EnOkuta_Shoot` uses `timer` to track how many projectiles are remaining in the current "volley". The Octorok
    ...
    this->actionFunc = MM_EnOkuta_Shoot;
}
...
void MM_EnOkuta_Shoot(EnOkuta* this, PlayState* play) {
    ...
    MM_Math_ApproachS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 3, 0x71C);
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        if (EN_OKUTA_GET_TYPE(&this->actor) == EN_OKUTA_TYPE_RED_OCTOROK) {
            ...
            if (curFrame < 13.0f) {
                this->actor.world.pos.y = MM_Math_SinF(0.2617889f * curFrame) * this->jumpHeight + this->actor.home.pos.y;
            }

            if ((this->jumpHeight > 50.0f) && MM_Animation_OnFrame(&this->skelAnime, 13.0f)) {
                MM_EnOkuta_SpawnSplash(this, play);
                Actor_PlaySfx(&this->actor, NA_SE_EN_DAIOCTA_LAND);
            }
        } else {
            ...
            if ((curFrame = this->skelAnime.curFrame) < 13.0f) {
                ...
                pitch = MM_Actor_WorldPitchTowardPoint(&this->actor, &targetPos);
                pitch = CLAMP(pitch, -0x2AAA, 0);

                this->actor.shape.rot.x = MM_Math_SinF(0.2617889f * curFrame) * pitch;
            }
        }

        if (MM_Animation_OnFrame(&this->skelAnime, 6.0f)) {
            MM_EnOkuta_SpawnProjectile(this, play);
        }
    }
...
void EnOkuta_SetupDamaged(EnOkuta* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gOctorokHitAnim, -5.0f);
    MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 255, COLORFILTER_BUFFLAG_OPA, 11);
...
void EnOkuta_Damaged(EnOkuta* this, PlayState* play) {
    ...
    MM_Math_ApproachF(&this->actor.world.pos.y, this->actor.home.pos.y, 0.5f, 5.0f);
}

void MM_EnOkuta_SetupDie(EnOkuta* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gOctorokDieAnim, -3.0f);
    this->timer = 0;
    this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
    this->actionFunc = MM_EnOkuta_Die;
}
...
void MM_EnOkuta_Die(EnOkuta* this, PlayState* play) {
    ...
    if (this->timer < 3) {
        MM_Actor_SetScale(&this->actor, ((this->timer * 0.25f) + 1.0f) * 0.01f);
        return;
    }

    if (this->timer < 6) {
        MM_Actor_SetScale(&this->actor, (1.5f - ((this->timer - 2) * 0.2333f)) * 0.01f);
        return;
    }

    if (this->timer < 11) {
        MM_Actor_SetScale(&this->actor, (((this->timer - 5) * 0.04f) + 0.8f) * 0.01f);
        return;
    }

    if (MM_Math_StepToF(&this->actor.scale.x, 0.0f, 0.0005f)) {
        MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 30, NA_SE_EN_COMMON_WATER_MID);
        ...
        for (i = 0; i < 20; i++) {
            ...
            MM_EffectSsDtBubble_SpawnCustomColor(play, &this->actor.world.pos, &velocity, &sBubbleAccel, &sBubblePrimColor,
                                              &sBubbleEnvColor, MM_Rand_S16Offset(100, 50), 25, false);
        }
...
void EnOkuta_SetupFrozen(EnOkuta* this, PlayState* play) {
    ...
    this->actor.child = MM_Actor_SpawnAsChild(
...
void EnOkuta_FrozenInIceBlock(EnOkuta* this, PlayState* play) {
    ...
    if ((this->actor.child == NULL) || (this->actor.child->update == NULL)) {
        ...
        if (MM_Math_StepToF(&this->actor.world.pos.y, this->actor.home.pos.y, 10.0f)) {
            this->actor.flags &= ~ACTOR_FLAG_UPDATE_CULLING_DISABLED;
            EnOkuta_SetupFloat(this);
        }
    }
...
void EnOkuta_SetupSpin(EnOkuta* this) {
    MM_Animation_Change(&this->skelAnime, &gOctorokHitAnim, 1.0f, 0.0f, MM_Animation_GetLastFrame(&gOctorokHitAnim) - 3.0f,
                     ANIMMODE_ONCE, -3.0f);
    this->timer = 20;
...
void EnOkuta_Spin(EnOkuta* this, PlayState* play) {
    this->timer--;
    MM_Math_ScaledStepToS(&this->actor.shape.rot.x, 0, 0x400);

    if (MM_SkelAnime_Update(&this->skelAnime)) {
        MM_Animation_Change(&this->skelAnime, &gOctorokHitAnim, 1.0f, 0.0f,
                         MM_Animation_GetLastFrame(&gOctorokHitAnim) - 3.0f, ANIMMODE_ONCE, -3.0f);
    }

    if (this->timer < 10) {
        this->actor.shape.rot.y += (s16)(0x2000 * MM_Math_SinF(this->timer * (M_PIf / 20.0f)));
    } else {
...
void EnOkuta_Projectile_Fly(EnOkuta* this, PlayState* play) {
    ...
    if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) || (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) ||
        ...
        MM_EffectSsHahen_SpawnBurst(play, &fragmentPos, 6.0f, 0, 1, 2, 15, OBJECT_OKUTA, 10, gOctorokProjectileDL);
 ...
 */
void MM_EnOkuta_UpdateHeadScale(EnOkuta* this) {
    f32 curFrame = this->skelAnime.curFrame;

    if (this->actionFunc == MM_EnOkuta_Appear) {
        if (curFrame < 8.0f) {
    ...
    } else if (this->actionFunc == MM_EnOkuta_Hide) {
        if (curFrame < 3.0f) {
        ...
        this->headScale.x = this->headScale.z = 1.0f;
    } else if (this->actionFunc == MM_EnOkuta_Shoot) {
        if (curFrame < 5.0f) {
        ...
        this->headScale.x = this->headScale.z = 1.0f;
        this->headScale.y = MM_Math_SinF((M_PIf / 16) * curFrame) * 0.2f + 1.0f;
    } else {
        this->headScale.x = this->headScale.y = this->headScale.z = 1.0f;
    }
}
...
void MM_EnOkuta_Update(Actor* thisx, PlayState* play2) {
    ...
    if (this->actionFunc != EnOkuta_FrozenInIceBlock) {
        MM_EnOkuta_UpdateHeadScale(this);
        ...
        if (this->actionFunc == MM_EnOkuta_Appear || this->actionFunc == MM_EnOkuta_Hide) {
        ...
        if (this->actor.draw != NULL) {
            ...
            MM_EnOkuta_SpawnRipple(this, play);
        }

        MM_Actor_SetFocus(&this->actor, 15.0f);
        ...
        if (this->drawDmgEffAlpha > 0.0f) {
            ...
            if (!MM_Math_StepToF(&this->drawDmgEffFrozenSteamScale, 0.6f, 15.0f * 0.001f)) {
...
void EnOkuta_Projectile_Update(Actor* thisx, PlayState* play) {
    ...
    if (!(player->stateFlags1 & (PLAYER_STATE1_2 | PLAYER_STATE1_TALKING | PLAYER_STATE1_DEAD | PLAYER_STATE1_200 |
        ...
        MM_Math_Vec3f_Copy(&prevPos, &this->actor.world.pos);
        MM_Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 15.0f, 30.0f,
                                UPDBGCHECKINFO_FLAG_1 | UPDBGCHECKINFO_FLAG_2 | UPDBGCHECKINFO_FLAG_4);

        if ((this->actor.bgCheckFlags & BGCHECKFLAG_WALL) &&
            (MM_SurfaceType_IsIgnoredByProjectiles(&play->colCtx, this->actor.wallPoly, this->actor.wallBgId))) {
            canRestorePrevPos = true;
            this->actor.bgCheckFlags &= ~BGCHECKFLAG_WALL;
        }

        if ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) &&
            (MM_SurfaceType_IsIgnoredByProjectiles(&play->colCtx, this->actor.floorPoly, this->actor.floorBgId))) {
            canRestorePrevPos = true;
            this->actor.bgCheckFlags &= ~BGCHECKFLAG_GROUND;
        }

        if (canRestorePrevPos && !(this->actor.bgCheckFlags & (BGCHECKFLAG_GROUND | BGCHECKFLAG_WALL))) {
            MM_Math_Vec3f_Copy(&this->actor.world.pos, &prevPos);
        }
 ...
 * `scale` parameter.
 */
s32 MM_EnOkuta_GetSnoutScale(EnOkuta* this, f32 curFrame, Vec3f* scale) {
    if (this->actionFunc == EnOkuta_Float) {
        scale->z = scale->y = 1.0f;
        scale->x = MM_Math_SinF((M_PIf / 16) * curFrame) * 0.4f + 1.0f;
    } else if (this->actionFunc == MM_EnOkuta_Shoot) {
        if (curFrame < 5.0f) {
            scale->z = 1.0f;
    ...
    } else if (this->actionFunc == MM_EnOkuta_Die) {
...
s32 MM_EnOkuta_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    EnOkuta* this = (EnOkuta*)thisx;
    s32 shouldScaleLimb = false;
    Vec3f scale;
    f32 curFrame = this->skelAnime.curFrame;

    if (this->actionFunc == MM_EnOkuta_Die) {
        curFrame += this->timer;
    }

    if (limbIndex == OCTOROK_LIMB_HEAD) {
        if (this->headScale.x != 1.0f || this->headScale.y != 1.0f || this->headScale.z != 1.0f) {
            MM_Math_Vec3f_Copy(&scale, &this->headScale);
            shouldScaleLimb = true;
        }
    } else if (limbIndex == OCTOROK_LIMB_SNOUT) {
        shouldScaleLimb = MM_EnOkuta_GetSnoutScale(this, curFrame, &scale);
    }
    ...
    if (bodyPartIndex != BODYPART_NONE) {
        if (bodyPartIndex == EN_OKUTA_BODYPART_SNOUT) {
            ...
            for (i = 0; i < ARRAY_COUNT(sEffectsBodyPartOffsets); i++) {
                MM_Matrix_MultVec3f(&sEffectsBodyPartOffsets[i], &this->bodyPartsPos[bodyPartIndex + i]);
            }
        } else {
...
void MM_EnOkuta_Draw(Actor* thisx, PlayState* play) {
    ...
    if (EN_OKUTA_GET_TYPE(&this->actor) < EN_OKUTA_TYPE_PROJECTILE_BASE) {
        ...
        POLY_OPA_DISP = &gfx[2];
        MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, MM_EnOkuta_OverrideLimbDraw,
