...
void EnTanron4_Init(Actor* thisx, PlayState* play2) {
    ...
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &gSeagullSkel, &gSeagullFlapAnim, this->jointTable, this->morphTable,
                       SEAGULL_LIMB_RIGHT_WING_MAX);

    thisx->flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
    thisx->speed = 3.0f + KREG(48);
    thisx->cullingVolumeDistance = 10000.0f + KREG(70);
    this->randRollTimer = MM_Rand_ZeroFloat(10.0f);
    ...
    if (thisx->params == SEAGULL_FOLLOW_ACTOR) {
        ...
        if (thisx->params >= 0) {
            ...
            for (i = 0; i < thisx->params; i++) {
                MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_TANRON4, thisx->world.pos.x + MM_Rand_CenteredFloat(500.0f),
                            thisx->world.pos.y + MM_Rand_CenteredFloat(100.0f),
                            thisx->world.pos.z + MM_Rand_CenteredFloat(500.0f), 0, MM_Rand_ZeroFloat(0x10000), 0,
                            SEAGULL_CLONE);
            }
        }
...
void EnTanron4_SetupFlyNearHome(EnTanron4* this) {
    ...
    MM_Animation_MorphToLoop(&this->skelAnime, &gSeagullFlapAnim, 0.0f);
}
...
void EnTanron4_FlyNearHome(EnTanron4* this, PlayState* play) {
    ...
    if ((CURRENT_TIME > CLOCK_TIME(20, 0)) || (CURRENT_TIME < CLOCK_TIME(4, 0))) {
        MM_Math_ApproachF(&this->timeInfluence, 1500.0f, 1.0f, 1.0f);
    } else {
        MM_Math_ApproachZeroF(&this->timeInfluence, 1.0f, 1.0f);
    }
    ...
    if ((this->timers[SEAGULL_TIMER_CHOOSE_TARGET] == 0) || (distToTarget < 100.0f)) {
        ...
        this->targetPos.y = this->actor.home.pos.y + MM_Rand_CenteredFloat(100.0f);
        this->targetPos.z = this->actor.home.pos.z + MM_Rand_CenteredFloat(500.0f);

        this->timers[SEAGULL_TIMER_CHOOSE_TARGET] = MM_Rand_ZeroFloat(100.0f) + 60.0f;
        this->step = 0;
    }

    yRot = MM_Math_Atan2S(xDiff, zDiff);
    xRot = MM_Math_Atan2S(yDiff, distToTarget);
    ...
    switch (this->flyState) {
        case SEAGULL_FLY_FLAP:
            if ((this->timers[SEAGULL_TIMER_FLY_STATE] == 0) && MM_Animation_OnFrame(&this->skelAnime, 2.0f + KREG(42))) {
                this->flyState = SEAGULL_FLY_GLIDE;
                this->timers[SEAGULL_TIMER_FLY_STATE] = MM_Rand_ZeroFloat(50.0f) + 50.0f;
                MM_Animation_MorphToLoop(&this->skelAnime, &gSeagullFlapAnim, -15.0f + KREG(43));
        ...
        case SEAGULL_FLY_GLIDE:
            if (((this->randRollTimer % 8) == 0) && (MM_Rand_ZeroOne() < 0.5f)) {
                this->rollTarget = MM_Rand_CenteredFloat(3000.0f + KREG(44));
            }

            if (this->timers[SEAGULL_TIMER_FLY_STATE] == 0) {
                this->flyState = SEAGULL_FLY_FLAP;
                this->timers[SEAGULL_TIMER_FLY_STATE] = MM_Rand_ZeroFloat(50.0f) + 70.0f;
                MM_Animation_MorphToLoop(&this->skelAnime, &gSeagullFlapAnim, -10.0f + KREG(43));
                this->skelAnime.curFrame = 2.0f + KREG(42);
            }
            break;
    ...
    MM_Math_ApproachS(&this->roll, this->rollTarget, 3, 0x3E8 + KREG(45));
}
...
void EnTanron4_FlyNearActor(EnTanron4* this, PlayState* play) {
    ...
    if ((this->timers[SEAGULL_TIMER_CHOOSE_TARGET] == 0) || (distToTarget < 100.0f)) {
        ...
        this->targetPos.y = targetActor->world.pos.y + KREG(80) + 100.0f + MM_Rand_CenteredFloat(100.0f + KREG(81));
        this->targetPos.z = targetActor->world.pos.z + MM_Rand_CenteredFloat(200.0f + KREG(82));
...
void EnTanron4_Update(Actor* thisx, PlayState* play) {
    ...
    MM_Actor_SetScale(&this->actor, 0.001f * KREG(16) + 0.01f);
