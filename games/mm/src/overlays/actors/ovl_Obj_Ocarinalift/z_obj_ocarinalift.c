...
void ObjOcarinalift_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->unk160 < 0.01f) {
        ...
        if ((OBJOCARINALIFT_GET_C(&this->dyna.actor) != OBJOCARINALIFT_PARAM_1) &&
            MM_Flags_GetSwitch(play, OBJOCARINALIFT_GET_SWITCH_FLAG(&this->dyna.actor))) {
            func_80AC96B4(this);
        } else {
            func_80AC9AB8(this);
        }
...
void func_80AC96D0(ObjOcarinalift* this, PlayState* play) {
    ...
    MM_Math_Vec3s_ToVec3f(&sp48, this->pathPoints + this->unk168 + this->unk16C);
    MM_Math_Vec3f_Diff(&sp48, &thisx->world.pos, &thisx->velocity);
    magnitude = MM_Math3D_Vec3fMagnitude(&thisx->velocity);
    ...
    if ((thisx->speed + 0.05f) < magnitude) {
        MM_Math_Vec3f_Scale(&thisx->velocity, thisx->speed / magnitude);
    ...
    if (Actor_NoOcarinaInteraction(&this->dyna.actor, play)) {
        if (play->msgCtx.ocarinaMode == OCARINA_MODE_END) {
            if (play->msgCtx.lastPlayedSong == 0) {
                if (OBJOCARINALIFT_GET_C(&this->dyna.actor) != OBJOCARINALIFT_PARAM_1) {
                    MM_Flags_SetSwitch(play, OBJOCARINALIFT_GET_SWITCH_FLAG(&this->dyna.actor));
                }
...
void ObjOcarinalift_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_Gfx_DrawDListOpa(play, object_raillift_DL_001E40);
    MM_Gfx_DrawDListXlu(play, object_raillift_DL_001DB0);
}
