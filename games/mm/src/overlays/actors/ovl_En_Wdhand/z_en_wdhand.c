...
static ColliderJntSphElementInit MM_sJntSphElementsInit[EN_WDHAND_NUM_COLLIDER_ELEMENTS] = {
    {
        {
            ELEM_MATERIAL_UNK1,
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 1, 25, 25, MASS_HEAVY };
...
void EnWdhand_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &gDexihandSkel, &gDexihandIdleAnim, this->jointTable, this->morphTable,
                       DEXIHAND_LIMB_MAX);
    ...
    MM_Matrix_MtxFCopy(&this->relativeToWorldTransform, MM_Matrix_GetCurrent());
    ...
    MM_Matrix_Transpose(&this->relativeToWorldTransform);
...
void EnWdhand_GetRelativeRotationsToPlayerRightFoot(EnWdhand* this, Player* player, s16* pitchOut, s16* yawOut) {
    ...
    MM_Math_Vec3f_Diff(&player->bodyPartsPos[PLAYER_BODYPART_RIGHT_FOOT], &this->actor.world.pos, &diffToPlayerRightFoot);
    // Re-express the difference in actor-relative coordinates
    MM_Matrix_Put(&this->relativeToWorldTransform);
    MM_Matrix_MultVec3f(&diffToPlayerRightFoot, &diffToPlayerRightFootRelative);
    // Return actor-relative yaw and pitch
    *yawOut = MM_Actor_WorldYawTowardPoint(&this->actor, &diffToPlayerRightFootRelative);
    *pitchOut = MM_Actor_WorldPitchTowardPoint(&this->actor, &diffToPlayerRightFootRelative) + 0x4000;
}

void EnWdhand_SetTransform(EnWdhand* this, s32 limbIndex, s32 arg2, Vec3f* translation) {
    MtxF* currentMatrix = MM_Matrix_GetCurrent();

    MM_Matrix_RotateZYX(this->limbRotations[limbIndex].x, this->limbRotations[limbIndex].y, 0, MTXMODE_APPLY);
    if (arg2) {
    ...
    if (limbIndex == DEXIHAND_LIMB_03) {
        ...
        if (this->limbScaleFactors[limbIndex] < 0.1f) {
            MM_Matrix_Scale(0.0f, this->globalLimbScaleFactor * this->limbScaleFactors[limbIndex], 0.0f, MTXMODE_APPLY);
        } else {
            MM_Matrix_Scale(1.0f, this->globalLimbScaleFactor * this->limbScaleFactors[limbIndex], 1.0f, MTXMODE_APPLY);
        }
    }
...
s16 EnWdhand_GetLimbXRotation(EnWdhand* this, s32 limbIndex) {
    return this->limbRotations[limbIndex].z * MM_Math_SinF((this->timer - limbIndex * 20) * (M_PIf / 40));
}

void EnWdhand_SetupIdle(EnWdhand* this) {
    s32 i;

    MM_Animation_MorphToLoop(&this->skelAnime, &gDexihandIdleAnim, -5.0f);

    if (this->actionFunc != EnWdhand_ReturnToIdle) {
        this->timer = 80;
        this->limbRotations[0].y = MM_Rand_Next() >> 16;

        for (i = 1; i < ARRAY_COUNT(this->limbRotations); i++) {
            this->limbRotations[i].y = this->limbRotations[i - 1].y + (s32)MM_Rand_ZeroFloat(0x2000) + 0x8000;
        }

        for (i = 0; i < ARRAY_COUNT(this->limbRotations); i++) {
            this->limbRotations[i].z = MM_Rand_S16Offset(0x1800, 0x1000);
        }
    }
...
void EnWdhand_Idle(EnWdhand* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->limbRotations); i++) {
        ...
        if (this->timer == 20 * i) {
            if (i != 0) {
                this->limbRotations[i].y = this->limbRotations[i - 1].y + (s32)MM_Rand_CenteredFloat(16384.0f);
                this->limbRotations[i].y -= 0x8000;
            } else {
                this->limbRotations[0].y += (s16)(s32)MM_Rand_CenteredFloat(16384.0f);
            }
            this->limbRotations[i].z = MM_Rand_S16Offset(0x1800, 0x1000);
        }
    }
...
void EnWdhand_SetupLungeForPlayer(EnWdhand* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gDexihandOpenAnim, -5.0f);
    this->timer = 5;
...
void EnWdhand_LungeForPlayer(EnWdhand* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->globalLimbScaleFactor, 1.5f, 0.05f);
    EnWdhand_GetRelativeRotationsToPlayerRightFoot(this, player, &pitchToPlayerRightFoot, &yawToPlayerRightFoot);
    ...
    for (i = 0; i < ARRAY_COUNT(this->limbRotations); i++) {
        ...
        if (ABS_ALT(yawDiff) < 0x4000) {
            allStepsDone &=
                MM_Math_ScaledStepToS(&this->limbRotations[i].x, pitchToPlayerRightFoot, (i * 0.1f + 1.0f) * 1920.0f);
            allStepsDone &= MM_Math_ScaledStepToS(&this->limbRotations[i].y, yawToPlayerRightFoot, 0x800);
        } else {
            allStepsDone &=
                MM_Math_ScaledStepToS(&this->limbRotations[i].x, -pitchToPlayerRightFoot, (i * 0.1f + 1.0f) * 1920.0f);
            allStepsDone &= MM_Math_ScaledStepToS(&this->limbRotations[i].y, yawToPlayerRightFoot + 0x8000, 0x800);
        }
    }
...
void EnWdhand_SetupFailedToGrabPlayer(EnWdhand* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gDexihandCloseAnim);
    this->collider.base.atFlags &= ~AT_ON;
    this->collider.base.atFlags &= ~AT_HIT;
...
void EnWdhand_SetupReturnToIdle(EnWdhand* this) {
    ...
    MM_Animation_MorphToLoop(&this->skelAnime, &gDexihandIdleAnim, -10.0f);
...
void EnWdhand_ReturnToIdle(EnWdhand* this, PlayState* play) {
    ...
    allStepsDone = MM_Math_ScaledStepToS(&this->limbRotations[0].x, EnWdhand_GetLimbXRotation(this, 0), 0x200);

    for (i = 1; i < ARRAY_COUNT(this->limbRotations); i++) {
        allStepsDone &= MM_Math_ScaledStepToS(&this->limbRotations[i].x, EnWdhand_GetLimbXRotation(this, i), 0x300);
        if (this->timer == 20 * i) {
            this->limbRotations[i].z = MM_Rand_S16Offset(0x1800, 0x1000);
        }
    }
    ...
    if (this->timer == 0) {
        this->timer = 80;
        this->limbRotations[0].z = MM_Rand_S16Offset(0x1800, 0x1000);
    }

    MM_Math_StepToF(&this->globalLimbScaleFactor, 1.0f, 0.05f);
...
void EnWdhand_SetupGrabbedPlayer(EnWdhand* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->limbRotations); i++) {
        this->limbRotations[i].z = MM_Rand_S16Offset(0x2000, 0x1000);
    }
    this->timer = 80;
...
void EnWdhand_GrabbedPlayer(EnWdhand* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->limbRotations); i++) {
        if (this->timer < 76) {
            this->limbRotations[i].x = this->limbRotations[i].z * MM_Math_SinF(t * (M_PIf / 8));
        } else {
            MM_Math_ScaledStepToS(&this->limbRotations[i].x, this->limbRotations[i].z * MM_Math_SinF(t * (M_PIf / 8)), 0x400);
        }
        if (t % 16 == 0) {
            if (t == 16) {
                this->limbRotations[i].y = 0;
            } else if (i != 0) {
                this->limbRotations[i].y = this->limbRotations[i - 1].y + (s32)MM_Rand_CenteredFloat(12288.0f);
            } else {
                this->limbRotations[0].y += (s16)(s32)MM_Rand_CenteredFloat(12288.0f);
            }
            this->limbRotations[i].z = MM_Rand_S16Offset(0x2000, 0x1000);
        }
        t += 2;
    ...
    if (this->timer < 4) {
        MM_Math_StepToF(&this->globalLimbScaleFactor, 1.5f, 1.0f / 8);
    } else if (this->timer < 16) {
        MM_Math_StepToF(&this->globalLimbScaleFactor, 2.0f, 1.0f / 24);
    }
    ...
    if (this->timer == 0) {
        ...
        Matrix_MtxFToYXZRot(MM_Matrix_GetCurrent(), &player->actor.shape.rot, false);
        ...
        if ((this->timer == 4) && (player->actor.parent == &this->actor)) {
            ...
            MM_Animation_PlayOnce(&this->skelAnime, &object_wdhand_Anim_000364);
        }
    }
}
...
void EnWdhand_SetupDie(EnWdhand* this) {
    ...
    this->actor.velocity.x = 2.0f * MM_Math_SinS(this->actor.world.rot.z);
    this->actor.velocity.z = 2.0f * MM_Math_CosS(this->actor.world.rot.z);
    this->timer = 5;
    this->actionFunc = EnWdhand_Die;
}

void EnWdhand_Die(EnWdhand* this, PlayState* play) {
    static Vec3f MM_sEffectVelocity = { 0.0f, 0.0f, 0.0f };
    static Vec3f MM_sEffectAccel = { 0.0f, 0.05f, 0.0f };
    ...
    MM_Math_Vec3f_Sum(endPoint, &this->actor.velocity, endPoint);
    ...
    if (this->limbIndexAfterCut < EN_WDHAND_NUM_SEGMENTS) {
        ...
        MM_Matrix_RotateZYX(this->actor.world.rot.x, this->actor.world.rot.z, 0, MTXMODE_NEW);
        Matrix_RotateYS(-this->actor.world.rot.z, MTXMODE_APPLY);
        MM_Matrix_Mult(&this->relativeToWorldTransform, MTXMODE_APPLY);
    ...
    for (i = 0; i < limbIndex; i++) {
        if (this->timer > 0) {
            MM_Math_ScaledStepToS(&this->limbRotations[i].x, this->limbRotations[i].z * MM_Math_SinF(t * (2 * M_PIf / 7)),
                               0x200);
        } else if (MM_Rand_ZeroOne() < 0.65f) {
            this->limbRotations[i].x = this->limbRotations[i].z * MM_Math_SinF(t * (2 * M_PIf / 7));
        }
        if (t % 7 == 0) {
            if (i != 0) {
                this->limbRotations[i].y = this->limbRotations[i - 1].y + (s32)MM_Rand_CenteredFloat(12288.0f);
            ...
            this->limbRotations[i].z = MM_Rand_S16Offset(i * 0x100 + 0xC00, 0x800);
        }
        t += 2;
    }
        ...
        if (MM_Math_StepToF(&this->actor.scale.x, 0.0f, 0.001f)) {
            for (pointIndex = 0; pointIndex < ARRAY_COUNT(this->endPoints); pointIndex++) {
                for (i = 0; i < 5; i++) {
                    ...
                    MM_sEffectVelocity.y = MM_Rand_ZeroOne() + 1.0f;
                    effectScale = MM_Rand_S16Offset(0x28, 0x28);
                    effPos.x = endPoint->x + MM_Rand_CenteredFloat(12.0f);
                    effPos.y = endPoint->y + MM_Rand_CenteredFloat(12.0f);
                    effPos.z = endPoint->z + MM_Rand_CenteredFloat(12.0f);
                    MM_EffectSsDtBubble_SpawnColorProfile(play, &effPos, &MM_sEffectVelocity, &MM_sEffectAccel, effectScale, 25,
                                                       2, true);
                }
            }
    ...
    for (i = 0; i < ARRAY_COUNT(this->endPoints); i++) {
        MM_sEffectVelocity.y = MM_Rand_ZeroOne() + 1.0f;
        MM_EffectSsDtBubble_SpawnColorProfile(play, &this->endPoints[i], &MM_sEffectVelocity, &MM_sEffectAccel,
                                           MM_Rand_S16Offset(40, 40), 25, 2, true);
    }
}
...
void EnWdhand_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_Math_Vec3f_Copy(&this->endPoints[EN_WDHAND_CUT_POINT], &limbPos);
    MM_Math_Vec3f_Copy(&limbPos, &this->endPoints[EN_WDHAND_HAND_POINT]);
    ...
    if (this->actor.scale.x < 0.01f) {
        MM_Matrix_Translate(0.0f, (10.0f / this->actor.scale.x) - 1000.0f, 0.0f, MTXMODE_APPLY);
    }

    MM_Math_Vec3f_Copy(&this->actor.focus.pos, &limbPos);
