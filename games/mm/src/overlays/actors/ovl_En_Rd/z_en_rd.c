...
void MM_EnRd_Init(Actor* thisx, PlayState* play);
void MM_EnRd_Destroy(Actor* thisx, PlayState* play);
void MM_EnRd_Update(Actor* thisx, PlayState* play);
void MM_EnRd_Draw(Actor* thisx, PlayState* play);
...
typedef enum {
    ...
    /* 0x6 */ EN_RD_DMGEFF_UNUSED_6 = 0x6,    // Referenced in MM_EnRd_Update, but no attack uses this damage effect.
...
void MM_EnRd_Init(Actor* thisx, PlayState* play) {
    ...
    if (EN_RD_GET_TYPE(&this->actor) > EN_RD_TYPE_GIBDO) {
        MM_SkelAnime_InitFlex(play, &this->skelAnime, &gRedeadSkel, &gGibdoRedeadIdleAnim, this->jointTable,
                           this->morphTable, REDEAD_LIMB_MAX);
    } else {
        MM_SkelAnime_InitFlex(play, &this->skelAnime, &gGibdoSkel, &gGibdoRedeadIdleAnim, this->jointTable,
                           this->morphTable, REDEAD_LIMB_MAX);
    }
 ...
 */
s32 EnRd_ShouldNotDance(PlayState* play) {
    if ((MM_Player_GetMask(play) == PLAYER_MASK_GIBDO) || (MM_Player_GetMask(play) == PLAYER_MASK_CAPTAIN) ||
        (MM_Player_GetMask(play) == PLAYER_MASK_GARO)) {
        return false;
    }
...
void EnRd_SetupIdle(EnRd* this) {
    if (EN_RD_GET_TYPE(&this->actor) != EN_RD_TYPE_CRYING) {
        MM_Animation_MorphToLoop(&this->skelAnime, &gGibdoRedeadIdleAnim, -6.0f);
    } else {
        MM_Animation_PlayLoop(&this->skelAnime, &gGibdoRedeadSobbingAnim);
    }

    this->action = EN_RD_ACTION_IDLE;
    this->animationJudderTimer = (MM_Rand_ZeroOne() * 10.0f) + 5.0f;
...
void EnRd_Idle(EnRd* this, PlayState* play) {
    MM_SkelAnime_Update(&this->skelAnime);
    MM_Math_SmoothStepToS(&this->headRotY, 0, 1, 0x64, 0);
    MM_Math_SmoothStepToS(&this->torsoRotY, 0, 1, 0x64, 0);

    if ((EN_RD_GET_TYPE(&this->actor) == EN_RD_TYPE_CRYING) && MM_Animation_OnFrame(&this->skelAnime, 0.0f)) {
        if (MM_Rand_ZeroOne() >= 0.5f) {
            MM_Animation_PlayLoop(&this->skelAnime, &gGibdoRedeadSobbingAnim);
        } else {
            MM_Animation_PlayLoop(&this->skelAnime, &gGibdoRedeadWipingTearsAnim);
        }
    } else {
        ...
        if (this->animationJudderTimer == 0) {
            ...
            this->animationJudderTimer = (MM_Rand_ZeroOne() * 10.0f) + 10.0f;
...
void EnRd_SetupSquattingDance(EnRd* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gGibdoRedeadSquattingDanceAnim, -6.0f);
...
void EnRd_SquattingDance(EnRd* this, PlayState* play) {
    ...
    if ((this->actor.xzDistToPlayer <= 150.0f) && EnRd_ShouldNotDance(play) && func_800B715C(play)) {
        ...
        MM_Animation_Change(&this->skelAnime, &gGibdoRedeadLookBackAnim, 0.0f, 0.0f, 19.0f, ANIMMODE_ONCE, -10.0f);
...
void EnRd_SetupClappingDance(EnRd* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gGibdoRedeadClappingDanceAnim, -6.0f);
...
void EnRd_SetupPirouette(EnRd* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &gGibdoRedeadPirouetteAnim, -6.0f);
...
void EnRd_Pirouette(EnRd* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
        this->pirouetteAngularVelocity = 0x1112;
    } else if (MM_Animation_OnFrame(&this->skelAnime, 15.0f)) {
...
void EnRd_SetupRiseFromCoffin(EnRd* this) {
    MM_Animation_Change(&this->skelAnime, &gGibdoRedeadIdleAnim, 0.0f, 0.0f, MM_Animation_GetLastFrame(&gGibdoRedeadIdleAnim),
                     ANIMMODE_LOOP, -6.0f);
...
void EnRd_RiseFromCoffin(EnRd* this, PlayState* play) {
    if (this->actor.shape.rot.x != -0x4000) {
        MM_Math_SmoothStepToS(&this->actor.shape.rot.x, 0, 1, 0x7D0, 0);
        if (MM_Math_SmoothStepToF(&this->actor.world.pos.y, this->actor.home.pos.y, 0.3f, 2.0f, 0.3f) == 0.0f) {
        ...
        if (MM_Math_SmoothStepToF(&this->actor.world.pos.y, this->actor.home.pos.y + 50.0f, 0.3f, 2.0f, 0.3f) == 0.0f) {
            if (this->coffinRiseForwardAccelTimer != 0) {
                this->coffinRiseForwardAccelTimer--;
                MM_Math_SmoothStepToF(&this->actor.speed, 6.0f, 0.3f, 1.0f, 0.3f);
            } else if (MM_Math_SmoothStepToF(&this->actor.speed, 0.0f, 0.3f, 1.0f, 0.3f) == 0.0f) {
                MM_Math_SmoothStepToS(&this->actor.shape.rot.x, 0, 1, 0x7D0, 0);
            }
        }
    }
...
void EnRd_SetupWalkToPlayer(EnRd* this, PlayState* play) {
    f32 endFrame = MM_Animation_GetLastFrame(&gGibdoRedeadWalkAnim);

    MM_Animation_Change(&this->skelAnime, &gGibdoRedeadWalkAnim, 1.0f, 4.0f, endFrame, ANIMMODE_LOOP_INTERP, -4.0f);
...
void EnRd_WalkToPlayer(EnRd* this, PlayState* play) {
    ...
    MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 1, 0xFA, 0);
    MM_Math_SmoothStepToS(&this->headRotY, 0, 1, 0x64, 0);
    MM_Math_SmoothStepToS(&this->torsoRotY, 0, 1, 0x64, 0);
    this->actor.world.rot.y = this->actor.shape.rot.y;
    MM_SkelAnime_Update(&this->skelAnime);

    if (MM_Actor_WorldDistXYZToPoint(&player->actor, &this->actor.home.pos) >= 150.0f) {
        EnRd_SetupWalkToHome(this, play);
    }

    if ((ABS_ALT(yaw) < 0x1554) && (MM_Actor_WorldDistXYZToActor(&this->actor, &player->actor) <= 150.0f)) {
    ...
    if (!this->grabWaitTimer && (MM_Actor_WorldDistXYZToActor(&this->actor, &player->actor) <= 45.0f) &&
        MM_Actor_IsFacingPlayer(&this->actor, 0x38E3)) {
        player->actor.freezeTimer = 0;
        if ((player->transformation == PLAYER_FORM_GORON) || (player->transformation == PLAYER_FORM_DEKU)) {
            if (MM_Actor_WorldDistXYZToPoint(&this->actor, &this->actor.home.pos) < 150.0f) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 10.0f) || MM_Animation_OnFrame(&this->skelAnime, 22.0f)) {
...
void EnRd_SetupWalkToHome(EnRd* this, PlayState* play) {
    MM_Animation_Change(&this->skelAnime, &gGibdoRedeadWalkAnim, 0.5f, 0.0f, MM_Animation_GetLastFrame(&gGibdoRedeadWalkAnim),
...
void EnRd_WalkToHome(EnRd* this, PlayState* play) {
    ...
    if (MM_Actor_WorldDistXYZToPoint(&this->actor, &this->actor.home.pos) >= 5.0f) {
        MM_Math_SmoothStepToS(&this->actor.shape.rot.y, yaw, 1, 0x1C2, 0);
    } else {
        this->actor.speed = 0.0f;
        if (!MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.home.rot.y, 1, 0x1C2, 0)) {
            if (EN_RD_GET_TYPE(&this->actor) != EN_RD_TYPE_CRYING) {
                EnRd_SetupIdle(this);
            } else {
    ...
    if (!(player->stateFlags1 & (PLAYER_STATE1_DEAD | PLAYER_STATE1_2000 | PLAYER_STATE1_4000 | PLAYER_STATE1_40000 |
        ...
        (MM_Actor_WorldDistXYZToPoint(&player->actor, &this->actor.home.pos) < 150.0f)) {
        this->actor.attentionRangeType = ATTENTION_RANGE_0;
...
void EnRd_SetupWalkToParent(EnRd* this) {
    ...
    MM_Animation_Change(&this->skelAnime, &gGibdoRedeadWalkAnim, 0.5f, 0.0f, endFrame, ANIMMODE_LOOP_INTERP, -4.0f);
...
void EnRd_WalkToParent(EnRd* this, PlayState* play) {
    ...
    if (this->actor.parent != NULL) {
        ...
        yaw = MM_Actor_WorldYawTowardPoint(&this->actor, &parentPos);

        MM_Math_SmoothStepToS(&this->actor.shape.rot.y, yaw, 1, 0xFA, 0);
        if (MM_Actor_WorldDistXYZToPoint(&this->actor, &parentPos) >= 45.0f) {
            this->actor.speed = 0.4f;
        } else {
            this->actor.speed = 0.0f;
            if (EN_RD_GET_TYPE(&this->actor) != EN_RD_TYPE_CRYING) {
...
void EnRd_SetupGrab(EnRd* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gGibdoRedeadGrabStartAnim);
...
void EnRd_Grab(EnRd* this, PlayState* play) {
    ...
    switch (this->grabState) {
        case EN_RD_GRAB_INITIAL_DAMAGE:
            MM_Animation_PlayLoop(&this->skelAnime, &gGibdoRedeadGrabAttackAnim);
            this->grabState++;
        ...
        case EN_RD_GRAB_START:
            MM_Math_SmoothStepToS(&this->headRotY, 0, 1, 0x5DC, 0);
            MM_Math_SmoothStepToS(&this->torsoRotY, 0, 1, 0x5DC, 0);
        ...
        case EN_RD_GRAB_ATTACK:
            if (!(player->stateFlags2 & PLAYER_STATE2_80) || (player->unk_B62 != 0)) {
                ...
                MM_Animation_Change(&this->skelAnime, &gGibdoRedeadGrabEndAnim, 0.5f, 0.0f,
                                 MM_Animation_GetLastFrame(&gGibdoRedeadGrabEndAnim), ANIMMODE_ONCE_INTERP, 0.0f);
                this->grabState++;
                this->action = EN_RD_ACTION_WALKING_TO_PLAYER_OR_RELEASING_GRAB;
                break;
            }
            ...
            switch (player->transformation) {
                case PLAYER_FORM_HUMAN:
                    MM_Math_SmoothStepToF(&this->actor.shape.yOffset, -1500.0f, 1.0f, 150.0f, 0.0f);
                    break;
            ...
            MM_Math_SmoothStepToF(&this->actor.world.pos.x,
                               (MM_Math_SinS(player->actor.shape.rot.y) * -25.0f) + player->actor.world.pos.x, 1.0f, 10.0f,
                               0.0f);
            MM_Math_SmoothStepToF(&this->actor.world.pos.y, player->actor.world.pos.y, 1.0f, 10.0f, 0.0f);
            MM_Math_SmoothStepToF(&this->actor.world.pos.z,
                               (MM_Math_CosS(player->actor.shape.rot.y) * -25.0f) + player->actor.world.pos.z, 1.0f, 10.0f,
                               0.0f);
            MM_Math_SmoothStepToS(&this->actor.shape.rot.y, player->actor.shape.rot.y, 1, 0x1770, 0);
        ...
        case EN_RD_GRAB_RELEASE:
            if (player->transformation != PLAYER_FORM_FIERCE_DEITY) {
                MM_Math_SmoothStepToF(&this->actor.shape.yOffset, 0.0f, 1.0f, 400.0f, 0.0f);
            }
            break;
...
void EnRd_SetupAttemptPlayerFreeze(EnRd* this) {
    MM_Animation_Change(&this->skelAnime, &gGibdoRedeadLookBackAnim, 0.0f, 0.0f,
                     MM_Animation_GetLastFrame(&gGibdoRedeadLookBackAnim), ANIMMODE_ONCE, 0.0f);
...
void EnRd_SetupGrabFail(EnRd* this) {
    ...
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gGibdoRedeadDamageAnim, -6.0f);
...
void EnRd_GrabFail(EnRd* this, PlayState* play) {
    ...
    MM_Math_SmoothStepToS(&this->headRotY, 0, 1, 0x12C, 0);
    MM_Math_SmoothStepToS(&this->torsoRotY, 0, 1, 0x12C, 0);
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnRd_TurnAwayAndShakeHead(EnRd* this, PlayState* play) {
    MM_Math_SmoothStepToS(&this->actor.world.rot.y, BINANG_ROT180(this->actor.yawTowardsPlayer), 5, 0xDAC, 0xC8);
    this->actor.shape.rot.y = this->actor.world.rot.y;
    if (this->headShakeTimer > 60) {
        EnRd_SetupWalkToHome(this, play);
        this->headShakeTimer = 0;
    } else {
        this->headRotY = MM_Math_SinS(this->headShakeTimer * 4000) * (0x256F * ((60 - this->headShakeTimer) / 60.0f));
        MM_SkelAnime_Update(&this->skelAnime);
        this->headShakeTimer++;
    }
}
...
void EnRd_SetupStandUp(EnRd* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gGibdoRedeadStandUpAnim, -4.0f);
...
void EnRd_SetupCrouch(EnRd* this) {
    MM_Animation_Change(&this->skelAnime, &gGibdoRedeadStandUpAnim, -1.0f,
                     MM_Animation_GetLastFrame(&gGibdoRedeadStandUpAnim), 0.0f, ANIMMODE_ONCE, -4.0f);
...
void EnRd_Damage(EnRd* this, PlayState* play) {
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        if (this->actor.parent != NULL) {
            EnRd_SetupWalkToParent(this);
        } else if (MM_Actor_WorldDistXYZToPoint(&player->actor, &this->actor.home.pos) >= 150.0f) {
            EnRd_SetupWalkToHome(this, play);
        } else {
            EnRd_SetupWalkToPlayer(this, play);
        }
...
void EnRd_SetupDead(EnRd* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gGibdoRedeadDeathAnim, -1.0f);
...
void EnRd_Dead(EnRd* this, PlayState* play) {
    ...
    MM_Math_SmoothStepToS(&this->headRotY, 0, 1, 0x7D0, 0);
    MM_Math_SmoothStepToS(&this->torsoRotY, 0, 1, 0x7D0, 0);

    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if (this->deathTimer == 0) {
            if (!MM_Flags_GetSwitch(play, EN_RD_GET_SWITCH_FLAG(this))) {
                MM_Flags_SetSwitch(play, EN_RD_GET_SWITCH_FLAG(this));
            }
    ...
    } else if (MM_Animation_OnFrame(&this->skelAnime, 33.0f) || MM_Animation_OnFrame(&this->skelAnime, 40.0f)) {
...
void EnRd_SetupStunned(EnRd* this) {
    ...
    if (gSaveContext.sunsSongState != SUNSSONG_INACTIVE) {
        ...
        MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_GRAY, COLORFILTER_INTENSITY_FLAG | 200,
                             COLORFILTER_BUFFLAG_OPA, 255);
    } else if (this->damageEffect == EN_RD_DMGEFF_STUN) {
        MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_BLUE, 200, COLORFILTER_BUFFLAG_OPA, 40);
...
void EnRd_UpdateDamage(EnRd* this, PlayState* play) {
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if (this->actor.colChkInfo.health == 0) {
            EnRd_UpdateParentForOtherRedeads(play, &this->actor, true);
            EnRd_SetupDead(this);
            MM_Item_DropCollectibleRandom(play, NULL, &this->actor.world.pos, 0x90);
        } else {
            EnRd_SetupDamage(this);
        }
    }
...
void EnRd_UpdateEffect(EnRd* this, PlayState* play) {
    ...
    if (this->drawDmgEffTimer < 20) {
        MM_Math_SmoothStepToF(&this->drawDmgEffScale, 0.0f, 0.5f, 0.03f, 0.0f);
        this->drawDmgEffAlpha = this->drawDmgEffTimer * 0.05f;
    } else {
        MM_Math_SmoothStepToF(&this->drawDmgEffScale, 0.5f, 0.1f, 0.02f, 0.0f);
    }
}
...
void MM_EnRd_Update(Actor* thisx, PlayState* play) {
    ...
    if ((this->damageEffect != EN_RD_DMGEFF_UNUSED_6) &&
        ...
        if ((this->actor.shape.rot.x == 0) && (this->action != EN_RD_ACTION_GRABBING) && (this->actor.speed != 0.0f)) {
            MM_Actor_UpdateBgCheckInfo(play, &this->actor, 30.0f, 20.0f, 35.0f,
...
s32 MM_EnRd_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx,
                          Gfx** gfx) {
    EnRd* this = (EnRd*)thisx;
...
void MM_EnRd_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx, Gfx** gfx) {
    EnRd* this = (EnRd*)thisx;
...
void MM_EnRd_Draw(Actor* thisx, PlayState* play) {
    static Vec3f MM_sScale = { 0.25f, 0.25f, 0.25f };
    s32 pad;
    EnRd* this = (EnRd*)thisx;
    ...
    if (this->alpha == 255) {
        ...
        POLY_OPA_DISP =
            MM_SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                               MM_EnRd_OverrideLimbDraw, MM_EnRd_PostLimbDraw, &this->actor, POLY_OPA_DISP);

        func_800BC620(&pos, &MM_sScale, 255, play);
    } else {
        ...
        POLY_XLU_DISP =
            MM_SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                               MM_EnRd_OverrideLimbDraw, NULL, &this->actor, POLY_XLU_DISP);
        func_800BC620(&pos, &MM_sScale, this->alpha, play);
    }
