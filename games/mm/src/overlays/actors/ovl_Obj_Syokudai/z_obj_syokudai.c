...
void MM_ObjSyokudai_Init(Actor* thisx, PlayState* play);
void MM_ObjSyokudai_Destroy(Actor* thisx, PlayState* play);
void MM_ObjSyokudai_Update(Actor* thisx, PlayState* play2);
void MM_ObjSyokudai_Draw(Actor* thisx, PlayState* play);
...
ActorProfile Obj_Syokudai_Profile = {
    ...
    /**/ sizeof(ObjSyokudai),
    /**/ MM_ObjSyokudai_Init,
    /**/ MM_ObjSyokudai_Destroy,
    /**/ MM_ObjSyokudai_Update,
    /**/ MM_ObjSyokudai_Draw,
};
...
void MM_ObjSyokudai_Init(Actor* thisx, PlayState* play) {
    ...
    MM_ActorShape_Init(&thisx->shape, 0.0f, func_800B4B50, 1.0f);
    ...
    MM_Lights_PointGlowSetInfo(&this->lightInfo, thisx->world.pos.x, thisx->world.pos.y + OBJ_SYOKUDAI_GLOW_HEIGHT,
    ...
    this->flameTexScroll = (u32)(MM_Rand_ZeroOne() * OBJ_SYOKUDAI_SNUFF_DEFAULT);
    MM_Actor_SetFocus(thisx, 60.0f);
}

void MM_ObjSyokudai_Destroy(Actor* thisx, PlayState* play) {
    ObjSyokudai* this = (ObjSyokudai*)thisx;

    MM_Collider_DestroyCylinder(play, &this->standCollider);
    MM_Collider_DestroyCylinder(play, &this->flameCollider);
...
void MM_ObjSyokudai_Update(Actor* thisx, PlayState* play2) {
    ...
    if (this->pendingAction != OBJ_SYOKUDAI_PENDING_ACTION_NONE) {
        ...
        if (WaterBox_GetSurface1_2(play, &play->colCtx, thisx->world.pos.x, thisx->world.pos.z, &waterSurface,
            ...
            if (this->flameCollider.base.acFlags & AC_HIT) {
                ...
                MM_Math_Vec3f_Diff(&player->meleeWeaponInfo[0].tip, &thisx->world.pos, &stickTipSeparationVec);
            ...
            if (interaction != OBJ_SYOKUDAI_INTERACTION_NONE) {
                if (this->snuffTimer != OBJ_SYOKUDAI_SNUFF_OUT) {
                    ...
                    AudioSfx_PlaySfx(NA_SE_EV_FLAME_IGNITION, &thisx->projectedPos, 4, &MM_gSfxDefaultFreqAndVolScale,
                                     &MM_gSfxDefaultFreqAndVolScale, &MM_gSfxDefaultReverb);
                }
            }
        }
    }
    MM_Collider_UpdateCylinder(thisx, &this->standCollider);
    MM_CollisionCheck_SetOC(play, &play->colChkCtx, &this->standCollider.base);
    MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->standCollider.base);
    MM_Collider_UpdateCylinder(thisx, &this->flameCollider);
    MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->flameCollider.base);
    if (this->snuffTimer > OBJ_SYOKUDAI_SNUFF_OUT) {
        this->snuffTimer--;
    ...
    if (this->snuffTimer != OBJ_SYOKUDAI_SNUFF_OUT) {
        ...
        lightIntensity = MM_Rand_ZeroOne() * 127;
        lightIntensity += 128;
        Actor_PlaySfx_Flagged(thisx, NA_SE_EV_TORCH - SFX_FLAG);
    }
    MM_Lights_PointSetColorAndRadius(&this->lightInfo, lightIntensity, lightIntensity * 0.7f, 0, lightRadius);
    this->flameTexScroll++;
}
...
void MM_ObjSyokudai_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_gSPDisplayList(POLY_OPA_DISP++, MM_sDLists[OBJ_SYOKUDAI_GET_TYPE(thisx)]);
    if (this->snuffTimer != OBJ_SYOKUDAI_SNUFF_OUT) {
        ...
        MM_gSPSegment(POLY_XLU_DISP++, 0x08,
                   MM_Gfx_TwoTexScroll(play->state.gfxCtx, 0, 0, 0, 0x20, 0x40, 1, 0,
                                    (this->flameTexScroll * -OBJ_SYOKUDAI_SNUFF_DEFAULT) & 0x1FF, 0x20, 0x80));
        gDPSetPrimColor(POLY_XLU_DISP++, 0x80, 0x80, 255, 255, 0, 255);
        gDPSetEnvColor(POLY_XLU_DISP++, 255, 0, 0, 0);
        MM_Matrix_Translate(0.0f, OBJ_SYOKUDAI_FLAME_HEIGHT, 0.0f, MTXMODE_APPLY);
        Matrix_RotateYS(BINANG_ROT180(MM_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) - thisx->shape.rot.y), MTXMODE_APPLY);
        MM_Matrix_Scale(flameScale, flameScale, flameScale, MTXMODE_APPLY);
