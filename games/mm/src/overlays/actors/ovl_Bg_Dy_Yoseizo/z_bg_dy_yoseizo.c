...
void MM_BgDyYoseizo_Init(Actor* thisx, PlayState* play);
void MM_BgDyYoseizo_Destroy(Actor* thisx, PlayState* play);
void MM_BgDyYoseizo_Update(Actor* thisx, PlayState* play);
void MM_BgDyYoseizo_Draw(Actor* thisx, PlayState* play);
...
ActorProfile Bg_Dy_Yoseizo_Profile = {
    ...
    /**/ sizeof(BgDyYoseizo),
    /**/ MM_BgDyYoseizo_Init,
    /**/ MM_BgDyYoseizo_Destroy,
    /**/ MM_BgDyYoseizo_Update,
    /**/ NULL,
};
...
static AnimationHeader* MM_sAnimations[GREATFAIRY_ANIM_MAX] = {
...
void MM_BgDyYoseizo_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &gGreatFairySkel, &gGreatFairyShowingItemAnim, this->jointTable,
...
void BgDyYoseizo_UpdateEyes(BgDyYoseizo* this) {
    ...
    if (this->blinkTimer == 0) {
        ...
        if (this->eyeIndex >= 3) {
            this->eyeIndex = 0;
            this->blinkTimer = (s32)MM_Rand_ZeroFloat(60.0f) + 20;
        }
    }
}

void MM_BgDyYoseizo_Bob(BgDyYoseizo* this, PlayState* play) {
    this->actor.shape.yOffset = MM_Math_SinS(play->gameplayFrames * 1000) * 15.0f;
}
...
void BgDyYoseizo_SpawnEffects(BgDyYoseizo* this, s16 trajectoryType, s32 count) {
    ...
    accel.x = MM_Rand_ZeroOne() - 0.5f;
    accel.y = MM_Rand_ZeroOne() - 0.5f;
    accel.z = MM_Rand_ZeroOne() - 0.5f;
        ...
        switch (trajectoryType) {
            case GREAT_FAIRY_EFFECT_TRAJECTORY_FAST_RADIANT:
                ...
                pos.y = this->actor.world.pos.y + spawnHeightVariation +
                        (MM_Rand_ZeroOne() - 0.5f) * (spawnHeightVariation * 0.5f);
            ...
            default: // all become convergent type
                ...
                pos.y = this->actor.world.pos.y + spawnHeightVariation + 50.0f +
                        (MM_Rand_ZeroOne() - 0.5f) * (spawnHeightVariation * 0.1f);
                pos.z = this->actor.world.pos.z + 30.0f;
                break;
        }
...
void func_80A0AD50(BgDyYoseizo* this) {
    ...
    MM_Math_ApproachF(&this->actor.world.pos.y, heightTarget, this->unk2F0, 100.0f);
    MM_Math_ApproachF(&scale, 0.035f, this->unk2F4, 0.005f);
    MM_Math_ApproachF(&this->unk2F0, 0.8f, 0.1f, 0.02f);
    MM_Math_ApproachF(&this->unk2F4, 0.2f, 0.03f, 0.05f);
...
void func_80A0AE1C(BgDyYoseizo* this, PlayState* play) {
    ...
    if (scale < 0.003f) {
        ...
        MM_Math_ApproachF(&this->actor.world.pos.y, heightTarget, this->unk2F0, 100.0f);
        MM_Math_ApproachZeroF(&scale, this->unk2F4, 0.005f);
        MM_Math_ApproachF(&this->unk2F0, 0.8f, 0.1f, 0.02f);
        MM_Math_ApproachF(&this->unk2F4, 0.2f, 0.03f, 0.05f);
...
void func_80A0AFDC(BgDyYoseizo* this) {
    MM_Animation_Change(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_SPIN_LAY_DOWN], 0.0f, 46.0f,
                     MM_Animation_GetLastFrame(MM_sAnimations[GREATFAIRY_ANIM_SPIN_LAY_DOWN]), ANIMMODE_ONCE, 0.0f);
...
void func_80A0B078(BgDyYoseizo* this, PlayState* play) {
    ...
    if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_103) &&
        ...
        MM_Animation_Change(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_LAYING_DOWN], 1.0f, 0.0f,
                         MM_Animation_GetLastFrame(MM_sAnimations[GREATFAIRY_ANIM_LAYING_DOWN]), ANIMMODE_LOOP, 0.0f);
...
void func_80A0B184(BgDyYoseizo* this, PlayState* play) {
    ...
    if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_103) &&
        ...
        MM_Animation_Change(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_SHOWING_ITEM], 1.0f, 0.0f,
                         MM_Animation_GetLastFrame(MM_sAnimations[GREATFAIRY_ANIM_SHOWING_ITEM]), ANIMMODE_LOOP, 0.0f);
...
void func_80A0B290(BgDyYoseizo* this, PlayState* play) {
    ...
    if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_103) &&
        (play->csCtx.actorCues[Cutscene_GetCueChannel(play, CS_CMD_ACTOR_CUE_103)]->id == 7)) {
        MM_Animation_Change(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_LAYING_DOWN], 1.0f, 0.0f,
                         MM_Animation_GetLastFrame(MM_sAnimations[GREATFAIRY_ANIM_LAYING_DOWN]), ANIMMODE_LOOP, -10.0f);
        this->actionFunc = func_80A0B184;
        this->mouthIndex = 0;
    }
}
...
void func_80A0B35C(BgDyYoseizo* this, PlayState* play) {
    ...
    if (this->timer == 60) {
        if (GameInteractor_Should(VB_GIVE_ITEM_FROM_GREAT_FAIRY,
                                  !MM_Flags_GetSwitch(play, GREAT_FAIRY_GET_SWITCHFLAG(&this->actor)), this)) {
...
void func_80A0B500(BgDyYoseizo* this, PlayState* play) {
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        MM_Animation_Change(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_GIVING_UPGRADE], 1.0f, 0.0f,
                         MM_Animation_GetLastFrame(MM_sAnimations[GREATFAIRY_ANIM_GIVING_UPGRADE]), ANIMMODE_LOOP, 0.0f);
        this->actionFunc = func_80A0B35C;
        pos.x = player->actor.world.pos.x;
        pos.y = player->actor.world.pos.y + 200.0f;
        pos.z = player->actor.world.pos.z;
        this->beam = (EnDyExtra*)MM_Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, ACTOR_EN_DY_EXTRA, pos.x,
...
void func_80A0B5F0(BgDyYoseizo* this, PlayState* play) {
    ...
    if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_103) &&
        ...
        MM_Animation_Change(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_START_GIVING_UPGRADE], 1.0f, 0.0f,
                         MM_Animation_GetLastFrame(MM_sAnimations[GREATFAIRY_ANIM_START_GIVING_UPGRADE]), ANIMMODE_ONCE,
                         -5.0f);
...
void func_80A0B75C(BgDyYoseizo* this, PlayState* play) {
    ...
    if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_103) &&
        ...
        MM_Animation_Change(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_LAY_DOWN_TRANSITION], 1.0f, 2.0f,
                         MM_Animation_GetLastFrame(MM_sAnimations[GREATFAIRY_ANIM_LAY_DOWN_TRANSITION]), ANIMMODE_ONCE, 0.0f);
        Actor_PlaySfx(&this->actor, NA_SE_VO_FR_SMILE_0);
        this->unk2F8 = 0;
    }
}
...
void func_80A0B834(BgDyYoseizo* this) {
    this->actor.draw = MM_BgDyYoseizo_Draw;
    MM_Animation_Change(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_SPIN_LAY_DOWN], 1.0f, 0.0f,
                     MM_Animation_GetLastFrame(MM_sAnimations[GREATFAIRY_ANIM_SPIN_LAY_DOWN]), ANIMMODE_ONCE, 0.0f);
    Actor_PlaySfx(&this->actor, NA_SE_VO_FR_LAUGH_0);
...
void BgDyYoseizo_TrainPlayer(BgDyYoseizo* this, PlayState* play) {
    ...
    if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_103)) {
        ...
        if (this->unk302 & 1) {
            ...
            if (play->msgCtx.currentTextId != 0x59A) {
                ...
                MM_Message_StartTextbox(play, 0x59A, &this->actor);
            }
        }
    }
    ...
    if (csId != this->csId) {
        switch (csId) {
            case 9:
                MM_Animation_PlayLoop(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_ARMS_FOLDED]);
                break;

            case 10:
                MM_Animation_PlayLoop(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_CLAPPING]);
                break;

            case 11:
                MM_Animation_PlayOnce(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_TEACH_SPIN_ATTACK]);
                break;
...
void func_80A0BB08(BgDyYoseizo* this, PlayState* play) {
    ...
    if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_103) &&
        (play->csCtx.actorCues[Cutscene_GetCueChannel(play, CS_CMD_ACTOR_CUE_103)]->id == 7)) {
        this->actor.draw = MM_BgDyYoseizo_Draw;
        MM_Animation_PlayLoop(&this->skelAnime, MM_sAnimations[GREATFAIRY_ANIM_LAYING_DOWN]);
        this->actionFunc = func_80A0B184;
        this->mouthIndex = 0;
        this->actor.world.pos.y = this->actor.home.pos.y + 40.0f;
        MM_Actor_SetScale(&this->actor, 0.035f);
        this->unk2F8 = 0;
    }
...
s32 MM_BgDyYoseizo_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    BgDyYoseizo* this = (BgDyYoseizo*)thisx;
...
void MM_BgDyYoseizo_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_DrawFlexOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                          MM_BgDyYoseizo_OverrideLimbDraw, NULL, &this->actor);
...
void BgDyYoseizo_SpawnEffect(BgDyYoseizo* this, Vec3f* initPos, Vec3f* initVelocity, Vec3f* accel,
    ...
    for (i = 0; i < BG_DY_YOSEIZO_EFFECT_COUNT; i++, effect++) {
        if (!effect->alive) {
            ...
            effect->type = type;
            effect->pitch = 0;
            effect->yaw = TRUNCF_BINANG(MM_Rand_CenteredFloat(30000.0f));
            effect->roll = 0;
            return;
        }
    }
    ...
    for (i = 0; i < BG_DY_YOSEIZO_EFFECT_COUNT; i++, effect++) {
        if (effect->alive) {
            ...
            if (effect->type < GREAT_FAIRY_EFFECT_TRAJECTORY_CONVERGE_ON_PLAYER) {
                ...
                targetPitch = MM_Math_Vec3f_Pitch(&effect->pos, &sp94);
                targetYaw = MM_Math_Vec3f_Yaw(&effect->pos, &sp94);

                floatAngle = effect->pitch;
                MM_Math_ApproachF(&floatAngle, targetPitch, 0.9f, 5000.0f);
                effect->pitch = floatAngle;

                floatAngle = effect->yaw;
                MM_Math_ApproachF(&floatAngle, targetYaw, 0.9f, 5000.0f);
...
void BgDyYoseizo_DrawEffects(BgDyYoseizo* this, PlayState* play) {
    ...
    for (i = 0; i < BG_DY_YOSEIZO_EFFECT_COUNT; i++, effect++) {
        if (effect->alive == true) {
            ...
            MM_Matrix_Scale(effect->scale, effect->scale * stretchFactor, 1.0f, MTXMODE_APPLY);
