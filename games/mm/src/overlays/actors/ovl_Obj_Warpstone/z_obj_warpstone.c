...
s32 ObjWarpstone_PlayOpeningCutscene(ObjWarpstone* this, PlayState* play) {
    if (this->openingCSTimer++ >= OBJ_WARPSTONE_TIMER_ACTIVATE_THRESHOLD) {
        ...
        MM_Math_StepToF(&this->dyna.actor.velocity.x, 0.01f, 0.001f);
        MM_Math_StepToS(&this->dyna.actor.home.rot.x, 0xFF, 0x12);
    } else {
        MM_Math_StepToF(&this->dyna.actor.velocity.x, 20.0f, 0.01f);
        if (this->dyna.actor.velocity.x > 0.2f) {
            this->modelIndex = SEK_MODEL_OPENED;
            MM_Math_StepToS(&this->dyna.actor.home.rot.x, 0, 0x14);
        }
    }
    return true;
}
...
void ObjWarpstone_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->isTalking) {
        ...
        } else if ((MM_Message_GetState(&play->msgCtx) == TEXT_STATE_CHOICE) && MM_Message_ShouldAdvance(play)) {
...
void ObjWarpstone_Draw(Actor* thisx, PlayState* play2) {
    ...
    MM_Gfx_DrawDListOpa(play, sOwlStatueDLs[this->modelIndex]);
    if (this->dyna.actor.home.rot.x != 0) {
        OPEN_DISPS(play->state.gfxCtx);

        Gfx_SetupDL25_Xlu(play->state.gfxCtx);
        MM_Matrix_Translate(this->dyna.actor.world.pos.x, this->dyna.actor.world.pos.y + 34.0f,
                         this->dyna.actor.world.pos.z, MTXMODE_NEW);
        MM_Matrix_Mult(&play->billboardMtxF, MTXMODE_APPLY);
        MM_Matrix_Translate(0.0f, 0.0f, 30.0f, MTXMODE_APPLY);
        MM_Matrix_Scale(this->dyna.actor.velocity.x, this->dyna.actor.velocity.x, this->dyna.actor.velocity.x,
                     MTXMODE_APPLY);
        MM_Matrix_Push();
