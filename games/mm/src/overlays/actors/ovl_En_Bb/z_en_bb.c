...
void MM_EnBb_Init(Actor* thisx, PlayState* play);
void MM_EnBb_Destroy(Actor* thisx, PlayState* play);
void MM_EnBb_Update(Actor* thisx, PlayState* play);
void MM_EnBb_Draw(Actor* thisx, PlayState* play);
...
void MM_EnBb_Down(EnBb* this, PlayState* play);
void EnBb_Dead(EnBb* this, PlayState* play);
void MM_EnBb_Damage(EnBb* this, PlayState* play);
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 2, 20, 40, 50 };
...
void MM_EnBb_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_Init(play, &this->skelAnime, &gBubbleSkel, &gBubbleFlyingAnim, this->jointTable, this->morphTable,
                   BUBBLE_LIMB_MAX);
...
void EnBb_UpdateStateForFlying(EnBb* this) {
    ...
    if (this->actor.floorHeight > BGCHECK_Y_MIN) {
        MM_Math_StepToF(&this->actor.world.pos.y, this->actor.floorHeight + this->flyHeightMod, 0.5f);
    }

    this->actor.world.pos.y += MM_Math_CosS(this->bobPhase);
    this->bobPhase += 0x826;
    MM_Math_StepToF(&this->flameScaleY, 0.8f, 0.1f);
    MM_Math_StepToF(&this->flameScaleX, 1.0f, 0.1f);
    EnBb_CheckForWall(this);
    MM_Math_StepToF(&this->actor.speed, this->maxSpeed, 0.5f);
    MM_Math_ApproachS(&this->actor.shape.rot.y, this->targetYRotation, 5, 0x3E8);
    this->actor.world.rot.y = this->actor.shape.rot.y;
}

void EnBb_SetupFlyIdle(EnBb* this) {
    if (this->actionFunc != EnBb_FlyIdle) {
        MM_Animation_PlayLoop(&this->skelAnime, &gBubbleFlyingAnim);
    }
    ...
    this->timer = (s32)MM_Rand_ZeroFloat(20.0f) + 40;
    this->actor.gravity = 0.0f;
    this->actor.velocity.y = 0.0f;
    this->flyHeightMod = (MM_Math_CosS(this->bobPhase) * 10.0f) + 30.0f;
    this->targetYRotation = MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos);

    if ((this->actor.xzDistToPlayer < (this->attackRange + 120.0f)) ||
        (MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) < 300.0f)) {
        this->targetYRotation += (s16)((s32)MM_Rand_Next() >> 0x11);
    }

    this->collider.base.atFlags |= AT_ON;
    this->maxSpeed = MM_Rand_ZeroFloat(1.5f) + 1.0f;
...
void EnBb_FlyIdle(EnBb* this, PlayState* play) {
    ...
    } else if (MM_Animation_OnFrame(&this->skelAnime, 0.0f) && (MM_Rand_ZeroOne() < 0.1f)) {
...
void EnBb_SetupAttack(EnBb* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &gBubbleAttackAnim);
    this->timer = (s32)MM_Rand_ZeroFloat(20.0f) + 60;
    this->flyHeightMod = (MM_Math_CosS(this->bobPhase) * 10.0f) + 30.0f;
    this->targetYRotation = this->actor.yawTowardsPlayer;
    this->maxSpeed = MM_Rand_ZeroFloat(1.5f) + 4.0f;
...
void EnBb_Attack(EnBb* this, PlayState* play) {
    ...
    } else if (MM_Animation_OnFrame(&this->skelAnime, 2.0f) || MM_Animation_OnFrame(&this->skelAnime, 7.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_BUBLE_WING);
    } else if (MM_Animation_OnFrame(&this->skelAnime, 0.0f) && (MM_Rand_ZeroOne() < 0.1f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_BUBLE_LAUGH);
    }
    ...
    if (((this->attackRange + 120.0f) < this->actor.xzDistToPlayer) || (this->timer == 0) ||
        (MM_Player_GetMask(play) == PLAYER_MASK_STONE) ||
        (MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) > 400.0f)) {
        EnBb_SetupFlyIdle(this);
    }
}

void MM_EnBb_SetupDown(EnBb* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &gBubbleFlyingAnim);
    this->collider.base.atFlags |= AT_ON;
    this->timer = 140;
...
void MM_EnBb_Down(EnBb* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        ...
        MM_Actor_SpawnFloorDustRing(play, &this->actor, &this->actor.world.pos, 7.0f, 2, 2.0f, 0, 0, false);
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, BINANG_ADD(this->actor.yawTowardsPlayer, 0x8000), 0xBB8);
    }
...
void EnBb_SetupDead(EnBb* this, PlayState* play) {
    ...
    MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 40, NA_SE_EN_BUBLE_DEAD);
    ...
    for (i = 0; i < BUBBLE_BODYPART_MAX; i++, bodyPartVelocity++) {
        MM_Math_Vec3f_Diff(&this->bodyPartsPos[i], &this->actor.world.pos, &posDiff);
        ...
        bodyPartVelocity->y = MM_Rand_ZeroFloat(3.5f) + 10.0f;
    }
...
void EnBb_Dead(EnBb* this, PlayState* play) {
    ...
    if (this->timer == 0) {
        ...
        for (i = 0; i < BUBBLE_BODYPART_MAX; i++) {
            MM_Math_Vec3f_Sum(&this->bodyPartsPos[i], &this->bodyPartsVelocity[i], &this->bodyPartsPos[i]);
            this->bodyPartsVelocity[i].y += this->actor.gravity;
        }
    }
}

void MM_EnBb_SetupDamage(EnBb* this) {
    this->collider.base.acFlags &= ~AC_ON;
    ...
    if (this->actor.colChkInfo.damageEffect == EN_BB_DMGEFF_ZORA_MAGIC) {
        ...
        MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_BLUE, 255, COLORFILTER_BUFFLAG_OPA, 20);
        this->actor.speed = 0.0f;
    ...
    this->actionFunc = MM_EnBb_Damage;
}
...
void EnBb_UpdateDamage(EnBb* this, PlayState* play) {
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        this->collider.base.atFlags &= ~(AT_HIT | AT_BOUNCED);
        if (this->actionFunc != MM_EnBb_Down) {
...
void MM_EnBb_Update(Actor* thisx, PlayState* play) {
    ...
    if ((this->actionFunc != EnBb_Dead) && (this->actionFunc != EnBb_WaitForRevive)) {
        ...
        MM_Actor_UpdateBgCheckInfo(play, &this->actor, 30.0f, 25.0f, 40.0f,
        ...
        MM_Math_Vec3s_ToVec3f(&this->actor.focus.pos, &this->collider.dim.worldSphere.center);
...
void MM_EnBb_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    s32 pad;
    EnBb* this = (EnBb*)thisx;
    ...
    if (this->bodyPartDrawStatus == BB_BODY_PART_DRAW_STATUS_ALIVE) {
        if (sLimbToBodyParts[limbIndex] != BODYPART_NONE) {
            if (sLimbToBodyParts[limbIndex] == BUBBLE_BODYPART_JAW) {
                ...
                MM_Matrix_MultVec3f(&sEffectsBodyPartOffset, &this->bodyPartsPos[BUBBLE_BODYPART_EFFECTS]);
            } else {
...
void MM_EnBb_Draw(Actor* thisx, PlayState* play) {
    ...
    POLY_OPA_DISP = &gfx[1];
    MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, EnBb_OverrideLimbDraw,
                      MM_EnBb_PostLimbDraw, &this->actor);
    ...
    if (this->flameScaleX > 0.0f) {
        ...
        Matrix_RotateYS(((MM_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) - this->actor.shape.rot.y) + 0x8000),
                        MTXMODE_APPLY);
        MM_Matrix_Scale(this->flameScaleX, this->flameScaleY, 1.0f, MTXMODE_APPLY);
        gDPSetPrimColor(POLY_XLU_DISP++, 0x80, 0x80, 255, 255, 255, 255);
        gDPSetEnvColor(POLY_XLU_DISP++, 0, 0, 255, 0);
        gSPSegment(
            POLY_XLU_DISP++, 0x08,
            MM_Gfx_TwoTexScroll(play->state.gfxCtx, 0, 0, 0, 32, 64, 1, 0, (play->gameplayFrames * -20) & 0x1FF, 32, 128));
