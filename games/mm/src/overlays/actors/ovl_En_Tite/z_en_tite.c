...
void MM_EnTite_Init(Actor* thisx, PlayState* play);
void MM_EnTite_Destroy(Actor* thisx, PlayState* play);
void MM_EnTite_Update(Actor* thisx, PlayState* play);
void MM_EnTite_Draw(Actor* thisx, PlayState* play);
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 2, 40, 40, MASS_HEAVY };
...
void MM_EnTite_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_Init(play, &this->skelAnime, &object_tite_Skel_003A20, &object_tite_Anim_0012E4, this->jointTable,
...
s32 func_80893B10(EnTite* this) {
    MM_Math_StepToF(&this->actor.velocity.y, 0.0f, 2.0f);
    return MM_Math_StepToF(&this->actor.world.pos.y, (this->actor.world.pos.y + this->actor.depthInWater) - 1.0f, 2.0f);
}
...
void func_80893BCC(EnTite* this, PlayState* play) {
    static Vec3f MM_sAccel = { 0.0f, 0.3f, 0.0f };
    Vec3f pos;
    s32 i;
    s32 j;
        ...
        if ((surfaceMaterial == SURFACE_MATERIAL_DIRT) || (surfaceMaterial == SURFACE_MATERIAL_SAND)) {
            ...
            for (i = ENTITE_BODYPART_5; i < ENTITE_BODYPART_MAX; i++) {
                for (j = 0; j < 2; j++) {
                    ...
                    pos.x = bodyPartPos->x + MM_Rand_CenteredFloat(1.0f);
                    pos.y = bodyPartPos->y + MM_Rand_CenteredFloat(1.0f);
                    pos.z = bodyPartPos->z + MM_Rand_CenteredFloat(1.0f);
                    func_800B0DE0(play, &pos, &gZeroVec3f, &MM_sAccel, &D_80896B3C, &D_80896B40,
                                  (s32)MM_Rand_ZeroFloat(16.0f) + 80, 15);
                }
            }
        }
...
void func_80893ED4(EnTite* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &object_tite_Anim_0012E4, 4.0f);
    this->unk_2BC = MM_Rand_S16Offset(15, 30);
...
void func_80893F30(EnTite* this, PlayState* play) {
    ...
    if (this->unk_2BC > 0) {
        this->unk_2BC--;
    } else if ((MM_Player_GetMask(play) != PLAYER_MASK_STONE) && (this->actor.xzDistToPlayer < 300.0f) &&
...
void func_80893FD0(EnTite* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &object_tite_Anim_00083C);
...
void func_80894024(EnTite* this, PlayState* play) {
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        func_80893B70(this);
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0x3E8);
        this->actor.world.rot.y = this->actor.shape.rot.y;
    }
}

void func_8089408C(EnTite* this, PlayState* play) {
    MM_Animation_PlayOnce(&this->skelAnime, &object_tite_Anim_0004F8);
    ...
    if (this->actor.shape.yOffset < 0.0f) {
        ...
        for (i = 0; i < 4; i++) {
            sp64.x = 2.0f * MM_Math_SinS(this->actor.shape.rot.y);
            sp64.z = 2.0f * MM_Math_CosS(this->actor.shape.rot.y);
...
void func_80894414(EnTite* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &object_tite_Anim_00069C);
    this->actionFunc = func_80894454;
}

void func_80894454(EnTite* this, PlayState* play) {
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if ((MM_Player_GetMask(play) == PLAYER_MASK_STONE) || (this->actor.xzDistToPlayer > 450.0f) ||
            (this->actor.playerHeightRel > 80.0f)) {
            func_80893ED4(this);
        } else if (!MM_Actor_IsFacingPlayer(&this->actor, 0x2328)) {
            func_808945EC(this);
        } else {
            func_80893A9C(this, play);
        }
    } else {
        MM_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 0x5DC);
        func_80893B70(this);
    }
}
...
void func_8089452C(EnTite* this, PlayState* play) {
    ...
    MM_EffectSsGRipple_Spawn(play, &sp2C, 0, 500, 0);
...
void func_808945EC(EnTite* this) {
    MM_Animation_PlayLoop(&this->skelAnime, &object_tite_Anim_000A14);
    this->actor.speed = 0.0f;
    this->actor.velocity.y = 0.0f;
...
void func_80894638(EnTite* this, PlayState* play) {
    ...
    if ((MM_Player_GetMask(play) == PLAYER_MASK_STONE) || (this->actor.xzDistToPlayer > 450.0f) ||
        ...
        func_80893ED4(this);
               ...
               MM_Actor_IsFacingPlayer(&this->actor, 0xE38)) {
...
void func_8089484C(EnTite* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &object_tite_Anim_000C70, -3.0f);
    if (this->actionFunc != func_80894910) {
        this->unk_2B8 = MM_Rand_S16Offset(1, 3);
    }
...
void func_80894910(EnTite* this, PlayState* play) {
    ...
    MM_Math_SmoothStepToF(&this->actor.speed, 0.0f, 0.1f, 1.0f, 0.0f);
    MM_SkelAnime_Update(&this->skelAnime);
    if (this->actor.bgCheckFlags & BGCHECKFLAG_WATER_TOUCH) {
        ...
        if (func_80893ADC(this)) {
            ...
            this->actor.velocity.y *= 0.75f;
            MM_EffectSsGRipple_Spawn(play, &sp34, 0, 500, 0);
        }
    } else {
        func_80893BCC(this, play);
    }
    ...
    if (((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) ||
        ...
        (this->actor.velocity.y <= 0.0f)) {
        this->actor.speed = 0.0f;
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 0xFA0);
        this->actor.world.rot.y = this->actor.shape.rot.y;
...
void func_80894BC8(EnTite* this, PlayState* play) {
    MM_Math_SmoothStepToF(&this->actor.speed, 0.0f, 1.0f, 0.5f, 0.0f);
    MM_SkelAnime_Update(&this->skelAnime);
    ...
    if ((this->actor.speed == 0.0f) && ((this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) || func_80893ADC(this))) {
        ...
        if ((MM_Player_GetMask(play) == PLAYER_MASK_STONE) ||
            ...
            func_80893ED4(this);
                   ...
                   MM_Actor_IsFacingPlayer(&this->actor, 0x1770)) {
            func_80893A9C(this, play);
        } else {
            func_8089484C(this);
        }
...
void func_80895020(EnTite* this, PlayState* play) {
    ...
    MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 40, NA_SE_EN_TEKU_DEAD);
    this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
    this->actor.flags |= ACTOR_FLAG_UPDATE_CULLING_DISABLED;
    this->unk_2BA = 1;
    MM_Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, this->unk_2BE);
    this->unk_2BC = 25;
    ...
    for (i = 0; i < ENTITE_BODYPART_MAX; i++, ptr++) {
        MM_Math_Vec3f_Diff(&this->bodyPartsPos[i], &this->actor.world.pos, &sp74);
        temp_f0 = MM_Math3D_Vec3fMagnitude(&sp74);
        if (temp_f0 > 1.0f) {
        ...
        ptr->y = MM_Rand_ZeroFloat(3.5f) + 5.5f;
    }
...
void func_808951B8(EnTite* this, PlayState* play) {
    ...
    this->unk_2BC--;
    MM_Math_SmoothStepToS(&this->actor.world.rot.z, 0x4000, 4, 0x1000, 0x400);

    if (this->unk_2BC == 0) {
        for (i = 0; i < ENTITE_BODYPART_MAX; i++) {
            func_800B3030(play, &this->bodyPartsPos[i], &gZeroVec3f, &gZeroVec3f, 40, 7, 1);
            MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->bodyPartsPos[i], 11, NA_SE_EN_EXTINCT);
        }
    ...
    for (i = 0; i < ENTITE_BODYPART_MAX; i++) {
        MM_Math_Vec3f_Sum(&this->bodyPartsPos[i], &this->unk_33C[i], &this->bodyPartsPos[i]);
        this->unk_33C[i].y += this->actor.gravity;
    }
}

void func_808952EC(EnTite* this) {
    MM_Animation_PlayLoopSetSpeed(&this->skelAnime, &object_tite_Anim_000A14, 1.5f);
    ...
    this->actor.speed = 0.0f;
    this->actor.gravity = -1.0f;
    this->unk_2B8 = MM_Rand_ZeroOne() * 50.0f;
...
void func_80895424(EnTite* this, PlayState* play) {
    MM_Math_ScaledStepToS(&this->actor.shape.rot.z, -0x8000, 0xFA0);
    if (this->unk_2B8 != 0) {
        this->unk_2B8--;
    } else {
        this->unk_2B8 = MM_Rand_ZeroOne() * 30.0f;
        this->skelAnime.curFrame = MM_Rand_ZeroOne() * 5.0f;
    }
    ...
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        this->collider.base.acFlags |= AC_ON;
        if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND_TOUCH) {
            MM_Actor_SpawnFloorDustRing(play, &this->actor, &this->actor.world.pos, 20.0f, 11, 4.0f, 0, 0, false);
            Actor_PlaySfx(&this->actor, NA_SE_EN_EYEGOLE_ATTACK);
        }
...
void func_80895640(EnTite* this, PlayState* play) {
    MM_Math_ScaledStepToS(&this->actor.shape.rot.z, 0, 0xFA0);
    MM_SkelAnime_Update(&this->skelAnime);
    if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
        this->collider.base.acFlags |= AC_ON;
        this->actor.shape.yOffset = 0.0f;
...
void func_80895738(EnTite* this, PlayState* play) {
    ...
    if (this->unk_2BC == -1) {
        MM_Math_StepToF(&this->actor.speed, 0.0f, 0.3f);
        if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
            this->actor.world.rot.y = this->actor.shape.rot.y;
            func_80893ED4(this);
            this->unk_2BC = 100;
        ...
        this->unk_2BC--;
        MM_Math_StepToF(&this->actor.speed, 10.0f, 0.3f);
        this->actor.flags |= ACTOR_FLAG_SFX_FOR_PLAYER_BODY_HIT;
        MM_CollisionCheck_SetAT(play, &play->colChkCtx, &this->collider.base);
        if (!func_80893A34(this, play)) {
            this->unk_2BC = 0;
        }
    } else if ((this->unk_2BC == 0) && MM_Math_StepToF(&this->actor.speed, 0.0f, 0.3f)) {
        this->actor.world.rot.y = this->actor.shape.rot.y;
        func_80893A18(this);
        if ((MM_Player_GetMask(play) == PLAYER_MASK_STONE) || (this->actor.xzDistToPlayer > 450.0f) ||
            (this->actor.playerHeightRel > 80.0f)) {
...
void func_8089595C(EnTite* this, PlayState* play) {
    ...
    sp2C.x = MM_Rand_CenteredFloat(20.0f) + this->actor.world.pos.x;
    sp2C.y = this->actor.world.pos.y + 15.0f;
    sp2C.z = MM_Rand_CenteredFloat(20.0f) + this->actor.world.pos.z;
...
void func_80895A10(EnTite* this) {
    ...
    MM_Animation_Change(&this->skelAnime, &object_tite_Anim_000A14, 2.0f, 0.0f, 0.0f, ANIMMODE_LOOP, 4.0f);
    this->actor.speed = 0.0f;
    rand = MM_Rand_S16Offset(20, 20);
    this->unk_2BC = ((MM_Rand_ZeroOne() < 0.5f) ? -1 : 1) * rand;
...
void func_80895AC0(EnTite* this, PlayState* play) {
    ...
    if ((this->unk_2BC == 1) || (this->unk_2BC == -1)) {
        if (this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
            ...
            temp_v1 = this->actor.shape.rot.y - MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos);
            temp_v0 = ABS_ALT(temp_v1);
            if (temp_v0 > 0x2000) {
                this->unk_2BC = (temp_v0 >> 9) * this->unk_2BC;
            }
    ...
    if ((this->actor.xzDistToPlayer < 240.0f) && (MM_Player_GetMask(play) != PLAYER_MASK_STONE)) {
        func_8089408C(this, play);
    } else if (this->unk_2BC > 0) {
        this->unk_2BC--;
...
void func_80895CB0(EnTite* this) {
    ...
    this->actor.world.rot.y = this->actor.shape.rot.y;
    this->unk_2BC = MM_Rand_S16Offset(20, 20);
...
void func_80895E28(EnTite* this, PlayState* play) {
    ...
    sp44.x = this->actor.world.pos.x + MM_Math_SinS(this->actor.shape.rot.y) * 25.0f;
    sp44.y = this->actor.world.pos.y + 15.0f;
    sp44.z = this->actor.world.pos.z + MM_Math_CosS(this->actor.shape.rot.y) * 25.0f;

    sp36 = BINANG_SUB(this->actor.shape.rot.y, 0x4000);

    sp38.x = 2.0f * MM_Math_SinS(sp36);
    sp38.y = 3.0f;
    sp38.z = 2.0f * MM_Math_CosS(sp36);
    ...
    if (MM_Math_StepToF(&this->actor.shape.yOffset, 0.0f, 200.0f)) {
        this->actor.flags |= ACTOR_FLAG_ATTENTION_ENABLED;
        this->actor.world.rot.y = this->actor.shape.rot.y;
        this->collider.base.acFlags |= AC_ON;
        func_808945EC(this);
...
void MM_EnTite_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != func_808951B8) {
        ...
        MM_Actor_UpdateBgCheckInfo(play, &this->actor, 25.0f, 40.0f, 20.0f, this->updBgCheckInfoFlags);
        ...
        if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND) {
            ...
            MM_Math_ScaledStepToS(&this->actor.shape.rot.x, 0, 0x3E8);
            if (this->unk_2B9 == 0) {
                MM_Math_ScaledStepToS(&this->actor.shape.rot.z, 0, 0x3E8);
                if (this->actor.shape.yOffset > 0.0f) {
                    this->actor.shape.yOffset -= 400.0f;
                }
            }
        ...
        MM_Actor_SetFocus(&this->actor, this->actor.scale.y * 2000.0f);
...
void MM_EnTite_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    EnTite* this = (EnTite*)thisx;
    MtxF* matrix;
    s8 bodyPart1Index;
...
void MM_EnTite_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, EnTite_OverrideLimbDraw,
                      MM_EnTite_PostLimbDraw, &this->actor);
