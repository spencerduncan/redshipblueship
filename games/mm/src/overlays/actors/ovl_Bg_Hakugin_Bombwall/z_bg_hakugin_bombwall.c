...
void func_80ABBFC0(BgHakuginBombwall* this, PlayState* play) {
    ...
    for (i = 0; i < 6; i++) {
        ...
        for (j = 0; j < ARRAY_COUNT(D_80ABD020); j++) {
            spD8.x = D_80ABD020[j] + (s32)(MM_Rand_Next() >> 0x1C);
            spD8.y = ((MM_Rand_ZeroOne() - 0.5f) * 15.0f) + temp;
            spD8.z = (MM_Rand_ZeroOne() * 20.0f) - 10.0f;

            spCC.x = ((MM_Rand_ZeroOne() - 0.5f) * 7.0f) + (spD8.x * (7.0f / 90.0f));
            spCC.y = (MM_Rand_ZeroOne() * 7.0f) - 2.0f;
            spCC.z = spD8.z * 0.3f;

            MM_Matrix_MultVec3f(&spD8, &spF0);
            MM_Matrix_MultVec3f(&spCC, &spE4);
            ...
            if ((s32)MM_Rand_Next() > 0) {
                ...
                func_800B0E48(play, &spF0, &gZeroVec3f, &D_80ABCFB4, &D_80ABCFAC, &D_80ABCFB0,
                              (MM_Rand_Next() >> 0x1B) + 70, (MM_Rand_Next() >> 0x1A) + 60);
            } else {
                phi_s1 = 0;
            }

            MM_EffectSsKakera_Spawn(play, &spF0, &spE4, &spF0, -550, phi_s0, 30, 0, 0, (s32)(MM_Rand_ZeroOne() * 22.0f) + 5,
...
void func_80ABC2E0(BgHakuginBombwall* this, PlayState* play) {
    ...
    for (i = 0, phi_s2 = 0; i < 25; i++, phi_s2 += 0x4E20) {
        temp_f20 = (25 - i) * 2.4f;
        temp_f22 = MM_Math_SinS(phi_s2) * temp_f20;
        temp_f24 = MM_Math_CosS(phi_s2) * temp_f20;

        spC8.x = this->dyna.actor.world.pos.x + temp_f22;
        spC8.y = this->dyna.actor.world.pos.y;
        spC8.z = this->dyna.actor.world.pos.z + temp_f24;

        spBC.x = ((MM_Rand_ZeroOne() - 0.5f) * 10.0f) + (temp_f22 * 0.13333334f);
        spBC.y = (MM_Rand_ZeroOne() * 17.0f) + 7.0f;
        spBC.z = ((MM_Rand_ZeroOne() - 0.5f) * 10.0f) + (temp_f24 * 0.13333334f);

        temp_s1 = (MM_Rand_Next() & 3) + (i >> 2) + 4;
        ...
        MM_EffectSsKakera_Spawn(play, &spC8, &spBC, &spC8, phi_t0, phi_v0, 30, 0, 0, temp_s1, phi_v1, 0, 50, -1,
                             OBJECT_HAKUGIN_OBJ, object_hakugin_obj_DL_009830);

        if ((i & 1) == 0) {
            func_800B0E48(play, &spC8, &D_80ABD034, &D_80ABCFB4, &D_80ABCFAC, &D_80ABCFB0, (MM_Rand_Next() >> 0x1B) + 60,
                          (MM_Rand_Next() >> 0x1A) + 50);
        }
    }
}
...
void func_80ABC58C(BgHakuginBombwall* this, PlayState* play) {
    ...
    for (i = 0, phi_s1 = 0; i < 21; i++, phi_s1 += 0x618) {
        temp_f20 = MM_Math_SinS(phi_s1);
        temp_f22 = MM_Math_CosS(phi_s1);

        sp9C.x = (i - 10) * 9.0f;
        sp9C.y = (MM_Rand_ZeroOne() * 40.0f) + 15.0f;
        sp9C.z = temp_f20 * 10.0f;

        spA8.x = ((MM_Rand_ZeroOne() - 0.5f) * 7.0f) + (temp_f22 * -12.0f);
        spA8.z = ((MM_Rand_ZeroOne() - 0.5f) * 7.0f) + (temp_f20 * 16.0f);

        spB4.x = spA8.x * -0.09f;
        spB4.z = spA8.z * -0.09f;

        MM_Matrix_MultVec3f(&sp9C, &spC0);
        MM_Matrix_MultVec3f(&spA8, &spCC);
        MM_Matrix_MultVec3f(&spB4, &spD8);

        spC0.x += this->dyna.actor.world.pos.x;
        spC0.y += this->dyna.actor.world.pos.y;
        spC0.z += this->dyna.actor.world.pos.z;

        func_800B0E48(play, &spC0, &spCC, &spD8, &D_80ABCFAC, &D_80ABCFB0, (MM_Rand_Next() >> 0x1A) + 60,
                      (MM_Rand_Next() >> 0x1B) + 60);
    }
}
...
void func_80ABC7FC(BgHakuginBombwall* this, PlayState* play) {
    ...
    for (i = 0, phi_s0 = 0; i < 20; i++, phi_s0 += 0xCCC) {
        temp_f20 = MM_Math_SinS(phi_s0);
        temp_f22 = MM_Math_CosS(phi_s0);
        temp_f24 = MM_Rand_ZeroOne() * 60.0f;

        spA0.x = (temp_f22 * temp_f24) + this->dyna.actor.world.pos.x;
        spA0.y = (MM_Rand_ZeroOne() * 20.0f) + this->dyna.actor.world.pos.y;
        spA0.z = (temp_f20 * temp_f24) + this->dyna.actor.world.pos.z;

        spAC.x = ((MM_Rand_ZeroOne() - 0.5f) * 7.0f) + (temp_f20 * 15.0f);
        spAC.z = ((MM_Rand_ZeroOne() - 0.5f) * 7.0f) + (temp_f22 * 15.0f);

        spB8.x = spAC.x * -0.095f;
        spB8.z = spAC.z * -0.095f;

        func_800B0E48(play, &spA0, &spAC, &spB8, &D_80ABCFAC, &D_80ABCFB0, (MM_Rand_Next() >> 0x1A) + 60,
                      (MM_Rand_Next() >> 0x1B) + 60);
    }
}
...
void BgHakuginBombwall_Init(Actor* thisx, PlayState* play) {
    ...
    if (MM_Flags_GetSwitch(play, BGHAKUGIN_BOMBWALL_SWITCH_FLAG(&this->dyna.actor))) {
    ...
    MM_Actor_SetFocus(&this->dyna.actor, ptr->unk_08);
...
s32 func_80ABCB5C(BgHakuginBombwall* this, PlayState* play) {
    if (this->collider.base.acFlags & AC_HIT) {
        if (this->collider.base.ac != NULL) {
            ...
            if (MM_Math3D_Vec3fDistSq(&thisx->world.pos, &this->collider.base.ac->world.pos) <
                D_80ABCFC0[BGHAKUGIN_BOMBWALL_100(thisx)].unk_1C) {
                MM_SoundSource_PlaySfxAtFixedWorldPos(play, &thisx->world.pos, 60, NA_SE_EV_WALL_BROKEN);
                return true;
            }
        }
    }
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        if (this->collider.elem.acHitElem->atDmgInfo.dmgFlags & 8) {
            if (this->collider.base.ac != NULL) {
                ...
                if (MM_Math3D_Vec3fDistSq(&thisx->world.pos, &this->collider.base.ac->world.pos) <
                    D_80ABCFC0[BGHAKUGIN_BOMBWALL_100(thisx)].unk_1C) {
                    MM_SoundSource_PlaySfxAtFixedWorldPos(play, &thisx->world.pos, 50, NA_SE_EV_WALL_BROKEN);
                    return true;
                }
            }
        } else if (MM_DynaPolyActor_IsPlayerOnTop(&this->dyna)) {
            MM_SoundSource_PlaySfxAtFixedWorldPos(play, &this->dyna.actor.world.pos, 50, NA_SE_EV_WALL_BROKEN);
            return true;
        }
    }
    return false;
...
void func_80ABCD98(BgHakuginBombwall* this, PlayState* play) {
    ...
    if (CutsceneManager_IsNext(this->dyna.actor.csId)) {
        ...
        this->unk_1AC = 20;
        MM_Flags_SetSwitch(play, BGHAKUGIN_BOMBWALL_SWITCH_FLAG(&this->dyna.actor));
...
void BgHakuginBombwall_Draw(Actor* thisx, PlayState* play) {
    MM_Gfx_DrawDListOpa(play, D_80ABCFC0[BGHAKUGIN_BOMBWALL_100(thisx)].unk_04);
}
