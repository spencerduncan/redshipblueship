...
void MM_EnDodongo_Init(Actor* thisx, PlayState* play);
void MM_EnDodongo_Destroy(Actor* thisx, PlayState* play);
void MM_EnDodongo_Update(Actor* thisx, PlayState* play2);
void MM_EnDodongo_Draw(Actor* thisx, PlayState* play);
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 3, 100, 100, 80 };
...
void MM_EnDodongo_Init(Actor* thisx, PlayState* play) {
    ...
    this->unk_330.g = 10;
    this->unk_330.a = 200;
    MM_Math_Vec3f_Copy(&this->unk_314, &gOneVec3f);
    MM_ActorShape_Init(&this->actor.shape, 0.0f, MM_ActorShadow_DrawCircle, 48.0f);
    MM_SkelAnime_Init(play, &this->skelAnime, &object_dodongo_Skel_008318, &object_dodongo_Anim_004C20, this->jointTable,
    ...
    MM_Effect_Add(play, &this->unk_338, EFFECT_BLURE2, 0, 0, &D_80879308);
    if (this->actor.params == 0) {
        MM_Actor_SetScale(&this->actor, 3.0f / 160.0f);
        this->unk_334 = 1.0f;
    } else {
        MM_Actor_SetScale(&this->actor, 3.0f / 80.0f);
...
void MM_EnDodongo_Destroy(Actor* thisx, PlayState* play) {
    ...
    MM_Collider_DestroyJntSph(play, &this->collider3);
...
void func_80876930(EnDodongo* this, PlayState* play, Vec3f* arg2) {
    ...
    for (i = 0; i < 3; i++) {
        sp88.x = MM_Rand_CenteredFloat(temp3) + arg2->x;
        sp88.y = MM_Rand_ZeroFloat(5.0f) + this->actor.floorHeight;
        sp88.z = MM_Rand_CenteredFloat(temp3) + arg2->z;
        D_8087933C.x = MM_Rand_CenteredFloat(2.0f);
        D_8087933C.z = MM_Rand_CenteredFloat(2.0f);
...
void func_80876B08(EnDodongo* this, PlayState* play) {
    ...
    if (ABS_ALT(yDiff) < 0x4000) {
        if (!(player->stateFlags1 & 0x00800000) && (MM_Player_GetMask(play) != PLAYER_MASK_STONE)) {
            func_808777A8(this);
        } else {
            func_80877494(this);
        }
...
void func_80876BD0(EnDodongo* this, PlayState* play, s32 arg2) {
    if (this->actor.colChkInfo.damageEffect == 2) {
        ...
        this->drawDmgEffScale = 0.75f;
        this->drawDmgEffAlpha = 4.0f;
        MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_CLEAR_TAG, this->collider1.elements[arg2].base.acDmgInfo.hitPos.x,
...
void func_80876DC4(EnDodongo* this, PlayState* play) {
    ...
    MM_Math_Vec3f_Copy(&sp68, &this->bodyPartsPos[DODONGO_BODYPART_0]);
    sp66 = ((s32)MM_Rand_Next() >> 0x12) + this->actor.shape.rot.y;
    temp_f20 = MM_Math_CosS(sp66);
    temp_f22 = MM_Math_SinS(sp66);
    ...
    sp74.x = (MM_Rand_ZeroFloat(0.1f) + 0.15f) * -temp_f22 * this->unk_334;
    sp74.y = this->unk_334 * 0.3f;
    sp74.z = (MM_Rand_ZeroFloat(0.1f) + 0.15f) * -temp_f20 * this->unk_334;
    ...
    sp66 = (MM_Rand_Next() >> 0x13) + this->actor.shape.rot.y;
    temp_f20 = MM_Math_CosS(sp66);
    temp_f22 = MM_Math_SinS(sp66);
    ...
    sp74.x = (MM_Rand_ZeroFloat(0.1f) + 0.15f) * temp_f20 * this->unk_334;
    sp74.z = (MM_Rand_ZeroFloat(0.1f) + 0.15f) * -temp_f22 * this->unk_334;
    func_800B0EB0(play, &sp68, &sp80, &sp74, &this->unk_32C, &this->unk_330, sp64, sp62, 0x14);
...
s32 func_80877278(EnDodongo* this, PlayState* play) {
    ...
    while (explosive != NULL) {
        if (!explosive->params && (explosive->parent == NULL) && (explosive->update != NULL) &&
            ...
            MM_Math_Vec3f_Diff(&explosive->world.pos, &this->unk_308, &sp44);
...
void func_808773C4(EnDodongo* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &object_dodongo_Anim_004C20, -4.0f);
    this->actor.speed = 0.0f;
    this->timer = MM_Rand_S16Offset(30, 50);
...
void func_80877494(EnDodongo* this) {
    MM_Animation_MorphToLoop(&this->skelAnime, &object_dodongo_Anim_008B1C, -4.0f);
    this->actor.speed = this->unk_334 * 1.5f;
    this->timer = MM_Rand_S16Offset(50, 70);
...
void func_80877500(EnDodongo* this, PlayState* play) {
    ...
    } else if (MM_Animation_OnFrame(&this->skelAnime, 39.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_DODO_J_WALK);
    ...
    if ((MM_Math_Vec3f_DistXZ(&this->actor.home.pos, &player->actor.world.pos) < 400.0f) &&
        (MM_Player_GetMask(play) != PLAYER_MASK_STONE)) {
        temp_v1 = this->actor.yawTowardsPlayer - this->actor.shape.rot.y;
        MM_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 0x1F4);
        ...
        if ((MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) > 150.0f) ||
            (MM_Player_GetMask(play) == PLAYER_MASK_STONE)) {
            MM_Math_ScaledStepToS(&this->actor.world.rot.y, MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos),
                               0x1F4);
        }
...
void func_808777A8(EnDodongo* this) {
    ...
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &object_dodongo_Anim_0028F0, -4.0f);
    this->actor.speed = 0.0f;
...
void func_8087784C(EnDodongo* this, PlayState* play) {
    ...
    if (func_8087721C(this)) {
        ...
        temp_f2 = MM_Math_SinS(this->actor.shape.rot.y) * this->unk_334;
        temp_f12 = MM_Math_CosS(this->actor.shape.rot.y) * this->unk_334;
        ...
        MM_EffectSsDFire_Spawn(play, &this->bodyPartsPos[DODONGO_BODYPART_0], &D_80879354, &D_80879348,
...
void func_80877D50(EnDodongo* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &object_dodongo_Anim_003088);
...
void func_80877D90(EnDodongo* this, PlayState* play) {
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        func_808773C4(this);
        this->timer = MM_Rand_S16Offset(10, 20);
    }
}

void func_80877DE0(EnDodongo* this) {
    MM_Animation_Change(&this->skelAnime, &object_dodongo_Anim_0028F0, -1.0f, 35.0f, 0.0f, ANIMMODE_ONCE, -4.0f);
...
void func_80877E60(EnDodongo* this, PlayState* play) {
    ...
    if (this->actor.child != NULL) {
        MM_Math_Vec3f_Copy(&this->actor.child->world.pos, &this->unk_308);
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 28.0f)) {
        ...
        if (this->timer == 10) {
            for (i = 10; i >= 0; i--) {
                sp84.x = MM_Rand_CenteredFloat(10.0f);
                sp84.y = MM_Rand_CenteredFloat(10.0f);
                sp84.z = MM_Rand_CenteredFloat(10.0f);
                sp78.x = sp84.x * -0.1f;
                sp78.y = sp84.y * -0.1f;
                sp78.z = sp84.z * -0.1f;
            ...
            MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 120, COLORFILTER_BUFFLAG_OPA, 8);
        }
    ...
    if (this->skelAnime.curFrame <= 27.0f) {
        if ((this->skelAnime.curFrame <= 25.0f) && (this->timer < 11)) {
            ...
            MM_Math_Vec3f_Copy(&sp64, &this->bodyPartsPos[DODONGO_BODYPART_0]);
            func_800B0DE0(play, &sp64, &gZeroVec3f, &D_80879360, &D_8087936C, &D_8087936C, sp5E, sp5C);
            sp64.x -= MM_Math_CosS(this->actor.shape.rot.y) * 6.0f * this->unk_334;
            sp64.z += MM_Math_SinS(this->actor.shape.rot.y) * 6.0f * this->unk_334;
            func_800B0DE0(play, &sp64, &gZeroVec3f, &D_80879360, &D_8087936C, &D_8087936C, sp5E, sp5C);
    ...
    this->unk_314.z = MM_Math_SinS((this->actor.colorFilterTimer * 0x1000) >> 1) + 1.0f;
    this->unk_314.x = MM_Math_SinS(this->actor.colorFilterTimer * 0x1000) + 1.0f;
...
void func_80878354(EnDodongo* this) {
    ...
    MM_Animation_PlayOnceSetSpeed(&this->skelAnime, anim, 2.0f);
    this->timer = 0;
...
void func_80878424(EnDodongo* this, PlayState* play) {
    ...
    this->timer++;
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if (!(player->stateFlags1 & PLAYER_STATE1_800000) && (MM_Player_GetMask(play) != PLAYER_MASK_STONE)) {
            this->collider1.base.atFlags &= ~AT_ON;
            func_808777A8(this);
        } else {
            func_80877494(this);
...
void func_8087864C(EnDodongo* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &object_dodongo_Anim_001A44, -4.0f);
...
void func_80878724(EnDodongo* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &object_dodongo_Anim_0013C4, -8.0f);
    this->timer = 0;
    this->unk_304 = 0;
...
void func_808787B0(EnDodongo* this, PlayState* play) {
    ...
    if (this->skelAnime.curFrame < 35.0f) {
        ...
        MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 120, COLORFILTER_BUFFLAG_OPA, 4);
    }
    ...
    } else if (MM_Animation_OnFrame(&this->skelAnime, 52.0f)) {
    ...
    if (this->timer != 0) {
        ...
        if (this->timer == 0) {
            if (this->actor.params == 0) {
                ...
                MM_Item_DropCollectible(play, &this->actor.world.pos, ITEM00_RUPEE_PURPLE);
            }
...
void EnDodongo_UpdateDamage(EnDodongo* this, PlayState* play) {
    ...
    if (this->collider2.base.acFlags & AC_HIT) {
        ...
        if ((i != ARRAY_COUNT(this->collider2Elements)) &&
            ...
            func_80876D28(this, play);
            MM_Math_Vec3s_ToVec3f(&sp3C, &this->collider2.elements[i].base.acDmgInfo.hitPos);
            if (this->actor.colChkInfo.damageEffect == 0xF) {
                MM_CollisionCheck_BlueBlood(play, NULL, &sp3C);
                ...
                MM_CollisionCheck_SpawnShieldParticlesMetalSound(play, &sp3C, &this->actor.projectedPos);
            }
        }
    } else if (this->collider1.base.acFlags & AC_HIT) {
        this->collider2.base.acFlags &= ~AC_HIT;
        this->collider1.base.acFlags &= ~AC_HIT;
        MM_Actor_SetDropFlagJntSph(&this->actor, &this->collider1);
...
void MM_EnDodongo_Update(Actor* thisx, PlayState* play2) {
    ...
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 75.0f, 60.0f, 70.0f,
    ...
    if (func_8087721C(this)) {
        MM_CollisionCheck_SetAT(play, &play->colChkCtx, &this->collider3.base);
    }
...
s32 MM_EnDodongo_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    EnDodongo* this = (EnDodongo*)thisx;

    if (limbIndex == OBJECT_DODONGO_LIMB_01) {
        pos->z += 1000.0f;
    } else if ((limbIndex == OBJECT_DODONGO_LIMB_0F) || (limbIndex == OBJECT_DODONGO_LIMB_10)) {
        MM_Matrix_Scale(this->unk_314.x, this->unk_314.y, this->unk_314.z, MTXMODE_APPLY);
    }
...
void MM_EnDodongo_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    EnDodongo* this = (EnDodongo*)thisx;
    ...
    if (limbIndex == OBJECT_DODONGO_LIMB_07) {
        MM_Matrix_MultVec3f(&D_80879370, &this->unk_308);
        MM_Matrix_MultVec3f(&D_8087937C, &this->bodyPartsPos[DODONGO_BODYPART_0]);
    ...
    if ((limbIndex == OBJECT_DODONGO_LIMB_1E) && (this->actionFunc == func_80878424) &&
        ...
        MM_EffectBlure_AddVertex(MM_Effect_GetByIndex(this->unk_338), &this->unk_320,
...
void MM_EnDodongo_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, MM_EnDodongo_OverrideLimbDraw,
                      MM_EnDodongo_PostLimbDraw, &this->actor);
