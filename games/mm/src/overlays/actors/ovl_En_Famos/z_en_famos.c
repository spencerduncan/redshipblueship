...
void EnFamos_Init(Actor* thisx, PlayState* play) {
    ...
    MM_ActorShape_Init(&this->actor.shape, 0.0f, ActorShadow_DrawSquare, 30.0f);
    MM_SkelAnime_Init(play, &this->skelAnime, &gFamosSkel, &gFamosIdleAnim, this->jointTable, this->morphTable,
                   FAMOS_LIMB_MAX);
    ...
    Collider_InitAndSetJntSph(play, &this->emblemCollider, &this->actor, &MM_sJntSphInit, this->emblemColliderElements);
...
void EnFamos_Destroy(Actor* thisx, PlayState* play) {
    ...
    MM_Collider_DestroyJntSph(play, &this->emblemCollider);
}
...
void EnFamos_SetupAttackDebris(EnFamos* this) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->rocks); i++, rock++) {
        randVelDirection = (s32)MM_Rand_Next() >> 0x10;
        randOffset = MM_Rand_S16Offset(0x1800, 0x2800);
        randFloat = MM_Rand_ZeroFloat(5.0f) + 5.0f;
        rock->velocity.x = randFloat * MM_Math_CosS(randOffset) * MM_Math_SinS(randVelDirection);
        rock->velocity.y = MM_Math_SinS(randOffset) * randFloat + 3.0f;
        rock->velocity.z = randFloat * MM_Math_CosS(randOffset) * MM_Math_CosS(randVelDirection);
        rock->rot.x = (s32)MM_Rand_Next() >> 0x10;
        rock->rot.y = (s32)MM_Rand_Next() >> 0x10;
        rock->rot.z = (s32)MM_Rand_Next() >> 0x10;
        rock->pos.x = (MM_Math_SinS(randVelDirection) * 20.0f) + this->actor.world.pos.x;
        rock->pos.y = this->actor.floorHeight;
        rock->pos.z = (MM_Math_CosS(randVelDirection) * 20.0f) + this->actor.world.pos.z;
        rock->scale = MM_Rand_ZeroFloat(0.0015f) + (2.0f * 0.001f);
    }
}
...
void EnFamos_SetupDeathDebris(EnFamos* this) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->rocks); i++, rock++) {
        randVelDirection = (s32)MM_Rand_Next() >> 0x10;
        randSmaller = MM_Rand_Next() >> 0x12;
        randFloat = MM_Rand_ZeroFloat(6.0f) + 7.0f;
        rock->velocity.x = randFloat * MM_Math_CosS(randSmaller) * MM_Math_SinS(randVelDirection);
        rock->velocity.y = MM_Math_SinS(randSmaller) * randFloat + 4.5f;
        rock->velocity.z = randFloat * MM_Math_CosS(randSmaller) * MM_Math_CosS(randVelDirection);
        rock->rot.x = (s32)MM_Rand_Next() >> 0x10;
        rock->rot.y = (s32)MM_Rand_Next() >> 0x10;
        rock->rot.z = (s32)MM_Rand_Next() >> 0x10;
        rock->pos.x = MM_Math_SinS(randVelDirection) * 20.0f + this->actor.world.pos.x;
        rock->pos.y = MM_Rand_CenteredFloat(60.0f) + (this->actor.world.pos.y + 40.0f);
        rock->pos.z = MM_Math_CosS(randVelDirection) * 20.0f + this->actor.world.pos.z;
        rock->scale = MM_Rand_ZeroFloat(0.002f) + (2.5f * 0.001f);
    }
}

s32 EnFamos_IsPlayerSeen(EnFamos* this, PlayState* play) {
    if ((MM_Player_GetMask(play) != PLAYER_MASK_STONE) &&
        (MM_Actor_WorldDistXZToPoint(&GET_PLAYER(play)->actor, &this->calmPos) < this->aggroDistance) &&
...
void EnFamos_UpdateBobbingHeight(EnFamos* this) {
    ...
    this->hoverTimer--;
    this->actor.world.pos.y = (MM_Math_SinS(this->hoverTimer * 0x888) * 10.0f) + this->baseHeight;
...
void EnFamos_UpdateFlipStatus(EnFamos* this) {
    ...
    if (this->hasFinishedRotating == false) {
        if (this->animatedMaterialIndex != FAMOS_ANIMATED_MAT_NORMAL) {
            hasFinishedRotating = MM_Math_ScaledStepToS(&this->flipRot, -0x8000, 0x1000);
        } else {
            hasFinishedRotating = MM_Math_ScaledStepToS(&this->flipRot, 0, 0x1000);
        }
...
void EnFamos_StillIdle(EnFamos* this, PlayState* play) {
    ...
    if (this->isCalm) {
        MM_Math_Vec3f_Copy(&this->calmPos, &this->actor.world.pos);
    }
...
void EnFamos_SetupPathingIdle(EnFamos* this) {
    ...
    MM_Math_Vec3s_ToVec3f(&this->targetDest, &this->pathPoints[this->waypointIndex]);
    this->targetYaw = MM_Actor_WorldYawTowardPoint(&this->actor, &this->targetDest);
...
void EnFamos_PathingIdle(EnFamos* this, PlayState* play) {
    ...
    if (EnFamos_IsPlayerSeen(this, play)) {
        EnFamos_SetupAlert(this);
    } else if (MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->targetYaw, 0x200)) {
 ...
 */
void EnFamos_SetupTurnHome(EnFamos* this) {
    this->targetYaw = MM_Actor_WorldYawTowardPoint(&this->actor, &this->calmPos);
    MM_Math_Vec3f_Copy(&this->targetDest, &this->calmPos);
...
void EnFamos_SetupReturnHome(EnFamos* this) {
    ...
    this->actor.world.rot.x = -MM_Actor_WorldPitchTowardPoint(&this->actor, &this->targetDest);
    this->actionFunc = EnFamos_ReturnHome;
}

void EnFamos_ReturnHome(EnFamos* this, PlayState* play) {
    f32 distanceToHome = MM_Actor_WorldDistXZToPoint(&this->actor, &this->targetDest);

    this->actor.shape.rot.y = MM_Actor_WorldYawTowardPoint(&this->actor, &this->targetDest);
    this->actor.world.rot.y = this->actor.shape.rot.y;
    EnFamos_UpdateBobbingHeight(this);
    if (this->isCalm) {
        MM_Math_Vec3f_Copy(&this->calmPos, &this->actor.world.pos);
    ...
    if (EnFamos_IsPlayerSeen(this, play)) {
        ...
        MM_Math_StepToF(&this->actor.speed, 0.5f, 0.3f);
    } else {
...
void EnFamos_Alert(EnFamos* this, PlayState* play) {
    ...
    if (this->stateTimer == 0) {
        ...
        this->actor.world.pos.y = MM_Math_SinS(this->stateTimer * 0x1000) * 30.0f + this->baseHeight;
    }
}
...
void EnFamos_Chase(EnFamos* this, PlayState* play) {
    ...
    this->actor.world.rot.x = -MM_Actor_WorldPitchTowardPoint(&this->actor, &abovePlayerPos);
    MM_Math_StepToF(&this->actor.speed, 6.0f, 0.5f);
    ...
    if ((this->actor.xzDistToPlayer < 30.0f) && (this->actor.floorHeight > BGCHECK_Y_MIN) &&
        ...
        EnFamos_SetupAttackAim(this);
               ...
               (this->aggroDistance < MM_Actor_WorldDistXZToPoint(&GET_PLAYER(play)->actor, &this->calmPos)) ||
               !MM_Actor_IsFacingPlayer(&this->actor, 0x6000)) {
        EnFamos_SetupScanForPlayer(this);
    }
}

void EnFamos_SetupAttackAim(EnFamos* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gFamosShakeAnim);
    this->actor.speed = 0.0f;
...
void EnFamos_Attack(EnFamos* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->actor.speed, 20.0f, 2.0f);
    this->stateTimer--;
    if (this->stateTimer == 0) {
    ...
    if (hitFloor || (this->actor.floorHeight == BGCHECK_Y_MIN) || (surfaceType == FLOOR_PROPERTY_12) ||
        ...
        if (hitFloor) {
            MM_Camera_AddQuake(GET_ACTIVE_CAM(play), 2, 15, 10);
            Rumble_Request(this->actor.xyzDistToPlayerSq, 180, 20, 100);
            EnFamos_SetupAttackDebris(this);

            // spawn crator on floor
            MM_Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, ACTOR_EN_TEST, this->actor.world.pos.x,
                               this->actor.floorHeight, this->actor.world.pos.z, 0, 0, 0, 0x0);

            if (this->actor.child != NULL) {
                MM_Actor_SetScale(this->actor.child, 0.015f);
            }
 ...
 */
void EnFamos_SetupFinishAttack(EnFamos* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gFamosIdleAnim);
...
void EnFamos_AttackRebound(EnFamos* this, PlayState* play) {
    MM_Math_StepToF(&this->actor.speed, 5.0f, 0.3f);
...
void EnFamos_ScanForPlayer(EnFamos* this, PlayState* play) {
    ...
    if (EnFamos_IsPlayerSeen(this, play)) {
        ...
        this->actor.shape.rot.y = (s32)(MM_Math_SinS(this->stateTimer * 0x888) * 8192.0f) + this->actor.world.rot.y;
    }
}
...
void EnFamos_SetupDeathExplosion(EnFamos* this) {
    this->actor.world.rot.x = 0x4000;
    MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 255, COLORFILTER_BUFFLAG_OPA, 4);
    this->stateTimer = 25;
    MM_Math_Vec3f_Copy(&this->targetDest, &this->actor.world.pos);
...
void EnFamos_DeathExplosion(EnFamos* this, PlayState* play) {
    ...
    this->actor.world.pos.x = MM_Rand_CenteredFloat(5.0f) + this->targetDest.x;
    this->actor.world.pos.z = MM_Rand_CenteredFloat(5.0f) + this->targetDest.z;
    if (this->stateTimer == 1) {
        EnBom* explosion = (EnBom*)MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_BOM, this->actor.world.pos.x,
        ...
        MM_Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos,
...
void EnFamos_UpdateDebrisPosRot(EnFamos* this) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->rocks); i++, rock++) {
        ...
        MM_Math_Vec3f_Sum(&rock->pos, &rock->velocity, &rock->pos);
        rock->rot.x += (s16)((MM_Rand_Next() >> 0x17) + 0x700);
        rock->rot.y += (s16)((MM_Rand_Next() >> 0x17) + 0x900);
        rock->rot.z += (s16)((MM_Rand_Next() >> 0x17) + 0xB00);
    }
}
...
void EnFamos_Update(Actor* thisx, PlayState* play) {
    ...
    if ((this->debrisTimer <= 0) ||
        ...
        if (this->flippedTimer >= 0) {
            MM_Actor_UpdateBgCheckInfo(play, &this->actor, 35.0f, 30.0f, 80.0f,
        ...
        if (this->emblemCollider.base.acFlags & AC_ON) {
            MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->emblemCollider.base);
        }
...
s32 EnFamos_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    ...
    if (limbIndex == FAMOS_LIMB_BODY) {
        MM_Matrix_Translate(0.0f, 4000.0f, 0.0f, MTXMODE_APPLY);
...
void EnFamos_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (limbIndex == FAMOS_LIMB_EMBLEM) {
        ...
        MM_Collider_UpdateSpheres(limbIndex, &this->emblemCollider);
    }
}
...
void EnFamos_DrawDebris(EnFamos* this, PlayState* play) {
    ...
    if (this->debrisTimer > 0) {
        ...
        for (i = 0; i < ARRAY_COUNT(this->rocks); i++, rock++) {
            ...
            MM_Matrix_SetTranslateRotateYXZ(rock->pos.x, rock->pos.y, rock->pos.z, &rock->rot);
            MM_Matrix_Scale(rock->scale, rock->scale, rock->scale, MTXMODE_APPLY);
...
void EnFamos_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != EnFamos_DeathFade) {
        ...
        MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, EnFamos_OverrideLimbDraw,
