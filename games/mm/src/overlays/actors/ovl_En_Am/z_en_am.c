...
void MM_EnAm_Init(Actor* thisx, PlayState* play);
void MM_EnAm_Destroy(Actor* thisx, PlayState* play);
void MM_EnAm_Update(Actor* thisx, PlayState* play);
void MM_EnAm_Draw(Actor* thisx, PlayState* play);
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 1, 23, 98, MASS_HEAVY };
...
void MM_EnAm_Init(Actor* thisx, PlayState* play) {
    ...
    MM_ActorShape_Init(&this->actor.shape, 0.0f, MM_ActorShadow_DrawCircle, 300.0f / 7.0f);
    ...
    Collider_InitAndSetCylinder(play, &this->interactCollider, &this->actor, &MM_sCylinderInit);
    MM_CollisionCheck_SetInfo(&this->actor.colChkInfo, &MM_sDamageTable, &MM_sColChkInfoInit);
    this->actor.home.pos.x -= 9.0f * MM_Math_SinS(this->actor.shape.rot.y);
    this->actor.home.pos.z -= 9.0f * MM_Math_CosS(this->actor.shape.rot.y);
...
void MM_EnAm_Destroy(Actor* thisx, PlayState* play) {
    ...
    MM_Collider_DestroyCylinder(play, &this->enemyCollider);
    MM_Collider_DestroyCylinder(play, &this->interactCollider);
}

void MM_EnAm_SpawnEffects(EnAm* this, PlayState* play) {
    static Vec3f MM_sVelocity = { 0.0f, -1.5f, 0.0f };
    static Vec3f MM_sAccel = { 0.0f, -(1.0f / 5.0f), 0.0f };
    ...
    for (i = 4; i > 0; i--) {
        effectPos.x = MM_Rand_CenteredFloat(65.0f) + this->actor.world.pos.x;
        effectPos.y = MM_Rand_CenteredFloat(10.0f) + (this->actor.world.pos.y + 40.0f);
        effectPos.z = MM_Rand_CenteredFloat(65.0f) + this->actor.world.pos.z;
        EffectSsKirakira_SpawnSmall(play, &effectPos, &MM_sVelocity, &MM_sAccel, &sPrimColor, &sEnvColor);
    }
    Actor_PlaySfx(&this->actor, NA_SE_EN_AMOS_WALK);
    MM_Actor_SpawnFloorDustRing(play, &this->actor, &this->actor.world.pos, 4.0f, 3, 8.0f, 300, 15, false);
}

void func_808AFF9C(EnAm* this) {
    f32 endFrame = MM_Animation_GetLastFrame(&gArmosPushedBackAnim);

    MM_Animation_Change(&this->skelAnime, &gArmosPushedBackAnim, 0.0f, endFrame, endFrame, ANIMMODE_LOOP, 0.0f);
...
void EnAm_ApplyEnemyTexture(EnAm* this, PlayState* play) {
    ...
    if (this->textureBlend + 20 >= 255) {
        ...
        rand = MM_Rand_CenteredFloat(10.0f);
        cos = MM_Math_CosS(this->actor.shape.rot.y) * rand;
        sin = MM_Math_SinS(this->actor.shape.rot.y) * rand;
...
void func_808B0208(EnAm* this, PlayState* play) {
    ...
    if ((this->actor.speed > 0.0f) && (this->actor.bgCheckFlags & BGCHECKFLAG_WALL)) {
        ...
        this->actor.world.pos.x += this->actor.speed * MM_Math_SinS(this->actor.world.rot.y);
        this->actor.world.pos.z += this->actor.speed * MM_Math_CosS(this->actor.world.rot.y);
    }
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 8.0f)) {
        ...
        if (!(this->actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
            ...
            MM_Math_ScaledStepToS(&this->actor.world.rot.y, this->armosYaw, 0x1F40);
            this->actor.speed = 0.0f;
            if (this->actor.bgCheckFlags & BGCHECKFLAG_GROUND_TOUCH) {
                MM_EnAm_SpawnEffects(this, play);
            }
        }
    }
...
void func_808B0358(EnAm* this) {
    MM_Animation_PlayLoopSetSpeed(&this->skelAnime, &gArmosHopAnim, 4.0f);
    this->explodeTimer = 3;
    this->actor.speed = 0.0f;
...
void func_808B03C0(EnAm* this, PlayState* play) {
    ...
    if (this->explodeTimer == 0) {
        func_808B0640(this);
    } else if ((this->returnHomeTimer == 0) || MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) > 240.0f) {
        func_808B0460(this);
    }
}
...
void func_808B0460(EnAm* this) {
    this->actor.world.rot.y = this->actor.shape.rot.y;
    this->speed = 0.0f;
    this->armosYaw = MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos);
...
void func_808B0508(EnAm* this, PlayState* play) {
    func_808B0208(this, play);
    if (!(this->actor.bgCheckFlags & BGCHECKFLAG_GROUND)) {
        MM_Math_StepToF(&this->actor.world.pos.x, this->actor.home.pos.x, 2.0f);
        MM_Math_StepToF(&this->actor.world.pos.z, this->actor.home.pos.z, 2.0f);
    }
...
void func_808B05C8(EnAm* this, PlayState* play) {
    this->armosYaw = MM_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos);
    func_808B0208(this, play);
    if (MM_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) < 8.0f) {
        func_808B04E4(this);
    }
}
...
void EnAm_TakeDamage(EnAm* this, PlayState* play) {
    f32 endFrame = MM_Animation_GetLastFrame(&gArmosTakeDamageAnim) - 6;

    MM_Animation_Change(&this->skelAnime, &gArmosTakeDamageAnim, 1.0f, 4.0f, endFrame, ANIMMODE_ONCE, 0.0f);
    ...
    MM_Actor_SetColorFilter(&this->actor, COLORFILTER_COLORFLAG_RED, 255, COLORFILTER_BUFFLAG_OPA,
                         MM_Animation_GetLastFrame(&gArmosTakeDamageAnim) - 10);
...
void func_808B0894(EnAm* this, PlayState* play) {
    ...
    if (this->explodeTimer == 1) {
        ...
        for (i = 0; i < 8; i++) {
            dustPos.x = (MM_Math_SinS(0) * 7.0f) + this->actor.world.pos.x;
            dustPos.y = (MM_Rand_CenteredFloat(10.0f) * 6.0f) + (this->actor.world.pos.y + 40.0f);
            dustPos.z = (MM_Math_CosS(0) * 7.0f) + this->actor.world.pos.z;
...
void func_808B0AD0(EnAm* this, PlayState* play) {
    MM_Animation_Change(&this->skelAnime, &gArmosPushedBackAnim, 1.0f, 0.0f, 8.0f, ANIMMODE_ONCE, 0.0f);
...
s32 MM_EnAm_UpdateDamage(EnAm* this, PlayState* play) {
    if (this->enemyCollider.base.acFlags & AC_HIT) {
        this->enemyCollider.base.acFlags &= ~AC_HIT;
        MM_Actor_SetDropFlag(&this->actor, &this->enemyCollider.elem);
        ...
        if (this->actor.colChkInfo.damageEffect == 0x4) {
            ...
            MM_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_CLEAR_TAG, this->enemyCollider.elem.acDmgInfo.hitPos.x,
...
void MM_EnAm_Update(Actor* thisx, PlayState* play) {
    ...
    if (MM_EnAm_UpdateDamage(this, play) == false) {
    ...
    MM_Actor_SetFocus(&this->actor, 64.0f);
    MM_Collider_UpdateCylinder(&this->actor, &this->enemyCollider);
    MM_Collider_UpdateCylinder(&this->actor, &this->interactCollider);
    MM_CollisionCheck_SetOC(play, &play->colChkCtx, &this->enemyCollider.base);
    if (this->enemyCollider.base.acFlags & AC_ON) {
        MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->enemyCollider.base);
    }
    MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->interactCollider.base);
    if (this->enemyCollider.base.atFlags & AT_ON) {
        this->actor.flags |= ACTOR_FLAG_SFX_FOR_PLAYER_BODY_HIT;
        MM_CollisionCheck_SetAT(play, &play->colChkCtx, &this->enemyCollider.base);
    }
...
void MM_EnAm_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    s32 i;
    s32 phi_s3;
    Vec3f* phi_s1;
    Vec3f* phi_s2;
    ...
    for (i = 0; i < phi_s3; i++, phi_s2++, phi_s1++) {
        MM_Matrix_MultVec3f(phi_s1, phi_s2);
    }
}
...
void MM_EnAm_Draw(Actor* thisx, PlayState* play) {
    ...
    POLY_OPA_DISP = &gfx[2];
    MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, NULL, MM_EnAm_PostLimbDraw,
                      &this->actor);
