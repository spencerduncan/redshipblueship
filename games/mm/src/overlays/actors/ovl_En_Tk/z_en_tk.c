...
void MM_EnTk_Init(Actor* thisx, PlayState* play);
void MM_EnTk_Destroy(Actor* thisx, PlayState* play);
void MM_EnTk_Update(Actor* thisx, PlayState* play);
void MM_EnTk_Draw(Actor* thisx, PlayState* play);
...
void func_80AEC460(EnTk* this) {
    if (DECR(this->unk_2C4) == 0) {
        ...
        if (this->unk_2C2 >= 3) {
            this->unk_2C0--;
            if (this->unk_2C0 < 0) {
                this->unk_2C4 = MM_Rand_S16Offset(30, 30);
                this->unk_2C0 = 2;
...
void MM_EnTk_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &object_tk_Skel_00B9E8, NULL, this->jointTable, this->morphTable,
                       OBJECT_TK_LIMB_MAX);
    MM_Animation_Change(&this->skelAnime, &object_tk_Anim_0030A4, 1.0f, 0.0f,
                     MM_Animation_GetLastFrame(&object_tk_Anim_0030A4), ANIMMODE_LOOP, 0.0f);
    this->unk_318 = 0;
...
void func_80AECE0C(EnTk* this, PlayState* play) {
    ...
    MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.world.rot.y, 4, 0x1555, 0xB6);
}
...
s32 func_80AECE60(EnTk* this, PlayState* play) {
    ...
    if (SubS_TimePathing_Update(this->timePath, &this->timePathProgress, &this->timePathElapsedTime,
        ...
        this->actor.world.rot.y = MM_Math_Vec3f_Yaw(&sp94, &sp88);
    }
    ...
    if (!(this->unk_2CA & 0xC00)) {
        ...
        do {
            ...
            if (doorIter != NULL) {
                if (MM_Actor_WorldDistXZToActor(&this->actor, doorIter) <= 120.0f) {
                    if (ABS(BINANG_SUB(MM_Actor_WorldYawTowardPoint(&this->actor, &doorIter->world.pos),
        ...
        do {
            doorIter = SubS_FindActor(play, doorIter, ACTORCAT_DOOR, ACTOR_EN_DOOR);
            if (doorIter != NULL) {
                if (MM_Actor_WorldDistXZToActor(&this->actor, doorIter) <= 160.0f) {
                    door = (EnDoor*)doorIter;
                    break;
                }
                doorIter = doorIter->next;
    ...
    if ((door != NULL) && (this->unk_2CA & 0x400)) {
        ...
        MM_Actor_WorldToActorCoords(&this->actor, &sp5C, &door->knobDoor.dyna.actor.world.pos);
        door->openTimer = 2;
        if (sp5C.z < -20.0f) {
            this->unk_2CA &= ~0x400;
            this->unk_2CA |= 0x800;
...
void func_80AED544(EnTk* this, PlayState* play) {
    if (!CHECK_WEEKEVENTREG(WEEKEVENTREG_31_10)) {
        MM_Message_StartTextbox(play, 0x13FE, &this->actor);
        SET_WEEKEVENTREG(WEEKEVENTREG_31_10);
    } else if (CURRENT_TIME < CLOCK_TIME(9, 0)) {
        MM_Message_StartTextbox(play, 0x13FF, &this->actor);
    } else if (CURRENT_TIME < CLOCK_TIME(12, 0)) {
        MM_Message_StartTextbox(play, 0x1400, &this->actor);
    } else if (CURRENT_TIME < CLOCK_TIME(15, 0)) {
        MM_Message_StartTextbox(play, 0x1401, &this->actor);
    } else {
        MM_Message_StartTextbox(play, 0x1402, &this->actor);
    }
}

void func_80AED610(EnTk* this, PlayState* play) {
    if ((this->animIndex == ENTK_ANIM_4) && MM_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
    ...
    switch (MM_Message_GetState(&play->msgCtx)) {
        case TEXT_STATE_NONE:
            if (MM_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer - 0x1555, 0x71C)) {
                if (MM_Player_GetMask(play) == PLAYER_MASK_CAPTAIN) {
                    SubS_ChangeAnimationBySpeedInfo(&this->skelAnime, sAnimationSpeedInfo, ENTK_ANIM_4,
                                                    &this->animIndex);
                    MM_Message_StartTextbox(play, 0x13FD, &this->actor);
                } else if (CURRENT_DAY != 2) {
                    func_80AED544(this, play);
                } else if (!MM_Flags_GetSwitch(play, ENTK_GET_SWITCH_FLAG(&this->actor))) {
                    MM_Message_StartTextbox(play, 0x1403, &this->actor);
                } else if (CHECK_WEEKEVENTREG(WEEKEVENTREG_60_02)) {
                    func_80AED544(this, play);
                } else {
                    MM_Message_StartTextbox(play, 0x1413, &this->actor);
                }
                break;
            }
        ...
        case TEXT_STATE_DONE:
            if (MM_Message_ShouldAdvance(play)) {
                switch (play->msgCtx.currentTextId) {
                    ...
                    case 0x13FE:
                        MM_Message_ContinueTextbox(play, 0x13FF);
                        break;

                    case 0x1413:
                        MM_Rupees_ChangeBy(30);
...
void func_80AED940(EnTk* this, PlayState* play) {
    ...
    if ((this->animIndex != ENTK_ANIM_4) && (this->animIndex != ENTK_ANIM_7)) {
        ...
        if (ABS(temp_v0) < 0x1800) {
            MM_Math_SmoothStepToS(&this->unk_31C, temp_v0, 3, 0x71C, 0);
        } else {
            MM_Math_SmoothStepToS(&this->unk_31C, 0, 3, 0x71C, 0);
        }

        MM_Math_Vec3f_Copy(&sp44, &player->actor.world.pos);
        sp44.y = player->bodyPartsPos[PLAYER_BODYPART_HEAD].y + 3.0f;
        temp_v0 = MM_Math_Vec3f_Pitch(&this->actor.focus.pos, &sp44);
        if (ABS(temp_v0) < 0x800) {
            MM_Math_SmoothStepToS(&this->unk_31A, temp_v0, 3, 0x16C, 0);
        } else {
            MM_Math_SmoothStepToS(&this->unk_31A, 0, 3, 0x16C, 0);
        }
    }
        ...
        do {
            ...
            if (actor != NULL) {
                if (ENTK_GET_F(actor) == 1) {
                    MM_Math_Vec3f_Copy(&this->unk_2EC, &actor->world.pos);
...
void func_80AEDC4C(EnTk* this, PlayState* play) {
    if ((this->actor.params >= 0) &&
        ...
        MM_Message_ContinueTextbox(play, 0x1411);
...
void func_80AEDCBC(EnTk* this, PlayState* play) {
    ...
    SubS_ChangeAnimationBySpeedInfo(&this->skelAnime, sAnimationSpeedInfo, ENTK_ANIM_5, &this->animIndex);
    MM_Math_Vec3f_Copy(&this->actor.world.pos, &this->unk_2EC);
    MM_Math_Vec3f_Copy(&this->actor.prevPos, &this->unk_2EC);
    ...
    switch (this->unk_2B0) {
        case 0:
            ...
            switch (this->unk_310) {
                ...
                case 4:
                    MM_Message_StartTextbox(play, 0x140F, &this->actor);
                    SubS_ChangeAnimationBySpeedInfo(&this->skelAnime, sAnimationSpeedInfo, ENTK_ANIM_2,
                                                    &this->animIndex);
                    break;

                case 3:
                    MM_Message_StartTextbox(play, 0x1410, &this->actor);
                    SubS_ChangeAnimationBySpeedInfo(&this->skelAnime, sAnimationSpeedInfo, ENTK_ANIM_2,
                                                    &this->animIndex);
                    break;
    ...
    switch (MM_Message_GetState(&play->msgCtx)) {
        case TEXT_STATE_NONE:
            switch (this->unk_2E6) {
                ...
                case 0x140B:
                case 0x140D:
                    MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer - 0x1555, 1, 0x71C, 0);
                    this->actor.world.rot.y = this->actor.shape.rot.y;
                    if (!MM_Math_SmoothStepToS(&this->unk_31A, 0, 3, 0x16C, 10) &&
                        !MM_Math_SmoothStepToS(&this->unk_31C, 0, 3, 0x71C, 10) &&
                        ...
                        MM_Message_StartTextbox(play, this->unk_2E6, &this->actor);
                    }
                    break;
        ...
        case TEXT_STATE_DONE:
            if (MM_Message_ShouldAdvance(play)) {
                switch (play->msgCtx.currentTextId) {
                    ...
                    case 0x1405:
                        MM_Message_ContinueTextbox(play, 0x1406);
                        break;

                    case 0x1406:
                        MM_Message_ContinueTextbox(play, 0x1407);
                        break;

                    case 0x1407:
                        if (play->msgCtx.choiceIndex == 0) {
                            Audio_PlaySfx_MessageDecide();
                            MM_Message_ContinueTextbox(play, 0x1409);
                        } else {
                            Audio_PlaySfx_MessageCancel();
                            MM_Message_ContinueTextbox(play, 0x1408);
                        }
                        break;
                    ...
                    case 0x1409:
                        MM_Message_ContinueTextbox(play, 0x140A);
                        break;
                    ...
                    case 0x140D:
                        ...
                        if (play->msgCtx.choiceIndex == 0) {
                            ...
                            Audio_PlaySfx_MessageCancel();
                            MM_Message_ContinueTextbox(play, 0x140E);
                        }
                        break;
...
s32 func_80AEE300(PlayState* play, Actor* arg1, Actor* arg2, void* _arg3) {
    ...
    if ((arg2 != arg1) && (ENTK_GET_F(arg2) == 3)) {
        temp_f0 = MM_Actor_WorldDistXYZToActor(arg1, arg2);
...
void func_80AEE374(EnTk* this, PlayState* play) {
    ...
    this->unk_2CC = MM_Actor_WorldYawTowardPoint(&this->actor, &sp30.unk_00->world.pos);
    this->actionFunc = func_80AEE414;
}

void func_80AEE414(EnTk* this, PlayState* play) {
    MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->unk_2CC, 2, 0xE38, 0x5B);
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void func_80AEE4D0(EnTk* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 33.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EV_DIG_UP);
    }

    if (!(this->unk_2CA & 0x20)) {
        if (MM_Animation_OnFrame(&this->skelAnime, 37.0f)) {
            bigPoe = NULL;
            do {
                bigPoe = SubS_FindActor(play, bigPoe, ACTORCAT_PROP, ACTOR_EN_BIGPO);

                if (bigPoe != NULL) {
                    if ((bigPoe->params == 3) && (MM_Actor_WorldDistXYZToActor(&this->actor, bigPoe) < 80.0f)) {
                        bigPoe->params = 4;
                        this->unk_2CA |= 0x20;
                        this->unk_2E4++;
                    }
...
void func_80AEE650(EnTk* this, PlayState* play) {
    if (this->unk_36C < ARRAY_COUNT(this->unk_324)) {
        MM_Math_Vec3f_Copy(&this->unk_324[this->unk_36C], &this->actor.world.pos);
        this->unk_36C++;
    }
...
s32 func_80AEE7E0(Vec3f* arg0, f32 arg1, Vec3f* arg2, s32 arg3) {
    ...
    for (i = 0; i < arg3; i++) {
        if (MM_Math_Vec3f_DistXZ(arg0, &arg2[i]) < arg1) {
            ret = false;
            break;
        }
    }
...
s32 func_80AEE86C(EnTk* this, PlayState* play) {
    ...
    if ((MM_BgCheck_EntityRaycastFloor3(&play->colCtx, &groundPoly, &bgId, &sp28) != BGCHECK_Y_MIN) &&
        ...
        (((this->unk_2CA & 2) && (MM_Math_Vec3f_DistXZ(&this->unk_300, &sp28) >= 100.0f)) || !(this->unk_2CA & 2)) &&
        !MM_Play_InCsMode(play)) {
        MM_Math_Vec3f_Copy(&this->unk_300, &sp28);
        ret = true;
    }
    return ret;
}
...
void func_80AEE9B0(EnTk* this, PlayState* play) {
    ...
    func_80AEEAD4(this, play);
    if (MM_Math_Vec3f_DistXZ(&this->actor.world.pos, &this->unk_300) >= 100.0f) {
        this->unk_2CA &= ~2;
    }
...
void func_80AEEB88(EnTk* this, PlayState* play) {
    ...
    for (i = 0; i < 8; i++) {
        sp68.x = MM_Math_SinS(this->actor.world.rot.y + (i * 0x2000)) * 80.0f;
        sp68.z = MM_Math_CosS(this->actor.world.rot.y + (i * 0x2000)) * 80.0f;
        ...
        temp =
            MM_BgCheck_EntityRaycastFloor3(&play->colCtx, &this->actor.floorPoly, &bgId, &sp68) - this->actor.world.pos.y;
        if (temp <= -80.0f) {
            break;
        }
    }
    ...
    if (i == 8) {
        this->unk_2CC = BINANG_ROT180((MM_Rand_Centered() * 0x8000) + this->actor.yawTowardsPlayer);
    } else {
...
void func_80AEED38(EnTk* this, PlayState* play) {
    ...
    MM_Math_Vec3f_Copy(&sp58, &this->actor.world.pos);
    sp58.x += (MM_Math_SinS(sp56) * 20.0f);
    sp58.y += 30.0f;
    sp58.z += MM_Math_CosS(sp56) * 20.0f;

    if (MM_BgCheck_SphVsFirstWall(&play->colCtx, &sp58, 20.0f)) {
        MM_Math_Vec3f_Copy(&sp48, &this->actor.world.pos);
        sp56 = BINANG_ADD(this->actor.shape.rot.y, 0x4000);
        sp48.x += (MM_Math_SinS(sp56) * 20.0f);
        sp48.y += 30.0f;
        sp48.z += MM_Math_CosS(sp56) * 20.0f;

        MM_Math_Vec3f_Copy(&sp3C, &this->actor.world.pos);
        sp56 = BINANG_SUB(this->actor.shape.rot.y, 0x4000);
        sp3C.x += (MM_Math_SinS(sp56) * 20.0f);
        sp3C.y += 30.0f;
        sp3C.z += MM_Math_CosS(sp56) * 20.0f;

        if (MM_BgCheck_SphVsFirstWall(&play->colCtx, &sp48, 20.0f)) {
            if (MM_BgCheck_SphVsFirstWall(&play->colCtx, &sp3C, 20.0f)) {
    ...
    if (this->actor.speed > 0.5f) {
        MM_Math_SmoothStepToS(&this->actor.world.rot.y, this->unk_2CC, 2, 0xAAA, 1);
        this->actor.shape.rot.y = this->actor.world.rot.y;
    }

    if ((MM_Message_GetState(&play->msgCtx) == TEXT_STATE_NONE) && !MM_Play_InCsMode(play) && (this->unk_2C6-- <= 0)) {
        MM_Message_StartTextbox(play, 0x140C, NULL);
        this->unk_2CA |= 0x4000;
        this->unk_2C6 = 200;
    }
}
...
void func_80AEF094(EnTk* this, PlayState* play) {
    ...
    if (this->actor.speed >= 0.5f) {
        MM_Math_SmoothStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 2, 0x38E, 1);
        this->actor.shape.rot.y = this->actor.world.rot.y;
    }
}
...
void func_80AEF278(EnTk* this, PlayState* play) {
    MM_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer - 0x1555, 3, 0x1C7, 0);
    this->actor.world.rot.y = this->actor.shape.rot.y;
}
...
void func_80AEF2D8(Actor* thisx, PlayState* play) {
    ...
    if ((this->actor.draw != NULL) && ((this->animIndex == ENTK_ANIM_0) || (this->animIndex == ENTK_ANIM_1)) &&
        (MM_Animation_OnFrame(&this->skelAnime, 0.0f) || MM_Animation_OnFrame(&this->skelAnime, 24.0f))) {
    ...
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 10.0f, 0.0f, UPDBGCHECKINFO_FLAG_4);
    func_80AEC460(this);
}
...
void MM_EnTk_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.floorBgId != BGCHECK_SCENE) {
        BgDanpeiMovebg* platform = (BgDanpeiMovebg*)MM_DynaPoly_GetActor(&play->colCtx, this->actor.floorBgId);
    ...
    Actor_MoveWithGravity(&this->actor);
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 10.0f, 0.0f, UPDBGCHECKINFO_FLAG_1 | UPDBGCHECKINFO_FLAG_4);
...
void func_80AEF5F4(Actor* thisx, PlayState* play) {
    ...
    this->unk_316 += 0x46C8;
    this->unk_318 = MM_Math_SinS(this->unk_316) * 900.0f;
    this->actionFunc(this, play);
}

s32 MM_EnTk_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx) {
    EnTk* this = (EnTk*)thisx;
...
void MM_EnTk_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx) {
    ...
    if (this->unk_2B0 != 2) {
        switch (limbIndex) {
            ...
            case OBJECT_TK_LIMB_0E:
                MM_Matrix_MultVec3f(&D_80AEFA84, &this->unk_2B4);

                OPEN_DISPS(play->state.gfxCtx);

                MM_gSPDisplayList(POLY_OPA_DISP++, object_tk_DL_00B530);
...
void MM_EnTk_Draw(Actor* thisx, PlayState* play) {
    ...
    MM_gSPSegment(POLY_OPA_DISP++, 0x08, Lib_SegmentedToVirtual(D_80AEFA90[this->unk_2C2]));

    Matrix_RotateYS(this->unk_318, MTXMODE_APPLY);
    MM_SkelAnime_DrawFlexOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                          MM_EnTk_OverrideLimbDraw, MM_EnTk_PostLimbDraw, &this->actor);
