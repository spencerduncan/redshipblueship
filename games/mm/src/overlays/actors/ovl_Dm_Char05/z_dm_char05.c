...
static AnimationInfo MM_sAnimationInfo[DMCHAR05_ANIM_MAX] = {
...
void func_80AAC63C(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_Init(play, &this->skelAnime, &object_dmask_Skel_010B0, NULL, NULL, NULL, 0);
    if (DMCHAR05_GET(&this->actor) == DMCHAR05_0) {
        DmChar05_ChangeAnim(&this->skelAnime, &MM_sAnimationInfo[DMCHAR05_ANIM_0], 0);
    } else {
        DmChar05_ChangeAnim(&this->skelAnime, &MM_sAnimationInfo[DMCHAR05_ANIM_1], 0);
    }
...
void func_80AAC6E4(Actor* thisx, PlayState* play) {
    ...
    MM_ActorShape_Init(&this->actor.shape, 0.0f, MM_ActorShadow_DrawCircle, 24.0f);
    MM_SkelAnime_Init(play, &this->skelAnime, &object_dmask_Skel_042B0, NULL, NULL, NULL, 0);
    DmChar05_ChangeAnim(&this->skelAnime, &MM_sAnimationInfo[DMCHAR05_ANIM_1], 0);
    this->actionFunc = func_80AACC48;
}
...
void func_80AAC770(Actor* thisx, PlayState* play) {
    ...
    MM_ActorShape_Init(&this->actor.shape, 0.0f, MM_ActorShadow_DrawCircle, 24.0f);
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &object_dmask_Skel_001D0, NULL, NULL, NULL, 0);
    DmChar05_ChangeAnim(&this->skelAnime, &MM_sAnimationInfo[DMCHAR05_ANIM_3], 0);
    this->actionFunc = func_80AACC48;
}
...
void func_80AAC7FC(Actor* thisx, PlayState* play) {
    ...
    MM_ActorShape_Init(&this->actor.shape, 0.0f, MM_ActorShadow_DrawCircle, 24.0f);
    MM_SkelAnime_Init(play, &this->skelAnime, &object_dmask_Skel_013D0, NULL, NULL, NULL, 0);
    DmChar05_ChangeAnim(&this->skelAnime, &MM_sAnimationInfo[DMCHAR05_ANIM_4], 0);
    this->actionFunc = func_80AACC48;
}
        ...
        if (play->csCtx.actorCues[cueChannel]->id == 2) {
            if (play->csCtx.curFrame == play->csCtx.actorCues[cueChannel]->startFrame) {
                if (GameInteractor_Should(VB_GIVE_ITEM_FROM_DMCHAR05, true, ITEM_MASK_COUPLE)) {
                    MM_Item_Give(play, ITEM_MASK_COUPLE);
                }
            }
            this->unk_18E = 1;
            this->unk_190.x = play->csCtx.actorCues[cueChannel]->startPos.x;
...
void func_80AACBE4(DmChar05* this, PlayState* play) {
    ...
    s32 objectSlot = Object_GetSlot(&play->objectCtx, MM_sObjectIds[params]);
        ...
        case DMCHAR05_0:
            if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_109)) {
                ...
                if (play->csCtx.curFrame == play->csCtx.actorCues[cueChannel]->startFrame) {
                    switch (play->csCtx.actorCues[cueChannel]->id) {
                        ...
                        case 4:
                            if (GameInteractor_Should(VB_GIVE_ITEM_FROM_DMCHAR05, true, ITEM_MASK_GORON)) {
                                MM_Item_Give(play, ITEM_MASK_GORON);
                            }
                            changeAnim = false;
                    ...
                    if (changeAnim) {
                        DmChar05_ChangeAnim(&this->skelAnime, &MM_sAnimationInfo[this->animIndex], 0);
                    }
                }
                    ...
                    switch (play->csCtx.actorCues[cueChannel]->id) {
                        ...
                        case 4:
                            if (GameInteractor_Should(VB_GIVE_ITEM_FROM_DMCHAR05, true, ITEM_MASK_ZORA)) {
                                MM_Item_Give(play, ITEM_MASK_ZORA);
                            }
                            changeAnim = false;
                    ...
                    switch (play->csCtx.actorCues[cueChannel]->id) {
                        ...
                        case 4:
                            if (GameInteractor_Should(VB_GIVE_ITEM_FROM_DMCHAR05, true, ITEM_MASK_GIBDO)) {
                                MM_Item_Give(play, ITEM_MASK_GIBDO);
                            }
                            changeAnim = false;
...
void func_80AAD4A8(DmChar05* this, PlayState* play) {
    if (DMCHAR05_GET(&this->actor) == DMCHAR05_0) {
        if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_109) &&
            ...
            if (MM_Animation_OnFrame(&this->skelAnime, 14.0f) || MM_Animation_OnFrame(&this->skelAnime, 15.0f)) {
                Actor_PlaySfx(&this->actor, NA_SE_IT_MASK_BOUND_0);
            } else if (MM_Animation_OnFrame(&this->skelAnime, 19.0f)) {
...
void func_80AAD998(Actor* thisx, PlayState* play) {
    ...
    if (this->unk_18E == 0) {
        if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_109) &&
            ...
            POLY_OPA_DISP = MM_Gfx_SetFog(POLY_OPA_DISP, this->unk_19C, this->unk_1A0, this->unk_1A4, this->unk_1A8,
                                       this->unk_1AC, this->unk_1B0);
            MM_SkelAnime_DrawOpa(play, this->skelAnime.skeleton, this->skelAnime.jointTable, DmChar05_OverrideLimbDraw,
...
void func_80AADC00(Actor* thisx, PlayState* play) {
    ...
    if (Cutscene_IsCueInChannel(play, CS_CMD_ACTOR_CUE_559)) {
        ...
        if (play->csCtx.actorCues[cueChannel]->id == 4) {
            MM_Matrix_Translate(-600.0f, 0.0f, 0.0f, MTXMODE_APPLY);
            MM_Gfx_DrawDListOpa(play, object_dmask_DL_001E70);
        }
    }
}
...
void func_80AADD9C(PlayState* play, DmChar05* this) {
    if (this->actor.objectSlot == this->objectSlot) {
        MM_Matrix_Translate(this->unk_190.x, this->unk_190.y, this->unk_190.z, MTXMODE_NEW);
        ...
        if (GameInteractor_Should(VB_DRAW_ITEM_FROM_DMCHAR05, true, GID_MASK_GORON)) {
            MM_GetItem_Draw(play, GID_MASK_GORON);
        }
    }
...
void func_80AADE78(PlayState* play, DmChar05* this) {
    if (this->actor.objectSlot == this->objectSlot) {
        ...
        if (GameInteractor_Should(VB_DRAW_ITEM_FROM_DMCHAR05, true, GID_MASK_ZORA)) {
            MM_GetItem_Draw(play, GID_MASK_ZORA);
        }
    }
...
void func_80AADF54(PlayState* play, DmChar05* this) {
    if (this->actor.objectSlot == this->objectSlot) {
        ...
        if (GameInteractor_Should(VB_DRAW_ITEM_FROM_DMCHAR05, true, GID_MASK_GIBDO)) {
            MM_GetItem_Draw(play, GID_MASK_GIBDO);
        }
    }
    ...
    if (this->unk_18E != 0) {
        if (this->actor.objectSlot == this->objectSlot) {
            ...
            if (GameInteractor_Should(VB_DRAW_ITEM_FROM_DMCHAR05, true, GID_MASK_COUPLE)) {
                MM_GetItem_Draw(play, GID_MASK_COUPLE);
            }
        }
...
void func_80AAE114(PlayState* play, DmChar05* this) {
    ...
    if (this->actor.objectSlot == this->objectSlot) {
        ...
        MM_Matrix_RotateZYX(0, play->gameplayFrames * 1000, 0, MTXMODE_APPLY);
        MM_Matrix_Scale(0.2f, 0.2f, 0.2f, MTXMODE_APPLY);
        MM_GetItem_Draw(play, getItemDrawIds[getItemDrawIndex]);
    }
