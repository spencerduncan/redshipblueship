...
void MM_EnHonotrap_Init(Actor* thisx, PlayState* play);
void MM_EnHonotrap_Destroy(Actor* thisx, PlayState* play);
void MM_EnHonotrap_Update(Actor* thisx, PlayState* play);
void MM_EnHonotrap_Draw(Actor* thisx, PlayState* play);

void MM_EnHonotrap_GetNormal(Vec3f* normal, Vec3f* vec);
void MM_EnHonotrap_InitEye(EnHonotrap* this, PlayState* play);
void MM_EnHonotrap_InitFlame(EnHonotrap* this, PlayState* play);
void EnHonotrap_InitFlameGroup(EnHonotrap* this, PlayState* play);
void MM_EnHonotrap_SetupEyeIdle(EnHonotrap* this);
void MM_EnHonotrap_EyeIdle(EnHonotrap* this, PlayState* play);
void MM_EnHonotrap_SetupEyeOpen(EnHonotrap* this);
void MM_EnHonotrap_EyeOpen(EnHonotrap* this, PlayState* play);
void MM_EnHonotrap_SetupEyeAttack(EnHonotrap* this);
void MM_EnHonotrap_EyeAttack(EnHonotrap* this, PlayState* play);
void MM_EnHonotrap_SetupEyeClose(EnHonotrap* this);
void MM_EnHonotrap_EyeClose(EnHonotrap* this, PlayState* play);
...
void MM_EnHonotrap_SetupFlameDrop(EnHonotrap* this);
void MM_EnHonotrap_FlameDrop(EnHonotrap* this, PlayState* play);
void MM_EnHonotrap_SetupFlameMove(EnHonotrap* this);
void MM_EnHonotrap_FlameMove(EnHonotrap* this, PlayState* play);
void MM_EnHonotrap_SetupFlameChase(EnHonotrap* this);
void MM_EnHonotrap_FlameChase(EnHonotrap* this, PlayState* play);
void MM_EnHonotrap_SetupFlameVanish(EnHonotrap* this);
void MM_EnHonotrap_FlameVanish(EnHonotrap* this, PlayState* play);
...
void MM_EnHonotrap_DrawFlame(Actor* thisx, PlayState* play);
...
ActorProfile En_Honotrap_Profile = {
    ...
    /**/ sizeof(EnHonotrap),
    /**/ MM_EnHonotrap_Init,
    /**/ MM_EnHonotrap_Destroy,
    /**/ MM_EnHonotrap_Update,
    /**/ MM_EnHonotrap_Draw,
};
...
static CollisionCheckInfoInit MM_sColChkInfoInit = { 0, 9, 23, 1 };
...
void MM_EnHonotrap_FlameCollisionCheck(EnHonotrap* this, PlayState* play) {
    s32 pad[3];

    MM_Collider_UpdateCylinder(&this->actor, &this->collider.cyl);
    MM_CollisionCheck_SetAT(play, &play->colChkCtx, &this->collider.tris.base);
    MM_CollisionCheck_SetAC(play, &play->colChkCtx, &this->collider.tris.base);
    MM_CollisionCheck_SetOC(play, &play->colChkCtx, &this->collider.tris.base);
...
void MM_EnHonotrap_InitEye(EnHonotrap* this, PlayState* play) {
    ...
    s32 j;
    s32 k;
    f32 sin = MM_Math_SinS(this->actor.home.rot.y);
    f32 cos = MM_Math_CosS(this->actor.home.rot.y);
    Vec3f triangle[3];
    Vec3f* vtx;
    ...
    MM_Collider_InitTris(play, &this->collider.tris);
    MM_Collider_SetTris(play, &this->collider.tris, &this->actor, &MM_sTrisInit, this->collider.elements);

    for (i = 0; i < 2; i++) {
        for (j = 0, vtx = triangle; j < 3; j++, vtx++) {
            Vec3f* baseVtx = &MM_sTrisInit.elements[i].dim.vtx[j];

            vtx->x = (baseVtx->z * sin) + (baseVtx->x * cos);
            vtx->y = baseVtx->y;
            vtx->z = (baseVtx->z * cos) - (baseVtx->x * sin);

            MM_Math_Vec3f_Sum(vtx, &this->actor.world.pos, vtx);
        }
        MM_Collider_SetTrisVertices(&this->collider.tris, i, &triangle[0], &triangle[1], &triangle[2]);
    }
    MM_Actor_SetFocus(&this->actor, 0.0f);
    if (this->actor.params == HONOTRAP_TYPE_EYE) {
        MM_EnHonotrap_SetupEyeIdle(this);
...
void MM_EnHonotrap_InitFlame(EnHonotrap* this, PlayState* play) {
    ...
    MM_Actor_SetScale(&this->actor, 0.0001f);
    MM_Collider_InitCylinder(play, &this->collider.cyl);
    MM_Collider_SetCylinder(play, &this->collider.cyl, &this->actor, &MM_sCylinderInit);
    MM_Collider_UpdateCylinder(&this->actor, &this->collider.cyl);
    ...
    this->flameScroll = MM_Rand_ZeroOne() * 511.0f;
...
void MM_EnHonotrap_Init(Actor* thisx, PlayState* play) {
    ...
    if ((params == HONOTRAP_TYPE_EYE) || (params == HONOTRAP_TYPE_EYE_MUTI_FLAME)) {
        MM_EnHonotrap_InitEye(this, play);
        ...
        MM_EnHonotrap_InitFlame(this, play);
        this->actor.update = EnHonotrap_UpdateFlame;
        this->actor.draw = MM_EnHonotrap_DrawFlame;
    }
}
...
void MM_EnHonotrap_Destroy(Actor* thisx, PlayState* play) {
    ...
    params = this->actor.params;
    if ((params == HONOTRAP_TYPE_EYE) || (params == HONOTRAP_TYPE_EYE_MUTI_FLAME)) {
        MM_Collider_DestroyTris(play, &this->collider.tris);
    } else {
        MM_Collider_DestroyCylinder(play, &this->collider.cyl);
    }
}

void MM_EnHonotrap_SetupEyeIdle(EnHonotrap* this) {
    this->actionFunc = MM_EnHonotrap_EyeIdle;
    ...
    if (this->actor.child != NULL) {
        ...
        if ((this->actor.playerHeightRel < 0.0f) && (this->actor.playerHeightRel > -700.0f)) {
            ...
            if ((angle > -0x4000) && (angle < 0x4000)) {
                MM_EnHonotrap_SetupEyeOpen(this);
            }
        }
    }
}
...
void MM_EnHonotrap_SetupEyeOpen(EnHonotrap* this) {
    this->actionFunc = MM_EnHonotrap_EyeOpen;
...
void MM_EnHonotrap_EyeOpen(EnHonotrap* this, PlayState* play) {
    ...
    if (this->eyeState <= HONOTRAP_EYE_OPEN) {
        MM_EnHonotrap_SetupEyeAttack(this);
        MM_Actor_SpawnAsChild(
            &play->actorCtx, &this->actor, play, ACTOR_EN_HONOTRAP,
            (MM_Math_SinS(this->actor.shape.rot.y) * 12.0f) + this->actor.home.pos.x, this->actor.home.pos.y - 10.0f,
            (MM_Math_CosS(this->actor.shape.rot.y) * 12.0f) + this->actor.home.pos.z, this->actor.home.rot.x,
            this->actor.home.rot.y, this->actor.home.rot.z, HONOTRAP_TYPE_FLAME_MOVE);
    }
}

void MM_EnHonotrap_SetupEyeAttack(EnHonotrap* this) {
    this->actionFunc = MM_EnHonotrap_EyeAttack;
    this->eyeState = HONOTRAP_EYE_OPEN;
}

void MM_EnHonotrap_EyeAttack(EnHonotrap* this, PlayState* play) {
    if (this->timer <= 0) {
        MM_EnHonotrap_SetupEyeClose(this);
    }
}

void MM_EnHonotrap_SetupEyeClose(EnHonotrap* this) {
    this->actionFunc = MM_EnHonotrap_EyeClose;
}
...
void EnHonotrap_EyeOpen2(EnHonotrap* this, PlayState* play) {
    ...
    if (this->eyeState <= HONOTRAP_EYE_OPEN) {
        MM_Actor_SpawnAsChild(
            &play->actorCtx, &this->actor, play, ACTOR_EN_HONOTRAP,
            (MM_Math_SinS(this->actor.shape.rot.y) * 12.0f) + this->actor.home.pos.x, this->actor.home.pos.y,
            (MM_Math_CosS(this->actor.shape.rot.y) * 12.0f) + this->actor.home.pos.z, this->actor.home.rot.x,
...
void EnHonotrap_FlameGrow(EnHonotrap* this, PlayState* play) {
    ...
    s32 targetReached = MM_Math_StepToF(&this->actor.scale.x, targetScale, 0.0006f);

    this->actor.scale.z = this->actor.scale.y = this->actor.scale.x;
    if (targetReached) {
        if (this->actor.params == HONOTRAP_TYPE_FLAME_MOVE) {
            MM_EnHonotrap_SetupFlameMove(this);
        } else { // HONOTRAP_TYPE_FLAME_DROP
            MM_EnHonotrap_SetupFlameDrop(this);
        }
    }
}
...
void MM_EnHonotrap_SetupFlameDrop(EnHonotrap* this) {
    ...
    this->actor.velocity.x = 2.0f * MM_Math_SinS(this->actor.world.rot.y);
    this->actor.velocity.z = 2.0f * MM_Math_CosS(this->actor.world.rot.y);
    this->actionFunc = MM_EnHonotrap_FlameDrop;
}
...
void MM_EnHonotrap_FlameDrop(EnHonotrap* this, PlayState* play) {
    if ((this->collider.tris.base.atFlags & AT_HIT) || (this->timer <= 0)) {
        ...
        MM_EnHonotrap_SetupFlameVanish(this);
        return;
    }
    if (this->actor.velocity.y > 0.0f) {
        this->actor.world.pos.x += this->actor.velocity.x;
        this->actor.world.pos.z += this->actor.velocity.z;
        MM_Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 12.0f, 0.0f, UPDBGCHECKINFO_FLAG_1 | UPDBGCHECKINFO_FLAG_4);
    }
    if (!MM_Math_StepToF(&this->actor.world.pos.y, this->actor.floorHeight + 1.0f, this->actor.velocity.y)) {
        this->actor.velocity.y += 1.0f;
    } else {
        this->actor.velocity.y = 0.0f;
    }
    MM_EnHonotrap_FlameCollisionCheck(this, play);
}

void MM_EnHonotrap_SetupFlameMove(EnHonotrap* this) {
    f32 distInverse;

    this->actionFunc = MM_EnHonotrap_FlameMove;
    distInverse = 1.0f / (MM_Actor_WorldDistXYZToPoint(&this->actor, &this->targetPos) + 1.0f);
...
void MM_EnHonotrap_FlameMove(EnHonotrap* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->speedMod, 13.0f, 0.5f);
    ...
    cond = true;
    cond &= MM_Math_StepToF(&thisx->world.pos.x, this->targetPos.x, speed.x);
    cond &= MM_Math_StepToF(&thisx->world.pos.y, this->targetPos.y, speed.y);
    cond &= MM_Math_StepToF(&thisx->world.pos.z, this->targetPos.z, speed.z);
    MM_Actor_UpdateBgCheckInfo(play, thisx, 10.0f, 10.0f, 0.0f,
    ...
    if (this->collider.tris.base.atFlags & AT_BOUNCED) {
        ...
        MM_EnHonotrap_GetNormal(&shieldNorm, &shieldVec);
        tempVel = thisx->velocity;
        MM_Math3D_Vec3fReflect(&tempVel, &shieldNorm, &thisx->velocity);
        ...
        MM_EnHonotrap_FlameCollisionCheck(this, play);
        if (cond != 0) {
            MM_EnHonotrap_SetupFlameChase(this);
        }
    }
}

void MM_EnHonotrap_SetupFlameChase(EnHonotrap* this) {
    this->actionFunc = MM_EnHonotrap_FlameChase;
...
void MM_EnHonotrap_FlameChase(EnHonotrap* this, PlayState* play) {
    ...
    MM_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 0x300);
    MM_Math_StepToF(&this->actor.speed, 3.0f, 0.1f);
    ...
    MM_Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 10.0f, 0.0f,
                            UPDBGCHECKINFO_FLAG_1 | UPDBGCHECKINFO_FLAG_4 | UPDBGCHECKINFO_FLAG_8 |
                                UPDBGCHECKINFO_FLAG_10);
...
void MM_EnHonotrap_SetupFlameVanish(EnHonotrap* this) {
    this->actionFunc = MM_EnHonotrap_FlameVanish;
}

void MM_EnHonotrap_FlameVanish(EnHonotrap* this, PlayState* play) {
    s32 pad;
    s32 targetReached = MM_Math_StepToF(&this->actor.scale.x, 0.0001f, 0.00015f);
...
void EnHonotrap_FlameGroup(EnHonotrap* this, PlayState* play) {
    ...
    sp80 = fabsf(MM_Math_CosS(MM_Camera_GetCamDirPitch(GET_ACTIVE_CAM(play))));
    flameScrollDisplacement = (s32)(sp80 * -10.5f) - 10;
    MM_Math_StepToF(&flameGroup->unk0, 1.0f, 0.05f);
    if (this->timer <= 40) {
        sp78 = MM_Math_StepToF(&flameGroup->unk4, 1.0f, 0.05f);
    ...
    sp88 = MM_Math_SinS(this->actor.shape.rot.y) * 120.0f;
    sp84 = MM_Math_CosS(this->actor.shape.rot.y) * 120.0f;
    ...
    for (i = 0; i < ARRAY_COUNT(flameGroup->flameList) && (var_fs0 <= flameGroup->unk0); i++) {
        ...
        flameElem->unkC = MM_Math_SinS(TRUNCF_BINANG(var_fs0 * 25486.223f)) * 1.6f;
    ...
    MM_Actor_WorldToActorCoords(&this->actor, &sp68, &GET_PLAYER(play)->actor.world.pos);
    ...
    var_fs0_2 = MM_Math_SinS(TRUNCF_BINANG(sp68.z * 212.3852f)) * 1.6f;
    if (var_fs0_2 > 1.0f) {
        var_fs0_2 = 1.0f;
    }
    ...
    if (sp80 > 0.2f) {
        this->collider.cyl.dim.pos.x =
            TRUNCF_BINANG((MM_Math_SinS(this->actor.shape.rot.y) * sp68.z) + this->actor.world.pos.x);
        this->collider.cyl.dim.pos.y = TRUNCF_BINANG(this->actor.world.pos.y - (24.0f * var_fs0_2));
        this->collider.cyl.dim.pos.z =
            TRUNCF_BINANG((MM_Math_CosS(this->actor.shape.rot.y) * sp68.z) + this->actor.world.pos.z);
...
void MM_EnHonotrap_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->collider.tris.base.acFlags & AC_HIT) {
        MM_EffectSsBomb2_SpawnLayered(play, &this->actor.world.pos, &MM_sVelocity, &MM_sAccel, 15, 8);
        MM_Actor_Kill(&this->actor);
        return;
    }
    if (this->eyeState < HONOTRAP_EYE_SHUT) {
...
void EnHonotrap_UpdateFlame(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params != HONOTRAP_TYPE_FLAME_GROUP) {
        this->actor.shape.yOffset = MM_Math_SinS(this->bobPhase) * 1000.0f + 600.0f;
    }
...
void EnHonotrap_DrawFlameGroup(Actor* thisx, PlayState* play) {
    ...
    gDPSetPrimColor(POLY_XLU_DISP++, 0x80, 0x80, 255, 200, 0, 255);
    gDPSetEnvColor(POLY_XLU_DISP++, 255, 0, 0, 0);
    camDir = MM_Camera_GetCamDir(GET_ACTIVE_CAM(play));
    camDir.y += 0x8000;
    ...
    for (i = 0; i < ARRAY_COUNT(flameGroup->flameList); i++) {
        ...
        if (flameElem->isDrawn) {
            ...
            MM_Matrix_Scale(((fabsf(MM_Math_SinS((s16)(camDir.y - thisx->shape.rot.y) >> 1)) * 0.2f) + 1.7f) *
                             flameElem->unkC,
