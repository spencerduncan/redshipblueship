...
void MM_EnFloormas_Init(Actor* thisx, PlayState* play2);
void MM_EnFloormas_Destroy(Actor* thisx, PlayState* play);
void MM_EnFloormas_Update(Actor* thisx, PlayState* play);
void MM_EnFloormas_Draw(Actor* thisx, PlayState* play);
...
ActorProfile En_Floormas_Profile = {
    ...
    /**/ sizeof(EnFloormas),
    /**/ MM_EnFloormas_Init,
    /**/ MM_EnFloormas_Destroy,
    /**/ MM_EnFloormas_Update,
    /**/ MM_EnFloormas_Draw,
};
...
void func_808D0B08(EnFloormas* this) {
    MM_Animation_PlayOnce(&this->skelAnime, &gWallmasterIdleAnim);
...
void func_808D0B50(EnFloormas* this, PlayState* play) {
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if ((this->actor.xzDistToPlayer < 400.0f) && !MM_Actor_IsFacingPlayer(&this->actor, 0x4000)) {
            this->unk_190 = this->actor.yawTowardsPlayer;
            func_808D108C(this);
        } else if ((this->actor.xzDistToPlayer < 280.0f) && MM_Actor_IsFacingPlayer(&this->actor, 0x2000)) {
...
void func_808D0C14(EnFloormas* this) {
    MM_Animation_MorphToPlayOnce(&this->skelAnime, &gWallmasterStandUpAnim, -3.0f);
...
void func_808D0CE4(EnFloormas* this) {
    if (this->actionFunc != func_808D0F80) {
        MM_Animation_PlayLoopSetSpeed(&this->skelAnime, &gWallmasterWalkAnim, 1.5f);
    } else {
        this->skelAnime.playSpeed = 1.5f;
    }

    this->unk_18E = MM_Rand_S16Offset(2, 4);
...
void func_808D0D70(EnFloormas* this, PlayState* play) {
    ...
    if (onAnimFirstFrame || MM_Animation_OnFrame(&this->skelAnime, 12.0f) || MM_Animation_OnFrame(&this->skelAnime, 24.0f) ||
        MM_Animation_OnFrame(&this->skelAnime, 36.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_FALL_WALK);
    }

    if ((this->actor.xzDistToPlayer < 320.0f) && MM_Actor_IsFacingPlayer(&this->actor, 0x4000)) {
        func_808D0F50(this);
    } else if (this->actor.bgCheckFlags & BGCHECKFLAG_WALL) {
        this->unk_190 = this->actor.wallYaw;
        func_808D108C(this);
    } else if ((this->actor.xzDistToPlayer < 400.0f) && !MM_Actor_IsFacingPlayer(&this->actor, 0x4000)) {
        this->unk_190 = this->actor.yawTowardsPlayer;
        func_808D108C(this);
...
void func_808D0F80(EnFloormas* this, PlayState* play) {
    ...
    if ((this->actor.xzDistToPlayer < 280.0f) && MM_Actor_IsFacingPlayer(&this->actor, 0x2000) &&
...
void func_808D108C(EnFloormas* this) {
    ...
    this->actor.speed = 0.0f;
    if (sp36 > 0) {
        MM_Animation_MorphToPlayOnce(&this->skelAnime, &gFloormasterTurnAnim, -3.0f);
    } else {
        MM_Animation_Change(&this->skelAnime, &gFloormasterTurnAnim, -1.0f, MM_Animation_GetLastFrame(&gFloormasterTurnAnim),
...
void func_808D11BC(EnFloormas* this, PlayState* play) {
    ...
    if (((this->skelAnime.playSpeed > 0.0f) && MM_Animation_OnFrame(&this->skelAnime, 21.0f)) ||
        ((this->skelAnime.playSpeed < 0.0f) && MM_Animation_OnFrame(&this->skelAnime, 6.0f))) {
        if (this->actor.scale.x > 0.009f) {
            Actor_PlaySfx(&this->actor, NA_SE_EN_FALL_WALK);
        } else {
    ...
    if ((curFrame >= 7.0f) && (curFrame < 22.0f)) {
        sp30 = MM_Math_SinS(this->actor.shape.rot.y + 0x4268);
        sp2C = MM_Math_CosS(this->actor.shape.rot.y + 0x4268);
        this->actor.shape.rot.y += this->unk_190;
        this->actor.world.pos.x -= this->actor.scale.x * 2700.0f * (MM_Math_SinS(this->actor.shape.rot.y + 0x4268) - sp30);
        this->actor.world.pos.z -= this->actor.scale.x * 2700.0f * (MM_Math_CosS(this->actor.shape.rot.y + 0x4268) - sp2C);
    }
}

void func_808D1380(EnFloormas* this, PlayState* play) {
    MM_Animation_Change(&this->skelAnime, &gWallmasterHoverAnim, 3.0f, 0.0f, MM_Animation_GetLastFrame(&gWallmasterHoverAnim),
                     ANIMMODE_ONCE, -3.0f);
    this->actor.speed = 0.0f;
...
void func_808D1458(EnFloormas* this, PlayState* play) {
    ...
    MM_Math_ApproachS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 3, 0xAAA);
    MM_Math_StepToS(&this->unk_192, 1200, 100);
}
...
void func_808D14DC(EnFloormas* this, PlayState* play) {
    ...
    sp28.y = 2.0f;
    sp28.x = MM_Math_SinS(this->actor.shape.rot.y + 0x6000) * 7.0f;
    sp28.z = MM_Math_CosS(this->actor.shape.rot.y + 0x6000) * 7.0f;
...
void func_808D1650(EnFloormas* this, PlayState* play) {
    ...
    MM_Math_StepToF(&this->actor.speed, 15.0f, SQ(this->actor.speed) * (1.0f / 3.0f));
    MM_Math_ScaledStepToS(&this->actor.shape.rot.x, -0x1680, 0x140);
...
void func_808D1740(EnFloormas* this) {
    MM_Animation_Change(&this->skelAnime, &gWallmasterJumpAnim, 1.0f, 41.0f, 42.0f, ANIMMODE_ONCE, 5.0f);
...
void func_808D17EC(EnFloormas* this, PlayState* play) {
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        ...
        if ((this->unk_18E == 0) && (sp24 != 0)) {
            if (this->skelAnime.endFrame < 45.0f) {
                this->skelAnime.endFrame = MM_Animation_GetLastFrame(&gWallmasterJumpAnim);
    ...
    MM_Math_ScaledStepToS(&this->actor.shape.rot.x, 0, 0x140);
    MM_Math_StepToS(&this->unk_192, -1600, 100);
}
...
void func_808D19D4(EnFloormas* this) {
    ...
    this->drawDmgEffAlpha = 0.0f;
    MM_Actor_SetScale(&this->actor, 0.004f);
    this->actor.flags |= ACTOR_FLAG_UPDATE_CULLING_DISABLED;
    if (CHECK_FLAG_ALL(this->actor.flags, ACTOR_FLAG_REACT_TO_LENS)) {
        this->actor.draw = func_808D3754;
    } else {
        this->actor.draw = MM_EnFloormas_Draw;
    }
    ...
    MM_Animation_Change(&this->skelAnime, &gWallmasterJumpAnim, 1.0f, 41.0f, MM_Animation_GetLastFrame(&gWallmasterJumpAnim),
                     ANIMMODE_ONCE, 0.0f);
    this->collider.dim.radius = MM_sCylinderInit.dim.radius * 0.6f;
    this->collider.dim.height = MM_sCylinderInit.dim.height * 0.6f;
    this->actor.flags &= ~ACTOR_FLAG_HOOKSHOT_PULLS_PLAYER;
    this->actor.flags |= ACTOR_FLAG_HOOKSHOT_PULLS_ACTOR;
...
void func_808D1BCC(EnFloormas* this) {
    MM_Animation_PlayLoopSetSpeed(&this->skelAnime, &gWallmasterWalkAnim, 4.5f);
    this->actor.speed = 5.0f;
...
void func_808D1C1C(EnFloormas* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 0.0f) || MM_Animation_OnFrame(&this->skelAnime, 18.0f)) {
        Actor_PlaySfx(&this->actor, NA_SE_EN_FLOORMASTER_SM_WALK);
    }
    ...
    if (this->unk_194 == 0) {
        ...
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, BINANG_ROT180(this->actor.yawTowardsPlayer), 0x38E);
    }
}
...
void func_808D1D6C(EnFloormas* this, PlayState* play) {
    ...
    if (flags) {
        ...
        MM_Math_ScaledStepToS(&this->actor.shape.rot.y, MM_Actor_WorldYawTowardActor(&this->actor, sp2C), 0x38E);

        if (MM_Actor_WorldDistXZToActor(&this->actor, sp2C) < 80.0f) {
            func_808D2484(this);
        }
    } else {
        MM_Math_ApproachS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 3, 0x71C);
...
void func_808D1FD4(EnFloormas* this) {
    MM_Animation_Change(&this->skelAnime, &gWallmasterJumpAnim, 2.0f, 0.0f, 41.0f, ANIMMODE_ONCE, 0.0f);
    this->actor.speed = 0.0f;
...
void func_808D2040(EnFloormas* this, PlayState* play) {
    ...
    if (this->skelAnime.curFrame < 20.0f) {
        MM_Math_ApproachS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 2, 0xE38);
    } else if (MM_Animation_OnFrame(&this->skelAnime, 20.0f)) {
        this->actor.speed = 5.0f;
        this->actor.velocity.y = 7.0f;
...
void func_808D217C(EnFloormas* this, Player* player) {
    ...
    MM_Animation_Change(&this->skelAnime, &gWallmasterJumpAnim, 1.0f, 36.0f, 45.0f, ANIMMODE_ONCE, -3.0f);
    this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
    this->actor.speed = 0.0f;
    this->actor.velocity.y = 0.0f;
    func_808D08D0(this);
    ptr = &D_808D3900[GET_PLAYER_FORM];
    this->actor.home.pos.x = ptr->z * MM_Math_SinS(this->actor.shape.rot.y);
    this->actor.home.pos.y = CLAMP(-this->actor.playerHeightRel, ptr->x, ptr->y);
    this->actor.home.pos.z = ptr->z * MM_Math_CosS(this->actor.shape.rot.y);
    MM_Math_Vec3f_Sum(&player->actor.world.pos, &this->actor.home.pos, &this->actor.world.pos);
...
void func_808D24F0(EnFloormas* this, PlayState* play) {
    ...
    if (MM_Animation_OnFrame(&this->skelAnime, 20.0f)) {
        ...
        MM_Math_ApproachS(&this->actor.shape.rot.y, MM_Actor_WorldYawTowardActor(&this->actor, phi_s1), 2, 0xE38);
...
void func_808D2764(EnFloormas* this, PlayState* play) {
    ...
    if (phi_a2 == 1) {
        MM_Math_StepToF(&this->actor.scale.x, 0.007f, 0.0005f);
    } else if (phi_a2 == 0) {
        MM_Math_StepToF(&this->actor.scale.x, 0.01f, 0.0005f);
    }
    ...
    this->collider.dim.radius = MM_sCylinderInit.dim.radius * 100.0f * this->actor.scale.x;
    this->collider.dim.height = MM_sCylinderInit.dim.height * 100.0f * this->actor.scale.x;
    ...
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        if (this->actor.scale.x >= 0.01f) {
            ...
            MM_Animation_PlayOnce(&this->skelAnime, &gFloormasterTapFingerAnim);
            this->unk_18E = 1;
        } else {
            MM_Animation_PlayOnce(&this->skelAnime, &gWallmasterIdleAnim);
            this->unk_18E = 0;
...
void MM_EnFloormas_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc != func_808D2AA8) {
        ...
        if (this->actionFunc != func_808D2AF4) {
            ...
            MM_Actor_UpdateBgCheckInfo(play, &this->actor, 20.0f, this->actor.scale.x * 3000.0f, 0.0f,
            ...
            MM_Actor_SetFocus(&this->actor, this->actor.scale.x * 2500.0f);
            if (this->collider.base.colMaterial == COL_MATERIAL_HARD) {
                if (this->unk_190 != 0) {
                    this->unk_190--;
                }
...
s32 MM_EnFloormas_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, Actor* thisx,
                                Gfx** gfx) {
    EnFloormas* this = (EnFloormas*)thisx;
...
void MM_EnFloormas_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, Actor* thisx, Gfx** gfx) {
    EnFloormas* this = (EnFloormas*)thisx;
...
void MM_EnFloormas_Draw(Actor* thisx, PlayState* play) {
    ...
    POLY_OPA_DISP =
        MM_SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                           MM_EnFloormas_OverrideLimbDraw, MM_EnFloormas_PostLimbDraw, &this->actor, POLY_OPA_DISP);
    if (this->collider.base.colMaterial == COL_MATERIAL_HARD) {
        func_800AE5A0(play);
    }
...
void func_808D3754(Actor* thisx, PlayState* play) {
    ...
    POLY_XLU_DISP =
        MM_SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                           MM_EnFloormas_OverrideLimbDraw, MM_EnFloormas_PostLimbDraw, &this->actor, POLY_XLU_DISP);
    if (this->collider.base.colMaterial == COL_MATERIAL_HARD) {
        func_800AE8EC(play);
    }
