...
void EnKitan_Init(Actor* thisx, PlayState* play) {
    ...
    MM_SkelAnime_InitFlex(play, &this->skelAnime, &gKeatonSkel, &gKeatonIdleAnim, this->jointTable, this->morphTable,
                       KEATON_LIMB_MAX);
    MM_Animation_PlayLoop(&this->skelAnime, &gKeatonIdleAnim);
    ...
    if ((MM_Player_GetMask(play) != PLAYER_MASK_KEATON) ||
        MM_Flags_GetCollectible(play, ENKITAN_GET_COLLECT_FLAG(&this->actor))) {
...
void EnKitan_SpawnEffects(EnKitan* this, PlayState* play, s32 numEffects) {
    ...
    for (i = 0; i < numEffects; i++) {
        ...
        velocity.y = MM_Rand_ZeroFloat(6.0f);
...
u16 EnKitan_GetQuestionMessageId(EnKitan* this) {
    ...
    while (true) {
        s32 rand = MM_Rand_ZeroFloat(30.0f);
...
void EnKitan_TalkAfterGivingPrize(EnKitan* this, PlayState* play) {
    ...
    if (Actor_TalkOfferAccepted(&this->actor, &play->state)) {
        ...
        MM_Message_ContinueTextbox(play, 0x04B5);
        this->actor.flags &= ~ACTOR_FLAG_TALK_OFFER_AUTO_ACCEPTED;
        MM_Animation_MorphToLoop(&this->skelAnime, &gKeatonChuckleAnim, -5.0f);
    } else {
...
void EnKitan_OfferPrize(EnKitan* this, PlayState* play) {
    ...
    if (CHECK_WEEKEVENTREG(WEEKEVENTREG_RECEIVED_KEATON_HEART_PIECE)) {
        MM_Actor_OfferGetItem(&this->actor, play, GI_RUPEE_RED, 2000.0f, 1000.0f);
    } else {
        MM_Actor_OfferGetItem(&this->actor, play, GI_HEART_PIECE, 2000.0f, 1000.0f);
    }
}

void EnKitan_Talk(EnKitan* this, PlayState* play) {
    if (MM_SkelAnime_Update(&this->skelAnime)) {
        MM_Animation_MorphToLoop(&this->skelAnime, &gKeatonIdleAnim, -10.0f);
        if (play->msgCtx.currentTextId != 0x04B4) {
            // If the quiz is ongoing, select a question
            MM_Message_ContinueTextbox(play, EnKitan_GetQuestionMessageId(this));
        }
    }
    ...
    switch (MM_Message_GetState(&play->msgCtx)) {
        case TEXT_STATE_CHOICE:
            ...
            if ((play->msgCtx.choiceIndex + 1) == play->msgCtx.unk1206C) {
                ...
                if (this->timer < 5) {
                    ...
                    this->timer = 0;
                    this->textBitSet = 0;
                    MM_Message_ContinueTextbox(play, 0x04B4);
                }
                MM_Animation_MorphToPlayOnce(&this->skelAnime, &gKeatonCelebrateAnim, -5.0f);
            } else {
                // Wrong answer, end quiz
                Audio_PlaySfx(NA_SE_SY_QUIZ_INCORRECT);
                MM_Animation_MorphToLoop(&this->skelAnime, &gKeatonChuckleAnim, -5.0f);
                MM_Message_ContinueTextbox(play, 0x04B3);
                this->timer = 0;
                this->textBitSet = 0;
            }
            break;
        ...
        case TEXT_STATE_EVENT:
            ...
            switch (play->msgCtx.currentTextId) {
                ...
                case 0x04B2:
                    // Quiz begins
                    MM_Animation_MorphToLoop(&this->skelAnime, &gKeatonIdleAnim, -5.0f);
                    MM_Message_ContinueTextbox(play, EnKitan_GetQuestionMessageId(this));
                    break;
                ...
                case 0x04B5:
                    ...
                    MM_Flags_SetCollectible(play, ENKITAN_GET_COLLECT_FLAG(&this->actor));
                    break;
...
void EnKitan_WaitForPlayer(EnKitan* this, PlayState* play) {
    ...
    if (Actor_TalkOfferAccepted(&this->actor, &play->state)) {
        // Began talking
        this->actionFunc = EnKitan_Talk;
        MM_Message_StartTextbox(play, 0x04B0, &this->actor);
        this->timer = 0;
        MM_Animation_MorphToLoop(&this->skelAnime, &gKeatonChuckleAnim, -5.0f);
        MM_Audio_PlayFanfare(NA_BGM_KEATON_QUIZ);
        return;
    }

    if ((this->timer <= 0) || (MM_Player_GetMask(play) != PLAYER_MASK_KEATON)) {
