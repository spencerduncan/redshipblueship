...
void EnTest5_Init(Actor* thisx, PlayState* play2) {
    ...
    MM_Math_Vec3s_ToVec3f(&this->minPos, &waterBox->minPos);
...
void EnTest5_HandleBottleAction(EnTest5* this, PlayState* play) {
    ...
    if ((player->interactRangeActor == NULL) || (player->getItemId != GI_MAX)) {
        MM_Math_Vec3f_DistXYZAndStoreDiff(&this->minPos, &player->actor.world.pos, &playerPosRelativeToWater);
        ...
        if ((playerPosRelativeToWater.x >= 0.0f) && (playerPosRelativeToWater.x <= this->xLength) &&
            ...
            MM_Actor_OfferGetItem(&this->actor, play, GI_MAX, this->actor.xzDistToPlayer,
...
void EnTest5_Update(Actor* thisx, PlayState* play2) {
    ...
    if (ENTEST5_IS_HOT_SPRING(&this->actor) && ((play->state.frames % 4) == 0)) {
        steamPos.x = this->minPos.x + (MM_Rand_ZeroOne() * this->xLength);
        steamPos.y = this->minPos.y + 100.0f;
        steamPos.z = this->minPos.z + (MM_Rand_ZeroOne() * this->zLength);

        if ((MM_BgCheck_EntityRaycastFloor2(play, &play->colCtx, &poly, &steamPos) + 10.0f) < this->minPos.y) {
            Vec3f steamVel;
            ...
            MM_EffectSsIceSmoke_Spawn(play, &steamPos, &steamVel, &gZeroVec3f, -200 - (s32)(MM_Rand_ZeroOne() * 50.0f));
        }
    }
}
