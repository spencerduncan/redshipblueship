...
s32 MM_Collider_InitBase(struct PlayState* play, Collider* col) {
...
s32 MM_Collider_DestroyBase(struct PlayState* play, Collider* col) {
    return 1;
}
 ...
 */
s32 MM_Collider_SetBaseToActor(struct PlayState* play, Collider* col, ColliderInitToActor* src) {
    col->actor = src->actor;
 ...
 */
s32 MM_Collider_SetBaseType1(struct PlayState* play, Collider* col, Actor* actor, ColliderInitType1* src) {
    col->actor = actor;
...
s32 MM_Collider_SetBase(struct PlayState* play, Collider* col, Actor* actor, ColliderInit* src) {
    col->actor = actor;
    col->colMaterial = src->colMaterial;
    col->atFlags = src->atFlags;
    col->acFlags = src->acFlags;
...
void MM_Collider_ResetATBase(struct PlayState* play, Collider* col) {
    col->at = NULL;
    col->atFlags &= ~(AT_HIT | AT_BOUNCED);
}

void MM_Collider_ResetACBase(struct PlayState* play, Collider* col) {
    col->ac = NULL;
    col->acFlags &= ~(AC_HIT | AC_BOUNCED);
}

void MM_Collider_ResetOCBase(struct PlayState* play, Collider* col) {
    col->oc = NULL;
...
s32 MM_Collider_InitJntSphElementDim(struct PlayState* play, ColliderJntSphElementDim* dim) {
...
s32 MM_Collider_DestroyJntSphElementDim(struct PlayState* play, ColliderJntSphElementDim* dim) {
    return 1;
}

s32 MM_Collider_SetJntSphElementDim(struct PlayState* play, ColliderJntSphElementDim* dest,
...
s32 MM_Collider_InitJntSphElement(struct PlayState* play, ColliderJntSphElement* jntSphElem) {
    Collider_InitElement(play, &jntSphElem->base);
    MM_Collider_InitJntSphElementDim(play, &jntSphElem->dim);
    return 1;
}

s32 MM_Collider_DestroyJntSphElement(struct PlayState* play, ColliderJntSphElement* jntSphElem) {
    Collider_DestroyElement(play, &jntSphElem->base);
    MM_Collider_DestroyJntSphElementDim(play, &jntSphElem->dim);
    return 1;
}

s32 MM_Collider_SetJntSphElement(struct PlayState* play, ColliderJntSphElement* dest, ColliderJntSphElementInit* src) {
    Collider_SetElement(play, &dest->base, &src->base);
    MM_Collider_SetJntSphElementDim(play, &dest->dim, &src->dim);
    return 1;
}

s32 MM_Collider_ResetJntSphElementAT(struct PlayState* play, ColliderJntSphElement* jntSphElem) {
    Collider_ResetATElement(play, &jntSphElem->base);
    return 1;
}

s32 MM_Collider_ResetJntSphElementAC(struct PlayState* play, ColliderJntSphElement* jntSphElem) {
    Collider_ResetACElement(play, &jntSphElem->base);
    return 1;
}

s32 MM_Collider_ResetJntSphElementOC(struct PlayState* play, ColliderJntSphElement* jntSphElem) {
 ...
 */
s32 MM_Collider_InitJntSph(struct PlayState* play, ColliderJntSph* jntSph) {
    MM_Collider_InitBase(play, &jntSph->base);
    jntSph->count = 0;
 ...
 */
s32 MM_Collider_FreeJntSph(struct PlayState* play, ColliderJntSph* jntSph) {
    ColliderJntSphElement* jntSphElem;

    MM_Collider_DestroyBase(play, &jntSph->base);
    for (jntSphElem = jntSph->elements; jntSphElem < &jntSph->elements[jntSph->count]; jntSphElem++) {
        MM_Collider_DestroyJntSphElement(play, jntSphElem);
    }

    jntSph->count = 0;
    if (jntSph->elements != NULL) {
        MM_ZeldaArena_Free(jntSph->elements);
    }
    jntSph->elements = NULL;
    return 1;
}
 ...
 */
s32 MM_Collider_DestroyJntSph(struct PlayState* play, ColliderJntSph* jntSph) {
    ColliderJntSphElement* jntSphElem;
 ...
 */
s32 MM_Collider_SetJntSphToActor(struct PlayState* play, ColliderJntSph* dest, ColliderJntSphInitToActor* src) {
    ColliderJntSphElement* destElem;
    ColliderJntSphElementInit* srcElem;

    MM_Collider_SetBaseToActor(play, &dest->base, &src->base);
    dest->count = src->count;
    dest->elements = MM_ZeldaArena_Malloc(src->count * sizeof(ColliderJntSphElement));
    ...
    for (destElem = dest->elements, srcElem = src->elements; destElem < &dest->elements[dest->count];
         destElem++, srcElem++) {
        MM_Collider_InitJntSphElement(play, destElem);
        MM_Collider_SetJntSphElement(play, destElem, srcElem);
    }
    return 1;
}
 ...
 * OC2_TYPE_1.
 */
s32 MM_Collider_SetJntSphAllocType1(struct PlayState* play, ColliderJntSph* dest, Actor* actor,
                                 ColliderJntSphInitType1* src) {
    ColliderJntSphElement* destElem;
    ColliderJntSphElementInit* srcElem;

    MM_Collider_SetBaseType1(play, &dest->base, actor, &src->base);
    dest->count = src->count;
    dest->elements = MM_ZeldaArena_Malloc(src->count * sizeof(ColliderJntSphElement));
 ...
 */
s32 MM_Collider_SetJntSph(struct PlayState* play, ColliderJntSph* dest, Actor* actor, ColliderJntSphInit* src,
                       ColliderJntSphElement* jntSphElements) {
    ColliderJntSphElement* destElem;
    ColliderJntSphElementInit* srcElem;

    MM_Collider_SetBase(play, &dest->base, actor, &src->base);
    dest->count = src->count;
...
s32 Collider_InitAndSetJntSph(struct PlayState* play, ColliderJntSph* dest, Actor* actor, ColliderJntSphInit* src,
                              ColliderJntSphElement* jntSphElements) {
    MM_Collider_InitJntSph(play, dest);
    MM_Collider_SetJntSph(play, dest, actor, src, jntSphElements);
    return 0;
}
 ...
 */
s32 MM_Collider_ResetJntSphAT(struct PlayState* play, Collider* col) {
    ColliderJntSphElement* jntSphElem;
    ColliderJntSph* jntSph = (ColliderJntSph*)col;

    MM_Collider_ResetATBase(play, &jntSph->base);

    for (jntSphElem = jntSph->elements; jntSphElem < &jntSph->elements[jntSph->count]; jntSphElem++) {
        MM_Collider_ResetJntSphElementAT(play, jntSphElem);
    }
    return 1;
}
 ...
 */
s32 MM_Collider_ResetJntSphAC(struct PlayState* play, Collider* col) {
    ColliderJntSphElement* jntSphElem;
    ColliderJntSph* jntSph = (ColliderJntSph*)col;

    MM_Collider_ResetACBase(play, &jntSph->base);

    for (jntSphElem = jntSph->elements; jntSphElem < &jntSph->elements[jntSph->count]; jntSphElem++) {
        MM_Collider_ResetJntSphElementAC(play, jntSphElem);
    }
    return 1;
}
 ...
 */
s32 MM_Collider_ResetJntSphOC(struct PlayState* play, Collider* col) {
    ColliderJntSphElement* jntSphElem;
    ColliderJntSph* jntSph = (ColliderJntSph*)col;

    MM_Collider_ResetOCBase(play, &jntSph->base);

    for (jntSphElem = jntSph->elements; jntSphElem < &jntSph->elements[jntSph->count]; jntSphElem++) {
        MM_Collider_ResetJntSphElementOC(play, jntSphElem);
    }
    return 1;
}

s32 MM_Collider_InitCylinderDim(struct PlayState* play, Cylinder16* dim) {
...
s32 MM_Collider_DestroyCylinderDim(struct PlayState* play, Cylinder16* dim) {
    return 1;
}

s32 MM_Collider_SetCylinderDim(struct PlayState* play, Cylinder16* dim, Cylinder16* src) {
    *dim = *src;
    return 1;
}
 ...
 */
s32 MM_Collider_InitCylinder(struct PlayState* play, ColliderCylinder* cyl) {
    MM_Collider_InitBase(play, &cyl->base);
    Collider_InitElement(play, &cyl->elem);
    MM_Collider_InitCylinderDim(play, &cyl->dim);
    return 1;
}
 ...
 */
s32 MM_Collider_DestroyCylinder(struct PlayState* play, ColliderCylinder* cyl) {
    MM_Collider_DestroyBase(play, &cyl->base);
    Collider_DestroyElement(play, &cyl->elem);
    MM_Collider_DestroyCylinderDim(play, &cyl->dim);
    return 1;
}
 ...
 */
s32 MM_Collider_SetCylinderToActor(struct PlayState* play, ColliderCylinder* dest, ColliderCylinderInitToActor* src) {
    MM_Collider_SetBaseToActor(play, &dest->base, &src->base);
    Collider_SetElement(play, &dest->elem, &src->elem);
    MM_Collider_SetCylinderDim(play, &dest->dim, &src->dim);
    return 1;
}
 ...
 */
s32 MM_Collider_SetCylinderType1(struct PlayState* play, ColliderCylinder* dest, Actor* actor,
 ...
 */
s32 MM_Collider_SetCylinder(struct PlayState* play, ColliderCylinder* dest, Actor* actor, ColliderCylinderInit* src) {
    MM_Collider_SetBase(play, &dest->base, actor, &src->base);
    Collider_SetElement(play, &dest->elem, &src->elem);
    MM_Collider_SetCylinderDim(play, &dest->dim, &src->dim);
    return 1;
...
s32 Collider_InitAndSetCylinder(struct PlayState* play, ColliderCylinder* dest, Actor* actor,
    ...
    MM_Collider_InitCylinder(play, dest);
    MM_Collider_SetCylinder(play, dest, actor, src);
    return 1;
}
 ...
 * Resets the collider's AT collision flags.
 */
s32 MM_Collider_ResetCylinderAT(struct PlayState* play, Collider* col) {
    ColliderCylinder* cyl = (ColliderCylinder*)col;

    MM_Collider_ResetATBase(play, &cyl->base);
 ...
 * Resets the collider's AC collision flags.
 */
s32 MM_Collider_ResetCylinderAC(struct PlayState* play, Collider* col) {
    ColliderCylinder* cyl = (ColliderCylinder*)col;

    MM_Collider_ResetACBase(play, &cyl->base);
 ...
 * Resets the collider's OC collision flags.
 */
s32 MM_Collider_ResetCylinderOC(struct PlayState* play, Collider* col) {
    ColliderCylinder* cyl = (ColliderCylinder*)col;

    MM_Collider_ResetOCBase(play, &cyl->base);
    Collider_ResetOCElement(play, &cyl->elem);
    return 1;
}

s32 MM_Collider_InitTrisElementDim(struct PlayState* play, TriNorm* dim) {
...
s32 MM_Collider_DestroyTrisElementDim(struct PlayState* play, TriNorm* dim) {
    return 1;
}

s32 MM_Collider_SetTrisElementDim(struct PlayState* play, TriNorm* dest, ColliderTrisElementDimInit* src) {
    Vec3f* destVtx;
    Vec3f* srcVtx;
    f32 nx;
    f32 ny;
    ...
    MM_Math3D_DefPlane(&src->vtx[0], &src->vtx[1], &src->vtx[2], &nx, &ny, &nz, &originDist);
...
s32 MM_Collider_InitTrisElement(struct PlayState* play, ColliderTrisElement* trisElem) {
    Collider_InitElement(play, &trisElem->base);
    MM_Collider_InitTrisElementDim(play, &trisElem->dim);
    return 1;
}

s32 MM_Collider_DestroyTrisElement(struct PlayState* play, ColliderTrisElement* trisElem) {
    Collider_DestroyElement(play, &trisElem->base);
    MM_Collider_DestroyTrisElementDim(play, &trisElem->dim);
    return 1;
}

s32 MM_Collider_SetTrisElement(struct PlayState* play, ColliderTrisElement* dest, ColliderTrisElementInit* src) {
    Collider_SetElement(play, &dest->base, &src->base);
    MM_Collider_SetTrisElementDim(play, &dest->dim, &src->dim);
    return 1;
}

s32 MM_Collider_ResetTrisElementAT(struct PlayState* play, ColliderTrisElement* trisElem) {
    Collider_ResetATElement(play, &trisElem->base);
    return 1;
}

s32 MM_Collider_ResetTrisElementAC(struct PlayState* play, ColliderTrisElement* trisElem) {
    Collider_ResetACElement(play, &trisElem->base);
    return 1;
}

s32 MM_Collider_ResetTrisElementOC(struct PlayState* play, ColliderTrisElement* trisElem) {
 ...
 */
s32 MM_Collider_InitTris(struct PlayState* play, ColliderTris* tris) {
    MM_Collider_InitBase(play, &tris->base);
    tris->count = 0;
    tris->elements = NULL;
    return 1;
}
 ...
 */
s32 MM_Collider_FreeTris(struct PlayState* play, ColliderTris* tris) {
    ColliderTrisElement* trisElem;

    MM_Collider_DestroyBase(play, &tris->base);

    for (trisElem = tris->elements; trisElem < &tris->elements[tris->count]; trisElem++) {
        MM_Collider_DestroyTrisElement(play, trisElem);
    }

    tris->count = 0;
    if (tris->elements != NULL) {
        MM_ZeldaArena_Free(tris->elements);
    }
    tris->elements = NULL;
 ...
 */
s32 MM_Collider_DestroyTris(struct PlayState* play, ColliderTris* tris) {
    ColliderTrisElement* trisElem;
 ...
 * OC2_TYPE_1.
 */
s32 MM_Collider_SetTrisAllocType1(struct PlayState* play, ColliderTris* dest, Actor* actor, ColliderTrisInitType1* src) {
    ColliderTrisElement* destElem;
    ColliderTrisElementInit* srcElem;

    MM_Collider_SetBaseType1(play, &dest->base, actor, &src->base);
    dest->count = src->count;
    dest->elements = MM_ZeldaArena_Malloc(dest->count * sizeof(ColliderTrisElement));
    ...
    for (destElem = dest->elements, srcElem = src->elements; destElem < &dest->elements[dest->count];
         destElem++, srcElem++) {
        MM_Collider_InitTrisElement(play, destElem);
        MM_Collider_SetTrisElement(play, destElem, srcElem);
    }
 ...
 */
s32 MM_Collider_SetTris(struct PlayState* play, ColliderTris* dest, Actor* actor, ColliderTrisInit* src,
...
s32 Collider_InitAndSetTris(struct PlayState* play, ColliderTris* dest, Actor* actor, ColliderTrisInit* src,
                            ColliderTrisElement* trisElements) {
    MM_Collider_InitTris(play, dest);
    MM_Collider_SetTris(play, dest, actor, src, trisElements);
    return 0;
}
 ...
 * Resets the collider's AT collision flags.
 */
s32 MM_Collider_ResetTrisAT(struct PlayState* play, Collider* col) {
    ColliderTrisElement* trisElem;
    ColliderTris* tris = (ColliderTris*)col;

    MM_Collider_ResetATBase(play, &tris->base);

    for (trisElem = tris->elements; trisElem < &tris->elements[tris->count]; trisElem++) {
        MM_Collider_ResetTrisElementAT(play, trisElem);
    }
    return 1;
}
 ...
 * Resets the collider's AC collision flags.
 */
s32 MM_Collider_ResetTrisAC(struct PlayState* play, Collider* col) {
    ColliderTrisElement* trisElem;
    ColliderTris* tris = (ColliderTris*)col;

    MM_Collider_ResetACBase(play, &tris->base);

    for (trisElem = tris->elements; trisElem < &tris->elements[tris->count]; trisElem++) {
        MM_Collider_ResetTrisElementAC(play, trisElem);
    }
    return 1;
}
 ...
 * Resets the collider's OC collision flags.
 */
s32 MM_Collider_ResetTrisOC(struct PlayState* play, Collider* col) {
    ColliderTrisElement* trisElem;
    ColliderTris* tris = (ColliderTris*)col;

    MM_Collider_ResetOCBase(play, &tris->base);

    for (trisElem = tris->elements; trisElem < &tris->elements[tris->count]; trisElem++) {
        MM_Collider_ResetTrisElementOC(play, trisElem);
    }
    return 1;
}

s32 MM_Collider_InitQuadDim(struct PlayState* play, ColliderQuadDim* dim) {
...
s32 MM_Collider_DestroyQuadDim(struct PlayState* play, ColliderQuadDim* dim) {
    return 1;
}

s32 MM_Collider_ResetQuadACDist(struct PlayState* play, ColliderQuadDim* dim) {
    dim->acDist = 1.0E38f;
    return 1;
}

void MM_Collider_SetQuadMidpoints(ColliderQuadDim* dim) {
...
s32 MM_Collider_SetQuadDim(struct PlayState* play, ColliderQuadDim* dest, ColliderQuadDimInit* src) {
    dest->quad[0] = src->quad[0];
    dest->quad[1] = src->quad[1];
    dest->quad[2] = src->quad[2];
    dest->quad[3] = src->quad[3];

    MM_Collider_SetQuadMidpoints(dest);
    return 1;
}
 ...
 */
s32 MM_Collider_InitQuad(struct PlayState* play, ColliderQuad* quad) {
    MM_Collider_InitBase(play, &quad->base);
    Collider_InitElement(play, &quad->elem);
    MM_Collider_InitQuadDim(play, &quad->dim);
    return 1;
}
 ...
 */
s32 MM_Collider_DestroyQuad(struct PlayState* play, ColliderQuad* quad) {
    MM_Collider_DestroyBase(play, &quad->base);
    Collider_DestroyElement(play, &quad->elem);
    MM_Collider_DestroyQuadDim(play, &quad->dim);
    return 1;
}
 ...
 */
s32 MM_Collider_SetQuadType1(struct PlayState* play, ColliderQuad* dest, Actor* actor, ColliderQuadInitType1* src) {
    MM_Collider_SetBaseType1(play, &dest->base, actor, &src->base);
    Collider_SetElement(play, &dest->elem, &src->elem);
    MM_Collider_SetQuadDim(play, &dest->dim, &src->dim);
    return 1;
}
 ...
 */
s32 MM_Collider_SetQuad(struct PlayState* play, ColliderQuad* dest, Actor* actor, ColliderQuadInit* src) {
    MM_Collider_SetBase(play, &dest->base, actor, &src->base);
    Collider_SetElement(play, &dest->elem, &src->elem);
    MM_Collider_SetQuadDim(play, &dest->dim, &src->dim);
    return 1;
...
s32 Collider_InitAndSetQuad(struct PlayState* play, ColliderQuad* dest, Actor* actor, ColliderQuadInit* src) {
    MM_Collider_InitQuad(play, dest);
    MM_Collider_SetQuad(play, dest, actor, src);
    return 0;
}
 ...
 * Resets the collider's AT collision flags.
 */
s32 MM_Collider_ResetQuadAT(struct PlayState* play, Collider* col) {
    ColliderQuad* quad = (ColliderQuad*)col;

    MM_Collider_ResetATBase(play, &quad->base);
    Collider_ResetATElement(play, &quad->elem);
    MM_Collider_ResetQuadACDist(play, &quad->dim);
    return 1;
}
 ...
 * Resets the collider's AC collision flags.
 */
s32 MM_Collider_ResetQuadAC(struct PlayState* play, Collider* col) {
    ColliderQuad* quad = (ColliderQuad*)col;

    MM_Collider_ResetACBase(play, &quad->base);
 ...
 * Resets the collider's OC collision flags.
 */
s32 MM_Collider_ResetQuadOC(struct PlayState* play, Collider* col) {
    ColliderQuad* quad = (ColliderQuad*)col;

    MM_Collider_ResetOCBase(play, &quad->base);
 ...
 */
s32 MM_Collider_QuadSetNearestAC(struct PlayState* play, ColliderQuad* quad, Vec3f* hitPos) {
    f32 acDist;
    Vec3f dcMid;
    ...
    MM_Math_Vec3s_ToVec3f(&dcMid, &quad->dim.dcMid);
    acDist = MM_Math3D_Vec3fDistSq(&dcMid, hitPos);

    if (acDist < quad->dim.acDist) {
        quad->dim.acDist = acDist;

        if (quad->elem.atHit != NULL) {
            MM_Collider_ResetACBase(play, quad->elem.atHit);
        }
...
s32 Collider_InitSphere(struct PlayState* play, ColliderSphere* sph) {
    MM_Collider_InitBase(play, &sph->base);
    Collider_InitElement(play, &sph->elem);
    MM_Collider_InitJntSphElementDim(play, &sph->dim);
    return 1;
}
...
s32 Collider_DestroySphere(struct PlayState* play, ColliderSphere* sph) {
    MM_Collider_DestroyBase(play, &sph->base);
    Collider_DestroyElement(play, &sph->elem);
    MM_Collider_DestroyJntSphElementDim(play, &sph->dim);
    return 1;
}
...
s32 Collider_ResetSphereAT(struct PlayState* play, Collider* col) {
    ...
    MM_Collider_ResetATBase(play, &sph->base);
...
s32 Collider_ResetSphereAC(struct PlayState* play, Collider* col) {
    ...
    MM_Collider_ResetACBase(play, &sph->base);
...
s32 Collider_ResetSphereOC(struct PlayState* play, Collider* col) {
    ...
    MM_Collider_ResetOCBase(play, &sph->base);
 ...
 */
s32 MM_Collider_InitLine(struct PlayState* play, OcLine* line) {
    static Vec3f sDefaultLinePoint = { 0.0f, 0.0f, 0.0f };

    MM_Math_Vec3f_Copy(&line->line.a, &sDefaultLinePoint);
    MM_Math_Vec3f_Copy(&line->line.b, &sDefaultLinePoint);
    return 1;
}
 ...
 * Destroys an OcLine
 */
s32 MM_Collider_DestroyLine(struct PlayState* play, OcLine* line) {
    return 1;
}
 ...
 */
s32 MM_Collider_SetLinePoints(struct PlayState* play, OcLine* line, Vec3f* a, Vec3f* b) {
    MM_Math_Vec3f_Copy(&line->line.a, a);
    MM_Math_Vec3f_Copy(&line->line.b, b);
    return 1;
}
 ...
 */
s32 MM_Collider_SetLine(struct PlayState* play, OcLine* dst, OcLine* src) {
    dst->ocFlags = src->ocFlags;
    MM_Collider_SetLinePoints(play, dst, &src->line.a, &src->line.b);
    return 1;
}
 ...
 */
s32 MM_Collider_ResetLineOC(struct PlayState* play, OcLine* line) {
 ...
 */
void MM_CollisionCheck_InitContext(struct PlayState* play, CollisionCheckContext* colChkCtx) {
    colChkCtx->sacFlags = 0;
    MM_CollisionCheck_ClearContext(play, colChkCtx);
}

void MM_CollisionCheck_DestroyContext(struct PlayState* play, CollisionCheckContext* colChkCtx) {
}

/**
 * Clears all collider lists in CollisionCheckContext when not in SAC mode.
 */
void MM_CollisionCheck_ClearContext(struct PlayState* play, CollisionCheckContext* colChkCtx) {
    Collider** colP;
    OcLine** lineP;
 ...
 */
void MM_CollisionCheck_EnableSAC(struct PlayState* play, CollisionCheckContext* colChkCtx) {
 ...
 */
void MM_CollisionCheck_DisableSAC(struct PlayState* play, CollisionCheckContext* colChkCtx) {
    colChkCtx->sacFlags &= ~SAC_ON;
}

ColChkResetFunc MM_sATResetFuncs[COLSHAPE_MAX] = {
    MM_Collider_ResetJntSphAT,   // COLSHAPE_JNTSPH
    MM_Collider_ResetCylinderAT, // COLSHAPE_CYLINDER
    MM_Collider_ResetTrisAT,     // COLSHAPE_TRIS
    MM_Collider_ResetQuadAT,     // COLSHAPE_QUAD
 ...
 */
s32 MM_CollisionCheck_SetAT(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    s32 index;

    if (MM_FrameAdvance_IsEnabled(play)) {
        return -1;
    }
    MM_sATResetFuncs[col->shape](play, col);
 ...
 */
s32 MM_CollisionCheck_SetAT_SAC(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col, s32 index) {
    if (MM_FrameAdvance_IsEnabled(play)) {
        return -1;
    }
    MM_sATResetFuncs[col->shape](play, col);
...
ColChkResetFunc MM_sACResetFuncs[COLSHAPE_MAX] = {
    MM_Collider_ResetJntSphAC,   // COLSHAPE_JNTSPH
    MM_Collider_ResetCylinderAC, // COLSHAPE_CYLINDER
    MM_Collider_ResetTrisAC,     // COLSHAPE_TRIS
    MM_Collider_ResetQuadAC,     // COLSHAPE_QUAD
 ...
 */
s32 MM_CollisionCheck_SetAC(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    s32 index;

    if (MM_FrameAdvance_IsEnabled(play)) {
        return -1;
    }
    MM_sACResetFuncs[col->shape](play, col);
 ...
 * will be inserted into the next slot
 */
s32 MM_CollisionCheck_SetAC_SAC(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col, s32 index) {
    if (MM_FrameAdvance_IsEnabled(play)) {
        return -1;
    }
    MM_sACResetFuncs[col->shape](play, col);
...
static ColChkResetFunc MM_sOCResetFuncs[COLSHAPE_MAX] = {
    MM_Collider_ResetJntSphOC,   // COLSHAPE_JNTSPH
    MM_Collider_ResetCylinderOC, // COLSHAPE_CYLINDER
    MM_Collider_ResetTrisOC,     // COLSHAPE_TRIS
    MM_Collider_ResetQuadOC,     // COLSHAPE_QUAD
 ...
 */
s32 MM_CollisionCheck_SetOC(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    s32 index;

    if (MM_FrameAdvance_IsEnabled(play)) {
        return -1;
    }
    MM_sOCResetFuncs[col->shape](play, col);
 ...
 * will be inserted into the next slot.
 */
s32 MM_CollisionCheck_SetOC_SAC(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col, s32 index) {
    if (MM_FrameAdvance_IsEnabled(play)) {
        return -1;
    }
    MM_sOCResetFuncs[col->shape](play, col);
 ...
 */
s32 MM_CollisionCheck_SetOCLine(struct PlayState* play, CollisionCheckContext* colChkCtx, OcLine* line) {
    s32 index;
    ...
    MM_Collider_ResetLineOC(play, line);
 ...
 */
s32 MM_CollisionCheck_NoSharedFlags(ColliderElement* atElem, ColliderElement* acElem) {
 ...
 */
void MM_CollisionCheck_NoBlood(struct PlayState* play, Collider* col, Vec3f* v) {
}
 ...
 */
void MM_CollisionCheck_BlueBlood(struct PlayState* play, Collider* col, Vec3f* v) {
    ...
    MM_Effect_Add(play, &effectIndex, EFFECT_SPARK, 0, 1, &D_801EEC00);
}
 ...
 */
void MM_CollisionCheck_GreenBlood(struct PlayState* play, Collider* col, Vec3f* v) {
    ...
    MM_Effect_Add(play, &effectIndex, EFFECT_SPARK, 0, 1, &D_801EF0C8);
}
 ...
 */
void MM_CollisionCheck_WaterBurst(struct PlayState* play, Collider* col, Vec3f* v) {
    MM_EffectSsSibuki_SpawnBurst(play, v);
    MM_CollisionCheck_SpawnWaterDroplets(play, v);
}
 ...
 */
void MM_CollisionCheck_RedBlood(struct PlayState* play, Collider* col, Vec3f* v) {
    MM_CollisionCheck_SpawnRedBlood(play, v);
}

/**
 * Spawns red blood drops.
 */
void MM_CollisionCheck_RedBloodUnused(struct PlayState* play, Collider* col, Vec3f* v) {
    MM_CollisionCheck_SpawnRedBlood(play, v);
}
 ...
 */
void MM_CollisionCheck_HitSolid(struct PlayState* play, ColliderElement* elem, Collider* col, Vec3f* hitPos) {
    ...
    if ((flags == ATELEM_SFX_NORMAL) && (col->colMaterial != COL_MATERIAL_METAL)) {
        ...
        if (col->actor == NULL) {
            ...
            MM_CollisionCheck_SpawnShieldParticlesMetalSound(play, hitPos, &col->actor->projectedPos);
        }
 ...
 */
s32 MM_CollisionCheck_SwordHitAudio(Collider* at, ColliderElement* acElem) {
...
ColChkBloodFunc MM_sBloodFuncs[] = {
    MM_CollisionCheck_NoBlood,    MM_CollisionCheck_BlueBlood, MM_CollisionCheck_GreenBlood,
    MM_CollisionCheck_WaterBurst, MM_CollisionCheck_RedBlood,  MM_CollisionCheck_RedBloodUnused,
};
 ...
 */
void MM_CollisionCheck_HitEffects(struct PlayState* play, Collider* at, ColliderElement* atElem, Collider* ac,
    ...
    if (ac->actor != NULL) {
        MM_sBloodFuncs[MM_sHitInfo[ac->colMaterial].blood](play, ac, hitPos);
    }
    if (ac->actor != NULL) {
        if (MM_sHitInfo[ac->colMaterial].effect == HIT_SOLID) {
            MM_CollisionCheck_HitSolid(play, atElem, ac, hitPos);
        } else if (MM_sHitInfo[ac->colMaterial].effect == HIT_WOOD) {
            if (at->actor == NULL) {
                MM_CollisionCheck_SpawnShieldParticles(play, hitPos);
                Audio_PlaySfx(NA_SE_IT_REFLECTION_WOOD);
            } else {
                MM_CollisionCheck_SpawnShieldParticlesWood(play, hitPos, &at->actor->projectedPos);
            }
        } else if (MM_sHitInfo[ac->colMaterial].effect != HIT_NONE) {
            EffectSsHitmark_SpawnFixedScale(play, MM_sHitInfo[ac->colMaterial].effect, hitPos);
            if (!(acElem->acElemFlags & ACELEM_NO_SWORD_SFX)) {
                MM_CollisionCheck_SwordHitAudio(at, acElem);
            }
        }
    } else {
        EffectSsHitmark_SpawnFixedScale(play, 0, hitPos);
 ...
 */
void MM_CollisionCheck_SetBounce(Collider* at, Collider* ac) {
 ...
 */
s32 MM_CollisionCheck_SetATvsAC(struct PlayState* play, Collider* atCol, ColliderElement* atElem, Vec3f* atPos,
    ...
    if ((acCol->acFlags & AC_HARD) && (atCol->actor != NULL) && (acCol->actor != NULL)) {
        MM_CollisionCheck_SetBounce(atCol, acCol);
    }
    ...
    if (!(atElem->atElemFlags & ATELEM_AT_HITMARK) && (acCol->colMaterial != COL_MATERIAL_METAL) &&
        ...
        MM_CollisionCheck_HitEffects(play, atCol, atElem, acCol, acElem, hitPos);
 ...
 */
void MM_CollisionCheck_AC_JntSphVsJntSph(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                      Collider* acCol) {
    ...
    if ((atJntSph->count > 0) && (atJntSph->elements != NULL) && (acJntSph->count > 0) &&
        ...
        for (atJntSphElem = atJntSph->elements; atJntSphElem < &atJntSph->elements[atJntSph->count]; atJntSphElem++) {
            ...
            for (acJntSphElem = acJntSph->elements; acJntSphElem < &acJntSph->elements[acJntSph->count];
                ...
                if (MM_CollisionCheck_NoSharedFlags(&atJntSphElem->base, &acJntSphElem->base)) {
                    continue;
                }
                ...
                if (Math3D_SphVsSphOverlapCenterDist(&atJntSphElem->dim.worldSphere, &acJntSphElem->dim.worldSphere,
                    ...
                    MM_Math_Vec3s_ToVec3f(&atPos, &atJntSphElem->dim.worldSphere.center);
                    MM_Math_Vec3s_ToVec3f(&acPos, &acJntSphElem->dim.worldSphere.center);
                    ...
                    if (!IS_ZERO(centerDist)) {
                        ...
                        MM_Math_Vec3f_Copy(&hitPos, &atPos);
                    }
                    MM_CollisionCheck_SetATvsAC(play, &atJntSph->base, &atJntSphElem->base, &atPos, &acJntSph->base,
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_JntSphVsCyl(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                   Collider* acCol) {
    ColliderJntSph* atJntSph = (ColliderJntSph*)atCol;
    ColliderJntSphElement* atJntSphElem;
    ...
    if ((atJntSph->count > 0) && (atJntSph->elements != NULL) && (acCyl->dim.radius > 0) && (acCyl->dim.height > 0)) {
        ...
        for (atJntSphElem = atJntSph->elements; atJntSphElem < &atJntSph->elements[atJntSph->count]; atJntSphElem++) {
            ...
            if (MM_CollisionCheck_NoSharedFlags(&atJntSphElem->base, &acCyl->elem)) {
                continue;
            }
            if (MM_Math3D_SphVsCylOverlapCenterDist(&atJntSphElem->dim.worldSphere, &acCyl->dim, &overlapSize,
                                                 &centerDist)) {
                Vec3f hitPos;
                Vec3f atPos;
                Vec3f acPos;
                ...
                MM_Math_Vec3s_ToVec3f(&atPos, &atJntSphElem->dim.worldSphere.center);
                MM_Math_Vec3s_ToVec3f(&acPos, &acCyl->dim.pos);
                if (!IS_ZERO(centerDist)) {
                ...
                MM_CollisionCheck_SetATvsAC(play, &atJntSph->base, &atJntSphElem->base, &atPos, &acCyl->base, &acCyl->elem,
                                         &acPos, &hitPos);
                return;
            }
        }
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_JntSphVsTris(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                    Collider* acCol) {
    ColliderJntSph* atJntSph = (ColliderJntSph*)atCol;
    ColliderJntSphElement* atJntSphElem;
    ...
    if ((atJntSph->count > 0) && (atJntSph->elements != NULL) && (acTris->count > 0) && (acTris->elements != NULL)) {
        for (atJntSphElem = atJntSph->elements; atJntSphElem < &atJntSph->elements[atJntSph->count]; atJntSphElem++) {
            ...
            for (acTrisElem = acTris->elements; acTrisElem < &acTris->elements[acTris->count]; acTrisElem++) {
                ...
                if (MM_CollisionCheck_NoSharedFlags(&atJntSphElem->base, &acTrisElem->base)) {
                    continue;
                }
                if (MM_Math3D_TriVsSphIntersect(&atJntSphElem->dim.worldSphere, &acTrisElem->dim, &hitPos)) {
                    Vec3f atPos;
                    Vec3f acPos;

                    MM_Math_Vec3s_ToVec3f(&atPos, &atJntSphElem->dim.worldSphere.center);
                    CollisionCheck_TrisAvgPoint(acTrisElem, &acPos);
                    MM_CollisionCheck_SetATvsAC(play, &atJntSph->base, &atJntSphElem->base, &atPos, &acTris->base,
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_JntSphVsQuad(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                    Collider* acCol) {
    ColliderJntSph* atJntSph = (ColliderJntSph*)atCol;
    ...
    if ((atJntSph->count > 0) && (atJntSph->elements != NULL)) {
        ...
        MM_Math3D_TriNorm(&D_801EF590, &acQuad->dim.quad[2], &acQuad->dim.quad[3], &acQuad->dim.quad[1]);
        MM_Math3D_TriNorm(&D_801EF5C8, &acQuad->dim.quad[1], &acQuad->dim.quad[0], &acQuad->dim.quad[2]);

        for (atJntSphElem = atJntSph->elements; atJntSphElem < &atJntSph->elements[atJntSph->count]; atJntSphElem++) {
            if (CollisionCheck_IsElementNotAT(&atJntSphElem->base)) {
                continue;
            }
            if (MM_CollisionCheck_NoSharedFlags(&atJntSphElem->base, &acQuad->elem)) {
                continue;
            }
            if (MM_Math3D_TriVsSphIntersect(&atJntSphElem->dim.worldSphere, &D_801EF590, &hitPos) ||
                MM_Math3D_TriVsSphIntersect(&atJntSphElem->dim.worldSphere, &D_801EF5C8, &hitPos)) {
                Vec3f atPos;
                Vec3f acPos;

                MM_Math_Vec3s_ToVec3f(&atPos, &atJntSphElem->dim.worldSphere.center);
                CollisionCheck_QuadAvgPoint(acQuad, &acPos);
                MM_CollisionCheck_SetATvsAC(play, &atJntSph->base, &atJntSphElem->base, &atPos, &acQuad->base,
...
void CollisionCheck_AC_JntSphVsSphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
    ...
    if ((atJntSph->count > 0) && (atJntSph->elements != NULL)) {
        ...
        for (atJntSphElem = atJntSph->elements; atJntSphElem < &atJntSph->elements[atJntSph->count]; atJntSphElem++) {
            ...
            if (MM_CollisionCheck_NoSharedFlags(&atJntSphElem->base, &acSph->elem)) {
                continue;
            }
            if (Math3D_SphVsSphOverlapCenterDist(&atJntSphElem->dim.worldSphere, &acSph->dim.worldSphere, &overlapSize,
                ...
                MM_Math_Vec3s_ToVec3f(&atPos, &atJntSphElem->dim.worldSphere.center);
                MM_Math_Vec3s_ToVec3f(&acPos, &acSph->dim.worldSphere.center);
                if (!IS_ZERO(centerDist)) {
                ...
                MM_CollisionCheck_SetATvsAC(play, &atJntSph->base, &atJntSphElem->base, &atPos, &acSph->base, &acSph->elem,
                                         &acPos, &hitPos);
            }
        }
    }
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_CylVsJntSph(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                   Collider* acCol) {
    ...
    if ((acJntSph->count > 0) && (acJntSph->elements != NULL) && (atCyl->dim.radius > 0) && (atCyl->dim.height > 0)) {
        ...
        for (acJntSphElem = acJntSph->elements; acJntSphElem < &acJntSph->elements[acJntSph->count]; acJntSphElem++) {
            ...
            if (MM_CollisionCheck_NoSharedFlags(&atCyl->elem, &acJntSphElem->base)) {
                continue;
            }
            if (MM_Math3D_SphVsCylOverlapCenterDist(&acJntSphElem->dim.worldSphere, &atCyl->dim, &overlapSize,
                                                 &centerDist)) {
                Vec3f hitPos;
                Vec3f atPos;
                Vec3f acPos;
                ...
                MM_Math_Vec3s_ToVec3f(&atPos, &atCyl->dim.pos);
                MM_Math_Vec3s_ToVec3f(&acPos, &acJntSphElem->dim.worldSphere.center);
                if (!IS_ZERO(centerDist)) {
                    acToHit = acJntSphElem->dim.worldSphere.radius / centerDist;
                    if (acToHit <= 1.0f) {
                ...
                MM_CollisionCheck_SetATvsAC(play, &atCyl->base, &atCyl->elem, &atPos, &acJntSph->base, &acJntSphElem->base,
                                         &acPos, &hitPos);
                if (!(acJntSph->base.ocFlags2 & OC2_FIRST_ONLY)) {
                    break;
                }
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_CylVsCyl(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                Collider* acCol) {
    ColliderCylinder* atCyl = (ColliderCylinder*)atCol;
    ColliderCylinder* acCyl = (ColliderCylinder*)acCol;
    f32 overlapSize;
    ...
    if ((atCyl->dim.radius > 0) && (atCyl->dim.height > 0) && (acCyl->dim.radius > 0) && (acCyl->dim.height > 0)) {
        ...
        if (MM_CollisionCheck_NoSharedFlags(&atCyl->elem, &acCyl->elem)) {
            return;
        }
        ...
        if (Math3D_CylVsCylOverlapCenterDist(&atCyl->dim, &acCyl->dim, &overlapSize, &centerDist)) {
            ...
            if (!IS_ZERO(centerDist)) {
                ...
                MM_Math_Vec3s_ToVec3f(&hitPos, &acCyl->dim.pos);
            }
            MM_CollisionCheck_SetATvsAC(play, &atCyl->base, &atCyl->elem, &atPos, &acCyl->base, &acCyl->elem, &acPos,
                                     &hitPos);
        }
    }
}
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_CylVsTris(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                 Collider* acCol) {
    ColliderCylinder* atCyl = (ColliderCylinder*)atCol;
    ColliderTris* acTris = (ColliderTris*)acCol;
    ColliderTrisElement* acTrisElem;
    ...
    if ((atCyl->dim.radius > 0) && (atCyl->dim.height > 0) && (acTris->count > 0) && (acTris->elements != NULL)) {
        ...
        for (acTrisElem = acTris->elements; acTrisElem < &acTris->elements[acTris->count]; acTrisElem++) {
            ...
            if (MM_CollisionCheck_NoSharedFlags(&atCyl->elem, &acTrisElem->base)) {
                continue;
            }

            if (MM_Math3D_CylTriVsIntersect(&atCyl->dim, &acTrisElem->dim, &hitPos)) {
                Vec3f atPos;
                Vec3f acPos;

                MM_Math_Vec3s_ToVec3f(&atPos, &atCyl->dim.pos);
                CollisionCheck_TrisAvgPoint(acTrisElem, &acPos);
                MM_CollisionCheck_SetATvsAC(play, &atCyl->base, &atCyl->elem, &atPos, &acTris->base, &acTrisElem->base,
                                         &acPos, &hitPos);
                return;
            }
        }
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_CylVsQuad(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                 Collider* acCol) {
    ColliderCylinder* atCyl = (ColliderCylinder*)atCol;
    ColliderQuad* acQuad = (ColliderQuad*)acCol;
    ...
    if ((atCyl->dim.height > 0) && (atCyl->dim.radius > 0)) {
        ...
        if (MM_CollisionCheck_NoSharedFlags(&atCyl->elem, &acQuad->elem)) {
            return;
        }
        MM_Math3D_TriNorm(&D_801EF600, &acQuad->dim.quad[2], &acQuad->dim.quad[3], &acQuad->dim.quad[1]);
        MM_Math3D_TriNorm(&D_801EF638, &acQuad->dim.quad[1], &acQuad->dim.quad[0], &acQuad->dim.quad[2]);

        if (MM_Math3D_CylTriVsIntersect(&atCyl->dim, &D_801EF600, &D_801EDE00)) {
            Vec3f atPos;
            Vec3f acPos;

            MM_Math_Vec3s_ToVec3f(&atPos, &atCyl->dim.pos);
            CollisionCheck_QuadAvgPoint(acQuad, &acPos);
            MM_CollisionCheck_SetATvsAC(play, &atCyl->base, &atCyl->elem, &atPos, &acQuad->base, &acQuad->elem, &acPos,
                                     &D_801EDE00);
        } else if (MM_Math3D_CylTriVsIntersect(&atCyl->dim, &D_801EF638, &D_801EDE00)) {
            Vec3f atPos;
            Vec3f acPos;
...
void CollisionCheck_AC_CylVsSphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
    ...
    if ((atCyl->dim.radius > 0) && (atCyl->dim.height > 0)) {
        ...
        if (MM_CollisionCheck_NoSharedFlags(&atCyl->elem, &acSph->elem)) {
            return;
        }

        if (MM_Math3D_SphVsCylOverlapCenterDist(&acSph->dim.worldSphere, &atCyl->dim, &overlapSize, &centerDist)) {
            Vec3f hitPos;
            Vec3f atPos;
            Vec3f acPos;
            f32 acToHit;
            ...
            MM_CollisionCheck_SetATvsAC(play, &atCyl->base, &atCyl->elem, &atPos, &acSph->base, &acSph->elem, &acPos,
                                     &hitPos);
        }
    }
}
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_TrisVsJntSph(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                    Collider* acCol) {
    ...
    if ((acJntSph->count > 0) && (acJntSph->elements != NULL) && (atTris->count > 0) && (atTris->elements != NULL)) {
        for (acJntSphElem = acJntSph->elements; acJntSphElem < &acJntSph->elements[acJntSph->count]; acJntSphElem++) {
            ...
            for (atTrisElem = atTris->elements; atTrisElem < &atTris->elements[atTris->count]; atTrisElem++) {
                ...
                if (MM_CollisionCheck_NoSharedFlags(&atTrisElem->base, &acJntSphElem->base)) {
                    continue;
                }
                if (MM_Math3D_TriVsSphIntersect(&acJntSphElem->dim.worldSphere, &atTrisElem->dim, &hitPos)) {
                    Vec3f atPos;
                    Vec3f acPos;

                    MM_Math_Vec3s_ToVec3f(&acPos, &acJntSphElem->dim.worldSphere.center);
                    CollisionCheck_TrisAvgPoint(atTrisElem, &atPos);
                    MM_CollisionCheck_SetATvsAC(play, &atTris->base, &atTrisElem->base, &atPos, &acJntSph->base,
                                             &acJntSphElem->base, &acPos, &hitPos);
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_TrisVsCyl(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                 Collider* acCol) {
    ColliderTris* atTris = (ColliderTris*)atCol;
    ColliderCylinder* acCyl = (ColliderCylinder*)acCol;
    ColliderTrisElement* atTrisElem;
    ...
    if ((acCyl->dim.radius > 0) && (acCyl->dim.height > 0) && (atTris->count > 0) && (atTris->elements != NULL)) {
        ...
        for (atTrisElem = atTris->elements; atTrisElem < &atTris->elements[atTris->count]; atTrisElem++) {
            ...
            if (MM_CollisionCheck_NoSharedFlags(&atTrisElem->base, &acCyl->elem)) {
                continue;
            }
            if (MM_Math3D_CylTriVsIntersect(&acCyl->dim, &atTrisElem->dim, &D_801EDE10)) {
                Vec3f atPos;
                Vec3f acPos;

                CollisionCheck_TrisAvgPoint(atTrisElem, &atPos);
                MM_Math_Vec3s_ToVec3f(&acPos, &acCyl->dim.pos);
                MM_CollisionCheck_SetATvsAC(play, &atTris->base, &atTrisElem->base, &atPos, &acCyl->base, &acCyl->elem,
                                         &acPos, &D_801EDE10);
                return;
            }
        }
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_TrisVsTris(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                  Collider* acCol) {
    ColliderTris* atTris = (ColliderTris*)atCol;
    ColliderTrisElement* atTrisElem;
    ColliderTris* acTris = (ColliderTris*)acCol;
    ...
    if ((acTris->count > 0) && (acTris->elements != NULL) && (atTris->count > 0) && (atTris->elements != NULL)) {
        for (acTrisElem = acTris->elements; acTrisElem < &acTris->elements[acTris->count]; acTrisElem++) {
            ...
            for (atTrisElem = atTris->elements; atTrisElem < &atTris->elements[atTris->count]; atTrisElem++) {
                ...
                if (MM_CollisionCheck_NoSharedFlags(&atTrisElem->base, &acTrisElem->base)) {
                    continue;
                }
                if (MM_Math3D_TriVsTriIntersect(&atTrisElem->dim, &acTrisElem->dim, &D_801EDE20)) {
                    Vec3f atPos;
                    Vec3f acPos;

                    CollisionCheck_TrisAvgPoint(atTrisElem, &atPos);
                    CollisionCheck_TrisAvgPoint(acTrisElem, &acPos);
                    MM_CollisionCheck_SetATvsAC(play, &atTris->base, &atTrisElem->base, &atPos, &acTris->base,
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_TrisVsQuad(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                  Collider* acCol) {
    ColliderTris* atTris = (ColliderTris*)atCol;
    ColliderQuad* acQuad = (ColliderQuad*)acCol;
    ColliderTrisElement* atTrisElem;
    ...
    if ((atTris->count > 0) && (atTris->elements != NULL)) {
        ...
        MM_Math3D_TriNorm(&D_801EDE40, &acQuad->dim.quad[2], &acQuad->dim.quad[3], &acQuad->dim.quad[1]);
        MM_Math3D_TriNorm(&D_801EDE78, &acQuad->dim.quad[1], &acQuad->dim.quad[0], &acQuad->dim.quad[2]);

        for (atTrisElem = atTris->elements; atTrisElem < &atTris->elements[atTris->count]; atTrisElem++) {
            if (CollisionCheck_IsElementNotAT(&atTrisElem->base)) {
                continue;
            }
            if (MM_CollisionCheck_NoSharedFlags(&atTrisElem->base, &acQuad->elem)) {
                continue;
            }
            if (MM_Math3D_TriVsTriIntersect(&D_801EDE40, &atTrisElem->dim, &D_801EDE30) ||
                MM_Math3D_TriVsTriIntersect(&D_801EDE78, &atTrisElem->dim, &D_801EDE30)) {
                Vec3f atPos;
                Vec3f acPos;

                CollisionCheck_TrisAvgPoint(atTrisElem, &atPos);
                CollisionCheck_QuadAvgPoint(acQuad, &acPos);
                MM_CollisionCheck_SetATvsAC(play, &atTris->base, &atTrisElem->base, &atPos, &acQuad->base, &acQuad->elem,
                                         &acPos, &D_801EDE30);
                return;
            }
        }
...
void CollisionCheck_AC_TrisVsSphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
    ...
    if ((atTris->count > 0) && (atTris->elements != NULL)) {
        ...
        for (atTrisElem = atTris->elements; atTrisElem < &atTris->elements[atTris->count]; atTrisElem++) {
            ...
            if (MM_CollisionCheck_NoSharedFlags(&atTrisElem->base, &acSph->elem)) {
                continue;
            }
            if (MM_Math3D_TriVsSphIntersect(&acSph->dim.worldSphere, &atTrisElem->dim, &hitPos)) {
                Vec3f atPos;
                Vec3f acPos;

                MM_Math_Vec3s_ToVec3f(&acPos, &acSph->dim.worldSphere.center);
                CollisionCheck_TrisAvgPoint(atTrisElem, &atPos);
                MM_CollisionCheck_SetATvsAC(play, &atTris->base, &atTrisElem->base, &atPos, &acSph->base, &acSph->elem,
                                         &acPos, &hitPos);
            }
        }
    }
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_QuadVsJntSph(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                    Collider* acCol) {
    ColliderJntSphElement* acJntSphElem;
    Vec3f hitPos;
    ...
    if ((acJntSph->count > 0) && (acJntSph->elements != NULL)) {
        ...
        MM_Math3D_TriNorm(&D_801EDEC8, &atQuad->dim.quad[2], &atQuad->dim.quad[3], &atQuad->dim.quad[1]);
        MM_Math3D_TriNorm(&D_801EDF00, &atQuad->dim.quad[2], &atQuad->dim.quad[1], &atQuad->dim.quad[0]);

        for (acJntSphElem = acJntSph->elements; acJntSphElem < &acJntSph->elements[acJntSph->count]; acJntSphElem++) {
            if (CollisionCheck_IsElementNotAC(&acJntSphElem->base)) {
                continue;
            }
            if (MM_CollisionCheck_NoSharedFlags(&atQuad->elem, &acJntSphElem->base)) {
                continue;
            }
            if (MM_Math3D_TriVsSphIntersect(&acJntSphElem->dim.worldSphere, &D_801EDEC8, &hitPos) ||
                MM_Math3D_TriVsSphIntersect(&acJntSphElem->dim.worldSphere, &D_801EDF00, &hitPos)) {
                Vec3f atPos;
                Vec3f acPos;

                if (!MM_Collider_QuadSetNearestAC(play, atQuad, &hitPos)) {
                    continue;
                }
                MM_Math_Vec3s_ToVec3f(&acPos, &acJntSphElem->dim.worldSphere.center);
                CollisionCheck_QuadAvgPoint(atQuad, &atPos);
                MM_CollisionCheck_SetATvsAC(play, &atQuad->base, &atQuad->elem, &atPos, &acJntSph->base,
                                         &acJntSphElem->base, &acPos, &hitPos);
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_QuadVsCyl(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                 Collider* acCol) {
    ColliderQuad* atQuad = (ColliderQuad*)atCol;
    ColliderCylinder* acCyl = (ColliderCylinder*)acCol;
    ...
    if ((acCyl->dim.height > 0) && (acCyl->dim.radius > 0)) {
        ...
        if (MM_CollisionCheck_NoSharedFlags(&atQuad->elem, &acCyl->elem)) {
            return;
        }

        MM_Math3D_TriNorm(&D_801EDF58, &atQuad->dim.quad[2], &atQuad->dim.quad[3], &atQuad->dim.quad[1]);
        MM_Math3D_TriNorm(&D_801EDF90, &atQuad->dim.quad[2], &atQuad->dim.quad[1], &atQuad->dim.quad[0]);

        if (MM_Math3D_CylTriVsIntersect(&acCyl->dim, &D_801EDF58, &D_801EDFE0)) {
            if (MM_Collider_QuadSetNearestAC(play, atQuad, &D_801EDFE0)) {
                Vec3f atPos;
                Vec3f acPos;

                CollisionCheck_QuadAvgPoint(atQuad, &atPos);
                MM_Math_Vec3s_ToVec3f(&acPos, &acCyl->dim.pos);
                MM_CollisionCheck_SetATvsAC(play, &atQuad->base, &atQuad->elem, &atPos, &acCyl->base, &acCyl->elem, &acPos,
                                         &D_801EDFE0);
                return;
            }
        }
        if (MM_Math3D_CylTriVsIntersect(&acCyl->dim, &D_801EDF90, &D_801EDFE0)) {
            if (MM_Collider_QuadSetNearestAC(play, atQuad, &D_801EDFE0)) {
                Vec3f atPos;
                Vec3f acPos;
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_QuadVsTris(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                  Collider* acCol) {
    ColliderQuad* atQuad = (ColliderQuad*)atCol;
    ColliderTris* acTris = (ColliderTris*)acCol;
    ColliderTrisElement* acTrisElem;
    ...
    if ((acTris->count > 0) && (acTris->elements != NULL)) {
        ...
        MM_Math3D_TriNorm(&D_801EE000, &atQuad->dim.quad[2], &atQuad->dim.quad[3], &atQuad->dim.quad[1]);
        MM_Math3D_TriNorm(&D_801EE038, &atQuad->dim.quad[1], &atQuad->dim.quad[0], &atQuad->dim.quad[2]);

        for (acTrisElem = acTris->elements; acTrisElem < &acTris->elements[acTris->count]; acTrisElem++) {
            if (CollisionCheck_IsElementNotAC(&acTrisElem->base)) {
                continue;
            }
            if (MM_CollisionCheck_NoSharedFlags(&atQuad->elem, &acTrisElem->base)) {
                continue;
            }

            if ((MM_Math3D_TriVsTriIntersect(&D_801EE000, &acTrisElem->dim, &D_801EDFF0)) ||
                (MM_Math3D_TriVsTriIntersect(&D_801EE038, &acTrisElem->dim, &D_801EDFF0))) {
                if (MM_Collider_QuadSetNearestAC(play, atQuad, &D_801EDFF0)) {
                    Vec3f atPos;
                    Vec3f acPos;

                    CollisionCheck_TrisAvgPoint(acTrisElem, &acPos);
                    CollisionCheck_QuadAvgPoint(atQuad, &atPos);
                    MM_CollisionCheck_SetATvsAC(play, &atQuad->base, &atQuad->elem, &atPos, &acTris->base,
 ...
 * AC overlap check. Calculates the center of each collider element and the point of contact.
 */
void MM_CollisionCheck_AC_QuadVsQuad(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
                                  Collider* acCol) {
    ColliderQuad* atQuad = (ColliderQuad*)atCol;
    ColliderQuad* acQuad = (ColliderQuad*)acCol;
    s32 i;
    ...
    if (MM_CollisionCheck_NoSharedFlags(&atQuad->elem, &acQuad->elem)) {
        return;
    }

    MM_Math3D_TriNorm(&D_801EE0E8[0], &atQuad->dim.quad[2], &atQuad->dim.quad[3], &atQuad->dim.quad[1]);
    MM_Math3D_TriNorm(&D_801EE0E8[1], &atQuad->dim.quad[2], &atQuad->dim.quad[1], &atQuad->dim.quad[0]);
    MM_Math3D_TriNorm(&D_801EE070[0], &acQuad->dim.quad[2], &acQuad->dim.quad[3], &acQuad->dim.quad[1]);
    MM_Math3D_TriNorm(&D_801EE070[1], &acQuad->dim.quad[2], &acQuad->dim.quad[1], &acQuad->dim.quad[0]);

    for (i = 0; i < 2; i++) {
        for (j = 0; j < 2; j++) {
            if (MM_Math3D_TriVsTriIntersect(&D_801EE0E8[j], &D_801EE070[i], &D_801EE0D8) &&
                MM_Collider_QuadSetNearestAC(play, atQuad, &D_801EE0D8)) {
                Vec3f atPos;
                Vec3f acPos;

                CollisionCheck_QuadAvgPoint(atQuad, &atPos);
                CollisionCheck_QuadAvgPoint(acQuad, &acPos);
                MM_CollisionCheck_SetATvsAC(play, &atQuad->base, &atQuad->elem, &atPos, &acQuad->base, &acQuad->elem,
                                         &acPos, &D_801EE0D8);
                return;
            }
        }
...
void CollisionCheck_AC_QuadVsSphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
    ...
    if (CollisionCheck_IsElementNotAC(&acSph->elem) || MM_CollisionCheck_NoSharedFlags(&atQuad->elem, &acSph->elem)) {
        return;
    }

    MM_Math3D_TriNorm(&D_801EE150, &atQuad->dim.quad[2], &atQuad->dim.quad[3], &atQuad->dim.quad[1]);
    MM_Math3D_TriNorm(&D_801EE188, &atQuad->dim.quad[2], &atQuad->dim.quad[1], &atQuad->dim.quad[0]);

    if ((MM_Math3D_TriVsSphIntersect(&acSph->dim.worldSphere, &D_801EE150, &hitPos)) ||
        (MM_Math3D_TriVsSphIntersect(&acSph->dim.worldSphere, &D_801EE188, &hitPos))) {
        if (MM_Collider_QuadSetNearestAC(play, atQuad, &hitPos)) {
            Vec3f atPos;
            Vec3f acPos;
            ...
            MM_Math_Vec3s_ToVec3f(&acPos, &acSph->dim.worldSphere.center);
            CollisionCheck_QuadAvgPoint(atQuad, &atPos);
            MM_CollisionCheck_SetATvsAC(play, &atQuad->base, &atQuad->elem, &atPos, &acSph->base, &acSph->elem, &acPos,
                                     &hitPos);
        }
    }
}
...
void CollisionCheck_AC_SphereVsJntSph(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
    ...
    if ((acJntSph->count > 0) && (acJntSph->elements != NULL)) {
        ...
        for (acJntSphElem = acJntSph->elements; acJntSphElem < &acJntSph->elements[acJntSph->count]; acJntSphElem++) {
            ...
            if (MM_CollisionCheck_NoSharedFlags(&atSph->elem, &acJntSphElem->base)) {
                continue;
            }
            ...
            if (Math3D_SphVsSphOverlapCenterDist(&atSph->dim.worldSphere, &acJntSphElem->dim.worldSphere, &overlapSize,
                ...
                MM_Math_Vec3s_ToVec3f(&atPos, &atSph->dim.worldSphere.center);
                MM_Math_Vec3s_ToVec3f(&acPos, &acJntSphElem->dim.worldSphere.center);
                if (!IS_ZERO(centerDist)) {
                    acToHit = acJntSphElem->dim.worldSphere.radius / centerDist;
                    hitPos.x = (atPos.x - acPos.x) * acToHit + acPos.x;
                ...
                MM_CollisionCheck_SetATvsAC(play, &atSph->base, &atSph->elem, &atPos, &acJntSph->base, &acJntSphElem->base,
                                         &acPos, &hitPos);
            }
        }
    }
...
void CollisionCheck_AC_SphereVsCylinder(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
    ...
    if (MM_CollisionCheck_NoSharedFlags(&atSph->elem, &acCyl->elem)) {
        return;
    }

    if (MM_Math3D_SphVsCylOverlapCenterDist(&atSph->dim.worldSphere, &acCyl->dim, &overlapSize, &centerDist)) {
        Vec3f hitPos;
        Vec3f atPos;
        Vec3f acPos;
        ...
        MM_CollisionCheck_SetATvsAC(play, &atSph->base, &atSph->elem, &atPos, &acCyl->base, &acCyl->elem, &acPos, &hitPos);
    }
...
void CollisionCheck_AC_SphereVsTris(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
    ...
    for (acTrisElem = acTris->elements; acTrisElem < &acTris->elements[acTris->count]; acTrisElem++) {
        ...
        if (MM_CollisionCheck_NoSharedFlags(&atSph->elem, &acTrisElem->base)) {
            continue;
        }
        if (MM_Math3D_TriVsSphIntersect(&atSph->dim.worldSphere, &acTrisElem->dim, &hitPos)) {
            Vec3f atPos;
            Vec3f acPos;

            MM_Math_Vec3s_ToVec3f(&atPos, &atSph->dim.worldSphere.center);
            CollisionCheck_TrisAvgPoint(acTrisElem, &acPos);
            MM_CollisionCheck_SetATvsAC(play, &atSph->base, &atSph->elem, &atPos, &acTris->base, &acTrisElem->base, &acPos,
                                     &hitPos);
            return;
        }
    }
...
void CollisionCheck_AC_SphereVsQuad(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
    ...
    if (CollisionCheck_IsElementNotAC(&acQuad->elem) || MM_CollisionCheck_NoSharedFlags(&atSph->elem, &acQuad->elem)) {
        return;
    }

    MM_Math3D_TriNorm(&D_801EE6C8.norm1, &acQuad->dim.quad[2], &acQuad->dim.quad[3], &acQuad->dim.quad[1]);
    MM_Math3D_TriNorm(&D_801EE6C8.norm2, &acQuad->dim.quad[1], &acQuad->dim.quad[0], &acQuad->dim.quad[2]);

    if (MM_Math3D_TriVsSphIntersect(&atSph->dim.worldSphere, &D_801EE6C8.norm1, &hitPos) ||
        MM_Math3D_TriVsSphIntersect(&atSph->dim.worldSphere, &D_801EE6C8.norm2, &hitPos)) {
        Vec3f atPos;
        Vec3f acPos;

        MM_Math_Vec3s_ToVec3f(&atPos, &atSph->dim.worldSphere.center);
        CollisionCheck_QuadAvgPoint(acQuad, &acPos);
        MM_CollisionCheck_SetATvsAC(play, &atSph->base, &atSph->elem, &atPos, &acQuad->base, &acQuad->elem, &acPos,
                                 &hitPos);
    }
}
...
void CollisionCheck_AC_SphereVsSphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol,
    ...
    if (MM_CollisionCheck_NoSharedFlags(&atSph->elem, &acSph->elem)) {
        return;
    }
    ...
    if (Math3D_SphVsSphOverlapCenterDist(&atSph->dim.worldSphere, &acSph->dim.worldSphere, &overlapSize, &centerDist)) {
        ...
        MM_CollisionCheck_SetATvsAC(play, &atSph->base, &atSph->elem, &atPos, &acSph->base, &acSph->elem, &acPos, &hitPos);
    }
}
 ...
 */
void MM_CollisionCheck_SetJntSphHitFX(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ColliderJntSph* jntSph = (ColliderJntSph*)col;
    ColliderJntSphElement* jntSphElem;
    ...
    for (jntSphElem = jntSph->elements; jntSphElem < &jntSph->elements[jntSph->count]; jntSphElem++) {
        if ((jntSphElem->base.acElemFlags & ACELEM_DRAW_HITMARK) && (jntSphElem->base.acHitElem != NULL) &&
            ...
            MM_Math_Vec3s_ToVec3f(&hitPos, &jntSphElem->base.acDmgInfo.hitPos);
            MM_CollisionCheck_HitEffects(play, jntSphElem->base.acHit, jntSphElem->base.acHitElem, &jntSph->base,
 ...
 */
void MM_CollisionCheck_SetCylHitFX(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ColliderCylinder* cyl = (ColliderCylinder*)col;
    ...
    if ((cyl->elem.acElemFlags & ACELEM_DRAW_HITMARK) && (cyl->elem.acHitElem != NULL) &&
        ...
        MM_Math_Vec3s_ToVec3f(&hitPos, &cyl->elem.acDmgInfo.hitPos);
        MM_CollisionCheck_HitEffects(play, cyl->elem.acHit, cyl->elem.acHitElem, &cyl->base, &cyl->elem, &hitPos);
        cyl->elem.acHitElem->atElemFlags |= ATELEM_DREW_HITMARK;
    }
}
 ...
 */
void MM_CollisionCheck_SetTrisHitFX(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ColliderTris* tris = (ColliderTris*)col;
    ColliderTrisElement* trisElem;
    ...
    for (trisElem = tris->elements; trisElem < &tris->elements[tris->count]; trisElem++) {
        if ((trisElem->base.acElemFlags & ACELEM_DRAW_HITMARK) && (trisElem->base.acHitElem != NULL) &&
            ...
            MM_Math_Vec3s_ToVec3f(&hitPos, &trisElem->base.acDmgInfo.hitPos);
            MM_CollisionCheck_HitEffects(play, trisElem->base.acHit, trisElem->base.acHitElem, &tris->base,
 ...
 */
void MM_CollisionCheck_SetQuadHitFX(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ColliderQuad* quad = (ColliderQuad*)col;
    ...
    if ((quad->elem.acElemFlags & ACELEM_DRAW_HITMARK) && (quad->elem.acHitElem != NULL) &&
        ...
        MM_Math_Vec3s_ToVec3f(&hitPos, &quad->elem.acDmgInfo.hitPos);
        MM_CollisionCheck_HitEffects(play, quad->elem.acHit, quad->elem.acHitElem, &quad->base, &quad->elem, &hitPos);
        quad->elem.acHitElem->atElemFlags |= ATELEM_DREW_HITMARK;
    }
}
...
void CollisionCheck_SetSphereHitFX(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ...
    if ((sph->elem.acElemFlags & ACELEM_DRAW_HITMARK) && (sph->elem.acHitElem != NULL) &&
        ...
        MM_Math_Vec3s_ToVec3f(&hitPos, &sph->elem.acDmgInfo.hitPos);
        MM_CollisionCheck_HitEffects(play, sph->elem.acHit, sph->elem.acHitElem, &sph->base, &sph->elem, &hitPos);
        sph->elem.acHitElem->atElemFlags |= ATELEM_DREW_HITMARK;
    }
}

ColChkApplyFunc MM_sColChkApplyFuncs[COLSHAPE_MAX] = {
    MM_CollisionCheck_SetJntSphHitFX, // COLSHAPE_JNTSPH
    MM_CollisionCheck_SetCylHitFX,    // COLSHAPE_CYLINDER
    MM_CollisionCheck_SetTrisHitFX,   // COLSHAPE_TRIS
    MM_CollisionCheck_SetQuadHitFX,   // COLSHAPE_QUAD
 ...
 */
void MM_CollisionCheck_SetHitEffects(struct PlayState* play, CollisionCheckContext* colChkCtx) {
    Collider** acColP;
    ...
    for (acColP = &colChkCtx->colAC[0]; acColP < &colChkCtx->colAC[colChkCtx->colACCount]; acColP++) {
        ...
        if ((acCol != NULL) && (acCol->acFlags & AC_ON)) {
            ...
            MM_sColChkApplyFuncs[acCol->shape](play, colChkCtx, acCol);
        }
    }
}

ColChkVsFunc MM_sACVsFuncs[COLSHAPE_MAX][COLSHAPE_MAX] = {
    // COLSHAPE_JNTSPH
    {
        MM_CollisionCheck_AC_JntSphVsJntSph, // COLSHAPE_JNTSPH
        MM_CollisionCheck_AC_JntSphVsCyl,    // COLSHAPE_CYLINDER
        MM_CollisionCheck_AC_JntSphVsTris,   // COLSHAPE_TRIS
        MM_CollisionCheck_AC_JntSphVsQuad,   // COLSHAPE_QUAD
        CollisionCheck_AC_JntSphVsSphere  // COLSHAPE_SPHERE
    },
    // COLSHAPE_CYLINDER
    {
        MM_CollisionCheck_AC_CylVsJntSph, // COLSHAPE_JNTSPH
        MM_CollisionCheck_AC_CylVsCyl,    // COLSHAPE_CYLINDER
        MM_CollisionCheck_AC_CylVsTris,   // COLSHAPE_TRIS
        MM_CollisionCheck_AC_CylVsQuad,   // COLSHAPE_QUAD
        CollisionCheck_AC_CylVsSphere  // COLSHAPE_SPHERE
    },
    // COLSHAPE_TRIS
    {
        MM_CollisionCheck_AC_TrisVsJntSph, // COLSHAPE_JNTSPH
        MM_CollisionCheck_AC_TrisVsCyl,    // COLSHAPE_CYLINDER
        MM_CollisionCheck_AC_TrisVsTris,   // COLSHAPE_TRIS
        MM_CollisionCheck_AC_TrisVsQuad,   // COLSHAPE_QUAD
        CollisionCheck_AC_TrisVsSphere  // COLSHAPE_SPHERE
    },
    // COLSHAPE_QUAD
    {
        MM_CollisionCheck_AC_QuadVsJntSph, // COLSHAPE_JNTSPH
        MM_CollisionCheck_AC_QuadVsCyl,    // COLSHAPE_CYLINDER
        MM_CollisionCheck_AC_QuadVsTris,   // COLSHAPE_TRIS
        MM_CollisionCheck_AC_QuadVsQuad,   // COLSHAPE_QUAD
 ...
 */
void MM_CollisionCheck_AC(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* atCol) {
    Collider** acColP;
    ...
    for (acColP = &colChkCtx->colAC[0]; acColP < &colChkCtx->colAC[colChkCtx->colACCount]; acColP++) {
        ...
        if ((acCol != NULL) && (acCol->acFlags & AC_ON)) {
            ...
            if ((acCol->acFlags & atCol->atFlags & AC_TYPE_ALL) && (atCol != acCol)) {
                ...
                MM_sACVsFuncs[atCol->shape][acCol->shape](play, colChkCtx, atCol, acCol);
            }
        }
    }
}
 ...
 */
void MM_CollisionCheck_AT(struct PlayState* play, CollisionCheckContext* colChkCtx) {
    Collider** acColP;
    ...
    for (acColP = &colChkCtx->colAT[0]; acColP < &colChkCtx->colAT[colChkCtx->colATCount]; acColP++) {
        ...
        if ((acCol != NULL) && (acCol->atFlags & AT_ON)) {
            ...
            MM_CollisionCheck_AC(play, colChkCtx, acCol);
        }
    }

    MM_CollisionCheck_SetHitEffects(play, colChkCtx);
}
 ...
 * colliders.
 */
s32 MM_CollisionCheck_GetMassType(u8 mass) {
 ...
 */
void MM_CollisionCheck_SetOCvsOC(struct PlayState* play, Collider* leftCol, ColliderElement* leftElem, Vec3f* leftPos,
    ...
    rightMassType = MM_CollisionCheck_GetMassType(leftActor->colChkInfo.mass);
    leftMassType = MM_CollisionCheck_GetMassType(rightActor->colChkInfo.mass);
    ...
    xzDist = MM_sqrtf(SQ(xDelta) + SQ(zDelta));
 ...
 */
void MM_CollisionCheck_OC_JntSphVsJntSph(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* leftCol,
                                      Collider* rightCol) {
    ...
    if ((leftJntSph->count > 0) && (leftJntSph->elements != NULL) && (rightJntSph->count > 0) &&
        ...
        for (leftJntSphElem = leftJntSph->elements; leftJntSphElem < &leftJntSph->elements[leftJntSph->count];
            ...
            for (rightJntSphElem = rightJntSph->elements; rightJntSphElem < &rightJntSph->elements[rightJntSph->count];
                ...
                if (MM_Math3D_SphVsSphOverlap(&leftJntSphElem->dim.worldSphere, &rightJntSphElem->dim.worldSphere,
                                           &overlapSize)) {
                    Vec3f leftPos;
                    Vec3f rightPos;

                    MM_Math_Vec3s_ToVec3f(&leftPos, &leftJntSphElem->dim.worldSphere.center);
                    MM_Math_Vec3s_ToVec3f(&rightPos, &rightJntSphElem->dim.worldSphere.center);
                    MM_CollisionCheck_SetOCvsOC(play, &leftJntSph->base, &leftJntSphElem->base, &leftPos,
 ...
 */
void MM_CollisionCheck_OC_JntSphVsCyl(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* leftCol,
                                   Collider* rightCol) {
    ColliderJntSph* leftJntSph = (ColliderJntSph*)leftCol;
    ...
    if ((leftJntSph->count > 0) && (leftJntSph->elements != NULL) && (leftJntSph->base.ocFlags1 & OCELEM_ON) &&
        ...
        for (leftJntSphElem = leftJntSph->elements; leftJntSphElem < &leftJntSph->elements[leftJntSph->count];
            ...
            if (Math3D_SphVsCylOverlap(&leftJntSphElem->dim.worldSphere, &rightCyl->dim, &overlapSize)) {
                ...
                MM_Math_Vec3s_ToVec3f(&leftPos, &leftJntSphElem->dim.worldSphere.center);
                MM_Math_Vec3s_ToVec3f(&rightPos, &rightCyl->dim.pos);
                MM_CollisionCheck_SetOCvsOC(play, &leftJntSph->base, &leftJntSphElem->base, &leftPos, &rightCyl->base,
...
void CollisionCheck_OC_JntSphVsSphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* leftCol,
    ...
    if ((leftJntSph->count > 0) && (leftJntSph->elements != NULL) && (leftJntSph->base.ocFlags1 & OCELEM_ON) &&
        ...
        for (leftJntSphElem = leftJntSph->elements; leftJntSphElem < &leftJntSph->elements[leftJntSph->count];
            ...
            if (MM_Math3D_SphVsSphOverlap(&leftJntSphElem->dim.worldSphere, &rightSph->dim.worldSphere, &overlapSize)) {
                Vec3f leftPos;
                Vec3f rightPos;

                MM_Math_Vec3s_ToVec3f(&leftPos, &leftJntSphElem->dim.worldSphere.center);
                MM_Math_Vec3s_ToVec3f(&rightPos, &rightSph->dim.worldSphere.center);
                MM_CollisionCheck_SetOCvsOC(play, &leftJntSph->base, &leftJntSphElem->base, &leftPos, &rightSph->base,
 ...
 */
void MM_CollisionCheck_OC_CylVsJntSph(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* leftCol,
                                   Collider* rightCol) {
    MM_CollisionCheck_OC_JntSphVsCyl(play, colChkCtx, rightCol, leftCol);
}

/**
 * OC overlap check for two Cylinders
 */
void MM_CollisionCheck_OC_CylVsCyl(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* leftCol,
                                Collider* rightCol) {
    ...
    if ((leftCyl->base.ocFlags1 & OCELEM_ON) && (rightCyl->base.ocFlags1 & OCELEM_ON) &&
        ...
        if (Math3D_CylVsCylOverlap(&leftCyl->dim, &rightCyl->dim, &overlapSize)) {
            ...
            MM_Math_Vec3s_ToVec3f(&leftPos, &leftCyl->dim.pos);
            MM_Math_Vec3s_ToVec3f(&rightPos, &rightCyl->dim.pos);
            MM_CollisionCheck_SetOCvsOC(play, &leftCyl->base, &leftCyl->elem, &leftPos, &rightCyl->base, &rightCyl->elem,
...
void CollisionCheck_OC_CylVsSphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* leftCol,
    ...
    if ((leftCyl->base.ocFlags1 & OCELEM_ON) && (leftCyl->elem.ocElemFlags & OCELEM_ON) &&
        ...
        if (Math3D_SphVsCylOverlap(&rightSph->dim.worldSphere, &leftCyl->dim, &overlapSize)) {
            ...
            MM_Math_Vec3s_ToVec3f(&leftPos, &leftCyl->dim.pos);
            MM_Math_Vec3s_ToVec3f(&rightPos, &rightSph->dim.worldSphere.center);
            MM_CollisionCheck_SetOCvsOC(play, &leftCyl->base, &leftCyl->elem, &leftPos, &rightSph->base, &rightSph->elem,
                                     &rightPos, overlapSize);
        }
    }
}
...
void CollisionCheck_OC_SphereVsSphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* leftCol,
    ...
    if ((leftSph->base.ocFlags1 & OCELEM_ON) && (leftSph->elem.ocElemFlags & OCELEM_ON) &&
        (rightSph->base.ocFlags1 & OCELEM_ON) && (rightSph->elem.ocElemFlags & OCELEM_ON)) {
        if (MM_Math3D_SphVsSphOverlap(&leftSph->dim.worldSphere, &rightSph->dim.worldSphere, &overlapSize)) {
            Vec3f leftPos;
            Vec3f rightPos;

            MM_Math_Vec3s_ToVec3f(&leftPos, &leftSph->dim.worldSphere.center);
            MM_Math_Vec3s_ToVec3f(&rightPos, &rightSph->dim.worldSphere.center);
            MM_CollisionCheck_SetOCvsOC(play, &leftSph->base, &leftSph->elem, &leftPos, &rightSph->base, &rightSph->elem,
                                     &rightPos, overlapSize);
        }
    }
}
 ...
 */
s32 MM_CollisionCheck_SkipOC(Collider* col) {
 ...
 */
s32 MM_CollisionCheck_Incompatible(Collider* leftCol, Collider* rightCol) {
...
ColChkVsFunc MM_sOCVsFuncs[COLSHAPE_MAX][COLSHAPE_MAX] = {
    // COLSHAPE_JNTSPH
    {
        MM_CollisionCheck_OC_JntSphVsJntSph, // COLSHAPE_JNTSPH
        MM_CollisionCheck_OC_JntSphVsCyl,    // COLSHAPE_CYLINDER
        NULL,                             // COLSHAPE_TRIS
        NULL,                             // COLSHAPE_QUAD
    ...
    // COLSHAPE_CYLINDER
    {
        MM_CollisionCheck_OC_CylVsJntSph, // COLSHAPE_JNTSPH
        MM_CollisionCheck_OC_CylVsCyl,    // COLSHAPE_CYLINDER
        NULL,                          // COLSHAPE_TRIS
        NULL,                          // COLSHAPE_QUAD
 ...
 */
void MM_CollisionCheck_OC(struct PlayState* play, CollisionCheckContext* colChkCtx) {
    Collider** leftColP;
    Collider** rightColP;
    ColChkVsFunc vsFunc;

    for (leftColP = colChkCtx->colOC; leftColP < colChkCtx->colOC + colChkCtx->colOCCount; leftColP++) {
        if ((*leftColP == NULL) || MM_CollisionCheck_SkipOC(*leftColP)) {
            continue;
        }
        for (rightColP = leftColP + 1; rightColP < colChkCtx->colOC + colChkCtx->colOCCount; rightColP++) {
            if ((*rightColP == NULL) || MM_CollisionCheck_SkipOC(*rightColP) ||
                MM_CollisionCheck_Incompatible(*leftColP, *rightColP)) {
                continue;
            }
            vsFunc = MM_sOCVsFuncs[(*leftColP)->shape][(*rightColP)->shape];
            if (vsFunc == NULL) {
                continue;
            }
 ...
 */
void MM_CollisionCheck_InitInfo(CollisionCheckInfo* info) {
 ...
 */
void MM_CollisionCheck_ResetDamage(CollisionCheckInfo* info) {
    info->damage = 0;
    info->damageEffect = 0;
    info->atHitEffect = 0;
    info->acHitEffect = 0;
 ...
 */
void MM_CollisionCheck_SetInfoNoDamageTable(CollisionCheckInfo* info, CollisionCheckInfoInit* init) {
 ...
 */
void MM_CollisionCheck_SetInfo(CollisionCheckInfo* info, DamageTable* damageTable, CollisionCheckInfoInit* init) {
    info->health = init->health;
 ...
 */
void MM_CollisionCheck_SetInfo2(CollisionCheckInfo* info, DamageTable* damageTable, CollisionCheckInfoInit2* init) {
    info->health = init->health;
    info->damageTable = damageTable;
    info->cylRadius = init->cylRadius;
    info->cylHeight = init->cylHeight;
 ...
 */
void MM_CollisionCheck_SetInfoGetDamageTable(CollisionCheckInfo* info, s32 index, CollisionCheckInfoInit2* init) {
    MM_CollisionCheck_SetInfo2(info, MM_DamageTable_Get(index), init);
}

/**
 * Apply AC damage effect
 */
void MM_CollisionCheck_ApplyDamage(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col,
 ...
 */
void MM_CollisionCheck_ApplyDamageJntSph(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ColliderJntSph* jntSph = (ColliderJntSph*)col;
    s32 i;

    if ((jntSph->count > 0) && (jntSph->elements != NULL)) {
        for (i = 0; i < jntSph->count; i++) {
            MM_CollisionCheck_ApplyDamage(play, colChkCtx, &jntSph->base, &jntSph->elements[i].base);
        }
    }
}
 ...
 */
void MM_CollisionCheck_ApplyDamageCyl(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ColliderCylinder* cyl = (ColliderCylinder*)col;

    MM_CollisionCheck_ApplyDamage(play, colChkCtx, &cyl->base, &cyl->elem);
}

/**
 * Apply ColliderTris AC damage effect
 */
void MM_CollisionCheck_ApplyDamageTris(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ColliderTris* tris = (ColliderTris*)col;
    s32 i;
    ...
    for (i = 0; i < tris->count; i++) {
        MM_CollisionCheck_ApplyDamage(play, colChkCtx, &tris->base, &tris->elements[i].base);
    }
}
 ...
 */
void MM_CollisionCheck_ApplyDamageQuad(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ColliderQuad* quad = (ColliderQuad*)col;

    MM_CollisionCheck_ApplyDamage(play, colChkCtx, &quad->base, &quad->elem);
}
...
void CollisionCheck_ApplyDamageSphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col) {
    ...
    MM_CollisionCheck_ApplyDamage(play, colChkCtx, &sph->base, &sph->elem);
}

ColChkApplyFunc MM_sApplyDamageFuncs[COLSHAPE_MAX] = {
    MM_CollisionCheck_ApplyDamageJntSph, // COLSHAPE_JNTSPH
    MM_CollisionCheck_ApplyDamageCyl,    // COLSHAPE_CYLINDER
    MM_CollisionCheck_ApplyDamageTris,   // COLSHAPE_TRIS
    MM_CollisionCheck_ApplyDamageQuad,   // COLSHAPE_QUAD
 ...
 * CollisionCheckInfo.
 */
void MM_CollisionCheck_Damage(struct PlayState* play, CollisionCheckContext* colChkCtx) {
    s32 i;
    ...
    for (i = 0; i < colChkCtx->colACCount; i++) {
        ...
        MM_sApplyDamageFuncs[col->shape](play, colChkCtx, col);
    }
}
 ...
 */
s32 MM_CollisionCheck_LineOC_JntSph(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col, Vec3f* a,
                                 Vec3f* b) {
    ColliderJntSph* jntSph = (ColliderJntSph*)col;
    s32 i;
    ...
    for (i = 0; i < jntSph->count; i++) {
        ...
        D_801EDEB0.a = *a;
        D_801EDEB0.b = *b;
        if (MM_Math3D_LineVsSph(&jntSphElem->dim.worldSphere, &D_801EDEB0)) {
            return true;
        }
    }
    return false;
 ...
 */
s32 MM_CollisionCheck_LineOC_Cyl(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col, Vec3f* a,
                              Vec3f* b) {
    ColliderCylinder* cyl = (ColliderCylinder*)col;
    ...
    if (MM_Math3D_CylVsLineSeg(&cyl->dim, a, b, &D_801EDF38, &D_801EDF48)) {
        return true;
    }
...
s32 CollisionCheck_LineOC_Sphere(struct PlayState* play, CollisionCheckContext* colChkCtx, Collider* col, Vec3f* a,
    ...
    D_801EDFC8.a = *a;
    D_801EDFC8.b = *b;
    if (MM_Math3D_LineVsSph(&sph->dim.worldSphere, &D_801EDFC8)) {
        return true;
    }
...
static ColChkLineFunc MM_sOCLineCheckFuncs[COLSHAPE_MAX] = {
    MM_CollisionCheck_LineOC_JntSph, // COLSHAPE_JNTSPH
    MM_CollisionCheck_LineOC_Cyl,    // COLSHAPE_CYLINDER
    NULL,                         // COLSHAPE_TRIS
    NULL,                         // COLSHAPE_QUAD
 ...
 */
s32 MM_CollisionCheck_LineOC(struct PlayState* play, CollisionCheckContext* colChkCtx, Vec3f* a, Vec3f* b,
    ...
    for (colP = colChkCtx->colOC; colP < &colChkCtx->colOC[colChkCtx->colOCCount]; colP++) {
        if (MM_CollisionCheck_SkipOC(*colP)) {
            continue;
        }
        ...
        lineCheck = MM_sOCLineCheckFuncs[(*colP)->shape];
        if (lineCheck == NULL) {
            continue;
        }
 ...
 * otherwise.
 */
s32 MM_CollisionCheck_LineOCCheckAll(struct PlayState* play, CollisionCheckContext* colChkCtx, Vec3f* a, Vec3f* b) {
    return MM_CollisionCheck_LineOC(play, colChkCtx, a, b, NULL, 0);
}
 ...
 */
s32 MM_CollisionCheck_LineOCCheck(struct PlayState* play, CollisionCheckContext* colChkCtx, Vec3f* a, Vec3f* b,
                               Actor** exclusions, s32 numExclusions) {
    return MM_CollisionCheck_LineOC(play, colChkCtx, a, b, exclusions, numExclusions);
}

/**
 * Moves the ColliderCylinder's position to the actor's position
 */
void MM_Collider_UpdateCylinder(Actor* actor, ColliderCylinder* cyl) {
 ...
 */
void MM_Collider_SetCylinderPosition(ColliderCylinder* cyl, Vec3s* pos) {
    cyl->dim.pos.x = pos->x;
    cyl->dim.pos.y = pos->y;
    cyl->dim.pos.z = pos->z;
}
 ...
 */
void MM_Collider_SetQuadVertices(ColliderQuad* quad, Vec3f* a, Vec3f* b, Vec3f* c, Vec3f* d) {
    MM_Math_Vec3f_Copy(&quad->dim.quad[2], c);
    MM_Math_Vec3f_Copy(&quad->dim.quad[3], d);
    MM_Math_Vec3f_Copy(&quad->dim.quad[0], a);
    MM_Math_Vec3f_Copy(&quad->dim.quad[1], b);
    MM_Collider_SetQuadMidpoints(&quad->dim);
}

/**
 * Sets the specified ColliderTrisElement's vertices
 */
void MM_Collider_SetTrisVertices(ColliderTris* tris, s32 elemIndex, Vec3f* a, Vec3f* b, Vec3f* c) {
    ...
    MM_Math_Vec3f_Copy(&trisElem->dim.vtx[0], a);
    MM_Math_Vec3f_Copy(&trisElem->dim.vtx[1], b);
    MM_Math_Vec3f_Copy(&trisElem->dim.vtx[2], c);
    MM_Math3D_DefPlane(a, b, c, &nx, &ny, &nz, &originDist);
 ...
 */
void MM_Collider_SetTrisDim(struct PlayState* play, ColliderTris* dest, s32 elemIndex, ColliderTrisElementDimInit* src) {
    ColliderTrisElement* trisElem = &dest->elements[elemIndex];

    MM_Collider_SetTrisElementDim(play, &trisElem->dim, src);
}

/**
 * Updates the world spheres for all of the collider's JntSph elements attached to the specified limb
 */
void MM_Collider_UpdateSpheres(s32 limb, ColliderJntSph* jntSph) {
    ...
    for (i = 0; i < jntSph->count; i++) {
        if (limb == jntSph->elements[i].dim.limb) {
            ...
            MM_Matrix_MultVec3f(&D_801EE1C0, &D_801EE1D0);
...
void Collider_UpdateSphere(s32 limb, ColliderSphere* sph) {
    ...
    if (limb == sph->dim.limb) {
        ...
        MM_Matrix_MultVec3f(&D_801EE1E0, &D_801EE1F0);
 ...
 */
void MM_CollisionCheck_SpawnRedBlood(struct PlayState* play, Vec3f* v) {
    ...
    MM_Effect_Add(play, &effectIndex, EFFECT_SPARK, 0, 1, &D_801EE200);
}
 ...
 */
void MM_CollisionCheck_SpawnWaterDroplets(struct PlayState* play, Vec3f* v) {
    ...
    MM_Effect_Add(play, &effectIndex, EFFECT_SPARK, 0, 1, &D_801EE738);
}

/**
 * Spawns streaks of light from hits against solid objects
 */
void MM_CollisionCheck_SpawnShieldParticles(struct PlayState* play, Vec3f* v) {
    ...
    MM_Effect_Add(play, &effectIndex, EFFECT_SHIELD_PARTICLE, 0, 1, &sShieldParticleInitMetal);
}

/**
 * Spawns streaks of light and makes a metallic sound
 */
void MM_CollisionCheck_SpawnShieldParticlesMetal(struct PlayState* play, Vec3f* v) {
    MM_CollisionCheck_SpawnShieldParticles(play, v);
 ...
 */
void MM_CollisionCheck_SpawnShieldParticlesMetalSound(struct PlayState* play, Vec3f* v, Vec3f* pos) {
    MM_CollisionCheck_SpawnShieldParticles(play, v);
 ...
 * Spawns streaks of light and makes a metallic sound
 */
void MM_CollisionCheck_SpawnShieldParticlesMetal2(struct PlayState* play, Vec3f* v) {
    MM_CollisionCheck_SpawnShieldParticlesMetal(play, v);
}

/**
 * Spawns streaks of light and makes a wooden sound
 */
void MM_CollisionCheck_SpawnShieldParticlesWood(struct PlayState* play, Vec3f* v, Vec3f* pos) {
    ...
    MM_Effect_Add(play, &effectIndex, EFFECT_SHIELD_PARTICLE, 0, 1, &sShieldParticleInitWood);
 ...
 * are not detected.
 */
s32 MM_CollisionCheck_CylSideVsLineSeg(f32 radius, f32 height, f32 offset, Vec3f* actorPos, Vec3f* itemPos,
    ...
    if ((actorToItem.y > 0.0f) && (actorToItem.y < height) && (MM_sqrtf(SQXZ(actorToItem)) < radius)) {
        return 3;
    }

    if ((actorToItemProj.y > 0.0f) && (actorToItemProj.y < height) && (MM_sqrtf(SQXZ(actorToItemProj)) < radius)) {
        return 3;
    }
    ...
    if (!IS_ZERO(SQXZ(itemStep))) {
        ...
        if ((SQ(actorDotItemXZ) - (4.0f * SQXZ(itemStep) * radSqDiff)) > MM_zero) {
            intersect1 = intersect2 = true;
        } else {
            intersect1 = true;
            intersect2 = false;
        }
        closeDist = MM_sqrtf(SQ(actorDotItemXZ) - (4.0f * SQXZ(itemStep) * radSqDiff));
        if (intersect1) {
