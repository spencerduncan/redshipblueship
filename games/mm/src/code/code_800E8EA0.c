...
s32 Actor_TrackNone(Vec3s* headRot, Vec3s* torsoRot) {
    MM_Math_SmoothStepToS(&headRot->y, 0, 6, 0x1838, 0x64);
    MM_Math_SmoothStepToS(&headRot->x, 0, 6, 0x1838, 0x64);
    MM_Math_SmoothStepToS(&torsoRot->y, 0, 6, 0x1838, 0x64);
    MM_Math_SmoothStepToS(&torsoRot->x, 0, 6, 0x1838, 0x64);
    return true;
}
...
s32 Actor_TrackPoint(Actor* actor, Vec3f* target, Vec3s* headRot, Vec3s* torsoRot) {
    ...
    targetPitch = MM_Math_Vec3f_Pitch(&actor->focus.pos, target);
    targetYaw = MM_Math_Vec3f_Yaw(&actor->focus.pos, target) - actor->world.rot.y;

    MM_Math_SmoothStepToS(&headRot->x, targetPitch, 6, 0x7D0, 1);
    headRot->x = CLAMP(headRot->x, -0x1770, 0x1770);

    yawDiff = MM_Math_SmoothStepToS(&headRot->y, targetYaw, 6, 0x7D0, 1);
    ...
    MM_Math_SmoothStepToS(&torsoRot->y, targetYaw - headRot->y, 4, 0x7D0, 1);
 ...
 *
 * @return true if rotated towards player, false if rotations were stepped back to MM_zero.
 *
 ...
 * head and torso towards the player if within a certain yaw, else smoothly returns the rotations back to MM_zero.
