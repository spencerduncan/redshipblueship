...
ActiveSequence MM_gActiveSeqs[5];
...
u8 MM_sNumSeqRequests[5];
u32 MM_sAudioSeqCmds[0x100];
...
GfxMasterList MM_D_0E000000;
Mtx D_01000000;
u16 MM_D_0F000000[SCREEN_WIDTH * SCREEN_HEIGHT];
...
s16 MM_sins(u16 x) {
    ...
    if (x & 0x400) {
        value = MM_sintable[0x3FF - (x & 0x3FF)];
    } else {
        value = MM_sintable[x & 0x3FF];
    }
...
s16 MM_coss(u16 angle) {
    return MM_sins(angle + 0x4000);
}
