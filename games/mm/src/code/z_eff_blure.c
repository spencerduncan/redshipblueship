...
void MM_EffectBlure_AddVertex(EffectBlure* this, Vec3f* p1, Vec3f* p2) {
    ...
    if (this != NULL) {
        ...
        if (!(this->flags & EFFECT_BLURE_FLAG_2)) {
            ...
            MM_Math_Vec3f_Diff(&sp160, &sp16C, &sp154);
            scale = MM_Math3D_Vec3fMagnitude(&sp154);
            if (!IS_ZERO(scale)) {
                scale = 1.0f / scale;
                MM_Math_Vec3f_Scale(&sp154, scale);

                MM_SkinMatrix_SetTranslate(&sp110, sp160.x, sp160.y, sp160.z);
                SkinMatrix_SetRotateAroundVec(&spD0, this->addAngle, sp154.x, sp154.y, sp154.z);
                MM_SkinMatrix_MtxFMtxFMult(&sp110, &spD0, &sp90);
                MM_SkinMatrix_SetTranslate(&sp110, -sp160.x, -sp160.y, -sp160.z);
                MM_SkinMatrix_MtxFMtxFMult(&sp90, &sp110, &sp50);
                MM_SkinMatrix_Vec3fMtxFMultXYZ(&sp50, p1, &sp38);
                MM_SkinMatrix_Vec3fMtxFMultXYZ(&sp50, p2, &sp44);
...
void MM_EffectBlure_AddSpace(EffectBlure* this) {
    EffectBlureElement* elem;
    s32 pad;
...
void MM_EffectBlure_InitElements(EffectBlure* this) {
    EffectBlureElement* elem;
    s32 i;
...
void MM_EffectBlure_Init1(void* thisx, void* initParamsx) {
    ...
    if ((this != NULL) && (initParams != NULL)) {
        ...
        MM_EffectBlure_InitElements(this);
...
void MM_EffectBlure_UpdateFlags(EffectBlureElement* elem) {
    Vec3f sp64;
    Vec3f sp58;
    Vec3f sp4C;
    Vec3f sp40;
    ...
    if (((elem - 1)->state == 0) || ((elem + 1)->state == 0)) {
        ...
        MM_Math_Vec3s_DiffToVec3f(&sp64, &elem->p1, &prev->p1);
        MM_Math_Vec3s_DiffToVec3f(&sp58, &elem->p2, &prev->p2);
        MM_Math_Vec3s_DiffToVec3f(&sp4C, &next->p1, &elem->p1);
        MM_Math_Vec3s_DiffToVec3f(&sp40, &next->p2, &elem->p2);

        if (MM_Math3D_CosOut(&sp64, &sp4C, &sp34) || MM_Math3D_CosOut(&sp58, &sp40, &sp30) ||
            MM_Math3D_CosOut(&sp4C, &sp40, &sp2C)) {
            elem->flags &= ~(EFFECT_BLURE_ELEMENT_FLAG_1 | EFFECT_BLURE_ELEMENT_FLAG_2);
            elem->flags |= 0;
...
void MM_EffectBlure_GetComputedValues(EffectBlure* this, s32 index, f32 ratio, Vec3s* vec1, Vec3s* vec2,
        ...
        if (player != NULL) {
            ...
            for (int i = 0; i < 3; i++) {
                if (player->meleeWeaponEffectIndex[i] != -1 &&
                    this == (EffectBlure*)MM_Effect_GetByIndex(player->meleeWeaponEffectIndex[i])) {
...
void MM_EffectBlure_SetupSmooth(EffectBlure* this, GraphicsContext* gfxCtx) {
    OPEN_DISPS(gfxCtx);
...
void MM_EffectBlure_DrawElemNoInterpolation(EffectBlure* this, EffectBlureElement* elem, s32 index,
    ...
    MM_Math_Vec3s_ToVec3f(&sp6C, &this->elements[0].p2);
    ...
    if (vtx == NULL) {
        ...
        MM_EffectBlure_GetComputedValues(this, index, ratio, &sp8C, &sp84, &sp7C, &sp78);

        sp60.x = sp84.x;
        sp60.y = sp84.y;
        sp60.z = sp84.z;
        MM_Math_Vec3f_Diff(&sp60, &sp6C, &sp54);
        MM_Math_Vec3f_Scale(&sp54, 10.0f);
        vtx[0].v.ob[0] = sp54.x;
        vtx[0].v.ob[1] = sp54.y;
        vtx[0].v.ob[2] = sp54.z;
        vtx[0].v.cn[0] = sp78.r;
        ...
        MM_EffectBlure_GetComputedValues(this, index + 1, ratio, &sp8C, &sp84, &sp7C, &sp78);
...
void MM_EffectBlure_DrawElemHermiteInterpolation(EffectBlure* this, EffectBlureElement* elem, s32 index,
                                              GraphicsContext* gfxCtx) {
    static Vtx_t baseVtx = VTX_T(0, 0, 0, 0, 0, 255, 255, 255, 255);
    Vtx* vtx;
    Vec3s sp1EC;
    ...
    MM_Math_Vec3s_ToVec3f(&sp138, &this->elements[0].p2);

    ratio = (f32)elem->timer / (f32)this->elemDuration;
    MM_EffectBlure_GetComputedValues(this, index, ratio, &sp1EC, &sp1E4, &sp1A4, &sp1A0);
    MM_Math_Vec3s_ToVec3f(&sp1CC, &sp1EC);
    MM_Math_Vec3s_ToVec3f(&sp1C0, &sp1E4);

    ratio = (f32)(elem + 1)->timer / (f32)this->elemDuration;
    MM_EffectBlure_GetComputedValues(this, index + 1, ratio, &sp1EC, &sp1E4, &sp19C, &sp198);
    MM_Math_Vec3s_ToVec3f(&sp18C, &sp1EC);
    MM_Math_Vec3s_ToVec3f(&sp180, &sp1E4);

    if ((elem->flags & (EFFECT_BLURE_ELEMENT_FLAG_1 | EFFECT_BLURE_ELEMENT_FLAG_2)) == EFFECT_BLURE_ELEMENT_FLAG_2) {
        MM_Math_Vec3f_Diff(&sp18C, &sp1CC, &sp1B4);
        MM_Math_Vec3f_Diff(&sp180, &sp1C0, &sp1A8);
    } else {
        Vec3f sp118;
        Vec3f sp10C;

        ratio = (f32)(elem - 1)->timer / (f32)this->elemDuration;
        MM_EffectBlure_GetComputedValues(this, index - 1, ratio, &sp1EC, &sp1E4, &sp1DC, &sp1D8);
        MM_Math_Vec3s_ToVec3f(&sp118, &sp1EC);
        MM_Math_Vec3s_ToVec3f(&sp10C, &sp1E4);
        MM_Math_Vec3f_Diff(&sp18C, &sp118, &sp1B4);
        MM_Math_Vec3f_Diff(&sp180, &sp10C, &sp1A8);
    }

    MM_Math_Vec3f_Scale(&sp1B4, 0.5f);
    MM_Math_Vec3f_Scale(&sp1A8, 0.5f);

    if (((elem + 1)->flags & (EFFECT_BLURE_ELEMENT_FLAG_1 | EFFECT_BLURE_ELEMENT_FLAG_2)) ==
        EFFECT_BLURE_ELEMENT_FLAG_2) {
        MM_Math_Vec3f_Diff(&sp18C, &sp1CC, &sp174);
        MM_Math_Vec3f_Diff(&sp180, &sp1C0, &sp168);
    } else {
        Vec3f sp100;
        Vec3f spF4;

        ratio = (f32)(elem + 2)->timer / (f32)this->elemDuration;
        MM_EffectBlure_GetComputedValues(this, index + 2, ratio, &sp1EC, &sp1E4, &sp1DC, &sp1D8);
        MM_Math_Vec3s_ToVec3f(&sp100, &sp1EC);
        MM_Math_Vec3s_ToVec3f(&spF4, &sp1E4);
        MM_Math_Vec3f_Diff(&sp100, &sp1CC, &sp174);
        MM_Math_Vec3f_Diff(&spF4, &sp1C0, &sp168);
    }

    MM_Math_Vec3f_Scale(&sp174, 0.5f);
    MM_Math_Vec3f_Scale(&sp168, 0.5f);

    vtx = GRAPH_ALLOC(gfxCtx, 16 * sizeof(Vtx));
    if (vtx == NULL) {
    } else {
        MM_Math_Vec3f_Diff(&sp1CC, &sp138, &sp158);
        MM_Math_Vec3f_Scale(&sp158, 10.0f);
        MM_Math_Vec3f_Diff(&sp1C0, &sp138, &sp14C);
        MM_Math_Vec3f_Scale(&sp14C, 10.0f);

        MM_Color_RGBA8_Copy(&sp148, &sp1A4);
        MM_Color_RGBA8_Copy(&sp144, &sp1A0);

        vtx[0].v = baseVtx;
        vtx[1].v = baseVtx;

        vtx[0].v.ob[0] = MM_Math_FNearbyIntF(sp158.x);
        vtx[0].v.ob[1] = MM_Math_FNearbyIntF(sp158.y);
        vtx[0].v.ob[2] = MM_Math_FNearbyIntF(sp158.z);
        vtx[0].v.cn[0] = sp148.r;
        vtx[0].v.cn[1] = sp148.g;
        vtx[0].v.cn[2] = sp148.b;
        vtx[0].v.cn[3] = sp148.a;
        vtx[1].v.ob[0] = MM_Math_FNearbyIntF(sp14C.x);
        vtx[1].v.ob[1] = MM_Math_FNearbyIntF(sp14C.y);
        vtx[1].v.ob[2] = MM_Math_FNearbyIntF(sp14C.z);
        ...
        for (i = 1; i < 8; i++) {
            ...
            MM_Math_Vec3f_Diff(&spE0, &sp138, &sp158);
            MM_Math_Vec3f_Scale(&sp158, 10.0f);
            ...
            MM_Math_Vec3f_Diff(&spE0, &sp138, &sp14C);
            MM_Math_Vec3f_Scale(&sp14C, 10.0f);

            vtx[j1].v = baseVtx;
            vtx[j2].v = baseVtx;

            vtx[j1].v.ob[0] = MM_Math_FNearbyIntF(sp158.x);
            vtx[j1].v.ob[1] = MM_Math_FNearbyIntF(sp158.y);
            vtx[j1].v.ob[2] = MM_Math_FNearbyIntF(sp158.z);
            ...
            vtx[j2].v.ob[0] = MM_Math_FNearbyIntF(sp14C.x);
            vtx[j2].v.ob[1] = MM_Math_FNearbyIntF(sp14C.y);
            vtx[j2].v.ob[2] = MM_Math_FNearbyIntF(sp14C.z);
...
void MM_EffectBlure_DrawSmooth(EffectBlure* this2, GraphicsContext* gfxCtx) {
    ...
    for (elem = &this->elements[1]; elem < this->elements + this->numElements - 1; elem++) {
        MM_EffectBlure_UpdateFlags(elem);
    }
    ...
    MM_EffectBlure_SetupSmooth(this, gfxCtx);
    MM_SkinMatrix_SetTranslate(&spDC, this->elements[0].p2.x, this->elements[0].p2.y, this->elements[0].p2.z);
    MM_SkinMatrix_SetScale(&sp9C, 0.1f, 0.1f, 0.1f);
    MM_SkinMatrix_MtxFMtxFMult(&spDC, &sp9C, &sp5C);

    mtx = MM_SkinMatrix_MtxFToNewMtx(gfxCtx, &sp5C);
    if (mtx == NULL) {
        goto close_disps;
    }
    ...
    for (i = 0, elem = &this->elements[0]; elem < this->elements + this->numElements - 1; i++, elem++) {
        ...
        if ((((elem->flags & (EFFECT_BLURE_ELEMENT_FLAG_1 | EFFECT_BLURE_ELEMENT_FLAG_2)) == 0) &&
            ...
            (((elem->flags & (EFFECT_BLURE_ELEMENT_FLAG_1 | EFFECT_BLURE_ELEMENT_FLAG_2)) ==
             ...
             (((elem + 1)->flags & (EFFECT_BLURE_ELEMENT_FLAG_1 | EFFECT_BLURE_ELEMENT_FLAG_2)) ==
              EFFECT_BLURE_ELEMENT_FLAG_2))) {
            MM_EffectBlure_DrawElemNoInterpolation(this, elem, i, gfxCtx);
        } else {
            MM_EffectBlure_DrawElemHermiteInterpolation(this, elem, i, gfxCtx);
        }
    }
...
void MM_EffectBlure_SetupSimple(GraphicsContext* gfxCtx, EffectBlure* this, Vtx* vtx) {
    OPEN_DISPS(gfxCtx);
...
void MM_EffectBlure_SetupSimpleAlt(GraphicsContext* gfxCtx, EffectBlure* this, Vtx* vtx) {
    OPEN_DISPS(gfxCtx);
...
void MM_EffectBlure_DrawSimpleVertices(GraphicsContext* gfxCtx, EffectBlure* this, Vtx* vtx) {
    Mtx* mtx;
...

    {
        ...
        for (i = 0; i < this->numElements - 1; i++, j += 4) {
            ...
            if (fabsf(scale) > 0.0005f) {
                scale = 1.0f / scale;
                MM_Math_Vec3f_Scale(&sp198, scale);

                MM_SkinMatrix_SetTranslate(&sp154, sp1B0.x, sp1B0.y, sp1B0.z);
                SkinMatrix_SetRotateAroundVec(&sp114, 0x3FFF, sp198.x, sp198.y, sp198.z);
                MM_SkinMatrix_MtxFMtxFMult(&sp154, &sp114, &spD4);
                MM_SkinMatrix_SetTranslate(&sp154, -sp1B0.x, -sp1B0.y, -sp1B0.z);
                MM_SkinMatrix_MtxFMtxFMult(&spD4, &sp154, &sp94);

                mtx = MM_SkinMatrix_MtxFToNewMtx(gfxCtx, &sp94);
                if (mtx == NULL) {
                    break;
                }
...
void MM_EffectBlure_DrawSimple(EffectBlure* this2, GraphicsContext* gfxCtx) {
    EffectBlure* this = this2;
    Vtx* vtx;
    Vtx* vtxIter;
    EffectBlureElement* elem;
    ...
    if (this->numElements >= 2) {
        ...
        for (i = 0; i < this->numElements; i++) {
            ...
            ratio = (f32)elem->timer / (f32)this->elemDuration;
            MM_EffectBlure_GetComputedValues(this, i, ratio, &sp74, &sp6C, &sp64, &sp60);
        ...
        MM_EffectBlure_DrawSimpleVertices(gfxCtx, this, vtx);
    }
}

void MM_EffectBlure_Draw(void* thisx, GraphicsContext* gfxCtx) {
    EffectBlure* this = (EffectBlure*)thisx;
    Vtx* vtx;
    EffectBlureElement* elem;
    s32 i;
    ...
    if (this->numElements != 0) {
        if (this->flags == 0) {
            ...
            MM_EffectBlure_DrawSimple(this, gfxCtx);
        } else {
            MM_EffectBlure_DrawSmooth(this, gfxCtx);
        }
    }
