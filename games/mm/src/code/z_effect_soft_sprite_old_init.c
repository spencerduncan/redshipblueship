...
void MM_EffectSs_DrawGEffect(PlayState* play, EffectSs* this, TexturePtr texture) {
...
void MM_EffectSsDust_Spawn(PlayState* play, u16 drawFlags, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
                        Color_RGBA8* primColor, Color_RGBA8* envColor, s16 scale, s16 scaleStep, s16 life,
                        u8 updateMode) {
    EffectSsDustInitParams initParams;

    MM_Math_Vec3f_Copy(&initParams.pos, pos);
    MM_Math_Vec3f_Copy(&initParams.velocity, velocity);
    MM_Math_Vec3f_Copy(&initParams.accel, accel);
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_DUST, 128, &initParams);
}
...
void func_800B0DE0(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
    ...
    MM_EffectSsDust_Spawn(play, DUST_DRAWFLAG0, pos, velocity, accel, primColor, envColor, scale, scaleStep, 10,
                       DUST_UPDATE_NORMAL);
}

void func_800B0E48(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep) {
    MM_EffectSsDust_Spawn(play, DUST_DRAWFLAG1, pos, velocity, accel, primColor, envColor, scale, scaleStep, 10,
                       DUST_UPDATE_NORMAL);
}
...
void func_800B0EB0(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
    ...
    MM_EffectSsDust_Spawn(play, DUST_DRAWFLAG0, pos, velocity, accel, primColor, envColor, scale, scaleStep, life,
                       DUST_UPDATE_NORMAL);
}

void func_800B0F18(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep, s16 life) {
    MM_EffectSsDust_Spawn(play, DUST_DRAWFLAG1, pos, velocity, accel, primColor, envColor, scale, scaleStep, life,
                       DUST_UPDATE_NORMAL);
}

void func_800B0F80(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                   Color_RGBA8* envColor, s16 scale, s16 scaleStep, s16 life) {
    MM_EffectSsDust_Spawn(play, DUST_DRAWFLAG2, pos, velocity, accel, primColor, envColor, scale, scaleStep, life,
                       DUST_UPDATE_NORMAL);
}
...
static Color_RGBA8 MM_sDustBrownPrim = { 170, 130, 90, 255 };
static Color_RGBA8 MM_sDustBrownEnv = { 100, 60, 20, 255 };

void func_800B10C0(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    MM_EffectSsDust_Spawn(play, DUST_DRAWFLAG_RAND_COLOR_OFFSET | DUST_DRAWFLAG0, pos, velocity, accel, &MM_sDustBrownPrim,
                       &MM_sDustBrownEnv, 100, 5, 10, DUST_UPDATE_NORMAL);
}

void func_800B1130(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    MM_EffectSsDust_Spawn(play, DUST_DRAWFLAG_RAND_COLOR_OFFSET | DUST_DRAWFLAG1, pos, velocity, accel, &MM_sDustBrownPrim,
                       &MM_sDustBrownEnv, 100, 5, 10, DUST_UPDATE_NORMAL);
}

void func_800B11A0(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep) {
    MM_EffectSsDust_Spawn(play, DUST_DRAWFLAG_RAND_COLOR_OFFSET | DUST_DRAWFLAG0, pos, velocity, accel, &MM_sDustBrownPrim,
                       &MM_sDustBrownEnv, scale, scaleStep, 10, DUST_UPDATE_NORMAL);
}
...
void func_800B1280(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep, s16 life) {
    MM_EffectSsDust_Spawn(play, DUST_DRAWFLAG_RAND_COLOR_OFFSET | DUST_DRAWFLAG0, pos, velocity, accel, &MM_sDustBrownPrim,
                       &MM_sDustBrownEnv, scale, scaleStep, life, DUST_UPDATE_NORMAL);
}
...
void func_800B13D8(Vec3f* srcPos, f32 randScale, Vec3f* newPos, Vec3f* velocity, Vec3f* accel) {
    s16 randAngle;
    f32 rand = MM_Rand_ZeroOne() * randScale;

    randAngle = MM_Rand_ZeroOne() * 0x10000;

    *newPos = *srcPos;

    newPos->x += MM_Math_SinS(randAngle) * rand;
    newPos->z += MM_Math_CosS(randAngle) * rand;

    velocity->y = 1.0f;
    velocity->x = MM_Math_SinS(randAngle);
    velocity->z = MM_Math_CosS(randAngle);
...
void EffectSsKirakira_SpawnDispersed(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
    ...
    MM_Math_Vec3f_Copy(&initParams.pos, pos);
    MM_Math_Vec3f_Copy(&initParams.velocity, velocity);
    initParams.velocity.y = ((MM_Rand_ZeroOne() * initParams.velocity.y) + initParams.velocity.y) / 2.0f;
    MM_Math_Vec3f_Copy(&initParams.accel, accel);
    initParams.accel.y = ((MM_Rand_ZeroOne() * initParams.accel.y) + initParams.accel.y) / 2.0f;
    initParams.life = life;
    initParams.updateMode = 0;
    initParams.rotSpeed = 0x1518;
    initParams.yaw = MM_Rand_ZeroOne() * 0x4000;
    initParams.scale = scale;
    initParams.primColor = *primColor;
    initParams.envColor = *envColor;
    initParams.alphaStep = (-(255.0f / initParams.life)) + (-(255.0f / initParams.life));

    MM_EffectSs_Spawn(play, EFFECT_SS_KIRAKIRA, 128, &initParams);
}
...
void EffectSsKirakira_SpawnFocused(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
    ...
    initParams.yaw = MM_Rand_ZeroOne() * 0x4000;
    initParams.scale = scale;
    MM_Color_RGBA8_Copy(&initParams.primColor, primColor);
    MM_Color_RGBA8_Copy(&initParams.envColor, envColor);
    initParams.alphaStep = (-(255.0f / initParams.life)) + (-(255.0f / initParams.life));

    MM_EffectSs_Spawn(play, EFFECT_SS_KIRAKIRA, 128, &initParams);
}

void MM_EffectSsBomb2_SpawnFade(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_BOMB2, 10, &initParams);
}

void MM_EffectSsBomb2_SpawnLayered(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep) {
    EffectSsBomb2InitParams initParams;
...
void MM_EffectSsBlast_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_BLAST, 128, &initParams);
}

void MM_EffectSsBlast_SpawnWhiteCustomScale(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale,
                                         s16 scaleStep, s16 life) {
    static Color_RGBA8 sPrimColor = { 255, 255, 255, 255 };
    static Color_RGBA8 sEnvColor = { 200, 200, 200, 0 };

    MM_EffectSsBlast_Spawn(play, pos, velocity, accel, &sPrimColor, &sEnvColor, scale, scaleStep, 35, life);
}

void MM_EffectSsBlast_SpawnShockwave(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
                                  Color_RGBA8* envColor, s16 life) {
    MM_EffectSsBlast_Spawn(play, pos, velocity, accel, primColor, envColor, 100, 375, 35, life);
}

void MM_EffectSsBlast_SpawnWhiteShockwave(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    static Color_RGBA8 sPrimColor = { 255, 255, 255, 255 };
    static Color_RGBA8 sEnvColor = { 200, 200, 200, 0 };

    MM_EffectSsBlast_SpawnShockwave(play, pos, velocity, accel, &sPrimColor, &sEnvColor, 10);
}

// EffectSsGSpk Spawn Functions

void MM_EffectSsGSpk_SpawnAccel(PlayState* play, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_G_SPK, 128, &initParams);
}

// unused
void MM_EffectSsGSpk_SpawnNoAccel(PlayState* play, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
                               Color_RGBA8* primColor, Color_RGBA8* envColor, s16 scale, s16 scaleStep) {
    EffectSsGSpkInitParams initParams;
...
void MM_EffectSsGSpk_SpawnFuse(PlayState* play, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel) {
    Color_RGBA8 primColor = { 255, 255, 150, 255 };
    Color_RGBA8 envColor = { 255, 0, 0, 0 };

    MM_EffectSsGSpk_SpawnSmall(play, actor, pos, velocity, accel, &primColor, &envColor);
}

// unused
void MM_EffectSsGSpk_SpawnRandColor(PlayState* play, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale,
                                 s16 scaleStep) {
    Color_RGBA8 primColor = { 255, 255, 150, 255 };
    Color_RGBA8 envColor = { 255, 0, 0, 0 };
    s32 randOffset = (MM_Rand_ZeroOne() * 20.0f) - 10.0f;
    ...
    MM_EffectSsGSpk_SpawnAccel(play, actor, pos, velocity, accel, &primColor, &envColor, scale, scaleStep);
}

void MM_EffectSsGSpk_SpawnSmall(PlayState* play, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
                             Color_RGBA8* primColor, Color_RGBA8* envColor) {
    MM_EffectSsGSpk_SpawnAccel(play, actor, pos, velocity, accel, primColor, envColor, 100, 5);
}

// EffectSsDFire Spawn Functions
void MM_EffectSsDFire_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep,
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_D_FIRE, 128, &initParams);
}

// EffectSsBubble Spawn Functions

void MM_EffectSsBubble_Spawn(PlayState* play, Vec3f* pos, f32 yPosOffset, f32 yPosRandScale, f32 xzPosRandScale,
                          f32 scale) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_BUBBLE, 128, &initParams);
}

// EffectSsGRipple Spawn Functions

void MM_EffectSsGRipple_Spawn(PlayState* play, Vec3f* pos, s16 radius, s16 radiusMax, s16 life) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_G_RIPPLE, 128, &initParams);
}

// EffectSsGSplash Spawn Functions

void MM_EffectSsGSplash_Spawn(PlayState* play, Vec3f* pos, Color_RGBA8* primColor, Color_RGBA8* envColor, s16 type,
                           s16 scale) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_G_SPLASH, 128, &initParams);
}

// EffectSsGFire Spawn Functions

void MM_EffectSsGFire_Spawn(PlayState* play, Vec3f* pos) {
    EffectSsGFireInitParams initParams;

    MM_Math_Vec3f_Copy(&initParams.pos, pos);

    MM_EffectSs_Spawn(play, EFFECT_SS_G_FIRE, 128, &initParams);
}

// EffectSsLightning Spawn Functions

void MM_EffectSsLightning_Spawn(PlayState* play, Vec3f* pos, Color_RGBA8* primColor, Color_RGBA8* envColor, s16 scale,
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_LIGHTNING, 128, &initParams);
}

// EffectSsDtBubble Spawn Functions

void MM_EffectSsDtBubble_SpawnColorProfile(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 life,
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_DT_BUBBLE, 128, &initParams);
}

void MM_EffectSsDtBubble_SpawnCustomColor(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
 ...
 */
void MM_EffectSsHahen_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 flags, s16 scale, s16 objectId,
                         s16 life, Gfx* dList) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_HAHEN, 128, &initParams);
}
 ...
 *       only live for 200 frames
 */
void MM_EffectSsHahen_SpawnBurst(PlayState* play, Vec3f* pos, f32 burstScale, s16 flags, s16 scale, s16 randScaleRange,
    ...
    for (i = 0; i < count; i++) {
        velocity.x = (MM_Rand_ZeroOne() - 0.5f) * burstScale;
        velocity.z = (MM_Rand_ZeroOne() - 0.5f) * burstScale;
        velocity.y = ((MM_Rand_ZeroOne() * 0.5f) + 0.5f) * burstScale;

        MM_EffectSsHahen_Spawn(play, pos, &velocity, &accel, flags, MM_Rand_S16Offset(scale, randScaleRange), objectId, life,
                            dList);
    }
}
...
void func_800B2364(PlayState* play, Vec3f* pos, Gfx* dList) {
    ...
    MM_EffectSsHahen_Spawn(play, pos, &gZeroVec3f, &accel, HAHEN_SMALL, 5, GAMEPLAY_KEEP, 10, dList);
}
 ...
 */
void MM_EffectSsStick_Spawn(PlayState* play, Vec3f* pos, s16 yaw) {
    EffectSsStickInitParams initParams;

    initParams.pos = *pos;
    initParams.yaw = yaw;

    MM_EffectSs_Spawn(play, EFFECT_SS_STICK, 128, &initParams);
}

// EffectSsSibuki Spawn Functions

void MM_EffectSsSibuki_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 moveDelay, s16 direction,
                          s16 scale) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_SIBUKI, 128, &initParams);
}

void MM_EffectSsSibuki_SpawnBurst(PlayState* play, Vec3f* pos) {
    s16 i;
    Vec3f zeroVec = { 0.0f, 0.0f, 0.0f };
    s16 randDirection = MM_Rand_ZeroOne() * 1.99f;

    for (i = 0; i < KREG(19) + 30; i++) {
        MM_EffectSsSibuki_Spawn(play, pos, &zeroVec, &zeroVec, i / (KREG(27) + 6), randDirection, KREG(18) + 40);
    }
}

// EffectSsStone1 Spawn Functions

void MM_EffectSsStone1_Spawn(PlayState* play, Vec3f* pos, s32 reg0) {
    EffectSsStone1InitParams initParams;

    initParams.pos = *pos;
    initParams.reg0 = reg0;

    MM_EffectSs_Spawn(play, EFFECT_SS_STONE1, 128, &initParams);
}
...
void EffectSsHitmark_Spawn(PlayState* play, s32 type, s16 scale, Vec3f* pos) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_HITMARK, 128, &initParams);
}
 ...
 */
void MM_EffectSsFhgFlash_SpawnShock(PlayState* play, Actor* actor, Vec3f* pos, s16 scale, u8 params) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_FHG_FLASH, 128, &initParams);
}

// EffectSsKFire Spawn Functions

void MM_EffectSsKFire_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scaleMax, u8 type) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_K_FIRE, 128, &initParams);
}

// EffectSsSolderSrchBall Spawn Functions

void MM_EffectSsSolderSrchBall_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale,
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_SOLDER_SRCH_BALL, 128, &initParams);
}

// EffectSsKakera Spawn Functions

void MM_EffectSsKakera_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* arg3, s16 gravity, s16 arg5, s16 arg6,
    ...
    MM_Math_Vec3f_Copy(&initParams.pos, pos);
    MM_Math_Vec3f_Copy(&initParams.velocity, velocity);
    MM_Math_Vec3f_Copy(&initParams.unk_18, arg3);
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_KAKERA, 101, &initParams);
}

// EffectSsIcePiece Spawn Functions

void MM_EffectSsIcePiece_Spawn(PlayState* play, Vec3f* pos, f32 scale, Vec3f* velocity, Vec3f* accel, s32 life) {
    ...
    initParams.scale = scale;
    initParams.life = life;
    MM_EffectSs_Spawn(play, EFFECT_SS_ICE_PIECE, 128, &initParams);
}

void MM_EffectSsIcePiece_SpawnBurst(PlayState* play, Vec3f* refPos, f32 scale) {
    ...
    for (i = 0; i < ARRAY_COUNT(sVecScales); i++) {
        pos = *refPos;
        velocityScale = MM_Rand_ZeroFloat(1.0f) + 0.5f;
        ...
        MM_EffectSsIcePiece_Spawn(play, &pos, (MM_Rand_ZeroFloat(1.0f) + 0.5f) * ((scale * 1.3f) * 100.0f), &velocity,
                               &MM_sAccel, 25);
    }
}
...
void EffectSsEnIce_SpawnFlying(PlayState* play, Actor* actor, Vec3f* pos, Color_RGBA8* prim, Color_RGBA8* env,
    ...
    initParams.actor = actor;
    MM_Math_Vec3f_Copy(&initParams.pos, pos);
    MM_Color_RGBA8_Copy(&initParams.primColor, prim);
    MM_Color_RGBA8_Copy(&initParams.envColor, env);
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_EN_ICE, 80, &initParams);
}
...
void func_800B2B7C(PlayState* play, Actor* actor, Vec3s* arg2, f32 scale) {
    ...
    MM_Math_Vec3s_ToVec3f(&dest, arg2);
    func_800B2B44(play, actor, &dest, scale);
}

void MM_EffectSsEnIce_Spawn(PlayState* play, Vec3f* pos, f32 scale, Vec3f* velocity, Vec3f* accel, Color_RGBA8* primColor,
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_EN_ICE, 128, &initParams);
}

// EffectSsFireTail Spawn Functions

void MM_EffectSsFireTail_Spawn(PlayState* play, Actor* actor, Vec3f* pos, f32 scale, Vec3f* arg4, s16 arg5,
                            Color_RGBA8* primColor, Color_RGBA8* envColor, s16 type, s16 bodyPart, s32 life) {
    EffectSsFireTailInitParams initParams;

    MM_Math_Vec3f_Copy(&initParams.pos, pos);
    MM_Math_Vec3f_Copy(&initParams.unk_14, arg4);
    MM_Color_RGBA8_Copy(&initParams.primColor, primColor);
    MM_Color_RGBA8_Copy(&initParams.envColor, envColor);
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_FIRE_TAIL, 128, &initParams);
}

void MM_EffectSsFireTail_SpawnFlame(PlayState* play, Actor* actor, Vec3f* pos, f32 arg3, s16 bodyPart,
                                 f32 colorIntensity) {
    ...
    MM_EffectSsFireTail_Spawn(play, actor, pos, arg3, &actor->velocity, 15, &sPrimColor, &sEnvColor,
                           (colorIntensity == 1.0f) ? 0 : 1, bodyPart, 1);
}

void MM_EffectSsFireTail_SpawnFlameOnPlayer(PlayState* play, f32 scale, s16 bodyPart, f32 colorIntensity) {
    Player* player = GET_PLAYER(play);

    MM_EffectSsFireTail_SpawnFlame(play, &player->actor, &player->bodyPartsPos[bodyPart], scale, bodyPart, colorIntensity);
}

// EffectSsEnFire Spawn Functions

void MM_EffectSsEnFire_SpawnVec3f(PlayState* play, Actor* actor, Vec3f* pos, s16 scale, s16 params, s16 flags,
                               s16 bodyPart) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_EN_FIRE, 128, &initParams);
}

void MM_EffectSsEnFire_SpawnVec3s(PlayState* play, Actor* actor, Vec3s* pos, s16 scale, s16 params, s16 flags,
                               s16 bodyPart) {
    EffectSsEnFireInitParams initParams;
...
void MM_EffectSsExtra_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scoreIndex) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_EXTRA, 100, &initParams);
}

// EffectSsDeadDb Spawn Functions

void MM_EffectSsDeadDb_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* prim,
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_DEAD_DB, 120, &initParams);
}
...
void func_800B3030(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep,
    ...
    MM_EffectSsDeadDb_Spawn(play, pos, velocity, accel, &sPrimColor, &MM_sEnvColors[colorIndex], scale, scaleStep, 9);
}

// EffectSsDeadDd Spawn Functions

void MM_EffectSsDeadDd_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, Color_RGBA8* prim,
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_DEAD_DD, 120, &initParams);
}

// EffectSsDeadDs Spawn Functions

void MM_EffectSsDeadDs_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale, s16 scaleStep,
                          s16 alpha, s32 life) {
    ...
    initParams.alpha = alpha;
    initParams.life = life;
    MM_EffectSs_Spawn(play, EFFECT_SS_DEAD_DS, 100, &initParams);
}

void func_800B31BC(PlayState* play, Vec3f* pos, s16 scale, s16 scaleStep, s16 alpha, s32 life) {
    MM_EffectSsDeadDs_Spawn(play, pos, &gZeroVec3f, &gZeroVec3f, scale, scaleStep, alpha, life);
}

// EffectSsIceSmoke Spawn Functions

void MM_EffectSsIceSmoke_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_SS_ICE_SMOKE, 128, &initParams);
}
...
void EffectSsIceBlock_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale) {
    ...
    MM_EffectSs_Spawn(play, EFFECT_EN_ICE_BLOCK, 128, &initParams);
}
