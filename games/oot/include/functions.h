...
void OoT_Main_ThreadEntry(void* arg);
void OoT_Idle_ThreadEntry(void* arg);
void OoT_ViConfig_UpdateVi(u32 mode);
void OoT_ViConfig_UpdateBlack(void);
s32 DmaMgr_CompareName(const char* name1, const char* name2);
s32 OoT_DmaMgr_DmaRomToRam(uintptr_t rom, uintptr_t ram, size_t size);
s32 OoT_DmaMgr_DmaHandler(OSPiHandle* pihandle, OSIoMesg* mb, s32 direction);
void DmaMgr_Error(DmaRequest* req, const char* file, const char* errorName, const char* errorDesc);
const char* DmaMgr_GetFileNameImpl(uintptr_t vrom);
const char* DmaMgr_GetFileName(uintptr_t vrom);
void OoT_DmaMgr_ProcessMsg(DmaRequest* req);
void OoT_DmaMgr_ThreadEntry(void* arg0);
s32 OoT_DmaMgr_SendRequestImpl(DmaRequest* req, uintptr_t ram, uintptr_t vrom, size_t size, u32 unk, OSMesgQueue* queue, OSMesg msg);
s32 OoT_DmaMgr_SendRequest0(uintptr_t ram, uintptr_t vrom, size_t size);
...
EnItem00* OoT_Item_DropCollectible(PlayState* play, Vec3f* spawnPos, s16 params);
EnItem00* OoT_Item_DropCollectible2(PlayState* play, Vec3f* spawnPos, s16 params);
...
void OoT_Item_DropCollectibleRandom(PlayState* play, Actor* fromActor, Vec3f* spawnPos, s16 params);
void EffectBlure_ChangeType(EffectBlure* this, int type);
void OoT_EffectBlure_AddVertex(EffectBlure* this, Vec3f* p1, Vec3f* p2);
void OoT_EffectBlure_AddSpace(EffectBlure* this);
void OoT_EffectBlure_Init1(void* thisx, void* initParamsx);
void OoT_EffectBlure_Init2(void* thisx, void* initParamsx);
void OoT_EffectBlure_Destroy(void* thisx);
s32 OoT_EffectBlure_Update(void* thisx);
void OoT_EffectBlure_Draw(void* thisx, GraphicsContext* gfxCtx);
...
void OoT_EffectSs_InitInfo(PlayState* play, s32 tableSize);
void OoT_EffectSs_ClearAll(PlayState* play);
void OoT_EffectSs_Delete(EffectSs* effectSs);
void OoT_EffectSs_Reset(EffectSs* effectSs);
void OoT_EffectSs_Insert(PlayState* play, EffectSs* effectSs);
void OoT_EffectSs_Spawn(PlayState* play, s32 type, s32 priority, void* initParams);
void OoT_EffectSs_UpdateAll(PlayState* play);
void OoT_EffectSs_DrawAll(PlayState* play);
...
void OoT_EffectSsBomb2_SpawnLayered(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale,
                                s16 scaleStep);
...
void OoT_EffectSsBlast_SpawnShockwave(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
...
void OoT_EffectSsGSpk_SpawnRandColor(PlayState* play, Actor* actor, Vec3f* pos, Vec3f* velocity, Vec3f* accel,
...
void OoT_EffectSsGSplash_Spawn(PlayState* play, Vec3f* pos, Color_RGBA8* primColor, Color_RGBA8* envColor,
                           s16 type, s16 scale);
void EffectSsGMagma_Spawn(PlayState* play, Vec3f* pos);
void OoT_EffectSsGFire_Spawn(PlayState* play, Vec3f* pos);
void OoT_EffectSsLightning_Spawn(PlayState* play, Vec3f* pos, Color_RGBA8* primColor, Color_RGBA8* envColor,
                             s16 scale, s16 yaw, s16 life, s16 numBolts);
void OoT_EffectSsDtBubble_SpawnColorProfile(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 scale,
...
void OoT_EffectSsHahen_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 unused, s16 scale,
                         s16 objId, s16 life, Gfx* dList);
void OoT_EffectSsHahen_SpawnBurst(PlayState* play, Vec3f* pos, f32 burstScale, s16 unused, s16 scale,
                              s16 randScaleRange, s16 count, s16 objId, s16 life, Gfx* dList);
void OoT_EffectSsStick_Spawn(PlayState* play, Vec3f* pos, s16 yaw);
void OoT_EffectSsSibuki_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 moveDelay,
...
void OoT_EffectSsSolderSrchBall_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* accel, s16 unused,
                                  s16* linkDetected);
void OoT_EffectSsKakera_Spawn(PlayState* play, Vec3f* pos, Vec3f* velocity, Vec3f* arg3, s16 gravity, s16 arg5,
...
void EffectSsEnIce_SpawnFlyingVec3s(PlayState* play, Actor* actor, Vec3s* pos, s16 primR, s16 primG, s16 primB,
                                    s16 primA, s16 envR, s16 envG, s16 envB, f32 scale);
void OoT_EffectSsEnIce_Spawn(PlayState* play, Vec3f* pos, f32 scale, Vec3f* velocity, Vec3f* accel,
...
void OoT_EffectSsEnFire_SpawnVec3f(PlayState* play, Actor* actor, Vec3f* pos, s16 scale, s16 unk_12, s16 flags,
                               s16 bodyPart);
void OoT_EffectSsEnFire_SpawnVec3s(PlayState* play, Actor* actor, Vec3s* vec, s16 scale, s16 arg4, s16 flags,
                               s16 bodyPart);
...
void OoT_Overlay_LoadGameState(GameStateOverlay* overlayEntry);
void OoT_Overlay_FreeGameState(GameStateOverlay* overlayEntry);
void OoT_ActorShape_Init(ActorShape* shape, f32 yOffset, ActorShadowFunc shadowDraw, f32 shadowScale);
void OoT_ActorShadow_DrawCircle(Actor* actor, Lights* lights, PlayState* play);
void OoT_ActorShadow_DrawWhiteCircle(Actor* actor, Lights* lights, PlayState* play);
void OoT_ActorShadow_DrawHorse(Actor* actor, Lights* lights, PlayState* play);
void OoT_ActorShadow_DrawFeet(Actor* actor, Lights* lights, PlayState* play);
void OoT_Actor_SetFeetPos(Actor* actor, s32 limbIndex, s32 leftFootIndex, Vec3f* leftFootPos, s32 rightFootIndex,
                      Vec3f* rightFootPos);
...
s32 OoT_Flags_GetSwitch(PlayState* play, s32 flag);
void OoT_Flags_SetSwitch(PlayState* play, s32 flag);
void OoT_Flags_UnsetSwitch(PlayState* play, s32 flag);
...
s32 OoT_Flags_GetTreasure(PlayState* play, s32 flag);
void OoT_Flags_SetTreasure(PlayState* play, s32 flag);
s32 OoT_Flags_GetClear(PlayState* play, s32 flag);
void OoT_Flags_SetClear(PlayState* play, s32 flag);
void OoT_Flags_UnsetClear(PlayState* play, s32 flag);
...
s32 OoT_Flags_GetCollectible(PlayState* play, s32 flag);
void OoT_Flags_SetCollectible(PlayState* play, s32 flag);
void OoT_TitleCard_InitBossName(PlayState* play, TitleCardContext* titleCtx, void* texture, s16 x, s16 y, u8 width,
                            u8 height, s16 hasTranslation);
void OoT_TitleCard_InitPlaceName(PlayState* play, TitleCardContext* titleCtx, void* texture, s32 x, s32 y,
                             s32 width, s32 height, s32 delay);
s32 func_8002D53C(PlayState* play, TitleCardContext* titleCtx);
void OoT_Actor_Kill(Actor* actor);
void OoT_Actor_SetFocus(Actor* actor, f32 offset);
void OoT_Actor_SetScale(Actor* actor, f32 scale);
void OoT_Actor_SetObjectDependency(PlayState* play, Actor* actor);
void OoT_Actor_UpdatePos(Actor* actor);
...
s16 OoT_Actor_WorldYawTowardActor(Actor* actorA, Actor* actorB);
s16 OoT_Actor_WorldYawTowardPoint(Actor* actor, Vec3f* refPoint);
f32 OoT_Actor_WorldDistXYZToActor(Actor* actorA, Actor* actorB);
f32 OoT_Actor_WorldDistXYZToPoint(Actor* actor, Vec3f* refPoint);
s16 OoT_Actor_WorldPitchTowardActor(Actor* actorA, Actor* actorB);
s16 OoT_Actor_WorldPitchTowardPoint(Actor* actor, Vec3f* refPoint);
f32 OoT_Actor_WorldDistXZToActor(Actor* actorA, Actor* actorB);
f32 OoT_Actor_WorldDistXZToPoint(Actor* actor, Vec3f* refPoint);
void OoT_Actor_WorldToActorCoords(Actor* actor, Vec3f* result, Vec3f* arg2);
f32 OoT_Actor_HeightDiff(Actor* actorA, Actor* actorB);
f32 OoT_Player_GetHeight(Player* player);
s32 OoT_Player_ActionHandler_2(Player* player, PlayState* play);
...
s32 OoT_Player_SetCsActionWithHaltedActors(PlayState* play, Actor* actor, u8 arg2);
void func_8002DF90(DynaPolyActor* dynaActor);
void func_8002DFA4(DynaPolyActor* dynaActor, f32 arg1, s16 arg2);
s32 OoT_Player_IsFacingActor(Actor* actor, s16 angle, PlayState* play);
s32 OoT_Actor_ActorBIsFacingActorA(Actor* actorA, Actor* actorB, s16 angle);
s32 OoT_Actor_IsFacingPlayer(Actor* actor, s16 angle);
s32 OoT_Actor_ActorAIsFacingActorB(Actor* actorA, Actor* actorB, s16 angle);
s32 OoT_Actor_IsFacingAndNearPlayer(Actor* actor, f32 range, s16 angle);
s32 OoT_Actor_ActorAIsFacingAndNearActorB(Actor* actorA, Actor* actorB, f32 range, s16 angle);
void OoT_Actor_UpdateBgCheckInfo(PlayState* play, Actor* actor, f32 wallCheckHeight, f32 wallCheckRadius,
...
PosRot* OoT_Actor_GetFocus(PosRot* arg0, Actor* actor);
PosRot* OoT_Actor_GetWorld(PosRot* arg0, Actor* actor);
PosRot* OoT_Actor_GetWorldPosShapeRot(PosRot* arg0, Actor* actor);
...
u32 OoT_Actor_TextboxIsClosing(Actor* actor, PlayState* play);
s8 func_8002F368(PlayState* play);
void OoT_Actor_GetScreenPos(PlayState* play, Actor* actor, s16* x, s16* y);
u32 OoT_Actor_HasParent(Actor* actor, PlayState* play);
// TODO: Rename the follwing 3 functions using whatever scheme we use when we rename OoT_Actor_OfferGetItem and OoT_Actor_OfferGetItemNearby.
...
s32 OoT_Actor_OfferGetItem(Actor* actor, PlayState* play, s32 getItemId, f32 xzRange, f32 yRange);
void OoT_Actor_OfferGetItemNearby(Actor* actor, PlayState* play, s32 getItemId);
void OoT_Actor_OfferCarry(Actor* actor, PlayState* play);
u32 OoT_Actor_HasNoParent(Actor* actor, PlayState* play);
...
u32 OoT_Actor_SetRideActor(PlayState* play, Actor* horse, s32 arg2);
...
void OoT_Player_PlaySfx(Actor* actor, u16 sfxId);
...
void OoT_Actor_UpdateAll(PlayState* play, ActorContext* actorCtx);
...
Actor* OoT_Actor_Spawn(ActorContext* actorCtx, PlayState* play, s16 actorId, f32 posX, f32 posY, f32 posZ,
                   s16 rotX, s16 rotY, s16 rotZ, s16 params, s16 canRandomize);
Actor* OoT_Actor_SpawnAsChild(ActorContext* actorCtx, Actor* parent, PlayState* play, s16 actorId, f32 posX,
                          f32 posY, f32 posZ, s16 rotX, s16 rotY, s16 rotZ, s16 params);
void OoT_Actor_SpawnTransitionActors(PlayState* play, ActorContext* actorCtx);
Actor* OoT_Actor_SpawnEntry(ActorContext* actorCtx, ActorEntry* actorEntry, PlayState* play);
Actor* OoT_Actor_Delete(ActorContext* actorCtx, Actor* actor, PlayState* play);
Actor* func_80032AF0(PlayState* play, ActorContext* actorCtx, Actor** actorPtr, Player* player);
Actor* Actor_Find(ActorContext* actorCtx, s32 actorId, s32 actorCategory);
void OoT_Enemy_StartFinishingBlow(PlayState* play, Actor* actor);
...
void OoT_Actor_SpawnFloorDustRing(PlayState* play, Actor* actor, Vec3f* posXZ, f32 radius, s32 amountMinusOne,
...
void OoT_Actor_ChangeCategory(PlayState* play, ActorContext* actorCtx, Actor* actor, u8 actorCategory);
void Actor_SetTextWithPrefix(PlayState* play, Actor* actor, s16 textIdLower);
s16 OoT_Actor_TestFloorInDirection(Actor* actor, PlayState* play, f32 distance, s16 angle);
s32 OoT_Actor_IsTargeted(PlayState* play, Actor* actor);
s32 OoT_Actor_OtherIsTargeted(PlayState* play, Actor* actor);
...
f32 OoT_Rand_ZeroFloat(f32 f);
f32 OoT_Rand_CenteredFloat(f32 f);
void OoT_Actor_DrawDoorLock(PlayState* play, s32 arg1, s32 arg2);
void func_8003424C(PlayState* play, Vec3f* arg1);
void OoT_Actor_SetColorFilter(Actor* actor, s16 colorFlag, s16 colorIntensityMax, s16 xluFlag, s16 duration);
Hilite* func_800342EC(Vec3f* object, PlayState* play);
Hilite* func_8003435C(Vec3f* object, PlayState* play);
s32 OoT_Npc_UpdateTalking(PlayState* play, Actor* actor, s16* talkState, f32 interactRange,
                      NpcGetTextIdFunc getTextId, NpcUpdateTalkStateFunc updateTalkState);
s16 OoT_Npc_GetTrackingPresetMaxPlayerYaw(s16 presetIndex);
void OoT_Npc_TrackPoint(Actor* actor, NpcInteractInfo* interactInfo, s16 presetIndex, s16 trackingMode);
...
void OoT_Actor_Noop(Actor* actor, PlayState* play);
void OoT_Gfx_DrawDListOpa(PlayState* play, Gfx* dlist);
void OoT_Gfx_DrawDListXlu(PlayState* play, Gfx* dlist);
Actor* OoT_Actor_FindNearby(PlayState* play, Actor* refActor, s16 actorId, u8 actorCategory, f32 range);
...
u8 OoT_Actor_ApplyDamage(Actor* actor);
void OoT_Actor_SetDropFlag(Actor* actor, ColliderInfo* colBody, s32 freezeFlag);
void OoT_Actor_SetDropFlagJntSph(Actor* actor, ColliderJntSph* colBody, s32 freezeFlag);
void func_80035844(Vec3f* arg0, Vec3f* arg1, Vec3s* arg2, s32 arg3);
Actor* func_800358DC(Actor* actor, Vec3f* spawnPos, Vec3s* spawnRot, f32* arg3, s32 timer, s16* unused,
                     PlayState* play, s16 params, s32 arg8);
void func_800359B8(Actor* actor, s16 arg1, Vec3s* arg2);
s32 OoT_Flags_GetEventChkInf(s32 flag);
void OoT_Flags_SetEventChkInf(s32 flag);
...
s32 OoT_Flags_GetInfTable(s32 flag);
void OoT_Flags_SetInfTable(s32 flag);
void Flags_UnsetInfTable(s32 flag);
s32 Flags_GetEventInf(s32 flag);
void OoT_Flags_SetEventInf(s32 flag);
...
// ? func_80038600(?);
u16 OoT_DynaSSNodeList_GetNextNodeIdx(DynaSSNodeList*);
void func_80038A28(CollisionPoly* poly, f32 tx, f32 ty, f32 tz, MtxF* dest);
f32 OoT_CollisionPoly_GetPointDistanceFromPlane(CollisionPoly* poly, Vec3f* point);
CollisionHeader* OoT_BgCheck_GetCollisionHeader(CollisionContext* colCtx, s32 bgId);
void OoT_CollisionPoly_GetVerticesByBgId(CollisionPoly* poly, s32 bgId, CollisionContext* colCtx, Vec3f* dest);
s32 OoT_BgCheck_CheckStaticCeiling(StaticLookup* lookup, u16 xpFlags, CollisionContext* colCtx, f32* outY, Vec3f* pos,
                               f32 checkHeight, CollisionPoly** outPoly);
s32 OoT_BgCheck_CheckLineAgainstSSList(SSList* headNodeId, CollisionContext* colCtx, u16 xpFlags1, u16 xpFlags2,
                                   Vec3f* posA, Vec3f* posB, Vec3f* outPos, CollisionPoly** outPoly, f32* outDistSq,
                                   f32 chkDist, s32 bccFlags);
void OoT_BgCheck_GetStaticLookupIndicesFromPos(CollisionContext* colCtx, Vec3f* pos, Vec3i* arg2);
void OoT_BgCheck_Allocate(CollisionContext* colCtx, PlayState* play, CollisionHeader* colHeader);
s32 OoT_BgCheck_PosInStaticBoundingBox(CollisionContext* colCtx, Vec3f* pos);
f32 OoT_BgCheck_EntityRaycastFloor1(CollisionContext* colCtx, CollisionPoly** outPoly, Vec3f* pos);
f32 OoT_BgCheck_EntityRaycastFloor2(PlayState* play, CollisionContext* colCtx, CollisionPoly** outPoly,
                                Vec3f* pos);
f32 OoT_BgCheck_EntityRaycastFloor3(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Vec3f* pos);
f32 BgCheck_EntityRaycastFloor4(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Actor* actor,
                                Vec3f* arg4);
f32 OoT_BgCheck_EntityRaycastFloor5(PlayState* play, CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId,
                                Actor* actor, Vec3f* pos);
f32 OoT_BgCheck_EntityRaycastFloor6(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Actor* actor, Vec3f* pos,
                                f32 chkDist);
f32 OoT_BgCheck_EntityRaycastFloor7(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Actor* actor, Vec3f* pos);
f32 OoT_BgCheck_AnyRaycastFloor1(CollisionContext* colCtx, CollisionPoly* outPoly, Vec3f* pos);
f32 OoT_BgCheck_AnyRaycastFloor2(CollisionContext* colCtx, CollisionPoly* outPoly, s32* bgId, Vec3f* pos);
f32 OoT_BgCheck_CameraRaycastFloor2(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Vec3f* pos);
f32 OoT_BgCheck_EntityRaycastFloor8(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Actor* actor, Vec3f* pos);
f32 OoT_BgCheck_EntityRaycastFloor9(CollisionContext* colCtx, CollisionPoly** outPoly, s32* bgId, Vec3f* pos);
s32 OoT_BgCheck_CheckWallImpl(CollisionContext* colCtx, u16 xpFlags, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev,
                          f32 radius, CollisionPoly** outPoly, s32* outBgId, Actor* actor, f32 checkHeight, u8 argA);
s32 OoT_BgCheck_EntitySphVsWall1(CollisionContext* colCtx, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev, f32 radius,
                             CollisionPoly** outPoly, f32 checkHeight);
s32 OoT_BgCheck_EntitySphVsWall2(CollisionContext* colCtx, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev, f32 radius,
                             CollisionPoly** outPoly, s32* outBgId, f32 checkHeight);
s32 OoT_BgCheck_EntitySphVsWall3(CollisionContext* colCtx, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev, f32 radius,
                             CollisionPoly** outPoly, s32* outBgId, Actor* actor, f32 checkHeight);
s32 OoT_BgCheck_EntitySphVsWall4(CollisionContext* colCtx, Vec3f* posResult, Vec3f* posNext, Vec3f* posPrev, f32 radius,
                             CollisionPoly** outPoly, s32* outBgId, Actor* actor, f32 checkHeight);
s32 OoT_BgCheck_AnyCheckCeiling(CollisionContext* colCtx, f32* outY, Vec3f* pos, f32 checkHeight);
s32 OoT_BgCheck_EntityCheckCeiling(CollisionContext* colCtx, f32* arg1, Vec3f* arg2, f32 arg3, CollisionPoly** outPoly,
                               s32* outBgId, Actor* actor);
s32 OoT_BgCheck_CheckLineImpl(CollisionContext* colCtx, u16 xpFlags1, u16 xpFlags2, Vec3f* posA, Vec3f* posB,
                          Vec3f* posResult, CollisionPoly** outPoly, s32* bgId, Actor* actor, f32 chkDist,
                          u32 bccFlags);
s32 OoT_BgCheck_CameraLineTest1(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32* bgId);
s32 OoT_BgCheck_CameraLineTest2(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32* bgId);
s32 OoT_BgCheck_EntityLineTest1(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32* bgId);
s32 OoT_BgCheck_EntityLineTest2(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32* bgId,
                            Actor* actor);
s32 OoT_BgCheck_EntityLineTest3(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                            CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32* bgId,
                            Actor* actor, f32 chkDist);
s32 OoT_BgCheck_ProjectileLineTest(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                               CollisionPoly** outPoly, s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace,
                               s32* bgId);
s32 OoT_BgCheck_AnyLineTest1(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult, CollisionPoly** outPoly,
                         s32 chkOneFace);
s32 OoT_BgCheck_AnyLineTest2(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult, CollisionPoly** outPoly,
                         s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace);
s32 OoT_BgCheck_AnyLineTest3(CollisionContext* colCtx, Vec3f* posA, Vec3f* posB, Vec3f* posResult, CollisionPoly** outPoly,
                         s32 chkWall, s32 chkFloor, s32 chkCeil, s32 chkOneFace, s32* bgId);
s32 OoT_BgCheck_SphVsFirstPoly(CollisionContext* colCtx, Vec3f* center, f32 radius);
void SSNodeList_Initialize(SSNodeList*);
void OoT_SSNodeList_Alloc(PlayState* play, SSNodeList* this, s32 tblMax, s32 numPolys);
u16 OoT_SSNodeList_GetNextNodeIdx(SSNodeList* this);
s32 OoT_DynaPoly_IsBgIdBgActor(s32 bgId);
void OoT_DynaPoly_Init(PlayState* play, DynaCollisionContext* dyna);
void OoT_DynaPoly_Alloc(PlayState* play, DynaCollisionContext* dyna);
...
s32 OoT_DynaPoly_SetBgActor(PlayState* play, DynaCollisionContext* dyna, Actor* actor, CollisionHeader* colHeader);
DynaPolyActor* OoT_DynaPoly_GetActor(CollisionContext* colCtx, s32 bgId);
void OoT_DynaPoly_DeleteBgActor(PlayState* play, DynaCollisionContext* dyna, s32 bgId);
...
void OoT_DynaPoly_UpdateBgActorTransforms(PlayState* play, DynaCollisionContext* dyna);
f32 OoT_BgCheck_RaycastFloorDyna(DynaRaycast* dynaRaycast);
s32 OoT_BgCheck_SphVsDynaWall(CollisionContext* colCtx, u16 xpFlags, f32* outX, f32* outZ, Vec3f* pos, f32 radius,
                          CollisionPoly** outPoly, s32* outBgId, Actor* actor);
s32 OoT_BgCheck_CheckDynaCeiling(CollisionContext* colCtx, u16 xpFlags, f32* outY, Vec3f* pos, f32 chkDist,
                             CollisionPoly** outPoly, s32* outBgId, Actor* actor);
s32 OoT_BgCheck_CheckLineAgainstDyna(CollisionContext* colCtx, u16 xpFlags, Vec3f* posA, Vec3f* posB, Vec3f* posResult,
                                 CollisionPoly** outPoly, f32* distSq, s32* outBgId, Actor* actor, f32 chkDist,
                                 s32 bccFlags);
s32 OoT_BgCheck_SphVsFirstDynaPoly(CollisionContext* colCtx, u16 xpFlags, CollisionPoly** outPoly, s32* outBgId,
                               Vec3f* center, f32 radius, Actor* actor, u16 bciFlags);
void OoT_CollisionHeader_GetVirtual(void* colHeader, CollisionHeader** dest);
void func_800418D0(CollisionContext* colCtx, PlayState* play);
void OoT_BgCheck_ResetPolyCheckTbl(SSNodeList* nodeList, s32 numPolys);
...
u32 OoT_SurfaceType_GetSceneExitIndex(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
...
u32 OoT_SurfaceType_IsHorseBlocked(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
...
u32 OoT_SurfaceType_GetLightSettingIndex(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
u32 OoT_SurfaceType_GetEcho(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
u32 OoT_SurfaceType_IsHookshotSurface(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
s32 OoT_SurfaceType_IsIgnoredByEntities(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
s32 OoT_SurfaceType_IsIgnoredByProjectiles(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
s32 SurfaceType_IsConveyor(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
u32 OoT_SurfaceType_GetConveyorSpeed(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
u32 OoT_SurfaceType_GetConveyorDirection(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
u32 OoT_SurfaceType_IsWallDamage(CollisionContext* colCtx, CollisionPoly* poly, s32 bgId);
s32 OoT_WaterBox_GetSurface1(PlayState* play, CollisionContext* colCtx, f32 x, f32 z, f32* ySurface,
                         WaterBox** outWaterBox);
s32 OoT_WaterBox_GetSurface2(PlayState* play, CollisionContext* colCtx, Vec3f* pos, f32 surfaceChkDist,
                         WaterBox** outWaterBox);
s32 OoT_WaterBox_GetSurfaceImpl(PlayState* play, CollisionContext* colCtx, f32 x, f32 z, f32* ySurface,
                            WaterBox** outWaterBox);
...
u32 OoT_WaterBox_GetLightSettingIndex(CollisionContext* colCtx, WaterBox* waterBox);
...
void OoT_DynaPolyActor_Init(DynaPolyActor* dynaActor, s32 flags);
void OoT_DynaPolyActor_UnsetAllInteractFlags(DynaPolyActor* dynaActor);
void OoT_DynaPolyActor_SetActorOnTop(DynaPolyActor* dynaActor);
void OoT_DynaPoly_SetPlayerOnTop(CollisionContext* colCtx, s32 floorBgId);
void OoT_DynaPoly_SetPlayerAbove(CollisionContext* colCtx, s32 floorBgId);
void OoT_DynaPolyActor_SetSwitchPressed(DynaPolyActor* dynaActor);
s32 OoT_DynaPolyActor_IsActorOnTop(DynaPolyActor* dynaActor);
s32 OoT_DynaPolyActor_IsPlayerOnTop(DynaPolyActor* dynaActor);
s32 OoT_DynaPolyActor_IsPlayerAbove(DynaPolyActor* dynaActor);
...
s32 OoT_Camera_ChangeMode(Camera* camera, s16 mode);
s32 OoT_Camera_CheckValidMode(Camera* camera, s16 mode);
s32 OoT_Camera_ChangeSetting(Camera* camera, s16 setting);
s32 Camera_ChangeDataIdx(Camera* camera, s32 camDataIdx);
s16 OoT_Camera_GetInputDirYaw(Camera* camera);
Vec3s* OoT_Camera_GetCamDir(Vec3s* dir, Camera* camera);
s16 OoT_Camera_GetCamDirPitch(Camera* camera);
s16 OoT_Camera_GetCamDirYaw(Camera* camera);
s32 OoT_Camera_AddQuake(Camera* camera, s32 arg1, s16 y, s32 countdown);
...
s32 OoT_Camera_ChangeDoorCam(Camera* camera, Actor* doorActor, s16 camDataIdx, f32 arg3, s16 timer1, s16 timer2,
                         s16 timer3);
s32 OoT_Camera_Copy(Camera* dstCamera, Camera* srcCamera);
...
s32 OoT_Collider_SetJntSphAllocType1(PlayState* play, ColliderJntSph* dest, Actor* actor,
...
s32 OoT_Collider_SetLinePoints(PlayState* play, OcLine* ocLine, Vec3f* a, Vec3f* b);
...
void OoT_Collider_SetTrisDim(PlayState* play, ColliderTris* collider, s32 index, ColliderTrisElementDimInit* init);
...
void OoT_CollisionCheck_SpawnShieldParticlesMetalSound(PlayState* play, Vec3f* v, Vec3f* actorPos);
void OoT_CollisionCheck_SpawnShieldParticlesMetal2(PlayState* play, Vec3f* v);
void OoT_CollisionCheck_SpawnShieldParticlesWood(PlayState* play, Vec3f* b, Vec3f* actorPos);
...
void OoT_Cutscene_HandleEntranceTriggers(PlayState* play);
...
void OoT_SoundSource_InitAll(PlayState* play);
void OoT_SoundSource_UpdateAll(PlayState* play);
void OoT_SoundSource_PlaySfxAtFixedWorldPos(PlayState* play, Vec3f* pos, s32 duration, u16 sfxId);
u16 ElfMessage_GetSariaText(PlayState* play);
u16 ElfMessage_GetCUpText(PlayState* play);
u16 OoT_Text_GetFaceReaction(PlayState* play, u32 reactionSet);
...
void OoT_SkelCurve_Draw(Actor* actor, PlayState* play, SkelAnimeCurve* skelCurve,
...
s32 OoT_Jpeg_Decode(void* data, void* zbuffer, void* workBuff, u32 workSize);
...
void OoT_Environment_GraphCallback(GraphicsContext* gfxCtx, void* param);
void OoT_Environment_Init(PlayState* play, EnvironmentContext* envCtx, s32 unused);
u8 OoT_Environment_SmoothStepToU8(u8* pvalue, u8 target, u8 scale, u8 step, u8 minStep);
u8 Environment_SmoothStepToS8(s8* pvalue, s8 target, u8 scale, u8 step, u8 minStep);
f32 OoT_Environment_LerpWeight(u16 max, u16 min, u16 val);
f32 OoT_Environment_LerpWeightAccelDecel(u16 endFrame, u16 startFrame, u16 curFrame, u16 accelDuration, u16 decelDuration);
void OoT_Environment_UpdateSkybox(PlayState* play, u8 skyboxId, EnvironmentContext* envCtx, SkyboxContext* skyboxCtx);
void OoT_Environment_EnableUnderwaterLights(PlayState* play, s32 waterLightsIndex);
void OoT_Environment_DisableUnderwaterLights(PlayState* play);
void OoT_Environment_Update(PlayState* play, EnvironmentContext* envCtx, LightContext* lightCtx,
                        PauseContext* pauseCtx, MessageContext* msgCtx, GameOverContext* gameOverCtx,
                        GraphicsContext* gfxCtx);
void Environment_DrawSunAndMoon(PlayState* play);
void OoT_Environment_DrawSunLensFlare(PlayState* play, EnvironmentContext* envCtx, View* view,
                                  GraphicsContext* gfxCtx, Vec3f pos, s32 unused);
void OoT_Environment_DrawLensFlare(PlayState* play, EnvironmentContext* envCtx, View* view,
                               GraphicsContext* gfxCtx, Vec3f pos, s32 unused, s16 arg6, f32 arg7, s16 arg8, u8 arg9);
void OoT_Environment_DrawRain(PlayState* play, View* view, GraphicsContext* gfxCtx);
void func_80074CE8(PlayState* play, u32 arg1);
void OoT_Environment_DrawSkyboxFilters(PlayState* play);
void OoT_Environment_UpdateLightningStrike(PlayState* play);
void OoT_Environment_AddLightningBolts(PlayState* play, u8 num);
void OoT_Environment_DrawLightning(PlayState* play, s32 unused);
void OoT_Environment_PlaySceneSequence(PlayState* play);
void OoT_Environment_DrawCustomLensFlare(PlayState* play);
void OoT_Environment_InitGameOverLights(PlayState* play);
void OoT_Environment_FadeInGameOverLights(PlayState* play);
void OoT_Environment_FadeOutGameOverLights(PlayState* play);
void OoT_Environment_FillScreen(GraphicsContext* gfxCtx, u8 red, u8 green, u8 blue, u8 alpha, u8 drawFlags);
void OoT_Environment_DrawSandstorm(PlayState* play, u8 sandstormState);
void OoT_Environment_AdjustLights(PlayState* play, f32 arg1, f32 arg2, f32 arg3, f32 arg4);
s32 Environment_GetBgsDayCount(void);
void Environment_ClearBgsDayCount(void);
s32 OoT_Environment_GetTotalDays(void);
void OoT_Environment_ForcePlaySequence(u16 seqId);
s32 OoT_Environment_IsForcedSequenceDisabled(void);
void OoT_Environment_PlayStormNatureAmbience(PlayState* play);
void OoT_Environment_StopStormNatureAmbience(PlayState* play);
void OoT_Environment_WarpSongLeave(PlayState* play);
f32 OoT_Math_CosS(s16 angle);
f32 OoT_Math_SinS(s16 angle);
f32 Math_AccurateCosS(s16 angle);
f32 Math_AccurateSinS(s16 angle);
s32 OoT_Math_ScaledStepToS(s16* pValue, s16 target, s16 step);
s32 OoT_Math_StepToS(s16* pValue, s16 target, s16 step);
s32 OoT_Math_StepToF(f32* pValue, f32 target, f32 step);
s32 OoT_Math_StepUntilAngleS(s16* pValue, s16 limit, s16 step);
s32 Math_StepUntilS(s16* pValue, s16 limit, s16 step);
s32 OoT_Math_StepToAngleS(s16* pValue, s16 target, s16 step);
s32 OoT_Math_StepUntilF(f32* pValue, f32 limit, f32 step);
s32 OoT_Math_AsymStepToF(f32* pValue, f32 target, f32 incrStep, f32 decrStep);
void func_80077D10(f32* arg0, s16* arg1, Input* input);
s16 OoT_Rand_S16Offset(s16 base, s16 range);
void OoT_Math_Vec3f_Copy(Vec3f* dest, Vec3f* src);
void OoT_Math_Vec3s_ToVec3f(Vec3f* dest, Vec3s* src);
void OoT_Math_Vec3f_Sum(Vec3f* a, Vec3f* b, Vec3f* dest);
void OoT_Math_Vec3f_Diff(Vec3f* a, Vec3f* b, Vec3f* dest);
void OoT_Math_Vec3s_DiffToVec3f(Vec3f* dest, Vec3s* a, Vec3s* b);
void OoT_Math_Vec3f_Scale(Vec3f* vec, f32 scaleF);
f32 OoT_Math_Vec3f_DistXYZ(Vec3f* a, Vec3f* b);
f32 OoT_Math_Vec3f_DistXYZAndStoreDiff(Vec3f* a, Vec3f* b, Vec3f* dest);
f32 OoT_Math_Vec3f_DistXZ(Vec3f* a, Vec3f* b);
s16 OoT_Math_Vec3f_Yaw(Vec3f* a, Vec3f* b);
s16 OoT_Math_Vec3f_Pitch(Vec3f* a, Vec3f* b);
void OoT_Actor_ProcessInitChain(Actor* actor, InitChainEntry* initChain);
f32 OoT_Math_SmoothStepToF(f32* pValue, f32 target, f32 fraction, f32 step, f32 minStep);
void OoT_Math_ApproachF(f32* pValue, f32 target, f32 fraction, f32 step);
void OoT_Math_ApproachZeroF(f32* pValue, f32 fraction, f32 step);
f32 Math_SmoothStepToDegF(f32* pValue, f32 target, f32 fraction, f32 step, f32 minStep);
s16 OoT_Math_SmoothStepToS(s16* pValue, s16 target, s16 scale, s16 step, s16 minStep);
void OoT_Math_ApproachS(s16* pValue, s16 target, s16 scale, s16 step);
void OoT_Color_RGBA8_Copy(Color_RGBA8* dst, Color_RGBA8* src);
...
void OoT_LightContext_SetFog(LightContext* lightCtx, u8 arg1, u8 arg2, u8 arg3, s16 numLights, s16 arg5);
...
f32 OoT_OLib_Vec3fDist(Vec3f* a, Vec3f* b);
f32 OoT_OLib_Vec3fDistXZ(Vec3f* a, Vec3f* b);
f32 OoT_OLib_ClampMinDist(f32 val, f32 min);
f32 OoT_OLib_ClampMaxDist(f32 val, f32 max);
Vec3f* OoT_OLib_Vec3fDistNormalize(Vec3f* dest, Vec3f* a, Vec3f* b);
Vec3f* OLib_VecSphGeoToVec3f(Vec3f* dest, VecSph* sph);
VecSph* OoT_OLib_Vec3fToVecSph(VecSph* dest, Vec3f* vec);
VecSph* OLib_Vec3fToVecSphGeo(VecSph* arg0, Vec3f* arg1);
VecSph* OLib_Vec3fDiffToVecSphGeo(VecSph* arg0, Vec3f* a, Vec3f* b);
Vec3f* OoT_OLib_Vec3fDiffRad(Vec3f* dest, Vec3f* a, Vec3f* b);
...
void OoT_Interface_SetSceneRestrictions(PlayState* play);
...
u8 OoT_Item_Give(PlayState* play, u8 item);
u16 Randomizer_Item_Give(PlayState* play, GetItemEntry giEntry);
u8 OoT_Item_CheckObtainability(u8 item);
void OoT_Inventory_DeleteItem(u16 item, u16 invSlot);
s32 OoT_Inventory_ReplaceItem(PlayState* play, u16 oldItem, u16 newItem);
s32 OoT_Inventory_HasEmptyBottle(void);
bool Inventory_HasEmptyBottleSlot(void);
s32 Inventory_HasSpecificBottle(u8 bottleItem);
void OoT_Inventory_UpdateBottleItem(PlayState* play, u8 item, u8 cButton);
s32 OoT_Inventory_ConsumeFairy(PlayState* play);
...
s32 OoT_Health_ChangeBy(PlayState* play, s16 healthChange);
void OoT_Rupees_ChangeBy(s16 rupeeChange);
void OoT_Inventory_ChangeAmmo(s16 item, s16 ammoChange);
void Magic_Fill(PlayState* play);
void OoT_Magic_Reset(PlayState* play);
...
void OoT_Interface_Draw(PlayState* play);
void Interface_DrawTotalGameplayTimer(PlayState* play);
void OoT_Interface_Update(PlayState* play);
Path* OoT_Path_GetByIndex(PlayState* play, s16 index, s16 max);
f32 OoT_Path_OrientAndGetDistSq(Actor* actor, Path* path, s16 waypoint, s16* yaw);
void OoT_Path_CopyLastPoint(Path* path, Vec3f* dest);
...
s32 OoT_Player_InBlockingCsMode(PlayState* play, Player* player);
s32 OoT_Player_TryCsAction(PlayState* play, Actor* actor, s32 csAction);
s32 OoT_Player_InCsMode(PlayState* play);
s32 OoT_Player_CheckHostileLockOn(Player* player);
s32 Player_IsChildWithHylianShield(Player* player);
s32 OoT_Player_ActionToModelGroup(Player* player, s32 actionParam);
void OoT_Player_SetModelsForHoldingShield(Player* player);
void OoT_Player_SetModels(Player* player, s32 modelGroup);
void OoT_Player_SetModelGroup(Player* player, s32 modelGroup);
void func_8008EC70(Player* player);
void OoT_Player_SetEquipmentData(PlayState* play, Player* player);
void OoT_Player_UpdateBottleHeld(PlayState* play, Player* player, s32 item, s32 actionParam);
void func_80837C0C(PlayState* play, Player* this, s32 arg2, f32 arg3, f32 arg4, s16 arg5, s32 arg6);
void OoT_Player_ReleaseLockOn(Player* player);
void OoT_Player_ClearZTargeting(Player* player);
void OoT_Player_SetAutoLockOnActor(PlayState* play, Actor* actor);
s32 func_8008EF44(PlayState* play, s32 ammo);
s32 OoT_Player_IsBurningStickInRange(PlayState* play, Vec3f* pos, f32 radius, f32 arg3);
s32 OoT_Player_GetStrength(void);
u8 OoT_Player_GetMask(PlayState* play);
Player* Player_UnsetMask(PlayState* play);
s32 OoT_Player_HasMirrorShieldEquipped(PlayState* play);
...
s32 OoT_Player_GetMeleeWeaponHeld(Player* player);
...
s32 OoT_Player_GetBottleHeld(Player* player);
s32 Player_ActionToExplosive(Player* player, s32 actionParam);
s32 OoT_Player_GetExplosiveHeld(Player* player);
s32 func_8008F2BC(Player* player, s32 actionParam);
s32 OoT_Player_GetEnvironmentalHazard(PlayState* play);
void OoT_Player_DrawImpl(PlayState* play, void** skeleton, Vec3s* jointTable, s32 dListCount, s32 lod, s32 tunic,
                   s32 boots, s32 face, OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* this);
s32 OoT_Player_OverrideLimbDrawGameplayCommon(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* data);
s32 OoT_Player_OverrideLimbDrawGameplayDefault(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* data);
s32 OoT_Player_OverrideLimbDrawGameplayFirstPerson(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* data);
s32 OoT_Player_OverrideLimbDrawGameplayCrawling(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* data);
u8 func_80090480(PlayState* play, ColliderQuad* collider, WeaponInfo* weaponDim, Vec3f* newTip,
                 Vec3f* newBase);
void OoT_Player_DrawGetItem(PlayState* play, Player* player);
void OoT_Player_PostLimbDrawGameplay(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* data);
...
void OoT_Quake_UpdateShakeInfo(QuakeRequest* req, ShakeInfo* shake, f32 y, f32 x);
...
s16 OoT_Quake_GetFreeIndex(void);
QuakeRequest* Quake_AddImpl(Camera* cam, u32 callbackIdx);
void OoT_Quake_Remove(QuakeRequest* req);
QuakeRequest* OoT_Quake_GetRequest(s16 idx);
QuakeRequest* OoT_Quake_SetValue(s16 idx, s16 valueType, s16 value);
u32 OoT_Quake_SetSpeed(s16 idx, s16 value);
...
Gfx* OoT_Gfx_TexScrollEx(GraphicsContext* gfxCtx, u32 x, u32 y, s32 width, s32 height, s32 xStep, s32 yStep);
Gfx* OoT_Gfx_TwoTexScroll(GraphicsContext* gfxCtx, s32 tile1, u32 x1, u32 y1, s32 width1, s32 height1, s32 tile2, u32 x2,
                      u32 y2, s32 width2, s32 height2);
Gfx* OoT_Gfx_TwoTexScrollEx(GraphicsContext* gfxCtx, s32 tile1, u32 x1, u32 y1, s32 width1, s32 height1, s32 tile2, u32 x2,
...
Gfx* OoT_Gfx_TwoTexScrollEnvColorEx(GraphicsContext* gfxCtx, s32 tile1, u32 x1, u32 y1, s32 width1, s32 height1, s32 tile2,
...
void OoT_Inventory_ChangeEquipment(s16 equipment, u16 value);
u8 OoT_Inventory_DeleteEquipment(PlayState* play, s16 equipment);
void OoT_Inventory_ChangeUpgrade(s16 upgrade, s16 value);
void Object_InitBank(PlayState* play, ObjectContext* objectCtx);
void Object_UpdateBank(ObjectContext* objectCtx);
s32 Object_GetIndex(ObjectContext* objectCtx, s16 objectId);
s32 OoT_Object_IsLoaded(ObjectContext* objectCtx, s32 bankIndex);
void func_800981B8(ObjectContext* objectCtx);
s32 OoT_Scene_ExecuteCommands(PlayState* play, SceneCmd* sceneCmd);
void TransitionActor_InitContext(GameState* state, TransitionActorContext* transiActorCtx);
void Scene_SetTransitionForNextEntrance(PlayState* play);
void OoT_Scene_Draw(PlayState* play);
void OoT_SkelAnime_DrawLod(PlayState* play, void** skeleton, Vec3s* jointTable,
                       OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg, s32 dListIndex);
void OoT_SkelAnime_DrawFlexLod(PlayState* play, void** skeleton, Vec3s* jointTable, s32 dListCount,
...
void OoT_SkelAnime_DrawOpa(PlayState* play, void** skeleton, Vec3s* jointTable,
                       OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg);
void OoT_SkelAnime_DrawFlexOpa(PlayState* play, void** skeleton, Vec3s* jointTable, s32 dListCount,
                           OverrideLimbDrawOpa overrideLimbDraw, PostLimbDrawOpa postLimbDraw, void* arg);
s16 OoT_Animation_GetLength(void* animation);
s16 OoT_Animation_GetLastFrame(void* animation);
s32 OoT_SkelAnime_GetFrameDataLegacy(LegacyAnimationHeader* animation, s32 frame, Vec3s* frameTable);
s16 OoT_Animation_GetLimbCountLegacy(LegacyAnimationHeader* animation);
s16 OoT_Animation_GetLengthLegacy(LegacyAnimationHeader* animation);
s16 OoT_Animation_GetLastFrameLegacy(LegacyAnimationHeader* animation);
Gfx* OoT_SkelAnime_Draw(PlayState* play, void** skeleton, Vec3s* jointTable, OverrideLimbDraw overrideLimbDraw,
                    PostLimbDraw postLimbDraw, void* arg, Gfx* gfx);
Gfx* OoT_SkelAnime_DrawFlex(PlayState* play, void** skeleton, Vec3s* jointTable, s32 dListCount,
                        OverrideLimbDraw overrideLimbDraw, PostLimbDraw postLimbDraw, void* arg, Gfx* gfx);
void OoT_SkelAnime_InterpFrameTable(s32 limbCount, Vec3s* dst, Vec3s* start, Vec3s* target, f32 weight);
...
void OoT_Animation_SetMorph(PlayState* play, SkelAnime* skelAnime, f32 morphFrames);
...
s32 OoT_SkelAnime_Init(PlayState* play, SkelAnime* skelAnime, SkeletonHeader* skeletonHeaderSeg,
                   AnimationHeader* animation, Vec3s* jointTable, Vec3s* morphTable, s32 limbCount);
s32 OoT_SkelAnime_InitFlex(PlayState* play, SkelAnime* skelAnime, FlexSkeletonHeader* skeletonHeaderSeg,
                       AnimationHeader* animation, Vec3s* jointTable, Vec3s* morphTable, s32 limbCount);
s32 OoT_SkelAnime_InitSkin(PlayState* play, SkelAnime* skelAnime, SkeletonHeader* skeletonHeaderSeg,
                       AnimationHeader* animation);
s32 OoT_SkelAnime_Update(SkelAnime* skelAnime);
void OoT_Animation_ChangeImpl(SkelAnime* skelAnime, AnimationHeader* animation, f32 playSpeed, f32 startFrame, f32 endFrame,
                          u8 mode, f32 morphFrames, s8 taper);
void OoT_Animation_Change(SkelAnime* skelAnime, AnimationHeader* animation, f32 playSpeed, f32 startFrame, f32 endFrame,
                      u8 mode, f32 morphFrames);
void OoT_Animation_PlayOnce(SkelAnime* skelAnime, AnimationHeader* animation);
void OoT_Animation_MorphToPlayOnce(SkelAnime* skelAnime, AnimationHeader* animation, f32 morphFrames);
void OoT_Animation_PlayOnceSetSpeed(SkelAnime* skelAnime, AnimationHeader* animation, f32 playSpeed);
void OoT_Animation_PlayLoop(SkelAnime* skelAnime, AnimationHeader* animation);
void OoT_Animation_MorphToLoop(SkelAnime* skelAnime, AnimationHeader* animation, f32 morphFrames);
void OoT_Animation_PlayLoopSetSpeed(SkelAnime* skelAnime, AnimationHeader* animation, f32 playSpeed);
void OoT_Animation_EndLoop(SkelAnime* skelAnime);
void OoT_Animation_Reverse(SkelAnime* skelAnime);
void OoT_SkelAnime_CopyFrameTableTrue(SkelAnime* skelAnime, Vec3s* dst, Vec3s* src, u8* copyFlag);
void OoT_SkelAnime_CopyFrameTableFalse(SkelAnime* skelAnime, Vec3s* dst, Vec3s* src, u8* copyFlag);
void OoT_SkelAnime_UpdateTranslation(SkelAnime* skelAnime, Vec3f* pos, s16 angle);
s32 OoT_Animation_OnFrame(SkelAnime* skelAnime, f32 frame);
void OoT_SkelAnime_Free(SkelAnime* skelAnime, PlayState* play);
void OoT_SkelAnime_CopyFrameTable(SkelAnime* skelAnime, Vec3s* dst, Vec3s* src);

void OoT_Skin_UpdateVertices(MtxF* mtx, SkinVertex* skinVertices, SkinLimbModif* modifEntry, Vtx* vtxBuf, Vec3f* pos);
void OoT_Skin_DrawAnimatedLimb(GraphicsContext* gfxCtx, Skin* skin, s32 limbIndex, s32 arg3, s32 drawFlags);
void OoT_Skin_DrawLimb(GraphicsContext* gfxCtx, Skin* skin, s32 limbIndex, Gfx* dlistOverride, s32 drawFlags);
...
void OoT_Skin_GetLimbPos(Skin* skin, s32 limbIndex, Vec3f* arg2, Vec3f* dst);
...
s32 OoT_Skin_ApplyAnimTransformations(Skin* skin, MtxF* mf, Actor* actor, s32 setTranslation);

void OoT_SkinMatrix_Vec3fMtxFMultXYZW(MtxF* mf, Vec3f* src, Vec3f* xyzDest, f32* wDest);
void OoT_SkinMatrix_Vec3fMtxFMultXYZ(MtxF* mf, Vec3f* src, Vec3f* dest);
void OoT_SkinMatrix_MtxFMtxFMult(MtxF* mfA, MtxF* mfB, MtxF* dest);
void OoT_SkinMatrix_GetClear(MtxF** mf);
void OoT_SkinMatrix_Clear(MtxF* mf);
void OoT_SkinMatrix_MtxFCopy(MtxF* src, MtxF* dest);
s32 OoT_SkinMatrix_Invert(MtxF* src, MtxF* dest);
void OoT_SkinMatrix_SetScale(MtxF* mf, f32 x, f32 y, f32 z);
void SkinMatrix_SetRotateZYX(MtxF* mf, s16 x, s16 y, s16 z);
void OoT_SkinMatrix_SetTranslate(MtxF* mf, f32 x, f32 y, f32 z);
...
void SkinMatrix_SetTranslateRotateZYX(MtxF* dest, s16 rotX, s16 rotY, s16 rotZ, f32 translateX, f32 translateY,
                                      f32 translateZ);
Mtx* OoT_SkinMatrix_MtxFToNewMtx(GraphicsContext* gfxCtx, MtxF* src);
void SkinMatrix_SetRotateAxis(MtxF* mf, s16 angle, f32 axisX, f32 axisY, f32 axisZ);
void OoT_Sram_OpenSave();
void OoT_Sram_InitSave(FileChooseContext* fileChoose);
...
void OoT_View_Init(View*, GraphicsContext*);
void func_800AA358(View* view, Vec3f* eye, Vec3f* lookAt, Vec3f* up);
void func_800AA3F0(View* view, Vec3f* eye, Vec3f* lookAt, Vec3f* up);
void OoT_View_SetScale(View* view, f32 scale);
void OoT_View_GetScale(View* view, f32* scale);
...
void OoT_View_SetViewport(View* view, Viewport* viewport);
void OoT_View_GetViewport(View* view, Viewport* viewport);
void OoT_View_SetDistortionOrientation(View* view, f32 rotX, f32 rotY, f32 rotZ);
void OoT_View_SetDistortionScale(View* view, f32 scaleX, f32 scaleY, f32 scaleZ);
s32 OoT_View_SetDistortionSpeed(View* view, f32 speed);
void OoT_View_InitDistortion(View* view);
void OoT_View_ClearDistortion(View* view);
void OoT_View_SetDistortion(View* view, Vec3f orientation, Vec3f scale, f32 speed);
s32 OoT_View_StepDistortion(View* view, Mtx* projectionMtx);
...
void OoT_ViMode_LogPrint(OSViMode* viMode);
...
void OoT_TransitionTriforce_Start(void* this);
void* OoT_TransitionTriforce_Init(void* this);
void OoT_TransitionTriforce_Destroy(void* this);
void OoT_TransitionTriforce_Update(void* this, s32 updateRate);
void OoT_TransitionTriforce_SetColor(void* this, u32 color);
void OoT_TransitionTriforce_SetType(void* this, s32 type);
void OoT_TransitionTriforce_Draw(void* this, Gfx** gfxP);
s32 OoT_TransitionTriforce_IsDone(void* this);
...
void OoT_TransitionFade_Start(void* this);
void* OoT_TransitionFade_Init(void* this);
void OoT_TransitionFade_Destroy(void* this);
void OoT_TransitionFade_Update(void* this, s32 updateRate);
void OoT_TransitionFade_Draw(void* this, Gfx** gfxP);
s32 OoT_TransitionFade_IsDone(void* this);
void OoT_TransitionFade_SetColor(void* this, u32 color);
void OoT_TransitionFade_SetType(void* this, s32 type);
...
void OoT_Play_Main(GameState* thisx);
u8 CheckStoneCount();
u8 CheckMedallionCount();
u8 CheckDungeonCount();
u8 CheckBridgeRewardCount();
u8 CheckLACSRewardCount();
s32 OoT_Play_InCsMode(PlayState* play);
f32 func_800BFCB8(PlayState* play, MtxF* mf, Vec3f* vec);
void* OoT_Play_LoadFile(PlayState* play, RomFile* file);
void OoT_Play_SpawnScene(PlayState* play, s32 sceneId, s32 spawn);
void func_800C016C(PlayState* play, Vec3f* src, Vec3f* dest);
s16 OoT_Play_CreateSubCamera(PlayState* play);
s16 OoT_Play_GetActiveCamId(PlayState* play);
s16 OoT_Play_ChangeCameraStatus(PlayState* play, s16 camId, s16 status);
void OoT_Play_ClearCamera(PlayState* play, s16 camId);
void OoT_Play_ClearAllSubCameras(PlayState* play);
Camera* OoT_Play_GetCamera(PlayState* play, s16 camId);
...
s32 OoT_Play_SetCameraRoll(PlayState* play, s16 camId, s16 roll);
void OoT_Play_CopyCamera(PlayState* play, s16 camId1, s16 camId2);
...
void OoT_Play_SetupRespawnPoint(PlayState* play, s32 respawnMode, s32 playerParams);
void Play_TriggerVoidOut(PlayState* play);
void Play_TriggerRespawn(PlayState* play);
s32 func_800C0CB8(PlayState* play);
s32 OoT_FrameAdvance_IsEnabled(PlayState* play);
...
void OoT_GameState_SetFBFilter(Gfx** gfx);
// ? func_800C4344(?);
void GameState_DrawInputDisplay(u16 input, Gfx** gfx);
void OoT_GameState_Draw(GameState* gameState, GraphicsContext* gfxCtx);
void OoT_GameState_SetFrameBuffer(GraphicsContext* gfxCtx);
// ? func_800C49F4(?);
void GameState_ReqPadData(GameState* gameState);
void OoT_GameState_Update(GameState* gameState);
void OoT_GameState_InitArena(GameState* gameState, size_t size);
void OoT_GameState_Realloc(GameState* gameState, size_t size);
void OoT_GameState_Init(GameState* gameState, GameStateFunc init, GraphicsContext* gfxCtx);
void OoT_GameState_Destroy(GameState* gameState);
GameStateFunc OoT_GameState_GetInit(GameState* gameState);
u32 OoT_GameState_IsRunning(GameState* gameState);
...
void OoT_Graph_ThreadEntry(void*);
...
void OoT_Sched_HandleReset(SchedContext* sc);
void Sched_HandleStart(SchedContext* sc);
void OoT_Sched_QueueTask(SchedContext* sc, OSScTask* task);
void OoT_Sched_Yield(SchedContext* sc);
OSScTask* func_800C89D4(SchedContext* sc, OSScTask* task);
s32 OoT_Sched_Schedule(SchedContext* sc, OSScTask** sp, OSScTask** dp, s32 state);
void func_800C8BC4(SchedContext* sc, OSScTask* task);
u32 Sched_IsComplete(SchedContext* sc, OSScTask* task);
void OoT_Sched_RunTask(SchedContext* sc, OSScTask* spTask, OSScTask* dpTask);
void OoT_Sched_HandleEntry(SchedContext* sc);
void OoT_Sched_HandleRetrace(SchedContext* sc);
void OoT_Sched_HandleRSPDone(SchedContext* sc);
void OoT_Sched_HandleRDPDone(SchedContext* sc);
void OoT_Sched_SendEntryMsg(SchedContext* sc);
void OoT_Sched_ThreadEntry(void* arg);
...
void OoT_SpeedMeter_InitAllocEntry(SpeedMeterAllocEntry* entry, u32 maxval, u32 val, u16 backColor, u16 foreColor, u32 ulx,
...
s32 OoT_Math3D_PlaneVsLineSegClosestPoint(f32 planeAA, f32 planeAB, f32 planeAC, f32 planeADist, f32 planeBA, f32 planeBB,
                                      f32 planeBC, f32 planeBDist, Vec3f* linePointA, Vec3f* linePointB,
                                      Vec3f* closestPoint);
void OoT_Math3D_LineClosestToPoint(Linef* line, Vec3f* pos, Vec3f* closestPoint);
s32 OoT_Math3D_PlaneVsPlaneVsLineClosestPoint(f32 planeAA, f32 planeAB, f32 planeAC, f32 planeADist, f32 planeBA,
                                          f32 planeBB, f32 planeBC, f32 planeBDist, Vec3f* point, Vec3f* closestPoint);
void OoT_Math3D_LineSplitRatio(Vec3f* v0, Vec3f* v1, f32 ratio, Vec3f* ret);
f32 OoT_Math3D_Cos(Vec3f* a, Vec3f* b);
s32 OoT_Math3D_CosOut(Vec3f* a, Vec3f* b, f32* dst);
void OoT_Math3D_Vec3fReflect(Vec3f* vec, Vec3f* normal, Vec3f* reflVec);
s32 OoT_Math3D_PointInSquare2D(f32 upperLeftX, f32 lowerRightX, f32 upperLeftY, f32 lowerRightY, f32 x, f32 y);
f32 OoT_Math3D_Dist1DSq(f32 a, f32 b);
f32 OoT_Math3D_Dist2DSq(f32 x0, f32 y0, f32 x1, f32 y1);
f32 OoT_Math3D_Vec3fMagnitudeSq(Vec3f* vec);
f32 OoT_Math3D_Vec3fMagnitude(Vec3f* vec);
f32 OoT_Math3D_Vec3fDistSq(Vec3f* a, Vec3f* b);
void OoT_Math3D_Vec3f_Cross(Vec3f* a, Vec3f* b, Vec3f* ret);
void OoT_Math3D_SurfaceNorm(Vec3f* va, Vec3f* vb, Vec3f* vc, Vec3f* normal);
f32 OoT_Math3D_Vec3f_DistXYZ(Vec3f* a, Vec3f* b);
s32 OoT_Math3D_PointRelativeToCubeFaces(Vec3f* point, Vec3f* min, Vec3f* max);
s32 OoT_Math3D_PointRelativeToCubeEdges(Vec3f* point, Vec3f* min, Vec3f* max);
s32 OoT_Math3D_PointRelativeToCubeVertices(Vec3f* point, Vec3f* min, Vec3f* max);
s32 OoT_Math3D_LineVsCube(Vec3f* min, Vec3f* max, Vec3f* a, Vec3f* b);
void OoT_Math3D_RotateXZPlane(Vec3f* pointOnPlane, s16 angle, f32* a, f32* c, f32* d);
void OoT_Math3D_DefPlane(Vec3f* va, Vec3f* vb, Vec3f* vc, f32* nx, f32* ny, f32* nz, f32* originDist);
...
s32 OoT_Math3D_TriChkPointParaYSlopedY(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 z, f32 x);
s32 OoT_Math3D_TriChkPointParaYIntersectDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, f32 z,
                                         f32 x, f32* yIntersect, f32 chkDist);
s32 OoT_Math3D_TriChkPointParaYIntersectInsideTri(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist,
                                              f32 z, f32 x, f32* yIntersect, f32 chkDist);
s32 OoT_Math3D_TriChkLineSegParaYIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, f32 z,
                                       f32 x, f32* yIntersect, f32 y0, f32 y1);
s32 OoT_Math3D_TriChkPointParaYDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, Plane* plane, f32 z, f32 x, f32 chkDist);
s32 OoT_Math3D_TriChkPointParaXIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, f32 y,
                                     f32 z, f32* xIntersect);
s32 OoT_Math3D_TriChkLineSegParaXIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, f32 y,
...
s32 OoT_Math3D_TriChkPointParaZIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, f32 x,
                                     f32 y, f32* zIntersect);
s32 OoT_Math3D_TriChkLineSegParaZIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, f32 x,
                                       f32 y, f32* zIntersect, f32 z0, f32 z1);
s32 OoT_Math3D_TriChkLineSegParaZDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, Plane* plane, f32 x, f32 y, f32 chkDist);
s32 OoT_Math3D_LineSegVsPlane(f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* linePointA, Vec3f* linePointB,
                          Vec3f* intersect, s32 fromFront);
void OoT_Math3D_TriNorm(TriNorm* tri, Vec3f* va, Vec3f* vb, Vec3f* vc);
s32 Math3D_PointDistToLine2D(f32 x0, f32 y0, f32 x1, f32 y1, f32 x2, f32 y2, f32* lineLenSq);
s32 OoT_Math3D_LineVsSph(Sphere16* sphere, Linef* line);
s32 OoT_Math3D_TriVsSphIntersect(Sphere16* sphere, TriNorm* tri, Vec3f* intersectPoint);
s32 OoT_Math3D_CylVsLineSeg(Cylinder16* cyl, Vec3f* linePointA, Vec3f* linePointB, Vec3f* intersectA, Vec3f* intersectB);
s32 OoT_Math3D_CylVsTri(Cylinder16* cyl, TriNorm* tri);
s32 OoT_Math3D_CylTriVsIntersect(Cylinder16* cyl, TriNorm* tri, Vec3f* intersect);
s32 OoT_Math3D_SphVsSph(Sphere16* sphereA, Sphere16* sphereB);
s32 OoT_Math3D_SphVsSphOverlap(Sphere16* sphereA, Sphere16* sphereB, f32* overlapSize);
...
s32 OoT_Math3D_SphVsCylOverlapCenterDist(Sphere16* sph, Cylinder16* cyl, f32* overlapSize, f32* centerDist);
...
s32 OoT_Math3D_TriVsTriIntersect(TriNorm* ta, TriNorm* tb, Vec3f* intersect);
s32 OoT_Math3D_XZInSphere(Sphere16* sphere, f32 x, f32 z);
s32 OoT_Math3D_XYInSphere(Sphere16* sphere, f32 x, f32 y);
s32 OoT_Math3D_YZInSphere(Sphere16* sphere, f32 y, f32 z);
void Math3D_DrawSphere(PlayState* play, Sphere16* sph);
void Math3D_DrawCylinder(PlayState* play, Cylinder16* cyl);
s16 OoT_Math_Atan2S(f32 x, f32 y);
f32 OoT_Math_Atan2F(f32 x, f32 y);
void OoT_Matrix_Init(GameState* gameState);
...
Mtx* OoT_Matrix_MtxFToNewMtx(MtxF* src, GraphicsContext* gfxCtx);
...
void Matrix_SetTranslateScaleMtx2(Mtx* mtx, f32 scaleX, f32 scaleY, f32 scaleZ, f32 translateX, f32 translateY,
                                  f32 translateZ);
uintptr_t OoT_SysUcode_GetUCodeBoot(void);
size_t OoT_SysUcode_GetUCodeBootSize(void);
uint32_t OoT_SysUcode_GetUCode(void);
...
void OoT_Fault_AddClient(FaultClient*, void*, void*, void*);
void OoT_Fault_RemoveClient(FaultClient*);
void OoT_Fault_AddAddrConvClient(FaultAddrConvClient*, void*, void*);
void OoT_Fault_RemoveAddrConvClient(FaultAddrConvClient*);
u32 OoT_Fault_ConvertAddress(FaultAddrConvClient*);
void OoT_Fault_Sleep(u32);
void OoT_Fault_PadCallback(Input*);
void OoT_Fault_UpdatePadImpl();
u32 OoT_Fault_WaitForInputImpl();
void OoT_Fault_WaitForInput();
void OoT_Fault_DrawRec(s32, s32, s32, s32, u16);
void OoT_Fault_FillScreenBlack();
void OoT_Fault_FillScreenRed();
void OoT_Fault_DrawCornerRec(u16);
void OoT_Fault_PrintFReg(s32, f32*);
void OoT_Fault_LogFReg(s32, f32*);
void OoT_Fault_PrintFPCR(u32);
void Fault_LogFPCR(u32);
void OoT_Fault_PrintThreadContext(OSThread*);
void Fault_LogThreadContext(OSThread*);
OSThread* OoT_Fault_FindFaultedThread();
void OoT_Fault_Wait5Seconds();
void OoT_Fault_WaitForButtonCombo();
void Fault_DrawMemDumpPage(const char*, u32*, u32);
void OoT_Fault_DrawMemDump(u32, u32, u32, u32);
void OoT_Fault_WalkStack(u32* spPtr, u32* pcPtr, u32* raPtr);
void OoT_Fault_DrawStackTrace(OSThread* thread, s32 x, s32 y, s32 height);
void OoT_Fault_LogStackTrace(OSThread* thread, s32 height);
void OoT_Fault_ResumeThread(OSThread*);
void Fault_CommitFB();
void OoT_Fault_ProcessClients();
void OoT_Fault_UpdatePad();
void OoT_Fault_ThreadEntry(void*);
void Fault_SetFB(void*, u16, u16);
void OoT_Fault_Init(void);
void OoT_Fault_HangupFaultClient(const char*, const char*);
void OoT_Fault_AddHungupAndCrashImpl(const char*, const char*);
void OoT_Fault_AddHungupAndCrash(const char*, u32);
void OoT_FaultDrawer_SetOsSyncPrintfEnabled(u32);
void OoT_FaultDrawer_DrawRecImpl(s32, s32, s32, s32, u16);
void OoT_FaultDrawer_DrawChar(char);
s32 OoT_FaultDrawer_ColorToPrintColor(u16);
void OoT_FaultDrawer_UpdatePrintColor();
void OoT_FaultDrawer_SetForeColor(u16);
void OoT_FaultDrawer_SetBackColor(u16);
void OoT_FaultDrawer_SetFontColor(u16);
void OoT_FaultDrawer_SetCharPad(s8, s8);
void OoT_FaultDrawer_SetCursor(s32, s32);
void OoT_FaultDrawer_FillScreen();
void* OoT_FaultDrawer_FormatStringFunc(void*, const char*, u32);
void OoT_FaultDrawer_VPrintf(const char*, va_list);
void OoT_FaultDrawer_Printf(const char*, ...);
void OoT_FaultDrawer_DrawText(s32, s32, const char*, ...);
...
Acmd* OoT_AudioSynth_Update(Acmd* cmdStart, s32* cmdCnt, s16* aiStart, s32 aiBufLen);
void OoT_AudioHeap_DiscardFont(s32 fontId);
void OoT_AudioHeap_DiscardSequence(s32 seqId);
void OoT_AudioHeap_WritebackDCache(void* mem, size_t size);
void* OoT_AudioHeap_AllocZeroedAttemptExternal(AudioAllocPool* pool, size_t size);
void* OoT_AudioHeap_AllocAttemptExternal(AudioAllocPool* pool, size_t size);
void* OoT_AudioHeap_AllocDmaMemory(AudioAllocPool* pool, size_t size);
void* OoT_AudioHeap_AllocDmaMemoryZeroed(AudioAllocPool* pool, size_t size);
void* OoT_AudioHeap_AllocZeroed(AudioAllocPool* pool, size_t size);
void* OoT_AudioHeap_Alloc(AudioAllocPool* pool, size_t size);
...
void* OoT_AudioHeap_AllocCached(s32 tableType, ptrdiff_t size, s32 cache, s32 id);
void* OoT_AudioHeap_SearchCaches(s32 tableType, s32 arg1, s32 id);
void* OoT_AudioHeap_SearchRegularCaches(s32 tableType, s32 cache, s32 id);
void OoT_AudioHeap_LoadFilter(s16* filter, s32 filter1, s32 filter2);
s32 OoT_AudioHeap_ResetStep(void);
void OoT_AudioHeap_Init(void);
void* OoT_AudioHeap_SearchPermanentCache(s32 tableType, s32 id);
void* OoT_AudioHeap_AllocPermanent(s32 tableType, s32 id, size_t size);
void* OoT_AudioHeap_AllocSampleCache(size_t size, s32 fontId, void* sampleAddr, s8 medium, s32 cache);
void OoT_AudioHeap_ApplySampleBankCache(s32 sampleBankId);
void OoT_AudioLoad_DecreaseSampleDmaTtls(void);
uintptr_t OoT_AudioLoad_DmaSampleData(uintptr_t devAddr, size_t size, s32 arg2, u8* dmaIndexRef, s32 medium);
void OoT_AudioLoad_InitSampleDmaBuffers(s32 arg0);
s32 OoT_AudioLoad_IsFontLoadComplete(s32 fontId);
s32 OoT_AudioLoad_IsSeqLoadComplete(s32 seqId);
void OoT_AudioLoad_SetFontLoadStatus(s32 fontId, s32 status);
void OoT_AudioLoad_SetSeqLoadStatus(s32 seqId, s32 status);
void OoT_AudioLoad_SyncLoadSeqParts(s32 seqId, s32 arg1);
s32 OoT_AudioLoad_SyncLoadInstrument(s32 fontId, s32 instId, s32 drumId);
void OoT_AudioLoad_AsyncLoadSeq(s32 seqId, s32 arg1, s32 retData, OSMesgQueue* retQueue);
void OoT_AudioLoad_AsyncLoadSampleBank(s32 sampleBankId, s32 arg1, s32 retData, OSMesgQueue* retQueue);
void OoT_AudioLoad_AsyncLoadFont(s32 fontId, s32 arg1, s32 retData, OSMesgQueue* retQueue);
u8* OoT_AudioLoad_GetFontsForSequence(s32 seqId, u32* arg1);
void OoT_AudioLoad_DiscardSeqFonts(s32 seqId);
s32 OoT_AudioLoad_SyncInitSeqPlayer(s32 playerIdx, s32 seqId, s32 arg2);
s32 OoT_AudioLoad_SyncInitSeqPlayerSkipTicks(s32 playerIdx, s32 seqId, s32 arg2);
void OoT_AudioLoad_ProcessLoads(s32 resetStatus);
void OoT_AudioLoad_SetDmaHandler(DmaHandler callback);
void OoT_AudioLoad_Init(void* heap, size_t heapSize);
...
void OoT_AudioLoad_ScriptLoad(s32 tableType, s32 arg1, s8* arg2);
void OoT_AudioLoad_ProcessScriptLoads(void);
...
void OoT_Audio_WritebackDCache(void* mem, size_t size);
...
void OoT_Audio_ResetIncreasingTranspose(void);
...
void OoT_Audio_ClearSariaBgm(void);
void OoT_Audio_ClearSariaBgmAtPos(Vec3f* pos);
void OoT_Audio_PlaySariaBgm(Vec3f* pos, u16 seqId, u16 distMax);
void OoT_Audio_ClearSariaBgm2(void);
...
void OoT_PreviewSequence(u16 seqId);
void func_800F5B58(void);
void func_800F5BF0(u8 natureAmbienceId);
void OoT_Audio_PlayFanfare(u16);
...
void OoT_GfxPrint_SetColor(GfxPrint* this, u32 r, u32 g, u32 b, u32 a);
void OoT_GfxPrint_SetPosPx(GfxPrint* this, s32 x, s32 y);
void OoT_GfxPrint_SetPos(GfxPrint* this, s32 x, s32 y);
void OoT_GfxPrint_SetBasePosPx(GfxPrint* this, s32 x, s32 y);
void OoT_GfxPrint_Init(GfxPrint* this);
void OoT_GfxPrint_Destroy(GfxPrint* this);
void OoT_GfxPrint_Open(GfxPrint* this, Gfx* dList);
Gfx* OoT_GfxPrint_Close(GfxPrint* this);
s32 OoT_GfxPrint_Printf(GfxPrint* this, const char* fmt, ...);
void func_800FBCE0();
void func_800FBFD8(void);
void* OoT_Overlay_AllocateAndLoad(uintptr_t vRomStart, uintptr_t vRomEnd, void* vRamStart, void* vRamEnd);
void OoT_MtxConv_F2L(Mtx* m1, MtxF* m2);
void OoT_MtxConv_L2F(MtxF* m1, Mtx* m2);
void OoT_Overlay_Relocate(void* allocatedVRamAddress, OverlayRelocationSection* overlayInfo, void* vRamAddress);
s32 OoT_Overlay_Load(uintptr_t vRomStart, uintptr_t vRomEnd, void* vRamStart, void* vRamEnd, void* allocatedVRamAddress);
// ? func_800FC800(?);
// ? func_800FC83C(?);
// ? func_800FCAB4(?);
...
s32 OoT_PadSetup_Init(OSMesgQueue* mq, u8* outMask, OSContStatus* status);
f32 OoT_Math_FTanF(f32 x);
f32 OoT_Math_FFloorF(f32 x);
f32 OoT_Math_FCeilF(f32 x);
f32 OoT_Math_FRoundF(f32 x);
f32 OoT_Math_FNearbyIntF(f32 x);
f32 OoT_Math_FTruncF(f32 x);
f32 OoT_Math_FAtanF(f32 x);
f32 OoT_Math_FAtan2F(f32 y, f32 x);
...
u32 OoT_Rand_Next(void);
void OoT_Rand_Seed(u32 seed);
f32 OoT_Rand_ZeroOne(void);
f32 OoT_Rand_Centered(void);
void OoT_Rand_Seed_Variable(u32* rndNum, u32 seed);
u32 OoT_Rand_Next_Variable(u32* rndNum);
f32 OoT_Rand_ZeroOne_Variable(u32* rndNum);
f32 OoT_Rand_Centered_Variable(u32* rndNum);
...
ArenaNode* OoT_ArenaImpl_GetLastBlock(Arena* arena);
...
s32 OoT_PrintUtils_VPrintf(PrintCallback* pfn, const char* fmt, va_list args);
s32 OoT_PrintUtils_Printf(PrintCallback* pfn, const char* fmt, ...);
...
void OoT_JpegUtils_ProcessQuantizationTable(u8* dqt, JpegQuantizationTable* qt, u8 count);
s32 OoT_JpegUtils_ParseHuffmanCodesLengths(u8* ptr, u8* codesLengths);
s32 OoT_JpegUtils_GetHuffmanCodes(u8* codesLengths, u16* codes);
s32 OoT_JpegUtils_SetHuffmanTable(u8* data, JpegHuffmanTable* ht, u16* codes);
u32 OoT_JpegUtils_ProcessHuffmanTableImpl(u8* data, JpegHuffmanTable* ht, u8* codesLengths, u16* codes, u8 isAc);
u32 OoT_JpegUtils_ProcessHuffmanTable(u8* dht, JpegHuffmanTable* ht, u8* codesLengths, u16* codes, u8 count);
void OoT_JpegUtils_SetHuffmanTableOld(u8* data, JpegHuffmanTableOld* ht, u8* codesLengths, u16* codes, s16 count, u8 isAc);
u32 OoT_JpegUtils_ProcessHuffmanTableImplOld(u8* dht, JpegHuffmanTableOld* ht, u8* codesLengths, u16* codes);
...
u8 OoT_Message_ShouldAdvance(PlayState* play);
u8 OoT_Message_ShouldAdvanceSilent(PlayState* play);
void OoT_Message_CloseTextbox(PlayState*);
void OoT_Message_StartTextbox(PlayState* play, u16 textId, Actor* actor);
void OoT_Message_ContinueTextbox(PlayState* play, u16 textId);
void func_8010BD58(PlayState* play, u16 arg1);
void func_8010BD88(PlayState* play, u16 arg1);
u8 OoT_Message_GetState(MessageContext* msgCtx);
void OoT_Message_Draw(PlayState* play);
void OoT_Message_Update(PlayState* play);
void OoT_Message_SetTables(void);
...
void OoT_Message_OpenText(PlayState* play, u16 textId);
void OoT_Message_Decode(PlayState* play);
void OoT_Message_DrawText(PlayState* play, Gfx** gfxP);
...
s32 OoT_Ship_CalcShouldDrawAndUpdate(PlayState* play, Actor* actor, Vec3f* projectedPos, f32 projectedW, bool* shouldDraw,
                                 bool* shouldUpdate);
