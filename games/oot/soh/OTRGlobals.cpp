...
extern "C" int OoT_RegisterModelResourceFactoriesHeadless(void) {
    ...
    loader->RegisterResourceFactory(std::make_shared<SOH::ResourceFactoryBinaryArrayV0>(), RESOURCE_FORMAT_BINARY,
                                    "Array", static_cast<uint32_t>(SOH::ResourceType::SOH_Array), 0);
    // Matrix: a display list can carry gsSPMatrix as a G_MTX_OTR reference to a
    // Matrix resource. object_mask_truth has none, but a caller re-pointing the
    // row at another object would otherwise get "reference did not load" from a
    // missing factory and read it as a cross-game failure.
    loader->RegisterResourceFactory(std::make_shared<Fast::ResourceFactoryBinaryMatrixV0>(), RESOURCE_FORMAT_BINARY,
                                    "Matrix", static_cast<uint32_t>(Fast::ResourceType::Matrix), 0);
    return 0;
}
