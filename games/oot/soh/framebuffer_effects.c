...
void FB_DrawFromFramebufferScaled(Gfx** gfxp, s32 fb, u8 alpha, float scaleX, float scaleY) {
    ...
    float x0 = OoT_gScreenWidth * 0.5f * scaleX;
    float y0 = OoT_gScreenHeight * 0.5f * scaleY;

    gDPImageRectangle(gfx++, OTRGetRectDimensionFromLeftEdge(x0) << 2, (int)(y0) << 2, 0, 0,
                      OTRGetRectDimensionFromRightEdge((float)(OoT_gScreenWidth - x0)) << 2,
                      (int)((float)(OoT_gScreenHeight - y0)) << 2, OTRGetGameRenderWidth(), OTRGetGameRenderHeight(),
