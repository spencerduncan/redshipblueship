...
void EnHeishi2_Init(Actor* thisx, PlayState* play) {
    ...
    if ((this->type == 6) || (this->type == 9)) {
        ...
        OoT_Actor_ChangeCategory(play, &play->actorCtx, &this->actor, 6);
        if (this->type == 6) {
            ...
            OoT_Player_SetCsActionWithHaltedActors(play, 0, 8);
        ...
        OoT_SkelAnime_Init(play, &this->skelAnime, &gEnHeishiSkel, &gEnHeishiIdleAnim, this->jointTable, this->morphTable,
                       17);
        collider = &this->collider;
        OoT_Collider_InitCylinder(play, collider);
        OoT_Collider_SetCylinder(play, collider, &this->actor, &OoT_sCylinderInit);
        ...
        switch (this->type) {
            ...
            case 6:
                ...
                OoT_Collider_DestroyCylinder(play, collider);
...
void func_80A53278(EnHeishi2* this, PlayState* play) {
    ...
    if (GameInteractor_Should(VB_CAN_BRIBE_HEISHI2, true, this)) {
        if (OoT_Text_GetFaceReaction(play, 5) != 0) {
            this->actor.textId = OoT_Text_GetFaceReaction(play, 5);
            this->unk_30B = 1;
...
void func_80A53538(EnHeishi2* this, PlayState* play) {
    OoT_SkelAnime_Update(&this->skelAnime);
    if (this->unk_300 == OoT_Message_GetState(&play->msgCtx) && OoT_Message_ShouldAdvance(play)) {
...
void func_80A535BC(EnHeishi2* this, PlayState* play) {
    f32 frameCount = OoT_Animation_GetLastFrame(&gEnHeishiSlamSpearAnim);

    this->unk_2EC = frameCount;
    OoT_Animation_Change(&this->skelAnime, &gEnHeishiSlamSpearAnim, 1.0f, 0.0f, frameCount, ANIMMODE_ONCE, -10.0f);
...
void func_80A53850(EnHeishi2* this, PlayState* play) {
    ...
    if ((this->unk_2F2[0] == 0) || (gate->unk_168 == 0)) {
        if (GameInteractor_Should(VB_PLAY_GATE_OPENING_OR_CLOSING_CS, true, this, true)) {
            OoT_Play_ClearCamera(play, this->cameraId);
...
void func_80A5399C(EnHeishi2* this, PlayState* play) {
    ...
    if (OoT_Flags_GetInfTable(INFTABLE_SHOWED_ZELDAS_LETTER_TO_GATE_GUARD)) {
        if (!OoT_Flags_GetInfTable(INFTABLE_GATE_GUARD_PUT_ON_KEATON_MASK)) {
            if (OoT_Player_GetMask(play) == PLAYER_MASK_KEATON) {
                if (this->unk_309 == 0) {
...
void func_80A54038(EnHeishi2* this, PlayState* play) {
    ...
    if ((OoT_Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && OoT_Message_ShouldAdvance(play)) {
        ...
        OoT_Message_CloseTextbox(play);
        OoT_Player_SetCsActionWithHaltedActors(play, 0, 7);
        this->actionFunc = func_80A53908;
    }
}
...
void func_80A544AC(EnHeishi2* this, PlayState* play) {
    OoT_Math_SmoothStepToS(&this->actor.shape.rot.z, -6100, 5, this->unk_2E4, 0);
    OoT_Math_ApproachF(&this->unk_2E4, 3000.0f, 1.0f, 500.0f);
    this->actor.world.rot.z = this->actor.shape.rot.z;
    if (this->actor.shape.rot.z < -6000) {
        OoT_Message_StartTextbox(play, 0x708F, NULL);
...
void func_80A5455C(EnHeishi2* this, PlayState* play) {
    ...
    if ((OoT_Message_GetState(&play->msgCtx) == TEXT_STATE_EVENT) && OoT_Message_ShouldAdvance(play)) {
        ...
        pos.x = OoT_Rand_CenteredFloat(20.0f) + this->unk_274.x;
        pos.y = OoT_Rand_CenteredFloat(20.0f) + (this->unk_274.y - 40.0f);
        pos.z = OoT_Rand_CenteredFloat(20.0f) + (this->unk_274.z - 20.0f);
        rotY = OoT_Rand_CenteredFloat(7000.0f) + this->actor.yawTowardsPlayer;
        bomb = (EnBom*)OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_BOM, pos.x, pos.y, pos.z, 0, rotY, 0, 0, true);
        if (bomb != NULL) {
            bomb->actor.speedXZ = OoT_Rand_CenteredFloat(5.0f) + 10.0f;
            bomb->actor.velocity.y = OoT_Rand_CenteredFloat(5.0f) + 10.0f;
        }
...
void func_80A546DC(EnHeishi2* this, PlayState* play) {
    OoT_Math_SmoothStepToS(&this->actor.shape.rot.z, 200, 5, this->unk_2E4, 0);
    OoT_Math_ApproachF(&this->unk_2E4, 3000.0f, 1.0f, 500.0f);
    this->actor.world.rot.z = this->actor.shape.rot.z;
...
void EnHeishi2_Update(Actor* thisx, PlayState* play) {
    ...
    OoT_Actor_SetFocus(&this->actor, this->unk_2E0);
    if ((this->type == 2) || (this->type == 5)) {
    ...
    switch (this->type) {
        ...
        case 9:
            break;
        default:
            OoT_Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 10.0f, 30.0f, 0x1D);
...
void EnHeishi2_Draw(Actor* thisx, PlayState* play) {
    ...
    if ((this->type == 5) && (OoT_Flags_GetInfTable(INFTABLE_GATE_GUARD_PUT_ON_KEATON_MASK))) {
        linkObjBankIndex = Object_GetIndex(&play->objectCtx, OBJECT_LINK_CHILD);
        if (linkObjBankIndex >= 0) {
            OoT_Matrix_Put(&this->mtxf_330);
            OoT_Matrix_Translate(-570.0f, 0.0f, 0.0f, MTXMODE_APPLY);
