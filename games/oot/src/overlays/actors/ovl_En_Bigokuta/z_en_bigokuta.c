...
void OoT_EnBigokuta_Init(Actor* thisx, PlayState* play);
void OoT_EnBigokuta_Destroy(Actor* thisx, PlayState* play);
void OoT_EnBigokuta_Update(Actor* thisx, PlayState* play);
void OoT_EnBigokuta_Draw(Actor* thisx, PlayState* play);
...
static Color_RGBA8 OoT_sEffectEnvColor = { 100, 255, 255, 255 };
...
const ActorInit En_Bigokuta_InitVars = {
    ...
    OBJECT_BIGOKUTA,
    sizeof(EnBigokuta),
    (ActorFunc)OoT_EnBigokuta_Init,
    (ActorFunc)OoT_EnBigokuta_Destroy,
    (ActorFunc)OoT_EnBigokuta_Update,
    (ActorFunc)OoT_EnBigokuta_Draw,
    NULL,
};
...
static CollisionCheckInfoInit OoT_sColChkInfoInit[] = { 4, 130, 120, 200 };
...
// possibly color data
static s32 OoT_sUnused[] = { 0xFFFFFFFF, 0x969696FF };
...
void OoT_EnBigokuta_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_SkelAnime_InitFlex(play, &this->skelAnime, &object_bigokuta_Skel_006BC0, &object_bigokuta_Anim_0014B8,
                       this->jointTable, this->morphTable, 20);

    OoT_Collider_InitJntSph(play, &this->collider);
    OoT_Collider_SetJntSph(play, &this->collider, &this->actor, &OoT_sJntSphInit, &this->element);

    this->collider.elements->dim.worldSphere.radius = this->collider.elements->dim.modelSphere.radius;

    for (i = 0; i < ARRAY_COUNT(OoT_sCylinderInit); i++) {
        OoT_Collider_InitCylinder(play, &this->cylinder[i]);
        OoT_Collider_SetCylinder(play, &this->cylinder[i], &this->actor, &OoT_sCylinderInit[i]);
    }
...
void OoT_EnBigokuta_Destroy(Actor* thisx, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->cylinder); i++) {
        OoT_Collider_DestroyCylinder(play, &this->cylinder[i]);
    }
...
void func_809BCE3C(EnBigokuta* this) {
    ...
    this->actor.world.pos.x = OoT_Math_SinS(this->actor.world.rot.y) * 263.0f + this->actor.home.pos.x;
    this->actor.world.pos.z = OoT_Math_CosS(this->actor.world.rot.y) * 263.0f + this->actor.home.pos.z;
}
...
void func_809BCEBC(EnBigokuta* this, PlayState* play) {
    ...
    if (((play->gameplayFrames % 7) == 0) && (yDistFromHome <= 0.0f) && (yDistFromHome > -100.0f)) {
        OoT_EffectSsGRipple_Spawn(play, &pos, 800, 1300, 0);
    }
}
...
void func_809BCF68(EnBigokuta* this, PlayState* play) {
    ...
    if (play->gameplayFrames & 1) {
        rot = OoT_Rand_S16Offset(0x1200, 0xC00) + this->actor.shape.rot.y - this->unk_194 * 0xA00;
    } else {
        rot = this->actor.shape.rot.y - this->unk_194 * 0xA00 - OoT_Rand_S16Offset(0x1200, 0xC00);
    }
    if (this->actionFunc != func_809BE4A4) {
        if (this->actionFunc == func_809BE3E4 || (play->gameplayFrames & 2)) {
            effectPos.x = this->actor.world.pos.x - OoT_Math_SinS(rot) * 80.0f;
            effectPos.z = this->actor.world.pos.z - OoT_Math_CosS(rot) * 80.0f;
            effectPos.y = this->actor.home.pos.y + 1.0f;
            OoT_EffectSsGRipple_Spawn(play, &effectPos, 100, 500, 0);
        } else {
            effectPos.x = this->actor.world.pos.x - OoT_Math_SinS(rot) * 120.0f;
            effectPos.z = this->actor.world.pos.z - OoT_Math_CosS(rot) * 120.0f;
            effectPos.y = this->actor.home.pos.y + 5.0f;
        }
    } else {
        effectPos.x = this->actor.world.pos.x - OoT_Math_SinS(rot) * 50.0f;
        effectPos.z = this->actor.world.pos.z - OoT_Math_CosS(rot) * 50.0f;
        effectPos.y = this->actor.home.pos.y + 1.0f;
        OoT_EffectSsGRipple_Spawn(play, &effectPos, 100, 500, 0);
    }
    OoT_EffectSsGSplash_Spawn(play, &effectPos, NULL, NULL, 1, 800);
    if (this->actionFunc != func_809BE4A4) {
...
void func_809BD1C8(EnBigokuta* this, PlayState* play) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        OoT_EffectSsGSplash_Spawn(play, &effectPos, NULL, NULL, 1, 2000);
    }
...
void func_809BD2E4(EnBigokuta* this) {
    ...
    if (parent != NULL) {
        OoT_Math_ScaledStepToS(&parent->world.rot.y, this->unk_19A, 0x10);
    }
}

void func_809BD318(EnBigokuta* this) {
    OoT_Animation_PlayLoop(&this->skelAnime, &object_bigokuta_Anim_0014B8);
    this->unk_19A = 0;
    this->actor.home.pos.y = -1025.0f;
    this->unk_196 = 121;
...
void func_809BD3F8(EnBigokuta* this) {
    OoT_Animation_MorphToLoop(&this->skelAnime, &object_bigokuta_Anim_001CA4, -5.0f);
    this->unk_196 = 350;
    this->unk_198 = 80;
...
void func_809BD4A4(EnBigokuta* this) {
    OoT_Animation_MorphToLoop(&this->skelAnime, &object_bigokuta_Anim_0014B8, -5.0f);
    this->unk_195 = true;
...
void func_809BD524(EnBigokuta* this) {
    OoT_Animation_MorphToPlayOnce(&this->skelAnime, &object_bigokuta_Anim_000D1C, -5.0f);
    this->unk_196 = 80;
    this->unk_19A = 0;
    this->cylinder[0].base.atFlags |= AT_ON;
    ...
    OoT_Actor_SetColorFilter(&this->actor, 0, 255, 0, this->unk_196);
    this->actionFunc = func_809BE058;
}

void func_809BD5E0(EnBigokuta* this) {
    OoT_Animation_MorphToPlayOnce(&this->skelAnime, &object_bigokuta_Anim_000444, -5.0f);
    this->unk_196 = 24;
    this->unk_19A = 0;
    this->cylinder[0].base.atFlags &= ~AT_ON;
    OoT_Actor_SetColorFilter(&this->actor, 0x4000, 255, 0, 24);
    this->actionFunc = func_809BE180;
}
void func_809BD658(EnBigokuta* this) {

    OoT_Animation_MorphToPlayOnce(&this->skelAnime, &object_bigokuta_Anim_000A74, -5.0f);
...
void func_809BD768(EnBigokuta* this) {
    this->unk_194 = OoT_Rand_ZeroOne() < 0.5f ? -1 : 1;
    this->unk_19A = 0;
...
void func_809BD7F0(EnBigokuta* this, PlayState* play) {
    this->actor.world.rot.y = OoT_Actor_WorldYawTowardPoint(&GET_PLAYER(play)->actor, &this->actor.home.pos);
...
void func_809BD8DC(EnBigokuta* this, PlayState* play) {
    ...
    if (this->unk_196 >= 0) {
        OoT_Math_StepToF(&this->actor.world.pos.x, this->actor.home.pos.x + 263.0f, 263.0f / 21);
...
void func_809BDAE8(EnBigokuta* this, PlayState* play) {
    if (OoT_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.home.rot.y + 0x4000, 0x400)) {
        if (this->unk_196 != 0) {
            this->unk_196--;
        }
        if (this->unk_196 == 0) {
...
void func_809BDC08(EnBigokuta* this, PlayState* play) {
    ...
    phi_v1 = (OoT_Actor_WorldDistXZToPoint(&player->actor, &this->actor.home.pos) - 180.0f) * (8.0f / 15);
    OoT_Actor_WorldToActorCoords(&this->actor, &sp28, &player->actor.world.pos);
    if (fabsf(sp28.x) > 263.0f || ((sp28.z > 0.0f) && !OoT_Actor_IsFacingPlayer(&this->actor, 0x1B00) &&
                                   !OoT_Player_IsFacingActor(&this->actor, 0x2000, play))) {
        phi_v1 -= 0x80;
        if (this->unk_196 != 0) {
            this->unk_196--;
        }
    ...
    if ((this->actor.xzDistToPlayer < 250.0f) && !OoT_Actor_IsFacingPlayer(&this->actor, 0x6000)) {
        if (this->unk_198 != 0) {
            this->unk_198--;
        }
...
void func_809BDFC8(EnBigokuta* this, PlayState* play) {
    ...
    if ((this->unk_196 == 0) && OoT_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.world.rot.x, 0x800)) {
...
void func_809BE058(EnBigokuta* this, PlayState* play) {
    ...
    if ((this->collider.base.ocFlags1 & OC1_HIT) || (this->cylinder[0].base.ocFlags1 & OC1_HIT) ||
        ...
        player->actor.world.pos.x -= speedXZ * OoT_Math_SinS(this->actor.shape.rot.y);
        player->actor.world.pos.z -= speedXZ * OoT_Math_CosS(this->actor.shape.rot.y);
    }
    if (this->unk_196 == 0) {
        func_809BD6B8(this);
    }
...
void func_809BE26C(EnBigokuta* this, PlayState* play) {
    ...
    if (this->unk_196 != 0) {
        ...
        if (this->unk_198 == 6) {
            ...
            func_8002829C(play, &effectPos, &sEffectPosAccel, &sEffectPosAccel, &OoT_sEffectPrimColor, &OoT_sEffectEnvColor,
                          1200, 20);
            Audio_PlayActorSound2(&this->actor, NA_SE_EN_OCTAROCK_DEAD2);
        }
        if (this->unk_198 == 0 && OoT_Math_StepToF(&this->actor.scale.y, 0.0f, 0.001f)) {
            ...
            OoT_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 50, NA_SE_EN_OCTAROCK_BUBLE);
...
void func_809BE568(EnBigokuta* this) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->cylinder); i++) {
        this->cylinder[i].dim.pos.x =
            this->actor.world.pos.x + OoT_sCylinderInit[i].dim.pos.z * sin + OoT_sCylinderInit[i].dim.pos.x * cos;
        this->cylinder[i].dim.pos.z =
            this->actor.world.pos.z + OoT_sCylinderInit[i].dim.pos.z * cos - OoT_sCylinderInit[i].dim.pos.x * sin;
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if (this->actor.colChkInfo.damageEffect != 0 || this->actor.colChkInfo.damage != 0) {
            if (this->actor.colChkInfo.damageEffect == 1) {
                ...
                func_809BD47C(this);
            } else if (!OoT_Actor_IsFacingPlayer(&this->actor, 0x4000)) {
                if (OoT_Actor_ApplyDamage(&this->actor) == 0) { // Dead
...
void OoT_EnBigokuta_Update(Actor* thisx, PlayState* play2) {
    EnBigokuta* this = (EnBigokuta*)thisx;
    s32 i;
    ...
    func_809BD2E4(this);
    func_809BE568(this);
    OoT_Camera_ChangeSetting(play->cameraPtrs[MAIN_CAM], CAM_SET_BIG_OCTO);
    func_8005AD1C(play->cameraPtrs[MAIN_CAM], 4);

    if (this->cylinder[0].base.atFlags & AT_ON) {
        if (this->actionFunc != func_809BE058) {
            for (i = 0; i < ARRAY_COUNT(this->cylinder); i++) {
                OoT_CollisionCheck_SetAT(play, &play->colChkCtx, &this->cylinder[i].base);
            }
            this->actor.flags |= ACTOR_FLAG_SFX_FOR_PLAYER_BODY_HIT;
        } else {
            for (i = 0; i < ARRAY_COUNT(this->cylinder); i++) {
                OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->cylinder[i].base);
            }
        }
        for (i = 0; i < ARRAY_COUNT(this->cylinder); i++) {
            OoT_CollisionCheck_SetAC(play, &play->colChkCtx, &this->cylinder[i].base);
        }
    ...
    OoT_Actor_SetFocus(&this->actor, this->actor.scale.y * 25.0f * 100.0f);
    func_809BCEBC(this, play);
}

s32 OoT_EnBigokuta_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    EnBigokuta* this = (EnBigokuta*)thisx;
    u8 intensity;
    f32 temp_f0;
    s32 temp_hi;
    ...
    if (limbIndex == 15) {
        ...
        if (this->unk_198 < 5) {
            OoT_Matrix_Scale((this->unk_198 * 0.2f * 0.25f) + 1.0f, 1.0f, 1.0f, MTXMODE_APPLY);
        } else if (this->unk_198 < 8) {
            temp_f0 = (this->unk_198 - 5) * (1.0f / 12);
            OoT_Matrix_Scale(1.25f - temp_f0, 1.0f + temp_f0, 1.0f + temp_f0, MTXMODE_APPLY);
        } else {
            temp_f0 = ((this->unk_198 - 8) * 0.125f);
            OoT_Matrix_Scale(1.0f, 1.25f - temp_f0, 1.25f - temp_f0, MTXMODE_APPLY);
        }
    }
    return false;
}
    ...
    if ((this->actionFunc != func_809BE26C) || (this->unk_196 != 0) || (this->unk_198 != 0)) {
        ...
        if (this->unk_196 & 1) {
            if ((this->actionFunc == func_809BE180 && this->unk_196 >= 8) ||
                ...
                f32 rotX = OoT_Rand_ZeroOne() * (M_PI * 2.0f);
                f32 rotY = OoT_Rand_ZeroOne() * (M_PI * 2.0f);

                Matrix_RotateY(rotY, MTXMODE_APPLY);
                Matrix_RotateX(rotX, MTXMODE_APPLY);
                OoT_Matrix_Scale(0.78999996f, 1.3f, 0.78999996f, MTXMODE_APPLY);
                Matrix_RotateX(-rotX, MTXMODE_APPLY);
                Matrix_RotateY(-rotY, MTXMODE_APPLY);
            }
        }
        SkelAnime_DrawSkeletonOpa(play, &this->skelAnime, OoT_EnBigokuta_OverrideLimbDraw, NULL, this);
    } else {
