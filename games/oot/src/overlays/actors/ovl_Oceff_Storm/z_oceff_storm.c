...
void OoT_OceffStorm_Init(Actor* thisx, PlayState* play);
void OoT_OceffStorm_Destroy(Actor* thisx, PlayState* play);
void OoT_OceffStorm_Update(Actor* thisx, PlayState* play);
void OoT_OceffStorm_Draw(Actor* thisx, PlayState* play);

void OoT_OceffStorm_Draw2(Actor* thisx, PlayState* play);

void OoT_OceffStorm_DefaultAction(OceffStorm* this, PlayState* play);
...
const ActorInit Oceff_Storm_InitVars = {
    ...
    sizeof(OceffStorm),
    (ActorFunc)OoT_OceffStorm_Init,
    (ActorFunc)OoT_OceffStorm_Destroy,
    (ActorFunc)OoT_OceffStorm_Update,
    (ActorFunc)OoT_OceffStorm_Draw,
    NULL,
};

void OoT_OceffStorm_SetupAction(OceffStorm* this, OceffStormActionFunc actionFunc) {
    this->actionFunc = actionFunc;
}

void OoT_OceffStorm_Init(Actor* thisx, PlayState* play) {
    OceffStorm* this = (OceffStorm*)thisx;
    OoT_OceffStorm_SetupAction(this, OoT_OceffStorm_DefaultAction);
    this->posYOffAdd = 0;
    this->counter = 0;
    ...
    if (this->actor.params == 1) {
        OoT_OceffStorm_SetupAction(this, OceffStorm_UnkAction);
        this->actor.draw = OoT_OceffStorm_Draw2;
    } else {
        OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_OKARINA_EFFECT, this->actor.world.pos.x,
...
void OceffStorm_UnkAction(OceffStorm* this, PlayState* play) {
    ...
    //! @bug OoT_Actor_Kill is never called so the actor will stay alive forever
}
...
void OoT_OceffStorm_Update(Actor* thisx, PlayState* play) {
    ...
    this->actor.shape.rot.y = OoT_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play));
...
void OoT_OceffStorm_Draw2(Actor* thisx, PlayState* play) {
    ...
    gSPDisplayList(POLY_XLU_DISP++, OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, scroll * 8, scroll * 4, 64, 64, 1,
...
void OoT_OceffStorm_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPDisplayList(POLY_XLU_DISP++, OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, scroll * 4, (0 - scroll) * 8, 32, 32, 1,
                                                       scroll * 8, (0 - scroll) * 12, 32, 32, 4, -8, 8, -12));
    gSPDisplayList(POLY_XLU_DISP++, sCylinderModelDL);

    CLOSE_DISPS(play->state.gfxCtx);

    OoT_OceffStorm_Draw2(&this->actor, play);
}
