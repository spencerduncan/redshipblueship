...
void OoT_EnPoSisters_Init(Actor* thisx, PlayState* play);
void OoT_EnPoSisters_Destroy(Actor* thisx, PlayState* play);
void OoT_EnPoSisters_Update(Actor* thisx, PlayState* play);
void OoT_EnPoSisters_Draw(Actor* thisx, PlayState* play);
...
void OoT_func_80ADAFC0(EnPoSisters* this, PlayState* play);
...
const ActorInit En_Po_Sisters_InitVars = {
    ...
    OBJECT_PO_SISTERS,
    sizeof(EnPoSisters),
    (ActorFunc)OoT_EnPoSisters_Init,
    (ActorFunc)OoT_EnPoSisters_Destroy,
    (ActorFunc)OoT_EnPoSisters_Update,
    (ActorFunc)OoT_EnPoSisters_Draw,
    NULL,
};
...
static CollisionCheckInfoInit OoT_sColChkInfoInit = { 10, 25, 60, 40 };
...
void OoT_EnPoSisters_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_ActorShape_Init(&this->actor.shape, 0.0f, OoT_ActorShadow_DrawCircle, 50.0f);
    OoT_SkelAnime_Init(play, &this->skelAnime, &gPoeSistersSkel, &gPoeSistersSwayAnim, this->jointTable, this->morphTable,
                   12);
    this->unk_22E.r = 255;
    this->unk_22E.g = 255;
    this->unk_22E.b = 210;
    ...
    OoT_Lights_PointGlowSetInfo(&this->lightInfo, this->actor.home.pos.x, this->actor.home.pos.y, this->actor.home.pos.z, 0,
                            0, 0, 0);
...
void func_80AD9240(EnPoSisters* this, s32 arg1, Vec3f* arg2) {
    ...
    for (i = 0; i < this->unk_198; i++) {
        vec = &this->unk_234[i];
        vec->x = arg2->x + OoT_Math_SinS((s16)(this->actor.shape.rot.y + (this->unk_19A * 0x800) + i * 0x2000)) * temp_f20;
        vec->z = arg2->z + OoT_Math_CosS((s16)(this->actor.shape.rot.y + (this->unk_19A * 0x800) + i * 0x2000)) * temp_f20;
        vec->y = arg2->y + arg1;
    }
}

void func_80AD9368(EnPoSisters* this) {
    OoT_Animation_MorphToLoop(&this->skelAnime, &gPoeSistersSwayAnim, -3.0f);
    this->unk_19A = OoT_Rand_S16Offset(2, 3);
...
void func_80AD93C4(EnPoSisters* this) {
    if (this->actionFunc != func_80ADA6A0) {
        OoT_Animation_MorphToLoop(&this->skelAnime, &gPoeSistersFloatAnim, -3.0f);
    }
    this->unk_19A = OoT_Rand_S16Offset(0xF, 3);
    this->unk_199 |= 7;
...
void func_80AD944C(EnPoSisters* this) {
    ...
    OoT_Animation_MorphToLoop(&this->skelAnime, &gPoeSistersAttackAnim, -5.0f);
    this->actor.speedXZ = 0.0f;
    this->unk_19A = OoT_Animation_GetLastFrame(&gPoeSistersAttackAnim) * 3 + 3;
    this->unk_199 &= ~2;
...
void func_80AD95D8(EnPoSisters* this) {
    OoT_Animation_MorphToPlayOnce(&this->skelAnime, &gPoeSistersDamagedAnim, -3.0f);
    if (this->collider.base.ac != NULL) {
        this->actor.world.rot.y = (this->collider.info.acHitInfo->toucher.dmgFlags & 0x1F824)
                                      ? this->collider.base.ac->world.rot.y
                                      : OoT_Actor_WorldYawTowardActor(&this->actor, this->collider.base.ac) + 0x8000;
    }
    if (this->unk_194 != 0) {
        this->actor.speedXZ = 10.0f;
    }
    this->unk_199 &= ~0xB;
    OoT_Actor_SetColorFilter(&this->actor, 0x4000, 0xFF, 0, 0x10);
    this->actionFunc = func_80ADAAA4;
}

void func_80AD96A4(EnPoSisters* this) {
    OoT_Animation_MorphToLoop(&this->skelAnime, &gPoeSistersFleeAnim, -3.0f);
...
void func_80AD9718(EnPoSisters* this) {
    OoT_Animation_Change(&this->skelAnime, &gPoeSistersAppearDisappearAnim, 1.5f, 0.0f,
                     OoT_Animation_GetLastFrame(&gPoeSistersAppearDisappearAnim), ANIMMODE_ONCE, -3.0f);
    this->actor.speedXZ = 0.0f;
    this->unk_19C = 100;
...
void func_80AD97C8(EnPoSisters* this, PlayState* play) {
    ...
    if (this->unk_195 == 0 || this->actionFunc != func_80ADAAA4) {
        if ((player->meleeWeaponState == 0 || player->meleeWeaponAnimation >= 24) &&
            ...
            OoT_Math_StepToF(&this->unk_294, 110.0f, 3.0f);
        } else {
            OoT_Math_StepToF(&this->unk_294, 170.0f, 10.0f);
        }
        sp20 = this->unk_294;
    } else if (this->unk_195 != 0) {
        sp20 = this->actor.parent->xzDistToPlayer;
    }
    this->actor.world.pos.x = (OoT_Math_SinS(this->actor.shape.rot.y + 0x8000) * sp20) + player->actor.world.pos.x;
    this->actor.world.pos.z = (OoT_Math_CosS(this->actor.shape.rot.y + 0x8000) * sp20) + player->actor.world.pos.z;
}
...
void func_80AD98F4(EnPoSisters* this, PlayState* play) {
    ...
    if (this->unk_194 == 0) {
        ...
        this->unk_22E.a = 0;
        this->actor.draw = OoT_EnPoSisters_Draw;
    } else {
        this->actor.world.rot.y = this->actor.shape.rot.y;
    }
    this->unk_19A = 15;
...
void func_80AD99D4(EnPoSisters* this, PlayState* play) {
    ...
    this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
    this->unk_199 = 0;
    this->actionFunc = OoT_func_80ADAFC0;
...
void func_80AD9A54(EnPoSisters* this, PlayState* play) {
    ...
    OoT_Item_DropCollectibleRandom(play, &this->actor, &this->actor.world.pos, 0x80);
    this->actionFunc = func_80ADB17C;
}

// Meg spawning fakes
void func_80AD9AA8(EnPoSisters* this, PlayState* play) {
    Actor* actor1 = OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_PO_SISTERS, this->actor.world.pos.x,
                                this->actor.world.pos.y, this->actor.world.pos.z, 0, 0, 0, 0x400, true);
    Actor* actor2 = OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_PO_SISTERS, this->actor.world.pos.x,
                                this->actor.world.pos.y, this->actor.world.pos.z, 0, 0, 0, 0x800, true);
    Actor* actor3 = OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_PO_SISTERS, this->actor.world.pos.x,
    ...
    if (actor1 == NULL || actor2 == NULL || actor3 == NULL) {
        if (actor1 != NULL) {
            OoT_Actor_Kill(actor1);
        }
        if (actor2 != NULL) {
            OoT_Actor_Kill(actor2);
        }
        if (actor3 != NULL) {
            OoT_Actor_Kill(actor3);
        }
        ...
        OoT_Animation_PlayLoop(&this->skelAnime, &gPoeSistersMegCryAnim);
        this->unk_198 = 0;
        this->unk_199 = 160;
...
void func_80AD9C24(EnPoSisters* this, PlayState* play) {
    ...
    if (play != NULL) {
        ...
        OoT_EffectSsDeadDb_Spawn(play, &vec, &OoT_sZeroVector, &OoT_sZeroVector, 150, 0, 255, 255, 255, 155, 150, 150, 150, 1, 9,
                             0);
    }
    OoT_Lights_PointSetColorAndRadius(&this->lightInfo, 0, 0, 0, 0);
    this->actionFunc = func_80ADB338;
}

void func_80AD9D44(EnPoSisters* this) {
    if (this->unk_194 == 3) {
        OoT_Animation_PlayOnce(&this->skelAnime, &gPoeSistersAppearDisappearAnim);
        Audio_PlayActorSound2(&this->actor, NA_SE_EN_PO_APPEAR);
    } else {
        OoT_Animation_Change(&this->skelAnime, &gPoeSistersAppearDisappearAnim, 0.5f, 0.0f,
                         OoT_Animation_GetLastFrame(&gPoeSistersAppearDisappearAnim), ANIMMODE_ONCE_INTERP, 0.0f);
    }
    this->unk_22E.a = 0;
    this->unk_199 = 32;
...
void func_80AD9DF0(EnPoSisters* this, PlayState* play) {
    OoT_Animation_MorphToPlayOnce(&this->skelAnime, &gPoeSistersAppearDisappearAnim, -5.0f);
    this->unk_198 = 1;
    this->unk_199 &= ~0x80;
...
void func_80AD9E60(EnPoSisters* this) {
    OoT_Animation_MorphToLoop(&this->skelAnime, &gPoeSistersFloatAnim, -3.0f);
    this->unk_19A = OoT_Animation_GetLastFrame(&gPoeSistersFloatAnim) * 7 + 7;
...
void func_80AD9F90(EnPoSisters* this) {
    ...
    OoT_Animation_PlayLoop(&this->skelAnime, &gPoeSistersFloatAnim);
    this->unk_199 |= 0xA;
...
void func_80ADA1B8(EnPoSisters* this) {
    OoT_Animation_Change(&this->skelAnime, &gPoeSistersAppearDisappearAnim, 0.833f, 0.0f,
                     OoT_Animation_GetLastFrame(&gPoeSistersAppearDisappearAnim), ANIMMODE_ONCE_INTERP, 0.0f);
...
void func_80ADA25C(EnPoSisters* this) {
    OoT_Animation_PlayLoop(&this->skelAnime, &gPoeSistersSwayAnim);
    this->unk_198 = 8;
    this->unk_19A = 32;
...
void func_80ADA35C(EnPoSisters* this, PlayState* play) {
    ...
    OoT_Math_ApproachF(&this->actor.world.pos.y, targetY, 0.5f, 3.0f);
    if (!this->unk_196) {
        this->unk_196 = 32;
    }
    if (this->unk_196 != 0) {
    ...
    this->actor.world.pos.y += (2.0f + 0.5f * OoT_Rand_ZeroOne()) * OoT_Math_SinS(this->unk_196 * 0x800);
...
void func_80ADA4A8(EnPoSisters* this, PlayState* play) {
    ...
    if (OoT_Animation_OnFrame(&this->skelAnime, 0.0f) && this->unk_19A != 0) {
        this->unk_19A--;
    }
...
void func_80ADA530(EnPoSisters* this, PlayState* play) {
    OoT_SkelAnime_Update(&this->skelAnime);
    OoT_Math_StepToF(&this->actor.speedXZ, 1.0f, 0.2f);
    if (OoT_Animation_OnFrame(&this->skelAnime, 0.0f) && this->unk_19A != 0) {
        this->unk_19A--;
    }
    if (this->actor.xzDistToPlayer < 200.0f && fabsf(this->actor.yDistToPlayer + 5.0f) < 30.0f) {
        func_80AD943C(this);
    } else if (this->unk_19A == 0 && OoT_Math_StepToF(&this->actor.speedXZ, 0.0f, 0.2f) != 0) {
        func_80AD9368(this);
    }
    if (this->actor.bgCheckFlags & 8) {
        OoT_Math_ScaledStepToS(&this->actor.world.rot.y, OoT_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos),
                           0x71C);
    } else if (OoT_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) > 300.0f) {
        OoT_Math_ScaledStepToS(&this->actor.world.rot.y, OoT_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos),
                           0x71C);
    }
}
...
void func_80ADA6A0(EnPoSisters* this, PlayState* play) {
    ...
    OoT_Math_StepToF(&this->actor.speedXZ, 2.0f, 0.2f);
    if (temp_v0 > 0x3000) {
        OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer + 0x3000, 0x71C);
    } else if (temp_v0 < -0x3000) {
        OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer - 0x3000, 0x71C);
    } else {
        OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 0x71C);
    }
...
void func_80ADA9E8(EnPoSisters* this, PlayState* play) {
    ...
    if (OoT_Math_StepToF(&this->actor.speedXZ, 0.0f, 0.1f) != 0) {
        this->actor.world.rot.y = this->actor.shape.rot.y;
        if (this->unk_194 != 0) {
            func_80AD93C4(this);
        } else {
...
void func_80ADAAA4(EnPoSisters* this, PlayState* play) {
    if (OoT_SkelAnime_Update(&this->skelAnime) && !(this->actor.flags & ACTOR_FLAG_ATTACHED_TO_ARROW)) {
    ...
    if (this->unk_195 != 0) {
        OoT_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.parent->shape.rot.y,
...
void func_80ADAC70(EnPoSisters* this, PlayState* play) {
    OoT_SkelAnime_Update(&this->skelAnime);
    OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer + 0x8000, 1820);
    if (OoT_Animation_OnFrame(&this->skelAnime, 0.0f) && this->unk_19A != 0) {
        this->unk_19A--;
    }
    if (this->actor.bgCheckFlags & 8) {
...
void OoT_func_80ADAFC0(EnPoSisters* this, PlayState* play) {
    ...
    this->unk_234[0].x =
        (OoT_Math_SinS((this->actor.shape.rot.y + this->unk_19A * 0x3000) - 0x4000) * (3000.0f * this->actor.scale.x)) +
        this->actor.world.pos.x;
    this->unk_234[0].z =
        (OoT_Math_CosS((this->actor.shape.rot.y + this->unk_19A * 0x3000) - 0x4000) * (3000.0f * this->actor.scale.x)) +
    ...
    if (this->unk_19A < 8) {
        ...
        if (this->unk_19A >= 16) {
            if (OoT_Math_StepToF(&this->actor.scale.x, 0.0f, 0.001f) != 0) {
...
void func_80ADB17C(EnPoSisters* this, PlayState* play) {
    ...
    if (this->unk_19A == 64) {
        ...
        OoT_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 30, NA_SE_EV_FLAME_IGNITION);
        if (this->unk_194 == 0) {
            OoT_Flags_UnsetSwitch(play, 0x1B);
        }
...
void func_80ADB338(EnPoSisters* this, PlayState* play) {
    ...
    if (this->unk_195 == 0) {
        if (OoT_Actor_WorldDistXZToPoint(&player->actor, &this->actor.home.pos) < 600.0f) {
            if (this->unk_19C != 0) {
                this->unk_19C--;
            }
        } else {
        ...
        if (this->unk_19C == 0) {
            this->actor.shape.rot.y = (s32)(4.0f * OoT_Rand_ZeroOne()) * 0x4000 + this->actor.yawTowardsPlayer;
        ...
        if (realMeg->actionFunc == func_80ADB51C) {
            ...
            this->actor.world.pos.y = player->actor.world.pos.y + 5.0f;
            func_80AD98F4(this, play);
        } else if (realMeg->actionFunc == OoT_func_80ADAFC0) {
            OoT_Actor_Kill(&this->actor);
        }
    }
}
...
void func_80ADB51C(EnPoSisters* this, PlayState* play) {
    ...
    if (this->unk_19A == 0) {
        this->actor.world.rot.y = this->actor.shape.rot.y += 0x4000 * (s32)(OoT_Rand_ZeroOne() * 4.0f);
        if (this->unk_195 == 0) {
...
void func_80ADB770(EnPoSisters* this, PlayState* play) {
            ...
            if (this->unk_195 == 0) {
                if (ABS((s16)(16 - this->unk_196)) < 14) {
                    this->actor.shape.rot.y +=
                        (0x580 - (this->unk_19C * 0x180)) * fabsf(OoT_Math_SinS(this->unk_196 * 0x800));
                }
    ...
    if (OoT_Actor_WorldDistXZToPoint(&GET_PLAYER(play)->actor, &this->actor.home.pos) > 600.0f) {
        this->unk_199 &= ~0x40;
        func_80AD9C24(this, play);
...
void func_80ADB9F0(EnPoSisters* this, PlayState* play) {
    ...
    if (this->unk_194 != 3 && OoT_Animation_OnFrame(&this->skelAnime, 1.0f)) {
        Audio_PlayActorSound2(&this->actor, NA_SE_EN_PO_APPEAR);
    }
    OoT_Actor_SetFocus(&this->actor, 40.0f);
}
...
void func_80ADBB6C(EnPoSisters* this, PlayState* play) {
    OoT_SkelAnime_Update(&this->skelAnime);
    if (OoT_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) < 10.0f) {
        func_80ADA028(this);
    } else {
        OoT_Math_ScaledStepToS(&this->actor.world.rot.y, OoT_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos),
                           1820);
...
void func_80ADBBF4(EnPoSisters* this, PlayState* play) {
    OoT_SkelAnime_Update(&this->skelAnime);
    OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 1820);
...
void func_80ADBC88(EnPoSisters* this, PlayState* play) {
    if (D_80ADD784 != 0 || !OoT_Player_InCsMode(play)) {
        if (this->unk_19A != 0) {
            this->unk_19A--;
        }
        if (this->unk_19A == 30) {
...
void func_80ADBF58(EnPoSisters* this, PlayState* play) {
    OoT_SkelAnime_Update(&this->skelAnime);
    this->unk_19A--;
    OoT_Math_ScaledStepToS(&this->actor.shape.rot.y, this->actor.world.rot.y, 0x500);
    if (this->unk_19A == 0 && this->unk_194 == 0) {
        play->envCtx.unk_BF = 4;
    }
    if (this->unk_19A < 0) {
        OoT_Math_StepToF(&this->actor.speedXZ, 5.0f, 0.2f);
    }
    if (this->unk_19A == -70 && this->unk_194 == 1) {
        OoT_SoundSource_PlaySfxAtFixedWorldPos(play, &D_80ADD7BC, 40, NA_SE_EN_PO_LAUGH);
    }
    if (this->unk_19A < -120) {
        OoT_Actor_Kill(&this->actor);
    }
...
void OoT_func_80ADC034(EnPoSisters* this, PlayState* play) {
    ...
    if (this->collider.base.acFlags & AC_HIT) {
        ...
        if (this->unk_195 != 0) {
            ...
            if (OoT_Rand_ZeroOne() < 0.2f) {
                ...
                OoT_Item_DropCollectible(play, &sp24, ITEM00_ARROWS_SMALL);
            }
...
void OoT_EnPoSisters_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->unk_199 & 0x1F) {
        ...
        if (this->unk_199 & 0x10) {
            OoT_Actor_UpdateBgCheckInfo(play, &this->actor, 20.0f, 20.0f, 0.0f, 5);
        } else {
            Vec3f vec;
            s32 sp34;
        ...
        } else if (this->actionFunc != OoT_func_80ADAFC0) {
            temp = this->unk_198 - 1;
...
s32 OoT_EnPoSisters_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx,
                                 Gfx** gfxP) {
    EnPoSisters* this = (EnPoSisters*)thisx;
    Color_RGBA8* color;
    ...
    if (this->unk_22E.a == 0 || limbIndex == 8 || (this->actionFunc == OoT_func_80ADAFC0 && this->unk_19A >= 8)) {
        *dList = NULL;
    } else if (limbIndex == 9) {
...
void OoT_EnPoSisters_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfxP) {
    EnPoSisters* this = (EnPoSisters*)thisx;
    s32 i;
    s32 pad;

    if (this->actionFunc == OoT_func_80ADAFC0 && this->unk_19A >= 8 && limbIndex == 9) {
    ...
    if (limbIndex == 8 && this->actionFunc != func_80ADB2B8) {
        if (this->unk_199 & 0x20) {
            ...
            OoT_Matrix_MultVec3f(&D_80ADD7F8, &this->unk_234[0]);
        } else if (this->actionFunc == func_80ADBD8C) {
            OoT_Matrix_MultVec3f(&D_80ADD7F8, &this->actor.home.pos);
        }
        if (this->unk_198 > 0) {
            Color_RGBA8* color = &D_80ADD6F0[this->unk_194];
            f32 temp_f2 = OoT_Rand_ZeroOne() * 0.3f + 0.7f;

            if (this->actionFunc == func_80ADB17C || this->actionFunc == func_80ADBD38 ||
                this->actionFunc == func_80ADBEE8) {
                OoT_Lights_PointNoGlowSetInfo(&this->lightInfo, this->unk_234[0].x, this->unk_234[0].y + 15.0f,
                                          this->unk_234[0].z, color->r * temp_f2, color->g * temp_f2,
                                          color->b * temp_f2, 200);
            } else {
                OoT_Lights_PointGlowSetInfo(&this->lightInfo, this->unk_234[0].x, this->unk_234[0].y + 15.0f,
...
void OoT_EnPoSisters_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->unk_22E.a == 255 || this->unk_22E.a == 0) {
        ...
        POLY_OPA_DISP = SkelAnime_DrawSkeleton2(play, &this->skelAnime, OoT_EnPoSisters_OverrideLimbDraw,
                                                OoT_EnPoSisters_PostLimbDraw, &this->actor, POLY_OPA_DISP);
    } else {
        gDPSetEnvColor(POLY_XLU_DISP++, 255, 255, 255, this->unk_22E.a);
        gSPSegment(POLY_XLU_DISP++, 0x09, D_80116280);
        POLY_XLU_DISP = SkelAnime_DrawSkeleton2(play, &this->skelAnime, OoT_EnPoSisters_OverrideLimbDraw,
                                                OoT_EnPoSisters_PostLimbDraw, &this->actor, POLY_XLU_DISP);
    }
    if (!(this->unk_199 & 0x80)) {
        OoT_Matrix_Put(&this->unk_2F8);
        gSPMatrix(POLY_OPA_DISP++, MATRIX_NEWMTX(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
        gSPDisplayList(POLY_OPA_DISP++, gPoSistersTorchDL);
    }
    gSPSegment(POLY_XLU_DISP++, 0x08,
               OoT_Gfx_TwoTexScroll(play->state.gfxCtx, 0, 0, 0, 0x20, 0x40, 1, 0, (play->gameplayFrames * -20) % 512, 0x20,
                                0x80));
    ...
    for (i = 0; i < this->unk_198; i++) {
        ...
        OoT_Matrix_RotateZYX(0, (s16)(OoT_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) + 0x8000), 0, MTXMODE_APPLY);
        if (this->actionFunc == OoT_func_80ADAFC0) {
            phi_f20 = (this->unk_19A - i) * 0.025f + 0.5f;
            phi_f20 = CLAMP(phi_f20, 0.5f, 0.8f) * 0.007f;
        }
        OoT_Matrix_Scale(phi_f20, phi_f20, phi_f20, MTXMODE_APPLY);
