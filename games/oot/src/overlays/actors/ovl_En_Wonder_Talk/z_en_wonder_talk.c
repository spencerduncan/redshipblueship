...
void func_80B3943C(EnWonderTalk* this, PlayState* play) {
    ...
    this->unk_15A++;
    if (this->unk_150 == 4 && (OoT_Flags_GetEventChkInf(EVENTCHKINF_DESTROYED_ROYAL_FAMILY_TOMB))) {
        OoT_Actor_Kill(&this->actor);
        return;
    }
    if (this->switchFlag < 0 || !OoT_Flags_GetSwitch(play, this->switchFlag)) {
...
void func_80B395F0(EnWonderTalk* this, PlayState* play) {
    if (this->unk_156 == OoT_Message_GetState(&play->msgCtx) && OoT_Message_ShouldAdvance(play)) {
        if (this->switchFlag >= 0) {
            this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
            OoT_Flags_SetSwitch(play, this->switchFlag);
        }
        switch (this->unk_150) {
            ...
            case 3:
                OoT_Message_CloseTextbox(play);
                if (this->unk_164 == 0) {
                    OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_POH, this->actor.world.pos.x, this->actor.world.pos.y,
