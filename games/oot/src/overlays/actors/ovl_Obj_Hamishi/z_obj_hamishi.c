...
void OoT_ObjHamishi_Init(Actor* thisx, PlayState* play);
void OoT_ObjHamishi_Destroy(Actor* thisx, PlayState* play);
void OoT_ObjHamishi_Update(Actor* thisx, PlayState* play);
void OoT_ObjHamishi_Draw(Actor* thisx, PlayState* play);
...
const ActorInit Obj_Hamishi_InitVars = {
    ...
    sizeof(ObjHamishi),
    (ActorFunc)OoT_ObjHamishi_Init,
    (ActorFunc)OoT_ObjHamishi_Destroy,
    (ActorFunc)OoT_ObjHamishi_Update,
    (ActorFunc)OoT_ObjHamishi_Draw,
    NULL,
};
...
void ObjHamishi_Shake(ObjHamishi* this) {
    if (this->shakeFrames > 0) {
        ...
        OoT_Math_StepToF(&this->shakePosSize, 0.0f, 0.15f);
        OoT_Math_StepToF(&this->shakeRotSize, 0.0f, 40.0f);

        this->actor.world.pos.x = this->actor.home.pos.x + (OoT_Math_SinS(this->shakePosPhase * 4) * this->shakePosSize);
        this->actor.world.pos.z = this->actor.home.pos.z + (OoT_Math_CosS(this->shakePosPhase * 7) * this->shakePosSize);
        this->actor.shape.rot.x =
            this->actor.home.rot.x + (s16)(OoT_Math_SinS(this->shakeRotPhase * 4) * this->shakeRotSize);
        this->actor.shape.rot.z =
            this->actor.home.rot.z + (s16)(OoT_Math_CosS(this->shakeRotPhase * 7) * this->shakeRotSize);
    } else {
        ...
        OoT_Math_ScaledStepToS(&this->actor.shape.rot.x, this->actor.home.rot.x, 0xBB8);
        OoT_Math_ScaledStepToS(&this->actor.shape.rot.z, this->actor.home.rot.z, 0xBB8);
    }
}
...
void ObjHamishi_Break(ObjHamishi* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(sEffectScales); i++) {
        ...
        temp_f20 = OoT_Rand_ZeroOne() * 10.0f;
        ...
        temp_f20 = (OoT_Rand_ZeroOne() * 10.0f) + 2.0f;
        velocity.x = OoT_Math_SinS(phi_s0) * temp_f20;
        temp_f22 = OoT_Rand_ZeroOne();
        velocity.y = (OoT_Rand_ZeroOne() * i * 2.5f) + (temp_f22 * 15.0f);
        ...
        OoT_EffectSsKakera_Spawn(play, &pos, &velocity, &this->actor.world.pos, gravity, phi_v0, 30, 5, 0, sEffectScales[i],
...
void OoT_ObjHamishi_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.shape.rot.y == 0) {
        this->actor.shape.rot.y = this->actor.world.rot.y = this->actor.home.rot.y = OoT_Rand_ZeroFloat(65536.0f);
    }
...
void OoT_ObjHamishi_Destroy(Actor* thisx, PlayState* play2) {
...
void OoT_ObjHamishi_Update(Actor* thisx, PlayState* play) {
    ...
    if ((this->collider.base.acFlags & AC_HIT) && (this->collider.info.acHitInfo->toucher.dmgFlags & 0x40000040)) {
        ...
        if (this->actor.xzDistToPlayer < 600.0f) {
            OoT_CollisionCheck_SetAC(play, colChkCtx, &this->collider.base);
            OoT_CollisionCheck_SetOC(play, colChkCtx, &this->collider.base);
        }
    }
}
