...
void EnHeishi4_Init(Actor* thisx, PlayState* play) {
    ...
    if (this->type == HEISHI4_AT_MARKET_DYING) {
        this->height = 30.0f;
        OoT_ActorShape_Init(&thisx->shape, 0.0f, NULL, 30.0f);
        OoT_SkelAnime_Init(play, &this->skelAnime, &gEnHeishiSkel, &gEnHeishiDyingGuardAnim_00C444, this->jointTable,
                       this->morphTable, 17);
    } else {
        this->height = 60.0f;
        OoT_ActorShape_Init(&thisx->shape, 0.0f, OoT_ActorShadow_DrawCircle, 30.0f);
...
void func_80A56328(EnHeishi4* this, PlayState* play) {
    f32 frames = OoT_Animation_GetLastFrame(&gEnHeishiIdleAnim);

    OoT_Animation_Change(&this->skelAnime, &gEnHeishiIdleAnim, 1.0f, 0.0f, (s16)frames, ANIMMODE_LOOP, -10.0f);
...
void func_80A563BC(EnHeishi4* this, PlayState* play) {
    ...
    if (OoT_Text_GetFaceReaction(play, sFaceReactionSets[reactionOffset]) != 0) {
        this->actor.textId = OoT_Text_GetFaceReaction(play, sFaceReactionSets[reactionOffset]);
        this->unk_2B4 = 1;
...
void func_80A5673C(EnHeishi4* this, PlayState* play) {
    ...
    if (OoT_Flags_GetEventChkInf(EVENTCHKINF_ZELDA_FLED_HYRULE_CASTLE)) {
        if (!OoT_Flags_GetInfTable(INFTABLE_6C)) {
            f32 frames = OoT_Animation_GetLastFrame(&gEnHeishiDyingGuardAnim_00C444);
            OoT_Animation_Change(&this->skelAnime, &gEnHeishiDyingGuardAnim_00C444, 1.0f, 0.0f, (s16)frames, ANIMMODE_LOOP,
                             -10.0f);
...
void func_80A56900(EnHeishi4* this, PlayState* play) {
    f32 frames = OoT_Animation_GetLastFrame(&gEnHeishiDyingGuardTalkAnim);

    OoT_Animation_Change(&this->skelAnime, &gEnHeishiDyingGuardTalkAnim, 1.0f, 0.0f, (s16)frames, ANIMMODE_LOOP, -10.0f);
...
void func_80A56994(EnHeishi4* this, PlayState* play) {
    ...
    if ((this->unk_282 == OoT_Message_GetState(&play->msgCtx)) && OoT_Message_ShouldAdvance(play)) {
...
void func_80A56A50(EnHeishi4* this, PlayState* play) {
    f32 frames = OoT_Animation_GetLastFrame(&gEnHeishiDyingGuardDieAnim);

    this->unk_288 = frames;
    OoT_Animation_Change(&this->skelAnime, &gEnHeishiDyingGuardDieAnim, 1.0f, 0.0f, frames, ANIMMODE_ONCE, -10.0f);
...
void EnHeishi4_Update(Actor* thisx, PlayState* play) {
    ...
    OoT_Actor_SetFocus(thisx, this->height);
    if (this->type != HEISHI4_AT_MARKET_DYING) {
    ...
    OoT_Actor_UpdateBgCheckInfo(play, thisx, 10.0f, 10.0f, 30.0f, 0x1D);
    OoT_Collider_UpdateCylinder(&this->actor, &this->collider);
    OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->collider.base);
}

s32 OoT_EnHeishi_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    EnHeishi4* this = (EnHeishi4*)thisx;
...
void EnHeishi4_Draw(Actor* thisx, PlayState* play) {
    ...
    SkelAnime_DrawSkeletonOpa(play, &this->skelAnime, OoT_EnHeishi_OverrideLimbDraw, NULL, this);
}
