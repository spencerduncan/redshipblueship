...
void OoT_EnPoh_Init(Actor* thisx, PlayState* play);
void OoT_EnPoh_Destroy(Actor* thisx, PlayState* play);
void OoT_EnPoh_Update(Actor* thisx, PlayState* play);
...
static CollisionCheckInfoInit OoT_sColChkInfoInit = { 4, 25, 50, 40 };
...
void OoT_EnPoh_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_Collider_SetJntSph(play, &this->colliderSph, &this->actor, &OoT_sJntSphInit, &this->colliderSphItem);
    ...
    OoT_Collider_SetCylinder(play, &this->colliderCyl, &this->actor, &OoT_sCylinderInit);
    OoT_CollisionCheck_SetInfo(&this->actor.colChkInfo, &OoT_sDamageTable, &OoT_sColChkInfoInit);
    this->unk_194 = 0;
    this->unk_195 = 32;
    this->visibilityTimer = OoT_Rand_S16Offset(700, 300);
    this->lightNode = OoT_LightContext_InsertLight(play, &play->lightCtx, &this->lightInfo);
    OoT_Lights_PointGlowSetInfo(&this->lightInfo, this->actor.home.pos.x, this->actor.home.pos.y, this->actor.home.pos.z,
                            255, 255, 255, 0);
    ...
    if (this->actor.params == EN_POH_RUPEE) {
        ...
        if (D_80AE1A50 >= 3) {
            ...
            collectible = OoT_Item_DropCollectible(play, &this->actor.world.pos, 0x4000 | ITEM00_RUPEE_BLUE);
            if (collectible != NULL) {
                collectible->actor.speedXZ = 0.0f;
            }
        }
    } else if (this->actor.params == EN_POH_FLAT) {
        if (OoT_Flags_GetSwitch(play, 0x28) || OoT_Flags_GetSwitch(play, 0x9)) {
            OoT_Actor_Kill(&this->actor);
        } else {
            OoT_Flags_SetSwitch(play, 0x28);
        }
    } else if (this->actor.params == EN_POH_SHARP) {
        if (OoT_Flags_GetSwitch(play, 0x29) || OoT_Flags_GetSwitch(play, 0x9)) {
            OoT_Actor_Kill(&this->actor);
        } else {
            OoT_Flags_SetSwitch(play, 0x29);
        }
    }
...
void func_80ADE114(EnPoh* this) {
    OoT_Animation_PlayLoop(&this->skelAnime, this->info->idleAnim);
    this->unk_198 = OoT_Rand_S16Offset(2, 3);
    this->actionFunc = func_80ADEAC4;
    this->actor.speedXZ = 0.0f;
}

void EnPoh_SetupIdle(EnPoh* this) {
    OoT_Animation_PlayLoop(&this->skelAnime, this->info->idleAnim2);
    this->unk_198 = OoT_Rand_S16Offset(15, 3);
...
void EnPoh_SetupAttack(EnPoh* this) {
    if (this->infoIdx == EN_POH_INFO_NORMAL) {
        OoT_Animation_MorphToLoop(&this->skelAnime, &gPoeAttackAnim, -6.0f);
    } else {
        OoT_Animation_PlayLoop(&this->skelAnime, &gPoeComposerAttackAnim);
    }
    this->unk_198 = 12;
    this->actor.speedXZ = 0.0f;
...
void func_80ADE28C(EnPoh* this) {
    if (this->infoIdx == EN_POH_INFO_NORMAL) {
        OoT_Animation_MorphToPlayOnce(&this->skelAnime, &gPoeDamagedAnim, -6.0f);
    } else {
        OoT_Animation_PlayOnce(&this->skelAnime, &gPoeComposerDamagedAnim);
    }
    if (this->colliderCyl.info.acHitInfo->toucher.dmgFlags & 0x0001F824) {
        ...
        this->actor.world.rot.y = OoT_Actor_WorldYawTowardActor(&this->actor, this->colliderCyl.base.ac) + 0x8000;
    }
...
void func_80ADE368(EnPoh* this) {
    OoT_Animation_MorphToLoop(&this->skelAnime, this->info->fleeAnim, -5.0f);
    this->actor.speedXZ = 5.0f;
...
void EnPoh_SetupInitialAction(EnPoh* this) {
    ...
    if (this->infoIdx == EN_POH_INFO_NORMAL) {
        OoT_Animation_PlayOnceSetSpeed(&this->skelAnime, &gPoeAppearAnim, 0.0f);
        this->actionFunc = func_80ADEF38;
    } else {
        OoT_Animation_PlayOnceSetSpeed(&this->skelAnime, &gPoeComposerAppearAnim, 1.0f);
...
void func_80ADE4C8(EnPoh* this) {
    OoT_Animation_PlayOnce(&this->skelAnime, this->info->idleAnim2);
...
void EnPoh_SetupDeath(EnPoh* this, PlayState* play) {
    ...
    OoT_Actor_ChangeCategory(play, &play->actorCtx, &this->actor, 8);
    this->unk_198 = 60;
...
void EnPoh_Talk(EnPoh* this, PlayState* play) {
    this->actor.home.pos.y = this->actor.world.pos.y;
    OoT_Actor_SetFocus(&this->actor, -10.0f);
    ...
    if (this->actor.params == EN_POH_FLAT || this->actor.params == EN_POH_SHARP) {
        ...
        } else if (!OoT_Flags_GetSwitch(play, 0xA) && !OoT_Flags_GetSwitch(play, 0xB)) {
            this->actor.textId = 0x500F;
        } else if ((this->actor.params == EN_POH_FLAT && OoT_Flags_GetSwitch(play, 0xA)) ||
                   (this->actor.params == EN_POH_SHARP && OoT_Flags_GetSwitch(play, 0xB))) {
...
void EnPoh_MoveTowardsPlayerHeight(EnPoh* this, PlayState* play) {
    ...
    OoT_Math_StepToF(&this->actor.world.pos.y, player->actor.world.pos.y, 1.0f);
    this->actor.world.pos.y += 2.5f * OoT_Math_SinS(this->unk_195 * 0x800);
    if (this->unk_195 != 0) {
        this->unk_195 -= 1;
    }
    if (this->unk_195 == 0) {
...
void func_80ADEA5C(EnPoh* this) {
    if (OoT_Actor_WorldDistXZToPoint(&this->actor, &this->actor.home.pos) > 400.0f) {
        this->unk_19C = OoT_Actor_WorldYawTowardPoint(&this->actor, &this->actor.home.pos);
    }
    OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->unk_19C, 0x71C);
}

void func_80ADEAC4(EnPoh* this, PlayState* play) {
    OoT_SkelAnime_Update(&this->skelAnime);
    if (OoT_Animation_OnFrame(&this->skelAnime, 0.0f) && this->unk_198 != 0) {
        this->unk_198--;
    }
...
void func_80ADEC9C(EnPoh* this, PlayState* play) {
    ...
    if (this->actor.xzDistToPlayer > 280.0f) {
        EnPoh_SetupIdle(this);
               ...
               !OoT_Player_IsFacingActor(&this->actor, 0x2AAA, play)) {
...
void EnPoh_Attack(EnPoh* this, PlayState* play) {
    ...
    EnPoh_MoveTowardsPlayerHeight(this, play);
    if (this->unk_198 >= 10) {
        OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 0xE38);
...
void func_80ADF15C(EnPoh* this, PlayState* play) {
    ...
    if (this->unk_198 < 8) {
        if (this->unk_198 < 5) {
            vec.y = OoT_Math_SinS((this->unk_198 * 0x1000) - 0x4000) * 23.0f + (this->actor.world.pos.y + 40.0f);
            multiplier = OoT_Math_CosS((this->unk_198 * 0x1000) - 0x4000) * 23.0f;
            vec.x =
                OoT_Math_SinS(OoT_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) + 0x4800) * multiplier + this->actor.world.pos.x;
            vec.z =
                OoT_Math_CosS(OoT_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) + 0x4800) * multiplier + this->actor.world.pos.z;
        } else {
            vec.y = (this->actor.world.pos.y + 40.0f) + (15.0f * (this->unk_198 - 5));
            vec.x = OoT_Math_SinS(OoT_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) + 0x4800) * 23.0f + this->actor.world.pos.x;
            vec.z = OoT_Math_CosS(OoT_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) + 0x4800) * 23.0f + this->actor.world.pos.z;
        }
        OoT_EffectSsDeadDb_Spawn(play, &vec, &D_80AE1B60, &D_80AE1B6C, this->unk_198 * 10 + 80, 0, 255, 255, 255, 255, 0, 0,
                             255, 1, 9, 1);
...
void func_80ADF5E0(EnPoh* this, PlayState* play) {
    OoT_SkelAnime_Update(&this->skelAnime);
    if (OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->unk_19C, 1820) != 0) {
        EnPoh_SetupIdle(this);
    }
    if (this->actor.xzDistToPlayer < 200.0f) {
        func_80ADE1BC(this);
...
void EnPoh_Disappear(EnPoh* this, PlayState* play) {
    ...
    if (this->unk_194 == 0) {
        this->visibilityTimer = OoT_Rand_S16Offset(100, 50);
        EnPoh_SetupIdle(this);
    }
}
...
void func_80ADF894(EnPoh* this, PlayState* play) {
    ...
    OoT_SkelAnime_Update(&this->skelAnime);
    multiplier = OoT_Math_SinS(this->unk_195 * 0x800) * 3.0f;
    this->actor.world.pos.x -= multiplier * OoT_Math_CosS(this->actor.shape.rot.y);
    this->actor.world.pos.z += multiplier * OoT_Math_SinS(this->actor.shape.rot.y);
    OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer + 0x8000, 0x71C);
    EnPoh_MoveTowardsPlayerHeight(this, play);
...
void EnPoh_Death(EnPoh* this, PlayState* play) {
    ...
    if (this->actor.bgCheckFlags & 1) {
        ...
        OoT_EffectSsHahen_SpawnBurst(play, &this->actor.world.pos, 6.0f, 0, 1, 1, 15, objId, 10,
    ...
    OoT_Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 10.0f, 10.0f, 4);
}
...
void func_80ADFE80(EnPoh* this, PlayState* play) {
    ...
    if (this->colliderCyl.base.ocFlags1 & OC1_HIT) {
        ...
        this->actor.flags &= ~ACTOR_FLAG_TALK_OFFER_AUTO_ACCEPTED;
        OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->colliderCyl.base);
    }
    this->actor.world.pos.y = OoT_Math_SinS(this->unk_195 * 0x800) * 5.0f + this->actor.home.pos.y;
    if (this->unk_195 != 0) {
        this->unk_195 -= 1;
    }
    if (this->unk_195 == 0) {
...
void EnPoh_TalkComposer(EnPoh* this, PlayState* play) {
    ...
    if (OoT_Message_GetState(&play->msgCtx) == TEXT_STATE_CHOICE) {
        if (OoT_Message_ShouldAdvance(play)) {
            if (play->msgCtx.choiceIndex == 0) {
                if (!OoT_Flags_GetSwitch(play, 0xB) && !OoT_Flags_GetSwitch(play, 0xA)) {
...
void OoT_EnPoh_Update(Actor* thisx, PlayState* play) {
    ...
    if (OoT_Object_IsLoaded(&play->objectCtx, this->objectIdx)) {
        ...
        if (this->infoIdx == EN_POH_INFO_NORMAL) {
            OoT_SkelAnime_Init(play, &this->skelAnime, &gPoeSkel, &gPoeFloatAnim, this->jointTable, this->morphTable, 21);
            this->actor.draw = EnPoh_DrawRegular;
        } else {
            OoT_SkelAnime_InitFlex(play, &this->skelAnime, &gPoeComposerSkel, &gPoeComposerFloatAnim, this->jointTable,
                               this->morphTable, 12);
...
void EnPoh_UpdateLiving(Actor* thisx, PlayState* play) {
    ...
    OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->colliderCyl.base);
    OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->colliderSph.base);
    OoT_Actor_SetFocus(&this->actor, 42.0f);
...
s32 OoT_EnPoh_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx,
                           Gfx** gfxP) {
    EnPoh* this = (EnPoh*)thisx;
...
void OoT_EnPoh_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx, Gfx** gfxP) {
    EnPoh* this = (EnPoh*)thisx;

    OoT_Collider_UpdateSpheres(limbIndex, &this->colliderSph);
    ...
    if (limbIndex == this->info->unk_6) {
        ...
        OoT_Lights_PointGlowSetInfo(&this->lightInfo, this->colliderSph.elements[0].dim.worldSphere.center.x,
...
void EnPoh_DrawRegular(Actor* thisx, PlayState* play) {
    ...
    if (this->lightColor.a == 255 || this->lightColor.a == 0) {
        ...
        POLY_OPA_DISP = SkelAnime_DrawSkeleton2(play, &this->skelAnime, OoT_EnPoh_OverrideLimbDraw, OoT_EnPoh_PostLimbDraw,
        ...
        POLY_XLU_DISP = SkelAnime_DrawSkeleton2(play, &this->skelAnime, OoT_EnPoh_OverrideLimbDraw, OoT_EnPoh_PostLimbDraw,
                                                &this->actor, POLY_XLU_DISP);
    }
    gDPPipeSync(POLY_OPA_DISP++);
    gDPSetEnvColor(POLY_OPA_DISP++, this->envColor.r, this->envColor.g, this->envColor.b, 255);
    OoT_Matrix_Put(&this->unk_368);
...
void EnPoh_DrawComposer(Actor* thisx, PlayState* play) {
    ...
    if (this->lightColor.a == 255 || this->lightColor.a == 0) {
        ...
        gSPSegment(POLY_OPA_DISP++, 0x08,
                   OoT_Gfx_EnvColor(play->state.gfxCtx, this->lightColor.r, this->lightColor.g, this->lightColor.b,
                                this->lightColor.a));
        gSPSegment(POLY_OPA_DISP++, 0x0A,
                   OoT_Gfx_EnvColor(play->state.gfxCtx, sp90->r, sp90->g, sp90->b, this->lightColor.a));
        gSPSegment(POLY_OPA_DISP++, 0x0B,
                   OoT_Gfx_EnvColor(play->state.gfxCtx, phi_t0->r, phi_t0->g, phi_t0->b, this->lightColor.a));
        gSPSegment(POLY_OPA_DISP++, 0x0C, D_80116280 + 2);
        POLY_OPA_DISP =
            OoT_SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                               OoT_EnPoh_OverrideLimbDraw, OoT_EnPoh_PostLimbDraw, &this->actor, POLY_OPA_DISP);
    } else {
        Gfx_SetupDL_25Opa(play->state.gfxCtx);
        Gfx_SetupDL_25Xlu(play->state.gfxCtx);
        ...
        POLY_XLU_DISP =
            OoT_SkelAnime_DrawFlex(play, this->skelAnime.skeleton, this->skelAnime.jointTable, this->skelAnime.dListCount,
                               OoT_EnPoh_OverrideLimbDraw, OoT_EnPoh_PostLimbDraw, &this->actor, POLY_XLU_DISP);
    }
    gDPPipeSync(POLY_OPA_DISP++);
    gDPSetEnvColor(POLY_OPA_DISP++, this->envColor.r, this->envColor.g, this->envColor.b, 255);
    OoT_Matrix_Put(&this->unk_368);
...
void EnPoh_DrawSoul(Actor* thisx, PlayState* play) {
    ...
    if (this->actionFunc == EnPoh_Death) {
        Gfx_SetupDL_25Opa(play->state.gfxCtx);
        gDPSetEnvColor(POLY_OPA_DISP++, this->envColor.r, this->envColor.g, this->envColor.b, 255);
        OoT_Lights_PointGlowSetInfo(&this->lightInfo, this->actor.world.pos.x, this->actor.world.pos.y,
        ...
        Matrix_RotateY((s16)(OoT_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) + 0x8000) * 9.58738e-05f, MTXMODE_APPLY);
