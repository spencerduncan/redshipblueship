...
OoT_Flags_GetEventChkInf(EVENTCHKINF_BOMBED_DODONGOS_CAVERN_ENTRANCE) - DC entrance boulder blown up as child

InfTable

OoT_Flags_GetInfTable(INFTABLE_B4)
OoT_Flags_GetInfTable(INFTABLE_E0) - Talked to DMT Goron at DC entrance (Before DC is opened as child)
OoT_Flags_GetInfTable(INFTABLE_E3) - Talked to GC Goron in bottom level stairwell
OoT_Flags_GetInfTable(INFTABLE_E6) - Talked to GC Goron at LW entrance (Before LW shortcut is opened)
OoT_Flags_GetInfTable(INFTABLE_EB) - Talked to DMT Goron at Bomb Flower with goron bracelet
OoT_Flags_GetInfTable(INFTABLE_F0) - Talked to Goron at GC Entrance (Before goron ruby is obtained)
OoT_Flags_GetInfTable(INFTABLE_F4) - Talked to Goron at GC Island (Before goron ruby is obtained)
OoT_Flags_GetInfTable(INFTABLE_F8) - (not on cloud modding) Talked to GC Goron outside Darunias door (after opening door,
before getting goron bracelet) OoT_Flags_GetInfTable(INFTABLE_GORON_CITY_DOORS_UNLOCKED) - Obtained Fire Tunic from Goron
Link OoT_Flags_GetInfTable(INFTABLE_10A) - (not on cloud modding) OoT_Flags_GetInfTable(INFTABLE_10B) - Spoke to Goron Link
About Volvagia OoT_Flags_GetInfTable(INFTABLE_STOPPED_GORON_LINKS_ROLLING) - Stopped Goron Link's Rolling
OoT_Flags_GetInfTable(INFTABLE_10D) - EnGo Exclusive
OoT_Flags_GetInfTable(INFTABLE_SPOKE_TO_GORON_LINK) - Spoke to Goron Link
OoT_Flags_GetInfTable(INFTABLE_10F) - (not on cloud modding)

OoT_Flags_GetInfTable(INFTABLE_11E) - Bomb bag upgrade obtained from rolling Goron
...
static Vec3f OoT_sAccel = { 0.0f, 0.3f, 0.0f };
...
void EnGo2_UpdateDust(EnGo2* this) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->dustEffects); i++, dustEffect++) {
        if (dustEffect->type) {
            ...
            dustEffect->accel.x = (OoT_Rand_ZeroOne() * 0.4f) - 0.2f;
            randomNumber = OoT_Rand_ZeroOne() * 0.4f;
...
void EnGo2_DrawDust(EnGo2* this, PlayState* play) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->dustEffects); i++, dustEffect++) {
        if (dustEffect->type) {
            ...
            OoT_Matrix_Translate(dustEffect->pos.x, dustEffect->pos.y, dustEffect->pos.z, MTXMODE_NEW);
            OoT_Matrix_ReplaceRotation(&play->billboardMtxF);
            OoT_Matrix_Scale(dustEffect->scale, dustEffect->scale, 1.0f, MTXMODE_APPLY);
...
s32 EnGo2_SpawnDust(EnGo2* this, u8 initialTimer, f32 scale, f32 scaleStep, s32 numDustEffects, f32 radius,
                    f32 yAccel) {
    Vec3f pos = sPos;
    Vec3f velocity = OoT_sVelocity;
    Vec3f accel = OoT_sAccel;
    s32 i;
    s16 angle;
    ...
    i = numDustEffects;
    while (i >= 0) {
        accel.y += OoT_Rand_ZeroOne() * yAccel;
...
void EnGo2_GetItem(EnGo2* this, PlayState* play, s32 getItemId) {
    ...
    if (GameInteractor_Should(VB_GIVE_ITEM_FROM_GORON, true, this)) {
        OoT_Actor_OfferGetItem(&this->actor, play, getItemId, this->actor.xzDistToPlayer + 1.0f,
                           fabsf(this->actor.yDistToPlayer) + 1.0f);
    }
}

s32 EnGo2_GetDialogState(EnGo2* this, PlayState* play) {
    s16 dialogState = OoT_Message_GetState(&play->msgCtx);
...
u16 EnGo2_GetTextIdGoronCityRollingBig(PlayState* play, EnGo2* this) {
    if (OoT_Flags_GetInfTable(INFTABLE_11E)) {
        return 0x3013;
        ...
        case TEXT_STATE_EVENT:
            if (OoT_Message_ShouldAdvance(play)) {
                if (this->actor.textId == 0x3012) {
                    ...
                    OoT_Flags_SetInfTable(INFTABLE_11E);
                    return NPC_TALK_STATE_ACTION;
                } else {
                    return NPC_TALK_STATE_ACTION;
                }
...
s16 EnGo2_UpdateTalkStateGoronDmtBombFlower(PlayState* play, EnGo2* this) {
    switch (OoT_Message_GetState(&play->msgCtx)) {
        case TEXT_STATE_CLOSING:
            if ((this->actor.textId == 0x300B) && (OoT_Flags_GetInfTable(INFTABLE_EB)) == 0) {
...
u16 EnGo2_GetTextIdGoronDmtRollingSmall(PlayState* play, EnGo2* this) {
    if (CHECK_QUEST_ITEM(QUEST_GORON_RUBY)) {
        ...
        return (OoT_Flags_GetEventChkInf(EVENTCHKINF_BOMBED_DODONGOS_CAVERN_ENTRANCE)) ? 0x3026 : 0x3009;
    }
}
...
u16 EnGo2_GetTextIdGoronDmtDcEntrance(PlayState* play, EnGo2* this) {
    if (GameInteractor_Should(VB_GORONS_CONSIDER_FIRE_TEMPLE_FINISHED, CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) &&
        ...
        return OoT_Flags_GetEventChkInf(EVENTCHKINF_BOMBED_DODONGOS_CAVERN_ENTRANCE) ? 0x3021
               : OoT_Flags_GetInfTable(INFTABLE_E0)                                  ? 0x302A
                                                                                 : 0x3008;
    }
}
...
u16 EnGo2_GetTextIdGoronCityEntrance(PlayState* play, EnGo2* this) {
    if (GameInteractor_Should(VB_GORONS_CONSIDER_FIRE_TEMPLE_FINISHED, CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) &&
        ...
        return OoT_Flags_GetInfTable(INFTABLE_F0) ? 0x3015 : 0x3014;
    }
}
...
u16 EnGo2_GetTextIdGoronCityIsland(PlayState* play, EnGo2* this) {
    if (GameInteractor_Should(VB_GORONS_CONSIDER_FIRE_TEMPLE_FINISHED, CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) &&
        ...
        return OoT_Flags_GetInfTable(INFTABLE_F4) ? 0x3017 : 0x3016;
    }
}
...
u16 EnGo2_GetTextIdGoronCityLowestFloor(PlayState* play, EnGo2* this) {
    if (GameInteractor_Should(VB_GORONS_CONSIDER_FIRE_TEMPLE_FINISHED, CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) &&
        ...
        return CUR_UPG_VALUE(UPG_STRENGTH) != 0 ? 0x302C
               : !OoT_Flags_GetSwitch(play, 0x1B)   ? 0x3017
               : OoT_Flags_GetInfTable(INFTABLE_F8) ? 0x3019
                                                : 0x3018;
    }
}
...
u16 EnGo2_GetTextIdGoronCityLink(PlayState* play, EnGo2* this) {
    ...
    if (GameInteractor_Should(VB_GORONS_CONSIDER_FIRE_TEMPLE_FINISHED, CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE))) {
        return OoT_Flags_GetInfTable(INFTABLE_10F) ? 0x3042 : 0x3041;
    } else if (GameInteractor_Should(VB_GORONS_CONSIDER_TUNIC_COLLECTED,
                                     CHECK_OWNED_EQUIP(EQUIP_TYPE_TUNIC, EQUIP_INV_TUNIC_GORON))) {
        return OoT_Flags_GetInfTable(INFTABLE_SPOKE_TO_GORON_LINK) ? 0x3038 : 0x3037;
    } else if (OoT_Flags_GetInfTable(INFTABLE_STOPPED_GORON_LINKS_ROLLING)) {
        this->unk_20C = 0;
        this->dialogState = TEXT_STATE_NONE;
        return OoT_Flags_GetInfTable(INFTABLE_10A) ? 0x3033 : 0x3032;
    } else {
        return 0x3030;
    }
}
    ...
    switch (EnGo2_GetDialogState(this, play)) {
        case TEXT_STATE_CLOSING:
            switch (this->actor.textId) {
                case 0x3036:
                    ...
                    return NPC_TALK_STATE_ACTION;
                case 0x3037:
                    OoT_Flags_SetInfTable(INFTABLE_SPOKE_TO_GORON_LINK);
                default:
                    return NPC_TALK_STATE_IDLE;
            }
        case TEXT_STATE_CHOICE:
            if (OoT_Message_ShouldAdvance(play)) {
                if (this->actor.textId == 0x3034) {
                    if (play->msgCtx.choiceIndex == 0) {
                        this->actor.textId = OoT_Flags_GetInfTable(INFTABLE_10B) ? 0x3033 : 0x3035;
                        if (this->actor.textId == 0x3035) {
                            Audio_StopSfxById(NA_SE_EN_GOLON_CRY);
                        }
                    } else {
                        this->actor.textId = OoT_Flags_GetInfTable(INFTABLE_10B) ? 0x3036 : 0x3033;
        ...
        case TEXT_STATE_EVENT:
            if (OoT_Message_ShouldAdvance(play)) {
                switch (this->actor.textId) {
                    case 0x3035:
                        OoT_Flags_SetInfTable(INFTABLE_10B);
                    case 0x3032:
                    case 0x3033:
            ...
            switch (this->actor.textId) {
                ...
                case 0x3059:
                    if (dialogState == TEXT_STATE_NONE) {
                        func_800F4524(&OoT_gSfxDefaultPos, NA_SE_EN_GOLON_WAKE_UP, 60);
                    }
                case 0x3054:
                    if (dialogState == TEXT_STATE_NONE) {
...
u16 EnGo2_GetTextIdGoronFireGeneric(PlayState* play, EnGo2* this) {
    if (OoT_Flags_GetSwitch(play, (this->actor.params & 0xFC00) >> 0xA)) {
        return 0x3071;
    } else {
        return 0x3051;
    }
...
u16 EnGo2_GetTextIdGoronCityStairwell(PlayState* play, EnGo2* this) {
    return !LINK_IS_ADULT ? OoT_Flags_GetInfTable(INFTABLE_E3) ? 0x3022 : 0x300E : 0x3043;
}

s16 EnGo2_UpdateTalkStateGoronCityStairwell(PlayState* play, EnGo2* this) {
    if (OoT_Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x300E) {
            OoT_Flags_SetInfTable(INFTABLE_E3);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextIdGoronCityLostWoods(PlayState* play, EnGo2* this) {
    if (!LINK_IS_ADULT) {
        if (OoT_Flags_GetSwitch(play, 0x1C)) {
            return 0x302F;
        } else {
            return OoT_Flags_GetInfTable(INFTABLE_E6) ? 0x3025 : 0x3024;
        }
    } else {
        return 0x3043;
    }
...
s16 EnGo2_UpdateTalkStateGoronCityLostWoods(PlayState* play, EnGo2* this) {
    if (OoT_Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
        if (this->actor.textId == 0x3024) {
            OoT_Flags_SetInfTable(INFTABLE_E6);
        }
        return NPC_TALK_STATE_IDLE;
    } else {
        return NPC_TALK_STATE_TALKING;
...
u16 EnGo2_GetTextId(PlayState* play, Actor* thisx) {
    ...
    u16 faceReaction = OoT_Text_GetFaceReaction(play, 0x20);
...
s32 func_80A44790(EnGo2* this, PlayState* play) {
    if ((this->actor.params & 0x1F) != GORON_DMT_BIGGORON && (this->actor.params & 0x1F) != GORON_CITY_ROLLING_BIG) {
        return OoT_Npc_UpdateTalking(play, &this->actor, &this->interactInfo.talkState, this->unk_218, EnGo2_GetTextId,
...
void EnGo2_SetShape(EnGo2* this) {
    ...
    OoT_Actor_SetScale(&this->actor, D_80A481F8[index].scale);
...
void EnGo2_CheckCollision(EnGo2* this, PlayState* play) {
    ...
    pos.x += (s16)(xzDist * OoT_Math_SinS(this->actor.shape.rot.y));
    pos.z += (s16)(xzDist * OoT_Math_CosS(this->actor.shape.rot.y));
...
s32 EnGo2_Orient(EnGo2* this, PlayState* play) {
    s16 targetYaw;
    f32 waypointDistSq = OoT_Path_OrientAndGetDistSq(&this->actor, this->path, this->waypoint, &targetYaw);

    OoT_Math_SmoothStepToS(&this->actor.world.rot.y, targetYaw, 6, 4000, 1);
...
s32 func_80A44D84(EnGo2* this) {
    ...
    OoT_Path_OrientAndGetDistSq(&this->actor, this->path, this->waypoint, &targetYaw);
...
s32 EnGo2_IsRollingOnGround(EnGo2* this, s16 arg1, f32 arg2, s16 arg3) {
    ...
    if (this->unk_59C <= 0) {
        if (this->unk_59C == 0) {
            this->unk_590 = OoT_Rand_S16Offset(60, 30);
            this->unk_59C = 0;
    ...
    if ((this->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        if (GameInteractor_Should(VB_BIGGORON_CONSIDER_TRADE_COMPLETE, gSaveContext.bgsFlag)) {
            ...
            if (func_8002F368(play) == EXCH_ITEM_SWORD_BROKEN) {
                if (OoT_Flags_GetInfTable(INFTABLE_B4)) {
                    textId = 0x3055;
                } else {
                    textId = 0x3054;
                }
...
void func_80A45288(EnGo2* this, PlayState* play) {
    ...
    if (this->actionFunc != EnGo2_GoronFireGenericAction) {
        ...
        OoT_Npc_TrackPoint(&this->actor, &this->interactInfo, 4, this->trackingMode);
    }
...
void func_80A45360(EnGo2* this, f32* alpha) {
    ...
    OoT_Math_ApproachF(alpha, alphaTarget, 0.4f, 100.0f);
...
void func_80A454CC(EnGo2* this) {
    switch (this->actor.params & 0x1F) {
        ...
        case GORON_CITY_STAIRWELL:
        case GORON_DMT_FAIRY_HINT:
            Animation_ChangeByInfo(&this->skelAnime, OoT_sAnimationInfo, ENGO2_ANIM_9);
            break;
        case GORON_DMT_BIGGORON:
            if (INV_CONTENT(ITEM_TRADE_ADULT) >= ITEM_SWORD_BROKEN && INV_CONTENT(ITEM_TRADE_ADULT) <= ITEM_EYEDROPS) {
                Animation_ChangeByInfo(&this->skelAnime, OoT_sAnimationInfo, ENGO2_ANIM_4);
                break;
            }
        default:
...
s32 EnGo2_IsCameraModified(EnGo2* this, PlayState* play) {
    ...
    if ((this->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        if (EnGo2_IsWakingUp(this)) {
            OoT_Camera_ChangeSetting(camera, CAM_SET_DIRECTED_YAW);
            func_8005AD1C(camera, 4);
        } else if (!EnGo2_IsWakingUp(this) && (camera->setting == CAM_SET_DIRECTED_YAW)) {
            OoT_Camera_ChangeSetting(camera, CAM_SET_DUNGEON1);
    ...
    if ((this->skelAnime.playSpeed != 0.0f) && (this->skelAnime.animation == &gGoronAnim_004930)) {
        if (this->skelAnime.playSpeed > 0.0f && this->skelAnime.curFrame == 14.0f) {
            if ((this->actor.params & 0x1F) != GORON_DMT_BIGGORON) {
                ...
                func_800F4524(&OoT_gSfxDefaultPos, NA_SE_EN_GOLON_SIT_DOWN, 60);
            }
        }
...
void EnGo2_RollingAnimation(EnGo2* this, PlayState* play) {
    if ((this->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
        this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
        Animation_ChangeByInfo(&this->skelAnime, OoT_sAnimationInfo, ENGO2_ANIM_10);
        this->skelAnime.playSpeed = -0.5f;
    } else {
        Animation_ChangeByInfo(&this->skelAnime, OoT_sAnimationInfo, ENGO2_ANIM_1);
...
void EnGo2_SetupRolling(EnGo2* this, PlayState* play) {
    if ((this->actor.params & 0x1F) == GORON_CITY_ROLLING_BIG || (this->actor.params & 0x1F) == GORON_CITY_LINK) {
        ...
        this->actor.speedXZ = OoT_Flags_GetInfTable(INFTABLE_11E) ? 6.0f : 3.6000001f;
    } else {
...
s32 EnGo2_IsGoronDmtBombFlower(EnGo2* this) {
    ...
    Animation_ChangeByInfo(&this->skelAnime, OoT_sAnimationInfo, ENGO2_ANIM_3);
...
void EnGo2_GoronLinkAnimation(EnGo2* this, PlayState* play) {
    s32 animation = ARRAY_COUNT(OoT_sAnimationInfo);
    ...
    if ((this->actor.params & 0x1F) == GORON_CITY_LINK) {
        ...
        if (animation != ARRAY_COUNT(OoT_sAnimationInfo)) {
...
void EnGo2_GoronFireCamera(EnGo2* this, PlayState* play) {
    ...
    OoT_Path_CopyLastPoint(this->path, &this->at);
    yaw = OoT_Math_Vec3f_Yaw(&this->actor.world.pos, &this->at) + 0xE38;
    this->eye.x = OoT_Math_SinS(yaw) * 100.0f + this->actor.world.pos.x;
    this->eye.z = OoT_Math_CosS(yaw) * 100.0f + this->actor.world.pos.z;
...
void EnGo2_BiggoronAnimation(EnGo2* this) {
    if (INV_CONTENT(ITEM_TRADE_ADULT) >= ITEM_SWORD_BROKEN && INV_CONTENT(ITEM_TRADE_ADULT) <= ITEM_EYEDROPS &&
        ...
        if (DECR(this->animTimer) == 0) {
            this->animTimer = OoT_Rand_S16Offset(30, 30);
            func_800F4524(&OoT_gSfxDefaultPos, NA_SE_EN_GOLON_EYE_BIG, 60);
        }
    }
}
...
void EnGo2_Init(Actor* thisx, PlayState* play) {
    ...
    EnGo2_SetShape(this);
    Animation_ChangeByInfo(&this->skelAnime, OoT_sAnimationInfo, ENGO2_ANIM_0);
    ...
    this->trackingMode = NPC_TRACKING_NONE;
    this->path = OoT_Path_GetByIndex(play, (this->actor.params & 0x3E0) >> 5, 0x1F);
    switch (this->actor.params & 0x1F) {
        case GORON_CITY_ENTRANCE:
        case GORON_CITY_ISLAND:
        case GORON_CITY_LOWEST_FLOOR:
        ...
        case GORON_CITY_LINK:
            if ((OoT_Flags_GetInfTable(INFTABLE_GORON_CITY_DOORS_UNLOCKED))) {
                OoT_Path_CopyLastPoint(this->path, &this->actor.world.pos);
        ...
        case GORON_DMT_BIGGORON:
            ...
            break;
        case GORON_DMT_BOMB_FLOWER:
            if (OoT_Flags_GetInfTable(INFTABLE_EB)) {
                OoT_Path_CopyLastPoint(this->path, &this->actor.world.pos);
                this->actor.home.pos = this->actor.world.pos;
            }
        case GORON_DMT_DC_ENTRANCE:
...
void EnGo2_CurledUp(EnGo2* this, PlayState* play) {
    ...
    if (OoT_Animation_OnFrame(&this->skelAnime, this->skelAnime.endFrame)) {
        if ((this->actor.params & 0x1F) == GORON_DMT_BIGGORON) {
            ...
            OoT_Quake_SetSpeed(quake, -0x3CB0);
...
void EnGo2_GoronDmtBombFlowerAnimation(EnGo2* this, PlayState* play) {
    ...
    this->actor.speedXZ = OoT_Math_SinS(float2);
...
void EnGo2_SlowRolling(EnGo2* this, PlayState* play) {
    ...
    if (!EnGo2_IsRolling(this)) {
        ...
        OoT_Math_ApproachF(&this->actor.speedXZ, EnGo2_GetTargetXZSpeed(this), 0.4f, 0.6f);
        this->actor.shape.rot = this->actor.world.rot;
    }
}
...
void EnGo2_ReverseRolling(EnGo2* this, PlayState* play) {
    if (!EnGo2_IsRolling(this)) {
        OoT_Math_ApproachF(&this->actor.speedXZ, 0.0f, 0.6f, 0.8f);
...
void EnGo2_SetupGetItem(EnGo2* this, PlayState* play) {
    if (OoT_Actor_HasParent(&this->actor, play) || !GameInteractor_Should(VB_GIVE_ITEM_FROM_GORON, true)) {
        this->actor.parent = NULL;
        this->actionFunc = EnGo2_SetGetItem;
    } else {
        OoT_Actor_OfferGetItem(&this->actor, play, this->getItemId, this->actor.xzDistToPlayer + 1.0f,
                           fabsf(this->actor.yDistToPlayer) + 1.0f);
    }
}
    ...
    if ((OoT_Message_GetState(&play->msgCtx) == TEXT_STATE_DONE) && OoT_Message_ShouldAdvance(play) ||
        ...
        switch (this->getItemId) {
            case GI_CLAIM_CHECK:
                ...
                return;
            case GI_TUNIC_GORON:
                OoT_Flags_SetInfTable(INFTABLE_GORON_CITY_DOORS_UNLOCKED);
                EnGo2_GetItemAnimation(this, play);
                return;
            case GI_SWORD_BGS:
...
void EnGo2_BiggoronEyedrops(EnGo2* this, PlayState* play) {
    switch (this->goronState) {
        case 0:
            Animation_ChangeByInfo(&this->skelAnime, OoT_sAnimationInfo, ENGO2_ANIM_5);
            this->actor.flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
        ...
        case 1:
            if (DECR(this->animTimer)) {
                if (this->animTimer == 60 || this->animTimer == 120) {
                    ...
                    func_800F4524(&OoT_gSfxDefaultPos, NA_SE_EV_GORON_WATER_DROP, 60);
                }
            } else {
                func_800F4524(&OoT_gSfxDefaultPos, NA_SE_EN_GOLON_GOOD_BIG, 60);
                Animation_ChangeByInfo(&this->skelAnime, OoT_sAnimationInfo, ENGO2_ANIM_6);
                OoT_Message_ContinueTextbox(play, 0x305A);
...
void EnGo2_GoronLinkStopRolling(EnGo2* this, PlayState* play) {
    ...
    switch (this->goronState) {
        case 0:
            if (OoT_Message_GetState(&play->msgCtx) != TEXT_STATE_NONE) {
                ...
                OoT_Message_StartTextbox(play, 0x3031, NULL);
    ...
    if (OoT_Message_GetState(&play->msgCtx) != TEXT_STATE_CLOSING) {
        ...
        OoT_Flags_SetInfTable(INFTABLE_STOPPED_GORON_LINKS_ROLLING);
        this->trackingMode = NPC_TRACKING_NONE;
        this->unk_211 = false;
        this->isAwake = false;
        this->actionFunc = EnGo2_CurledUp;
    ...
    switch (this->goronState) {
        case 0: // Wake up
            if (OoT_Message_GetState(&play->msgCtx) == TEXT_STATE_CLOSING) {
                ...
                Animation_ChangeByInfo(&this->skelAnime, OoT_sAnimationInfo, ENGO2_ANIM_2);
                this->waypoint = 1;
                ...
                player->actor.world.pos.x =
                    (f32)((OoT_Math_SinS(this->actor.world.rot.y) * -30.0f) + this->actor.world.pos.x);
                player->actor.world.pos.z =
                    (f32)((OoT_Math_CosS(this->actor.world.rot.y) * -30.0f) + this->actor.world.pos.z);
                if (GameInteractor_Should(VB_PLAY_GORON_FREE_CS, true)) {
        ...
        case 3: // Walking away
            ...
            if (this->animTimer > 44) {
                OoT_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 20, NA_SE_EV_IRON_DOOR_CLOSE);
            } else {
                break;
            }
...
void EnGo2_Update(Actor* thisx, PlayState* play) {
    ...
    OoT_Actor_UpdateBgCheckInfo(play, &this->actor, (f32)this->collider.dim.height * 0.5f,
...
s32 EnGo2_DrawCurledUp(EnGo2* this, PlayState* play) {
    ...
    CLOSE_DISPS(play->state.gfxCtx);
    OoT_Matrix_MultVec3f(&D_80A48554, &this->actor.focus.pos);
...
s32 EnGo2_DrawRolling(EnGo2* this, PlayState* play) {
    ...
    OoT_Matrix_RotateZYX((play->state.frames * ((s16)speedXZ * 1400)), 0, this->actor.shape.rot.z, MTXMODE_APPLY);
    gSPMatrix(POLY_OPA_DISP++, MATRIX_NEWMTX(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_OPA_DISP++, gGoronDL_00C140);
    CLOSE_DISPS(play->state.gfxCtx);
    OoT_Matrix_MultVec3f(&D_80A48560, &this->actor.focus.pos);
    return 1;
}
...
s32 EnGo2_OverrideLimbDraw(PlayState* play, s32 limb, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    if (limb == 17) {
        OoT_Matrix_Translate(2800.0f + CVarGetFloat(CVAR_COSMETIC("Goron.NeckLength"), 0.0f), 0.0f, 0.0f, MTXMODE_APPLY);
        ...
        OoT_Matrix_Translate(-2800.0f, 0.0f, 0.0f, MTXMODE_APPLY);
    }
    if (limb == 10) {
    ...
    if ((limb == 10) || (limb == 11) || (limb == 14)) {
        float1 = OoT_Math_SinS(this->unk_226[limb]);
        rot->y += float1 * 200.0f;
        float1 = OoT_Math_CosS(this->unk_24A[limb]);
        rot->z += float1 * 200.0f;
    }
    return 0;
}
...
void EnGo2_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 17) {
        OoT_Matrix_MultVec3f(&D_80A4856C, &this->actor.focus.pos);
    }
}
