...
void OoT_EnGo_Init(Actor* thisx, PlayState* play);
void OoT_EnGo_Destroy(Actor* thisx, PlayState* play);
void OoT_EnGo_Update(Actor* thisx, PlayState* play);
void OoT_EnGo_Draw(Actor* thisx, PlayState* play);
...
void EnGo_SpawnEffectDust(EnGo* this, Vec3f* pos, Vec3f* velocity, Vec3f* accel, u8 initialTimer, f32 scale,
                          f32 scaleStep);
void OoT_EnGo_UpdateEffects(EnGo* this);
void OoT_EnGo_DrawEffects(EnGo* this, PlayState* play);
...
const ActorInit En_Go_InitVars = {
    ...
    OBJECT_OF1D_MAP,
    sizeof(EnGo),
    (ActorFunc)OoT_EnGo_Init,
    (ActorFunc)OoT_EnGo_Destroy,
    (ActorFunc)OoT_EnGo_Update,
    (ActorFunc)OoT_EnGo_Draw,
    NULL,
};
    ...
    switch (thisx->params & 0xF0) {
        ...
        case 0x00:
            if (CHECK_QUEST_ITEM(QUEST_MEDALLION_FIRE)) {
                if (OoT_Flags_GetInfTable(INFTABLE_10F)) {
                    return 0x3042;
                } else {
                    return 0x3041;
                }
            } else if (CHECK_OWNED_EQUIP(EQUIP_TYPE_TUNIC, EQUIP_INV_TUNIC_GORON) ||
                       (OoT_Flags_GetInfTable(INFTABLE_10D))) {
                if (OoT_Flags_GetInfTable(INFTABLE_SPOKE_TO_GORON_LINK)) {
                    return 0x3038;
                } else {
                    return 0x3037;
                }
                ...
                if (OoT_Flags_GetInfTable(INFTABLE_GORON_CITY_DOORS_UNLOCKED)) {
                    if (OoT_Flags_GetInfTable(INFTABLE_10A)) {
                        return 0x3033;
                    } else {
                        return 0x3032;
                    }
        ...
        case 0x10:
            if (OoT_Flags_GetSwitch(play, thisx->params >> 8)) {
                return 0x3052;
            } else {
                return 0x3051;
            }
        ...
        case TEXT_STATE_CLOSING:
            switch (thisx->textId) {
                ...
                case 0x3018:
                    ...
                    break;
                case 0x3036:
                    OoT_Actor_OfferGetItem(thisx, play, GI_TUNIC_GORON, xzRange, yRange);
                    OoT_Flags_SetInfTable(INFTABLE_10D); // EnGo exclusive flag
                ...
                case 0x3037:
                    ...
                    break;
                case 0x3041:
                    OoT_Flags_SetInfTable(INFTABLE_10F);
                    talkState = NPC_TALK_STATE_IDLE;
                    break;
                case 0x3059:
                    talkState = NPC_TALK_STATE_ACTION;
                ...
                switch (thisx->textId) {
                    case 0x300A:
                        if (play->msgCtx.choiceIndex == 0) {
                            if (CUR_UPG_VALUE(UPG_STRENGTH) != 0 || (OoT_Flags_GetInfTable(INFTABLE_EB))) {
                                thisx->textId = 0x300B;
                            } else {
                                thisx->textId = 0x300C;
                            }
                        ...
                        OoT_Message_ContinueTextbox(play, thisx->textId);
                        talkState = NPC_TALK_STATE_TALKING;
                        break;
                    case 0x3034:
                        if (play->msgCtx.choiceIndex == 0) {
                            if (OoT_Flags_GetInfTable(INFTABLE_10B)) {
                                thisx->textId = 0x3033;
                            } else {
                                thisx->textId = 0x3035;
                            }
                        } else if (OoT_Flags_GetInfTable(INFTABLE_10B)) {
                            thisx->textId = 0x3036;
                        } else {
                            thisx->textId = 0x3033;
                        }
                    ...
                    case 0x3055:
                        ...
                        OoT_Flags_SetInfTable(INFTABLE_B4);
                        break;
                }
            }
            break;
...
s32 OoT_EnGo_UpdateTalking(PlayState* play, Actor* thisx, s16* talkState, f32 interactRange, NpcGetTextIdFunc getTextId,
...
void OoT_EnGo_ChangeAnim(EnGo* this, s32 index) {
    OoT_Animation_Change(&this->skelAnime, OoT_sAnimationInfo[index].animation,
                     OoT_sAnimationInfo[index].playSpeed * ((this->actor.params & 0xF0) == 0x90 ? 0.5f : 1.0f), 0.0f,
                     OoT_Animation_GetLastFrame(OoT_sAnimationInfo[index].animation), OoT_sAnimationInfo[index].mode,
                     OoT_sAnimationInfo[index].morphFrames);
}

s32 EnGo_IsActorSpawned(EnGo* this, PlayState* play) {
    if (((this->actor.params) & 0xF0) == 0x90) {
        return true;
    } else if (play->sceneNum == SCENE_FIRE_TEMPLE && !OoT_Flags_GetSwitch(play, (this->actor.params) >> 8) &&
...
void func_80A3F060(EnGo* this, PlayState* play) {
    ...
    OoT_Npc_TrackPoint(&this->actor, &this->interactInfo, 4, trackingMode);
}
...
void func_80A3F0E4(EnGo* this) {
    if (DECR(this->unk_214) == 0) {
        this->unk_216++;
        if (this->unk_216 >= 3) {
            this->unk_214 = OoT_Rand_S16Offset(30, 30);
            this->unk_216 = 0;
        }
    }
}
...
s32 EnGo_IsCameraModified(EnGo* this, PlayState* play) {
    ...
    if ((this->actor.params & 0xF0) == 0x90) {
        OoT_Camera_ChangeSetting(mainCam, CAM_SET_DIRECTED_YAW);
        xyzDistSq *= 4.8f;
    }

    if (fabsf(this->actor.xyzDistToPlayerSq) > xyzDistSq) {
        if (mainCam->setting == CAM_SET_DIRECTED_YAW) {
            OoT_Camera_ChangeSetting(mainCam, CAM_SET_NORMAL0);
        }
        return 0;
    } else {
        return 1;
...
void EnGo_UpdateShadow(EnGo* this) {
    ...
    OoT_Math_SmoothStepToS(&shadowAlpha, shadowAlphaTarget, 10, 60, 1);
...
s32 EnGo_FollowPath(EnGo* this, PlayState* play) {
    ...
    OoT_Math_SmoothStepToS(&this->actor.world.rot.y, RADF_TO_BINANG(OoT_Math_FAtan2F(xDist, zDist)), 10, 1000, 1);
    ...
    if ((SQ(xDist) + SQ(zDist)) < 600.0f) {
        ...
        if ((this->actor.params & 0xF0) != 0x00) {
            return true;
        } else if (OoT_Flags_GetSwitch(play, this->actor.params >> 8)) {
            return true;
...
s32 EnGo_IsRollingOnGround(EnGo* this, s16 unkArg1, f32 unkArg2) {
    if ((this->actor.bgCheckFlags & 1) == 0 || this->actor.velocity.y > 0.0f) {
        ...
        if (this->unk_21A <= 0) {
            if (this->unk_21A == 0) {
                this->unk_21C = OoT_Rand_S16Offset(60, 30);
                this->unk_21A = 0;
...
void func_80A3F908(EnGo* this, PlayState* play) {
    ...
    if (this->actionFunc == EnGo_BiggoronActionFunc || this->actionFunc == EnGo_GoronLinkRolling ||
        ...
        if ((this->actor.params & 0xF0) == 0x90) {
            dialogStarted = OoT_EnGo_UpdateTalking(play, &this->actor, &this->interactInfo.talkState, interactRange,
                                               EnGo_GetTextID, EnGo_UpdateTalkState);
        } else {
            dialogStarted = OoT_Npc_UpdateTalking(play, &this->actor, &this->interactInfo.talkState, interactRange,
                                              EnGo_GetTextID, EnGo_UpdateTalkState);
        }
...
void OoT_EnGo_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_EnGo_ChangeAnim(this, ENGO_ANIM_0);
    ...
    switch (this->actor.params & 0xF0) {
        case 0x00:
            ...
            break;
        case 0x10:
            this->skelAnime.curFrame = OoT_Animation_GetLastFrame(&gGoronAnim_004930);
        ...
        case 0x90:
            ...
            OoT_Actor_SetScale(&this->actor, 0.16f);
            EnGo_SetupAction(this, EnGo_CurledUp);
            break;
        case 0x20:
        case 0x50:
...
void EnGo_GoronLinkRolling(EnGo* this, PlayState* play) {
    if ((EnGo_FollowPath(this, play) == true) && OoT_Flags_GetSwitch(play, this->actor.params >> 8) &&
        (this->unk_218 == 0)) {
        this->actor.speedXZ = 0.0f;
        EnGo_SetupAction(this, func_80A4008C);
...
void EnGo_CurledUp(EnGo* this, PlayState* play) {
    if ((DECR(this->unk_210) == 0) && EnGo_IsCameraModified(this, play)) {
        Audio_PlaySoundGeneral(NA_SE_EN_GOLON_WAKE_UP, &this->actor.projectedPos, 4, &OoT_gSfxDefaultFreqAndVolScale,
...
void EnGo_WakeUp(EnGo* this, PlayState* play) {
    ...
    if (this->skelAnime.playSpeed != 0.0f) {
        OoT_Math_SmoothStepToF(&this->skelAnime.playSpeed, ((this->actor.params & 0xF0) == 0x90 ? 0.5f : 1.0f) * 0.5f, 0.1f,
                           1000.0f, 0.1f);
    ...
    if (DECR(this->unk_212) == 0) {
        Audio_PlaySoundGeneral(NA_SE_EN_GOLON_SIT_DOWN, &this->actor.projectedPos, 4, &OoT_gSfxDefaultFreqAndVolScale,
                               &OoT_gSfxDefaultFreqAndVolScale, &OoT_gSfxDefaultReverb);
...
void func_80A40494(EnGo* this, PlayState* play) {
    ...
    if (!(frame >= 0.0f)) {
        Audio_PlaySoundGeneral(NA_SE_EN_DODO_M_GND, &this->actor.projectedPos, 4, &OoT_gSfxDefaultFreqAndVolScale,
                               &OoT_gSfxDefaultFreqAndVolScale, &OoT_gSfxDefaultReverb);
        EnGo_SpawnDust(this, 10, 0.4f, 0.1f, 16, 26.0f, 2.0f);
        EnGo_ReverseAnimation(this);
        this->skelAnime.playSpeed = 0.0f;
        this->skelAnime.curFrame = 0.0f;
        this->unk_210 = OoT_Rand_S16Offset(30, 30);
        EnGo_SetupAction(this, EnGo_CurledUp);
    }
}
...
void func_80A405CC(EnGo* this, PlayState* play) {
    ...
    lastFrame = OoT_Animation_GetLastFrame(&gGoronAnim_004930);
    OoT_Math_SmoothStepToF(&this->skelAnime.playSpeed, (this->actor.params & 0xF0) == 0x90 ? 0.5f : 1.0f, 0.1f, 1000.0f,
                       0.1f);
    ...
    if (!(frame < lastFrame)) {
        ...
        this->skelAnime.playSpeed = 0.0f;
        this->unk_212 = OoT_Rand_S16Offset(30, 30);
        if (((this->actor.params & 0xF0) == 0x40) && ((OoT_Flags_GetInfTable(INFTABLE_EB)) == 0)) {
...
void EnGo_BiggoronActionFunc(EnGo* this, PlayState* play) {
    if (((this->actor.params & 0xF0) == 0x90) && (this->interactInfo.talkState == NPC_TALK_STATE_ACTION)) {
        if (gSaveContext.bgsFlag) {
            ...
            if (INV_CONTENT(ITEM_TRADE_ADULT) == ITEM_EYEDROPS) {
                OoT_EnGo_ChangeAnim(this, ENGO_ANIM_2);
                this->unk_21E = 100;
                this->interactInfo.talkState = NPC_TALK_STATE_IDLE;
...
void func_80A408D8(EnGo* this, PlayState* play) {
    ...
    if (this->skelAnime.playSpeed != 0.0f) {
        OoT_Math_SmoothStepToF(&this->skelAnime.playSpeed, ((this->actor.params & 0xF0) == 0x90 ? 0.5f : 1.0f) * -1.0f,
                           0.1f, 1000.0f, 0.1f);
        frame = this->skelAnime.curFrame;
        frame += this->skelAnime.playSpeed;
        if (frame >= 12.0f) {
...
void func_80A40A54(EnGo* this, PlayState* play) {
    f32 float1 = ((f32)0x8000 / OoT_Animation_GetLastFrame(&gGoronAnim_010590));
    f32 float2 = this->skelAnime.curFrame * float1;

    this->actor.speedXZ = OoT_Math_SinS((s16)float2);
    if (EnGo_FollowPath(this, play) && this->unk_218 == 0) {
        OoT_EnGo_ChangeAnim(this, ENGO_ANIM_1);
        this->skelAnime.curFrame = OoT_Animation_GetLastFrame(&gGoronAnim_004930);
        this->actor.speedXZ = 0.0f;
        EnGo_SetupAction(this, EnGo_BiggoronActionFunc);
    }
...
void func_80A40B1C(EnGo* this, PlayState* play) {
    if (OoT_Flags_GetInfTable(INFTABLE_EB)) {
        OoT_EnGo_ChangeAnim(this, ENGO_ANIM_3);
...
void EnGo_GetItem(EnGo* this, PlayState* play) {
    ...
    if (OoT_Actor_HasParent(&this->actor, play)) {
        ...
        OoT_Actor_OfferGetItem(&this->actor, play, getItemId, xzDist, yDist);
    }
}
...
void EnGo_DrawCurledUp(EnGo* this, PlayState* play) {
    ...
    OoT_Matrix_MultVec3f(&D_80A41BB4, &this->actor.focus.pos);
    OoT_Matrix_Pop();
...
void EnGo_DrawRolling(EnGo* this, PlayState* play) {
    ...
    OoT_Matrix_Push();
    Gfx_SetupDL_25Opa(play->state.gfxCtx);
    OoT_Matrix_RotateZYX((s16)(play->state.frames * ((s16)this->actor.speedXZ * 1400)), 0, this->actor.shape.rot.z,
                     MTXMODE_APPLY);
    gSPMatrix(POLY_OPA_DISP++, MATRIX_NEWMTX(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_OPA_DISP++, gGoronDL_00C140);
    OoT_Matrix_MultVec3f(&D_80A41BC0, &this->actor.focus.pos);
    OoT_Matrix_Pop();

    CLOSE_DISPS(play->state.gfxCtx);
}

s32 OoT_EnGo_OverrideLimbDraw(PlayState* play, s32 limb, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    EnGo* this = (EnGo*)thisx;
    Vec3s limbRot;
    f32 float1;
    ...
    if ((limb == 10) || (limb == 11) || (limb == 14)) {
        rot->y += OoT_Math_SinS(this->jointTable[limb]) * 200.0f;
        rot->z += OoT_Math_CosS(this->morphTable[limb]) * 200.0f;
    }
...
void EnGo_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == 17) {
        OoT_Matrix_MultVec3f(&D_80A41BCC, &this->actor.focus.pos);
    }
}
...
void OoT_EnGo_Draw(Actor* thisx, PlayState* play) {
    ...
    OoT_EnGo_UpdateEffects(this);
    OoT_Matrix_Push();
    OoT_EnGo_DrawEffects(this, play);
    OoT_Matrix_Pop();
    ...
    if (this->actionFunc == EnGo_CurledUp) {
        ...
        SkelAnime_DrawSkeletonOpa(play, &this->skelAnime, OoT_EnGo_OverrideLimbDraw, EnGo_PostLimbDraw, &this->actor);
        OoT_EnGo_DrawEffects(this, play);
    }
    CLOSE_DISPS(play->state.gfxCtx);
}
