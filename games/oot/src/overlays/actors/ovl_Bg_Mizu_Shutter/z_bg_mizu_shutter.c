...
static Gfx* OoT_sDisplayLists[] = { gObjectMizuObjectsShutterDL_007130, gObjectMizuObjectsShutterDL_0072D0 };
...
void BgMizuShutter_Init(BgMizuShutter* thisx, PlayState* play) {
    ...
    this->displayList = OoT_sDisplayLists[SIZE_PARAM];
    OoT_DynaPolyActor_Init(&this->dyna, DPM_PLAYER);
    OoT_CollisionHeader_GetVirtual(sCollisionHeaders[SIZE_PARAM], &sp30);
    this->dyna.bgId = OoT_DynaPoly_SetBgActor(play, &play->colCtx.dyna, &this->dyna.actor, sp30);
    if ((SIZE_PARAM == BGMIZUSHUTTER_SMALL) || (SIZE_PARAM == BGMIZUSHUTTER_LARGE)) {
        ...
        OoT_Matrix_MultVec3f(&sDisplacements[SIZE_PARAM], &this->openPos);
        ...
        if (this->timerMax != 0x3F * 20) {
            OoT_Flags_UnsetSwitch(play, (u16)this->dyna.actor.params & 0x3F);
            this->dyna.actor.world.pos = this->closedPos;
        }
        if (OoT_Flags_GetSwitch(play, (u16)this->dyna.actor.params & 0x3F)) {
...
void BgMizuShutter_Move(BgMizuShutter* this, PlayState* play) {
    if (OoT_Flags_GetSwitch(play, (u16)this->dyna.actor.params & 0x3F)) {
        OoT_Math_SmoothStepToF(&this->dyna.actor.world.pos.x, this->openPos.x, 1.0f, 4.0f, 0.1f);
        OoT_Math_SmoothStepToF(&this->dyna.actor.world.pos.y, this->openPos.y, 1.0f, 4.0f, 0.1f);
        OoT_Math_SmoothStepToF(&this->dyna.actor.world.pos.z, this->openPos.z, 1.0f, 4.0f, 0.1f);
        ...
        OoT_Math_SmoothStepToF(&this->maxSpeed, 20.0f, 1.0f, 3.0f, 0.1f);
        OoT_Math_SmoothStepToF(&this->dyna.actor.world.pos.x, this->closedPos.x, 1.0f, this->maxSpeed, 0.1f);
        OoT_Math_SmoothStepToF(&this->dyna.actor.world.pos.y, this->closedPos.y, 1.0f, this->maxSpeed, 0.1f);
        OoT_Math_SmoothStepToF(&this->dyna.actor.world.pos.z, this->closedPos.z, 1.0f, this->maxSpeed, 0.1f);
