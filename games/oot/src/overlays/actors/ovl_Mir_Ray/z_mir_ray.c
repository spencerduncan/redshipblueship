...
void OoT_MirRay_Init(Actor* thisx, PlayState* play);
void OoT_MirRay_Destroy(Actor* thisx, PlayState* play);
void OoT_MirRay_Update(Actor* thisx, PlayState* play);
void OoT_MirRay_Draw(Actor* thisx, PlayState* play);

s32 OoT_MirRay_CheckInFrustum(Vec3f* vecA, Vec3f* vecB, f32 pointx, f32 pointy, f32 pointz, s16 radiusA, s16 radiusB);

// Locations of light beams in OoT_sMirRayData
typedef enum {
...
const ActorInit Mir_Ray_InitVars = {
    ...
    OBJECT_MIR_RAY,
    sizeof(MirRay),
    (ActorFunc)OoT_MirRay_Init,
    (ActorFunc)OoT_MirRay_Destroy,
    (ActorFunc)OoT_MirRay_Update,
    (ActorFunc)OoT_MirRay_Draw,
    NULL,
};
...
static MirRayDataEntry OoT_sMirRayData[] = {
...
void OoT_MirRay_SetupCollider(MirRay* this) {
    Vec3f colliderOffset;
    MirRayDataEntry* dataEntry = &OoT_sMirRayData[this->actor.params];
...
void OoT_MirRay_MakeShieldLight(MirRay* this, PlayState* play) {
    Player* player = GET_PLAYER(play);
    MirRayDataEntry* dataEntry = &OoT_sMirRayData[this->actor.params];
    Vec3f reflectionPt;
    Vec3s lightPt;

    if (OoT_MirRay_CheckInFrustum(&this->sourcePt, &this->poolPt, player->actor.world.pos.x,
                              player->actor.world.pos.y + 30.0f, player->actor.world.pos.z, this->sourceEndRad,
                              this->poolEndRad)) {

        if (dataEntry->params & 8) { // Light beams from mirrors
            OoT_Math_Vec3f_Diff(&player->actor.world.pos, &this->sourcePt, &reflectionPt);
        } else { // Light beams from windows
            OoT_Math_Vec3f_Diff(&this->poolPt, &this->sourcePt, &reflectionPt);
        }
        ...
        // Fade up
        OoT_Math_StepToS(&this->lightPointRad, dataEntry->lgtPtMaxRad, 6);
        OoT_Lights_PointNoGlowSetInfo(&this->lightInfo, lightPt.x, lightPt.y, lightPt.z, dataEntry->color.r,
                                  dataEntry->color.g, dataEntry->color.b, this->lightPointRad);
    } else {
        // Fade down
        OoT_Math_StepToS(&this->lightPointRad, 0, 6);
        OoT_Lights_PointSetColorAndRadius(&this->lightInfo, dataEntry->color.r, dataEntry->color.g, dataEntry->color.b,
                                      this->lightPointRad);
    }
}
...
void OoT_MirRay_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_Lights_PointNoGlowSetInfo(&this->lightInfo, this->sourcePt.x, this->sourcePt.y, this->sourcePt.z, 255, 255, 255,
                              100);
    ...
    if (dataEntry->params & 2) {
        ...
        if (!(dataEntry->params & 4)) { // Beams not from mirrors
            OoT_MirRay_SetupCollider(this);
        }
    }

    OoT_Collider_InitQuad(play, &this->shieldRay);
    OoT_Collider_SetQuad(play, &this->shieldRay, &this->actor, &OoT_sQuadInit);
...
void OoT_MirRay_Destroy(Actor* thisx, PlayState* play) {
    ...
    if (OoT_sMirRayData[this->actor.params].params & 2) {
        OoT_Collider_DestroyJntSph(play, &this->colliderSph);
    }

    OoT_Collider_DestroyQuad(play, &this->shieldRay);
}
...
void OoT_MirRay_Update(Actor* thisx, PlayState* play) {
    ...
    if (!this->unLit) {
        if (OoT_sMirRayData[this->actor.params].params & 2) {
            if (OoT_sMirRayData[this->actor.params].params & 4) { // Beams from mirrors
                OoT_MirRay_SetupCollider(this);
            }
            OoT_CollisionCheck_SetAT(play, &play->colChkCtx, &this->colliderSph.base);
        }
        if (this->reflectIntensity > 0.0f) {
            OoT_CollisionCheck_SetAT(play, &play->colChkCtx, &this->shieldRay.base);
        }
        OoT_MirRay_MakeShieldLight(this, play);
...
void OoT_MirRay_SetIntensity(MirRay* this, PlayState* play) {
    f32 sp4C[3];
    f32 temp_f0;
    f32 temp_f0_2;
    f32 temp_f2_2;
    ...
    if (OoT_MirRay_CheckInFrustum(&this->sourcePt, &this->poolPt, shieldMtx->xw, shieldMtx->yw, shieldMtx->zw,
        ...
        if (OoT_sMirRayData[this->actor.params].params & 1) {
...
void OoT_MirRay_SetupReflectionPolys(MirRay* this, PlayState* play, MirRayShieldReflection* reflection) {
    Player* player = GET_PLAYER(play);
    MtxF* shieldMtx;
    s32 i;
    Vec3f posA;
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (OoT_BgCheck_AnyLineTest1(&play->colCtx, &posA, &posB, &posResult, &outPoly, 1)) {
...
void OoT_MirRay_RemoveSimilarReflections(MirRayShieldReflection* reflection) {
    s32 i;
    s32 j;
...
void OoT_MirRay_ReflectedBeam(MirRay* this, PlayState* play, MirRayShieldReflection* reflection) {
    Player* player = GET_PLAYER(play);
    s32 i;
    f32 temp_f0;
    Vec3f vecB;
    ...
    OoT_Collider_SetQuadVertices(&this->shieldRay, &vecA, &vecB, &vecC, &vecD);
    ...
    for (i = 0; i < 6; i++) {
        ...
        if (currentReflection->reflectionPoly != NULL) {
            ...
            if (OoT_Math3D_LineSegVsPlane(normalVec.x, normalVec.y, normalVec.z, currentReflection->reflectionPoly->dist,
                                      &vecB, &vecD, &sp118, 1)) {
                ...
                if (OoT_Math3D_LineSegVsPlane(normalVec.x, normalVec.y, normalVec.z,
...
void OoT_MirRay_Draw(Actor* thisx, PlayState* play) {
    ...
    if ((D_80B8E670 == 0) && !this->unLit && Player_HasMirrorShieldSetToDraw(play)) {
        ...
        OoT_MirRay_SetIntensity(this, play);
        if (!(this->reflectIntensity <= 0.0f)) {
            OPEN_DISPS(play->state.gfxCtx);

            Gfx_SetupDL_25Xlu(play->state.gfxCtx);
            OoT_Matrix_Scale(1.0f, 1.0f, this->reflectIntensity * 5.0f, MTXMODE_APPLY);
            ...
            OoT_MirRay_SetupReflectionPolys(this, play, reflection);
            OoT_MirRay_RemoveSimilarReflections(reflection);
            OoT_MirRay_ReflectedBeam(this, play, reflection);
            ...
            for (i = 0; i < 6; i++) {
                if (reflection[i].reflectionPoly != NULL) {
                    ...
                    OoT_Matrix_Translate(reflection[i].pos.x, reflection[i].pos.y, reflection[i].pos.z, MTXMODE_NEW);
                    OoT_Matrix_Scale(0.01f, 0.01f, 0.01f, MTXMODE_APPLY);
                    OoT_Matrix_Mult(&reflection[i].mtx, MTXMODE_APPLY);
...
s32 OoT_MirRay_CheckInFrustum(Vec3f* vecA, Vec3f* vecB, f32 pointx, f32 pointy, f32 pointz, s16 radiusA, s16 radiusB) {
    ...
    if ((SQ(closestPtx - pointx) + SQ(closestPty - pointy) + SQ(closestPtz - pointz)) <= SQ(coneRadius)) {
        ...
        OoT_Math_Vec3f_Diff(vecB, vecA, &sp5C);

        sp50.x = pointx - vecA->x;
        sp50.y = pointy - vecA->y;
        sp50.z = pointz - vecA->z;

        if (OoT_Math3D_Cos(&sp5C, &sp50) < 0.0f) {
            return 0;
        }
        ...
        if (OoT_Math3D_Cos(&sp5C, &sp44) > 0.0f) {
            return 0;
        }
        return 1;
    }
