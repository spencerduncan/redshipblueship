...
static ColliderTrisElementInit OoT_sTrisElementsInit[4] = {
    {
        {
            ELEMTYPE_UNK2,
...
static CollisionCheckInfoInit OoT_sColChkInfoInit = { 0, 80, 130, MASS_IMMOVABLE };
...
void BgHakaSgami_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_Collider_InitTris(play, colliderScythe);
    OoT_Collider_SetTris(play, colliderScythe, thisx, &OoT_sTrisInit, this->colliderScytheItems);
    OoT_Collider_InitCylinder(play, &this->colliderScytheCenter);
    OoT_Collider_SetCylinder(play, &this->colliderScytheCenter, thisx, &OoT_sCylinderInit);
    ...
    OoT_Effect_Add(play, &this->blureEffectIndex[0], EFFECT_BLURE1, 0, 0, &blureInit);
    OoT_Effect_Add(play, &this->blureEffectIndex[1], EFFECT_BLURE1, 0, 0, &blureInit);
    ...
    if (thisx->params == SCYTHE_TRAP_SHADOW_TEMPLE) {
        ...
        OoT_Actor_SetFocus(thisx, 40.0f);
    }
...
void BgHakaSgami_Destroy(Actor* thisx, PlayState* play) {
    ...
    OoT_Collider_DestroyTris(play, &this->colliderScythe);
    OoT_Collider_DestroyCylinder(play, &this->colliderScytheCenter);
}
...
void BgHakaSgami_Spin(BgHakaSgami* this, PlayState* play) {
    ...
    actorRotYSin = OoT_Math_SinS(this->actor.shape.rot.y);
    actorRotYCos = OoT_Math_CosS(this->actor.shape.rot.y);

    iterateCount = (this->actor.params != 0) ? 4 : 2;

    for (i = iterateCount - 2; i < iterateCount; i++) {
        elementInit = &OoT_sTrisInit.elements[i];
        ...
        OoT_Collider_SetTrisVertices(&this->colliderScythe, i, &scytheVertices[0], &scytheVertices[1], &scytheVertices[2]);
        ...
        OoT_Collider_SetTrisVertices(&this->colliderScythe, (i + 2) % 4, &scytheVertices[0], &scytheVertices[1],
                                 &scytheVertices[2]);
    }
    ...
    if ((this->unk_151 == 0) || play->actorCtx.lensActive) {
        ...
        OoT_EffectBlure_AddVertex(OoT_Effect_GetByIndex(this->blureEffectIndex[0]), &scytheVertices[0], &scytheVertices[1]);
        ...
        OoT_EffectBlure_AddVertex(OoT_Effect_GetByIndex(this->blureEffectIndex[1]), &scytheVertices[0], &scytheVertices[1]);
    }

    OoT_CollisionCheck_SetAT(play, &play->colChkCtx, &this->colliderScythe.base);
    OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->colliderScytheCenter.base);
...
void BgHakaSgami_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->unk_151 != 0) {
        OoT_Gfx_DrawDListXlu(play, object_haka_objects_DL_00BF20);
    } else if (this->actor.params == SCYTHE_TRAP_SHADOW_TEMPLE) {
        OoT_Gfx_DrawDListOpa(play, object_haka_objects_DL_00BF20);
    } else {
        OoT_Gfx_DrawDListOpa(play, object_ice_objects_DL_0021F0);
    }
}
