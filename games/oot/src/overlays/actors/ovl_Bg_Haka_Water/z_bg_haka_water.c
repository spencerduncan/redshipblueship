...
void BgHakaWater_Wait(BgHakaWater* this, PlayState* play) {
    if ((!this->isLowered && OoT_Flags_GetSwitch(play, this->actor.params)) ||
        (this->isLowered && !OoT_Flags_GetSwitch(play, this->actor.params))) {
        if (this->isLowered) {
            this->isLowered = false;
...
void BgHakaWater_ChangeWaterLevel(BgHakaWater* this, PlayState* play) {
    if (!this->isLowered && OoT_Flags_GetSwitch(play, this->actor.params)) {
        this->isLowered = true;
        this->actor.home.pos.y -= 200.0f;
    } else if (this->isLowered && !OoT_Flags_GetSwitch(play, this->actor.params)) {
        this->isLowered = false;
        this->actor.home.pos.y += 200.0f;
    }
    ...
    if (OoT_Math_StepToF(&this->actor.world.pos.y, this->actor.home.pos.y, 0.5f) != 0) {
        this->actionFunc = BgHakaWater_Wait;
        if (this->isLowered) {
...
void BgHakaWater_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPSegment(POLY_XLU_DISP++, 0x08,
               OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, play->gameplayFrames % 128, play->gameplayFrames % 128, 32, 32,
                                  1, 0, (0 - play->gameplayFrames) % 128, 32, 32, 1, 1, 0, -1));

    gSPMatrix(POLY_XLU_DISP++, MATRIX_NEWMTX(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
    gSPDisplayList(POLY_XLU_DISP++, gBotwWaterRingDL);

    OoT_Matrix_Translate(0.0f, 92.0f, -1680.0f, MTXMODE_NEW);
