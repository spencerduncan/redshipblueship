...
void OoT_ObjLightswitch_Init(Actor* thisx, PlayState* play);
void OoT_ObjLightswitch_Destroy(Actor* thisx, PlayState* play);
void OoT_ObjLightswitch_Update(Actor* thisx, PlayState* play);
void OoT_ObjLightswitch_Draw(Actor* thisx, PlayState* play);
...
const ActorInit Obj_Lightswitch_InitVars = {
    ...
    (ActorFunc)OoT_ObjLightswitch_Init,
...
void OoT_ObjLightswitch_InitCollider(ObjLightswitch* this, PlayState* play) {
    s32 pad;
    ...
    if (sunLightArrowsEnabledOnSunSwitchLoad) {
        OoT_Collider_SetJntSph(play, &this->collider, &this->actor, &sColliderLightArrowInit, this->colliderItems);
    } else {
        OoT_Collider_SetJntSph(play, &this->collider, &this->actor, &sColliderJntSphInit, this->colliderItems);
    }
    OoT_Matrix_SetTranslateRotateYXZ(this->actor.world.pos.x,
...
void ObjLightswitch_ClearSwitchFlag(ObjLightswitch* this, PlayState* play) {
    if (OoT_Flags_GetSwitch(play, this->actor.params >> 8 & 0x3F)) {
        OoT_Flags_UnsetSwitch(play, this->actor.params >> 8 & 0x3F);
...
void ObjLightswitch_SpawnDisappearEffects(ObjLightswitch* this, PlayState* play) {
    Vec3f pos;
    f32 s = OoT_Math_SinS(this->actor.shape.rot.y);
    f32 c = OoT_Math_CosS(this->actor.shape.rot.y);
    f32 x;
    f32 y;
    f32 z;
    s32 pad;
    ...
    if (this->alpha >= (100 << 6)) {
        x = (CLAMP_MAX((1.0f - 1.0f / (255 << 6) * this->alpha) * 400.0f, 60.0f) - 30.0f + 30.0f) * OoT_Rand_ZeroOne();
        y = x - 30.0f;
        if (x > 30.0f) {
            x = 30.0f;
        } else {
        ...
        OoT_EffectSsDeadDb_Spawn(play, &pos, &D_80B97F74, &D_80B97F74, 100, 0, 255, 255, 160, 160, 255, 0, 0, 1, 9, true);
    }
}

void OoT_ObjLightswitch_Init(Actor* thisx, PlayState* play) {
    ObjLightswitch* this = (ObjLightswitch*)thisx;
    s32 switchFlagSet = OoT_Flags_GetSwitch(play, this->actor.params >> 8 & 0x3F);
    ...
    if ((this->actor.params & 1) == 1) {
        if (switchFlagSet) {
            OoT_Math_Vec3f_Copy(&this->actor.world.pos, &D_80B97F68);
            OoT_Math_Vec3f_Copy(&this->actor.home.pos, &D_80B97F68);
        }
        ...
        if (OoT_Actor_SpawnAsChild(&play->actorCtx, &this->actor, play, ACTOR_OBJ_OSHIHIKI, this->actor.home.pos.x,
    ...
    OoT_ObjLightswitch_InitCollider(this, play);
...
void OoT_ObjLightswitch_Destroy(Actor* thisx, PlayState* play2) {
...
void ObjLightswitch_TurnOn(ObjLightswitch* this, PlayState* play) {
    if (func_8005B198() == this->actor.category || this->toggleDelay <= 0) {
        ...
        OoT_Math_StepToS(&this->flameRingRotSpeed, -0xAA, 0xA);
...
void ObjLightswitch_TurnOff(ObjLightswitch* this, PlayState* play) {
    if ((this->actor.params >> 4 & 3) != OBJLIGHTSWITCH_TYPE_1 || func_8005B198() == this->actor.category ||
        ...
        OoT_Math_StepToS(&this->flameRingRotSpeed, 0, 0xA);
        this->flameRingRot += this->flameRingRotSpeed;
...
void OoT_ObjLightswitch_Update(Actor* thisx, PlayState* play2) {
    ObjLightswitch* this = (ObjLightswitch*)thisx;
    PlayState* play = play2;
...
void ObjLightswitch_DrawOpa(ObjLightswitch* this, PlayState* play) {
    ...
    if ((this->actor.params & 1) == 1) {
        ...
        OoT_Matrix_SetTranslateRotateYXZ(pos.x, pos.y, pos.z, &this->actor.shape.rot);
        OoT_Matrix_Scale(this->actor.scale.x, this->actor.scale.y, this->actor.scale.z, MTXMODE_APPLY);
    } else {
    ...
    OoT_Matrix_SetTranslateRotateYXZ(pos.x, pos.y, pos.z, &rot);
    OoT_Matrix_Scale(this->actor.scale.x, this->actor.scale.y, this->actor.scale.z, MTXMODE_APPLY);
    gSPMatrix(POLY_OPA_DISP++, MATRIX_NEWMTX(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
...
void ObjLightswitch_DrawXlu(ObjLightswitch* this, PlayState* play) {
    ...
    OoT_Matrix_SetTranslateRotateYXZ(sp68.x, sp68.y, sp68.z, &sp60);
    OoT_Matrix_Scale(this->actor.scale.x, this->actor.scale.y, this->actor.scale.z, MTXMODE_APPLY);
    gSPMatrix(POLY_XLU_DISP++, MATRIX_NEWMTX(play->state.gfxCtx), G_MTX_NOPUSH | G_MTX_LOAD | G_MTX_MODELVIEW);
