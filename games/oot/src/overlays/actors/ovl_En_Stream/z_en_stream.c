...
void OoT_EnStream_Init(Actor* thisx, PlayState* play);
void OoT_EnStream_Destroy(Actor* thisx, PlayState* play);
void OoT_EnStream_Update(Actor* thisx, PlayState* play);
void OoT_EnStream_Draw(Actor* thisx, PlayState* play);
void OoT_EnStream_WaitForPlayer(EnStream* this, PlayState* play);
...
const ActorInit En_Stream_InitVars = {
    ...
    OBJECT_STREAM,
    sizeof(EnStream),
    (ActorFunc)OoT_EnStream_Init,
    (ActorFunc)OoT_EnStream_Destroy,
    (ActorFunc)OoT_EnStream_Update,
    (ActorFunc)OoT_EnStream_Draw,
    NULL,
};
...
void OoT_EnStream_SetupAction(EnStream* this, EnStreamActionFunc actionFunc) {
...
void OoT_EnStream_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_EnStream_SetupAction(this, OoT_EnStream_WaitForPlayer);
}
...
void OoT_EnStream_SuckPlayer(EnStream* this, PlayState* play) {
    ...
    if (func_80B0B81C(&this->actor.world.pos, &player->actor.world.pos, &posDifference, this->actor.scale.y) != 0) {
        ...
        if (xzDist > 3.0f) {
            OoT_Math_SmoothStepToF(&player->pushedSpeed, 3.0f, 0.5f, xzDist, 0.0f);
        } else {
            player->pushedSpeed = 0.0f;
            OoT_Math_SmoothStepToF(&player->actor.world.pos.x, this->actor.world.pos.x, 0.5f, 3.0f, 0.0f);
            OoT_Math_SmoothStepToF(&player->actor.world.pos.z, this->actor.world.pos.z, 0.5f, 3.0f, 0.0f);
        }
        if (yDistWithOffset > 0.0f) {
            OoT_Math_SmoothStepToF(&player->actor.velocity.y, -3.0f, 0.7f, yDistWithOffset, 0.0f);
...
void OoT_EnStream_WaitForPlayer(EnStream* this, PlayState* play) {
    ...
    if (func_80B0B81C(&this->actor.world.pos, &player->actor.world.pos, &temp, this->actor.scale.y) != 0) {
        OoT_EnStream_SetupAction(this, OoT_EnStream_SuckPlayer);
    }
}
...
void OoT_EnStream_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPSegment(POLY_XLU_DISP++, 0x08,
               OoT_Gfx_TwoTexScroll(play->state.gfxCtx, 0, frames * 30, -multipliedFrames, 0x40, 0x40, 1, multipliedFrames,
