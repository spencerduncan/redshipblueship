...
u16 func_80AA2AA0(PlayState* play, Actor* thisx) {
    ...
    if (!OoT_Flags_GetInfTable(INFTABLE_B8)) {
        return 0x2000;
    }
    ...
    if (gSaveContext.eventInf[0] & 0x400) {
        ...
        if (!OoT_Flags_GetEventChkInf(EVENTCHKINF_WON_COW_IN_MALONS_RACE) && (gSaveContext.timerSeconds < 0x32)) {
            return 0x208F;
    ...
    if ((!(player->stateFlags1 & PLAYER_STATE1_ON_HORSE)) &&
        (OoT_Actor_FindNearby(play, thisx, ACTOR_EN_HORSE, 1, 1200.0f) == NULL)) {
        return 0x2001;
    }
    if (!OoT_Flags_GetInfTable(INFTABLE_B9)) {
        return 0x2002;
    } else {
        return 0x2003;
    }
...
s16 func_80AA2BD4(PlayState* play, Actor* thisx) {
    ...
    switch (OoT_Message_GetState(&play->msgCtx)) {
        ...
        case TEXT_STATE_CHOICE:
            if (OoT_Message_ShouldAdvance(play)) {
                OoT_Flags_SetInfTable(INFTABLE_B9);
                if (play->msgCtx.choiceIndex == 0) {
                    if (OoT_Flags_GetEventChkInf(EVENTCHKINF_WON_COW_IN_MALONS_RACE)) {
                        OoT_Message_ContinueTextbox(play, 0x2091);
                    } else if (HIGH_SCORE(HS_HORSE_RACE) == 0) {
                        OoT_Message_ContinueTextbox(play, 0x2092);
                    } else {
                        OoT_Message_ContinueTextbox(play, 0x2090);
                    }
                }
            }
            break;
        case TEXT_STATE_CLOSING:
            switch (thisx->textId) {
                case 0x2000:
                    OoT_Flags_SetInfTable(INFTABLE_B8);
                    ret = NPC_TALK_STATE_IDLE;
                    break;
                case 0x208F:
                    OoT_Flags_SetEventChkInf(EVENTCHKINF_WON_COW_IN_MALONS_RACE);
                case 0x2004:
                case 0x2012:
...
void EnMa3_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_SkelAnime_InitFlex(play, &this->skelAnime, &gMalonAdultSkel, NULL, NULL, NULL, 0);
...
void EnMa3_Update(Actor* thisx, PlayState* play) {
    ...
    OoT_Npc_UpdateTalking(play, &this->actor, &this->interactInfo.talkState, (f32)this->collider.dim.radius + 150.0f,
...
s32 EnMa3_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    if ((limbIndex == MALON_ADULT_LIMB_CHEST_AND_NECK) || (limbIndex == MALON_ADULT_LIMB_LEFT_SHOULDER) ||
        ...
        rot->y += OoT_Math_SinS(this->unk_212[limbIndex].y) * 200.0f;
        rot->z += OoT_Math_CosS(this->unk_212[limbIndex].z) * 200.0f;
    }
    return false;
}
...
void EnMa3_Draw(Actor* thisx, PlayState* play) {
    static void* OoT_sMouthTextures[] = { gMalonAdultMouthNeutralTex, gMalonAdultMouthSadTex, gMalonAdultMouthHappyTex };
    static void* OoT_sEyeTextures[] = { gMalonAdultEyeOpenTex, gMalonAdultEyeHalfTex, gMalonAdultEyeClosedTex };
    EnMa3* this = (EnMa3*)thisx;
    Camera* camera;
    f32 someFloat;
    s32 pad;
    ...
    someFloat = OoT_Math_Vec3f_DistXZ(&this->actor.world.pos, &camera->eye);
