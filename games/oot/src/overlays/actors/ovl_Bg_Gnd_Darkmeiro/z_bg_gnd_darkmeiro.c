...
void BgGndDarkmeiro_Init(Actor* thisx, PlayState* play2) {
    ...
    switch (this->dyna.actor.params & 0xFF) {
        ...
        case DARKMEIRO_CLEAR_BLOCK:
            OoT_CollisionHeader_GetVirtual(&gClearBlockCol, &colHeader);
        ...
        case DARKMEIRO_BLOCK_TIMER:
            ...
            thisx->draw = NULL;
            if (OoT_Flags_GetSwitch(play, ((this->dyna.actor.params >> 8) & 0x3F) + 1)) {
                this->timer1 = 64;
                this->actionFlags |= 4;
            }
            if (OoT_Flags_GetSwitch(play, ((this->dyna.actor.params >> 8) & 0x3F) + 2)) {
                this->timer2 = 64;
                this->actionFlags |= 8;
            }
    ...
    if (OoT_Flags_GetSwitch(play, ((this->dyna.actor.params >> 8) & 0x3F) + 1)) {
        if (this->actionFlags & 4) {
            if (this->timer1 > 0) {
                ...
                OoT_Flags_UnsetSwitch(play, ((this->dyna.actor.params >> 8) & 0x3F) + 1);
                this->actionFlags &= ~4;
            }
        } else {
            this->actionFlags |= 4;
            this->timer1 = 304;
            Audio_PlaySoundGeneral(NA_SE_EV_RED_EYE, &OoT_gSfxDefaultPos, 4, &OoT_gSfxDefaultFreqAndVolScale,
    ...
    if (OoT_Flags_GetSwitch(play, ((this->dyna.actor.params >> 8) & 0x3F) + 2)) {
        if (this->actionFlags & 8) {
            if (this->timer2 > 0) {
                ...
                OoT_Flags_UnsetSwitch(play, ((this->dyna.actor.params >> 8) & 0x3F) + 2);
                this->actionFlags &= ~8;
            }
        } else {
            this->actionFlags |= 8;
...
void BgGndDarkmeiro_DrawInvisiblePath(Actor* thisx, PlayState* play) {
    OoT_Gfx_DrawDListXlu(play, gShadowTrialPathDL);
}
...
void BgGndDarkmeiro_DrawSwitchBlock(Actor* thisx, PlayState* play) {
    ...
    if (vanishTimer != 0) {
        if (vanishTimer > 64) {
            ...
            this->timer2 = (OoT_Math_CosS((u16)this->timer1 * 0x1000) * 64.0f) + 127.0f;
            if (this->timer2 > 127) {
                this->timer2 = 127;
            }
        } else {
        ...
        OoT_Gfx_DrawDListXlu(play, gClearBlockDL);
    }
}
