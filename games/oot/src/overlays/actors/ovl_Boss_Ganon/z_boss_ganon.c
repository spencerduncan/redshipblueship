            ...
            for (i = 1; i < 15; i++) {
                ...
                if (this->shockGlow) {
                    gSPSegment(POLY_XLU_DISP++, 0x08,
                               OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0, 0, 32, 64, 1, 0, (this->unk_1A2 + i) * -15,
                                                  32, 64, 0, 0, 0, -15));
...
void BossGanon_DrawBigMagicCharge(BossGanon* this, PlayState* play) {
    ...
    if (this->unk_284 > 0.0f) {
        ...
        gSPSegment(POLY_XLU_DISP++, 0x08,
                   OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, this->unk_1A2 * -2, 0, 0x40, 0x40, 1, 0,
        ...
        gSPSegment(POLY_XLU_DISP++, 0x09,
                   OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0, 0, 0x20, 0x20, 1, 0, this->unk_1A2 * -4, 0x20, 0x20, 0,
                                      0, 0, -4));
        ...
        gSPSegment(POLY_XLU_DISP++, 0x0A,
                   OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0, 0, 0x20, 0x20, 1, this->unk_1A2 * 2,
...
void BossGanon_DrawDarkVortex(BossGanon* this, PlayState* play) {
    ...
    if (this->fwork[GDF_VORTEX_ALPHA] > 0.0f) {
        ...
        gDPPipeSync(POLY_XLU_DISP++);
        gSPSegment(POLY_XLU_DISP++, 0x08,
                   OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, this->unk_1A2 * -8, 0, 0x20, 0x40, 1, this->unk_1A2 * -4,
...
void BossGanon_DrawEffects(PlayState* play) {
    ...
    for (i = 0; i < 150; i++, eff++) {
        if (eff->type == GDF_EFF_IMPACT_DUST_DARK) {
            ...
            gSPSegment(POLY_XLU_DISP++, 0x08,
                       OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, eff->timer * 4, 0, 32, 64, 1, eff->timer * 2,
    ...
    for (i = 0; i < 150; i++, eff++) {
        if (eff->type == GDF_EFF_SHOCKWAVE) {
            ...
            gDPSetEnvColor(POLY_XLU_DISP++, 150, 255, 0, 128);
            gSPSegment(POLY_XLU_DISP++, 0x08,
                       OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, (eff->timer * 100), 0, 64, 32, 1, (eff->timer * 100),
                                          0, 64, 32, 100, 0, 100, 0));
            OoT_Matrix_Translate(eff->pos.x, eff->pos.y, eff->pos.z, MTXMODE_NEW);
    ...
    for (i = 0; i < 150; i++, eff++) {
        if (eff->type == GDF_EFF_BLACK_DOT) {
            ...
            gSPSegment(POLY_XLU_DISP++, 0x0A,
                       OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0, 0, 32, 32, 1, eff->timer * 2, eff->timer * -20, 64,
                                          64, 0, 0, 2, -20));
            OoT_Matrix_Translate(eff->pos.x, eff->pos.y, eff->pos.z, MTXMODE_NEW);
            OoT_Matrix_ReplaceRotation(&play->billboardMtxF);
            OoT_Matrix_Scale(eff->scale, eff->scale, 1.0f, MTXMODE_APPLY);
