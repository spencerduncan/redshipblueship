...
void EnHorseNormal_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_Collider_SetCylinder(play, &this->bodyCollider, &this->actor, &OoT_sCylinderInit1);
    ...
    OoT_Collider_InitCylinder(play, &this->cloneCollider);
    OoT_Collider_SetCylinder(play, &this->cloneCollider, &this->actor, &OoT_sCylinderInit2);
    ...
    if (play->sceneNum == SCENE_LON_LON_RANCH) {
        ...
        } else if (OoT_Flags_GetEventChkInf(EVENTCHKINF_EPONA_OBTAINED) || (DREG(1) != 0)) {
        ...
        OoT_Skin_Init(play, &this->skin, &gHorseNormalSkel, &gHorseNormalIdleAnim);
...
void EnHorseNormal_Destroy(Actor* thisx, PlayState* play) {
    ...
    OoT_Collider_DestroyCylinder(play, &this->cloneCollider);
...
void func_80A6B91C(EnHorseNormal* this, PlayState* play) {
    ...
    OoT_Animation_Change(&this->skin.skelAnime, OoT_sAnimations[this->animationIdx], func_80A6B30C(this), 0.0f,
...
void EnHorseNormal_FollowPath(EnHorseNormal* this, PlayState* play) {
    ...
    OoT_Math_SmoothStepToS(&this->actor.world.rot.y, OoT_Math_FAtan2F(dx, dz) * (0x8000 / M_PI), 0xA, 0x7D0, 1);
...
void EnHorseNormal_Wander(EnHorseNormal* this, PlayState* play) {
    ...
    switch (D_80A6D510[this->animationIdx]) {
        ...
        case 1:
            if (OoT_Rand_ZeroOne() < 0.1f) {
                this->unk_218 = 2.0f * OoT_Rand_ZeroOne() - 1.0f;
            }
            ...
            if (OoT_Rand_ZeroOne() < 0.1f || (this->unk_21E == 0 && ((this->actor.bgCheckFlags & 8) ||
                                                                 (this->bodyCollider.base.ocFlags1 & OC1_HIT) ||
                                                                 (this->headCollider.base.ocFlags1 & OC1_HIT)))) {
                this->unk_21E += (OoT_Rand_ZeroOne() * 30.0f) - 15.0f;
                if (this->unk_21E > 50) {
                    this->unk_21E = 50;
            ...
            if (this->unk_21C < -300) {
                ...
                this->unk_21C = 300;
            } else if (OoT_Rand_ZeroOne() < 0.25f && fabsf(this->unk_21C) < 100.0f) {
                this->unk_21C = 0;
                this->unk_21E = 0;
            }
    ...
    if (phi_t0 != this->animationIdx || OoT_SkelAnime_Update(&this->skin.skelAnime)) {
        if (phi_t0 != this->animationIdx) {
            this->animationIdx = phi_t0;
            this->unk_1E4 &= ~1;
            this->unk_1E4 &= ~2;
            if (phi_t0 == 1) {
                Audio_PlaySoundGeneral(NA_SE_EV_HORSE_GROAN, &this->unk_204, 4, &OoT_gSfxDefaultFreqAndVolScale,
                                       &OoT_gSfxDefaultFreqAndVolScale, &OoT_gSfxDefaultReverb);
            } else if (phi_t0 == 3) {
                Audio_PlaySoundGeneral(NA_SE_EV_HORSE_NEIGH, &this->unk_204, 4, &OoT_gSfxDefaultFreqAndVolScale,
                                       &OoT_gSfxDefaultFreqAndVolScale, &OoT_gSfxDefaultReverb);
            } else {
                func_80A6BCEC(this);
            }
            OoT_Animation_Change(&this->skin.skelAnime, OoT_sAnimations[this->animationIdx], func_80A6B30C(this), 0.0f,
                             OoT_Animation_GetLastFrame(OoT_sAnimations[this->animationIdx]), ANIMMODE_ONCE, -3.0f);
        } else {
            switch (D_80A6D510[this->animationIdx]) {
                case 0:
                    if (OoT_Rand_ZeroOne() < 0.25f) {
                        this->unk_218 = 1.0f;
                        phi_t0 = 4;
                    } else {
                        phi_t0 = D_80A6D4F4[(s32)(OoT_Rand_ZeroOne() * 2)];
                        this->actor.speedXZ = 0.0f;
                        this->unk_218 = 0.0f;
                    }
                    break;
...
void func_80A6C8E0(EnHorseNormal* this, PlayState* play) {
    ...
    sp28.x = (OoT_Math_SinS(this->actor.shape.rot.y) * 30.0f) + this->actor.world.pos.x;
    sp28.y = this->actor.world.pos.y + 60.0f;
    sp28.z = (OoT_Math_CosS(this->actor.shape.rot.y) * 30.0f) + this->actor.world.pos.z;
    this->unk_220 = OoT_BgCheck_EntityRaycastFloor3(&play->colCtx, &sp38, &sp24, &sp28);
    this->actor.shape.rot.x = OoT_Math_FAtan2F(this->actor.world.pos.y - this->unk_220, 30.0f) * (0x8000 / M_PI);
}

static EnHorseNormalActionFunc OoT_sActionFuncs[] = {
...
void EnHorseNormal_Update(Actor* thisx, PlayState* play) {
    ...
    OoT_Actor_UpdateBgCheckInfo(play, &this->actor, 20.0f, 35.0f, 100.0f, 0x1D);
...
void EnHorseNormal_PostDraw(Actor* thisx, PlayState* play, Skin* skin) {
    ...
    for (i = 0; i < this->headCollider.count; i++) {
        ...
        OoT_Skin_GetLimbPos(skin, this->headCollider.elements[i].dim.limb, &sp4C, &sp40);
...
void func_80A6CC88(PlayState* play, EnHorseNormal* this, Vec3f* arg2) {
    ...
    OoT_SkinMatrix_Vec3fMtxFMultXYZW(&play->viewProjectionMtxF, arg2, &this->unk_1E8, &wDest);
    this->unk_1F4 = this->unk_1E8;
    this->unk_1F4.y += 120.0f;

    if (this->animationIdx == 0 && curFrame > 28.0f && !(this->unk_1E4 & 8)) {
        this->unk_1E4 |= 8;
        Audio_PlaySoundGeneral(NA_SE_EV_HORSE_SANDDUST, &this->unk_1E8, 4, &OoT_gSfxDefaultFreqAndVolScale,
                               &OoT_gSfxDefaultFreqAndVolScale, &OoT_gSfxDefaultReverb);
    } else if (this->animationIdx == 3 && curFrame > 25.0f && !(this->unk_1E4 & 0x10)) {
        this->unk_1E4 |= 0x10;
        Audio_PlaySoundGeneral(NA_SE_EV_HORSE_LAND2, &this->unk_1E8, 4, &OoT_gSfxDefaultFreqAndVolScale,
                               &OoT_gSfxDefaultFreqAndVolScale, &OoT_gSfxDefaultReverb);
    } else if (this->animationIdx == 3 && this->unk_1E4 & 0x20) {
        this->unk_1E4 &= ~0x20;
        Audio_PlaySoundGeneral(NA_SE_EV_HORSE_NEIGH, &this->unk_1F4, 4, &OoT_gSfxDefaultFreqAndVolScale,
                               &OoT_gSfxDefaultFreqAndVolScale, &OoT_gSfxDefaultReverb);
    } else if (this->animationIdx == 1 && this->unk_1E4 & 0x20) {
        this->unk_1E4 &= ~0x20;
        Audio_PlaySoundGeneral(NA_SE_EV_HORSE_GROAN, &this->unk_1F4, 4, &OoT_gSfxDefaultFreqAndVolScale,
                               &OoT_gSfxDefaultFreqAndVolScale, &OoT_gSfxDefaultReverb);
    }
}
...
void EnHorseNormal_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->action == HORSE_WAIT_CLONE) {
        ...
        SkinMatrix_SetTranslateRotateYXZScale(&skinMtx, this->actor.scale.x, this->actor.scale.y, this->actor.scale.z,
                                              ...
                                              clonePos.z);
        mtx1 = OoT_SkinMatrix_MtxFToNewMtx(play->state.gfxCtx, &skinMtx);
        if (mtx1 == NULL) {
            return;
        }
        ...
        OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->cloneCollider.base);
        Gfx_SetupDL_44Xlu(play->state.gfxCtx);
        gDPSetPrimColor(POLY_XLU_DISP++, 0, 0, 0, 0, 0, 255);
        OoT_Matrix_Translate(clonePos.x, clonePos.y, clonePos.z, MTXMODE_NEW);
        temp_f0_4 = (1.0f - (distFromGround * 0.01f)) * this->actor.shape.shadowScale;
        OoT_Matrix_Scale(this->actor.scale.x * temp_f0_4, 1.0f, this->actor.scale.z * temp_f0_4, MTXMODE_APPLY);
