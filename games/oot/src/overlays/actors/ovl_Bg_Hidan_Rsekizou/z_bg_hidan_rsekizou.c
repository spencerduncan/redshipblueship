...
void BgHidanRsekizou_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_CollisionHeader_GetVirtual(&gFireTempleSpinningFlamethrowerCol, &colHeader);
...
void BgHidanRsekizou_Update(Actor* thisx, PlayState* play) {
    ...
    yawSine = OoT_Math_SinS(this->dyna.actor.shape.rot.y);
    yawCosine = OoT_Math_CosS(this->dyna.actor.shape.rot.y);
...
Gfx* BgHidanRsekizou_DrawFireball(PlayState* play, BgHidanRsekizou* this, s16 frame, MtxF* mf, s32 a,
    ...
    if (a == 0) {
        OoT_sins = -OoT_Math_SinS(this->dyna.actor.shape.rot.y - (frame * 1500));
        OoT_coss = -OoT_Math_CosS(this->dyna.actor.shape.rot.y - (frame * 1500));
    } else {
        OoT_sins = OoT_Math_SinS(this->dyna.actor.shape.rot.y - (frame * 1500));
        OoT_coss = OoT_Math_CosS(this->dyna.actor.shape.rot.y - (frame * 1500));
    }
    ...
    mf->xw = (tmpf7 * OoT_sins) + this->dyna.actor.world.pos.x;
    mf->yw = (this->dyna.actor.world.pos.y + 30.0f) + ((7.0f / 10.0f) * fVar6);
    mf->zw = (tmpf7 * OoT_coss) + this->dyna.actor.world.pos.z;

    gSPMatrix(displayList++, OoT_Matrix_MtxFToMtx(MATRIX_CHECKFLOATS(mf), Graph_Alloc(play->state.gfxCtx, sizeof(Mtx))),
...
void BgHidanRsekizou_Draw(Actor* thisx, PlayState* play) {
    ...
    OoT_Matrix_MtxFCopy(&mf, &gMtxFClear);

    POLY_XLU_DISP = OoT_Gfx_SetupDL(POLY_XLU_DISP, 0x14);

    // Strange original code. Add || 1 for frame interpolation to get correct.
    if ((s16)((OoT_Camera_GetCamDirYaw(GET_ACTIVE_CAM(play)) - this->dyna.actor.shape.rot.y) - 0x2E6C) >= 0 || 1) {
        for (i = 3; i >= 0; i--) {
