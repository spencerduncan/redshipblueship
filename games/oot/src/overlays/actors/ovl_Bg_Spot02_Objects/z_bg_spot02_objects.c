...
void BgSpot02Objects_Init(Actor* thisx, PlayState* play) {
    ...
    switch (thisx->params) {
        ...
        case 2:
            ...
            if (thisx->params == 0) {
                if (OoT_Flags_GetSwitch(play, this->unk_16B)) {
                ...
                OoT_CollisionHeader_GetVirtual(&object_spot02_objects_Col_012BA4, &colHeader);
            } else if (thisx->params == 1) {
                this->actionFunc = func_808AC8FC;
                OoT_CollisionHeader_GetVirtual(&object_spot02_objects_Col_0128D8, &colHeader);
                ...
                OoT_CollisionHeader_GetVirtual(&object_spot02_objects_Col_0133EC, &colHeader);
            }

            this->dyna.bgId = OoT_DynaPoly_SetBgActor(play, &play->colCtx.dyna, thisx, colHeader);

            if (((OoT_Flags_GetEventChkInf(EVENTCHKINF_DESTROYED_ROYAL_FAMILY_TOMB)) &&
        ...
        case 3:
            this->unk_16A = 0;
            OoT_Actor_ChangeCategory(play, &play->actorCtx, thisx, ACTORCAT_ITEMACTION);
...
void func_808AC908(BgSpot02Objects* this, PlayState* play) {
    ...
    if (GameInteractor_Should(VB_PLAY_ROYAL_FAMILY_TOMB_EXPLODE,
        ...
        OoT_Flags_SetEventChkInf(EVENTCHKINF_DESTROYED_ROYAL_FAMILY_TOMB);
        this->timer = 25;
        pos.x = (OoT_Math_SinS(this->dyna.actor.shape.rot.y) * 50.0f) + this->dyna.actor.world.pos.x;
        pos.y = this->dyna.actor.world.pos.y + 30.0f;
        pos.z = (OoT_Math_CosS(this->dyna.actor.shape.rot.y) * 50.0f) + this->dyna.actor.world.pos.z;
        OoT_EffectSsBomb2_SpawnLayered(play, &pos, &zeroVec, &zeroVec, 70, 30);
...
void func_808ACA08(BgSpot02Objects* this, PlayState* play) {
    ...
    if (this->timer == 20) {
        ...
        OoT_EffectSsHahen_SpawnBurst(play, &this->dyna.actor.world.pos, 30.0f, 0, 25, 5, 40, OBJECT_SPOT02_OBJECTS, 20,
    ...
    if (play->csCtx.frames == 402) {
        if (!LINK_IS_ADULT) {
            OoT_Player_PlaySfx(&player->actor, NA_SE_VO_LI_DEMO_DAMAGE_KID);
        } else {
            OoT_Player_PlaySfx(&player->actor, NA_SE_VO_LI_DEMO_DAMAGE);
        }
    }
}
...
void func_808ACB58(BgSpot02Objects* this, PlayState* play) {
    if (OoT_Math_StepToF(&this->dyna.actor.world.pos.y, this->dyna.actor.home.pos.y + 255.0f, 1.0f)) {
...
void func_808AD450(Actor* thisx, PlayState* play) {
    ...
    if (play->csCtx.state != 0 && play->csCtx.npcActions[2] != NULL) {
        ...
        lerp = OoT_Environment_LerpWeight(play->csCtx.npcActions[2]->endFrame, play->csCtx.npcActions[2]->startFrame,
                                      play->csCtx.frames);

        // should be able to remove & 0xFFFF with some other change
        if ((play->csCtx.npcActions[2]->action & 0xFFFF) == 2) {
            OoT_Matrix_Translate(play->csCtx.npcActions[2]->startPos.x, play->csCtx.npcActions[2]->startPos.y,
            ...
            OoT_Matrix_Scale(0.9f, 0.9f, (((this->unk_170 - this->unk_172) * lerp) + this->unk_172) * 0.1f, MTXMODE_APPLY);
            Gfx_SetupDL_25Xlu(play->state.gfxCtx);
            ...
            gSPSegment(POLY_XLU_DISP++, 0x08,
                       OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 2 * this->timer, -3 * this->timer, 32, 64, 1,
