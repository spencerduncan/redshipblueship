...
void func_80AE72D0(EnRl* this) {
    ...
    if (DECR(*timer) == 0) {
        *timer = OoT_Rand_S16Offset(60, 60);
    }
...
void func_80AE7358(EnRl* this) {
    OoT_Animation_Change(&this->skelAnime, &object_rl_Anim_000A3C, 1.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&object_rl_Anim_000A3C), ANIMMODE_LOOP, 0.0f);
    this->action = 4;
    this->drawConfig = 0;
    this->alpha = 0;
...
void func_80AE7544(EnRl* this, PlayState* play) {
    OoT_SkelAnime_InitFlex(play, &this->skelAnime, &object_rl_Skel_007B38, &object_rl_Anim_000A3C, NULL, NULL, 0);
}
...
void func_80AE7590(EnRl* this, PlayState* play) {
    ...
    if (gSaveContext.sceneSetupIndex == 4 && sceneNum == SCENE_CHAMBER_OF_THE_SAGES &&
        ...
        OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_DEMO_EFFECT, pos.x, pos.y, pos.z, 0, 0, 0, 0xE, true);
        if (GameInteractor_Should(VB_GIVE_ITEM_LIGHT_MEDALLION, true)) {
            OoT_Item_Give(play, ITEM_MEDALLION_LIGHT);
        }
...
void func_80AE7698(EnRl* this, PlayState* play) {
    ...
    if (play->csCtx.state != CS_STATE_IDLE) {
        ...
        if (csCmdActorAction != NULL && csCmdActorAction->action == 3) {
            OoT_Animation_Change(&this->skelAnime, &object_rl_Anim_00040C, 1.0f, 0.0f,
                             OoT_Animation_GetLastFrame(&object_rl_Anim_00040C), ANIMMODE_ONCE, 0.0f);
            this->action = 2;
        }
    }
}
...
void func_80AE772C(EnRl* this, s32 arg1) {
    if (arg1) {
        OoT_Animation_Change(&this->skelAnime, &object_rl_Anim_000830, 1.0f, 0.0f,
                         OoT_Animation_GetLastFrame(&object_rl_Anim_000830), ANIMMODE_LOOP, 0.0f);
        this->action = 3;
    }
}
...
void func_80AE7AF8(EnRl* this, PlayState* play) {
    if (func_80AE74B4(this, play, 3, 0)) {
        OoT_Animation_Change(&this->skelAnime, &object_rl_Anim_00040C, 1.0f, 0.0f,
                         OoT_Animation_GetLastFrame(&object_rl_Anim_00040C), ANIMMODE_ONCE, -8.0f);
        this->action = 6;
...
static EnRlActionFunc OoT_sActionFuncs[] = {
...
void EnRl_Update(Actor* thisx, PlayState* play) {
    ...
    if ((this->action < 0) || (this->action > 7) || (OoT_sActionFuncs[this->action] == NULL)) {
...
static EnRlDrawFunc OoT_sDrawFuncs[] = {
    func_80AE7FD0,
    func_80AE7FDC,
    func_80AE7D94,
};
