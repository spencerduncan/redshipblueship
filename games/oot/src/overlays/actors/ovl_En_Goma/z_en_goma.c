...
void EnGoma_Init(Actor* thisx, PlayState* play) {
    ...
    this->eggTimer = OoT_Rand_ZeroOne() * 200.0f;
    ...
    if (params >= 100) { // piece of boss goma
        ...
        this->eggScale = 1.0f;
        this->actor.velocity.y = OoT_Rand_ZeroOne() * 5.0f + 5.0f;
        this->actionFunc = EnGoma_Debris;
        this->actor.speedXZ = OoT_Rand_ZeroOne() * 2.3f + 1.5f;
        this->actionTimer = 30;
        this->actor.scale.x = OoT_Rand_ZeroOne() * 0.005f + 0.01f;
        this->actor.scale.y = OoT_Rand_ZeroOne() * 0.005f + 0.01f;
        this->actor.scale.z = OoT_Rand_ZeroOne() * 0.005f + 0.01f;
        OoT_ActorShape_Init(&this->actor.shape, 0.0f, OoT_ActorShadow_DrawCircle, 0.0f);
    } else { // Egg
        OoT_ActorShape_Init(&this->actor.shape, 0.0f, OoT_ActorShadow_DrawCircle, 40.0f);
        OoT_SkelAnime_Init(play, &this->skelanime, &gObjectGolSkel, &gObjectGolStandAnim, this->jointTable,
                       this->morphTable, GOMA_LIMB_MAX);
        OoT_Animation_PlayLoop(&this->skelanime, &gObjectGolStandAnim);
        ...
        this->eggScale = 1.0f;
        this->eggSquishAngle = OoT_Rand_ZeroOne() * 1000.0f;
        this->actionTimer = 50;
        OoT_Collider_InitCylinder(play, &this->colCyl1);
        OoT_Collider_SetCylinder(play, &this->colCyl1, &this->actor, &D_80A4B7A0);
        OoT_Collider_InitCylinder(play, &this->colCyl2);
        OoT_Collider_SetCylinder(play, &this->colCyl2, &this->actor, &D_80A4B7CC);
    }
}
...
void EnGoma_Destroy(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params < 10) {
        OoT_Collider_DestroyCylinder(play, &this->colCyl1);
        OoT_Collider_DestroyCylinder(play, &this->colCyl2);
...
void EnGoma_SetupFlee(EnGoma* this) {
    OoT_Animation_Change(&this->skelanime, &gObjectGolRunningAnim, 2.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gObjectGolRunningAnim), ANIMMODE_LOOP, -2.0f);
...
void EnGoma_Flee(EnGoma* this, PlayState* play) {
    ...
    OoT_Math_ApproachS(&this->actor.world.rot.y, OoT_Actor_WorldYawTowardActor(&this->actor, &GET_PLAYER(play)->actor) + 0x8000,
                   3, 2000);
    OoT_Math_ApproachS(&this->actor.shape.rot.y, this->actor.world.rot.y, 2, 3000);
...
void EnGoma_EggFallToGround(EnGoma* this, PlayState* play) {
    ...
    OoT_Math_ApproachZeroF(&this->eggSquishAmount, 1.0f, 0.005f);
    OoT_Math_ApproachF(&this->eggYOffset, 1500.0f, 1.0f, 150.0f);
    ...
    switch (this->hatchState) {
        case 0:
            if (this->actor.bgCheckFlags & 1) { // floor
                ...
                if (this->actor.params > 5) {
                    ...
                    this->hatchState = 1;
                    this->actionTimer = 3;
                    OoT_Math_ApproachF(&this->eggScale, 1.5f, 0.5f, 1.0f);
                }
            }
            break;
        ...
        case 1:
            if (this->actionTimer == 0) {
                this->hatchState = 2;
                this->actionTimer = 3;
                OoT_Math_ApproachF(&this->eggScale, 0.75f, 0.5f, 1.0f);
        ...
        case 3:
            OoT_Math_ApproachF(&this->eggScale, 1.0f, 0.1f, 0.1f);
            if (this->actionTimer == 0) {
                EnGoma_SetupHatch(this, play);
            }
            break;
    ...
    if (this->actor.bgCheckFlags & 1) {
        OoT_Math_ApproachZeroF(&this->actor.speedXZ, 0.2f, 0.05f);
    }
...
void EnGoma_Egg(EnGoma* this, PlayState* play) {
    ...
    OoT_Math_ApproachF(&this->eggSquishAmount, 0.1f, 1.0f, 0.005f);
    ...
    if (!(this->eggTimer & 0xF) && OoT_Rand_ZeroOne() < 0.5f) {
        for (i = 0; i < 2; i++) {
            Vec3f vel = { 0.0f, 0.0f, 0.0f };
            Vec3f acc = { 0.0f, -0.5f, 0.0f };
            Vec3f pos;

            pos.x = OoT_Rand_CenteredFloat(30.0f) + this->actor.world.pos.x;
            pos.y = OoT_Rand_ZeroFloat(30.0f) + this->actor.world.pos.y;
            pos.z = OoT_Rand_CenteredFloat(30.0f) + this->actor.world.pos.z;
            OoT_EffectSsHahen_Spawn(play, &pos, &vel, &acc, 0, (s16)(OoT_Rand_ZeroOne() * 5.0f) + 10, HAHEN_OBJECT_DEFAULT, 10,
                                NULL);
        }
    }
}
...
void EnGoma_SetupHatch(EnGoma* this, PlayState* play) {
    OoT_Animation_Change(&this->skelanime, &gObjectGolJumpHeadbuttAnim, 1.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gObjectGolJumpHeadbuttAnim), ANIMMODE_ONCE, 0.0f);
...
void EnGoma_SetupHurt(EnGoma* this, PlayState* play) {
    OoT_Animation_Change(&this->skelanime, &gObjectGolDamagedAnim, 1.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gObjectGolDamagedAnim), ANIMMODE_ONCE, -2.0f);
...
void EnGoma_SetupDie(EnGoma* this) {
    OoT_Animation_Change(&this->skelanime, &gObjectGolDeathAnim, 1.0f, 0.0f, OoT_Animation_GetLastFrame(&gObjectGolDeathAnim),
                     ANIMMODE_ONCE, -2.0f);
...
void EnGoma_SetupDead(EnGoma* this) {
    OoT_Animation_Change(&this->skelanime, &gObjectGolDeadTwitchingAnim, 1.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gObjectGolDeadTwitchingAnim), ANIMMODE_LOOP, -2.0f);
...
void EnGoma_Dead(EnGoma* this, PlayState* play) {
    ...
    if (this->actionTimer == 2) {
        ...
        accel.y = 0.03f;
        OoT_EffectSsKFire_Spawn(play, &pos, &sDeadEffectVel, &accel, 40, 0);
    }

    if (this->actionTimer == 0 && OoT_Math_SmoothStepToF(&this->actor.scale.y, 0.0f, 0.5f, 0.00225f, 0.00001f) <= 0.001f) {
        if (this->actor.params < 6) {
            BossGoma* parent = (BossGoma*)this->actor.parent;

            parent->childrenGohmaState[this->actor.params] = -1;
        }
        Audio_PlaySoundGeneral(NA_SE_EN_EXTINCT, &this->actor.projectedPos, 4, &OoT_gSfxDefaultFreqAndVolScale,
                               &OoT_gSfxDefaultFreqAndVolScale, &OoT_gSfxDefaultReverb);
        OoT_Actor_Kill(&this->actor);
        OoT_Item_DropCollectibleRandom(play, NULL, &this->actor.world.pos, 0x30);
    }
    this->visualState = 2;
}
...
void EnGoma_SetupStand(EnGoma* this) {
    ...
    lastFrame = OoT_Animation_GetLastFrame(&gObjectGolStandAnim);
    this->actionTimer = OoT_Rand_S16Offset(10, 30);
    OoT_Animation_Change(&this->skelanime, &gObjectGolStandAnim, 1.0f, 0.0f, lastFrame, ANIMMODE_LOOP, -5.0f);
    this->actionFunc = EnGoma_Stand;
    this->gomaType = ENGOMA_NORMAL;
}

void EnGoma_SetupChasePlayer(EnGoma* this) {
    OoT_Animation_Change(&this->skelanime, &gObjectGolRunningAnim, 1.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gObjectGolRunningAnim), ANIMMODE_LOOP, -5.0f);
    this->actionFunc = EnGoma_ChasePlayer;
    this->actionTimer = OoT_Rand_S16Offset(70, 110);
}

void EnGoma_SetupPrepareJump(EnGoma* this) {
    OoT_Animation_Change(&this->skelanime, &gObjectGolPrepareJumpAnim, 1.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gObjectGolPrepareJumpAnim), ANIMMODE_ONCE, -5.0f);
...
void EnGoma_PrepareJump(EnGoma* this, PlayState* play) {
    ...
    targetAngle = OoT_Actor_WorldYawTowardActor(&this->actor, &GET_PLAYER(play)->actor);
    OoT_Math_ApproachS(&this->actor.world.rot.y, targetAngle, 2, 4000);
    OoT_Math_ApproachS(&this->actor.shape.rot.y, targetAngle, 2, 3000);
...
void EnGoma_SetupLand(EnGoma* this) {
    OoT_Animation_Change(&this->skelanime, &gObjectGolLandFromJumpAnim, 1.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gObjectGolLandFromJumpAnim), ANIMMODE_ONCE, 0.0f);
...
void EnGoma_Jump(EnGoma* this, PlayState* play) {
    ...
    OoT_SkelAnime_Update(&this->skelanime);
    OoT_Math_ApproachF(&this->actor.speedXZ, 10.0f, 0.5f, 5.0f);
...
void EnGoma_Stand(EnGoma* this, PlayState* play) {
    OoT_SkelAnime_Update(&this->skelanime);
    OoT_Math_ApproachZeroF(&this->actor.speedXZ, 0.5f, 2.0f);
    OoT_Math_ApproachS(&this->actor.shape.rot.y, OoT_Actor_WorldYawTowardActor(&this->actor, &GET_PLAYER(play)->actor), 2,
                   3000);
...
void EnGoma_ChasePlayer(EnGoma* this, PlayState* play) {
    ...
    if (OoT_Animation_OnFrame(&this->skelanime, 1.0f) || OoT_Animation_OnFrame(&this->skelanime, 5.0f)) {
        if (this->actor.params < 6) {
    ...
    OoT_Math_ApproachS(&this->actor.world.rot.y, this->actor.yawTowardsPlayer, 3, 2000);
    OoT_Math_ApproachS(&this->actor.shape.rot.y, this->actor.world.rot.y, 2, 3000);
...
void EnGoma_SetupStunned(EnGoma* this, PlayState* play) {
    ...
    this->stunTimer = 100;
    OoT_Animation_MorphToLoop(&this->skelanime, &gObjectGolStandAnim, -5.0f);
    this->actionTimer = (s16)OoT_Rand_ZeroFloat(15.0f) + 3;
...
void EnGoma_Stunned(EnGoma* this, PlayState* play) {
    OoT_Actor_SetColorFilter(&this->actor, 0, 180, 0, 2);
    this->visualState = 2;
...
void EnGoma_LookAtPlayer(EnGoma* this, PlayState* play) {
    ...
    eyeYaw = OoT_Actor_WorldYawTowardActor(&this->actor, &GET_PLAYER(play)->actor) - this->actor.shape.rot.y;
    eyePitch = OoT_Actor_WorldPitchTowardActor(&this->actor, &GET_PLAYER(play)->actor) - this->actor.shape.rot.x;
    ...
    OoT_Math_ApproachS(&this->eyeYaw, eyeYaw, 3, 2000);
    OoT_Math_ApproachS(&this->eyePitch, eyePitch, 3, 2000);
}
            ...
            if (this->gomaType == ENGOMA_NORMAL) {
                ...
                if (dmgFlags & 0x100000) {
                    if (this->actionFunc == EnGoma_Jump) {
                        ...
                        OoT_Matrix_MultVec3f(&sShieldKnockbackVel, &this->shieldKnockbackVel);
                    ...
                    OoT_Actor_SetColorFilter(&this->actor, 0x4000, 255, 0, 5);
                    this->hurtTimer = 13;
                }
            } else {
                // die if still an egg
...
void EnGoma_UpdateEyeEnvColor(EnGoma* this) {
    ...
    OoT_Math_ApproachF(&this->eyeEnvColor[0], sTargetEyeEnvColors[0][this->visualState], 0.5f, 20.0f);
    OoT_Math_ApproachF(&this->eyeEnvColor[1], sTargetEyeEnvColors[1][this->visualState], 0.5f, 20.0f);
    OoT_Math_ApproachF(&this->eyeEnvColor[2], sTargetEyeEnvColors[2][this->visualState], 0.5f, 20.0f);
}
...
void EnGoma_SetFloorRot(EnGoma* this) {
    ...
    if (this->actor.floorPoly != NULL) {
        ...
        OoT_Math_ApproachS(&this->slopePitch, -OoT_Math_FAtan2F(-nz * ny, 1.0f) * (0x8000 / M_PI), 1, 1000);
        OoT_Math_ApproachS(&this->slopeRoll, OoT_Math_FAtan2F(-nx * ny, 1.0f) * (0x8000 / M_PI), 1, 1000);
    }
}
...
void EnGoma_Update(Actor* thisx, PlayState* play) {
    ...
    OoT_Math_ApproachZeroF(&this->shieldKnockbackVel.x, 1.0f, 3.0f);
    OoT_Math_ApproachZeroF(&this->shieldKnockbackVel.z, 1.0f, 3.0f);

    if (this->actor.params < 10) {
        this->eggTimer++;
        OoT_Math_SmoothStepToF(&this->actor.scale.x, 0.01f, 0.5f, 0.00075f, 0.000001f);
        OoT_Math_SmoothStepToF(&this->actor.scale.y, 0.01f, 0.5f, 0.00075f, 0.000001f);
        OoT_Math_SmoothStepToF(&this->actor.scale.z, 0.01f, 0.5f, 0.00075f, 0.000001f);
        EnGoma_UpdateHit(this, play);
        OoT_Actor_UpdateBgCheckInfo(play, &this->actor, 50.0f, 50.0f, 100.0f, 5);
        ...
        if (this->invincibilityTimer == 0) {
            OoT_Collider_UpdateCylinder(&this->actor, &this->colCyl1);
            OoT_Collider_UpdateCylinder(&this->actor, &this->colCyl2);
            OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->colCyl1.base);
            OoT_CollisionCheck_SetAC(play, &play->colChkCtx, &this->colCyl2.base);
            OoT_CollisionCheck_SetAT(play, &play->colChkCtx, &this->colCyl1.base);
        }
    }
}
...
s32 EnGoma_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == GOMA_LIMB_EYE_IRIS_ROOT1) {
        ...
        gDPSetEnvColor(POLY_OPA_DISP++, (s16)(OoT_Rand_ZeroOne() * 255.0f), (s16)(OoT_Rand_ZeroOne() * 255.0f),
                       (s16)(OoT_Rand_ZeroOne() * 255.0f), 255);
    }
...
void EnGoma_Draw(Actor* thisx, PlayState* play) {
    ...
    switch (this->gomaType) {
        case ENGOMA_NORMAL:
            ...
            OoT_Matrix_Translate(this->actor.world.pos.x,
        ...
        case ENGOMA_EGG:
            ...
            OoT_Matrix_Push();
            OoT_Matrix_Scale(this->eggScale, 1.0f / this->eggScale, this->eggScale, MTXMODE_APPLY);
            Matrix_RotateY(this->eggSquishAngle * 0.15f, MTXMODE_APPLY);
            Matrix_RotateZ(this->eggSquishAngle * 0.1f, MTXMODE_APPLY);
            OoT_Matrix_Scale(0.95f - this->eggSquishAmount, this->eggSquishAmount + 1.05f, 0.95f - this->eggSquishAmount,
                         MTXMODE_APPLY);
            Matrix_RotateZ(-(this->eggSquishAngle * 0.1f), MTXMODE_APPLY);
            Matrix_RotateY(-(this->eggSquishAngle * 0.15f), MTXMODE_APPLY);
            OoT_Matrix_Translate(0.0f, this->eggYOffset, 0.0f, MTXMODE_APPLY);
...
void EnGoma_SpawnHatchDebris(EnGoma* this, PlayState* play2) {
    ...
    if (this->actor.params < 6) {
        OoT_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 40, NA_SE_EN_GOMA_BJR_EGG2);
    } else {
        OoT_SoundSource_PlaySfxAtFixedWorldPos(play, &this->actor.world.pos, 40, NA_SE_EN_GOMA_EGG2);
    }

    for (i = 0; i < 15; i++) {
        OoT_Actor_SpawnAsChild(
            &play->actorCtx, &this->actor, play, ACTOR_EN_GOMA, OoT_Rand_CenteredFloat(10.0f) + this->actor.world.pos.x,
            OoT_Rand_CenteredFloat(10.0f) + this->actor.world.pos.y + 15.0f,
            OoT_Rand_CenteredFloat(10.0f) + this->actor.world.pos.z, 0, OoT_Rand_CenteredFloat(0x10000 - 0.01f), 0, i + 10);
    }
}
...
void EnGoma_BossLimb(EnGoma* this, PlayState* play) {
    ...
    if (this->actionTimer < 121) {
        if (OoT_Math_SmoothStepToF(&this->actor.scale.y, 0.0f, 1.0f, 0.00075f, 0) <= 0.001f) {
            OoT_Actor_Kill(&this->actor);
        }
