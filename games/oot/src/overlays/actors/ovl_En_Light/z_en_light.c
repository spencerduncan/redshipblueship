...
void OoT_EnLight_Init(Actor* thisx, PlayState* play);
void OoT_EnLight_Destroy(Actor* thisx, PlayState* play);
void OoT_EnLight_Update(Actor* thisx, PlayState* play);
void OoT_EnLight_Draw(Actor* thisx, PlayState* play);
...
const ActorInit En_Light_InitVars = {
    ...
    sizeof(EnLight),
    (ActorFunc)OoT_EnLight_Init,
    (ActorFunc)OoT_EnLight_Destroy,
    (ActorFunc)OoT_EnLight_Update,
    (ActorFunc)OoT_EnLight_Draw,
    NULL,
};
...
void OoT_EnLight_Init(Actor* thisx, PlayState* play) {
    ...
    if (gSaveContext.gameMode == GAMEMODE_END_CREDITS) {
        ...
        OoT_Lights_PointNoGlowSetInfo(&this->lightInfo, this->actor.world.pos.x, yOffset + (s16)this->actor.world.pos.y,
                                  this->actor.world.pos.z, 255, 255, 180, -1);
    } else {
        yOffset = (this->actor.params < 0) ? 1 : 40;
        OoT_Lights_PointGlowSetInfo(&this->lightInfo, this->actor.world.pos.x, yOffset + (s16)this->actor.world.pos.y,
                                this->actor.world.pos.z, 255, 255, 180, -1);
    }

    this->lightNode = OoT_LightContext_InsertLight(play, &play->lightCtx, &this->lightInfo);
    OoT_Actor_SetScale(&this->actor, D_80A9E840[this->actor.params & 0xF].scale * 0.0001f);
    this->timer = (s32)(OoT_Rand_ZeroOne() * 255.0f);
...
void EnLight_UpdatePosRot(EnLight* this, PlayState* play) {
    ...
    if (this->actor.parent != NULL) {
        OoT_Math_Vec3f_Copy(&this->actor.world.pos, &(this->actor.parent)->world.pos);
...
void OoT_EnLight_Update(Actor* thisx, PlayState* play) {
    ...
    intensity = (OoT_Rand_ZeroOne() * 0.5f) + 0.5f;
    radius = (this->actor.params < 0) ? 100 : 300;
    OoT_Lights_PointSetColorAndRadius(&this->lightInfo, (flameParams->primColor.r * intensity),
...
void EnLight_UpdateSwitch(Actor* thisx, PlayState* play) {
    ...
    if ((this->actor.params & 0x800) != 0) {
        if (OoT_Flags_GetSwitch(play, (this->actor.params & 0x3F0) >> 4)) {
            OoT_Math_StepToF(&scale, 1.0f, 0.05f);
        } else {
            if (scale < 0.1f) {
                OoT_Actor_SetScale(&this->actor, 0.0f);
                return;
            }
            OoT_Math_StepToF(&scale, 0.0f, 0.05f);
        }
    } else {
        if (OoT_Flags_GetSwitch(play, (this->actor.params & 0x3F0) >> 4)) {
            if (scale < 0.1f) {
    ...
    OoT_Actor_SetScale(&this->actor, ((f32)flameParams->scale * 0.0001) * scale);
    intensity = (OoT_Rand_ZeroOne() * 0.5f) + 0.5f;
    OoT_Lights_PointSetColorAndRadius(&this->lightInfo, (flameParams->primColor.r * intensity),
                                  (flameParams->primColor.g * intensity), (flameParams->primColor.b * intensity),
                                  300.0f * scale);
...
void OoT_EnLight_Draw(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.params >= 0) {
        gSPSegment(POLY_XLU_DISP++, 0x08,
                   OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0, 0, 32, 64, 1, 0, (this->timer * -20) & 511, 32, 128, 0,
                                      0, 0, -20));
        ...
        gSPSegment(POLY_XLU_DISP++, 0x08,
                   OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0, 0, 16, 32, 1, ((this->timer * 2) & 63),
