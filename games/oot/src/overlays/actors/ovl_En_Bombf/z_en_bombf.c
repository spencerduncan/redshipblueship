...
void OoT_EnBombf_Init(Actor* thisx, PlayState* play);
void OoT_EnBombf_Destroy(Actor* thisx, PlayState* play);
void OoT_EnBombf_Update(Actor* thisx, PlayState* play);
void OoT_EnBombf_Draw(Actor* thisx, PlayState* play);
...
const ActorInit En_Bombf_InitVars = {
    ...
    OBJECT_BOMBF,
    sizeof(EnBombf),
    (ActorFunc)OoT_EnBombf_Init,
    (ActorFunc)OoT_EnBombf_Destroy,
    (ActorFunc)OoT_EnBombf_Update,
    (ActorFunc)OoT_EnBombf_Draw,
    NULL,
};
...
void OoT_EnBombf_SetupAction(EnBombf* this, EnBombfActionFunc actionFunc) {
...
void OoT_EnBombf_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_Collider_InitCylinder(play, &this->bombCollider);
    OoT_Collider_InitJntSph(play, &this->explosionCollider);
    OoT_Collider_SetCylinder(play, &this->bombCollider, thisx, &OoT_sCylinderInit);
    OoT_Collider_SetJntSph(play, &this->explosionCollider, thisx, &OoT_sJntSphInit, &this->explosionColliderItems[0]);

    if (thisx->params == BOMBFLOWER_BODY) {
        shapeUnk10 = 1000.0f;
    }

    OoT_ActorShape_Init(&thisx->shape, shapeUnk10, OoT_ActorShadow_DrawCircle, 12.0f);
    thisx->focus.pos = thisx->world.pos;

    if (OoT_Actor_FindNearby(play, thisx, ACTOR_BG_DDAN_KD, ACTORCAT_BG, 10000.0f) != NULL) {
    ...
    if (thisx->params == BOMBFLOWER_BODY) {
        ...
        thisx->gravity = -1.5f;
        OoT_Actor_ChangeCategory(play, &play->actorCtx, thisx, ACTORCAT_EXPLOSIVE);
        thisx->colChkInfo.mass = 200;
        thisx->flags &= ~ACTOR_FLAG_ATTENTION_ENABLED;
        OoT_EnBombf_SetupAction(this, EnBombf_Move);
    } else {
...
void OoT_EnBombf_Destroy(Actor* thisx, PlayState* play) {
    ...
    OoT_Collider_DestroyCylinder(play, &this->bombCollider);
    OoT_Collider_DestroyJntSph(play, &this->explosionCollider);
}

void EnBombf_SetupGrowBomb(EnBombf* this, s16 params) {
    OoT_EnBombf_SetupAction(this, EnBombf_GrowBomb);
}
...
void EnBombf_GrowBomb(EnBombf* this, PlayState* play) {
    ...
    if (this->flowerBombScale >= 1.0f) {
        if (OoT_Actor_HasParent(&this->actor, play)) {
            bombFlower = (EnBombf*)OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_BOMBF, this->actor.world.pos.x,
            ...
            if (OoT_Player_IsBurningStickInRange(play, &this->actor.world.pos, 30.0f, 50.0f)) {
                bombFlower = (EnBombf*)OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_BOMBF, this->actor.world.pos.x,
                                                   this->actor.world.pos.y, this->actor.world.pos.z, 0, 0, 0, 0, true);
                if (bombFlower != NULL) {
...
void EnBombf_Move(EnBombf* this, PlayState* play) {
    if (OoT_Actor_HasParent(&this->actor, play)) {
        ...
        this->flowerBombScale = 0.0f;
        OoT_EnBombf_SetupAction(this, EnBombf_WaitForRelease);
        this->actor.room = -1;
        return;
    }
    ...
    if (!(this->actor.bgCheckFlags & 1)) {
        OoT_Math_SmoothStepToF(&this->actor.speedXZ, 0.0f, 1.0f, 0.025f, 0.0f);
    } else {
...
void EnBombf_Explode(EnBombf* this, PlayState* play) {
    ...
    if (this->actor.params == BOMBFLOWER_EXPLOSION) {
        OoT_CollisionCheck_SetAT(play, &play->colChkCtx, &this->explosionCollider.base);
    }
...
void OoT_EnBombf_Update(Actor* thisx, PlayState* play) {
    ...
    if ((!this->bumpOn) && (!OoT_Actor_HasParent(thisx, play)) &&
    ...
    if (thisx->gravity != 0.0f) {
        DREG(6) = 1;
        OoT_Actor_UpdateBgCheckInfo(play, thisx, 5.0f, 10.0f, 0.0f, 0x1F);
        DREG(6) = 0;
    }
    ...
    if (thisx->params == BOMBFLOWER_BODY) {
        ...
        if ((this->bombCollider.base.acFlags & AC_HIT) || ((this->bombCollider.base.ocFlags1 & OC1_HIT) &&
            ...
            if ((this->timer > 100) && OoT_Player_IsBurningStickInRange(play, &thisx->world.pos, 30.0f, 50.0f)) {
                this->timer = 100;
            }
        }
        ...
        if (this->isFuseEnabled != 0) {
            ...
            if (this->timer < 127) {
                ...
                if ((play->gameplayFrames % 2) == 0) {
                    OoT_EffectSsGSpk_SpawnFuse(play, thisx, &effPos, &effVelocity, &effAccel);
                }
            ...
            if ((this->timer < 100) && ((this->timer & (this->flashSpeedScale + 1)) != 0)) {
                OoT_Math_SmoothStepToF(&this->flashIntensity, 150.0f, 1.0f, 150.0f / this->flashSpeedScale, 0.0f);
            } else {
                OoT_Math_SmoothStepToF(&this->flashIntensity, 0.0f, 1.0f, 150.0f / this->flashSpeedScale, 0.0f);
            }

            if (this->timer < 3) {
                OoT_Actor_SetScale(thisx, thisx->scale.x + 0.002f);
            }
            ...
            if (this->timer == 0) {
                ...
                OoT_EffectSsBomb2_SpawnLayered(play, &effPos, &effVelocity, &bomb2Accel, 100, 19);

                effPos.y = thisx->floorHeight;
                if (thisx->floorHeight > BGCHECK_Y_MIN) {
                    OoT_EffectSsBlast_SpawnWhiteShockwave(play, &effPos, &effVelocity, &effAccel);
                }
                ...
                play->envCtx.adjAmbientColor[0] = play->envCtx.adjAmbientColor[1] = play->envCtx.adjAmbientColor[2] =
                    250;
                OoT_Camera_AddQuake(&play->mainCamera, 2, 0xB, 8);
                thisx->params = BOMBFLOWER_EXPLOSION;
                this->timer = 10;
                thisx->flags |= ACTOR_FLAG_DRAW_CULLING_DISABLED;
                OoT_EnBombf_SetupAction(this, EnBombf_Explode);
            }
        }
    }
    ...
    if (thisx->params <= BOMBFLOWER_BODY) {
        ...
        OoT_Collider_UpdateCylinder(thisx, &this->bombCollider);

        if ((this->flowerBombScale >= 1.0f) && (this->bumpOn)) {
            OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->bombCollider.base);
        }

        OoT_CollisionCheck_SetAC(play, &play->colChkCtx, &this->bombCollider.base);
    }
...
void OoT_EnBombf_Draw(Actor* thisx, PlayState* play) {
    ...
    if (thisx->params <= BOMBFLOWER_BODY) {
        ...
        if (thisx->params != BOMBFLOWER_BODY) {
            ...
            OoT_Matrix_Scale(this->flowerBombScale, this->flowerBombScale, this->flowerBombScale, MTXMODE_APPLY);
        }
        ...
        OoT_Collider_UpdateSpheres(0, &this->explosionCollider);
    }
