...
#define AT_FINISH_LINE(actor)                                                                                     \
    (OoT_Math3D_PointInSquare2D(sFinishLine[0], sFinishLine[1], sFinishLine[2], sFinishLine[3], (actor)->world.pos.x, \
                            (actor)->world.pos.z))
#define AT_RANCH_EXIT(actor)                                                                                  \
    (OoT_Math3D_PointInSquare2D(sRanchExit[0], sRanchExit[1], sRanchExit[2], sRanchExit[3], (actor)->world.pos.x, \
                            (actor)->world.pos.z))
...
void OoT_EnHorseGameCheck_Init(Actor* thisx, PlayState* play);
void OoT_EnHorseGameCheck_Destroy(Actor* thisx, PlayState* play);
void OoT_EnHorseGameCheck_Update(Actor* thisx, PlayState* play);
void OoT_EnHorseGameCheck_Draw(Actor* thisx, PlayState* play);
...
const ActorInit En_Horse_Game_Check_InitVars = {
    ...
    (ActorFunc)OoT_EnHorseGameCheck_Init,
    (ActorFunc)OoT_EnHorseGameCheck_Destroy,
    (ActorFunc)OoT_EnHorseGameCheck_Update,
    (ActorFunc)OoT_EnHorseGameCheck_Draw,
    NULL,
};
...
s32 EnHorseGameCheck_InitIngoRace(EnHorseGameCheckBase* base, PlayState* play) {
    ...
    this->ingoHorse =
        OoT_Actor_Spawn(&play->actorCtx, play, ACTOR_EN_HORSE, -250.0f, 1.0f, -1650.0f, 0, 0x4000, 0, 0x8003, true);
...
void EnHorseGameCheck_FinishIngoRace(EnHorseGameCheckIngoRace* this, PlayState* play) {
    ...
    if (this->result == INGORACE_PLAYER_WIN) {
        ...
        if (gSaveContext.eventInf[0] & 0x40) {
            ...
            OoT_Environment_ForcePlaySequence(NA_BGM_INGO);
        } else {
...
s32 EnHorseGameCheck_UpdateIngoRace(EnHorseGameCheckBase* base, PlayState* play) {
    ...
    if ((this->startTimer > 50) && !(this->startFlags & INGORACE_SET_TIMER)) {
        ...
        Audio_PlaySoundGeneral(NA_SE_SY_START_SHOT, &OoT_gSfxDefaultPos, 4, &OoT_gSfxDefaultFreqAndVolScale,
    ...
    for (i = 0; i < 3; i++) {
        if ((player->rideActor != NULL) &&
            (OoT_Math3D_Vec3f_DistXYZ(&sIngoRaceCheckpoints[i], &player->rideActor->world.pos) < 400.0f)) {
        ...
        if (OoT_Math3D_Vec3f_DistXYZ(&sIngoRaceCheckpoints[i], &this->ingoHorse->world.pos) < 400.0f) {
...
s32 EnHorseGameCheck_UpdateMalonRace(EnHorseGameCheckBase* base, PlayState* play) {
    ...
    if (this->result == MALONRACE_NO_RESULT) {
        ...
        for (i = 0; i < 16; i++) {
            ...
            dist = OoT_Math_Vec3f_DistXZ(&sFencePos[i % 8], &player2->rideActor->world.pos);
            if ((player->rideActor != NULL) && (dist < 250.0f)) {
                ...
                if (horse->stateFlags & ENHORSE_JUMPING) {
                    ...
                    if ((this->fenceCheck[i - 1] == 0) && !(this->raceFlags & MALONRACE_BROKE_RULE)) {
                        ...
                        OoT_Message_StartTextbox(play, 0x208C, NULL);
                        this->result = 4;
                        this->finishTimer = 30;
                    }
                }
        ...
        if ((player2->rideActor != NULL) && (this->raceFlags & MALONRACE_PLAYER_START) &&
            ...
            if ((this->lapCount == 1) && (this->fenceCheck[15] == 0) && (player2->rideActor->prevPos.x < -200.0f)) {
                ...
                OoT_Message_StartTextbox(play, 0x208D, NULL);
...
static EnHorseGameCheckFunc OoT_sInitFuncs[] = {
    NULL,
...
static EnHorseGameCheckFunc OoT_sDestroyFuncs[] = {
    NULL,
...
static EnHorseGameCheckFunc OoT_sUpdateFuncs[] = {
    NULL,
...
void OoT_EnHorseGameCheck_Init(Actor* thisx, PlayState* play) {
    ...
    if ((play->sceneNum == SCENE_LON_LON_RANCH) && (OoT_Flags_GetEventChkInf(EVENTCHKINF_EPONA_OBTAINED) || DREG(1))) {
        this->actor.params = HORSEGAME_MALON_RACE;
    }
    if (OoT_sInitFuncs[this->actor.params] != NULL) {
...
void OoT_EnHorseGameCheck_Destroy(Actor* thisx, PlayState* play) {
    ...
    if (OoT_sDestroyFuncs[this->actor.params] != NULL) {
        OoT_sDestroyFuncs[this->actor.params](this, play);
    }
}
...
void OoT_EnHorseGameCheck_Update(Actor* thisx, PlayState* play) {
    ...
    if (OoT_sUpdateFuncs[this->type] != NULL) {
        OoT_sUpdateFuncs[this->type](this, play);
    }
}
