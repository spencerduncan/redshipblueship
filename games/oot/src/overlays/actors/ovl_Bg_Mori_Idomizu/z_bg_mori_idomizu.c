...
void BgMoriIdomizu_Init(Actor* thisx, PlayState* play) {
    ...
    this->prevSwitchFlagSet = OoT_Flags_GetSwitch(play, this->actor.params & 0x3F);
...
void BgMoriIdomizu_Main(BgMoriIdomizu* this, PlayState* play) {
    ...
    switchFlagSet = OoT_Flags_GetSwitch(play, thisx->params & 0x3F);
    if (switchFlagSet) {
    ...
    if ((roomNum == 7) || (roomNum == 8) || (roomNum == 9)) {
        if (this->drainTimer < 70) {
            OoT_Math_StepToF(&thisx->world.pos.y, this->targetWaterLevel, 3.5f);
...
void BgMoriIdomizu_Draw(Actor* thisx, PlayState* play) {
    ...
    gSPSegment(POLY_XLU_DISP++, 0x09,
               OoT_Gfx_TwoTexScrollEx(play->state.gfxCtx, 0, 0x7F - (gameplayFrames & 0x7F), gameplayFrames % 0x80, 0x20,
