...
void EnWeiyer_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_ActorShape_Init(&this->actor.shape, 1000.0f, OoT_ActorShadow_DrawCircle, 65.0f);
...
void func_80B32494(EnWeiyer* this) {
    OoT_Animation_Change(&this->skelAnime, &gStingerPopOutAnim, 2.0f, 0.0f, 0.0f, ANIMMODE_LOOP, -8.0f);
    this->unk_194 = 40;
    this->collider.base.atFlags |= AT_ON;
...
void func_80B325A0(EnWeiyer* this) {
    OoT_Animation_Change(&this->skelAnime, &gStingerHitAnim, 2.0f, 0.0f, 0.0f, ANIMMODE_LOOP, -3.0f);
    this->unk_194 = 40;
    this->collider.base.atFlags &= ~AT_ON;
    ...
    OoT_Actor_SetColorFilter(&this->actor, 0x4000, 0xC8, 0, 0x28);
    this->collider.dim.height = OoT_sCylinderInit.dim.height;
...
void func_80B32660(EnWeiyer* this) {
    ...
    this->collider.dim.height = OoT_sCylinderInit.dim.height + 15;
...
void func_80B328E8(EnWeiyer* this, PlayState* play) {
    ...
    OoT_Math_ScaledStepToS(&this->actor.shape.rot.x, 0, 0x800);
    sp34 = OoT_Animation_OnFrame(&this->skelAnime, 0.0f);
    curFrame = this->skelAnime.curFrame;
    OoT_Math_StepToF(&this->unk_27C, this->unk_280, 0.5f);
    this->actor.world.pos.y = this->unk_27C - cosf((curFrame - 5.0f) * (M_PI / 40)) * 3.0f;

    if (curFrame <= 45.0f) {
        OoT_Math_StepToF(&this->actor.speedXZ, 1.0f, 0.03f);
    } else {
        OoT_Math_StepToF(&this->actor.speedXZ, 1.3f, 0.03f);
    }
    ...
    if (OoT_Math_ScaledStepToS(&this->actor.shape.rot.y, this->unk_196, 182)) {
        if (this->unk_194 != 0) {
            this->unk_194--;
        }
        ...
        if (this->unk_194 == 0) {
            this->unk_196 =
                OoT_Rand_S16Offset(0x2000, 0x2000) * ((OoT_Rand_ZeroOne() < 0.5f) ? -1 : 1) + this->actor.shape.rot.y;
            this->unk_194 = 30;

            if (OoT_Rand_ZeroOne() < 0.3333f) {
    ...
    if (this->actor.home.pos.y < this->actor.world.pos.y) {
        if (this->actor.home.pos.y < this->actor.floorHeight) {
            ...
            this->unk_280 =
                OoT_Rand_ZeroOne() * ((this->actor.home.pos.y - this->actor.floorHeight) / 2.0f) + this->actor.floorHeight;
        }
    } else {
        ...
        if (this->actor.bgCheckFlags & 1) {
            this->unk_280 =
                this->actor.home.pos.y - OoT_Rand_ZeroOne() * ((this->actor.home.pos.y - this->actor.floorHeight) / 2.0f);
        } else if (sp34 && (OoT_Rand_ZeroOne() < 0.1f)) {
            this->unk_280 =
                OoT_Rand_ZeroOne() * (this->actor.home.pos.y - this->actor.floorHeight) + this->actor.floorHeight;
        }
...
void func_80B32C2C(EnWeiyer* this, PlayState* play) {
    ...
    if (this->unk_194 == 0) {
        if (OoT_Math_ScaledStepToS(&this->actor.shape.rot.x, -0x4000, 0x800)) {
        ...
        if (this->actor.world.pos.y < this->actor.home.pos.y) {
            if (this->actor.shape.rot.x > 0) {
                OoT_EffectSsGSplash_Spawn(play, &this->actor.world.pos, NULL, NULL, 1, 400);
...
s16 func_80B32DEC(EnWeiyer* this, PlayState* play) {
    ...
    return OoT_Actor_WorldPitchTowardPoint(&this->actor, &vec);
}
...
void func_80B32E34(EnWeiyer* this, PlayState* play) {
    ...
    if ((this->unk_194 == 0) || ((this->actor.home.pos.y + 20.0f) <= player->actor.world.pos.y) ||
        ...
        if (OoT_Actor_IsFacingPlayer(&this->actor, 0x2800)) {
            OoT_Math_StepToF(&this->actor.speedXZ, 4.0f, 0.2f);
        } else {
            OoT_Math_StepToF(&this->actor.speedXZ, 1.3f, 0.2f);
        }
        ...
        if (this->actor.home.pos.y < this->actor.world.pos.y) {
            ...
            this->actor.world.pos.y = this->actor.home.pos.y;
            OoT_Math_SmoothStepToS(&this->actor.shape.rot.x, 0x1000, 2, 0x100, 0x40);
        } else {
            OoT_Math_SmoothStepToS(&this->actor.shape.rot.x, func_80B32DEC(this, play), 2, 0x100, 0x40);
        }

        OoT_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 2, 0x200, 0x80);

        if ((player->actor.yDistToWater < 50.0f) && (this->actor.yDistToWater < 20.0f) &&
            OoT_Actor_IsFacingPlayer(&this->actor, 0x2000)) {
            func_80B327D8(this);
        }
    }
}
...
void func_80B33018(EnWeiyer* this, PlayState* play) {
    ...
    OoT_Math_ScaledStepToS(&this->actor.shape.rot.x, 0, 0x800);
    curFrame = this->skelAnime.curFrame;
    OoT_Math_StepToF(&this->unk_27C, (this->actor.home.pos.y - this->actor.floorHeight) / 4.0f + this->actor.floorHeight,
                 1.0f);
    this->actor.world.pos.y = this->unk_27C - cosf((curFrame - 5.0f) * (M_PI / 40)) * 3.0f;
    ...
    if (OoT_Math_SmoothStepToS(&this->actor.shape.rot.y, this->unk_196, 2, 0x200, 0x80) == 0) {
        this->unk_196 = this->actor.yawTowardsPlayer + 0x8000;
    }
...
void func_80B331CC(EnWeiyer* this, PlayState* play) {
    ...
    OoT_Math_ScaledStepToS(&this->actor.world.rot.y, this->unk_196, 0x38E);
...
void func_80B3349C(EnWeiyer* this, PlayState* play) {
    ...
    if (this->unk_194 == -1) {
        ...
        if (this->actor.shape.rot.x < phi_a1) {
            OoT_Math_ScaledStepToS(&this->actor.shape.rot.x, phi_a1, 0x400);
        }
        ...
        if (this->actor.bgCheckFlags & 1) {
            ...
            OoT_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 8, 0x100, 0x80);
        }
    }
}
...
void EnWeiyer_Update(Actor* thisx, PlayState* play) {
    ...
    OoT_Actor_UpdateBgCheckInfo(play, &this->actor, 10.0f, 30.0f, 45.0f, 7);
