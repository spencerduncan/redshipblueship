...
void func_80A2F180(EnGb* this) {
    if (OoT_Flags_GetInfTable(INFTABLE_SPOKE_TO_POE_COLLECTOR_IN_RUINED_MARKET)) {
        this->textId = 0x70F5;
    } else {
        this->textId = 0x70F4;
    }
...
void EnGb_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_CollisionHeader_GetVirtual(&gPoeSellerCol, &colHeader);
    this->dyna.bgId = OoT_DynaPoly_SetBgActor(play, &play->colCtx.dyna, &this->dyna.actor, colHeader);
    OoT_SkelAnime_InitFlex(play, &this->skelAnime, &gPoeSellerSkel, &gPoeSellerIdleAnim, this->jointTable, this->morphTable,
                       12);
    OoT_Collider_InitCylinder(play, &this->collider);
    OoT_Collider_SetCylinderType1(play, &this->collider, &this->dyna.actor, &OoT_sCylinderInit);

    for (i = 0; i < ARRAY_COUNT(sBottlesCylindersInit); i++) {
        OoT_Collider_InitCylinder(play, &this->bottlesColliders[i]);
        OoT_Collider_SetCylinderType1(play, &this->bottlesColliders[i], &this->dyna.actor, &sBottlesCylindersInit[i]);
    }

    this->light = OoT_LightContext_InsertLight(play, &play->lightCtx, &this->lightInfo);
    OoT_Lights_PointNoGlowSetInfo(&this->lightInfo, this->dyna.actor.home.pos.x, this->dyna.actor.home.pos.y,
                              this->dyna.actor.home.pos.z, 255, 255, 255, 200);

    OoT_ActorShape_Init(&this->dyna.actor.shape, 0.0f, OoT_ActorShadow_DrawCircle, 35.0f);
    OoT_Actor_SetScale(&this->dyna.actor, 0.01f);
    this->dyna.actor.colChkInfo.mass = 0xFF;
    this->dyna.actor.speedXZ = 0.0f;
    this->dyna.actor.velocity.y = 0.0f;
    this->dyna.actor.gravity = -1.0f;
    this->actionTimer = (s16)OoT_Rand_ZeroFloat(100.0f) + 100;

    for (i = 0; i < ARRAY_COUNT(sCagedSoulPositions); i++) {
        this->cagedSouls[i].epoch++;
        this->cagedSouls[i].infoIdx = (s32)OoT_Rand_ZeroFloat(30.0f) % 3;
        ...
        this->cagedSouls[i].unk_6 = OoT_Rand_ZeroFloat(40.0f);
    ...
    focusOffset.z = 44.0f;
    OoT_Matrix_MultVec3f(&focusOffset, &this->dyna.actor.focus.pos);
...
void EnGb_Destroy(Actor* thisx, PlayState* play) {
    ...
    OoT_LightContext_RemoveLight(play, &play->lightCtx, this->light);
...
void func_80A2F608(EnGb* this) {
    ...
    for (i = 0; i < ARRAY_COUNT(sBottlesPositions); i++) {
        ...
        OoT_Matrix_RotateZYX(this->dyna.actor.world.rot.x, this->dyna.actor.world.rot.y, this->dyna.actor.world.rot.z,
                         MTXMODE_APPLY);
        OoT_Matrix_MultVec3f(&sBottlesPositions[i], &sp3C);
...
void func_80A2F7C0(EnGb* this) {
    OoT_Animation_Change(&this->skelAnime, &gPoeSellerSwingStickAnim, 1.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gPoeSellerSwingStickAnim), ANIMMODE_ONCE, 0.0f);
...
void func_80A2FA50(EnGb* this, PlayState* play) {
    if (OoT_Message_GetState(&play->msgCtx) == TEXT_STATE_DONE && OoT_Message_ShouldAdvance(play)) {
        ...
        if (HIGH_SCORE(HS_POE_POINTS) != 1000) {
            ...
            this->textId = 0x70F8;
            OoT_Message_ContinueTextbox(play, this->textId);
...
void func_80A2FB40(EnGb* this, PlayState* play) {
    if (OoT_Message_GetState(&play->msgCtx) == TEXT_STATE_DONE && OoT_Message_ShouldAdvance(play)) {
        if (GameInteractor_Should(VB_GIVE_ITEM_FROM_POE_COLLECTOR, true, this)) {
            OoT_Actor_OfferGetItem(&this->dyna.actor, play, GI_BOTTLE, 100.0f, 10.0f);
...
void func_80A2FC70(EnGb* this, PlayState* play) {
    if (this->skelAnime.curFrame == OoT_Animation_GetLastFrame(&gPoeSellerSwingStickAnim)) {
        OoT_Animation_Change(&this->skelAnime, &gPoeSellerIdleAnim, 1.0f, 0.0f, OoT_Animation_GetLastFrame(&gPoeSellerIdleAnim),
                         ANIMMODE_LOOP, 0.0f);
        this->actionFunc = func_80A2F83C;
        ...
        if (this->actionFunc) {}
        this->actionTimer = (s16)OoT_Rand_ZeroFloat(600.0f) + 600;
...
void EnGb_Update(Actor* thisx, PlayState* play2) {
    ...
    for (i = 0; i < ARRAY_COUNT(this->bottlesColliders); i++) {
        OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->bottlesColliders[i].base);
    }
...
void EnGb_Draw(Actor* thisx, PlayState* play) {
    ...
    OoT_Lights_PointNoGlowSetInfo(&this->lightInfo, this->dyna.actor.world.pos.x, this->dyna.actor.world.pos.y,
...
void EnGb_UpdateCagedSouls(EnGb* this, PlayState* play) {
    ...
    for (i = 0; i < 4; i++) {
        switch (this->cagedSouls[i].unk_1) {
            case 0:
                OoT_Math_ApproachF(&this->cagedSouls[i].unk_20, 1.0f, 0.02f, this->cagedSouls[i].unk_24);
                OoT_Math_ApproachF(&this->cagedSouls[i].unk_24, 1.0f, 0.001f, 1.0f);
            ...
            case 1:
                ...
                break;
            case 2:
                OoT_Math_ApproachF(&this->cagedSouls[i].unk_20, 0.0f, 0.02f, this->cagedSouls[i].unk_24);
                OoT_Math_ApproachF(&this->cagedSouls[i].unk_24, 1.0f, 0.001f, 1.0f);
                ...
                break;
            case 3:
                OoT_Math_ApproachF(&this->cagedSouls[i].unk_20, 0.0f, 0.5f, 1.0f);
                if (this->cagedSouls[i].unk_20 <= 0.01f) {
                    this->cagedSouls[i].unk_28 = 0.2f;
                    this->cagedSouls[i].unk_20 = 0.0f;
                    this->cagedSouls[i].unk_24 = 0.0f;
                    this->cagedSouls[i].unk_1 = 1;
                    this->cagedSouls[i].unk_2 = 0;
                    this->cagedSouls[i].unk_6 = (s16)OoT_Rand_ZeroFloat(60.0f) + 60;
                }
                break;
        }
        ...
        if ((i == 0) || (i == 3)) {
            ...
            this->cagedSouls[i].translation.x = this->cagedSouls[i].unk_14.x + OoT_Math_SinS(rot) * temp_f20;
            this->cagedSouls[i].translation.z = this->cagedSouls[i].unk_14.z + OoT_Math_CosS(rot) * temp_f20;
...
void EnGb_DrawCagedSouls(EnGb* this, PlayState* play) {
    ...
    for (i = 0; i < 4; i++) {
        ...
        OoT_Matrix_Push();
        OoT_Matrix_Translate(this->cagedSouls[i].translation.x, this->cagedSouls[i].translation.y,
                         this->cagedSouls[i].translation.z, MTXMODE_NEW);
        OoT_Matrix_ReplaceRotation(&play->billboardMtxF);

        if (this->cagedSouls[i].rotate180) {
            OoT_Matrix_RotateZYX(0, -0x8000, 0, MTXMODE_APPLY);
        }
        OoT_Matrix_Scale(0.007f, 0.007f, 1.0f, MTXMODE_APPLY);
