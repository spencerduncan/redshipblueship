...
void OoT_EnWf_Init(Actor* thisx, PlayState* play);
void OoT_EnWf_Destroy(Actor* thisx, PlayState* play);
void OoT_EnWf_Update(Actor* thisx, PlayState* play);
void OoT_EnWf_Draw(Actor* thisx, PlayState* play);
...
static ColliderCylinderInit OoT_sTailCylinderInit = {
    {
        COLTYPE_HIT5,
        AT_NONE,
        AC_ON | AC_TYPE_PLAYER,
...
const ActorInit En_Wf_InitVars = {
    ...
    OBJECT_WF,
    sizeof(EnWf),
    (ActorFunc)OoT_EnWf_Init,
    (ActorFunc)OoT_EnWf_Destroy,
    (ActorFunc)OoT_EnWf_Update,
    (ActorFunc)OoT_EnWf_Draw,
    NULL,
};
...
void OoT_EnWf_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_Collider_InitJntSph(play, &this->colliderSpheres);
    OoT_Collider_SetJntSph(play, &this->colliderSpheres, thisx, &OoT_sJntSphInit, this->colliderSpheresElements);
    OoT_Collider_InitCylinder(play, &this->colliderCylinderBody);
    OoT_Collider_SetCylinder(play, &this->colliderCylinderBody, thisx, &OoT_sBodyCylinderInit);
    OoT_Collider_InitCylinder(play, &this->colliderCylinderTail);
    OoT_Collider_SetCylinder(play, &this->colliderCylinderTail, thisx, &OoT_sTailCylinderInit);

    if (thisx->params == WOLFOS_NORMAL) {
        OoT_SkelAnime_InitFlex(play, &this->skelAnime, &gWolfosNormalSkel, &gWolfosWaitingAnim, this->jointTable,
                           this->morphTable, WOLFOS_LIMB_MAX);
        OoT_Actor_SetScale(thisx, 0.0075f);
        thisx->naviEnemyId = 0x4C; // Wolfos
    } else {                       // WOLFOS_WHITE
        OoT_SkelAnime_InitFlex(play, &this->skelAnime, &gWolfosWhiteSkel, &gWolfosWaitingAnim, this->jointTable,
                           this->morphTable, WOLFOS_LIMB_MAX);
    ...
    if ((this->switchFlag != 0xFF) && OoT_Flags_GetSwitch(play, this->switchFlag)) {
...
void OoT_EnWf_Destroy(Actor* thisx, PlayState* play) {
    ...
    OoT_Collider_DestroyJntSph(play, &this->colliderSpheres);
    OoT_Collider_DestroyCylinder(play, &this->colliderCylinderBody);
    OoT_Collider_DestroyCylinder(play, &this->colliderCylinderTail);
...
s32 EnWf_ChangeAction(PlayState* play, EnWf* this, s16 mustChoose) {
    ...
    if (mustChoose) {
        ...
        if ((this->actor.xzDistToPlayer <= 80.0f) && !OoT_Actor_OtherIsTargeted(play, &this->actor) &&
...
void EnWf_SetupWaitToAppear(EnWf* this) {
    OoT_Animation_Change(&this->skelAnime, &gWolfosRearingUpFallingOverAnim, 0.5f, 0.0f, 7.0f, ANIMMODE_ONCE_INTERP, 0.0f);
...
void EnWf_WaitToAppear(EnWf* this, PlayState* play) {
    if (this->actionTimer >= 6) {
        ...
        OoT_Math_SmoothStepToF(&this->actor.shape.shadowScale, 70.0f, 1.0f, 14.0f, 0.0f);
        this->actionTimer--;
...
void EnWf_SetupWait(EnWf* this) {
    OoT_Animation_MorphToLoop(&this->skelAnime, &gWolfosWaitingAnim, -4.0f);
    this->action = WOLFOS_ACTION_WAIT;
    this->actionTimer = (OoT_Rand_ZeroOne() * 10.0f) + 2.0f;
...
void EnWf_Wait(EnWf* this, PlayState* play) {
    ...
    if (!EnWf_DodgeRanged(play, this)) {
        // Only use of unk_2E0: never not OoT_zero, so this if block never runs
        if (this->unk_2E0 != 0) {
            this->unk_2E0--;
        ...
        if ((this->actor.xzDistToPlayer < 80.0f) && (player->meleeWeaponState != 0) && (angle >= 0x1F40)) {
            ...
            if (this->actionTimer == 0) {
                if (OoT_Actor_IsFacingPlayer(&this->actor, 0x1555)) {
...
void EnWf_SetupRunAtPlayer(EnWf* this, PlayState* play) {
    f32 lastFrame = OoT_Animation_GetLastFrame(&gWolfosRunningAnim);

    OoT_Animation_Change(&this->skelAnime, &gWolfosRunningAnim, 1.0f, 0.0f, lastFrame, ANIMMODE_LOOP_INTERP, -4.0f);
...
void EnWf_RunAtPlayer(EnWf* this, PlayState* play) {
    ...
    if (!EnWf_DodgeRanged(play, this)) {
        ...
        if (!OoT_Actor_IsFacingPlayer(&this->actor, 0x11C7)) {
            ...
            if (!OoT_Actor_OtherIsTargeted(play, &this->actor) &&
                ((OoT_Rand_ZeroOne() > 0.03f) || ((this->actor.xzDistToPlayer <= 80.0f) && (ABS(temp_v1) < 0x38E0)))) {
                EnWf_SetupSlash(this);
        ...
        if (!EnWf_ChangeAction(play, this, false)) {
            ...
            if ((animPrevFrame != (s32)this->skelAnime.curFrame) && (sp58 <= 0) && ((playSpeed + animPrevFrame) > 0)) {
                ...
                OoT_Actor_SpawnFloorDustRing(play, &this->actor, &this->actor.world.pos, 20.0f, 3, 3.0f, 50, 50, true);
            }
        }
    }
}
...
void EnWf_SetupSearchForPlayer(EnWf* this) {
    OoT_Animation_MorphToLoop(&this->skelAnime, &gWolfosSidesteppingAnim, -4.0f);
...
void EnWf_SetupRunAroundPlayer(EnWf* this) {
    ...
    this->actor.world.rot.y = this->actor.shape.rot.y;
    this->actionTimer = (OoT_Rand_ZeroOne() * 30.0f) + 30.0f;
...
void EnWf_RunAroundPlayer(EnWf* this, PlayState* play) {
    ...
    OoT_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer + this->runAngle, 1, 4000, 1);

    if (!EnWf_DodgeRanged(play, this) && !EnWf_ChangeAction(play, this, false)) {
        this->actor.world.rot.y = this->actor.shape.rot.y;
        angle1 = player->actor.shape.rot.y + this->runAngle + 0x8000;

        // OoT_Actor_TestFloorInDirection is useless here (see comment below)
        if ((this->actor.bgCheckFlags & 8) ||
            !OoT_Actor_TestFloorInDirection(&this->actor, play, this->actor.speedXZ, this->actor.shape.rot.y)) {
        ...
        if (this->actor.xzDistToPlayer <= (60.0f + baseRange)) {
            OoT_Math_SmoothStepToF(&this->runSpeed, -4.0f, 1.0f, 1.5f, 0.0f);
        } else if ((80.0f + baseRange) < this->actor.xzDistToPlayer) {
            OoT_Math_SmoothStepToF(&this->runSpeed, 4.0f, 1.0f, 1.5f, 0.0f);
        } else {
            OoT_Math_SmoothStepToF(&this->runSpeed, 0.0f, 1.0f, 6.65f, 0.0f);
        }

        if (this->runSpeed != 0.0f) {
            this->actor.world.pos.x += OoT_Math_SinS(this->actor.shape.rot.y) * this->runSpeed;
            this->actor.world.pos.z += OoT_Math_CosS(this->actor.shape.rot.y) * this->runSpeed;
        }
        ...
        if ((OoT_Math_CosS(angle1 - this->actor.shape.rot.y) < -0.85f) && !OoT_Actor_OtherIsTargeted(play, &this->actor) &&
            ...
            if (this->actionTimer == 0) {
                if (OoT_Actor_OtherIsTargeted(play, &this->actor) && (OoT_Rand_ZeroOne() > 0.5f)) {
                    ...
                    EnWf_SetupWait(this);
                    this->actionTimer = (OoT_Rand_ZeroOne() * 3.0f) + 1.0f;
                }
            }
        }
    }
...
void EnWf_SetupSlash(EnWf* this) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gWolfosSlashingAnim);
...
void EnWf_Slash(EnWf* this, PlayState* play) {
    ...
    if (((curFrame == 15) && !OoT_Actor_IsTargeted(play, &this->actor) &&
         (!OoT_Actor_IsFacingPlayer(&this->actor, 0x2000) || (this->actor.xzDistToPlayer >= 100.0f))) ||
        OoT_SkelAnime_Update(&this->skelAnime)) {
        if ((curFrame != 15) && (this->actionTimer != 0)) {
            this->actor.shape.rot.y += (s16)(3276.0f * (1.5f + (this->actionTimer - 4) * 0.4f));
            OoT_Actor_SpawnFloorDustRing(play, &this->actor, &this->actor.world.pos, 15.0f, 1, 2.0f, 50, 50, true);
            this->actionTimer--;
        } else if (!OoT_Actor_IsFacingPlayer(&this->actor, 0x1554) && (curFrame != 15)) {
            EnWf_SetupWait(this);
            this->actionTimer = (OoT_Rand_ZeroOne() * 5.0f) + 5.0f;
...
void EnWf_SetupRecoilFromBlockedSlash(EnWf* this) {
    ...
    OoT_Animation_Change(&this->skelAnime, &gWolfosSlashingAnim, -0.5f, this->skelAnime.curFrame - 1.0f, endFrame,
...
void EnWf_RecoilFromBlockedSlash(EnWf* this, PlayState* play) {
    ...
    if (OoT_SkelAnime_Update(&this->skelAnime)) {
        if (!OoT_Actor_IsFacingPlayer(&this->actor, 0x1554)) {
            ...
            if ((OoT_Rand_ZeroOne() > 0.7f) || (this->actor.xzDistToPlayer >= 120.0f)) {
                EnWf_SetupWait(this);
                this->actionTimer = (OoT_Rand_ZeroOne() * 5.0f) + 5.0f;
            } else {
                this->actor.world.rot.y = this->actor.yawTowardsPlayer;
...
void EnWf_SetupBackflipAway(EnWf* this) {
    OoT_Animation_MorphToPlayOnce(&this->skelAnime, &gWolfosBackflippingAnim, -3.0f);
...
void EnWf_SetupStunned(EnWf* this) {
    ...
    OoT_Animation_PlayOnceSetSpeed(&this->skelAnime, &gWolfosDamagedAnim, 0.0f);
...
void EnWf_SetupDamaged(EnWf* this) {
    OoT_Animation_MorphToPlayOnce(&this->skelAnime, &gWolfosDamagedAnim, -4.0f);
...
void EnWf_Damaged(EnWf* this, PlayState* play) {
    ...
    if (!EnWf_ChangeAction(play, this, false) && OoT_SkelAnime_Update(&this->skelAnime)) {
        if (this->actor.bgCheckFlags & 1) {
...
void EnWf_SetupSomersaultAndAttack(EnWf* this) {
    f32 lastFrame = OoT_Animation_GetLastFrame(&gWolfosBackflippingAnim);

    OoT_Animation_Change(&this->skelAnime, &gWolfosBackflippingAnim, -1.0f, lastFrame, 0.0f, ANIMMODE_ONCE, -3.0f);
    this->actionTimer = 0;
    this->unk_300 = false;
...
void EnWf_SomersaultAndAttack(EnWf* this, PlayState* play) {
    ...
    if (OoT_SkelAnime_Update(&this->skelAnime) && (this->actor.bgCheckFlags & (1 | 2))) {
...
void EnWf_SetupBlocking(EnWf* this) {
    f32 lastFrame = OoT_Animation_GetLastFrame(&gWolfosBlockingAnim);
    ...
    OoT_Animation_Change(&this->skelAnime, &gWolfosBlockingAnim, 0.0f, 0.0f, lastFrame, ANIMMODE_ONCE_INTERP, -4.0f);
...
void EnWf_SetupSidestep(EnWf* this, PlayState* play) {
    ...
    if (OoT_Math_SinS(angle - this->actor.yawTowardsPlayer) > 0.0f) {
        this->runAngle = 16000;
    } else if (OoT_Math_SinS(angle - this->actor.yawTowardsPlayer) < 0.0f) {
        this->runAngle = -16000;
    } else if (OoT_Rand_ZeroOne() > 0.5f) {
        this->runAngle = 16000;
    } else {
    ...
    this->actor.world.rot.y = this->actor.shape.rot.y;
    this->runSpeed = 0.0f;
    this->actionTimer = (OoT_Rand_ZeroOne() * 10.0f) + 5.0f;
...
void EnWf_Sidestep(EnWf* this, PlayState* play) {
    ...
    OoT_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer + this->runAngle, 1, 3000, 1);
...
void EnWf_SetupDie(EnWf* this) {
    OoT_Animation_MorphToPlayOnce(&this->skelAnime, &gWolfosRearingUpFallingOverAnim, -4.0f);
    this->actor.world.rot.y = this->actor.yawTowardsPlayer;
...
void EnWf_Die(EnWf* this, PlayState* play) {
    ...
    if (OoT_SkelAnime_Update(&this->skelAnime)) {
        ...
        for (i = ((s32)this->skelAnime.animLength - this->actionTimer) >> 1; i >= 0; i--) {
            ...
            pos.y = OoT_Rand_CenteredFloat(50.0f) + (this->actor.world.pos.y + 20.0f);
            OoT_EffectSsDeadDb_Spawn(play, &pos, &velAndAccel, &velAndAccel, 100, 0, 255, 255, 255, 255, 0, 0, 255, 1, 9,
                                 true);
        }
    }
}
...
void func_80B36F40(EnWf* this, PlayState* play) {
    if ((this->action == WOLFOS_ACTION_WAIT) && (this->unk_2E2 != 0)) {
        this->unk_4D4.y = OoT_Math_SinS(this->unk_2E2 * 4200) * 8920.0f;
    } else if (this->action != WOLFOS_ACTION_STUNNED) {
        if (this->action != WOLFOS_ACTION_SLASH) {
            OoT_Math_SmoothStepToS(&this->unk_4D4.y, this->actor.yawTowardsPlayer - this->actor.shape.rot.y, 1, 1500, 0);
    ...
    if (this->colliderSpheres.base.acFlags & AC_BOUNCED) {
        ...
        if (this->action >= WOLFOS_ACTION_WAIT) {
            ...
            if (this->actor.colChkInfo.damageEffect != ENWF_DMGEFF_ICE_MAGIC) {
                ...
                OoT_Actor_SetDropFlag(&this->actor, &this->colliderCylinderBody.info, 1);
                this->slashStatus = 0;
...
void OoT_EnWf_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.colChkInfo.damageEffect != ENWF_DMGEFF_ICE_MAGIC) {
        ...
        OoT_Actor_UpdateBgCheckInfo(play, &this->actor, 32.0f, 30.0f, 60.0f, 0x1D);
    ...
    if (this->actor.bgCheckFlags & (1 | 2)) {
        ...
        OoT_Math_SmoothStepToS(&this->actor.shape.rot.x, 0, 1, 1000, 0);
        OoT_Math_SmoothStepToS(&this->actor.shape.rot.z, 0, 1, 1000, 0);
    }

    OoT_CollisionCheck_SetOC(play, &play->colChkCtx, &this->colliderSpheres.base);

    if (this->action >= WOLFOS_ACTION_WAIT) {
        if ((this->actor.colorFilterTimer == 0) || !(this->actor.colorFilterParams & 0x4000)) {
            OoT_Collider_UpdateCylinder(&this->actor, &this->colliderCylinderBody);
            OoT_CollisionCheck_SetAC(play, &play->colChkCtx, &this->colliderCylinderTail.base);
            OoT_CollisionCheck_SetAC(play, &play->colChkCtx, &this->colliderCylinderBody.base);
        }
    }

    if (this->action == WOLFOS_ACTION_BLOCKING) {
        OoT_CollisionCheck_SetAC(play, &play->colChkCtx, &this->colliderSpheres.base);
    }

    if (this->slashStatus > 0) {
        if (!(this->colliderSpheres.base.atFlags & AT_BOUNCED)) {
            OoT_CollisionCheck_SetAT(play, &play->colChkCtx, &this->colliderSpheres.base);
        } else {
    ...
    if (this->eyeIndex == 0) {
        if ((OoT_Rand_ZeroOne() < 0.2f) && ((play->gameplayFrames % 4) == 0) && (this->actor.colorFilterTimer == 0)) {
            this->eyeIndex++;
        }
    } else {
...
s32 OoT_EnWf_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    EnWf* this = (EnWf*)thisx;
...
void OoT_EnWf_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    static Vec3f colliderVec = { 1200.0f, 0.0f, 0.0f };
    static Vec3f bodyPartVec = { 0.0f, 0.0f, 0.0f };
    EnWf* this = (EnWf*)thisx;
    s32 bodyPartIndex = -1;

    OoT_Collider_UpdateSpheres(limbIndex, &this->colliderSpheres);

    if (limbIndex == WOLFOS_LIMB_TAIL) {
        Vec3f colliderPos;

        bodyPartIndex = -1;
        OoT_Matrix_MultVec3f(&colliderVec, &colliderPos);
    ...
    if ((this->fireTimer != 0) || ((this->actor.colorFilterTimer != 0) && (this->actor.colorFilterParams & 0x4000))) {
        ...
        if (bodyPartIndex >= 0) {
            ...
            OoT_Matrix_MultVec3f(&bodyPartVec, &bodyPartPos);
...
void OoT_EnWf_Draw(Actor* thisx, PlayState* play) {
    ...
    if ((this->action != WOLFOS_ACTION_WAIT_TO_APPEAR) || !this->unk_300) {
        ...
        SkelAnime_DrawSkeletonOpa(play, &this->skelAnime, OoT_EnWf_OverrideLimbDraw, OoT_EnWf_PostLimbDraw, &this->actor);
        ...
        if (this->fireTimer != 0) {
            ...
            if ((this->fireTimer % 4) == 0) {
                ...
                OoT_EffectSsEnFire_SpawnVec3s(play, &this->actor, &this->bodyPartsPos[fireIndex], 75, 0, 0, fireIndex);
            }
        }
    }
