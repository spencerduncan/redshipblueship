...
void BgIceObjects_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_CollisionHeader_GetVirtual(&object_ice_objects_Col_0003F0, &colHeader);
    OoT_Math_Vec3f_Copy(&this->targetPos, &this->dyna.actor.home.pos);
...
void BgIceObjects_CheckPits(BgIceObjects* this, PlayState* play) {
    ...
    if ((thisx->velocity.y > 0.0f) || ((thisx->world.pos.x <= -1660.0f) && (thisx->world.pos.z <= -1060.0f)) ||
        ...
        if (OoT_Math_StepToF(&thisx->world.pos.y, -300.0f, thisx->velocity.y)) {
...
void BgIceObjects_Idle(BgIceObjects* this, PlayState* play) {
    ...
    if (this->dyna.unk_150 != 0.0f) {
        ...
        if ((this->dyna.unk_150 > 0.0f) && !OoT_Player_InCsMode(play)) {
            BgIceObjects_SetNextTarget(this, play);
            if (OoT_Actor_WorldDistXZToPoint(thisx, &this->targetPos) > 1.0f) {
                thisx->flags |= ACTOR_FLAG_UPDATE_CULLING_DISABLED;
                OoT_Player_SetCsActionWithHaltedActors(play, thisx, 8);
                thisx->params = 1;
...
void BgIceObjects_Slide(BgIceObjects* this, PlayState* play) {
    ...
    OoT_Math_StepToF(&thisx->speedXZ, 10.0f, 0.5f);
    atTarget = OoT_Math_StepToF(&thisx->world.pos.x, this->targetPos.x, thisx->speedXZ);
    atTarget &= OoT_Math_StepToF(&thisx->world.pos.z, this->targetPos.z, thisx->speedXZ);
    if (atTarget) {
        thisx->speedXZ = 0.0f;
        ...
        spread = OoT_Rand_CenteredFloat(120.0f);
        velocity.x = -(1.5f + OoT_Rand_ZeroOne()) * OoT_Math_SinS(this->dyna.unk_158);
        velocity.y = OoT_Rand_ZeroOne() + 1.0f;
        velocity.z = -(1.5f + OoT_Rand_ZeroOne()) * OoT_Math_CosS(this->dyna.unk_158);
        pos.x = thisx->world.pos.x - (60.0f * OoT_Math_SinS(this->dyna.unk_158)) - (OoT_Math_CosS(this->dyna.unk_158) * spread);
        pos.z = thisx->world.pos.z - (60.0f * OoT_Math_CosS(this->dyna.unk_158)) + (OoT_Math_SinS(this->dyna.unk_158) * spread);
        pos.y = thisx->world.pos.y;
        func_8002829C(play, &pos, &velocity, &OoT_sZeroVec, &sWhite, &sGray, 250, OoT_Rand_S16Offset(40, 15));
        spread = OoT_Rand_CenteredFloat(120.0f);
        pos.x = thisx->world.pos.x - (60.0f * OoT_Math_SinS(this->dyna.unk_158)) + (OoT_Math_CosS(this->dyna.unk_158) * spread);
        pos.z = thisx->world.pos.z - (60.0f * OoT_Math_CosS(this->dyna.unk_158)) - (OoT_Math_SinS(this->dyna.unk_158) * spread);
        func_8002829C(play, &pos, &velocity, &OoT_sZeroVec, &sWhite, &sGray, 250, OoT_Rand_S16Offset(40, 15));
...
void BgIceObjects_Reset(BgIceObjects* this, PlayState* play) {
    ...
    if (OoT_Math_StepToF(&thisx->world.pos.y, thisx->home.pos.y, 1.0f)) {
        thisx->flags &= ~ACTOR_FLAG_UPDATE_CULLING_DISABLED;
        OoT_Math_Vec3f_Copy(&this->targetPos, &thisx->home.pos);
        this->actionFunc = BgIceObjects_Idle;
        thisx->speedXZ = 0.0f;
    }
}
...
void BgIceObjects_Draw(Actor* thisx, PlayState* play) {
    ...
    OoT_Gfx_DrawDListOpa(play, object_ice_objects_DL_000190);
}
