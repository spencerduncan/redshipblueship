...
void OoT_EnTest_Init(Actor* thisx, PlayState* play);
void OoT_EnTest_Destroy(Actor* thisx, PlayState* play);
void OoT_EnTest_Update(Actor* thisx, PlayState* play);
void OoT_EnTest_Draw(Actor* thisx, PlayState* play);
...
const ActorInit En_Test_InitVars = {
    ...
    OBJECT_SK2,
    sizeof(EnTest),
    (ActorFunc)OoT_EnTest_Init,
    (ActorFunc)OoT_EnTest_Destroy,
    (ActorFunc)OoT_EnTest_Update,
    (ActorFunc)OoT_EnTest_Draw,
    NULL,
};
...
void OoT_EnTest_Init(Actor* thisx, PlayState* play) {
    ...
    OoT_SkelAnime_Init(play, &this->skelAnime, &gStalfosSkel, &gStalfosMiddleGuardAnim, this->jointTable, this->morphTable,
                   STALFOS_LIMB_MAX);
    OoT_SkelAnime_Init(play, &this->upperSkelanime, &gStalfosSkel, &gStalfosMiddleGuardAnim, this->upperJointTable,
    ...
    OoT_Collider_InitCylinder(play, &this->shieldCollider);
    OoT_Collider_SetCylinder(play, &this->shieldCollider, &this->actor, &sShieldColliderInit);

    OoT_Collider_InitQuad(play, &this->swordCollider);
    OoT_Collider_SetQuad(play, &this->swordCollider, &this->actor, &sSwordColliderInit);
    ...
    OoT_Effect_Add(play, &this->effectIndex, EFFECT_BLURE1, 0, 0, &slashBlure);
...
void OoT_EnTest_Destroy(Actor* thisx, PlayState* play) {
    ...
    if ((this->actor.params != STALFOS_TYPE_2) &&
        !OoT_Actor_FindNearby(play, &this->actor, ACTOR_EN_TEST, ACTORCAT_ENEMY, 8000.0f)) {
        func_800F5B58();
    }

    Effect_Delete(play, this->effectIndex);
    OoT_Collider_DestroyCylinder(play, &this->shieldCollider);
...
void EnTest_ChooseRandomAction(EnTest* this, PlayState* play) {
    switch ((u32)(OoT_Rand_ZeroOne() * 10.0f)) {
        case 0:
        case 1:
        case 5:
        case 6:
            if ((this->actor.xzDistToPlayer < 220.0f) && (this->actor.xzDistToPlayer > 170.0f) &&
                OoT_Actor_IsFacingPlayer(&this->actor, 0x71C) && OoT_Actor_IsTargeted(play, &this->actor)) {
    ...
    if (yawDiff >= 0x61A8) {
        ...
        if (ABS((s16)(this->actor.yawTowardsPlayer - this->actor.shape.rot.y)) > 0x3E80) {
            if (((play->gameplayFrames % 2) != 0) && (this->actor.params != STALFOS_TYPE_CEILING)) {
                ...
                if (OoT_Actor_IsFacingPlayer(&this->actor, 0x71C) && !OoT_Actor_IsTargeted(play, &this->actor)) {
                    EnTest_SetupJumpslash(this);
                }
            } else {
                EnTest_SetupWalkAndBlock(this);
...
void EnTest_SetupWaitGround(EnTest* this) {
    OoT_Animation_PlayLoop(&this->skelAnime, &gStalfosMiddleGuardAnim);
    this->unk_7C8 = 0;
    this->timer = 15;
    ...
    if (!EnTest_ReactToProjectile(play, this)) {
        ...
        if (this->actor.xzDistToPlayer < 100.0f) {
            if ((player->meleeWeaponState != 0) && (ABS(yawDiff) >= 0x1F40)) {
                ...
                if (OoT_Rand_ZeroOne() > 0.7f && player->meleeWeaponAnimation != 0x11) {
                    EnTest_SetupJumpBack(this);
                } else {
                    func_808627C4(this, play);
                }
...
void EnTest_Fall(EnTest* this, PlayState* play) {
    OoT_Animation_PlayOnceSetSpeed(&this->skelAnime, &gStalfosLandFromLeapAnim, 0.0f);
    OoT_SkelAnime_Update(&this->skelAnime);
...
void EnTest_SetupWalkAndBlock(EnTest* this) {
    OoT_Animation_Change(&this->upperSkelanime, &gStalfosBlockWithShieldAnim, 2.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gStalfosBlockWithShieldAnim), 2, 2.0f);
    OoT_Animation_PlayLoop(&this->skelAnime, &gStalfosSlowAdvanceAnim);
    this->timer = (s16)(OoT_Rand_ZeroOne() * 5.0f);
    this->unk_7C8 = 0xD;
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void EnTest_WalkAndBlock(EnTest* this, PlayState* play) {
    ...
    if (!EnTest_ReactToProjectile(play, this)) {
        ...
        if (this->actor.xzDistToPlayer <= (80.0f + checkDist)) {
            OoT_Math_SmoothStepToF(&this->actor.speedXZ, -5.0f, 1.0f, 0.8f, 0.0f);
        ...
        if ((this->actor.params == STALFOS_TYPE_CEILING) &&
            !OoT_Actor_TestFloorInDirection(&this->actor, play, this->actor.speedXZ, this->actor.world.rot.y)) {
            this->actor.speedXZ *= -1.0f;
        }

        if (ABS(this->actor.speedXZ) < 3.0f) {
            OoT_Animation_Change(&this->skelAnime, &gStalfosSlowAdvanceAnim, 0.0f, this->skelAnime.curFrame,
                             OoT_Animation_GetLastFrame(&gStalfosSlowAdvanceAnim), 0, -6.0f);
            playSpeed = this->actor.speedXZ * 10.0f;
        } else {
            OoT_Animation_Change(&this->skelAnime, &gStalfosFastAdvanceAnim, 0.0f, this->skelAnime.curFrame,
                             OoT_Animation_GetLastFrame(&gStalfosFastAdvanceAnim), 0, -4.0f);
        ...
        if (!OoT_Actor_IsFacingPlayer(&this->actor, 0x11C7)) {
            EnTest_SetupIdle(this);
            this->timer = (OoT_Rand_ZeroOne() * 10.0f) + 10.0f;
            return;
        }
...
void func_80860BDC(EnTest* this) {
    OoT_Animation_PlayLoop(&this->skelAnime, &gStalfosSidestepAnim);
    this->unk_7C8 = 0xE;
...
void func_80860C24(EnTest* this, PlayState* play) {
    ...
    if (!EnTest_ReactToProjectile(play, this)) {
        ...
        if (OoT_Actor_IsFacingPlayer(&this->actor, 0x71C)) {
...
void func_80860EC0(EnTest* this) {
    OoT_Animation_PlayLoop(&this->skelAnime, &gStalfosSidestepAnim);
    this->unk_7C8 = 0xF;
    this->actor.speedXZ = (OoT_Rand_ZeroOne() > 0.5f) ? -0.5f : 0.5f;
    this->timer = (s16)((OoT_Rand_ZeroOne() * 15.0f) + 25.0f);
    this->unk_7EC = 0.0f;
    this->actor.world.rot.y = this->actor.shape.rot.y;
...
void func_80860F84(EnTest* this, PlayState* play) {
    ...
    if (!EnTest_ReactToProjectile(play, this)) {
        ...
        if (this->actor.xzDistToPlayer <= (80.0f + checkDist)) {
            OoT_Math_SmoothStepToF(&this->unk_7EC, -2.5f, 1.0f, 0.8f, 0.0f);
        } else if (this->actor.xzDistToPlayer > (110.0f + checkDist)) {
            OoT_Math_SmoothStepToF(&this->unk_7EC, 2.5f, 1.0f, 0.8f, 0.0f);
        } else {
            OoT_Math_SmoothStepToF(&this->unk_7EC, 0.0f, 1.0f, 6.65f, 0.0f);
        }

        if (this->unk_7EC != 0.0f) {
            this->actor.world.pos.x += OoT_Math_SinS(this->actor.shape.rot.y) * this->unk_7EC;
            this->actor.world.pos.z += OoT_Math_CosS(this->actor.shape.rot.y) * this->unk_7EC;
        }
...
void EnTest_SetupSlashDown(EnTest* this) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosDownSlashAnim);
...
void EnTest_SlashDown(EnTest* this, PlayState* play) {
    ...
    if ((s32)this->skelAnime.curFrame < 4) {
        OoT_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 1, 0xBB8, 0);
    }
...
void EnTest_SetupSlashDownEnd(EnTest* this) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosRecoverFromDownSlashAnim);
    this->unk_7C8 = 0x12;
    this->actor.speedXZ = 0.0f;
...
void EnTest_SlashDownEnd(EnTest* this, PlayState* play) {
    ...
    if (OoT_SkelAnime_Update(&this->skelAnime)) {
        ...
        this->actor.world.rot.y = OoT_Actor_WorldYawTowardActor(&this->actor, &player->actor);
...
void EnTest_SetupSlashUp(EnTest* this) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosUpSlashAnim);
    this->swordCollider.base.atFlags &= ~AT_BOUNCED;
    this->unk_7C8 = 0x11;
    this->swordCollider.info.toucher.damage = 16;
    this->actor.speedXZ = 0.0f;
...
void EnTest_SetupJumpBack(EnTest* this) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosJumpBackwardsAnim);
    ...
    if (OoT_SkelAnime_Update(&this->skelAnime)) {
        if (!EnTest_ReactToProjectile(play, this)) {
            if (this->actor.xzDistToPlayer <= 100.0f) {
                ...
                if ((this->actor.xzDistToPlayer <= 220.0f) && OoT_Actor_IsFacingPlayer(&this->actor, 0xE38)) {
                    EnTest_SetupJumpslash(this);
                } else {
                    EnTest_SetupIdle(this);
                    this->timer = (OoT_Rand_ZeroOne() * 5.0f) + 5.0f;
...
void EnTest_SetupJumpslash(EnTest* this) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosJumpAnim);
    Audio_StopSfxByPosAndId(&this->actor.projectedPos, NA_SE_EN_STAL_WARAU);
    this->timer = 0;
    this->unk_7C8 = 0x17;
...
void EnTest_Jumpslash(EnTest* this, PlayState* play) {
    if (OoT_SkelAnime_Update(&this->skelAnime)) {
        if (this->timer == 0) {
            OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosJumpslashAnim);
            this->timer = 1;
            this->meleeWeaponState = 1;
            Audio_PlayActorSound2(&this->actor, NA_SE_EN_STAL_SAKEBI);
            Audio_PlayActorSound2(&this->actor, NA_SE_EN_STAL_JUMP);
...
void EnTest_JumpUp(EnTest* this, PlayState* play) {
    ...
    if (this->actor.world.pos.y <= this->actor.floorHeight) {
        ...
        this->unk_7C8 = 0xC;
        this->timer = 4;
        OoT_Animation_Change(&this->skelAnime, &gStalfosLandFromLeapAnim, 0.0f, 0.0f, 0.0f, 2, 0.0f);
        EnTest_SetupAction(this, EnTest_Land);
    }
}

void EnTest_SetupStopAndBlock(EnTest* this) {
    OoT_Animation_Change(&this->skelAnime, &gStalfosBlockWithShieldAnim, 2.0f, 0.0f,
                     OoT_Animation_GetLastFrame(&gStalfosBlockWithShieldAnim), 2, 2.0f);
    this->unk_7C8 = 0x15;
    this->actor.speedXZ = 0.0f;
    this->timer = (OoT_Rand_ZeroOne() * 10.0f) + 11.0f;
    this->actor.world.rot.y = this->actor.shape.rot.y;
    this->unk_7DE = 5;
...
void EnTest_SetupIdleFromBlock(EnTest* this) {
    OoT_Animation_MorphToLoop(&this->skelAnime, &gStalfosMiddleGuardAnim, -4.0f);
    this->unk_7C8 = 0x16;
...
void func_80862154(EnTest* this) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosFlinchFromHitFrontAnim);
...
void func_808621D4(EnTest* this, PlayState* play) {
    ...
    OoT_Math_SmoothStepToF(&this->actor.speedXZ, 0.0f, 1.0f, 0.1f, 0.0f);
    ...
    if (player->meleeWeaponState != 0) {
        if ((this->actor.bgCheckFlags & 8) && ((ABS((s16)(this->actor.wallYaw - this->actor.shape.rot.y)) < 0x38A4) &&
                                               (this->actor.xzDistToPlayer < 80.0f))) {
            EnTest_SetupJumpUp(this);
        } else if ((OoT_Rand_ZeroOne() > 0.7f) && (this->actor.params != STALFOS_TYPE_CEILING) &&
...
void func_80862398(EnTest* this) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosFlinchFromHitBehindAnim);
    Audio_PlayActorSound2(&this->actor, NA_SE_EN_STAL_DAMAGE);
    this->unk_7C8 = 9;
    this->actor.speedXZ = -2.0f;
    OoT_Actor_SetColorFilter(&this->actor, 0x4000, 0xFF, 0, 8);
...
void EnTest_SetupStunned(EnTest* this) {
    ...
    if (this->lastDamageEffect == STALFOS_DMGEFF_LIGHT) {
        OoT_Actor_SetColorFilter(&this->actor, -0x8000, 0x78, 0, 0x50);
    } else {
        ...
        if (this->lastDamageEffect == STALFOS_DMGEFF_FREEZE) {
            ...
            OoT_Animation_PlayOnceSetSpeed(&this->skelAnime, &gStalfosFlinchFromHitFrontAnim, 0.0f);
        }
    }
        ...
        if (this->actor.colChkInfo.health == 0) {
            ...
            if ((this->actor.bgCheckFlags & 8) &&
                ((ABS((s16)(this->actor.wallYaw - this->actor.shape.rot.y)) < 0x38A4) &&
                 (this->actor.xzDistToPlayer < 80.0f))) {
                EnTest_SetupJumpUp(this);
            } else if ((OoT_Rand_ZeroOne() > 0.7f) && (player->meleeWeaponAnimation != 0x11)) {
                EnTest_SetupJumpBack(this);
            } else {
                EnTest_SetupStopAndBlock(this);
            }
...
void func_808627C4(EnTest* this, PlayState* play) {
    ...
    OoT_Animation_MorphToLoop(&this->skelAnime, &gStalfosSidestepAnim, -2.0f);
    OoT_Math_SmoothStepToS(&this->actor.shape.rot.y, this->actor.yawTowardsPlayer, 1, 0xFA0, 1);
    this->actor.speedXZ = ((play->gameplayFrames % 2) != 0) ? -4.0f : 4.0f;
    this->actor.world.rot.y = this->actor.shape.rot.y + 0x3FFF;
    this->timer = (OoT_Rand_ZeroOne() * 20.0f) + 20.0f;
    this->unk_7C8 = 0x18;
...
void func_808628C8(EnTest* this, PlayState* play) {
    ...
    if (this->timer == 0) {
        if (OoT_Actor_OtherIsTargeted(play, &this->actor)) {
            EnTest_SetupIdle(this);
        } else if (OoT_Actor_IsTargeted(play, &this->actor)) {
            if (!EnTest_ReactToProjectile(play, this)) {
                EnTest_ChooseAction(this, play);
            }
        } else if (player->heldItemAction != PLAYER_IA_NONE) {
...
void func_80862DBC(EnTest* this, PlayState* play) {
    ...
    if (this->meleeWeaponState >= 0) {
        OoT_EffectBlure_AddSpace(OoT_Effect_GetByIndex(this->effectIndex));
        this->meleeWeaponState = -1;
    }
...
void func_80862FA8(EnTest* this, PlayState* play) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosFallOverBackwardsAnim);
...
void func_808630F0(EnTest* this, PlayState* play) {
    OoT_Animation_PlayOnce(&this->skelAnime, &gStalfosFallOverForwardsAnim);
    Audio_PlayActorSound2(&this->actor, NA_SE_EN_STAL_DEAD);
    this->unk_7C8 = 6;
    this->actor.colorFilterTimer = 0;
    this->unk_7DE = 0;
...
void EnTest_UpdateDamage(EnTest* this, PlayState* play) {
    ...
    if (this->shieldCollider.base.acFlags & AC_BOUNCED) {
        ...
        if ((this->actor.colChkInfo.damageEffect != STALFOS_DMGEFF_SLING) &&
            ...
            this->actor.world.rot.y = this->actor.yawTowardsPlayer;
            OoT_Actor_SetDropFlag(&this->actor, &this->bodyCollider.info, false);
            Audio_StopSfxByPosAndId(&this->actor.projectedPos, NA_SE_EN_STAL_WARAU);
            ...
            if ((this->actor.colChkInfo.damageEffect == STALFOS_DMGEFF_STUN) ||
                ...
                if (OoT_Actor_IsFacingPlayer(&this->actor, 0x4000)) {
                ...
                } else if (OoT_Actor_ApplyDamage(&this->actor) == 0) {
...
void OoT_EnTest_Update(Actor* thisx, PlayState* play) {
    ...
    if (this->actor.colChkInfo.damageEffect != STALFOS_DMGEFF_FIREMAGIC) {
        ...
        switch (this->unk_7DE) {
            ...
            case 2:
                OoT_SkelAnime_Update(&this->upperSkelanime);
                OoT_SkelAnime_CopyFrameTableTrue(&this->skelAnime, this->skelAnime.jointTable,
                                             this->upperSkelanime.jointTable, sJointCopyFlags);
                break;
            ...
            case 4:
                ...
                OoT_SkelAnime_InterpFrameTable(this->skelAnime.limbCount, this->upperSkelanime.jointTable,
        ...
        if ((this->actor.colorFilterTimer == 0) && (this->actor.colChkInfo.health != 0)) {
            if ((this->unk_7C8 != 0x10) && (this->unk_7C8 != 0x17)) {
                ...
                OoT_Math_SmoothStepToS(&this->headRot.y, 0, 1, 0x3E8, 0);
            }
        }
    }
...
s32 OoT_EnTest_OverrideLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* thisx) {
    EnTest* this = (EnTest*)thisx;
    s32 pad;
    ...
    if (limbIndex == STALFOS_LIMB_HEAD_ROOT) {
        ...
        gDPSetEnvColor(POLY_OPA_DISP++, 80 + ABS((s16)(OoT_Math_SinS(play->gameplayFrames * 2000) * 175.0f)), 0, 0, 255);
...
void EnTest_PostLimbDraw(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    ...
    if (limbIndex == STALFOS_LIMB_SWORD) {
        OoT_Matrix_MultVec3f(&D_8086467C, &this->swordCollider.dim.quad[1]);
        OoT_Matrix_MultVec3f(&D_80864688, &this->swordCollider.dim.quad[0]);
        OoT_Matrix_MultVec3f(&D_80864694, &this->swordCollider.dim.quad[3]);
        OoT_Matrix_MultVec3f(&D_808646A0, &this->swordCollider.dim.quad[2]);
        ...
        OoT_Matrix_MultVec3f(&D_80864664, &sp70);
        OoT_Matrix_MultVec3f(&D_80864670, &sp64);

        if ((this->meleeWeaponState >= 1) &&
            ((this->actor.params != STALFOS_TYPE_INVISIBLE) || play->actorCtx.lensActive)) {
            OoT_EffectBlure_AddVertex(OoT_Effect_GetByIndex(this->effectIndex), &sp70, &sp64);
        ...
        OoT_Actor_SetFeetPos(&this->actor, limbIndex, STALFOS_LIMB_FOOT_L, &D_80864658, STALFOS_LIMB_ANKLE_R, &D_80864658);

        if ((limbIndex == STALFOS_LIMB_FOOT_L) || (limbIndex == STALFOS_LIMB_ANKLE_R)) {
            if ((this->unk_7C8 == 0x15) || (this->unk_7C8 == 0x16)) {
                if (this->actor.speedXZ != 0.0f) {
                    OoT_Matrix_MultVec3f(&D_80864658, &sp64);
                    OoT_Actor_SpawnFloorDustRing(play, &this->actor, &sp64, 10.0f, 1, 8.0f, 100, 15, false);
                }
            }
        }
    }
    ...
    if (this->iceTimer != 0) {
        ...
        if (bodyPart >= 0) {
            OoT_Matrix_MultVec3f(&D_80864670, &sp50);
...
void OoT_EnTest_Draw(Actor* thisx, PlayState* play) {
    ...
    if ((thisx->params <= STALFOS_TYPE_CEILING) || (thisx->child == NULL)) {
        SkelAnime_DrawSkeletonOpa(play, &this->skelAnime, OoT_EnTest_OverrideLimbDraw, EnTest_PostLimbDraw, this);
    }
...
void func_80864158(EnTest* this, f32 xzSpeed) {
    ...
    this->actor.world.rot.y = this->actor.shape.rot.y + 0x3FFF;
    this->timer = (OoT_Rand_ZeroOne() * 20.0f) + 15.0f;
    this->unk_7C8 = 0x18;
    EnTest_SetupAction(this, func_808628C8);
}
...
s32 EnTest_ReactToProjectile(PlayState* play, EnTest* this) {
    ...
    if (projectileActor != NULL) {
        yawToProjectile = OoT_Actor_WorldYawTowardActor(&this->actor, projectileActor) - (u16)this->actor.shape.rot.y;
        ...
        if (OoT_Math_Vec3f_DistXYZ(&this->actor.world.pos, &projectileActor->world.pos) < 200.0f) {
            if (OoT_Actor_IsTargeted(play, &this->actor) && (projectileActor->id == ACTOR_ARMS_HOOK)) {
                EnTest_SetupJumpUp(this);
