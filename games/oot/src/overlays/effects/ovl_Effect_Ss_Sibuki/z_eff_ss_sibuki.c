...
u32 OoT_EffectSsSibuki_Init(PlayState* play, u32 index, EffectSs* this, void* initParamsx);
void OoT_EffectSsSibuki_Draw(PlayState* play, u32 index, EffectSs* this);
void OoT_EffectSsSibuki_Update(PlayState* play, u32 index, EffectSs* this);

EffectSsInit Effect_Ss_Sibuki_InitVars = {
    EFFECT_SS_SIBUKI,
    OoT_EffectSsSibuki_Init,
};
...
u32 OoT_EffectSsSibuki_Init(PlayState* play, u32 index, EffectSs* this, void* initParamsx) {
    ...
    this->life = ((s16)((OoT_Rand_ZeroOne() * (500.0f + KREG(64))) * 0.01f)) + KREG(65) + 10;
    this->rMoveDelay = initParams->moveDelay + 1;
    this->draw = OoT_EffectSsSibuki_Draw;
    this->update = OoT_EffectSsSibuki_Update;
...
void OoT_EffectSsSibuki_Update(PlayState* play, u32 index, EffectSs* this) {
    ...
    if (this->rMoveDelay != 0) {
        ...
        if (this->rMoveDelay == 0) {
            yaw = OoT_Camera_GetInputDirYaw(OoT_Play_GetCamera(play, 0));
            xzVelScale = ((200.0f + KREG(20)) * 0.01f) + ((0.1f * OoT_Rand_ZeroOne()) * (KREG(23) + 20.0f));

            if (this->rDirection != 0) {
                xzVelScale *= -1.0f;
            }

            this->velocity.x = OoT_Math_CosS(yaw) * xzVelScale;
            this->velocity.z = -OoT_Math_SinS(yaw) * xzVelScale;

            this->velocity.y = ((700.0f + KREG(21)) * 0.01f) + ((0.1f * OoT_Rand_ZeroOne()) * (KREG(24) + 20.0f));
            this->accel.y = ((-100.0f + KREG(22)) * 0.01f) + ((0.1f * OoT_Rand_ZeroOne()) * KREG(25));
