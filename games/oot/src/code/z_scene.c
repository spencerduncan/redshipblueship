...
s32 Object_Spawn(ObjectContext* objectCtx, s16 objectId) {
    ...
    DmaMgr_SendRequest1(objectCtx->status[objectCtx->num].segment, OoT_gObjectTable[objectId].vromStart, size, __FILE__,
                        __LINE__);
...
static u8 OoT_sObjectFirstUpdateSkippedForScene = false;
...
void Object_InitBank(PlayState* play, ObjectContext* objectCtx) {
    ...
    OoT_gSegments[4] = VIRTUAL_TO_PHYSICAL(objectCtx->status[objectCtx->mainKeepIndex].segment);

    OoT_sObjectFirstUpdateSkippedForScene = false;
}
...
void Object_UpdateBank(ObjectContext* objectCtx) {
    ...
    if (!OoT_sObjectFirstUpdateSkippedForScene) {
        OoT_sObjectFirstUpdateSkippedForScene = true;
        return;
    }
    ...
    for (i = 0; i < objectCtx->num; i++) {
        if (status->id < 0) {
            ...
            if (status->dmaRequest.vromAddr == 0) {
                OoT_osCreateMesgQueue(&status->loadQueue, &status->loadMsg, 1);
                objectFile = &OoT_gObjectTable[-status->id];
            ...
            } else if (!OoT_osRecvMesg(&status->loadQueue, NULL, OS_MESG_NOBLOCK)) {
                status->id = -status->id;
            }
            */
            status->id = -status->id;
...
void OoT_Scene_CommandSpawnList(PlayState* play, SceneCmd* cmd) {
...
void OoT_Scene_CommandActorList(PlayState* play, SceneCmd* cmd) {
...
void OoT_Scene_CommandCollisionHeader(PlayState* play, SceneCmd* cmd) {
    ...
    OoT_BgCheck_Allocate(&play->colCtx, play, colHeader);
}

void OoT_Scene_CommandRoomList(PlayState* play, SceneCmd* cmd) {
    play->numRooms = cmd->roomList.num;
    play->roomList = SEGMENTED_TO_VIRTUAL(cmd->roomList.segment);
}

void OoT_Scene_CommandEntranceList(PlayState* play, SceneCmd* cmd) {
    play->setupEntranceList = SEGMENTED_TO_VIRTUAL(cmd->entranceList.segment);
}

void OoT_Scene_CommandSpecialFiles(PlayState* play, SceneCmd* cmd) {
    ...
    if (cmd->specialFiles.cUpElfMsgNum != 0) {
        play->cUpElfMsgs = OoT_Play_LoadFile(play, &sNaviMsgFiles[cmd->specialFiles.cUpElfMsgNum - 1]);
    }
}

void OoT_Scene_CommandRoomBehavior(PlayState* play, SceneCmd* cmd) {
...
void OoT_Scene_CommandObjectList(PlayState* play, SceneCmd* cmd) {
    s32 i;
    s32 j;
    s32 k;
    ObjectStatus* status;
...
void OoT_Scene_CommandLightList(PlayState* play, SceneCmd* cmd) {
    s32 i;
    LightInfo* lightInfo = SEGMENTED_TO_VIRTUAL(cmd->lightList.segment);

    for (i = 0; i < cmd->lightList.num; i++) {
        OoT_LightContext_InsertLight(play, &play->lightCtx, lightInfo);
        lightInfo++;
    }
}

void OoT_Scene_CommandPathList(PlayState* play, SceneCmd* cmd) {
...
void OoT_Scene_CommandSkyboxSettings(PlayState* play, SceneCmd* cmd) {
...
void OoT_Scene_CommandSkyboxDisables(PlayState* play, SceneCmd* cmd) {
    play->envCtx.skyboxDisabled = cmd->skyboxDisables.unk_04;
    play->envCtx.sunMoonDisabled = cmd->skyboxDisables.unk_05;
}

void OoT_Scene_CommandTimeSettings(PlayState* play, SceneCmd* cmd) {
...
void OoT_Scene_CommandWindSettings(PlayState* play, SceneCmd* cmd) {
...
void OoT_Scene_CommandExitList(PlayState* play, SceneCmd* cmd) {
...
void OoT_Scene_CommandSoundSettings(PlayState* play, SceneCmd* cmd) {
...
void Scene_CommandAlternateHeaderList(PlayState* play, SceneCmd* cmd) {
    ...
    if (gSaveContext.sceneSetupIndex != 0) {
        ...
        if (altHeader != NULL) {
            OoT_Scene_ExecuteCommands(play, SEGMENTED_TO_VIRTUAL(altHeader));
...
void Scene_CommandMiscSettings(PlayState* play, SceneCmd* cmd) {
    ...
    if (((play->sceneNum >= SCENE_HYRULE_FIELD) && (play->sceneNum <= SCENE_OUTSIDE_GANONS_CASTLE)) ||
        ...
        if (gSaveContext.cutsceneIndex < 0xFFF0) {
            gSaveContext.worldMapAreaData |= OoT_gBitFlags[gSaveContext.worldMapArea];
...
u32 OoT_gObjectTableSize = ARRAY_COUNT(OoT_gObjectTable);
...
#define OoT_DEFINE_OBJECT_UNSET(_0)
...
#define OoT_DEFINE_OBJECT_UNSET(_0) { 0, 0, "" },

RomFile OoT_gObjectTable[] = {
#include "tables/object_table.h"
};
