...
u8 OoT_gPlayerModelTypes[PLAYER_MODELGROUP_MAX][PLAYER_MODELGROUPENTRY_MAX] = {
    /* PLAYER_MODELGROUP_0 */
...
Gfx* OoT_gPlayerLeftHandOpenDLs[] = {
...
Gfx* OoT_gPlayerLeftHandClosedDLs[] = {
...
Gfx** OoT_sPlayerDListGroups[PLAYER_MODELTYPE_MAX] = {
    OoT_gPlayerLeftHandOpenDLs,           // PLAYER_MODELTYPE_LH_OPEN
    OoT_gPlayerLeftHandClosedDLs,         // PLAYER_MODELTYPE_LH_CLOSED
...
Gfx OoT_gCullBackDList[] = {
    gsSPSetGeometryMode(G_CULL_BACK),
    gsSPEndDisplayList(),
};

Gfx OoT_gCullFrontDList[] = {
...
s32 OoT_Player_InBlockingCsMode(PlayState* play, Player* this) {
...
s32 OoT_Player_InCsMode(PlayState* play) {
    ...
    return OoT_Player_InBlockingCsMode(play, this) || (this->unk_6AD == 4);
}
 ...
 *
 * Note that within Player, `OoT_Player_UpdateHostileLockOn` exists, which updates the flag and also returns the check.
 * Player can use this function instead if the flag should be checked, but not updated.
 */
s32 OoT_Player_CheckHostileLockOn(Player* this) {
...
s32 OoT_Player_ActionToModelGroup(Player* this, s32 actionParam) {
    s32 modelGroup = OoT_sActionModelGroups[actionParam];
...
void OoT_Player_SetModelsForHoldingShield(Player* this) {
    if ((this->stateFlags1 & PLAYER_STATE1_SHIELDING) &&
        ...
        if ((CVarGetInteger(CVAR_CHEAT("ShieldTwoHanded"), 0) && (this->heldItemAction != PLAYER_IA_DEKU_STICK) ||
            ...
            if (LINK_IS_CHILD && (CVarGetInteger(CVAR_ENHANCEMENT("EquipmentAlwaysVisible"), 0)) &&
                ...
                this->rightHandDLists = &OoT_sPlayerDListGroups[PLAYER_MODELTYPE_RH_SHIELD][0];
            } else if (LINK_IS_ADULT && (CVarGetInteger(CVAR_ENHANCEMENT("EquipmentAlwaysVisible"), 0)) &&
                       (this->currentShield == PLAYER_SHIELD_DEKU)) {
                this->rightHandDLists = &OoT_sPlayerDListGroups[PLAYER_MODELTYPE_RH_SHIELD][1];
            } else {
                this->rightHandDLists = &OoT_sPlayerDListGroups[PLAYER_MODELTYPE_RH_SHIELD][gSaveContext.linkAge];
            }
            ...
            this->sheathDLists = &OoT_sPlayerDListGroups[this->sheathType][gSaveContext.linkAge];
            if ((CVarGetInteger(CVAR_ENHANCEMENT("EquipmentAlwaysVisible"), 0)) && LINK_IS_CHILD &&
                gSaveContext.equips.buttonItems[0] != ITEM_SWORD_KOKIRI) {
                this->sheathDLists = &OoT_sPlayerDListGroups[this->sheathType][0];
            } else if ((CVarGetInteger(CVAR_ENHANCEMENT("EquipmentAlwaysVisible"), 0)) && LINK_IS_ADULT &&
                       gSaveContext.equips.buttonItems[0] == ITEM_SWORD_KOKIRI) {
                this->sheathDLists = &OoT_sPlayerDListGroups[this->sheathType][1];
            }
...
void OoT_Player_SetModels(Player* this, s32 modelGroup) {
    // Left hand
    this->leftHandType = OoT_gPlayerModelTypes[modelGroup][PLAYER_MODELGROUPENTRY_LEFT_HAND];
    this->leftHandDLists = &OoT_sPlayerDListGroups[this->leftHandType][gSaveContext.linkAge];
        ...
        if (LINK_IS_CHILD &&
            (this->leftHandType == PLAYER_MODELTYPE_LH_HAMMER ||
             ((this->leftHandType == PLAYER_MODELTYPE_LH_SWORD || this->leftHandType == PLAYER_MODELTYPE_LH_BGS) &&
              (gSaveContext.equips.buttonItems[0] != ITEM_SWORD_KOKIRI)))) {
            this->leftHandDLists = &OoT_sPlayerDListGroups[this->leftHandType][0];
        }

        if (LINK_IS_ADULT && (this->leftHandType == PLAYER_MODELTYPE_LH_BOOMERANG ||
                              (this->leftHandType == PLAYER_MODELTYPE_LH_SWORD &&
                               gSaveContext.equips.buttonItems[0] == ITEM_SWORD_KOKIRI))) {
            this->leftHandDLists = &OoT_sPlayerDListGroups[this->leftHandType][1];
        }
    }

    // Right hand
    this->rightHandType = OoT_gPlayerModelTypes[modelGroup][PLAYER_MODELGROUPENTRY_RIGHT_HAND];
    this->rightHandDLists = &OoT_sPlayerDListGroups[this->rightHandType][gSaveContext.linkAge];
    ...
    if (CVarGetInteger(CVAR_ENHANCEMENT("EquipmentAlwaysVisible"), 0)) {
        if (LINK_IS_CHILD &&
            ...
            this->rightHandDLists = &OoT_sPlayerDListGroups[this->rightHandType][0];
        }
        if (LINK_IS_ADULT &&
            (this->rightHandType == PLAYER_MODELTYPE_RH_SHIELD && this->currentShield == PLAYER_SHIELD_DEKU)) {
            this->rightHandDLists = &OoT_sPlayerDListGroups[this->rightHandType][1];
        }
    }
    if ((CVarGetInteger(CVAR_ENHANCEMENT("BowSlingshotAmmoFix"), 0) ||
        ...
        this->rightHandDLists = &OoT_sPlayerDListGroups[this->rightHandType][Player_HoldsSlingshot(this)];
    }

    // Sheath
    this->sheathType = OoT_gPlayerModelTypes[modelGroup][PLAYER_MODELGROUPENTRY_SHEATH];
    this->sheathDLists = &OoT_sPlayerDListGroups[this->sheathType][gSaveContext.linkAge];
    ...
    // Waist
    this->waistDLists = &OoT_sPlayerDListGroups[OoT_gPlayerModelTypes[modelGroup][4]][gSaveContext.linkAge];

    OoT_Player_SetModelsForHoldingShield(this);
    GameInteractor_ExecuteOnPlayerSetModels(this, modelGroup);
}

void OoT_Player_SetModelGroup(Player* this, s32 modelGroup) {
    this->modelGroup = modelGroup;

    if (modelGroup == PLAYER_MODELGROUP_CHILD_HYLIAN_SHIELD) {
        this->modelAnimType = PLAYER_ANIMTYPE_0;
    } else {
        this->modelAnimType = OoT_gPlayerModelTypes[modelGroup][PLAYER_MODELGROUPENTRY_ANIM];
    }
    ...
    OoT_Player_SetModels(this, modelGroup);
}

void func_8008EC70(Player* this) {
    this->itemAction = this->heldItemAction;
    OoT_Player_SetModelGroup(this, OoT_Player_ActionToModelGroup(this, this->heldItemAction));
    this->unk_6AD = 0;
}

void OoT_Player_SetEquipmentData(PlayState* play, Player* this) {
...
void OoT_Player_UpdateBottleHeld(PlayState* play, Player* this, s32 item, s32 actionParam) {
    OoT_Inventory_UpdateBottleItem(play, item, this->heldItemButton);
...
void OoT_Player_ReleaseLockOn(Player* this) {
 ...
 */
void OoT_Player_ClearZTargeting(Player* this) {
...
void OoT_Player_SetAutoLockOnActor(PlayState* play, Actor* actor) {
    ...
    Camera_SetParam(OoT_Play_GetCamera(play, 0), 8, actor);
    OoT_Camera_ChangeMode(OoT_Play_GetCamera(play, 0), 2);
}
...
s32 OoT_Player_IsBurningStickInRange(PlayState* play, Vec3f* pos, f32 xzRange, f32 yRange) {
    Player* this = GET_PLAYER(play);
    Vec3f diff;
    s32 pad;

    if ((this->heldItemAction == PLAYER_IA_DEKU_STICK) && (this->unk_860 != 0)) {
        OoT_Math_Vec3f_Diff(&this->meleeWeaponInfo[0].tip, pos, &diff);
...
s32 OoT_Player_GetMeleeWeaponHeld(Player* this) {
...
s32 OoT_Player_GetBottleHeld(Player* this) {
...
s32 OoT_Player_GetExplosiveHeld(Player* this) {
            ...
            (((envHazard == (PLAYER_ENV_HAZARD_HOTROOM - 1)) &&
             ...
             (((envHazard == (PLAYER_ENV_HAZARD_UNDERWATER_FLOOR - 1)) ||
              ...
              (this->currentTunic != PLAYER_TUNIC_ZORA && CVarGetInteger(CVAR_CHEAT("SuperTunic"), 0) == 0 &&
               CVarGetInteger(CVAR_ENHANCEMENT("DisableTunicWarningText"), 0) == 0)))) {
            OoT_Message_StartTextbox(play, triggerEntry->textId, NULL);
...
void OoT_Player_DrawImpl(PlayState* play, void** skeleton, Vec3s* jointTable, s32 dListCount, s32 lod, s32 tunic, s32 boots,
...
#if defined(MODDING) || defined(_MSC_VER) || defined(__GNUC__)
    gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(OoT_sEyeTextures[gSaveContext.linkAge][eyeIndex]));
#else
    gSPSegment(POLY_OPA_DISP++, 0x08, SEGMENTED_TO_VIRTUAL(OoT_sEyeTextures[eyeIndex]));
#endif
    if (mouthIndex < 0) {
...
#if defined(MODDING) || defined(_MSC_VER) || defined(__GNUC__)
    gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(OoT_sMouthTextures[gSaveContext.linkAge][mouthIndex]));
#else
    gSPSegment(POLY_OPA_DISP++, 0x09, SEGMENTED_TO_VIRTUAL(OoT_sMouthTextures[eyeIndex]));
#endif
    ...
    OoT_SkelAnime_DrawFlexLod(play, skeleton, jointTable, dListCount, overrideLimbDraw, postLimbDraw, data, lod);
    ...
    if (((CVarGetInteger(CVAR_ENHANCEMENT("FirstPersonGauntlets"), 0) && LINK_IS_ADULT) ||
        ...
        if (LINK_IS_ADULT) {
            ...
            if (OoT_Player_GetStrength() > PLAYER_STR_NONE) {
...
Vec3f OoT_sZeroVec = { 0.0f, 0.0f, 0.0f };
...
void func_8008F87C(PlayState* play, Player* this, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot, s32 thighLimbIndex,
    ...
    if ((this->actor.scale.y >= 0.0f) && !(this->stateFlags1 & PLAYER_STATE1_DEAD) &&
        ...
        OoT_Matrix_MultVec3f(&OoT_sZeroVec, &spA4);
        OoT_Matrix_TranslateRotateZYX(&D_80126038[gSaveContext.linkAge], &skelAnime->jointTable[shinLimbIndex]);
        OoT_Matrix_Translate(D_80126050[gSaveContext.linkAge], 0.0f, 0.0f, MTXMODE_APPLY);
        OoT_Matrix_MultVec3f(&OoT_sZeroVec, &sp98);
        OoT_Matrix_MultVec3f(&D_80126070, &footprintPos);
        OoT_Matrix_Pop();
        ...
        if (sp98.y < sp80) {
            ...
            temp1 = (M_PI - (OoT_Math_FAtan2F(sp5C, sp58) + ((M_PI / 2) - sp50))) * (0x8000 / M_PI);
            ...
            if ((temp3 >= 2) && (temp3 < 4) && !OoT_SurfaceType_IsWallDamage(&play->colCtx, sp88, sp84)) {
                footprintPos.y = sp80;
                OoT_EffectSsGFire_Spawn(play, &footprintPos);
            }
        }
    }
}

s32 OoT_Player_OverrideLimbDrawGameplayCommon(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
                                          void* thisx) {
    ...
    if (CVarGetInteger(CVAR_ENHANCEMENT("EquipmentAlwaysVisible"), 0) &&
        ...
        if (limbIndex == PLAYER_LIMB_SHEATH) {
            if ((this->currentShield == PLAYER_SHIELD_MIRROR ||
                ...
                OoT_Matrix_Translate(218, -100, 62, MTXMODE_APPLY);
                OoT_Matrix_Scale(0.8, 0.8, 0.8, MTXMODE_APPLY);
            }
    ...
    if (limbIndex == PLAYER_LIMB_ROOT) {
        ...
        if (this->unk_6C2 != 0) {
            OoT_Matrix_Translate(pos->x, ((OoT_Math_CosS(this->unk_6C2) - 1.0f) * 200.0f) + pos->y, pos->z, MTXMODE_APPLY);
        ...
        if (limbIndex == PLAYER_LIMB_HEAD) {
            if (CVarGetInteger(CVAR_COSMETIC("Link.HeadScale.Changed"), 0)) {
                ...
                if (scale > 1.2f) {
                    OoT_Matrix_Translate(-((LINK_IS_ADULT ? 320.0f : 200.0f) * scale), 0.0f, 0.0f, MTXMODE_APPLY);
                } else if (scale < 1.0f) {
                    OoT_Matrix_Translate((LINK_IS_ADULT ? 3600.0f : 2900.0f) * ABS(scale - 1.0f), 0.0f, 0.0f,
                                     MTXMODE_APPLY);
                }
            }
...
s32 OoT_Player_OverrideLimbDrawGameplayDefault(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
                                           void* thisx) {
    Player* this = (Player*)thisx;

    if (!OoT_Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, thisx)) {
        if (limbIndex == PLAYER_LIMB_L_HAND) {
            ...
            if ((sLeftHandType == PLAYER_MODELTYPE_LH_BGS) && (gSaveContext.swordHealth <= 0.0f)) {
                ...
                dLists = &OoT_gPlayerLeftHandOpenDLs[gSaveContext.linkAge];
                ...
                dLists = &OoT_gPlayerLeftHandClosedDLs[gSaveContext.linkAge];
...
s32 OoT_Player_OverrideLimbDrawGameplayFirstPerson(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
                                               void* thisx) {
    Player* this = (Player*)thisx;
...
s32 OoT_Player_OverrideLimbDrawGameplayCrawling(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot,
                                            void* thisx) {
    if (!OoT_Player_OverrideLimbDrawGameplayCommon(play, limbIndex, dList, pos, rot, thisx)) {
        *dList = NULL;
    }
...
u8 func_80090480(PlayState* play, ColliderQuad* collider, WeaponInfo* weaponInfo, Vec3f* newTip, Vec3f* newBase) {
    if (weaponInfo->active == 0) {
        if (collider != NULL) {
            OoT_Collider_ResetQuadAT(play, &collider->base);
        }
        OoT_Math_Vec3f_Copy(&weaponInfo->tip, newTip);
        OoT_Math_Vec3f_Copy(&weaponInfo->base, newBase);
        weaponInfo->active = 1;
        return 1;
        ...
        if (collider != NULL) {
            OoT_Collider_SetQuadVertices(collider, newBase, newTip, &weaponInfo->base, &weaponInfo->tip);
...
void OoT_Player_UpdateShieldCollider(PlayState* play, Player* this, ColliderQuad* collider, Vec3f* quadSrc) {
    ...
    if (this->stateFlags1 & PLAYER_STATE1_SHIELDING) {
        ...
        OoT_Matrix_MultVec3f(&quadSrc[0], &quadDest[0]);
        OoT_Matrix_MultVec3f(&quadSrc[1], &quadDest[1]);
        OoT_Matrix_MultVec3f(&quadSrc[2], &quadDest[2]);
        OoT_Matrix_MultVec3f(&quadSrc[3], &quadDest[3]);
        OoT_Collider_SetQuadVertices(collider, &quadDest[0], &quadDest[1], &quadDest[2], &quadDest[3]);
...
void func_800906D4(PlayState* play, Player* this, Vec3f* newTipPos) {
    ...
    OoT_Matrix_MultVec3f(&D_801260A4[0], &newBasePos[0]);
    OoT_Matrix_MultVec3f(&D_801260A4[1], &newBasePos[1]);
    OoT_Matrix_MultVec3f(&D_801260A4[2], &newBasePos[2]);
    ...
    if (func_80090480(play, NULL, &this->meleeWeaponInfo[0], &newTipPos[0], &newBasePos[0]) &&
        ...
        OoT_EffectBlure_AddVertex(OoT_Effect_GetByIndex(this->meleeWeaponEffectIndex), &this->meleeWeaponInfo[0].tip,
...
void Player_DrawGetItemIceTrap(PlayState* play, Player* this, Vec3f* refPos, s32 drawIdPlusOne, f32 height) {
    ...
    if (CVarGetInteger(CVAR_GENERAL("LetItSnow"), 0)) {
        ...
        OoT_Matrix_Translate(0.0f, -40.0f, 0.0f, MTXMODE_APPLY);
        OoT_Matrix_Scale(iceTrapScale, iceTrapScale, iceTrapScale, MTXMODE_APPLY);
        ...
        OoT_Matrix_Translate(refPos->x + (3.3f * OoT_Math_SinS(this->actor.shape.rot.y)), refPos->y + height,
                         refPos->z + ((3.3f + (IREG(90) / 10.0f)) * OoT_Math_CosS(this->actor.shape.rot.y)), MTXMODE_NEW);
        OoT_Matrix_RotateZYX(0, play->gameplayFrames * 1000, 0, MTXMODE_APPLY);
        OoT_Matrix_Scale(0.2f, 0.2f, 0.2f, MTXMODE_APPLY);
        // Draw fake item model.
        if (this->getItemEntry.drawFunc != NULL) {
            this->getItemEntry.drawFunc(play, &this->getItemEntry);
        } else {
            OoT_GetItem_Draw(play, drawIdPlusOne - 1);
        }
    }
...
void OoT_Player_DrawGetItemImpl(PlayState* play, Player* this, Vec3f* refPos, s32 drawIdPlusOne) {
    f32 height = (this->exchangeItemId != EXCH_ITEM_NONE) ? 6.0f : 14.0f;

    OPEN_DISPS(play->state.gfxCtx);

    OoT_gSegments[6] = VIRTUAL_TO_PHYSICAL(this->giObjectSegment);
...
void func_80090A28(Player* this, Vec3f* vecs) {
    ...
    OoT_Matrix_MultVec3f(&D_80126080, &vecs[0]);
    OoT_Matrix_MultVec3f(&D_8012608C, &vecs[1]);
    OoT_Matrix_MultVec3f(&D_80126098, &vecs[2]);
}

void OoT_Player_DrawHookshotReticle(PlayState* play, Player* this, f32 hookshotRange) {
    ...
    D_801260C8.z = 0.0f;
    OoT_Matrix_MultVec3f(&D_801260C8, &hookshotStart);
    D_801260C8.z = hookshotRange;
    OoT_Matrix_MultVec3f(&D_801260C8, &hookshotEnd);

    if (OoT_BgCheck_AnyLineTest3(&play->colCtx, &hookshotStart, &hookshotEnd, &firstHit, &colPoly, 1, 1, 1, 1, &bgId)) {
        OPEN_DISPS(play->state.gfxCtx);

        OVERLAY_DISP = OoT_Gfx_SetupDL(OVERLAY_DISP, 0x07);

        OoT_SkinMatrix_Vec3fMtxFMultXYZW(&play->viewProjectionMtxF, &firstHit, &sp68, &sp64);

        const f32 sp60 = (sp64 < 200.0f) ? 0.08f : (sp64 / 200.0f) * 0.08f;

        OoT_Matrix_Translate(firstHit.x, firstHit.y, firstHit.z, MTXMODE_NEW);
        OoT_Matrix_Scale(sp60, sp60, sp60, MTXMODE_APPLY);
        ...
        if (OoT_SurfaceType_IsHookshotSurface(&play->colCtx, colPoly, bgId) &&
...
void OoT_Player_PostLimbDrawGameplay(PlayState* play, s32 limbIndex, Gfx** dList, Vec3s* rot, void* thisx) {
    Player* this = (Player*)thisx;
    ...
    if (*dList != NULL) {
        OoT_Matrix_MultVec3f(&OoT_sZeroVec, D_80160000);
    }
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        ...
        OoT_Math_Vec3f_Copy(&this->leftHandPos, D_80160000);
        ...
        if (this->itemAction == PLAYER_IA_DEKU_STICK) {
            ...
            if (this->actor.scale.y >= 0.0f) {
                ...
                if (this->meleeWeaponState != 0) {
                    EffectBlure_ChangeType(OoT_Effect_GetByIndex(this->meleeWeaponEffectIndex), TRAIL_TYPE_STICK);
                    func_800906D4(play, this, sp124);
                } else {
                    OoT_Math_Vec3f_Copy(&this->meleeWeaponInfo[0].tip, &sp124[0]);
                }
            }

            OoT_Matrix_Translate(-428.26f, 267.2f, -33.82f, MTXMODE_APPLY);
            OoT_Matrix_RotateZYX(-0x8000, 0, 0x4000, MTXMODE_APPLY);
            OoT_Matrix_Scale(1.0f, this->unk_85C, 1.0f, MTXMODE_APPLY);
            ...
            if (Player_HoldsBrokenKnife(this)) {
                ...
                D_80126080.x = sMeleeWeaponLengths[OoT_Player_GetMeleeWeaponHeld(this)];
                EffectBlure_ChangeType(OoT_Effect_GetByIndex(this->meleeWeaponEffectIndex),
                                       sSwordTypes[OoT_Player_GetMeleeWeaponHeld(this)]);
            }
        ...
        if (this->actor.scale.y >= 0.0f) {
            if (!Player_HoldsHookshot(this) && ((hookedActor = this->heldActor) != NULL)) {
                if (this->stateFlags1 & PLAYER_STATE1_READY_TO_FIRE) {
                    OoT_Matrix_MultVec3f(&sLeftHandArrowVec3, &hookedActor->world.pos);
                    OoT_Matrix_RotateZYX(0x69E8, -0x5708, 0x458E, MTXMODE_APPLY);
                    OoT_Matrix_Get(&sp14C);
        ...
        if (this->rightHandType == PLAYER_MODELTYPE_RH_FF) {
            ...
            OoT_Matrix_Push();
            OoT_Matrix_Translate(stringData->pos.x, stringData->pos.y, stringData->pos.z, MTXMODE_APPLY);

            if ((this->stateFlags1 & PLAYER_STATE1_READY_TO_FIRE) && (this->unk_860 >= 0) && (this->unk_834 <= 10)) {
                Vec3f sp90;
                f32 distXYZ;

                OoT_Matrix_MultVec3f(&OoT_sZeroVec, &sp90);
                distXYZ = OoT_Math_Vec3f_DistXYZ(D_80160000, &sp90);
            ...
            OoT_Matrix_Scale(1.0f, this->unk_858, 1.0f, MTXMODE_APPLY);
            ...
            OoT_Matrix_Get(&this->shieldMf);
            OoT_Player_UpdateShieldCollider(play, this, &this->shieldQuad, sRightHandLimbModelShieldQuadVertices);
        }

        if (this->actor.scale.y >= 0.0f) {
            if ((this->heldItemAction == PLAYER_IA_HOOKSHOT) || (this->heldItemAction == PLAYER_IA_LONGSHOT)) {
                OoT_Matrix_MultVec3f(&D_80126184, &this->unk_3C8);

                if (heldActor != NULL) {
                    MtxF sp44;
                    s32 pad;

                    OoT_Matrix_MultVec3f(&D_80126190, &heldActor->world.pos);
                    ...
                    if (func_8002DD78(this) != 0) {
                        OoT_Matrix_Translate(500.0f, 300.0f, 0.0f, MTXMODE_APPLY);
                        OoT_Player_DrawHookshotReticle(
                            play, this,
                ...
                if (heldActor != NULL) {
                    ...
                    OoT_Matrix_RotateZYX(0, -15216, -17496, MTXMODE_APPLY);
                    OoT_Matrix_Get(&sp44);

                    if (func_8002DD78(this) != 0) {
                        OoT_Matrix_Translate(500.0f, 300.0f, 0.0f, MTXMODE_APPLY);
                        OoT_Player_DrawHookshotReticle(play, this, RETICLE_MAX);
                    }
                }
                ...
                if (Player_HoldsBoomerang(this)) {
                    if (LINK_IS_ADULT) {
                        OoT_Matrix_RotateZYX(BoomerangViewAdult.x, BoomerangViewAdult.y, BoomerangViewAdult.z,
                                         MTXMODE_APPLY);
                    } else {
                        OoT_Matrix_RotateZYX(BoomerangViewChild.x, BoomerangViewChild.y, BoomerangViewChild.z,
                                         MTXMODE_APPLY);
                    }
            ...
            if ((this->unk_862 != 0) || ((func_8002DD6C(this) == 0) && (heldActor != NULL))) {
                if (!(this->stateFlags1 & PLAYER_STATE1_GETTING_ITEM) && (this->unk_862 != 0) &&
                    ...
                    OoT_Math_Vec3f_Copy(&sGetItemRefPos, &this->leftHandPos);
                } else {
                ...
                if (this->unk_862 == 0) {
                    OoT_Math_Vec3f_Copy(&heldActor->world.pos, &sGetItemRefPos);
                }
            }
        }
        ...
        if (limbIndex == PLAYER_LIMB_SHEATH) {
            if ((this->rightHandType != PLAYER_MODELTYPE_RH_SHIELD) &&
                ...
                if (Player_IsChildWithHylianShield(this)) {
                    OoT_Player_UpdateShieldCollider(play, this, &this->shieldQuad, sSheathLimbModelShieldQuadVertices);
                }

                OoT_Matrix_TranslateRotateZYX(&sSheathLimbModelShieldOnBackPos, &sSheathLimbModelShieldOnBackZyxRot);
                OoT_Matrix_Get(&this->shieldMf);
            }
        } else if (limbIndex == PLAYER_LIMB_HEAD) {
            OoT_Matrix_MultVec3f(&D_801260D4, &this->actor.focus.pos);
        } else {
            Vec3f* vec = &sLeftRightFootLimbModelFootPos[(gSaveContext.linkAge)];

            OoT_Actor_SetFeetPos(&this->actor, limbIndex, PLAYER_LIMB_L_FOOT, vec, PLAYER_LIMB_R_FOOT, vec);
        }
    }
}
...
u32 func_80091738(PlayState* play, u8* segment, SkelAnime* skelAnime) {
    ...
    size = OoT_gObjectTable[OBJECT_GAMEPLAY_KEEP].vromEnd - OoT_gObjectTable[OBJECT_GAMEPLAY_KEEP].vromStart;
    ptr = segment + 0x3800;
    DmaMgr_SendRequest1(ptr, OoT_gObjectTable[OBJECT_GAMEPLAY_KEEP].vromStart, size, __FILE__, __LINE__);

    size = OoT_gObjectTable[linkObjectId].vromEnd - OoT_gObjectTable[linkObjectId].vromStart;
    ptr = segment + 0x8800;
    DmaMgr_SendRequest1(ptr, OoT_gObjectTable[linkObjectId].vromStart, size, __FILE__, __LINE__);

    ptr = (void*)ALIGN16((intptr_t)ptr + size);

    OoT_gSegments[4] = VIRTUAL_TO_PHYSICAL(segment + 0x3800);
    OoT_gSegments[6] = VIRTUAL_TO_PHYSICAL(segment + 0x8800);
...
s32 Player_OverrideLimbDrawPause(PlayState* play, s32 limbIndex, Gfx** dList, Vec3f* pos, Vec3s* rot, void* arg) {
    ...
    if (limbIndex == PLAYER_LIMB_L_HAND) {
        type = OoT_gPlayerModelTypes[modelGroup][PLAYER_MODELGROUPENTRY_LEFT_HAND];
        sLeftHandType = type;
        ...
        type = OoT_gPlayerModelTypes[modelGroup][PLAYER_MODELGROUPENTRY_RIGHT_HAND];
        sRightHandType = type;
        if (type == PLAYER_MODELTYPE_RH_SHIELD) {
            dListOffset = playerSwordAndShield[1] * ptrSize;
        }
    } else if (limbIndex == PLAYER_LIMB_SHEATH) {
        type = OoT_gPlayerModelTypes[modelGroup][PLAYER_MODELGROUPENTRY_SHEATH];
        if ((type == PLAYER_MODELTYPE_SHEATH_18) || (type == PLAYER_MODELTYPE_SHEATH_19)) {
            dListOffset = playerSwordAndShield[1] * ptrSize;
        }
    } else if (limbIndex == PLAYER_LIMB_WAIST) {
        type = OoT_gPlayerModelTypes[modelGroup][PLAYER_MODELGROUPENTRY_WAIST];
    ...
    dLists = &OoT_sPlayerDListGroups[type][gSaveContext.linkAge];
...
void Player_DrawPauseImpl(PlayState* play, void* gameplayKeep, void* linkObject, SkelAnime* skelAnime, Vec3f* pos,
    ...
    OoT_Matrix_Scale(scale * (mirrorWorldActive ? -1 : 1), scale, scale, MTXMODE_APPLY);
    ...
    POLY_OPA_DISP = OoT_Gfx_SetFog2(POLY_OPA_DISP++, 0, 0, 0, 0, 997, 1000);

    func_8002EABC(pos, &play->view.eye, &lightDir, play->state.gfxCtx);

    gSPSegment(POLY_OPA_DISP++, 0x0C, OoT_gCullBackDList);

    OoT_Player_DrawImpl(play, skelAnime->skeleton, skelAnime->jointTable, skelAnime->dListCount, 0, tunic, boots, 0,
                    Player_OverrideLimbDrawPause, NULL, &playerSwordAndShield);

    if (CVarGetInteger(CVAR_GENERAL("PauseMenuAnimatedLinkTriforce"), 0)) {
        OoT_Matrix_SetTranslateRotateYXZ(pos->x - (LINK_AGE_IN_YEARS == YEARS_ADULT ? 25 : 0),
                                     pos->y + 280 + (LINK_AGE_IN_YEARS == YEARS_ADULT ? 48 : 0), pos->z, rot);
        OoT_Matrix_Scale(scale * (mirrorWorldActive ? -1 : 1), scale * 1, scale * 1, MTXMODE_APPLY);
...
void Player_DrawPause(PlayState* play, u8* segment, SkelAnime* skelAnime, Vec3f* pos, Vec3s* rot, f32 scale, s32 sword,
    ...
    if (CVarGetInteger(CVAR_ENHANCEMENT("PauseMenuAnimatedLink"), 0) ||
        ...
        if (skelAnime->animation != anim) {
            LinkAnimation_Change(play, skelAnime, anim, 1.0f, 0.0f, OoT_Animation_GetLastFrame(anim), ANIMMODE_LOOP, -6.0f);
        }
