...
u16 OoT_Math_GetAtan2Tbl(f32 x, f32 y) {
    s32 tblIdx = ((x / y) * 1024.0f) + 0.5f;
    u16 ret;

    if (y == 0.0f) {
        ret = OoT_sATan2Tbl[0];
    } else if (tblIdx >= ARRAY_COUNT(OoT_sATan2Tbl)) {
        ret = OoT_sATan2Tbl[0];
    } else {
        ret = OoT_sATan2Tbl[tblIdx];
    }
    return ret;
}
...
s16 OoT_Math_Atan2S(f32 x, f32 y) {
    ...
    if (y >= 0.0f) {
        if (x >= 0.0f) {
            if (y <= x) {
                ret = OoT_Math_GetAtan2Tbl(y, x);
            } else {
                ret = 0x4000 - OoT_Math_GetAtan2Tbl(x, y);
            }
        } else {
            if (-x < y) {
                ret = OoT_Math_GetAtan2Tbl(-x, y) + 0x4000;
            } else {
                ret = 0x8000 - OoT_Math_GetAtan2Tbl(y, -x);
            }
        }
    } else {
        if (x < 0.0f) {
            if (-y <= -x) {
                ret = OoT_Math_GetAtan2Tbl(-y, -x) + 0x8000;
            } else {
                ret = 0xC000 - OoT_Math_GetAtan2Tbl(-x, -y);
            }
        } else {
            if (x < -y) {
                ret = OoT_Math_GetAtan2Tbl(x, -y) + 0xC000;
            } else {
                ret = -OoT_Math_GetAtan2Tbl(-y, x);
            }
        }
...
f32 OoT_Math_Atan2F(f32 x, f32 y) {
    return OoT_Math_Atan2S(x, y) * (M_PI / 32768.0f);
}
