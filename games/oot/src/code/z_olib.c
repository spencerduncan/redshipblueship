...
f32 OoT_OLib_Vec3fDistOutDiff(Vec3f* a, Vec3f* b, Vec3f* dest) {
    dest->x = a->x - b->x;
    dest->y = a->y - b->y;
    dest->z = a->z - b->z;
...
/**
 * Clamps `val` to a maximum of -`min` as `val` approaches OoT_zero, and a minimum of
 * `min` as `val` approaches OoT_zero
 */
 ...
 */
Vec3f* OoT_OLib_VecSphToVec3f(Vec3f* dest, VecSph* sph) {
    Vec3f v;
    f32 sinPitch;
    f32 cosPitch;
    f32 sinYaw;
    ...
    if (CVarGetInteger(CVAR_ENHANCEMENT("FixCameraDrift"), 0)) {
        ...
        cosPitch = OoT_Math_CosS(sph->pitch);
        cosYaw = OoT_Math_CosS(sph->yaw);
        sinPitch = OoT_Math_SinS(sph->pitch);
        sinYaw = OoT_Math_SinS(sph->yaw);
    }
...
Vec3f* OLib_VecSphGeoToVec3f(Vec3f* dest, VecSph* sph) {
    ...
    return OoT_OLib_VecSphToVec3f(dest, &geo);
}
...
VecSph* OoT_OLib_Vec3fToVecSph(VecSph* dest, Vec3f* vec) {
    ...
    if ((vec->x == 0.0f) && (vec->z == 0.0f)) {
        ...
        sph.yaw = DEGF_TO_BINANG(RADF_TO_DEGF(OoT_Math_FAtan2F(vec->x, vec->z)));
    }
...
VecSph* OLib_Vec3fToVecSphGeo(VecSph* dest, Vec3f* vec) {
    ...
    OoT_OLib_Vec3fToVecSph(&sph, vec);
 ...
 */
VecSph* OoT_OLib_Vec3fDiffToVecSph(VecSph* dest, Vec3f* a, Vec3f* b) {
    Vec3f sph;
    ...
    return OoT_OLib_Vec3fToVecSph(dest, &sph);
}
...
Vec3f* OoT_OLib_Vec3fDiffRad(Vec3f* dest, Vec3f* a, Vec3f* b) {
    ...
    anglesRad.x = OoT_Math_FAtan2F(b->z - a->z, b->y - a->y);
    anglesRad.y = OoT_Math_FAtan2F(b->x - a->x, b->z - a->z);
    anglesRad.z = 0;
 ...
 */
Vec3f* OoT_OLib_Vec3fDiffDegF(Vec3f* dest, Vec3f* a, Vec3f* b) {
    Vec3f anglesRad;
    Vec3f anglesDegrees;

    OoT_OLib_Vec3fDiffRad(&anglesRad, a, b);
 ...
 */
Vec3s* OoT_OLib_Vec3fDiffBinAng(Vec3s* dest, Vec3f* a, Vec3f* b) {
    Vec3f anglesRad;
    Vec3s anglesBinAng;
