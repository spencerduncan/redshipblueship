...
s32 OoT_Math3D_TriLineIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* linePointA,
                            Vec3f* linePointB, Vec3f* intersect, s32 fromFront);
s32 OoT_Math3D_PlaneVsPlaneNewLine(f32 planeAA, f32 planeAB, f32 planeAC, f32 planeADist, f32 planeBA, f32 planeBB,
...
s32 OoT_Math3D_SphCubeVsTriCube(Vec3f* v0, Vec3f* v1, Vec3f* v2, Vec3f* center, f32 radius);
...
s32 OoT_Math3D_PlaneVsLineSegClosestPoint(f32 planeAA, f32 planeAB, f32 planeAC, f32 planeADist, f32 planeBA, f32 planeBB,
    ...
    if (!OoT_Math3D_PlaneVsPlaneNewLine(planeAA, planeAB, planeAC, planeADist, planeBA, planeBB, planeBC, planeBDist,
                                    &planeIntersectLine)) {
    ...
    OoT_Math_Vec3f_Copy(&planeIntersectSeg.a, &planeIntersectLine.point);
...
void OoT_Math3D_LineClosestToPoint(Linef* line, Vec3f* pos, Vec3f* closestPoint) {
    ...
    dirVectorSize = OoT_Math3D_Vec3fMagnitudeSq(&line->b);
    if (IS_ZERO(dirVectorSize)) {
        ...
        osSyncPrintf(VT_RST);
        OoT_Math_Vec3f_Copy(closestPoint, pos);
    }
...
void OoT_Math3D_FindPointOnPlaneIntersect(f32 planeAAxis1Norm, f32 planeAAxis2Norm, f32 planeBAxis1Norm,
...
s32 OoT_Math3D_PlaneVsPlaneNewLine(f32 planeAA, f32 planeAB, f32 planeAC, f32 planeADist, f32 planeBA, f32 planeBB,
    ...
    OoT_Math3D_Vec3f_Cross(&planeANormal, &planeBNormal, &intersect->dir);
    ...
    if ((dirX >= dirY) && (dirX >= dirZ)) {
        OoT_Math3D_FindPointOnPlaneIntersect(planeAB, planeAC, planeBB, planeBC, intersect->dir.x, planeADist, planeBDist,
                                         &intersect->point.y, &intersect->point.z);
        intersect->point.x = 0.0f;
    } else if ((dirY >= dirX) && (dirY >= dirZ)) {
        OoT_Math3D_FindPointOnPlaneIntersect(planeAC, planeAA, planeBC, planeBA, intersect->dir.y, planeADist, planeBDist,
                                         &intersect->point.z, &intersect->point.x);
        intersect->point.y = 0.0f;
    } else {
        OoT_Math3D_FindPointOnPlaneIntersect(planeAA, planeAB, planeBA, planeBB, intersect->dir.z, planeADist, planeBDist,
...
s32 OoT_Math3D_PlaneVsPlaneVsLineClosestPoint(f32 planeAA, f32 planeAB, f32 planeAC, f32 planeADist, f32 planeBA,
    ...
    OoT_Math3D_LineClosestToPoint(&planeIntersect, point, closestPoint);
    return true;
}
...
void OoT_Math3D_LineSplitRatio(Vec3f* v0, Vec3f* v1, f32 ratio, Vec3f* ret) {
    ...
    OoT_Math_Vec3f_Diff(v1, v0, &diff);
...
f32 OoT_Math3D_Cos(Vec3f* a, Vec3f* b) {
    ...
    OoT_Math3D_CosOut(a, b, &ret);
    return ret;
}

/**
 * Calculates the cosine between bectors `a` and `b` and places the result in `ret`
 * returns true if the cosine cannot be calculated because the product of the magnitudes is OoT_zero
 */
s32 OoT_Math3D_CosOut(Vec3f* a, Vec3f* b, f32* dst) {
    f32 magProduct;

    magProduct = OoT_Math3D_Vec3fMagnitude(a) * OoT_Math3D_Vec3fMagnitude(b);
...
void OoT_Math3D_Vec3fReflect(Vec3f* vec, Vec3f* normal, Vec3f* reflVec) {
    ...
    vecDotNorm = OoT_Math3D_Cos(&negVec, normal);
...
f32 OoT_Math3D_Dist2DSq(f32 x0, f32 y0, f32 x1, f32 y1) {
    return OoT_Math3D_Dist1DSq(x0 - x1, y0 - y1);
}
...
f32 OoT_Math3D_Vec3fMagnitude(Vec3f* vec) {
    return sqrt(OoT_Math3D_Vec3fMagnitudeSq(vec));
}
...
f32 OoT_Math3D_Vec3fDistSq(Vec3f* a, Vec3f* b) {
    ...
    OoT_Math_Vec3f_Diff(a, b, &diff);
    return OoT_Math3D_Vec3fMagnitudeSq(&diff);
}
...
f32 OoT_Math3D_Vec3f_DistXYZ(Vec3f* a, Vec3f* b) {
    return OoT_Math_Vec3f_DistXYZ(a, b);
}

/*
 * Calculates the distance between `a` and `b`.
 */
f32 OoT_Math3D_DistXYZ16toF(Vec3s* a, Vec3f* b) {
    Vec3f diff;

    diff.x = a->x - b->x;
    diff.y = a->y - b->y;
    diff.z = a->z - b->z;
    return OoT_Math3D_Vec3fMagnitude(&diff);
}

/**
 * Gets the Z portion of the cross product of vectors `a - (`dx`,`dy`,z) and `b` - (`dx`,`dy`,z)
 */
f32 OoT_Math3D_Vec3fDiff_CrossZ(Vec3f* a, Vec3f* b, f32 dx, f32 dy) {
 ...
 */
f32 OoT_Math3D_Vec3fDiff_CrossX(Vec3f* a, Vec3f* b, f32 dy, f32 dz) {
 ...
 */
f32 OoT_Math3D_Vec3fDiff_CrossY(Vec3f* a, Vec3f* b, f32 dz, f32 dx) {
...
void OoT_Math3D_SurfaceNorm(Vec3f* va, Vec3f* vb, Vec3f* vc, Vec3f* normal) {
    ...
    OoT_Math_Vec3f_Diff(vb, va, &abDiff);
    OoT_Math_Vec3f_Diff(vc, va, &acDiff);
    OoT_Math3D_Vec3f_Cross(&abDiff, &acDiff, normal);
}
...
s32 OoT_Math3D_LineVsCube(Vec3f* min, Vec3f* max, Vec3f* a, Vec3f* b) {
    ...
    flags[0] = flags[1] = 0;
    flags[0] = OoT_Math3D_PointRelativeToCubeFaces(a, min, max);
    if (!flags[0]) {
        return true;
    }

    flags[1] = OoT_Math3D_PointRelativeToCubeFaces(b, min, max);
    if (!flags[1]) {
        return true;
    }
    ...
    flags[0] |= (OoT_Math3D_PointRelativeToCubeEdges(a, min, max) << 8);
    flags[1] |= (OoT_Math3D_PointRelativeToCubeEdges(b, min, max) << 8);
    if (flags[0] & flags[1]) {
        return false;
    }

    flags[0] |= (OoT_Math3D_PointRelativeToCubeVertices(a, min, max) << 0x18);
    flags[1] |= (OoT_Math3D_PointRelativeToCubeVertices(b, min, max) << 0x18);
    if (flags[0] & flags[1]) {
        return false;
    }
    ...
    triVtx2.y = max->y;
    triVtx2.z = max->z;
    if (OoT_Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, -1.0f, 0.0f, 0.0f, min->x, a, b, &intersectPoint, 0)) {
        return true;
    }
    ...
    triVtx2.y = max->y;
    triVtx2.z = max->z;
    if (OoT_Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, 0.0f, 0.0f, 1.0f, -max->z, a, b, &intersectPoint, 0)) {
        return true;
    }
    triVtx0.x = max->x;
    triVtx0.y = max->y;
    ...
    triVtx2.y = max->y;
    triVtx2.z = max->z;
    if (OoT_Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, 0.0f, 1.0f, 0.0f, -max->y, a, b, &intersectPoint, 0)) {
        return true;
    }
    triVtx0.x = max->x;
    triVtx0.y = max->y;
    ...
    triVtx2.y = max->y;
    triVtx2.z = min->z;
    if (OoT_Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, 0.0f, 0.0f, -1.0f, min->z, a, b, &intersectPoint, 0)) {
        return true;
    }
    triVtx0.x = min->x;
    triVtx0.y = min->y;
    ...
    triVtx2.y = min->y;
    triVtx2.z = max->z;
    if (OoT_Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, 0.0f, -1.0f, 0.0f, min->y, a, b, &intersectPoint, 0)) {
        return true;
    }
    triVtx0.x = min->x;
    triVtx0.y = min->y;
    ...
    triVtx2.y = max->y;
    triVtx2.z = min->z;
    if (OoT_Math3D_TriLineIntersect(&triVtx0, &triVtx1, &triVtx2, 1.0f, 0.0f, 0.0f, -max->x, a, b, &intersectPoint, 0)) {
        return true;
    }
    triVtx0.x = max->x;
    triVtx0.y = max->y;
 ...
 * Checks if a line segment with endpoints `a` and `b` intersect a cube
 */
s32 OoT_Math3D_LineVsCubeShort(Vec3s* min, Vec3s* max, Vec3s* a, Vec3s* b) {
    static Vec3f minF;
    static Vec3f maxF;
    static Vec3f aF;
    static Vec3f bF;
    ...
    bF.y = b->y;
    bF.z = b->z;
    return OoT_Math3D_LineVsCube(&minF, &maxF, &aF, &bF);
}
...
void OoT_Math3D_RotateXZPlane(Vec3f* pointOnPlane, s16 angle, f32* a, f32* c, f32* d) {
    *a = OoT_Math_SinS(angle) * 32767.0f;
    *c = OoT_Math_CosS(angle) * 32767.0f;
 ...
 */
f32 OoT_Math3D_Plane(Plane* plane, Vec3f* pointOnPlane) {
...
f32 OoT_Math3D_UDistPlaneToPos(f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* p) {
    ...
    return fabsf(OoT_Math3D_DistPlaneToPos(nx, ny, nz, originDist, p));
}
 ...
 */
s32 OoT_Math3D_TriChkPointParaYImpl(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 z, f32 x, f32 detMax, f32 chkDist, f32 ny) {
    f32 detv0v1;
    f32 detv1v2;
    f32 detv2v0;
    f32 distToEdgeSq;
...
s32 OoT_Math3D_TriChkPointParaYDeterminate(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 z, f32 x, f32 detMax, f32 ny) {
    return OoT_Math3D_TriChkPointParaYImpl(v0, v1, v2, z, x, detMax, 1.0f, ny);
}

s32 OoT_Math3D_TriChkPointParaYSlopedY(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 z, f32 x) {
    return OoT_Math3D_TriChkPointParaYImpl(v0, v1, v2, z, x, 300.0f, 1.0f, 0.6f);
}
...
s32 OoT_Math3D_TriChkPointParaYIntersectDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, f32 z,
    ...
    if (OoT_Math3D_TriChkPointParaYImpl(v0, v1, v2, z, x, 300.0f, chkDist, ny)) {
...
s32 OoT_Math3D_TriChkPointParaYIntersectInsideTri(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist,
    ...
    if (OoT_Math3D_TriChkPointParaYImpl(v0, v1, v2, z, x, 0.0f, chkDist, ny)) {
        *yIntersect = (f32)((((-nx * x) - (nz * z)) - originDist) / ny);
        return true;
    }
...
s32 OoT_Math3D_TriChkPointParaY(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 ny, f32 z, f32 x) {
    if (IS_ZERO(ny)) {
        return false;
    }
    if (OoT_Math3D_TriChkPointParaYImpl(v0, v1, v2, z, x, 300.0f, 1.0f, ny)) {
        return true;
    }
    return false;
}
...
s32 OoT_Math3D_TriChkPointParaYDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, Plane* plane, f32 z, f32 x, f32 chkDist) {
    ...
    if (OoT_Math3D_TriChkPointParaYImpl(v0, v1, v2, z, x, 0.0f, chkDist, plane->normal.y)) {
        return true;
    }
...
s32 OoT_Math3D_TriChkPointParaXImpl(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 y, f32 z, f32 detMax, f32 chkDist, f32 nx) {
    f32 detv0v1;
    f32 detv1v2;
    f32 detv2v0;
    f32 distToEdgeSq;
...
s32 OoT_Math3D_TriChkPointParaXDeterminate(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 y, f32 z, f32 detMax, f32 nx) {
    return OoT_Math3D_TriChkPointParaXImpl(v0, v1, v2, y, z, detMax, 1.0f, nx);
}
...
s32 OoT_Math3D_TriChkPointParaXIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, f32 y,
    ...
    if (OoT_Math3D_TriChkPointParaXImpl(v0, v1, v2, y, z, 300.0f, 1.0f, nx)) {
...
s32 OoT_Math3D_TriChkPointParaX(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 y, f32 z) {
    if (IS_ZERO(nx)) {
        return false;
    }
    if (OoT_Math3D_TriChkPointParaXImpl(v0, v1, v2, y, z, 300.0f, 1.0f, nx)) {
...
s32 Math3D_TriChkPointParaXDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, Plane* plane, f32 y, f32 z, f32 chkDist) {
    ...
    if (OoT_Math3D_TriChkPointParaXImpl(v0, v1, v2, y, z, 0.0f, chkDist, plane->normal.x)) {
        return true;
    }
    return false;
}

s32 OoT_Math3D_TriChkPointParaZImpl(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 x, f32 y, f32 detMax, f32 chkDist, f32 nz) {
    f32 detv0v1;
    f32 detv1v2;
    f32 detv2v0;
    f32 distToEdgeSq;
...
s32 OoT_Math3D_TriChkPointParaZDeterminate(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 x, f32 y, f32 detMax, f32 nz) {
    return OoT_Math3D_TriChkPointParaZImpl(v0, v1, v2, x, y, detMax, 1.0f, nz);
}
...
s32 OoT_Math3D_TriChkPointParaZIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, f32 x,
    ...
    if (OoT_Math3D_TriChkPointParaZImpl(v0, v1, v2, x, y, 300.0f, 1.0f, nz)) {
...
s32 OoT_Math3D_TriChkPointParaZ(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nz, f32 x, f32 y) {
    if (IS_ZERO(nz)) {
        return false;
    }
    if (OoT_Math3D_TriChkPointParaZImpl(v0, v1, v2, x, y, 300.0f, 1.0f, nz)) {
...
s32 OoT_Math3D_TriChkLineSegParaZDist(Vec3f* v0, Vec3f* v1, Vec3f* v2, Plane* plane, f32 x, f32 y, f32 chkDist) {
    ...
    if (OoT_Math3D_TriChkPointParaZImpl(v0, v1, v2, x, y, 0.0f, chkDist, plane->normal.z)) {
        return true;
    }
    return false;
}

s32 OoT_Math3D_LineSegFindPlaneIntersect(f32 pointADist, f32 pointBDist, Vec3f* pointA, Vec3f* pointB, Vec3f* intersect) {
    f32 distDiff;
    ...
    if (pointADist == 0.0f) {
        ...
        *intersect = *pointB;
        ...
        OoT_Math3D_LineSplitRatio(pointA, pointB, pointADist / distDiff, intersect);
    }
    return true;
}
...
s32 OoT_Math3D_LineSegVsPlane(f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* linePointA, Vec3f* linePointB,
    ...
    return OoT_Math3D_LineSegFindPlaneIntersect(pointADist, pointBDist, linePointA, linePointB, intersect);
}
...
s32 OoT_Math3D_TriLineIntersect(Vec3f* v0, Vec3f* v1, Vec3f* v2, f32 nx, f32 ny, f32 nz, f32 originDist, Vec3f* linePointA,
    ...
    if (!OoT_Math3D_LineSegVsPlane(nx, ny, nz, originDist, linePointA, linePointB, intersect, fromFront)) {
        return false;
    }

    if (((nx == 0.0f) || (OoT_Math3D_TriChkPointParaX(v0, v1, v2, nx, intersect->y, intersect->z))) &&
        ((ny == 0.0f) || (OoT_Math3D_TriChkPointParaY(v0, v1, v2, ny, intersect->z, intersect->x))) &&
        ((nz == 0.0f) || (OoT_Math3D_TriChkPointParaZ(v0, v1, v2, nz, intersect->x, intersect->y)))) {
        return true;
    }
...
void OoT_Math3D_TriNorm(TriNorm* tri, Vec3f* va, Vec3f* vb, Vec3f* vc) {
    ...
    tri->vtx[1] = *vb;
    tri->vtx[2] = *vc;
    OoT_Math3D_DefPlane(va, vb, vc, &tri->plane.normal.x, &tri->plane.normal.y, &tri->plane.normal.z,
 ...
 */
s32 OoT_Math3D_PointInSph(Sphere16* sphere, Vec3f* point) {

    if (OoT_Math3D_DistXYZ16toF(&sphere->center, point) < sphere->radius) {
        return true;
    }
    return false;
}
...
s32 OoT_Math3D_LineVsSph(Sphere16* sphere, Linef* line) {
    ...
    if ((OoT_Math3D_PointInSph(sphere, &line->a)) || (OoT_Math3D_PointInSph(sphere, &line->b))) {
 ...
 */
void OoT_Math3D_GetSphVsTriIntersectPoint(Sphere16* sphere, TriNorm* tri, Vec3f* intersectPoint) {
    ...
    dist = OoT_Math3D_Vec3f_DistXYZ(&v0v1Center, &sphereCenter);
    ...
    OoT_Math3D_LineSplitRatio(&sphereCenter, &v0v1Center, splitRatio, intersectPoint);
}
...
s32 OoT_Math3D_TriVsSphIntersect(Sphere16* sphere, TriNorm* tri, Vec3f* intersectPoint) {
    ...
    if (!OoT_Math3D_SphCubeVsTriCube(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2], &sphereCenter, radius)) {
        return false;
    }

    planeDist = OoT_Math3D_UDistPlaneToPos(tri->plane.normal.x, tri->plane.normal.y, tri->plane.normal.z,
    ...
    if (OoT_Math3D_LineVsSph(sphere, &triTestLine)) {
        OoT_Math3D_GetSphVsTriIntersectPoint(sphere, tri, intersectPoint);
        return true;
    }
    ...
    if (fabsf(tri->plane.normal.y) > 0.5f) {
        if (OoT_Math3D_TriChkPointParaYDeterminate(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2], sphPlanePos.z, sphPlanePos.x,
                                               0.0f, tri->plane.normal.y)) {
            OoT_Math3D_GetSphVsTriIntersectPoint(sphere, tri, intersectPoint);
            return true;
        }
    } else if (fabsf(tri->plane.normal.x) > 0.5f) {
        if (OoT_Math3D_TriChkPointParaXDeterminate(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2], sphPlanePos.y, sphPlanePos.z,
                                               0.0f, tri->plane.normal.x)) {
            OoT_Math3D_GetSphVsTriIntersectPoint(sphere, tri, intersectPoint);
            return true;
        }
    } else if (OoT_Math3D_TriChkPointParaZDeterminate(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2], sphPlanePos.x,
 ...
 */
s32 OoT_Math3D_PointInCyl(Cylinder16* cyl, Vec3f* point) {
    f32 bottom;
    f32 top;
    f32 x;
    f32 z;
...
s32 OoT_Math3D_CylVsLineSeg(Cylinder16* cyl, Vec3f* linePointA, Vec3f* linePointB, Vec3f* intersectA, Vec3f* intersectB) {
    ...
    if (OoT_Math3D_PointInCyl(cyl, linePointA) && OoT_Math3D_PointInCyl(cyl, linePointB)) {
    ...
    OoT_Math_Vec3f_Diff(&cylToPtB, &cylToPtA, &ptAToPtB);
    ...
    if (!IS_ZERO(2.0f * SQXZ(ptAToPtB))) {
        ...
        if (SQ(dot2AB) - (4.0f * SQXZ(ptAToPtB) * radSqDiff) > OoT_zero) {
            sideIntA = sideIntB = 1;
        } else {
        ...
        if (intFlags & (1 << i)) {
            if (count == 0) {
                *intersectA = intPts[i];
                ...
                if (OoT_Math3D_Vec3fDistSq(intersectA, linePointA) < OoT_Math3D_Vec3fDistSq(intersectA, &intPts[i])) {
                    *intersectB = intPts[i];
                } else {
...
s32 OoT_Math3D_CylTriVsIntersect(Cylinder16* cyl, TriNorm* tri, Vec3f* intersect) {
    ...
    minDistSq = 1.e38f;
    if (OoT_Math3D_CylVsLineSeg(cyl, &tri->vtx[0], &tri->vtx[1], &cylIntersectA, &cylIntersectB)) {
        distFromPointAToIntersectASq = OoT_Math3D_Vec3fDistSq(&cylIntersectA, &tri->vtx[0]);
        minDistSq = distFromPointAToIntersectASq;
        *intersect = cylIntersectA;
    }

    if (OoT_Math3D_CylVsLineSeg(cyl, &tri->vtx[2], &tri->vtx[1], &cylIntersectA, &cylIntersectB)) {
        distFromPointAToIntersectASq = OoT_Math3D_Vec3fDistSq(&cylIntersectA, &tri->vtx[2]);
    ...
    if (OoT_Math3D_CylVsLineSeg(cyl, &tri->vtx[0], &tri->vtx[2], &cylIntersectA, &cylIntersectB)) {
        distFromPointAToIntersectASq = OoT_Math3D_Vec3fDistSq(&cylIntersectA, &tri->vtx[0]);
        if (distFromPointAToIntersectASq < minDistSq) {
            *intersect = cylIntersectA;
            minDistSq = distFromPointAToIntersectASq;
    ...
    if (OoT_Math3D_TriChkLineSegParaYIntersect(&tri->vtx[0], &tri->vtx[1], &tri->vtx[2], tri->plane.normal.x,
    ...
    if ((OoT_Math3D_TriVsSphIntersect(&topSphere, tri, intersect)) ||
        (OoT_Math3D_TriVsSphIntersect(&bottomSphere, tri, intersect))) {
        return true;
    }
    return false;
}
...
s32 OoT_Math3D_CylVsTri(Cylinder16* cyl, TriNorm* tri) {
    ...
    return OoT_Math3D_CylTriVsIntersect(cyl, tri, &intersect);
}
...
s32 OoT_Math3D_SphVsSph(Sphere16* sphereA, Sphere16* sphereB) {
    ...
    return OoT_Math3D_SphVsSphOverlap(sphereA, sphereB, &overlapSize);
}
...
s32 Math3D_SphVsCylOverlapDist(Sphere16* sph, Cylinder16* cyl, f32* overlapSize) {
    ...
    return OoT_Math3D_SphVsCylOverlapCenterDist(sph, cyl, overlapSize, &centerDist);
}
...
s32 Math3D_CylOutsideCylDist(Cylinder16* ca, Cylinder16* cb, f32* deadSpace, f32* xzDist) {
    ...
    OoT_Math_Vec3s_ToVec3f(&caf.pos, &ca->pos);
    caf.radius = ca->radius;
    caf.yShift = ca->yShift;
    caf.height = ca->height;

    OoT_Math_Vec3s_ToVec3f(&cbf.pos, &cb->pos);
...
s32 OoT_Math3D_TriVsTriIntersect(TriNorm* ta, TriNorm* tb, Vec3f* intersect) {
    ...
    dist0 = OoT_Math3D_Plane(&ta->plane, &tb->vtx[0]);
    dist1 = OoT_Math3D_Plane(&ta->plane, &tb->vtx[1]);
    dist2 = OoT_Math3D_Plane(&ta->plane, &tb->vtx[2]);
    ...
    dist0 = OoT_Math3D_Plane(&tb->plane, &ta->vtx[0]);
    dist1 = OoT_Math3D_Plane(&tb->plane, &ta->vtx[1]);
    dist2 = OoT_Math3D_Plane(&tb->plane, &ta->vtx[2]);
    ...
    if (OoT_Math3D_TriLineIntersect(&tb->vtx[0], &tb->vtx[1], &tb->vtx[2], tb->plane.normal.x, tb->plane.normal.y,
    ...
    if (OoT_Math3D_TriLineIntersect(&ta->vtx[0], &ta->vtx[1], &ta->vtx[2], ta->plane.normal.x, ta->plane.normal.y,
