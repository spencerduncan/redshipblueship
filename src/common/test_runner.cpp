...
// #670 MM mods/ mount + shared-mods-tree partition lock. FILE SCOPE (compiled as
// C++): it drives the C++-linkage ArchiveManager to ask which archive owns a
// contested path, calls MM's production glob through
// MM_MountModArchivesHeadless, and calls the production
// Combo_EnsureGameArchivesLoaded for both switch directions. Must be included
// AFTER test_curated_archive_order.c so the wrapper can reuse CaoResolveArchive.
#include "tests/test_mm_mods_mount.c"
...
// #670: MM mounted no mod archives at all in single-exe (its glob is in the
// excluded BenPort.cpp), so Combo_GetModArchiveCount(GAME_MM) was structurally 0
// and #593's switch-time re-apply was a permanent no-op for MM. The body stages a
// private mods/ tree from copies of the two staged archives, runs MM's real
// glob/mount over it, and drives the production Combo_EnsureGameArchivesLoaded in
// both directions.
//
// The display-free shared bring-up is needed because the body asks the shared
// ArchiveManager who owns a contested path, and because
...
TestResult Test_MMModsMount(void) {
    ...
    int rc = MMModsMount_RunHeadless(sohArchive.c_str(), mmArchive.c_str());
    printf("[TEST] %s: mm mods mount rc=%d\n", rc == 0 ? "PASS" : "FAIL", rc);
    return rc == 0 ? TEST_PASS : TEST_FAIL;
}
...
const TestDescriptor gTests[] = {
    ...
    // MM_Rando_Init, which is irreversible in-process (the once-guard cannot
    // be cleared) — it used to be skipped by `--test all` for exactly that
    // reason, until it conflicted with mm-startup-restore's byte-exact tail
    // assertion. #617 fixed that assertion (see TestRunner_Run's "all"
    // dispatch for the retirement), so this entry now also runs there,
    ...
    // #670: MM's mods/mm mount, the shared mods/ tree partition, and an MM mod
    // override that survives a switch without shadowing an OoT path. Placed here,
    // next to mm-extension-rescan, for the same reason: it MOUNTS extra archives
    // (two stand-in mods plus whatever Combo_EnsureGameArchivesLoaded re-adds) into
    // the shared ArchiveManager, so every row that reads soh.o2r paths through the
    // ResourceManager must have run first. It restores the manager's archive list
    // from a snapshot on the way out, so nothing after it inherits the re-order.
...
int TestRunner_Run(const char* testName) {
    ...
    if (strcmp(testName, "all") == 0) {
        ...
        for (int i = 0; gTests[i].name != nullptr; i++) {
            ...
            // that runs the real MM_Rando_Init, which is IRREVERSIBLE in-process
            // (the once-guard cannot be cleared), and mm-startup-restore's old
            // byte-exact tail assertion broke once RegisterSavingEnhancements'
            // live OnSaveLoad hook seeded shipSaveContext.lastTimeLog --
            // shipSaveContext was the LAST member of SaveContext, so the seed
            // overwrote the very tail byte that assertion pinned.
            //
            // #617 resolved that as a test bug, not a production one:
            // shipSaveContext is documented (z64save.h) as "values added by
            // 2S2H that aren't persisted to the save file", production zeroes
            // lastTimeLog in lockstep on every new-file/continue path
            // (z_sram_NES.c), and MM_Play_ConsumeStartupEntrance dispatching
            // OnSaveLoad after every restore is deliberate (#439). The tail
            // assertion is now byte-exact only up to (excluding) shipSaveContext,
            // plus a freshness check on lastTimeLog — both true whether or not
            // MM_Rando_Init has already run in this process, since
            // mm-startup-restore now brings its own seeder up (see
            // Test_MMStartupRestore / MM_StartupRestore_RunHeadless). Running
            // mm-registrar-coverage's real MM_Rando_Init here, ahead of it in
            // this array, matches real boot order and no longer costs anything.
            printf("\n--- Running: %s ---\n", gTests[i].name);
